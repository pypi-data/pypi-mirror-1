...
import gpkmisc
import mcmc_helper
import multivariance_classes as MC
from multivariance_classes import modeldesc, model_with_numbers
from multivariance_classes import vec_inv_variance, diag_inv_variance
import multivariance_q as MQ
from multivariance_q import quadratic, diag_quadratic, quadratic_with_numbers, diag_quadratic_with_numbers
import multivariance_mm as MM
from multivariance_mm import multi_mu, multi_mu_diag
...
def meanvar(dataset, N, modelchoice=quadratic):
...
def test_quadratic():
	...
	for l in sys.stdin:
		...
		d.append(a)
	for x in meanvar(d, 200, modelchoice=quadratic):
		print 'Mu:', x.mu
...
def test_diag_quadratic():
	...
	for l in sys.stdin:
		...
		a = Num.array( [float(s) for s in l.split()], Num.Float)
		d.append(a)
	for x in meanvar(d, 200, modelchoice=diag_quadratic):
		print 'Mu:', x.mu
		print 'Inverse:', x.invsigma
		print
...
def test_multimu():
	...
	for l in sys.stdin:
		...
		d.append(multivariance_mm.datum_c(vector=af, classid=a[0]))
	for x in meanvar(d, 200, modelchoice=multi_mu):
		print 'Inverse:', x.invsigma
		print 'Mu:', x.mu
		print
...
def test_multimu_diag():
	...
	for l in sys.stdin:
		...
		af = Num.array([float(s) for s in a[1:]], Num.Float)
		d.append(multivariance_mm.datum_c(vector=af, classid=a[0]))
	for x in meanvar(d, 200, modelchoice=multi_mu_diag):
		print 'Inverse:', x.invsigma
		print 'Mu:', x.mu
		print


FromName = {'quadratic' : quadratic, 'diag_quadratic': diag_quadratic,
		'multi_mu': multi_mu, 'multi_mu_diag': multi_mu_diag
	}
