...
class FiatError(RuntimeError):
	def __init__(self, s):
		RuntimeError.__init__(self, s)


class ConflictingColumnSpecification(FiatError):
	def __init__(self, s):
...
class rheader(object):
	...
	def parse(self, s):
		...
		if len(a) > 1 and len(a[0].split()) == 1:
			attr = a[0].strip()
			val = dequote(a[1].strip())
			if attr.upper().startswith('TTYPE'):
				ic = int(attr[self.LTTYPE:])-1
				if ic in self.header['__COLUMNS'] and self.header['__COLUMNS'][ic] != val:
					raise ConflictingColumnSpecification, 'column=%d: "%s" vs. "%s"' % (
										ic, val, self.header['__COLUMNS'][ic]
										)
				if val in self.header['__NAME_TO_COL'] and self.header['__NAME_TO_COL'][val] != ic:
					raise ConflictingColumnSpecification, 'val="%s": columns %d vs. %d' % (
										val, ic, self.header['__COLUMNS'][ic]
										)
				self.extend_icol(ic+1)
				self.icol[ic] = val
				self.header['__COLUMNS'][ic] = val
				self.header['__NAME_TO_COL'][val] = ic
			elif attr == 'COL_EMPTY':
				self.blank = val
...
def read_merged(fd):
	"""Read in a fiat file. Return (data, comments),
	data is a list of dictionaries, one for each line in the file,
	and comments is a list of strings.
	Each line in the FIAT file is represented by a dictionary that maps
	column name into the data (data is a string).
	Header data is merged into the per-column data.
	Lines without data in a certain column will not have an entry in that line's
	dictionary for that column name.
	Two special entries are added to header: __COLUMNS points to a mapping
	from column numbers (the order in which they appeared in the file,
...
def readiter(fd):
	"""Read in a fiat file. Yield (header, data, comments),
	where header is a dictionary of header information,
	data is a list of dictionaries, one for each line in the file,
	and comments is a list of strings.
	Each line in the FIAT file is represented by a dictionary that maps
	...
	while line:
		...
		if line.startswith('#'):
			...
			yield (hobj.header, {}, hobj.comments)
		else:
			...
			yield (hobj.header, tmp, hobj.comments)
		line = fd.readline()
	yield (hobj.header, None, hobj.comments)
...
def test3():
	...
	comments = ['Comment1', 'Comment2']
	header = {'SAMPRATE': 2.3, 'DATE':'2001/09/21'}
	data = [
