...
class problem_definition(object):
	"""This is the default class.    Often, you would
	...
	def fixer(self, x):
		...
		vector.    The function can modify the position.
		Generally, it is used to restrict the possible
		solution space by folding position vectors that
		escape outside the solution space back into
		the solution space.
		@note: It's always safe to fold if logp(fixer(x)) == logp(x);
	...
	def guess(self):
		...
		Returns:
		a list of guess vectors; each guess vector is
...
class position_base(object):
	...
	def __init__(self, x, problem_def):
		...
		self.pd = problem_def
		# print "position:x=", x
		self.x = Num.array(self.pd.fixer(Num.array(x, Num.Float, copy=True)), copy=True)
		assert isinstance(self.x, _Ntype), "Output of fixer() is not numpy array."
		assert len(self.x.shape) == 1, "Output of fixer() must be 1-dimensional."
	...
	def __cmp__(self, other):
		...
		return cmp(self.logp_nocompute(), other.logp_nocompute())
...
class position_repeatable(position_base):
	"""This is for the normal case, where logp is a well-behaved
	...
	def __init__(self, x, problem_def, logp=None):
		...
		# self.x0 = Num.array(self.x, copy=True)
		self.cache = logp
		if logp is None:
			self.cache_valid = -1
		else:
	...
	def __check_cache(self):
		# assert Num.equal(self.x0, self.x).all(), "Parameters unexpectedly changed!"
		if self.cache_valid <= 0:
			tmp = self.pd.logp(self.prms())
			if self.cache_valid==0 and abs(tmp - self.cache) > 0.1:
				raise ValueError, 'Recomputing position cache; found mismatch %g to %g' % (self.cache, tmp)
			self.cache = tmp
			self.cache_valid = self.CACHE_LIFE
		self.cache_valid -= 1
		return self.cache
	...
	def nogood(self):
		return self.__check_cache() is None


	def logp(self):
		tmp = self.__check_cache()
		if tmp is None:
			return -self.HUGE
		return tmp
...
class position_nonrepeatable(position_base):
	...
	def nogood(self):
		"""This always uses the cache.
		Strictly speaking, it should always recompute,
		but that would be very expensive, and probably not
		make a difference in a case I care about.
		"""
		if not self.cache:
			self.cache.append( self.pd.logp(self.prms()) )
		return random.choice(self.cache) is None
...
class position_history_dependent(position_base):
	...
	def nogood(self):
		...
		make a difference in a case I care about.
		"""
		if self.cache is not self.EMPTY:
			return self.cache is None
		self.cache = self.pd.logp(self.prms())
		return self.cache is None
...
class stepper(object):
	...
	def __init__(self):
		...
		self.T = 1.0	# MCMC -- might be different for simulated annealing.
	...
	def needs_a_reset(self):
		...
		if self.last_reset is None:
			...
			rst = current.logp_nocompute() > self.last_reset.logp_nocompute() + self.T
			if rst:
				self.last_reset = current
		self.lock.release()
		if rst and Debug>1:
...
class adjuster(object):
	...
	def __init__(self, F, np, alpha, vscale, vse=0.0, vsmax=1e30):
		assert 0.0 < F < 1.0
...
class Archive(object):
	...
	def __init__(self, lop, sort=SSAUTO, maxdups=-1):
		...
		self._seq_dups = hash_seq_c()
		for p in self.lop:
			idp = p.uid()
			self._hashes.incr(idp)
			self._seq_dups.add(idp)


	def distinct_count(self):
		self.lock.acquire()
		tmp = len(self._hashes)
		self.lock.release()
		return tmp
	...
	def truncate(self, desired_length):
		...
		try:
			...
			j = len(self.lop) - int( desired_length
						* float(len(self.lop))/(1.0+len(self._hashes))
						)
			assert j <= len(self.lop)
			if j > 3:
				# We could truncate for any j>0, but it's not worth the
				# time expended.   It's better to save it up and truncate in a clump.
				to_be_dropped = self.lop[:j]
				for tmp in to_be_dropped:
					self._hashes.decr(tmp.uid())
				self.lop = self.lop[j:]
				self.truncate_hook(to_be_dropped)
			assert len(self._hashes) <= len(self.lop)
	...
	def append(self, x, forbid_dups):
		...
		with self.lock:
			...
			assert len(self._hashes) <= len(self.lop)
			idx = x.uid()
			if forbid_dups and idx in self._hashes:
				return 'f'
			self._seq_dups.add(idx)
			if self._seq_dups.n >= self.maxdups:
				# We don't want the clutter up the archive
				# with duplicates until the step sizes are
				# well established.
				if Debug>1:
					print 'Too many duplicates (%d vs %d): %s not archived.' % (self._seq_dups.n, self.maxdups, idx)
				assert len(self._hashes) <= len(self.lop)
				return 'd'
			self._hashes.incr(idx)
			...
			if not self.sorted or self.sortstrategy==self.SSNEVER:
				...
				if self.since_last_reset > self.np and self.sortstrategy!=self.SSALWAYS:
					self.sorted = False
					if Debug:
...
class ContPrmArchive(Archive):
	def __init__(self, lop, sort=Archive.SSAUTO, maxdups=-1):
		...
		"""
		Archive.__init__(self, lop, sort=sort, maxdups=maxdups)
		self.p0 = lop[-1].vec()
...
class BootStepper(stepper):
	F = 0.25	# Target acceptance rate.
	alpha = 0.1
	PBootLim = 10.0	# Limits the probability of taking a bootstrap step to 90%
	...
	def __init__(self, lop, v, sort=SSAUTO, maxArchSize=None, parallelSizeDiv=1):
		...
		@param parallelSizeDiv: For use when there are several cooperating MCMC
			...
			equal to the number of cooperating processes.
		"""
		stepper.__init__(self)
		_check_list_of_positions(lop)
		self.archive = ContPrmArchive(lop, sort=sort, maxdups=int(round(4.0/self.F)))

		self.np = self.current().vec().shape[0]
		if not self.np > 0:
		...
		self.V = MVN.multivariate_normal(Num.zeros(v.shape[0], Num.Float),
								v)
		self.aB = adjuster(self.F, self.np, self.alpha, vscale=0.5, vse=0.5, vsmax=1.3)
		self.aV = adjuster(self.F, self.np, self.alpha, vscale=1.0)

		if maxArchSize is None:
			maxArchSize = 2*self.np**2 + 1000	# Big!
		if maxArchSize < self.np//parallelSizeDiv + 7:
			die.warn("maxArchSize is too small for trustworthy operation: %d vs. %d" % (maxArchSize, self.np))
		self.min_l = min(maxArchSize,
				(self.np//parallelSizeDiv+maxArchSize)//2+7,
				(self.np*2)//parallelSizeDiv+7
				)
		self.max_l = min(maxArchSize, (self.np**2)//parallelSizeDiv+self.min_l)
		self.steptype = None
	...
	def step(self):
		...
		leff = min( self.archive.distinct_count(),	# Bootstrapping requires a big archive.
				self.F*self.since_last_rst + self.np,	# Reduce P to 50% after a reset.
				self.PBootLim*self.np		# Keeps P from becoming too large.
				)
		P = leff / float(leff + self.np * (self.aB.f()/self.F) )
		if random.random() < P:
			try:
	...
	def stepV(self):
		...
		if tmp.nogood():
			if Debug>2:
				die.warn('StepV: Nogood position')
			...
			accepted = 1
			if Debug>2:
				die.warn('StepV: Accepted logp=%g' % tmp.logp_nocompute())
			...
			accepted = 0
			if Debug>2:
				die.warn('StepV: Rejected logp=%g' % tmp.logp_nocompute())
			self.aV.inctry(accepted)
	...
	def step_boot(self):
		...
		tmp = self.current().new(move)
		delta = tmp.logp() - self.current().logp()
		# print "tmp.logp=", tmp.logp()
		# print "self.current()=", self.current()
		# print "self.current().logp=", self.current().logp()
		if tmp.nogood():
			if Debug>2:
			...
			accepted = 1
			if Debug>2:
				die.warn('StepBoot: Accepted logp=%g' % tmp.logp_nocompute())
			self._set_current(tmp)
			...
			accepted = 0
			if Debug>2:
				die.warn('StepBoot: Rejected logp=%g' % tmp.logp_nocompute())
			self.aB.inctry(accepted)
			self._set_current(self.current())
		return accepted
	...
	def _set_current(self, newcurrent, forbid_dups=False):
		g_implements.check(newcurrent, position_base)
		assert not newcurrent.nogood()
		...
		with self.lock:
			...
			if self.needs_a_reset():
				...
				self.reset()
				out.append('r')
			self.archive.truncate( min(int( self.min_l + self.F*self.alpha*self.since_last_rst ),
						self.max_l)
					)
			out.append('t')
		return ''.join(out)
	...
	def ergodic(self):
		...
		"""
		if self.since_last_rst*self.aB.f() < self.np:
