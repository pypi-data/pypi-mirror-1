...
class BootStepper(mcmc.BootStepper):
	def __init__(self, lop, v, sort=mcmc.BootStepper.SSAUTO):
		...
		self.aM = mcmc.adjuster(self.F, self.np, self.alpha, vscale=0.5, vse=0.5, vsmax=1.1)
	...
	def step(self):
		...
		try:
			if P < Wboot/W:
				...
				elif Q < 0.6 and self.since_last_rst < 2*self.np:
	...
	def step_mixed(self):
		...
		if self.archive.distinct_count() >= min(5, self.np):
	...
	def step_parab(self):
		...
		pbase, vbase = find_closest_p(vs0, (0.0, p1), (1.0, p2))
		...
		pbase, vbase = find_closest_p(vsnew, (0.0, p1), (1.0, p2), (vs0, tmp))
