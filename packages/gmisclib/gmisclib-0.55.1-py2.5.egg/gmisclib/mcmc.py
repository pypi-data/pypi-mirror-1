...
class position_repeatable(position_base):
	...
	def new(self, shift):
		"""Returns a new position, shifted by the specified amount."""
		return position_repeatable(self.vec() + shift, self.pd)
...
class position_nonrepeatable(position_base):
	...
	def new(self, shift):
		"""Returns a new position, shifted by the specified amount."""
		return position_nonrepeatable(self.vec() + shift, self.pd)
...
class position_history_dependent(position_base):
	...
	def new(self, shift):
		"""Returns a new position, shifted by the specified amount."""
		return position_history_dependent(self.vec() + shift, self.pd)
...
class Archive(object):
	...
	def distinct_count(self):
		self.lock.acquire()
		tmp = len(self.lop) - int(round(self.dups))
		self.lock.release()
		return tmp
	...
	def truncate(self, desired_length):
		...
		try:
			...
			assert self.dups <= len(self.lop)
			j = len(self.lop) - int( desired_length
						* float(len(self.lop))/(1.0+len(self.lop)-self.dups)
						)
			...
			if j > 2:
				...
				self.dups *= float(len(self.lop)-j)/float(len(self.lop))
	...
	def append(self, x, notdup):
		...
		try:
			...
			if notdup:
				...
				self._seq_dups += 1
				if self._seq_dups >= self.maxdups:
					# We don't want the clutter up the archive
					# with duplicates until the step sizes are
					# well established.
					if Debug:
						print 'Too many duplicates: not adding to archive.'
					assert self.dups <= len(self.lop)
					return
			self.dups += not notdup
...
class BootStepper(stepper):
	...
	def __init__(self, lop, v, sort=SSAUTO):
		...
		self.min_l = self.np*2 + 3	# Minimum number of non-identical positions.
		self.max_l = self.np**2 + self.min_l
		self.steptype = None
	...
	def stepV(self):
		...
		if tmp.nogood():
			...
			self._set_current(tmp, accepted)
			...
			self.aV.inctry(accepted)
			self._set_current(self.current(), accepted)
		return accepted
	...
	def _set_current(self, newcurrent, notdup):
		...
		self.lock.acquire()
		try:
			self.archive.append(newcurrent, notdup)
