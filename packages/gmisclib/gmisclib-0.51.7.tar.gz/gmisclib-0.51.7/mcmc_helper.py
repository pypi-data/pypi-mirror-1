...
class stepper(object):
	...
	def run_to_ergodic(self, ncw=1):
		"""Run the MCMC until it has explored all of parameter space
		$ncw times (as best as we can estimate).
		Returns the new position.
		@param X: a MCMC stepper of some kind.
	...
	def run_to_bottom(self):
		m = self.x.np
		self.run_to_change(5+2*m)	# get it warmed up.
		before = self.run_to_change(NS)
		mid = self.run_to_change(NS)
		es = 0.0
