...
def prefix(text_array, accent_spec, map_fcn=lambda x:x):
	...
	for t in map_fcn(text_array):
		# print "AS=", as, "t=", t
		if as == t:	# A match for alignment purposes.
			# print "Align match"
			if len(asa) == 0:
				return out
			else:
				as = asa.pop(0)
		elif as.endswith(t) and _goodprefix.match(as[:-len(t)]):
				# A match with an accent.
			# print "Acc match:", as[:-len(t)]
			out.append( (j, as[:-len(t)]) )
			if len(asa) == 0:
				return out
			else:
				as = asa.pop(0)
	...
	for t in map_fcn(text_array):
		...
		if as == t:	# A match for alignment purposes.
			...
			if len(asa) == 0:
				...
				as = asa.pop(0)
		elif as.startswith(t) and _goodsuffix.match(as[len(t):]):
				# A match with an accent.
			# print "Acc match:", as[:-len(t)]
			out.append( (j, as[len(t):]) )
			if len(asa) == 0:
