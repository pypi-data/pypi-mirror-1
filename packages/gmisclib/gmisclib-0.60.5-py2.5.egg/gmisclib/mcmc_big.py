...
class BootStepper(mcmc.BootStepper):
	...
	def step(self):
		...
		if self.since_last_rst < self.np_eff:
			Wparab = min(self.archive.distinct_count(), self.np_eff)
		else:
			Wparab = 0
	...
	def step_mixed(self):
		...
		if self.acceptable(delta):
			...
			if Debug>2:
				die.warn('StepMixed: Rejected logp=%g vs. %g, T=%g'
	...
	def step_parab(self):
		...
		if self.acceptable(delta):
			...
			self._set_current(tmp)
			if Debug>2:
				die.info('StepParab: Accepted logp=%g' % tmp.logp_nocompute())
		else:
			self._set_failed( tmp )
			accepted = 0
			self._set_current(self.current())
			if Debug>2:
				die.warn('StepParab: Rejected logp=%g vs. %g, T=%g'
						% (tmp.logp_nocompute(), self.current().logp(), self.acceptable.T()))
		return accepted
