...
def err_before_fit(data, minsv=None, minsvr=None, constant=True):
	...
	A, B, errs, sv, ranks = localfit(tmp, minsv=minsv, minsvr=minsvr, constant=constant)
	return errs
...
def pack(data, constant=True):
	...
	use_c = bool(constant)
	if not (n+use_c > 0):
		raise ValueError, "Input coordinates have zero dimension."
...
def localfit(data, minsv=None, minsvr=None, constant=True):
	...
	errs = numpy.sum( soln.residual()**2, axis=0 )
	sv = soln.sv()
	x = soln.x(copy=False)
	rank = soln.rank()
...
def robust_localfit(data, minsv=None, minsvr=None, constant=True):
	...
	for j in range(m):
		...
		errs.append( numpy.sum( soln.residual()**2 ) )
