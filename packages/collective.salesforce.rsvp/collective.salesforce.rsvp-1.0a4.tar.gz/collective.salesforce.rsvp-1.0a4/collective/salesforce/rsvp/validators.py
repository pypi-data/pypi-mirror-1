...
class ValidateObjectIdOfTypeExistence(object):
    ...
    def __call__(self, request):
        ...
        if obj_id and obj_type: # we'll catch missing field values through other validation mechanisms
            ...
            res = sbc.query(['Id',], obj_type, "Id='%s'" % obj_id)
            num_results = res['size']
            
            if num_results == 0:
                return {self.obj_id_field_name : _(u"There must be exactly one object of type %s with the id %s within \
                                                     your Salesforce.com instance for RSVP capabilities.") % (obj_type, obj_id)}
...
class ValidateCapacityCheckingConfiguration(object):
    ...
    def __call__(self, request):
        ...
        if cap_val and cnt_val or not cap_val and not cnt_val:
            ...
            if cap_val:
                return {self.attendee_count_field_name : _(u"You've added a value for the 'Maximum Capacity Field', \
            ...
            if cnt_val:
                return {self.max_cap_field_name : _(u"You've added a value for the 'Attendee Count Field', \
