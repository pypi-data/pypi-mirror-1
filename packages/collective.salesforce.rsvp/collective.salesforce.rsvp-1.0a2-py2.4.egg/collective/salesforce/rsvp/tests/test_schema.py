...
class TestSchemaExtender(SFRSVPTestCase):
    ...
    def test_sfobject_type_field_vocabulary_choices(self):
        ...
        for sfobject_field in ('Total Leads','Active',): # some obvious field labels on the 'Campaign'
    ...
    def test_set_get_on_rsvpable_schema(self):
        ...
        self.assertEqual('0089HG_BOGUS_PLONE_TEST_CASE', self.rsvpable.getField('sfObjectId').get(self.rsvpable))
...
class TestSchemaValidation(SFRSVPTestCase):
    ...
    def test_validation_fails_with_nonexistent_objectid(self):
        ...
        # setup the request
        request = FakeRequest(sfObjectId = '0089HG_BOGUS_PLONE_TEST_CASE', sfObjectSignupType = 'Campaign')
    ...
    def test_validate_capacity_fields_configuration(self):
        ...
        self.failUnless(cap_config_validator(FakeRequest(maxCapacityField='', 
                                                        attendeeCountField='foo')).has_key('maxCapacityField'))
        self.failUnless(cap_config_validator(FakeRequest(maxCapacityField='bar', 
                                                        attendeeCountField='')).has_key('attendeeCountField'))
        
        # something in both of the fields good
        self.failIf(cap_config_validator(FakeRequest(maxCapacityField='foo', 
