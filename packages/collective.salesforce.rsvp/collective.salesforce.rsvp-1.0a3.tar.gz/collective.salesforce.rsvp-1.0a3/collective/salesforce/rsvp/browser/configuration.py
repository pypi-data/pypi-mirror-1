...
class RSVPConfigurationView(BrowserView):
    ...
    def enableRSVPs(self):
        ...
        plone_utils.addPortalMessage(
            _(u"Your item is ready for RSVP configuration via the '%s' section under the edit tab." % RSVP_SCHEMATA))
    ...
    def disableRSVPs(self):
        ...
        if hasattr(self.request, 'RESPONSE'):
            plone_utils = getToolByName(self.context, 'plone_utils')
            plone_utils.addPortalMessage(_(u"Disabled RSVP capabilities"))
            self.request.RESPONSE.redirect(self.context.absolute_url())
...
class DynamicSFObjectFields(PloneKSSView):
    ...
    def _listFieldsForSFObjectType(self, sfobject):
        ...
        return [(k, v.label) for k, v in obj_schema.fields.items()]
