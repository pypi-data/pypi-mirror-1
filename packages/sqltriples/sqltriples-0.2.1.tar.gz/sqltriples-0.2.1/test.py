...
if not args.has_key("no-test"):
    ...
    print s2
    print
    # Test of expression as object plus negation.
    # NOTE: Since negation wraps existing results and doesn't offer a proper query,
    # NOTE: we must wrap it in another call.
    not_s2 = s.triples([s.negation(s.subjects(None, ("_ > ?", [Literal("2")])))])
    print "Should be 0 results:", len(not_s2) == 0
    print not_s2
    print
    # Test of presence.
    ...
    # NOTE: Since negation wraps existing results and doesn't offer a proper query,
    # NOTE: we must wrap it in another call.
    not_has_b = s.triples([s.negation(s.triples((None, URIRef("b"), s.Defined())))])
    print "Should be 6 results:", len(not_has_b) == 6
    print not_has_b
    print
    # Test of triple combination.
    s1_and_s2 = s.triples([s1, s2])
    print "Should be 6 triples:", len(s1_and_s2) == 6
    print s1_and_s2
    print
    # Test of triple combination using intersections.
    s1_and_s2 = s.intersection([s1, s2])
    print "Should be 1 triple:", len(s1_and_s2) == 1
    print s1_and_s2
    print
    # Test of triple combination using conjunctions and negations.
    not_s1_and_s2 = s.triples(s.Conjunction([s.negation(s1), s2]))
    print "Should be 3 triples:", len(not_s1_and_s2) == 3
    print not_s1_and_s2
    print
    # Test of triple combination.
    ...
    print result
    print
    # Test of triple combination using expressions.
    result = s.triples(s.Expression("_ or _", [t2, subject_a]))
