...
class SugarService(ElementSOAP.SoapService):
    ...
    def get_module_fields(self, session_id, module_name):
        '''
        returns the list of modules names (strings)
        we can access with our login
        '''
        ...
        ret = response.find('return')
        error_elem = ret.find('error')
        error = error_elem.findtext('number')
        if error:
            error = int(error_elem.findtext('number'))
            name = error_elem.findtext('name')
            desc = error_elem.findtext('description')
            raise SugarError('number: %s, name: "%s", desc: "%s"' % (
                    error, name, desc))
        fields = [a for a in ret.find('module_fields')]
        return fields

    def get_module_fields_as_dict(self, session_id, module_name):
        """same as get_module_fields but with convenient dict for easy use.
        for each field compute
        - its attributes (name, required, label, type, options)
        - a reference to itself
        - a dict of options with:
            - a ref to the option
            - option name
            - option value
        """
            ...
            [(field.findtext("name"),
              {
                  ...
                  'required': bool(int(field.findtext("required"))),
                  ...
                  'options': dict(
                      [(a.findtext('name'),
                       ...
                       for a in field.find('options')
                       if field.find('options').getchildren()
                      ]
                  )
              }
             )
             for field in self.get_module_fields(session_id, module_name)
            ]
        )
        return fields_dict

    def get_field_options(self, session_id, module_name, field_name):
        fields = self.get_module_fields_as_dict(session_id, module_name)
...
class SugarSession:
    ...
    def __init__(self, username, password, base_url,
            debug=False, user_management=False, nusoapfile='soap.php'):
        '''
        ...
        # try to get moved location

        from elementsoap.HTTPClient import HTTPClient, HTTPError
        c = HTTPClient(soap_url)
        try:
            c.do_request('get /')
        except HTTPError, e:
            if e[0] in (301, 302):
                soap_url =  e.args[2].getheaders('location')[0]
            else:
                print "%s"% (
                    "PySUGARNG\n"
                    "Error while trying to connect to SugarCRM Index, "
                    "it can be a false positive if you need "
                    "HTTP BASIC AUTH.\n"
                    "If the webservice behaves not correctly, please inspect this error.\n"
                    "Error: %s" % e
                )
    ...
    def login(self, username, password):
        ...
        if self._debug:
            print "We are logged-in with user-id: '%s'" %\
            self.get_user_id()
    ...
    def get_module_fields(self, module_name):
        self.__validate_login()
        return self.service.get_module_fields(
                self._session_id,
                module_name)
    ...
    def get_module_fields_as_dict(self, module_name):
        """See service for doc."""
        self.__validate_login()
        return self.service.get_module_fields_as_dict(
            self._session_id, module_name
        )
