...
class siteobj:
    ...
    def createModel(self,init,par,modtype='SEIR',name='model1',v=[],bi=None,bp=None):
        """
        ...
        self.par = par
        self.model = popmodels(self.id,par,type=modtype,v=self.values,bi = self.bi, bp = self.bp)
    ...
    def runModel(self):
        ...
        self.ts.append(self.model.step(self.ts[-1],self.par,theta,npass))
...
##                    self.parentGraph.lightGRNode(self,'r') 
    ...
    def getTheta(self,npass, delay):
        ...
        if not self.stochtransp:
            ...
            #theta = poisson(p*popul[Iposition])
    ...
    def getCentrality(self):
        ...
        if not self.parentGraph.allPairs:
...
class popmodels:
    ...
    """
    def __init__(self,parentsite,par,type='SIR',v=[],bi=None,bp=None):
        """
    ...
    def stepFlu(self,vars,par,theta=0,npass=0):
        """
        ...
        alpha,beta,r,e,c,g,d,pc1,pc2,pc3,pc4,pp1,pp2,pp3,pp4,B = (self.bp['alpha'],self.bp['beta'],self.bp['r'],self.bp['e'],self.bp['c']
        ,self.bp['g'],self.bp['d'],self.bp['pc1'],self.bp['pc2'],self.bp['pc3'],self.bp['pc4'],
        self.bp['pp1'],self.bp['pp2'], self.bp['pp3'], self.bp['pp4'],self.bp['b'])
        ...
        L1pos = beta*S1*(Infectantes/(N+npass))**alpha
        L2pos = beta*S2*(Infectantes/(N+npass))**alpha
        L3pos = beta*S3*(Infectantes/(N+npass))**alpha
        L4pos = beta*S4*(Infectantes/(N+npass))**alpha
    ...
    def stepSIS(self,inits,par,theta=0, npass=0):
        """
    ...
    def stepSIS_s(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0,dist='poisson'):
        """
    ...
    def stepSIR(self,inits,par,theta=0, npass=0):
        """
    ...
    def stepSIR_s(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0,dist='poisson'):
        """
    ...
    def stepSEIS(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0):
        """
    ...
    def stepSEIS_s(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0,dist='poisson'):
        """
    ...
    def stepSEIR(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0):
        """
    ...
    def stepSEIR_s(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0,dist='poisson'):
        """
        ...
        if dist == 'poisson':
            Lpos = rpy.r.rpois(1,Lpos_esp) #poisson(Lpos_esp)
...
##                print Lpos,Lpos_esp,S,I,theta,N,self.parentSite.sitename
    ...
    def stepSIpRpS(self,inits,par,theta=0, npass=0):
        """
    ...
    def stepSIpRpS_s(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0,dist='poisson'):
        """
    ...
    def stepSEIpRpS(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0):
        """
    ...
    def stepSEIpRpS_s(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0,dist='poisson'):
        """
    ...
    def stepSIpR(self,inits,par,theta=0, npass=0):
        """
        ...
        Lpos = beta*S*((I+theta)/(N+npass))**alpha #Number of new cases
        Lpos2 = p*beta*R*((I+theta)/(N+npass))**alpha
    ...
    def stepSIpR_s(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0,dist='poisson'):
        """
        ...
        Lpos_esp = beta*S*((I+theta)/(N+npass))**alpha #Number of new cases
        Lpos2_esp = p*beta*R*((I+theta)/(N+npass))**alpha
    ...
    def stepSEIpR(self,inits,par,theta=0, npass=0):
        """
    ...
    def stepSEIpR_s(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0,dist='poisson'):
        """
    ...
    def stepSIRS(self,inits,par,theta=0, npass=0):
        """
    ...
    def stepSIRS_s(self,inits=(0,0,0),par=(0.001,1,1,0.5,1,0,0,0),theta=0, npass=0,dist='poisson'):
        """
        Defines an stochastic model SIR:
        - inits = (E,I,S)
        - par = (Beta, alpha, E,r,delta,B,w,p) see docs.
...
class edge:
    ...
    def __init__(self, source, dest, fmig=0, bmig=0, Leng=0):
        ...
        if not isinstance(source, siteobj):
            raise Error, 'source received a non siteobj class object'
        if not isinstance(dest, siteobj):
            raise Error, 'destination received a non siteobj class object'
        self.dest = dest
        self.source = source
...
class graph:
    ...
    def shortestPath(self,G,start,end):
        ...
        The input has the same conventions as self.sijkstra().
...
##            o.visible = 0
    ...
    def getAllPairs(self):
        ...
        """
        if self.allPairs: #don't run twice
        ...
        dm = zeros((d,d),Float)
        ap = zeros((d,d),Float)
        for i,sitei in enumerate(g.keys()):
             for j, sitej in enumerate(g.keys()[:i]): #calculates only the lower triangle
    ...
    def getShortestPathLength(self,origin,sp):
        ...
        for i in range(len(sp)-1):
            Length += sp[i].getDistanceFromNeighbor(sp[i+1])
        return Length
    ...
    def getConnMatrix(self):
        ...
        """
        if self.connmatrix: return self.connmatrix #don't run twice
