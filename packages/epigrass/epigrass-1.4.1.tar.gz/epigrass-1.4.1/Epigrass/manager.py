...
"""
import cPickle, pickle,time, rpy, os
...
from Epigrass.rinterface import *
import ConfigParser
...
class simulate:
    ...
    def __init__(self, fname=None, host='localhost',port=3306,db='epigrass',user='epigrass', password='epigrass'):
        ...
        sitios = loadData(self.sites, sep=',')
        ed = loadData(self.edges,sep=',')
    ...
    # generate bunch object for parameters and inits
        self.inits = {}
        self.parms = {}
...
##        show()
    ...
    def outToODb(self,table):
        ...
        DO.Site._uri = r'mysql://%s:%s@%s/%s'%(user,pw,host,db)
        DO.Edge._uri = r'mysql://%s:%s@%s/%s'%(user,pw,host,db)
        DO.Site._table = name
        DO.Edge._table = name+'e'
        #adding columns to sites table
        DO.Site.addColumn(FloatCol('incidence'))
        DO.Site.addColumn(FloatCol('arrivals'))
        if self.g.site_list[0].values:#add columns for 'values' variables
            for i in range(len(self.g.site_list[0].values)):
                DO.Site.addColumn(FloatCol('values%s'%i))
        for s in self.g.site_list[0].vnames:
            DO.Site.addColumn(FloatCol(s))
        #creating tables
        DO.Site.createTable()
        DO.Edge.createTable()
        ...
        for site in self.g.site_list:
            ...
            t = 0
            for i in ts:
                dicin['incidence']= site.incidence[t]
                dicin['arrivals'] = site.thetahist[t]
                dicin['time'] = t
    ...
    def dumpData(self):
        ...
        curdir = os.getcwd()
        if not self.outdir:
            outdir = 'outdata-'+self.modelName.split('/')[-1]
            try:
        ...
        if self.round:
            epp = open('epipath%s.csv'%str(self.round),'w')
        else:
            epp = open('epipath.csv','w')
        ...
        if os.path.exists('epistats.csv'):
            stf = open('epistats.csv','a')#append to file
        else:
            stf = open('epistats.csv','w')#create a new file
        ...
        if os.path.exists('sitestats.csv'):
            sitef = open('sitestats.csv','a') #append to file
        else:
            sitef = open('sitestats.csv','w')
...
##            sitef.write(str(s.sitename)+','+st+it+'\n')
            sitef.write(str(self.round)+','+str(s.geocode)+','+str(s.sitename)+','+it+','+degree+','+central+','+bet+','+thidx+','+distseed+','+seedgc+','+seedname+'\n')
