...
##        self.sphere.mass = self # necessary to access the mass in the DnD loop
        self.box = visual.box(pos=visual.vector(pos),length=r,height=r,width=r,color=color, name='',**keywords)
        self.m = float(m)
##        self.sphere.pos = visual.vector(pos)
        ...
        self. graph = None
        self.painted = 0
        self.box.Name = visual.label(pos=visual.vector(pos), text=str(name), xoffset=20,
    ...
    def calcGravityForce(self, g):
        ...
        for n in self.graph.nodes:
            if not self==n:
                self.F -= (self.sphere.pos-n.sphere.pos)
        # gforce = m * g
...
class _Edge:
    ...
    def calcSpringForce(self):
        ...
        """
        delta = self.n1.sphere.pos - self.n0.box.pos
        l = visual.mag(delta)
...
class Graph:
    ...
    def centerView(self):
        cnt = average(array([i.box.pos for i in self.nodes]))
