...
##        show()
    ...
    def outToODb(self,table):
        ...
        DO.Site.sqlmeta.addColumn(FloatCol('incidence'))
        DO.Site.sqlmeta.addColumn(FloatCol('arrivals'))
        if self.g.site_list[0].values:#add columns for 'values' variables
            for i in range(len(self.g.site_list[0].values)):
                DO.Site.sqlmeta.addColumn(FloatCol('values%s'%i))
        for s in self.g.site_list[0].vnames:
            DO.Site.sqlmeta.addColumn(FloatCol(s))
        #creating tables
        DO.Site.createTable()
        DO.Edge.createTable()
