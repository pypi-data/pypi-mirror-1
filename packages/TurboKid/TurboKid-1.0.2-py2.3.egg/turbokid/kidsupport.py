...
class KidSupport(object):
    ...
    def render(self, info, format="html default", fragment=False, template=None):
        ...
        if len(format) < 2:
            output, format = format[0], 'default'
            if output.endswith('-straight'):
                # support old notation 'html-straight' instead of 'html straight'
                output, format = output[:-9], output[-8:]
        else:
            output, format = format

        return t.serialize(encoding=self.defaultencoding, fragment=fragment,
