...
class Scheduler:
    ...
    def add_interval_task(self, action, interval, args=None, kw=None, initialdelay=0, 
        ...
        """
        Pass in initialdelay with a number of seconds to wait before running and an 
        interval with the number of seconds between runs.  
        
        For example, an initialdelay of 600 and interval of 60 would mean "start running
        after 10 minutes and run every 1 minute after that".
    ...
    def add_weekday_task(self, action, weekdays, timeonday, args=None, kw=None,
            processmethod=method.threaded, taskname=None):
        """
        Runs on certain days of the week. Pass in a list or tuple of weekdays from
        1-7 (where 1 is Monday). Additionally, you need to pass in timeonday which is
        the time of day to run. timeonday should be a tuple with (hour, minute).
