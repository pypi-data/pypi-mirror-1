...
def refresh_info( app, table, processes_rows ):
    ...
    idx, ( pid, cpu, mem, name ) = processes_rows[ 0 ]
    ...
    table[ 2 ][ 1 ] = str( info.pid )
    table[ 3 ][ 1 ] = units( info.vmsize )
    table[ 4 ][ 1 ] = units( info.vmrss )
    table[ 5 ][ 1 ] = info.cmdline
...
def refresh_processes( app, table ):
    ...
    for idx, ( pid, c, m, name ) in enumerate( table ):
...
""",
           ...
           left=( Table( id="processes",
                           label=None,
                           headers=( "PID", "C%", "M%", "Name" ),
