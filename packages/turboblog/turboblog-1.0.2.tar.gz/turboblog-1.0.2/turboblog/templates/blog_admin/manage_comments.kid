...
<script src="${tg.url('/static/javascript/refreshcomment.js')}" type="text/javascript"></script>
<body>
      ${submenu}

      <div class="wrap">
          <div class="searchtext" style="float: left; width: 16em; margin-right: 3em;">
              ${search_text.display_with_params(search_text, st_params)}
          </div>
          <div class="searchdate" style="float: left; width: 35em; margin-bottom: 1em;">
              ${search_date.display_with_params(search_date, sd_params)}
          </div>

          <br style="clear: both; " />
          <p><a href="?bid=${blog.id};mode=view">View Mode</a>
          | <a href="?bid=${blog.id};mode=mass">Mass Edit Mode</a></p>
          <ol py:if="not mass" id='the-list' class='commentlist'>
              ...
              <p id="${'commentp-%s' % comment.id}">${comment.content}</p>

              <p>Posted ${comment.creation_time} | <a href="#"
                  onclick="showmymodal('${tg.url('/blog_admin/edit_comment?bid=%s;cid=%s' % (blog.id, comment.id))}', 500, 450, '${tg.url('/blog_admin/comment_value?cid=%s' % comment.id)}');">Edit Comment</a>
              | <a href="${comment.delete_link()}"
                  onclick="return confirm('You are about to delete this comment?. Cancel to stop, OK to delete.');">
                  Delete Comment</a>
