...
<body>
      <ul id="submenu">
          ...
          <li><a href='/blog_admin/manage_tags?bid=${bid}'>Tags</a></li>
          ...
          <table id="the-list-x" cellpadding="3" cellspacing="3" width="100%">                    
              <tbody>
                  ...
                  <tr class="${i%2 and 'alternate' or ''}" py:for="i,item in enumerate(files)">
                      ...
                      <td>${item.filename}</td>
                      <td>${tg.url('/show_file?bid=%s;file_id=%s' % (bid, item.id))}</td>
                      <td>${tg.url('/download_file?bid=%s;file_id=%s' % (bid, item.id))}</td>
                      <td><a href="/blog_admin/delete_file?bid=${bid};file_id=${item.id}"
