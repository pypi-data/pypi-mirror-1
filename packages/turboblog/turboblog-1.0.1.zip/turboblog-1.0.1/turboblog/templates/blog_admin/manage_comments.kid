...
<body>
      <ul id="submenu">
          <li><a href='/blog_admin/manage_posts?bid=${blog.id}' >Posts</a></li>
          <li><a href='/blog_admin/manage_comments?bid=${blog.id}' class="current">Comments</a></li>
      ...
      <div class="wrap">
          ...
          <ol py:if="not mass" id='the-list' class='commentlist'>
              <li class='${i%2 and "alternate" or ""}' py:for="i,comment in enumerate(blog.get_comments())" id='comment-${comment.id}' >        
              <p>
              <strong>Namie:</strong> ${comment.author.display_name} | <strong>E-mail:</strong>
          ...
          <table id="the-list-x" cellpadding="3" cellspacing="3" width="100%"> 
              <tbody>
                  ...
                  <tr class="${i%2 and 'alternate' or ''}" py:for="i,item in enumerate(blog.get_comments())">
