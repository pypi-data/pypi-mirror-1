...
<body>
  ...
  <span py:strip="1" py:for="blog in Blog.select()">
      ...
      <a href="${tg.url('/admin/delete_blog?bid=%s' % blog.id)}" onclick="return confirm('Are you sure you want to delete?')">[ Delete ]</a>
      </input>
      <br/>
  </span><br/>
  Site Administrator:
  ...
  <div id="blogCreator" style="display: none;" >
      <form action="${tg.url('/admin/create_blog')}" method="POST">
          <table>
              <tr>
