...
'''
data = []
data_allowed_ext = ('css', 'png', 'svg', 'ttf')
for root, subFolders, files in os.walk(os.path.join('pymt', 'data')):
    if 'sandbox' in root:
        continue
    for file in files:
        ext = file.split('.')[-1].lower()
...
# write manifest.in
data_includes = ['data/icons/filetype/*.png',
        'data/icons/svg/*.svg',
        'data/icons/*.png',
        'data/*.css',
        'data/*.png',
...
with open('MANIFEST.in', 'w') as fd:
    fd.write('recursive-include
    for filename in data_includes:
        fd.write('%s\n' % filename)
'''
