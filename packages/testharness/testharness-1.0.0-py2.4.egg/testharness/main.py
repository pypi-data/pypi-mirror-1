...
def run_scripts(options, test_harness):
	...
	"""
	log = logging.getLogger("testharness.main.run_scripts")
...
def main():
	...
	"""
	parser = OptionParser()
	parser.add_option("-c", "--config", action="store", dest="config_filename", default="tester.ini",
		help="This is the config file to be used.")
	parser.add_option("-l", "--logconfig", action="store", dest="logconfig_filename", default="testharness_log.ini",
	...
	parser.add_option("--testpath", action="store", dest="test_path", default="acceptancetests",
		help="This is the config file to be used by the logging module.")
	(options, args) = parser.parse_args()

	log = logging.getLogger("testharness.main.main")
	...
	if os.path.isfile(options.logconfig_filename):
		...
		# No log configuration file given, just print out to console instead:
	...
	sys.path.insert(0, options.test_path)
