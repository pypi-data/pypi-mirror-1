...
function initEditor(textarea, toolbar, preview, prev_text, options) {
    var editor = new Editor(
        ['textarea', AJS.$(textarea)],
        ['toolbar', AJS.$(toolbar)],
        ['smileybar', AJS.DIV({id: 'smileybar'})],
        ['buttonbar', AJS.DIV({id: 'buttonbar'})],
        ['selectbar', AJS.DIV({id: 'selectbar'})],
        ['accesskeys', []],
        ['formats', options['buttons']],
        ['buttoninfo', AJS.DIV()],
        ['smilies', options['smilies']],
        ['preview', AJS.$(preview)],
        ['prev_text', AJS.$(prev_text)]
    );
    ...
    for (var z = 0; z < editor.formats.length; z++) {
        ...
        if (format["values"] == null) { // if button
            ...
            for (var y = 0; y < format["values"].length; y++) {
                ...
                option.innerHTML = format["values"][y][1];
                option.value = format["values"][y][0];
...
function preview(area) {
    ...
    var call = new pocoo.lib.async.RPC('core.newpost.preview');
    call.send([text], {}, function (result) {
