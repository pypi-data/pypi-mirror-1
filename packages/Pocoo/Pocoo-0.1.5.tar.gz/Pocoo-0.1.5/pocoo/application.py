...
class Page(Component):
    ...
    `handler_regexes` of all registered `Page` components
...
class RequestWrapper(Component):
    ...
    to the Page and process the response before
    ...
    def process_request(self, req):
        ...
        (especially any `pocoo.application.Page` components),
        else ``None``.
        """
...
class Pocoo(object):
    ...
    def __init__(self, ctx):
        ...
        collect the Pages here.
        """
        self.ctx = ctx

        mapping = {}
        for comp in ctx.get_components(Page):
    ...
    def __call__(self, environ, start_response):
        ...
        This tries to call `Page.handle_request` on the first
        Page for which one of the handler regexes matches the
