...
class Connection(Connection):
	...
	def iterprivileges(self, schema="user"):
		...
		if schema == "all":
			cursor.execute("select decode(grantor, user, null, grantor) as owner, privilege, table_name as object, grantee from all_tab_privs order by table_schema, table_name, privilege")
		else:
			cursor.execute("select decode(grantor, user, null, grantor) as owner, privilege, table_name as object, grantee from user_tab_privs where owner=user order by table_name, privilege")
		for rec in cursor.fetchall():
			yield Privilege(rec.privilege, rec.object, rec.grantee, rec.owner, cursor.connection)
...
class Callable(MixinNormalDates, MixinCodeDDL, Object):
	...
	def __init__(self, name, owner=None, connection=None, encoding=None):
	...
	def _getparamarray(self, cursor, *args, **kwargs):
		...
		for arginfo in self._argsbypos:
			...
			if arginfo.isout or (isinstance(realarg, basestring) and len(realarg) >= 32768):
				try:
...
class Procedure(Callable):
	...
	def __call__(self, cursor, *args, **kwargs):
		...
		result = Record()
		for (i, value) in enumerate(cursor.callproc(name, realargs)):
			result[self._argsbypos[i].name] = value
		return result
...
class Function(Callable):
	...
	def __call__(self, cursor, *args, **kwargs):
		...
		returnvalue = cursor.callfunc(name, self._ora2cx[self._returnvalue.datatype], realargs)
		result = Record()
...
class Privilege(object):
	...
	"""
	def __init__(self, privilege, object, grantee, owner=None, connection=None):
	...
	def __repr__(self):
		if self.owner is not None:
			return "%s.%s(%r, %r, %r, %r)" % (self.__class__.__module__, self.__class__.__name__, self.privilege, self.object, self.grantee, self.owner)
		else:
			return "%s.%s(%r, %r, %r)" % (self.__class__.__module__, self.__class__.__name__, self.privilege, self.object, self.grantee)

	def __str__(self):
		if self.owner is not None:
			return "%s(%r, %r, %r, %r)" % (self.__class__.__name__, self.privilege, self.object, self.grantee, self.owner)
		else:
			return "%s(%r, %r, %r)" % (self.__class__.__name__, self.privilege, self.object, self.grantee)
	...
	def grantddl(self, connection=None, term=True, mapgrantee=True):
		...
		if grantee is None:
			return ""
		code = "grant %s on %s to %s" % (self.privilege, self.object, grantee)
		if term:
			code += ";\n"
		return code
