...
class Procedure(MixinNormalDates, MixinCodeDDL, Object):
	...
	def _calcargs(self, cursor):
		if self._argsbypos is None:
			cursor.execute("select object_id from all_objects where owner=nvl(:owner, user) and lower(object_name)=lower(:name) and object_type='PROCEDURE'", owner=self.owner, name=self.name)
