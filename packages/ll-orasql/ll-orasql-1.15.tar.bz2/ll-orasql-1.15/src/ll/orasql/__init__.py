...
class Record(dict):
	...
	def fromdata(cls, cursor, row):
		...
		for (i, field) in enumerate(row):
			if isinstance(field, LOB) and (cursor.readlobs is True or (isinstance(cursor.readlobs, (int, long)) and field.size() <= cursor.readlobs)):
				field = field.read()
...
class Connection(Connection):
	...
	def __init__(self, *args, **kwargs):
		"""
		Create a new connection. In addition to the parameters supported
		by <function>cx_Oracle.connect</function> the keyword argument
		<arg>readlobs</arg> is supported. If <arg>readlobs</arg> is <lit>False</lit>
		...
		all <class>LOB</class>s will be returned as strings.
		"""
		if "readlobs" in kwargs:
			kwargs = kwargs.copy()
			self.readlobs = kwargs.pop("readlobs")
		else:
			self.readlobs = False
...
class Cursor(Cursor):
	...
	def __init__(self, connection, readlobs=None):
		...
		For the meaning of <arg>readlobs</arg> see
