...
*	A parameter :var:`readlobs` has been added to various constructors and
	methods that can be used to get small (or all) :class:`LOB`s as strings in
	cursor fetch calls.
...
*	Dependencies involving :class:`MaterializedViewDefinition`s and
	:class:`IndexDefinition`s generated by constraints work properly now, so that
	iterating all definitions in create order really results in a working SQL
	script.
