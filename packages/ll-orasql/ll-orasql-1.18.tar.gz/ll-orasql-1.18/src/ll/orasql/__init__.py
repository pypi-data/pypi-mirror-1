...
"""
<p><mod>ll.orasql</mod> contains utilities for working with
<a href="http://www.computronix.com/utilities.shtml#Oracle"><app>cx_Oracle</app></a>:</p>
<ul>
...
class Record(dict):
	...
	<meth>__getitem__</meth> and <meth>__getattr__</meth>) are available.
...
class Connection(Connection):
	...
	def __init__(self, *args, **kwargs):
		"""
		<p>Create a new connection. In addition to the parameters supported
		by <func>cx_Oracle.connect</func> the following keyword arguments
		are supported.</p>
		<dl>
	...
	def cursor(self, readlobs=None, unicode=None):
		...
		<pyref method="__init__"><meth>__init__</meth></pyref>.
		"""
	...
	def iterschema(self, schema="user"):
		"""
		<p>Generator that returns the number of different object types for this
		database.</p>
		
		<p>For the meaning of <arg>schema</arg> see
		<pyref method="iterobjects"><meth>iterobjects</meth></pyref>.</p>
		"""
		cursor = self.cursor()
		if schema == "all":
	...
	def itertables(self, schema="user"):
		"""
		<p>Generator that yields all table definitions in the current users schema
		(or all users schemas).</p>
		<p><arg>schema</arg> specifies from which tables should be
		yielded:</p>
		<dl>
		...
		<p>Tables that are materialized view will be skipped in both casess.</p>
		"""
	...
	def iterfks(self, schema="user"):
		"""
		<p>Generator that yields all foreign key constraints in the current users schema
		(or all users schemas).</p>
		<p><arg>schema</arg> specifies from which tables should be
		yielded:</p>
		<dl>
	...
	def iterobjects(self, mode="create", schema="user"):
		"""
		<p>Generator that yields the sequences, tables, primary keys,
		...
		schemas) in a specified order.</p>
		<p><arg>mode</arg> specifies the order in which objects will be yielded:</p>
		<dl>
		...
		<p><arg>schema</arg> specifies from which schema objects should be
		yielded:</p>
		<dl>
	...
	def iterprivileges(self, schema="user"):
		"""
		<p>Generator that yields object privileges for the current users
		(or all users) objects.</p>
		<p><arg>schema</arg> specifies which privileges should be yielded:</p>
		<dl>
...
class Cursor(Cursor):
	"""
	A subclass of <mod>cx_Oracle</mod>s cursor class. Database results
	returned from <meth>fetchone</meth>, <meth>fetchmany</meth>,
	<meth>fetchall</meth> and <meth>fetch</meth> or by iterating the
...
class Object(object):
	"""
	<p>The base class for all Python classes modelling schema objects in the
	database.</p>
	...
	def iterreferences(self, connection=None):
		"""
		<p>Objects directly used by <self/>.</p>

		<p>If <arg>connection</arg> is not <lit>None</lit> it will be used as
		...
		there is not such connection, you'll get an exception.</p>
		"""
		(connection, cursor) = self.getcursor(connection)
	...
	def iterreferencesall(self, connection=None, done=None):
		"""
		<p>All objects used by <self/> (recursively).</p>

		<p>For the meaning of <arg>connection</arg> see
		<pyref method="iterreferences"><meth>iterreferences</meth></pyref>.</p>

		<p><arg>done</arg> is used internally and shouldn't be passed.</p>
		"""
		if done is None:
			done = set()
		if self not in done:
	...
	def iterreferencedbyall(self, connection=None, done=None):
		"""
		<p>All objects depending on <self/> (recursively).</p>
	...
	def iterobjects(cls, connection, schema="user"):
		"""
		<p>Generator that yields all objects of this type in the database schema
		of <arg>cursor</arg>.</p>
		"""
...
class Table(MixinNormalDates, Object):
	...
	def itercolumns(self, connection=None):
		"""
		<p>Generator that yields all column objects of the <class>Table</class> <self/>.</p>
		"""
		(connection, cursor) = self.getcursor(connection)
	...
	def iterrecords(self, connection=None):
		"""
		<p>Generator that yields all records of the table <self/>.</p>
		"""
		(connection, cursor) = self.getcursor(connection)
...
class Callable(MixinNormalDates, MixinCodeDDL, Object):
	...
	def iterarguments(self, connection=None):
		"""
		<p>Generator that yields all <pyref class="ArgumentInfo">arguments</pyref> of the function/procedure <self/>.</p>
		"""
		(connection, cursor) = self.getcursor(connection)
...
class Procedure(Callable):
	...
	def __call__(self, cursor, *args, **kwargs):
		...
		<arg>kwargs</arg>. <arg>cursor</arg> must be a <mod>cx_Oracle</mod>
		or <mod>ll.orasql</mod> cursor. This will return a
...
class Function(Callable):
	...
	def __call__(self, cursor, *args, **kwargs):
		...
		<arg>kwargs</arg>. <arg>cursor</arg> must be a <mod>cx_Oracle</mod>
		or <mod>ll.orasql</mod> cursor. This will return a tuple containing
