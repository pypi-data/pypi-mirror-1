...
__ http://python.net/crew/atuining/cx_Oracle/
"""
...
class SQLObjectNotFoundError(Exception):
	...
	def __repr__(self):
		return "<%s.%s obj=%r at 0x%x>" % (self.__class__.__module__, self.__class__.__name__, self.obj, id(self))
		...
		if mode == "flat":
			...
			def do(table):
				if table not in done:
					...
					cursor.execute("select ac2.table_name, decode(ac2.owner, user, null, ac2.owner) as owner from all_constraints ac1, all_constraints ac2 where ac1.constraint_type = 'R' and ac1.table_name=:name and ac1.owner = nvl(:owner, user) and ac1.r_constraint_name = ac2.constraint_name and ac1.r_owner = ac2.owner", name=table.name, owner=table.owner)
					for rec in cursor.fetchall():
						try:
