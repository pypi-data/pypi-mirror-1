...
class Connection(Connection):
	...
	def __xiter__(self, mode):
		if mode is None:
			...
			for item in self.iterobjects("create", "user"):
				yield item
		elif mode == "objectsdropuser":
			for item in self.iterobjects("drop", "user"):
				yield item
		elif mode == "objectsflatuser":
			for item in self.iterobjects("flat", "user"):
				yield item
		elif mode == "objectscreateall":
			for item in self.iterobjects("create", "all"):
				yield item
		elif mode == "objectsdropall":
			for item in self.iterobjects("drop", "all"):
				yield item
		elif mode == "objectsflatall":
			for item in self.iterobjects("flat", "all"):
				yield item
		else:
...
class Object(object):
	...
	def __xiter__(self, mode):
		if mode is None:
			...
			for line in ipipe.xiter(self.dropddl()):
				yield line
		else:
			for line in ipipe.xiter(self.createddl()):
				yield line
...
class Sequence(MixinNormalDates, Object):
	...
	def __xiter__(self, mode):
		if mode is None:
			...
			for line in ipipe.xiter(self.createddlcopy()):
				yield line
		else:
			for line in ipipe.xiter(self.createddl()):
				yield line
...
class Function(MixinNormalDates, MixinCodeDDL, Object):
	"""
	Models a function in the database.
	"""
	type = "function"
...
class Procedure(MixinNormalDates, MixinCodeDDL, Object):
	"""
	...
	def _calcargs(self, cursor):
		if self._argsbypos is None:
			self._argsbypos = []
			self._argsbyname = {}
			cursor.execute("select lower(argument_name) as name, lower(in_out) as in_out, lower(data_type) as datatype from user_arguments where lower(object_name)=lower(:name) and data_level=0 order by sequence", name=self.name)
	...
	def iterarguments(self, connection=None):
		"""
		<par>Generator that yields all <pyref class="ArgumentInfo">arguments</pyref> of the procedure <self/>.</par>
		"""
		(connection, cursor) = self.getcursor(connection)
		self._calcargs(cursor)
		for arginfo in self._argsbypos:
