...
class Record(dict):
	"""
	A :class:`Record` is a subclass of :class:`dict` that is used for storing
	results of database queries. Both item and attribute access (i.e.
	:meth:`__getitem__` and :meth:`__getattr__`) are available. Field names are
	case insensitive.
	"""
	...
	def fromdata(cls, cursor, row):
		"""
		This class method can be used to create a :class:`Record` object from the
		database data.
		"""
		return cls((descr[0].lower(), cursor._decode(field, descr[1] in (BLOB, BINARY))) for (descr, field) in itertools.izip(cursor.description, row))
...
class Connection(Connection):
	...
	def iterschema(self, schema="user"):
		...
		cursor = self.cursor()
		if schema == "all":
			cursor.xexecute("select object_type as type, count(*) as count from all_objects group by object_type")
		else:
			cursor.xexecute("select object_type as type, count(*) as count from user_objects group by object_type")
		for row in cursor.xfetchall():
	...
	def iterobjects(self, mode="create", schema="user"):
		...
		def dosequences():
			...
			if schema == "all":
				cursor.xexecute("select decode(sequence_owner, user, null, sequence_owner) as sequence_owner, sequence_name from all_sequences")
			else:
				cursor.xexecute("select null as sequence_owner, sequence_name from user_sequences")
			for rec in cursor.xfetchall():
		...
		def dotables():
			if schema == "all":
				cursor.xexecute("select decode(owner, user, null, owner) as owner, table_name from all_tables")
			else:
				cursor.xexecute("select null as owner, table_name from user_tables")
			for rec in cursor.xfetchall():
				...
				if not obj.ismview(self):
					# Primary key
					if schema == "all":
						cursor.xexecute("select decode(owner, user, null, owner) as owner, constraint_name from all_constraints where constraint_type='P' and owner=:owner and table_name=:name", owner=rec.owner, name=rec.table_name)
					else:
						cursor.xexecute("select null as owner, constraint_name from user_constraints where constraint_type='P' and table_name=:name", name=rec.table_name)
					for rec2 in cursor.xfetchall():
						for subobj in do(PrimaryKey(rec2.constraint_name, rec2.owner, self)):
							yield subobj
	
					# Comments
					if schema == "all":
						cursor.xexecute("select column_name from all_tab_columns where owner=:owner and table_name=:name order by column_id", owner=rec.owner, name=rec.table_name)
					else:
						cursor.xexecute("select column_name from user_tab_columns where table_name=:name order by column_id", name=rec.table_name)
					for rec2 in cursor.xfetchall():
		...
		def doconstraints():
			if schema == "all":
				cursor.xexecute("select constraint_type, decode(owner, user, null, owner) as owner, constraint_name from all_constraints where constraint_type in ('R', 'U') order by owner, table_name, constraint_type, constraint_name")
			else:
				cursor.xexecute("select constraint_type, null as owner, constraint_name from user_constraints where constraint_type in ('R', 'U') order by table_name, constraint_type, constraint_name")
		...
		def doindexes():
			if schema == "all":
				cursor.xexecute("select decode(owner, user, null, owner) as owner, index_name from all_indexes where index_type in ('NORMAL', 'FUNCTION-BASED NORMAL') order by owner, table_name, index_name")
			else:
				cursor.xexecute("select null as owner, index_name from user_indexes where index_type in ('NORMAL', 'FUNCTION-BASED NORMAL') order by table_name, index_name")
			for rec in cursor.xfetchall():
		...
		def dosynonyms():
			if schema == "all":
				cursor.xexecute("select decode(owner, user, null, owner) as owner, synonym_name from all_synonyms")
			else:
				cursor.xexecute("select null as owner, synonym_name from user_synonyms")
			for rec in cursor.xfetchall():
		...
		def doviews():
			if schema == "all":
				cursor.xexecute("select decode(owner, user, null, owner) as owner, view_name from all_views")
			else:
				cursor.xexecute("select null as owner, view_name from user_views")
			for rec in cursor.xfetchall():
		...
		def domviews():
			if schema == "all":
				cursor.xexecute("select decode(owner, user, null, owner) as owner, mview_name from all_mviews")
			else:
				cursor.xexecute("select null as owner, mview_name from user_mviews")
			for rec in cursor.xfetchall():
		...
		def docode():
			for type in (Function, Procedure, Package, PackageBody, Type, Trigger, JavaSource):
				if schema == "all":
					cursor.xexecute("select decode(owner, user, null, owner) as owner, object_name from all_objects where lower(object_type)=lower(:type)", type=type.type)
				else:
					cursor.xexecute("select null as owner, object_name from user_objects where lower(object_type)=lower(:type)", type=type.type)
				for rec in cursor.xfetchall():
	...
	def getobject(self, name, owner=None):
		...
		for query in (
			...
			cursor.xexecute(query, object_name=name, owner=owner)
			rec = cursor.xfetchone()
			if rec is not None:
...
class Cursor(Cursor):
	"""
	A subclass of the cursor class in :mod:`cx_Oracle`. A :class:`Cursor` object
	provides the following additional methods: :meth:`xexecute`,
	:meth:`xexecutemany`, :meth:`xfetchone`, :meth:`xfetchmany`, :meth:`xfetchall`
	and :meth:`xfetch`. The "execute" methods support a unicode statement and
	unicode parameters (they will be encoded in the client encoding before being
	passed to the database). The "fetch" methods will return records as
	:class:`Record` objects and string values and ``CLOB`` values, if the
	cursors :attr:`readlobs` attribute has the appropriate value) will be
	returned as :class:`unicode` objects (except for :class:`BLOB` values).
	(Note that strings in the national character set (and :class:`NCLOB` values)
	are not supported).
	"""
	...
	def xexecute(self, statement, parameters=None, **kwargs):
		...
		if parameters is not None:
			...
			return self.execute(self._encode(statement), parameters, **kwargs)
		return self.execute(self._encode(statement), **kwargs)

	def xexecutemany(self, statement, parameters):
		def _encode(value):
			if isinstance(value, dict):
				return dict((self._encode(key), self._encode(value)) for (key, value) in value.iteritems())
			else:
				return map(self._encode, value)
		return self.executemany(self._encode(statement), map(_encode, parameters))

	def xfetchone(self, type=Record):
		row = self.fetchone()
		if row is not None:
			row = type.fromdata(self, row)
		return row

	def xfetchmany(self, rows=0, type=Record):
		return [type.fromdata(self, row) for row in self.fetchmany(rows)]

	def xfetchall(self, type=Record):
		return [type.fromdata(self, row) for row in self.fetchall()]

	def xfetch(self, type=Record):
		while True:
...
class MixinNormalDates(object):
	...
	def cdate(self, connection=None):
		...
		cursor.xexecute("select created from all_objects where lower(object_type)=:type and object_name=:name and owner=nvl(:owner, user)", type=self.__class__.type, name=self.name, owner=self.owner)
		row = cursor.xfetchone()
		if row is None:
	...
	def udate(self, connection=None):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select last_ddl_time from all_objects where lower(object_type)=:type and object_name=:name and owner=nvl(:owner, user)", type=self.__class__.type, name=self.name, owner=self.owner)
		row = cursor.xfetchone()
		if row is None:
			raise SQLObjectNotFoundError(self)
...
class MixinCodeDDL(object):
	...
	def createddl(self, connection=None, term=True):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select text from all_source where lower(type)=lower(:type) and owner=nvl(:owner, user) and name=:name order by line", type=self.__class__.type, owner=self.owner, name=self.name)
...
class Object(object):
	...
	def iterreferences(self, connection=None):
		...
		"""
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select referenced_type, decode(referenced_owner, user, null, referenced_owner) as referenced_owner, referenced_name from all_dependencies where type=upper(:type) and name=:name and owner=nvl(:owner, user) and type != 'NON-EXISTENT'", type=self.type, name=self.name, owner=self.owner)
		for rec in cursor.xfetchall():
			try:
	...
	def iterreferencedby(self, connection=None):
		...
		"""
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select type, decode(owner, user, null, owner) as owner, name from all_dependencies where referenced_type=upper(:type) and referenced_name=:name and referenced_owner=nvl(:owner, user) and type != 'NON-EXISTENT'", type=self.type, name=self.name, owner=self.owner)
		for rec in cursor.xfetchall():
			try:
	...
	def iterobjects(cls, connection, schema="user"):
		...
		if schema=="all":
			cursor.xexecute("select decode(owner, user, null, owner) as owner, object_name from all_objects where lower(object_type) = :type", type=cls.type)
		else:
			cursor.xexecute("select null as owner, object_name from user_objects where lower(object_type) = :type", type=cls.type)
		return (cls(row.object_name, row.owner, connection) for row in cursor.xfetch())
...
class Sequence(MixinNormalDates, Object):
	...
	def _createddl(self, connection, term, copyvalue):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select * from all_sequences where sequence_owner=nvl(:owner, user) and sequence_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		if rec is None:
			raise SQLObjectNotFoundError(self)
...
class Table(MixinNormalDates, Object):
	...
	def createddl(self, connection=None, term=True):
		...
		if self.ismview(connection):
			return ""
		cursor.xexecute("select * from all_tab_columns where owner=nvl(:owner, user) and table_name=:name order by column_id asc", owner=self.owner, name=self.name)
		recs = cursor.xfetchall()
		if not recs:
			raise SQLObjectNotFoundError(self)
		code = ["create table %s\n(\n" % self.getfullname()]
	...
	def mview(self, connection=None):
		...
		"""
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select mview_name from all_mviews where owner=nvl(:owner, user) and mview_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		if rec is not None:
	...
	def iterobjects(cls, connection, schema="user"):
		...
		if schema == "all":
			...
			cursor.xexecute("select null as owner, table_name from user_tables")
		return (cls(row.table_name, row.owner, connection=connection) for row in cursor.xfetch())
	...
	def itercolumns(self, connection=None):
		...
		"""
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select column_name from all_tab_columns where owner=nvl(:owner, user) and table_name=:name order by column_id", owner=self.owner, name=self.name)
	...
	def iterrecords(self, connection=None):
		...
		cursor.xexecute(query)
		return cursor.xfetch()
	...
	def iterconstraints(self, connection=None):
		...
		cursor.xexecute("select decode(owner, user, null, owner) as owner, constraint_type, constraint_name from all_constraints where constraint_type in ('P', 'U', 'R') and owner=nvl(:owner, user) and table_name=:name", owner=self.owner, name=self.name)
...
class PrimaryKey(Constraint):
	...
	def createddl(self, connection=None, term=True):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select decode(owner, user, null, owner) as owner, constraint_name, table_name, r_owner, r_constraint_name from all_constraints where constraint_type='P' and owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		rec2 = cursor.xfetchone()
		if rec2 is None:
			raise SQLObjectNotFoundError(self)
		cursor.xexecute("select column_name from all_cons_columns where owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		tablename = getfullname(rec2.table_name, rec2.owner)
		pkname = getfullname(self.name, None)
		code = "alter table %s add constraint %s primary key(%s)" % (tablename, pkname, ", ".join(r.column_name for r in cursor.xfetch()))
		if term:
			code += ";\n"
		else:
			code += "\n"
	...
	def dropddl(self, connection=None, term=True):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select decode(owner, user, null, owner) as owner, table_name from all_constraints where owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
	...
	def cdate(self, connection=None):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select last_change from all_constraints where constraint_type='P' and constraint_name=:name and owner=nvl(:owner, user)", name=self.name, owner=self.owner)
		row = cursor.fetchone()
		if row is None:
			raise SQLObjectNotFoundError(self)
		return None
	...
	def iterreferencedby(self, connection=None):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select decode(owner, user, null, owner) as owner, constraint_name from all_constraints where constraint_type='R' and r_owner=nvl(:owner, user) and r_constraint_name=:name", owner=self.owner, name=self.name)
		for rec in cursor.xfetchall():
			yield ForeignKey(rec.constraint_name, rec.owner, connection)

		cursor.xexecute("select decode(owner, user, null, owner) as owner, index_name from all_indexes where owner=nvl(:owner, user) and index_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone() # Is there an index for this constraint?
		if rec is not None:
			yield Index(rec.index_name, rec.owner, connection)

	def iterreferences(self, connection=None):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select decode(owner, user, null, owner) as owner, table_name from all_constraints where constraint_type='P' and owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		for rec in cursor.xfetchall():
	...
	def table(self, connection=None):
		...
		"""
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select table_name from all_constraints where constraint_type='P' and owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
...
class Comment(Object):
	...
	def createddl(self, connection=None, term=True):
		...
		cursor.xexecute("select comments from all_col_comments where owner=nvl(:owner, user) and table_name=:tname and column_name=:cname", owner=self.owner, tname=tcname[0], cname=tcname[1])
		row = cursor.xfetchone()
		if row is None:
			raise SQLObjectNotFoundError(self)
...
class ForeignKey(Constraint):
	...
	def createddl(self, connection=None, term=True):
		...
		cursor.xexecute("select decode(r_owner, user, null, r_owner) as r_owner, r_constraint_name, table_name from all_constraints where constraint_type='R' and owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		if rec is None:
			raise SQLObjectNotFoundError(self)
		cursor.xexecute("select column_name from all_cons_columns where owner=nvl(:owner, user) and constraint_name=:name order by position", owner=self.owner, name=self.name)
		fields1 = ", ".join(r.column_name for r in cursor.xfetch())
		cursor.xexecute("select table_name, column_name from all_cons_columns where owner=nvl(:owner, user) and constraint_name=:name order by position", owner=rec.r_owner, name=rec.r_constraint_name)
		fields2 = ", ".join("%s(%s)" % (getfullname(r.table_name, rec.r_owner), r.column_name) for r in cursor.xfetch())
	...
	def _ddl(self, connection, cmd, term):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select table_name from all_constraints where owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		if rec is None:
			raise SQLObjectNotFoundError(self)
		tablename = getfullname(rec.table_name, self.owner)
	...
	def cdate(self, connection=None):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select last_change from all_constraints where constraint_type='R' and constraint_name=:name and owner=nvl(:owner, user)", name=self.name, owner=self.owner)
		row = cursor.fetchone()
		if row is None:
			raise SQLObjectNotFoundError(self)
		return None
	...
	def table(self, connection=None):
		...
		"""
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select table_name from all_constraints where constraint_type='R' and owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		return Table(rec.table_name, self.owner, connection)
	...
	def pk(self, connection=None):
		...
		"""
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select decode(r_owner, user, null, r_owner) as r_owner, r_constraint_name from all_constraints where constraint_type='R' and owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
	...
	def isenabled(self, connection=None):
		...
		"""
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select status from all_constraints where constraint_type='R' and owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		rec = cursor.fetchone()
...
class Index(MixinNormalDates, Object):
	...
	def createddl(self, connection=None, term=True):
		...
		if self.isconstraint(connection):
			return ""
		cursor.xexecute("select index_name, table_name, uniqueness from all_indexes where table_owner=nvl(:owner, user) and index_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		if rec is None:
			raise SQLObjectNotFoundError(self)
		tablename = getfullname(rec.table_name, self.owner)
		...
		code = "create%s index %s on %s (%s)" % (unique, indexname, tablename, ", ".join(r.column_expression or r.column_name for r in cursor.xfetch()))
		if term:
			code += ";\n"
		else:
			code += "\n"
	...
	def constraint(self, connection=None):
		...
		"""
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select constraint_type from all_constraints where owner=nvl(:owner, user) and constraint_name=:name and constraint_type in ('U', 'P')", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		if rec is not None:
	...
	def table(self, connection=None):
		...
		"""
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select table_name from all_indexes where table_owner=nvl(:owner, user) and index_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		return Table(rec.table_name, self.owner, connection)
...
class UniqueConstraint(Constraint):
	...
	def createddl(self, connection=None, term=True):
		(connection, cursor) = self.getcursor(connection)
		# Add constraint_type to the query, so we don't pick up another constraint by accident
		cursor.xexecute("select table_name from all_constraints where constraint_type='U' and owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		if rec is None:
			raise SQLObjectNotFoundError(self)
		tablename = getfullname(rec.table_name, self.owner)
		...
		cursor.xexecute("select column_name from all_cons_columns where owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		code = "alter table %s add constraint %s unique(%s)" % (tablename, uniquename, ", ".join(r.column_name for r in cursor.xfetch()))
		if term:
			code += ";\n"
		else:
			code += "\n"
	...
	def cdate(self, connection=None):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select last_change from all_constraints where constraint_type='U' and constraint_name=:name and owner=nvl(:owner, user)", name=self.name, owner=self.owner)
		row = cursor.fetchone()
		if row is None:
			raise SQLObjectNotFoundError(self)
		return None
	...
	def iterreferencedby(self, connection=None):
		...
		cursor.xexecute("select decode(owner, user, null, owner) as owner, index_name from all_indexes where owner=nvl(:owner, user) and index_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone() # Ist there an index for this constraint?
		if rec is not None:
			yield Index(rec.index_name, rec.owner, connection)

	def iterreferences(self, connection=None):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select decode(owner, user, null, owner) as owner, table_name from all_constraints where constraint_type='U' and owner=nvl(:owner, user) and constraint_name=:name", owner=self.owner, name=self.name)
		for rec in cursor.xfetchall():
			yield Table(rec.table_name, rec.owner, connection)
...
class Synonym(Object):
	...
	def createddl(self, connection=None, term=True):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select table_owner, table_name, db_link from all_synonyms where owner=nvl(:owner, user) and synonym_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		if rec is None:
			raise SQLObjectNotFoundError(self)
		owner = self.owner
	...
	def getobject(self, connection=None):
		...
		if rec is None:
			raise SQLObjectNotFoundError(self)
		return connection.getobject(rec.table_name, rec.table_owner)
...
class View(MixinNormalDates, Object):
	...
	def createddl(self, connection=None, term=True):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select text from all_views where owner=nvl(:owner, user) and view_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		if rec is None:
			raise SQLObjectNotFoundError(self)
...
class MaterializedView(View):
	...
	def createddl(self, connection=None, term=True):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select * from all_mviews where owner=nvl(:owner, user) and mview_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		if rec is None:
			raise SQLObjectNotFoundError(self)
...
class Library(Object):
	...
	def createddl(self, connection=None, term=True):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select file_spec from all_libraries where owner=nvl(:owner, user) and library_name=:name", owner=self.owner, name=self.name)
		rec = cursor.xfetchone()
		if rec is None:
			raise SQLObjectNotFoundError(self)
...
class Callable(MixinNormalDates, MixinCodeDDL, Object):
	...
	def _calcargs(self, cursor):
		if self._argsbypos is None:
			cursor.xexecute("select object_id from all_objects where owner=nvl(:owner, user) and object_name=:name and object_type=:type", owner=self.owner, name=self.name, type=self.type.upper())
			if cursor.fetchone() is None:
				raise SQLObjectNotFoundError(self)
			self._argsbypos = []
			self._argsbyname = {}
			cursor.xexecute("select lower(argument_name) as name, lower(in_out) as in_out, lower(data_type) as datatype from all_arguments where owner=nvl(:owner, user) and object_name=:name and data_level=0 order by sequence", owner=self.owner, name=self.name)
			i = 0 # argument position (skip return value)
			for record in cursor.xfetch():
	...
	def _getparamarray(self, cursor, *args, **kwargs):
		...
		if la > lra:
			raise TypeError("too many parameters for %r: %d given, %d expected" % (self, la, lra))
		...
		for (key, value) in kwargs.iteritems():
			...
			realargs[arginfo.position] = cursor._encode(value)

		# Replace out parameters (and strings that are longer than the allowed
		# maximum) with variables; replace unspecified parameters with :const:`None`
		for arginfo in self._argsbypos:
			realarg = realargs[arginfo.position]
			if realarg is _default:
				realarg = None
				realargs[arginfo.position] = realarg
			if arginfo.isout or arginfo.datatype == "blob" or (isinstance(realarg, basestring) and len(realarg) >= 4000):
				try:
...
class Procedure(Callable):
	...
	def __call__(self, cursor, *args, **kwargs):
		...
		:var:`kwargs`. :var:`cursor` must be a :mod:`cx_Oracle` or
		:mod:`ll.orasql` cursor. This will return a :class:`Record` object
		containing the result of the call (i.e. this record will contain all in
		and out parameters).
		"""
		self._calcargs(cursor)
		...
		if self.owner is None:
			...
			name = "%s.%s" % (self.owner, self.name)
		name = cursor._encode(name)

		return Record((self._argsbypos[i].name, cursor._decode(value, self._argsbypos[i].datatype == "blob")) for (i, value) in enumerate(cursor.callproc(name, realargs)))
...
class Function(Callable):
	...
	def __call__(self, cursor, *args, **kwargs):
		...
		:var:`kwargs`. :var:`cursor` must be a :mod:`cx_Oracle` or
		:mod:`ll.orasql` cursor. This will return a tuple containing the result
		and a :class:`Record` object containing the modified parameters (i.e.
		this record will contain all in and out parameters).
		"""
		self._calcargs(cursor)
		...
		returnvalue = cursor._decode(cursor.callfunc(name, self._ora2cx[self._returnvalue.datatype], realargs), self._returnvalue.datatype == "blob")
		result = Record()
		for (i, value) in enumerate(realargs):
			arginfo = self._argsbypos[i]
			if arginfo.isout:
				value = value.getvalue(0)
			result[arginfo.name] = cursor._decode(value, arginfo.datatype == "blob")
...
class JavaSource(MixinNormalDates, Object):
	...
	def createddl(self, connection=None, term=True):
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select text from all_source where type='JAVA SOURCE' and owner=nvl(:owner, user) and name=:name order by line", owner=self.owner, name=self.name)
...
class Privilege(object):
	...
	def iterobjects(self, connection, schema="user"):
		...
		if schema == "all":
			cursor.xexecute("select decode(table_schema, user, null, table_schema) as owner, privilege, table_name as object, decode(grantor, user, null, grantor) as grantor, grantee from all_tab_privs order by table_schema, table_name, privilege")
		else:
			cursor.xexecute("select null as owner, privilege, table_name as object, decode(grantor, user, null, grantor) as grantor, grantee from user_tab_privs where owner=user order by table_name, privilege")
		return (Privilege(rec.privilege, rec.object, rec.grantor, rec.grantee, rec.owner, cursor.connection) for rec in cursor.xfetch())
...
class Column(Object):
	...
	def _getcolumnrecord(self, cursor):
		...
		cursor.xexecute("select * from all_tab_columns where owner=nvl(:owner, user) and table_name=:table_name and column_name=:column_name", owner=self.owner, table_name=name[0], column_name=name[1])
		rec = cursor.xfetchone()
		if rec is None:
			raise SQLObjectNotFoundError(self)
		return rec
	...
	def cdate(self, connection=None):
		...
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select created from all_objects where lower(object_type)='table' and object_name=:name and owner=nvl(:owner, user)", name=self.name.split(".")[0], owner=self.owner)
		row = cursor.fetchone()
		if row is None:
			raise SQLObjectNotFoundError(self)
		return row[0]
	...
	def udate(self, connection=None):
		...
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select last_ddl_time from all_objects where lower(object_type)='table' and object_name=:name and owner=nvl(:owner, user)", name=self.name.split(".")[0], owner=self.owner)
		row = cursor.fetchone()
		if row is None:
			raise SQLObjectNotFoundError(self)
		return row[0]
	...
	def comment(self, connection=None):
		...
		name = self.name.split(".")
		(connection, cursor) = self.getcursor(connection)
		cursor.xexecute("select comments from all_col_comments where owner=nvl(:owner, user) and table_name=:table_name and column_name=:column_name", owner=self.owner, table_name=name[0], column_name=name[1])
		rec = cursor.xfetchone()
		if rec is None:
			raise SQLObjectNotFoundError(self)
