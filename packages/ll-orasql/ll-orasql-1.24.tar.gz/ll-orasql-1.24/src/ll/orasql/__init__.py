...
class Connection(Connection):
	...
	def getobject(self, name, owner=None):
		...
		if "." in name:
			...
			"""
			query.execute(object_name=name, owner=owner)
	...
	def _calcargs(self, cursor):
		if self._argsbypos is None:
			if "." in self.name:
				...
				cursor.execute("select object_name from all_procedures where owner=nvl(:owner, user) and object_name=:name and procedure_name is null", owner=self.owner, name=name)
