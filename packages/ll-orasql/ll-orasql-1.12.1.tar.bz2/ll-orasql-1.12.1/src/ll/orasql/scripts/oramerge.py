...
def showaction(out, action, obj):
	out.writeln(comment(action, " ", df(obj)))
...
def main(args=None):
	...
	def fetch(connection):
		...
		for (i, obj) in enumerate(connection.iterobjects(mode="flat", schema="user")):
			...
			if options.verbose:
				msg = astyle.style_default("oramerge.py: ", cs(connection), " fetching #%d " % (i+1), df(obj))
				if not keep:
					msg += s4error(" (skipped)")
				stderr.writeln(msg)
			if keep:
	...
	for (i, obj) in enumerate(allobjects):
		...
		if in1 != in2: # ignore changes from in2 to in3, because only if something changed in the sources we have to do something
			if not in1 and in2 and not in3: # added in in2 => copy it to db3
				msg = astyle.style_default(df(obj), ": added in ", cs(connection2), " => add it to ", cs(connection3))
				action = "create"
			elif not in1 and in2 and in3: # added in both in2 and in3 => collision?
				if obj.createddl(connection2) != obj.createddl(connection3):
					msg = astyle.style_default(df(obj), ": added in ", cs(connection2), " and ", cs(connection3), " => ", s4error("conflict"))
					action = "collision"
					retcode = 2
				else:
					msg = astyle.style_default(df(obj), ": added in ", cs(connection2), " and ", cs(connection3), " and identical => keep it")
					action = None
			elif in1 and not in2 and not in3: # removed in in2 and in3 => not needed
				msg = astyle.style_default(df(obj), ": removed in ", cs(connection2), " and ", cs(connection3), " => not needed")
				action = None
			elif in1 and not in2 and in3: # removed in in2 => remove in db3
				msg = astyle.style_default(df(obj), ": removed in ", cs(connection2), " => remove in ", cs(connection3))
				action = "drop"
			else:
			...
			if options.verbose:
				stderr.writeln("oramerge.py: diffing #%d/%d " % (i+1, len(allobjects)), df(obj))
			...
			if ddl1 != ddl2: # ignore changes between ddl2 and ddl3 here too
				...
				if isinstance(obj, orasql.Table):
					...
					msg = astyle.style_default(df(obj), ": merge them")
					action = "merge"
			else:
				msg = astyle.style_default(df(obj), ": identical")
				action = None

		if action is not None:
			if action == "collision":
				showaction(stdout, action, obj)
				stdout.writeln(conflictmarker(7*"<", "added in ", cs(connection2), " and ", cs(connection3), " with different content"))
			elif action == "create":
				showaction(stdout, action, obj)
						...
						try:
							...
							if diffretcode == 0: # no conflict
								...
								if finalddl != ddl3 and (not finalddl.endswith("\n") or finalddl[:-1] != ddl3):
									showaction(stdout, "merge", obj)
									stdout.write(finalddl)
							elif diffretcode == 1: # conflict
								showaction(stdout, "conflict", obj)
								retcode = 2
