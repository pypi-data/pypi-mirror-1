...
# Version 1.1
#
# Copyright (C) 2003-2007 Antti Kervinen (ask@cs.tut.fi)
#
...
def _with_wrap(advice, method, instances=[]):
    ...
    try:
        ...
        # Replace the original method with generator_middleman
...
# ??? peel_wrap NOT YET RELEASED, UNIT TESTS MISSING
def _peel_wrap(method, wrap):
	...
	# method variable needs to be refreshed,
	# otherwise peeling the next wraps fails
...
def _find_wrap(method, wrap_number):
    ...
    while wrapid != wrap_number:
        if wrapid==None: raise AspectsException("Wrap %s on method %s not found."
