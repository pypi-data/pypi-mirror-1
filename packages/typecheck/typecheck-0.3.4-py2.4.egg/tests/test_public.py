...
class Test_typecheck_args(TestCase):
	...
	def test_failure_unpacked_tuples_2(self):
		...
		try:
			...
			self.assertEqual(str(e), "Argument (b, (c, d)): for (6, 4), at index 1, expected (<type 'int'>, <type 'int'>), got <type 'int'>")
		else:
			raise AssertionError("Succeeded incorrectly")	
...
class TypeVariableTests(TestCase):
	...
	def test_args_fail(self):
		from typecheck import typecheck_args, calculate_type
	...
	def test_return_fail(self):
		from typecheck import typecheck_return, calculate_type
		from typecheck import TypeCheckError, _TC_IndexError, _TC_TypeError
		from typecheck import TypeVariables
