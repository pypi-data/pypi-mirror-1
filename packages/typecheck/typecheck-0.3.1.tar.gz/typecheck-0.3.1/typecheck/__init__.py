...
class _TC_Exception(Exception):
	...
	def format_bad_object(self, bad_object):
		return ("in %s, " % str(bad_object), self)
...
class _TC_DictError(_TC_NestedError):
	def format_bad_object(self, bad_object):
		message = "in {%s}, " % ', '.join(["%s: %s" % (repr(k), repr(bad_object[k])) for k in sorted(bad_object.keys())])
...
class CheckType(object):
	...
	def __hash__(self):
		raise NotImplementedError("Incomplete CheckType subclass")
...
class Single(CheckType):
	...
	def __init__(self, type):
		# XXX: Is this first test correct?
		# Shouldn't it be isinstance(type, Single)?
...
### Provide a way to enforce the empty-ness of lists and dicts
### That is, Empty(list) matches only []'s, and Empty(dict)
### only approves {}'s		
class Empty(Single):
	name = "Empty"
...
class List(CheckType):
	...
	def __hash__(self):
		return hash(str(hash(self.__class__)) + str([hash(t) for t in self.type]))
...
for c in (CheckType, List, Tuple, Dict, Single, TypeVariables):
	register_type(c)
...
class _Boolean(CheckType):
	def __init__(self, type, *types):
		self._types = set([Type(t) for t in (type,)+types])
		self.type = self
	...
	def __eq__(self, other):
		...
		return self._types.issubset(other._types)
...
def Self():
	return 'this is the class of the invocant'
...
def _analyse_params(pos_names, kw_names, vargs, kwargs, handle_missing_kw, tuplizer):
	...
	# names is a dictionary at this point, but we'd like
	# it to be a tuple, s'il vous plait
	reconstructed = []
	for name in pos_names:
		if isinstance(name, list):
			name = _tuplize(name)
...
def _make_fake_function(func):
	...
	def _pass_args(*vargs, **kwargs): pass
	...
	for attr in ('args_pos', 'args_kw', 'args_kwargs', 'args_vargs', 'yield', 'return'):
		attr = 'type_%s' % attr
		if hasattr(func, attr):
			setattr(fake_function, attr, getattr(func, attr))
...
def typecheck_args(*v_sig, **kw_sig):
	...
	def decorator(func):
		...
		# and inform the user
		try:
			types, kw_types, t_vargs, t_kwargs = _analyse_params(pos_names, kw_names, v_sig, kw_sig, _types__missing_kw, _tuplize_and_check)
		...
		if kwarg_name is not None:
			...
			for k in t_kwargs:
				if k in kw_types:
					...
					raise TypeSignatureError(_TS_TwiceTypedError(param, t_kwargs[param], kw_types[param]))
		...
		T_kw_types = dict([(k, Type(v)) for (k, v) in kw_types.items()])
		T_types = Type(types)
		T_t_vargs = Type(t_vargs)
		T_t_kwargs = Type(t_kwargs)
		...
		def __check_args(*__vargs, **__kwargs):
			...
			if enable_checking:
				args, kw_args, a_vargs, a_kwargs = _analyse_params(pos_names, kw_names, __vargs, __kwargs, _args__handle_missing, _tuplize)

				# Type-check the keyword arguments
				for (name, val) in kw_args.items():
					type = T_kw_types[name]
					try:
						check_type(type, wrapped_func, val)
					except _TC_Exception, e:
						middle_exc = _TC_KeyValError(name, val, e)
						raise TypeCheckError(kw_args, middle_exc)

				# Type-check everything else
				for (arg, type) in ((args, T_types), (a_vargs, T_t_vargs), (a_kwargs, T_t_kwargs)):
					try:
						check_type(type, wrapped_func, arg)
					except _TC_Exception, e:
		...
		fake_function.type_args_pos = types
		fake_function.type_args_kw = kw_types
		
		# Adjust these so that they're consistent
		for (attr, var) in (('vargs', t_vargs), ('kwargs', t_kwargs)):
			if isinstance(var, Empty):
				var = var.type()
			setattr(fake_function, 'type_args_%s' % attr, var)
		return fake_function
	return decorator
