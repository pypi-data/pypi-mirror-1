...
================================================================================
	...
	The same function in other modules (currently diatonic_keys) will
...
# note_dict is a mapping of the C scale to half notes. It is used to calculate 
...
def get_enharmonic(note, new_note_base, tokens = -1):
	"""Returns a note enharmonic to `note`, but with base new_note_base.
		tokens = 1: generate b's to create enharmonic
		tokens = 0: generate #'s to create enharmonic
		tokens = -1: use the one that's shortest"""
	pass

def is_valid_note(note):
	"""Returns true if note is in a recognised format. False if not"""
	if not(note_dict.has_key(note[0])):
		return False
	for post in note[1:]:
		if post != 'b' and post != '#':
			return False
...
def diminish(note):
	...
	Examples: 
		diminish("C") returns  "Cb"
		diminish("C#") returns "C" """
	if note[-1] != '#':
		return note + 'b'
	else:
		return note[:-1]
