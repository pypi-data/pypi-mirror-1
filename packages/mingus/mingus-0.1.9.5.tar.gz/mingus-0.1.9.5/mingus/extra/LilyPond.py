...
def from_Note(note, process_octaves = True):
	"""Expects a Note object and returns the LilyPond equivalent
	in a string"""
	# Throw exception
...
def from_NoteContainer(nc, duration = None):
	"""Expects a NoteContainer object and returns the LilyPond 
	equivalent in a string. The second argument determining the 
	duration of the NoteContainer is optional."""
...
def from_Bar(bar, showkey = True, showtime = True):
	"""Expects a Bar object and returns the LilyPond equivalent
	in a string. showkey and showtime can be set to determine whether
...
def from_Track(track):
	"""Processes a Track object and returns the Lilypond equivalent
	in a string."""
	# Throw exception
...
def from_Composition(composition):
	"""Get's the LilyPond equivalent of a Composition in a string"""
	# Throw exception
	if not ( hasattr ( composition, "tracks" ) ):
		return False

	result = "\\version 2.10.33 "
