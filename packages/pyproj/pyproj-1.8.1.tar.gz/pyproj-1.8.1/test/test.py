...
x = llcrnrx+dx*N.indices((ny,nx),'d')[1,:,:]
y = llcrnry+dy*N.indices((ny,nx),'d')[0,:,:]
t1 = time.clock()
lons, lats = awips221(x.flatten().tolist(), y.flatten().tolist(), inverse=True)
t2 = time.clock()
print 'data in lists:'
...
xa = array.array('d',x.flatten().tolist())
ya = array.array('d',y.flatten().tolist())
t1 = time.clock()
...
print N.fabs(x-xx).min(),N.fabs(x-xx).max()
print 'max abs error for y'
print N.fabs(y-yy).min(),N.fabs(y-yy).max()
print 'took',t2-t1,'secs'
