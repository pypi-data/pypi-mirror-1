...
class Project(AnyEntity):
    ...
    def versions_in_state(self, states, order='ASC', limit=False):
        ...
        return self.req.execute(rql, {'p': self.eid})

    # number of columns to display
    tickets_rql_nb_displayed_cols = 7
    sort_defs = (('in_state', 'S'), ('num', 'VN'), ('priority', 'PR'))
    def tickets_rql(self, limit=None):
        rql = ('Any B, NOW - CD, NOW - BMD, U,PR,S,V,BD,VN,P,BT,CD,BMD,UL '
               'GROUPBY B,CD,PR,S,V,U,VN,BD,P,BT,BMD,UL %s WHERE '
               'B priority PR, B in_state S, B creation_date CD, '
               'B description BD, '
