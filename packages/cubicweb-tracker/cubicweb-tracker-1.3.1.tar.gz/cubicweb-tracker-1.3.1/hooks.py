...
class CheckProjectCyclesHook(hooksmanager.Hook):
    events = ('before_add_relation',)
    accepts = ('subproject_of',)
    def call(self, session, childeid, rtype, parenteid):
        # detect cycles
        proj = session.entity_from_eid(childeid)
        for child in transitive_closure_of(proj, 'reverse_subproject_of'):
            if parenteid == child.eid:
                msg = session._('you cannot link these entities, they would form a cycle')
                raise ValidationError(parenteid, {'child' : msg})
...
class BeforeInVersionChangeHook(hooksmanager.Hook):
    ...
    def call(self, session, fromeid, rtype, toeid):
        ...
        if oldversion != newversion:
            ...
            notification.EntityUpdatedNotificationOp(session)
