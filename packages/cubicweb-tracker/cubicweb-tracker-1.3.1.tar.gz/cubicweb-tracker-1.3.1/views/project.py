...
class ProjectPrimaryView(tabs.TabbedPrimaryView):
    ...
    # XXX remove when 3.6 is out
    def render_entity_title(self, entity):
        self.render_entity_toolbox(entity)
        super(ProjectPrimaryView, self).render_entity_title(entity)
...
class ProjectTreeVComponent(component.EntityVComponent):
    ...
    def cell_call(self, row, col, view=None):
        ...
        if rset:
            self.w(tags.h2(self.req._('Project tree')))
...
class SubscribeToReleasesComponent(component.EntityVComponent):
    ...
    def cell_call(self, row, col, view=None):
        entity = self.rset.get_entity(row, col)
        label = self.req._(u'Subscribe to project releases')
        logo = u'<img src="%s" alt="%s"/>' % (
            self.req.external_resource('RSS_LOGO_16'), label)
        ...
        url = self.build_url('view', vid='rss', rql=rql)
