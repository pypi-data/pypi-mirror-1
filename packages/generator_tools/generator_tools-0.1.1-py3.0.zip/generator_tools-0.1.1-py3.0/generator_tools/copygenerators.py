...
__all__ = ["GeneratorSnapshot", "CodeObj", "CopyGeneratorException", "copy_generator", "IteratorView", "for_iter", "generatorcopy"]
...
class CodeObj(object):
    ...
    def __setstate__(self, dct):
        ...
        self.co_code = str8(bytes(ord(c) for c in self.co_code))
...
class IteratorView(object):
    ...
    def __next__(self):
        item = self.itwrap.__next__(self)
...
def copy_generator(f_gen):
    ...
    if offset:
        ...
        offset += offset_inc
        modified_code = str8(bytes(bcm.create_new_bytecode(offset, argcount, opcodes)))
    else:
    ...
    new_code = new.code(argcount,
             ...
             modified_code,
             f_code.co_consts,
             f_code.co_names+("jump_forward_on", "jump_forward_off"),
             tuple(varnames),
             str8(f_code.co_filename),    # deserialization trustworthy?
             str(f_code.co_name),         # deserialization trustworthy?
             f_code.co_firstlineno,
             str8(f_code.co_lnotab))      # deserialization trustworthy?
