...
def daemonize(pid, cd, chroot, umask, files_preserve=[], do_open=True):
    """
...
def start_server(pid, FORCE, chroot, cd, uid, gid, umask, settings_loader):
    check_for_pid(pid, FORCE)

    daemonize(pid, cd, chroot, umask, files_preserve=[])
