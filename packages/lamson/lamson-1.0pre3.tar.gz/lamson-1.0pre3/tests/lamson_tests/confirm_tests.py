...
def test_ConfirmationStorage():
    storage.store('testing', 'zedshaw@zedshaw.com',
                  '12345', '567890')
    secret, pending_id = storage.get('testing', 'zedshaw@zedshaw.com')
    assert_equal(secret, '12345')
    assert_equal(pending_id, '567890')

    storage.delete('testing', 'zedshaw@zedshaw.com')
...
def test_ConfirmationEngine_send():
    ...
    message = mail.MailRequest('fakepeer', 'zedshaw@zedshaw.com',
                               'testing-subscribe@test.com', 'Fake body.')

    engine.send(relay(port=8899), 'testing', message, 'confirmation.msg', locals())
   
    confirm = delivered('confirm')
    assert delivered('zedshaw', to_queue=engine.pending)
    assert confirm
...
def test_ConfirmationEngine_verify():
    ...
    resp = mail.MailRequest('fakepeer', 'zedshaw@zedshaw.com',
    ...
    assert_equal(pending['from'], 'zedshaw@zedshaw.com')
    assert_equal(pending['to'], 'testing-subscribe@test.com')
