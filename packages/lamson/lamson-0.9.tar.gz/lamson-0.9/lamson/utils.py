...
def import_settings(boot_also, from_dir=None):
...
def daemonize(options, files_preserve):
    """
    ...
    context.pidfile=pidlockfile.PIDLockFile(options["pid"])
    context.stdout=open("logs/lamson.out","a+")
    context.stderr=open("logs/lamson.err","a+")
    context.files_preserve = files_preserve
    context.working_directory=os.path.expanduser(options["cd"])
    
    if options['chroot']: 
        context.chroot_directory = os.path.expanduser(options['chroot'])
    if options['uid']: 
        context.uid = options['uid']
    if options['gid']: 
        context.gid = options['gid']
    if options['umask']:
        context.umask = options['umask']
...
def make_fake_settings(options):
    """
    ...
    settings.receiver = server.SMTPReceiver(options["host"], options["port"])
    settings.relay = None
