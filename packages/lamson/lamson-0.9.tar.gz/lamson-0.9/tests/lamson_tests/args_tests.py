...
def test_defaults():
    ...
    args.defaults(options, num=2, help=True, stuff=None)
    assert options['help'] == True
    assert options['num'] == 1
   
    command, options = args.parse(['test', '--num', '1', '--help', '--stuff', 'The remainder.'])
    args.defaults(options, num=2, extras=3, help=None, stuff=None)
    assert options['extras'] == 3
    assert options['num'] == 1

    try:
        args.defaults(options, num=2, extras=3, help=None, bad=None)
        assert False, "Oops, no exception."
    except RuntimeError:
        pass
