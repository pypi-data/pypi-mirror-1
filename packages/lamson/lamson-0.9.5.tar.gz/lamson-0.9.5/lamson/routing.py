...
class RoutingBase(object):
    ...
    def get_state(self, module_name, message):
        ...
        return self.STATE_STORE.get(module_name, address)
    ...
    def set_state(self, module_name, message, state):
        ...
        """
        _, address = email.utils.parseaddr(message['from'])
        self.STATE_STORE.set(module_name, address, state)
    ...
    def __call__(self, func):
        ...
        if lamson_setting(func, 'stateless'):
            ...
            def routing_wrapper(message, *args, **kw):
                ...
                if next_state:
                    Router.set_state(func.__module__, message, next_state.__name__)
