...
def test_craft_from_sample():
    list_name = "test.list"
    user_full_address = "tester@test.com"

    sample = mail.MailResponse(To=list_name + "@librelist.com",
...
def test_route_to_from_works():
    msg = mail.MailRequest("fakepeer", "from@fromdude.com",
                                   [u"<to1@to.com>", u"to2@to.com"], "")
...
def test_decode_header_randomness():
    ...
    assert_equal(mail._decode_header_randomness(["z@a.com", '"Z A" <z@j.com>']), 
                 set(["z@a.com", "z@j.com"]))
    assert_equal(mail._decode_header_randomness("z@a.com"), set(["z@a.com"]))
