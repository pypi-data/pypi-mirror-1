...
class MemoryStorage(StateStorage):
    ...
    def get(self, key, sender):
        ...
        try:
            ...
            self.set(key, sender, ROUTE_FIRST_STATE)
...
class ShelveStorage(MemoryStorage):
    ...
    def set(self, key, sender, state):
        ...
        with self.lock:
            store = shelve.open(self.database_path)
            store[self.key(key, sender)] = state
            store.close()
...
class RoutingBase(object):
    ...
    def register_route(self, format, func):
        ...
        with self.lock:
            if format in self.REGISTERED:
                ...
                self.REGISTERED[format] = (re.compile(format), [func])
    ...
    def get_state(self, module_name, message):
        ...
        _, address = email.utils.parseaddr(message['from'])
        key = self.state_key(module_name, message)
        return self.STATE_STORE.get(key, address)
    ...
    def set_state(self, module_name, message, state):
        ...
        _, address = email.utils.parseaddr(message['from'])
        key = self.state_key(module_name, message)
        self.STATE_STORE.set(key, address, state)

    def _collect_matches(self, message, route_on):
        in_state_found = False

        for functions, matchkw in self.match(route_on):
    ...
    def deliver(self, message):
        ...
        for func, matchkw in self._collect_matches(message, message.To):
    ...
    def call_safely(self, func, message, kwargs):
        ...
        try:
            ...
            if self.UNDELIVERABLE_QUEUE: self.UNDELIVERABLE_QUEUE.push(message)
