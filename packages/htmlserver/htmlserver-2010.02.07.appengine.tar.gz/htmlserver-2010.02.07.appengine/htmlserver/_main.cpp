...
#define NUMBYTE_SWITCH(tcode, check, cc, ii, ff, _) switch (tcode) {    \
    ...
    // template<typename TT> struct numbyte: PyObject, std::iterator<std::random_access_iterator_tag, TT> { int tcode; PyObject *bytes; TT *offset; int shape0, shape1, stride0, stride1;
...
#include <iterator>
  namespace TYPE {
    template<typename TT> struct iterator: std::iterator<std::random_access_iterator_tag, TT> { TT *words; int stride0, stride1, i0, i1;
      ~iterator() {}
      iterator(TT *_words, int _stride0, int _stride1, int _i0 = 0, int _i1 = 0) { words = _words; stride0 = _stride0; stride1 = _stride1; i0 = _i0; i1 = _i1; }
      // iterator(iterator &other) { words = other.words; stride0 = other.stride0; stride1 = other.stride1; i0 = other.i0; i1 = other.i1; }
      int offset() { return i0 * stride0 + i1 * stride1; }
      TT *ptr() { return words + offset; }
      TT &operator*() { return *ptr(); }
      bool operator==(iterator &other) { return ptr() == other.ptr(); }
      bool operator!=(iterator &other) { return ptr() != other.ptr(); }
      iterator &operator++() { ++ i1; if (i1 >= stride1) { ++ i0; i1 =           0; } return *this; }
      iterator &operator--() { -- i1; if (i1 <        0) { -- i0; i1 = stride1 - 1; } return *this; }
      iterator &operator+=(int ii) {
        i1 += ii;
        if      (i1 >= stride1) { ++ i0; i1 =           0; }
        else if (i1 <        0) { -- i0; i1 = stride1 - 1; }
        return *this;
      }
      iterator &operator-=(int ii) { return (*this) += -ii; }
      iterator operator+(int ii) { iterator tmp(*this); return tmp += ii; }
      iterator operator-(int ii) { return (*this) + -ii; }
    };
    //// init
    void init() {
      iterator<double> aa(NULL, 8, 8, 0, 1);
      iterator<double> bb = aa;
      ++ aa;
      // PY::print("\nhello %p %p %i %i\n\n", &aa, &bb, aa.i1, bb.i1);
