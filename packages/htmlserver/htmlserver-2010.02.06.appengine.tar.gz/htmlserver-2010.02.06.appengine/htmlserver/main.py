...
if 0: ######## INIT
  ...
  class PseudoSugar(ast.NodeVisitor, importlib.abc.Finder, importlib.abc.PyLoader):
    ...
    # @staticmethod
    # def unparse_str(ss): return ss.replace(',__postfixop4__.', '>>>>').replace('.__prefixop4__,', '<<<<').replace('()<<<<', '@@').replace('.__pseudometh4__.', '....').replace('.__pseudometh3__.', '...').replace('.__pseudometh2__.', '..')
    ...
    # def compile(ss, fpath, mode, flags = 0, dont_inherit = 0):
      # try:
        # if not PseudoSugar.magic.search(ss): return builtins.__pseudocompile__(ss, fpath, mode, flags, dont_inherit)
    ...
    def compile(ss, fpath, mode, flags = 0, dont_inherit = 0):
      try:
        if not PseudoSugar.hasmagic(ss): return builtins.__pseudocompile__(ss, fpath, mode, flags, dont_inherit)
      except SyntaxError: pass
      node = PseudoSugar().parse_ast(ss, fpath, mode)
      if flags & ast.PyCF_ONLY_AST: return node
      return builtins.__pseudocompile__(node, fpath, mode, flags, dont_inherit)
...
## END
  ...
  class Py3to2(Ast2Src):
    ...
    # name25 = dict(aa.split(':') for aa in 'print:__print25__'.split(' '))

    # def visit(self, node, ss = ''):
      # type = node.__class__
      # # ## pre
      # # if type is ast.ExceptHandler: ss = 'except {}{}:{}' ..self.format(node.type if node.type else '', '({})'.format(node.name) if node.name else '', node.body, indent = 1)
      # ss = super().visit(node, ss)
      # ## post
      # if type is ast.Call:
        # if isinstance(node.func, ast.Name) and node.func.id == 'print': ss = ss.replace('(', ' ', 1) ..sreverse().replace(')', '', 1) ..sreverse() ## print statement
      # # if type is ast.Call:
        # # # if super():
        # # # if self.visit(node.func) == 'print': return super().visit(node).replace('(', ' ', 1) ..sreverse().replace(')', '', 1) ..sreverse() ## print statement
      # # return super().visit(node, ss)
      # # elif type is ast.ExceptHandler: ss = 'except {}{}:{}' ..self.format(node.type if node.type else '', '({})'.format(node.name) if node.name else '', node.body, indent = 1)
      # return ss

    # def visit_pre(self, node):
      # ss = ''; type = node.__class__
      # if type is ast.Call:
        # if isinstance(node.func, ast.Name) and node.func.id == 'print':
          # # ss = ss.replace('(', ' ', 1) ..sreverse().replace(')', '', 1) ..sreverse() ## print statement
          # args = list(self.generic_visit(node.args + node.keywords))
          # if node.starargs: args.append('*' + self.visit(node.starargs))
          # if node.kwargs: args.append('**' + self.visit(node.kwargs))
  ...
  class Distribution(distutils.dist.Distribution):
    ...
    class HtmlServer(Dev):
      ...
      user_options = [('name=', None, 'server name'),
                      ...
                      # ('google-appengine-path=', None, 'google_appengine path'),
                      ('serve=', None, ''),
                     ]
    ...
    def run_dev(self):
      ...
      # if self.htmlserver: import htmlserver.htmlserver; htmlserver.htmlserver.serve_forever(self.htmlserver); exit()
    ...
    # def run_html(self):
      ...
      # htmlserver.name = self.name
      # htmlserver.serve_forever(); exit()
    ...
    # def append_pkginfo(self):
      # if 'htmlserver' == 'htmlserver':
        # # cmd = 'python3.1 setup.py htmlserver --name=myserver\n'
        # # print@@ system( cmd )
        # # # self.metadata.long_description += cmd + system( cmd )
        # cmd = 'python3.1 setup.py htmlserver --name=myserver --serve=0'
        # # print@@ system( cmd )
        # self.metadata.long_description += '\n$ {}\n{}'.format@@ cmd, system( cmd + ' 2>&1', debug = None )
      # else: self.metadata.long_description += system( 'python3.1 -c "import htmlserver; htmlserver.quicktest()"' )
      # self.metadata.write_pkg_file(open('README', 'w')); pydoc.pager(open('README').read())

    def post_install(self, cmd, **kwds): dpath = cmd.install_lib + 'htmlserver/'; self.get_command_obj('build_ext').force = True; self.run_command('build_ext', rerun = True, install_path = dpath)
...
def quicktest(type = None, pkginfo = None):
  ...
  # if type == 'htmlserver':
    # # print@@ '''python3.1 setup.py htmlserver --name=myserver
    # # '''
    # cmd = 'python3.1 setup.py htmlserver --name=myserver --serve=0'
    # # print@@ system( cmd )
    # print@@ '\n$ {}\n{}'.format@@ cmd, system( cmd + ' 2>&1', debug = None )
  ...
  # if type == 'htmlserver':
    # cmd = 'python3.1 setup.py htmlserver --name=myserver'
    # print@@ '\n$ {}\n{}'.format@@ cmd, system( cmd + (' --serve=0 2>&1' if pkginfo else ''), debug = None )
    # return
  # if type == 'htmlserver': return 
    # cmd = 'python3.1 setup.py htmlserver --name=myserver'
    # print@@ '\n$ {}\n{}'.format@@ cmd, system( cmd + (' --serve=0 2>&1' if pkginfo else ''), debug = None )
    # return
