...
class subscribe(object):
    ...
    def subscribe(self, name):
        """
        Subscribe this callable to another event.
        """
...
class Handler(object):
    """
    A handler is different, in that it does not recieve dispatched events, 
    but only events which are posted directly to it, via its own post method.
    """
    def post(self, _event_identity, **kw):
        return tuple([baseevent.call_with_keywords(getattr(self, meth), kw) for meth in dir(self) if HANDLER_PREFIX+_event_identity == meth])
...
if __name__ == "__main__":
    ...
    def ev(name, **kw):
        print "EVHOOK", name, kw

    def fv(name, **kw):
        print "FVHOOK", name, kw
