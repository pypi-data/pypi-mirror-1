...
class Fibra(object):
    ...
    def preempt(self):
        """
        Preempt the schedule of the Fibra, moving it to the top of the 
        execution pool.
        """
...
class Pool(object):
    ...
    def register(self, func, *args, **kw):
        """
        Creates a generator from func, and installs it into the pool. 
        Returns a fibra instance, which can control the task.
        """
        generator = func(*args, **kw)
        return self.install(generator)
    ...
    def poll(self):
        ...
        """
        if len(self.pool)  or (self.wait_count > 0):
            try:
                if len(self.pool) > 0:
                    ...
                    if  isinstance(r, UNBLOCK):
                        ...
                        task._state.appendleft('SLEEPING')
                        self._resume_later(r.seconds, task)
                    else:
    ...
    def shutdown(self):
        ...
        """
        for task in self.pool: task.kill()
    ...
    def _resume_later(self, seconds, task):
        def resume_task():
            self.pool.append(task)
            task._state.popleft()
