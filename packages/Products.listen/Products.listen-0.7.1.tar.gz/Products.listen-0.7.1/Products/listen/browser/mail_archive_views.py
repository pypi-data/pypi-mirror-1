...
class ArchiveForumView(ArchiveBaseView):
    ...
    def Title(self):
        title = _('label_archive_date_view', u'Forum view for ${list}')
    ...
    def getTopics(self):
        ...
        topic_list = []
        getToplevelMessages = self.search.getToplevelMessages
        getMessageReferrers = self.search.getMessageReferrers
        messages = getToplevelMessages(sort_on='modification_date',
                                       filter_on='modification_date', reversed=True)
...
class ArchiveDateView(ArchiveBaseView):
    ...
    def Title(self):
        title = _('label_archive_date_view', u'Archive by date for ${title}')
...
class SubFolderDateView(ArchiveBaseView):
    ...
    def Title(self):
        title = _('label_archive_date_view', u'${list} archive for ${date}')
        title = Message(title, mapping={ u'list': self.listTitle(),
...
class ArchiveNewTopicView(ArchiveBaseView):
    ...
    def __call__(self):
        ...
        if not self.member_address():
            # User does not have an address set, forward to personal prefs
            pt = getToolByName(self.context, 'portal_url')
            plone_utils = getToolByName(self.context, 'plone_utils')
            plone_utils.addPortalMessage("You must set an e-mail address to post a message to the list.")
            return self.request.response.redirect("%s/personalize_form" % pt())
    ...
    invalid_name_re = re.compile("[^-a-zA-Z_0-9. ']")
    def member_address(self):
        ...
        if name:
            #we need to RFC822-ify name.  There are a bunch of possible complicated
            #ways to do this, but I think a simple one will do:
            name = self.invalid_name_re.sub('', name)
...
class ArchiveSearchView(ArchiveBaseView):
    ...
    def Title(self):
        search_text = self.request.form.get('search_text', None)
        if search_text:
            title = _('label_archive_search', u'Search results for "${search_text}"')
            title = Message(title, mapping={u'search_text': search_text}) 
        else:
            title = _('label_archive_search', u'Search results')
        return title
        ...
        if text:
            ...
            try:
                ...
                if "Token 'ATOM' required, 'EOF' found" in str(inst) or "Token 'EOF' required" in str(inst):
                    self.request.set('portal_status_message', 'Invalid Search: Search phrase cannot end with "and", "or", or "not"')
                elif "Token 'ATOM' required" in str(inst):
                    self.request.set('portal_status_message', 'Invalid Search: Search phrase cannot begin with "and", "or", or "not"')
                elif "a term must have at least one positive word" in str(inst):
                    self.request.set('portal_status_message', 'Invalid Search: Search phrase cannot begin with "not"')
                elif "Query contains only common words" in str(inst):
                    self.request.set('portal_status_message', 'Invalid Search: Search phrase must contain words other than "and", "or", and "not"')
            else:
                messages = catalogMessageIterator(messages, sub_mgr=subscription)
                if len(messages) == 0:
                    self.request.set('portal_status_message', 'There were no messages found')
        if batch:
            messages = Batch(messages, batch_size, batch_start)
