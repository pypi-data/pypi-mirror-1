...
"""
base MailingList class definition
"""
...
email_from_subject_regex = re.compile('.*\<\<([^>]*)\>\>.*')


def addMailingList(self, id, title=u''):
    """ Add a Mailing List """
    ml = MailingList(id, title)
    self._setObject(id,ml)
    # ensure creator of mailing list is also a manager
    mstool = getToolByName(self, 'portal_membership')
    cur_user_id = mstool.getAuthenticatedMember().getId()
    cur_user_id = decode(cur_user_id, self)
    # only allow users with email addresses to be placed on the manager list
    try:
        cur_email = to_email(cur_user_id)[0]
    except IndexError:
        cur_email = None
    if cur_email:
        ml.managers = (cur_user_id,)
        assign_local_role('Owner', (cur_user_id,), IRoleManager(ml))
        # and make the creator an allowed sender too
        mem_list = IWriteMembershipList(ml)
        mem_list.add_allowed_sender(cur_user_id)


class MailingList(DynamicType, CMFCatalogAware, MailBoxerMailingList, FiveSite, DefaultDublinCoreImpl):
    ...
    Try again, but this time have the response email from: header differ from that
    specified in the subject header to make sure we use the right one.
        >>> request = {}
        >>> request['Mail'] = '''To: list1@example.com
        ... From: test_alias1@example.com
        ... Subject: subscribe
        ... Date: Wed, 5 Mar 2005 12:00:00 -0000
        ... '''
        >>> sent_emails = []
        ...
        True
        >>> request['Mail'] = '''To: list1@example.com
        ... From: test_alias2@example.com
        ... Subject: %s
        ... Date: Wed, 5 Mar 2005 12:00:00 -0000
        ... ''' % sent_emails[0][1]
        >>> test_list.requestMail(request)
        True
        >>> 'You are now subscribed' in sent_emails[1][2]
        True
    ...
    Another unsubcribe request on a public list, but w/ different from address
    different than subject address
        >>> request = {}
        >>> request['Mail'] = '''To: list1@example.com
        ... From: test_alias1@example.com
        ... Subject: unsubscribe
        ...
        >>> test_list.requestMail(request)
        True
        >>> 'You have been unsubscribed' in sent_emails[1][2]
        True
    ...
    def requestMail(self, request):
        ...
        (name, from_email) = self.parseaddr(sender)
        # we want to use the email address that comes from the subject
        # header, since we know that will match the original request,
        # whereas the address in the from header might be different
        match = email_from_subject_regex.search(subject)
        if match is None:
            # BBB: might be a confirmation from before we were using
            # the <<email_address>> delimiter
            match = email_regex.search(subject)
            if match is None:
                # we really couldn't find the address in the subject,
                # use the from: header
                req_email = from_email
        if match is not None:
            req_email = match.group(1)
        policy_result = sub_policy.enforce({'email':req_email, 'name':name,
                                            'subject':subject})
        ...
        if policy_result == MEMBERSHIP_ALLOWED:
            if 'unsubscribe' in subject:
                sub_list.unsubscribe(req_email)
                mail_sender.user_unsubscribe_confirm(req_email, name)
            elif 'subscribe' in subject:
                sub_list.subscribe(req_email)
                mail_sender.user_welcome(req_email, name)
        elif policy_result == MEMBERSHIP_PIN_MISMATCH:
            # send email to reject pin
            mail_sender.user_pin_mismatch(req_email, name)
        elif policy_result == MEMBERSHIP_DENIED:
            # check to see if the user has subscribed in the mean time
            if not sub_list.is_subscribed(req_email):
                # send email to say was denied, invalid confirmation, no record of request
                mail_sender.user_denied(req_email, name)
    ...
    def listMail(self, request):
        ...
        if emailmsg.is_multipart():
            ...
            for part in emailmsg.walk():
                if part.get_content_maintype().lower() == 'multipart' and \
                         part.get_content_subtype().lower() == 'alternative':
                    ...
                    break
                elif part.get_content_maintype().lower() == 'text':
                    if part.get_content_subtype().lower() == 'plain':
                        if textbody is None:
                            textbody = part
                            if alternatives is None:
                                break
                    elif part.get_content_subtype().lower() == 'html':
                        htmlbody = part
                    if alternatives is None:
            ...
            if emailmsg.get_content_maintype() is None:
                # no Content-Type specified, assume plain text
                textbody = emailmsg
            elif emailmsg.get_content_maintype().lower() == 'text':
                if emailmsg.get_content_subtype().lower() == 'plain':
                    textbody = emailmsg
                elif emailmsg.get_content_subtype().lower() == 'html':
                    htmlbody = emailmsg
    ...
    def has_moderation(self):
        return IPostModeratedList.providedBy(self)
