...
class ListLookup(SimpleItem):
    ...
    def deliverMessage(self, request):
        ...
        if not address:
            import pprint
            logger.error("No destination found in headers:\n%s" % pprint.pformat(message))
            raise NotFound, "The message destination cannot be deterimined."
        address = address.lower()
        ...
        for ml_address in address_list:
           ...
           if ml is not None:
            ...
            logger.error("no list found for any of %r" % str(address_list))
            raise NotFound, "The message address does not correspond to a "\
                             "known mailing list."
        setSite(ml)
