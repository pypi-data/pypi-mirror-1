    ...
    def __call__(self):
        ...
        for key in self.request.form:
            if key.startswith(UNDO_SUBMIT):
                ...
                if msg_count:
                    txt = (u"Removed %s message%s from the list archive."
        ...
        if self.request.form.get("import_messages"):
            ...
            if file:
                ...
                psm_text = (u'Imported %s message%s from "%s"'
                    % (msg_count, self._plural(msg_count), file.filename))
                psm_type = 'info'
            else:
                psm_text = "No file selected. Please select an mbox file before importing."
                psm_type = 'error'
        ...
        if self.request.form.get('import_subscribers'):
            if not self.can(ImportMailingListSubscribers):
                raise Unauthorized('Cannot import mailing list subscribers')
            ...
            if file:
                ...
                if emails:
                    ...
                    n_emails = len(emails)
                    if n_emails != 1:
                        psm_text = u'%s email addresses imported' % n_emails
                    else:
                        psm_text = u'1 email address imported'
                    psm_type = 'info'
                else:
                    psm_text = u'No emails found'
                    psm_type = 'error'
            else:
                psm_text = u"No file selected. Please select a csv file before importing."
                psm_type = 'error'
            plone_utils.addPortalMessage(psm_text, type=psm_type)
            self.request.response.redirect(self.nextURL())
            return
