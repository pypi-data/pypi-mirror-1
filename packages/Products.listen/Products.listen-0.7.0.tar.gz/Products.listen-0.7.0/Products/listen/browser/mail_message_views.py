...
class MailMessageView(BrowserView):
    ...
    def archiveURL(self):
        # we want the archive url to point to the mailing list itself
...
class ForumMailMessageView(ThreadedMailMessageView):
    ...
    def _thread_url_pointer(self, direction):
        # XXX this needs to be wrapped or we don't have the request in the
        # aq_chain, so getting the url fails
        cur_msg = ISearchableMessage(self.context)
        date = cur_msg.modification_date()
        self.search = getUtility(ISearchableArchive)
        if direction == 'next':
            msgs = self.search.getToplevelMessages(sort_on='modification_date', 
                                                   filter_on='modification_date', 
                                                   start=date)[1:2]
        else: # prev
            msgs = self.search.getToplevelMessages(sort_on='modification_date',
                                                   filter_on='modification_date',
                                                   reversed=True, end=date)[1:2]
        if msgs:
            msg = msgs[0]
            return msg.getURL() + '/forum_view'
        return ''

    def next_thread_url(self):
        return self._thread_url_pointer('next')

    def prev_thread_url(self):
        return self._thread_url_pointer('prev')
