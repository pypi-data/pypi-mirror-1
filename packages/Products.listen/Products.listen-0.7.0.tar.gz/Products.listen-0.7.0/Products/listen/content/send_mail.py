...
class MailSender(object):
    ...
    def user_subscribe_request(self, user_email, user_name, pin, body=None):
        if not user_name: user_name = user_email
        subject = "Subscription confirmation (mail-command:subscribe-member <<%s>> [%s])" % (user_email, pin)
        body = body or default_email_text.user_subscribe_request
    ...
    def manager_mod_post_request(self, user_email, user_name, post):
        ...
        if isinstance(user_name, str):
            user_name = user_name.decode('utf-8', 'replace')
