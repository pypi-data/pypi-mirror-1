...
def to_email(tokens):
    """If tokens already an email, it will return it. Otherwise, it
       will return the result of lookup_email. Order is not preserved to keep
       the lookups efficient. """
    context = getSite()
    ...
    for token in tokens:
        if is_email(token):
            already_emails.append(token)
        else:
            non_emails.append(token)
    all_emails = already_emails + lookup_emails(non_emails)
    # this makes setting the list of managers, which needs to be unicode
    # more convenient
    return [m.decode(encoding) for m in all_emails]
