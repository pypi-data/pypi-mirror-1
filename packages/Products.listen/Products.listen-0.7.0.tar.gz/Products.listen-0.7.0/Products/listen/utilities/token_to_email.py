...
class MembraneMemberToEmail(MemberToEmail):
    ...
    def _lookup_memberid(self, member_id):
        #XXX admins are a special case because they are not membrane users
        if member_id == 'admin':
            return super(MembraneMemberToEmail, self)._lookup_memberid(member_id)

        query = {self.id_index: member_id}
        return self._search(query, self.email_index)

    def admin_email(self):
        return self.mtool.getMemberById('admin').getProperty('email', None)

    def to_memberid(self, email):
        query = {self.email_index: email}
        mem_id = self._search(query, self.id_index)
        #XXX we put a hack in here to see if it is the admin email address
        # because admins are not membrane users
        if mem_id is None:
