...
"""
    ...
    def getToplevelMessages(self, start=None, end=None, recent_first=NoDefault,
                            sort_on='date', filter_on='date', reversed=False):
        """See interface docs."""
        ...
        if recent_first is not NoDefault:
            import warnings
            warnings.warn("Use of the 'recent_first' argument to the "
                          ...
                          "'reversed' arguments to achieve the "
                          "desired results.", DeprecationWarning)
            if recent_first:
                query['sort_on'] = 'modification_date'
                query['sort_order'] = 'descending'
            # recent_first=False is already handled by the default
            # behavior (assuming sort_on, filter_on, and descending
            # are not explicitly set).
        if start and not end:
            query[filter_on]={'query':[start], 'range':'min'}
        elif end and not start:
            query[filter_on]={'query':[end], 'range':'max'}
        elif end and start:
            query[filter_on]={'query':[start,end], 'range':'min:max'}
