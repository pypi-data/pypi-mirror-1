...
base = FormFieldsets(IMemberProfileConfigSchema)
#base.id = 'memberprofile_config'
#base.label = _(u'Member Profile Configuration')

class MemberProfileControlPanel(ControlPanelForm):
    implements(IMemberProfileConfigSchema)
    form_fields = FormFieldsets(base)

    label = _(u'control_panel', default = u"Member Profile Configuration")
    description = _(u'control_panel_description', default = u"")
    form_name = _(u'control_panel', default = u"Member Profile Configuration")
    ...
    def handle_edit_action(self, action, data):
        ...
        if form.applyChanges(self.context, self.form_fields, data,
                             self.adapters):
            self.status = _(u'control_panel_saved', default = u"Changes saved. You may need to reindex Member Profiles.")
    ...
    def handle_quick_reindex_action(self, action, data):
        ...
        brains = catalog(portal_type='MemberProfile')
        for brain in brains:
            obj = brain.getObject()
            obj.reindexObject()
