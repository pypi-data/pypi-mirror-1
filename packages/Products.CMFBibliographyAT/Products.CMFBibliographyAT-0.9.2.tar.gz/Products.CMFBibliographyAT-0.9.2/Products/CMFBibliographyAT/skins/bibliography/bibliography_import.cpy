...
if up_text:
    ...
    if not source or not same_type(source,''):
        state.setError('file', "Could not read the file '%s'." %file.filename)
        return state.set(status='failure',
                         portal_status_message='Please correct the indicated errors.')
...
try:
    ...
    return state.set(status='failure',
                     portal_status_message="""%s Parser's 'checkFormat' and guessing the format""" \
...
if not entries or not same_type(entries, []):
    ...
    return state.set(status='failure',
                     portal_status_message="There must be something wrong with the parser")
...
msg = "Processed Entries: %i. Errors: %i. Import Processed in %f seconds. See Import Report below." \
