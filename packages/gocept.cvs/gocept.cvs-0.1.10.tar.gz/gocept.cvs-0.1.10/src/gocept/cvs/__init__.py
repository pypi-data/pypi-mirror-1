...
class Recipe:
    ...
    def update(self):
        ...
        for module, tag, dest in modules_tags_dest:
            if os.path.isdir(os.path.join(destination, dest)):
                if self.newest:
                    single_update(destination, dest, tag, self.get_verbosity())
            else:
    ...
    def get_verbosity(self):
        return int(self.buildout['buildout'].get('verbosity', 0))
...
def single_checkout(path, location, dest, tag, cvsroot, verbosity):
...
def single_update(location, dest, tag, verbosity):
    cmd = ['cvs']
    if verbosity >= 10:
        pass
    elif verbosity >= 0:
