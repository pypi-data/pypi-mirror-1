...
class TestAggregator(AggregationTestCase):
    ...
    def test_translated_querying(self):
        ...
        annotations['aggregation_sources'] =  ['/master-1', '/master-2']
        annotations['keyword_list'] = []
        annotations['restrict_language'] = True

        sklave = slave.addTranslation('de')
        subtyper = component.getUtility(ISubtyper)
        subtyper.change_type(sklave, 'slc.aggregation.aggregator')
        ...
        for i in range(0, 5):  
            meister_1.invokeFactory('News Item', 'another-news-item-%s' % i)
            item = meister_1._getOb('another-news-item-%s' % i)
            item.setSubject(['filter_keyword'])
            item.setLanguage('fr')
            workflowTool.doActionFor(item, 'publish')
            item.reindexObject()
        ...
        self.assertEquals(len(meister_1.objectIds()), 25)

        annotations['restrict_language'] = False
        self.assertEquals(len(view.query_items()),  25)
