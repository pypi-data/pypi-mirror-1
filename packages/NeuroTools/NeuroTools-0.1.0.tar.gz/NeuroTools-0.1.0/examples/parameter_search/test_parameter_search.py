"""
Tests for parameter_search.py.

$Id: test_parameter_search.py 197 2008-09-11 12:40:30Z mschmucker $
"""
import unittest
...
class TestParameterSearch(unittest.TestCase):
    ...
    def parametersearch(self, searcher, **kwargs):
        def func(param = None):
            ...
            """
            return param
        dictlist = [{'lala':1, 'lulu':2},
                    {'lala':3, 'lulu':4},
                    {'lala':5, 'lulu':6}]
        ps = searcher(dictlist = dictlist, func = func, **kwargs)
        ps.search()
        result = ps.harvest()
...
class TestPlainParameterSearcher(TestParameterSearch):
    def test_ParameterSearcher(self):
        (dictlist, result) = self.parametersearch(ParameterSearcher)
        self.assertEqual(dictlist, result)
...
#        while len(mec.get_ids()) < 1:
#            time.sleep(0.33)
#        (dictlist, result) = self.parametersearch(IPythonParameterSearcher, 
...
#        dictlist = [{'str':'failure'},
#                    {'str':'failure'},
#                    {'str':'failure'}]
#        ps = searcher(dictlist = dictlist, func = will_fail, **kwargs)
#        ps.search()
#        result = ps.harvest()
#        return (dictlist, result, ps)
#        
#    def test_failingIPythonParameterSearcher(self):
#        """
#        tests if failing tasks are kept track of correctly.
#        """
#        (dictlist, result, ps) = self.parametersearch(IPythonParameterSearcher, 
#                                    task_furl = self.task_furl,
#                                    multiengine_furl = self.multiengine_furl,
#                                    engine_furl = self.engine_furl)
...
#        (dictlist, result) = self.parametersearch(
#                                    RestartingIPythonParameterSearcher, 
#                                    take_down = True,
#                                    task_furl = self.task_furl,
#                                    multiengine_furl = self.multiengine_furl,
