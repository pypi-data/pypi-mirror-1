...
# VERSION: 2.0.0a19 (19SEPTEMBER2005)
...
# @return A 1 for success and a 0 for failure.
# @defreturn integer
def plot_line_xy(xydict,gfilename='plot_line_xy',gtitle='Value by key',gxlabel='X',gylabel='Y'):
    try:
        import matplotlib
        matplotlib.use('Agg')
        import pylab
        ...
        if pedobj.kw['messages'] == 'verbose':
            print 'ERROR: pyp_graphics/plot_line_xy() was unable to create the plot \'%s\' (%s.png).' % (gtitle,gfilename)
        logging.error('pyp_graphics/plot_line_xy() was unable to create the plot \'%s\' (%s.png).' % (gtitle,gfilename))
        _status = 0
    return _status