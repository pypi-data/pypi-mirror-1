...
# @defreturn list
def related_animals(anim_a,pedobj):
    # Use dictionaries for the lookups!
    try: logging.info('Entered related_animals()')
    except: pass
    _ped = []
    try:
        _ped = pyp_nrm.recurse_pedigree_idonly(pedobj,anim_a,_ped)
    except:
        pass
...
def generation_intervals(pedobj,units='y'):
    ...
    # If you are a mouse or fly person, you will not get the answers that you are
    # expecting here.
    #
    _ssy = 0.
    _sdy = 0.
...
# the oldest known ancestor in in each selection path.  Ancestors with unknown parents
# are assigned to generation 0.
# See: Valera, M., Molina, A., Gutierrez, J. P., Gomez, J., and Goyache, F.  2005.
#      Pedigree analysis in the Andalusian horse: population structure, genetic
#      variability and the influence of the Carthusian strain.  Livestock Production
#      Science.  95:57-66.
# @param pedobj A PyPedal pedigree object.
# @return A dictionary containing the five average ages.
# @defreturn dictionary
def num_traced_gens(pedobj):
    try: logging.info('Entered num_traced_gens()')
    except: pass
    _sire_son = 0.
    _sire_dau = 0.
    _dam_son = 0.
    ...
    for _p in pedobj.pedigree:
        if int(_p.sireID) > 0:
            _sire_ancestors = related_animals(_p.sireID,pedobj)
            if _p.sex == 'm' or _p.sex == 'M':
                _sire_son = _sire_son + len(_sire_ancestors)
                _n_sire_son = _n_sire_son + 1
            elif _p.sex == 'f' or _p.sex == 'F':
                _sire_dau = _sire_dau + len(_sire_ancestors)
                _n_sire_dau = _n_sire_dau + 1
            else:
                pass
        if int(_p.damID) > 0:
            _dam_ancestors = related_animals(_p.damID,pedobj)
            print _dam_ancestors
            if _p.sex == 'm' or _p.sex == 'M':
                _dam_son = _dam_son + len(_dam_ancestors)
                _n_dam_son = _n_dam_son + 1
            elif _p.sex == 'f' or _p.sex == 'F':
                _dam_dau = _dam_dau + len(_dam_ancestors)
                _n_dam_dau = _n_dam_dau + 1
            else:
                pass

    try:
        avg_traced_gens['ss'] = _sire_son / _n_sire_son
    except ZeroDivisionError:
        avg_traced_gens['ss'] = 0.
    try:
        avg_traced_gens['sd'] = _sire_dau / _n_sire_dau
    except ZeroDivisionError:
        avg_traced_gens['sd'] = 0.
    try:
        avg_traced_gens['ds'] = _dam_son / _n_dam_son
    except ZeroDivisionError:
        avg_traced_gens['ds'] = 0.
    try:
        avg_traced_gens['dd'] = _dam_dau / _n_dam_dau
    except ZeroDivisionError:
        avg_traced_gens['dd'] = 0.
    try:
        avg_traced_gens['mean'] = ( avg_traced_gens['ss'] + avg_traced_gens['sd'] + avg_traced_gens['dd'] + avg_traced_gens['ds'] ) / 4.
    except ZeroDivisionError:
        avg_traced_gens['mean'] = 0.

    try: logging.info('Exited num_traced_gens()')
    except: pass
    return avg_traced_gens

##
# num_equiv_gens() computes the number of equivalent generations as the sum of (1/2)^n,
# where n is the number of generations separating an individual and each of its known
# ancestors.
# @param pedobj A PyPedal pedigree object.
# @return A dictionary containing the five average ages.
# @defreturn dictionary
def num_equiv_gens(pedobj):
    try: logging.info('Entered num_equiv_gens()')
    except: pass
    try: logging.info('Exited num_equiv_gens()')
    except: pass
    return

##
# partial_inbreeding() computes the number of equivalent generations as the sum
# of (1/2)^n, where n is the number of generations separating an individual and
# each of its known ancestors.
# @param pedobj A PyPedal pedigree object.
# @return A dictionary containing the five average ages.
# @defreturn dictionary
def partial_inbreeding(pedobj):
...
#     print '\t\tUnks: %s' % (pedobj.pedigree[int(anid)-1].unks)
