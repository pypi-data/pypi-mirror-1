...
# VERSION: 2.0.0b3 (23NOVEMBER2005)
...
#         print self.kw
    ...
    def preprocess(self):
        ...
        pedformat_codes = ['a','s','d','g','x','b','f','r','n','y','l','e','p','A','S','D','L','Z','h','H']
...
#                 self.by = 1901
        ...
        if locations['herd'] != -999:
            if 'H' in mykw['pedformat']:
                ...
                self.originalHerd = string.strip(data[locations['originalHerd']])
            else:
...
class PedigreeMetadata:
    ...
    # @param self Reference to the current Pedigree() object
    # @return The number of unique herds in the pedigree and a list of those herds
    # @defreturn integer-and-list
    def nuherds(self):
        """Count the number of unique herds in the pedigree."""
        herddict = {}
        for l in range(self.num_records):
            if self.myped[l].originalHerd != 'u':
                try:
                    _h = herddict[self.myped[l].originalHerd]
                except KeyError:
                    herddict[self.myped[l].originalHerd] = self.myped[l].originalHerd
        n = len(herddict.keys())
        return n, herddict.keys()
...
class NewAMatrix:
    ...
    # @param self Reference to the current NewAMatrix() object
    # @param kw A dictionary of options.
    # @return An instance of a NewAMatrix() object
    # @defreturn object
    def __init__(self,kw):
