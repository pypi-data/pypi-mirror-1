...
relative_to_db_config = os.path.abspath
if 'CONCEPTNET_DB_CONFIG' in os.environ:
    ...
    def relative_to_db_config(path):
        if not os.path.isabs(path):
            path = os.path.join(os.path.dirname(db_config_path), path)
    ...
    try:
        ...
        # Fail early and hard.
        print >> sys.stderr, """
    Importing csamoa.settings failed because you don't have a 'db_config.py'
    file on your Python path. See http://conceptnet.media.mit.edu/ for an
...
# def get_thing(container, name, default):
...
def get_db_config(param, default=''):
    ...
    if isinstance(db_config, dict):
        ...
        if hasattr(db_config, long_param): return getattr(db_config, long_param)
        if hasattr(db_config, short_param): return getattr(db_config, short_param)
    return default
...
AUTHENTICATION_BACKENDS = (
        'pseudo_auth.backends.LegacyBackend',
