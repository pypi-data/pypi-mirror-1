...
from conceptnet4.models import Assertion, Relation, Language
...
def addAssertionString(tensor, relationtype, ltext, rtext, value):
...
def addAssertionTuple(tensor, relation, ltext, rtext, value):
    ltext = peopl_person(ltext)
    rtext = peopl_person(rtext)
    lprop = ('left',relation,ltext)
    rprop = ('right',relation,rtext)
    tensor[ltext, rprop] = value
    tensor[rtext, lprop] = value
...
def assertion_queryset_to_tensor(queryset, identities, addToTensor=addAssertionTuple):
    ...
    for (relation, concept1, concept2, score, freq) in queryset.values_list(
        ...
        value = freq * scorecurve(score) / 10.0
        addToTensor(tensor, relation, concept1, concept2, value)
...
def selfblend(lang, identities=0, cutoff=DEFAULT_CUTOFF):
    from divisi.blend import Blend
    byrel = conceptnet_by_relations(lang)
    t=identities_for_all_relations(byrel)
    return Blend(byrel.values()+[t])
