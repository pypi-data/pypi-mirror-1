...
class MemcachedRLock(object):
    ...
    def acquire(self,blocking=True):
        ...
        res =  not not self._set(timeout=self.timeout)
    ...
    def release(self):
        ...
        release = self._set(timeout=self.timeout)
        ...
        def decorator(fun):
            def replacement(*args, **kwargs):
                ...
                try:
                    ...
                    if not locked:
                        # other zeo thread is working, waiting until other lock is released
                        reserved = lock.acquire()

                        if reserved:
                            # restart transaction via ConflictError
                            raise ConflictError, 'memcachedlock.zlock - lock acquired but raising a ConflictError for proper transaction retrial'
