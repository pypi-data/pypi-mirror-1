...
try:
    t=gettext.translation('PyDumpFS')
    _=t.ugettext
except IOError:
    _=lambda str: str
        ...
        for name in names:
            ...
            try:
                if os.path.isdir(srcname) and not os.path.islink(srcname):
                    if verbose >1:
                        print _("make directoryi:")+"%s"%(dstname)
                    os.makedirs(dstname)
...
if __name__ == '__main__':
    ...
    parser=OptionParser(usage=usage,version=u"%prog 1.0.6 by HASEGAWA Masahiro<hasegawa@mapse.eng.osaka-u.ac.jp>")
