...
class BBConnection(s3amazon.S3.AWSAuthConnection):
    ...
    def head(self, bucket, key, headers={}):
        # another ugly hack to get around hinky httplib.py
        self.connection._HTTPConnection__response = None
    ...
    def get_bucket(self, bucket_name, prefix=None):
        if bucket_name in self._buckets.keys():
            return self._buckets[bucket_name]
        self._resp = s3amazon.S3.AWSAuthConnection.create_bucket(self,
                                                                 bucket_name)
    ...
    def delete_bucket(self, bucket_name):
        self._resp = s3amazon.S3.AWSAuthConnection.delete_bucket(self,
                                                                 bucket_name)
        status = self._resp.http_response.status
        if status == 204:
            if bucket_name in self._buckets.keys():
    ...
    def set_canned_acl(self, b, policy):
        ...
        self._resp = s3amazon.S3.AWSAuthConnection.put_acl(self, b_name,
                                                           key, '', headers)
