...
class ForwardFriendlyStatusBasedForward(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if url:
            ...
            # use virtual hosted path since plone.app.redirector uses path from zope root
            vhm_root = environ.get('repoze.vhm.virtual_root', '')
            environ['collective.fourohfour.original_path'] = vhm_root + request.path
            environ['collective.fourohfour.original_path_qs'] = vhm_root + request.path_qs
