...
def load_json(environ):
    ...
    """
    # Get the real json string
    if 'CONTENT_LENGTH' in environ:
        length = int(environ['CONTENT_LENGTH'])
    else:
        length = -1
    entity = environ['wsgi.input'].read(length)
...
def httpserv(host, port, i_queue, o_queue):
    ...
    def handler(environ, start_response):
        ...
        print "Got a request from %s, %s" % (environ['fapws.remote_addr'],
        ...
        # Generate a id so that we know if a item input queue and that
        # in output queue matches
        req_id = uuid.uuid1()
        ...
        # Polling output queue for matching task result, use Lock to
        # avoid deadlock
        with lock:
            temp_buf = []
            while True:
        ...
        return reply_json(start_response, reply[1])
