...
class Declaration(object):
    ...
    def record(self, context=None, inset='   '):
        ...
        lines = []
        lines.append('%s<%s:%s' % (inset, self.namespace, self.tagname))
    ...
    def record(self, context=None, inset='   '):
        ...
        for declaration in self.declarations:
            result.append(declaration.record(context=context, 
                                                inset=inset+'    '))
        result.append('%s</%s>' % (inset, self.tagname))
        return '\n'.join(result)
...
class DefaultGenericFunction(GenericFunction):
    """A generic function that use queryAdapter calls.
