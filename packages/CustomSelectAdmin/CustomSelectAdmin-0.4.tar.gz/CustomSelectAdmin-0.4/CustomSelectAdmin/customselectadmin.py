...
class CustomSelectPanel(Component):
    ...
    def _get_custom_fields(self):
        ...
        for name in [option for option, value in config.options()
            ...
            field = {
                ...
                'panel': config.get(name + '.panel') or config.get(name + '.label') or name.capitalize()
            }
    ...
    def get_admin_panels(self, req):
        if req.perm.has_permission('TRAC_ADMIN'):
            for field in self._get_custom_fields():
                yield ('ticket', 'Ticket System', field[2].replace(' ', '').lower(), field[2])
    ...
    def render_admin_panel(self, req, cat, page, path_info):
        ...
        data.update(dict(enums=fieldoptions,
                         default=default, placeholder=placeholder, view='list'))
