...
def kill_process(pid, sig=signal.SIGTERM):
    ...
    if subprocess.mswindows:
       ...
       #
       if misc.GlobalData.debug:
          print "Killing process",pid,"with signal",sig
       os.kill(pid,sig)


misc.GlobalData.current_process=None
misc.GlobalData.signal_handler_busy=False
#
...
def signal_handler(signum, frame):
        ...
        if misc.GlobalData.signal_handler_busy:
           print ""
           print "  Signal handler is busy.  Aborting."
           sys.exit(-signum)
        if misc.GlobalData.current_process is None:
           print "  Signal",signum,"recieved, but no process queued"
           print "  Exiting now"
           sys.exit(-signum)
        if misc.GlobalData.current_process is not None and\
           misc.GlobalData.current_process.pid is not None and\
           misc.GlobalData.current_process.poll() is None:
           misc.GlobalData.signal_handler_busy=True
           kill_process(misc.GlobalData.current_process.pid, signum)
           print "  Signaled process", misc.GlobalData.current_process.pid,"with signal",signum
           print "  Waiting...",
           endtime = timer()+15.0
           while timer() < endtime:
             status = misc.GlobalData.current_process.poll()
             if status is None:
                break
             time.sleep(0.1)
           misc.GlobalData.current_process.wait()
           status = misc.GlobalData.current_process.poll()
           if status is not None:
              print "Done."
              raise OSError, "Interrupted by signal " + `signum`
           else:
              raise OSError, "Problem terminating process" + `misc.GlobalData.current_process.pid`
        raise OSError, "Interrupted by signal " + `signum`

#
# Run a command line
#
def run_command(cmd, outfile=None, cwd=None, ostream=None, valgrind=False, valgrind_log=None, valgrind_options=None, memmon=False):
    if cwd is not None:
       oldpwd = os.getcwd()
       os.chdir(cwd)
    if memmon:
        memmon = plugin_services.registered_executable("memmon")
        if memmon is None:
            raise IOError, "Unable to find the 'memmon' executable"
        cmd = memmon.get_path()+" "+cmd
    if valgrind:
        #
        # The valgrind_log option specifies a logfile that is used to store
        # valgrind output.
        #
        valgrind = plugin_services.registered_executable("valgrind")
        if valgrind is None:
            raise IOError, "Unable to find the 'valgrind' executable"
        if valgrind_options is None:
            valgrind_options = " -v --tool=memcheck --trace-children=yes"
        if not valgrind_log is None:
            valgrind_log = " --log-file-exactly="+valgrind_log
        else:
            valgrind_log = ""
        cmd = valgrind.get_path()+valgrind_options+valgrind_log+" "+cmd
    output = ""
    if sys.version_info[0:2] < (2,4):
       sys.stdout.flush()
       if outfile is None:
          ...
          output = commands.getoutput(cmd.strip() + " > " + outfile + " 2>&1")
    else:
       #
       ...
       if ostream is not None:
          OUTPUT=ostream
       try:
         if outfile is not None or ostream is not None:
            misc.GlobalData.current_process = subprocess.Popen(cmd.strip(),shell=True,stdout=OUTPUT,stderr=subprocess.STDOUT)
         else:
            misc.GlobalData.current_process = subprocess.Popen(cmd.strip(),shell=True)
         misc.GlobalData.current_process.wait()
         misc.GlobalData.current_process = None
       except OSError:
         #
...
plugin_services.register_executable(name="valgrind")
plugin_services.register_executable(name="memmon")
