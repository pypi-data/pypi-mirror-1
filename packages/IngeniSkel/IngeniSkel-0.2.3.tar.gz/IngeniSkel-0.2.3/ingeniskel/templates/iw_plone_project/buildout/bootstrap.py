...
def _import(egg_name):
    ...
    if not egg_location.startswith(eggs_folder):
        ...
        if len(path_) > 0:
            ...
            shutil.copytree(path_, os.path.join(eggs_folder, folder_name))
...
def link_egg(working_set, egg_name, required=True):
    ...
    if egg != []:
        ...
        try:
            ...
            retcode = call([sys.executable, '-c', cmd, '-mqNxd', 
                            eggs_folder, egg_name])

            if retcode != 0:
                print 'Could not find %s in eggs, aborting' % egg_name
                sys.exit(1)
