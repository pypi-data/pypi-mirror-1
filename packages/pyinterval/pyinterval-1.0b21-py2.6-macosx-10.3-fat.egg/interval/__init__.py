...
class interval(tuple):
    ...
    def to_s(self):
        """Return a non-parsable compact representation of the interval."""
        return ' | '.join(c.to_s() for c in self)
    ...
    class Component(tuple):
        ...
        def to_s(self):
            ...
            if self.inf == self.sup:
                return repr(self.inf)
            def fix(a, b = None):
                return (a, 'e'+b if b is not None else '')
            rr = [fix(*repr(x).split('e')) for x in self]
            def common_prefix(s, t):
                return s[:i], (s[i:], t[i:])
            if rr[0][1] == rr[1][1]:
                s, t, e = rr[0][0], rr[1][0], rr[0][1]
                if len(s) != len(t) and '.' in s and '.' in t:
                    l = max(len(s), len(t))
                    s += '0' * (l - len(s))
                    t += '0' * (l - len(t))
                if len(s) == len(t):
                    i = min(n for n,(a,b) in enumerate(zip(s, t)) if a!=b)
                else:
                    i = 0
                if i:
                    return s[:i] + '(' + s[i:] + ',' + t[i:] + ')' + e
            return '[' + ', '.join(''.join(s for s in r) for r in rr) + ']'
