...
class dailytweetView(BrowserView):
    ...
    def daily_tweet(self):
        ...
        next_post = float(time_sec) + float(self.context.t_interval)
        
        print '========================================='
        print next_post, time.mktime(datetime.datetime.utcnow().timetuple())
        
        if next_post < time.mktime(datetime.datetime.utcnow().timetuple()):
            print 'post now',
            ...
            if int(returned_dates['days']) > 0 :
                t_text = t_text.replace('$days$',(str(returned_dates['days']) + ' day'))
            elif int(returned_dates['days']) > 1 :
                t_text = t_text.replace('$days$',(str(returned_dates['days']) + ' days'))
            else:
                t_text = t_text.replace('$days$','')
                
            if int(returned_dates['hours']) > 0 :
                t_text = t_text.replace('$hours$',(str(returned_dates['hours']) + ' hour'))
            if int(returned_dates['hours']) > 1 :
                t_text = t_text.replace('$hours$',(str(returned_dates['hours']) + ' hours'))
            else:
                t_text = t_text.replace('$hours$','')
                
            if int(returned_dates['mins']) > 0 :
                t_text = t_text.replace('$mins$',(str(returned_dates['mins']) + ' min'))
            if int(returned_dates['mins']) > 1 :
                t_text = t_text.replace('$mins$',(str(returned_dates['mins']) + ' mins'))
            else:
                t_text = t_text.replace('$mins$','')
