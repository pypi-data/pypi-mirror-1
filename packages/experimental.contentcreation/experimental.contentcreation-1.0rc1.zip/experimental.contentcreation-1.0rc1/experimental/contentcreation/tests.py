...
import experimental.contentcreation
...
class TestPortalFactory(TestCase):
    ...
    def testTempFolderLocalRoles(self):
        ...
        # Clear cache
        del self.app.REQUEST.other['__factory__info__']
        del self.app.REQUEST.other['listTypeInfo-None']
    ...
    def DONTtestTempFolderPermissionAq(self):
...
class TestCreateObjectByURL(ptc.FunctionalTestCase):
...
class TestAllowedContentTypes(TestCase):
    def testAlloweContentTypes(self):
        self.portal.allowedContentTypes()
        self.folder.allowedContentTypes()
...
def test_suite():
    ...
    suite.addTest(makeSuite(TestAllowedContentTypes))
    return suite
