...
Ext.ux.ItemSelector = Ext.extend(Ext.form.Field,  {
    ...
    onRender: function(ct, position){
		...
		var el=icons.getEl();
		if (!this.toSortField) {
			this.toTopIcon = el.createChild({tag:'button', cls:'ux-multiselect-top-button'});
			this.upIcon =  el.createChild({tag:'button', cls:'ux-multiselect-up-button'});
		}
		this.addIcon = el.createChild({tag:'button', cls:'ux-multiselect-right-button'});
		this.removeIcon = el.createChild({tag:'button', cls:'ux-multiselect-left-button'});
		if (!this.toSortField) {
			this.downIcon =  el.createChild({tag:'button', cls:'ux-multiselect-down-button'});
			this.toBottomIcon =  el.createChild({tag:'button', cls:'ux-multiselect-bottom-button'});
		}
		if (!this.readOnly) {
			if (!this.toSortField) {
