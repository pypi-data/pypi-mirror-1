...
from benri.client.client import Conflict
...
class ConsoleClient(object):
    ...
    def _collection_backup(self, collection_name, output_path, recursive=False):
        ...
        """
        g_client = GraphClient(self.cfg.service_url, base_path=self.cfg.base_path, graphs_prefix=self.cfg.graph_prefix)
        sys_client = SystemClient(self.cfg.service_url, base_path=self.cfg.base_path, system_prefix=self.cfg.system_prefix)
        
        cols, graphs = sys_client.collection_retrieve(collection_name, recursive=recursive)
        ...
        for g in graphs:
            try:
                stat, graph_data = g_client.retrieve(g)
...
#                print e
    ...
    def _restore_backup(self, file_path, prefix='/', replace=False):
        ...
        for graph in tar:
            ...
            try:
                g_client.create(graph_name, tar.extractfile(graph), mime_type)
            except Conflict, e:
                if replace:
                    g_client.replace(graph_name, tar.extractfile(graph), mime_type)
            except Exception, e:
    ...
    def _bulk_upload(self, prefix, dir_path):
        ...
        for path in gen_paths(dir_path):
            ...
            try:
                client.create(graph_name, data, mime_type)
            except Conflict, e:
                client.update(graph_name, data, mime_type) 
            except Exception, e:
