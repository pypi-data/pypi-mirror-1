...
class InvitationsTool(PloneBaseTool, ObjectManager, SimpleItem, ActionProviderBase, PropertyManager):
    """
    ...
    toolicon = 'skins/ploneinvite/invitations_icon.png'
    
    _actions = (
        ActionInformation(id='invite',
                          title='Invite',
                          action=Expression( text='string:${portal_url}/invite_form'),
                          condition=Expression( text='python:member and len(portal.portal_invitations.getInvitesUser(used=0, sent=0))'),
    ...
    def mailInvite(self, variables = {}):
        ...
        recipient_email = variables['invite_to_address']
        subject = _(u'email_invitation_subject', default=u"Invitation to join")
        ...
        host = self.MailHost
        host.send(mail_text, recipient_email, sender_email, subject)
    ...
    def getInviteCode(self, enforce_address=False):
        ...
        if enforce_address:
            # Tries to re-use enforce_address marked as True
    ...
    security.declareProtected(Permissions.ManagePortalInvites, 'getInvites')
...
class InviteToken(CatalogAware, SimpleItem):
    """
    An invite token. This class provides no ZMI management interface.
    All handling is via the portal_invitations tool.
    """
