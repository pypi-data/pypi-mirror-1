...
__revision__ = "$Rev: 225 $"
__date__ = "$Date: 2008-03-07 09:23:44 +0100 (Fri, 07 Mar 2008) $"
...
class DocBook2Sla(object):
    ...
    def create(self, docbookfn, scribusfn, output_filename=None, validate=False):
        ...
        if _debug:
            print "DEBUG: docbook_with_ids written to '%s'" % docbook_with_ids
            #return open(docbook_with_ids, 'r').read()

        # docbook2pageobject : 01_content.id.xml -> pageobjects.xml
        scribus_pageobjects = self.transform(docbook_with_ids, self.xsl_docbook2pageobject)

        if _debug:
            print "DEBUG: scribus_pageobjects written to '%s'" % scribus_pageobjects
        ...
        if _debug:
            print "DEBUG: output written to '%s'" % output
    ...
    def syncronize(self, docbookfn, scribusfn, output_filename=None, validate=False):
        ...
        # wrapper : pageobjects.xml + clean134.sla -> output.sla
        output = self.transform(scribusfn, xsl_wrapper, secondinputfn=scribus_pageobjects)
