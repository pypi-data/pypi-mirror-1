...
class ArgumentParser(_AttributeHolder, _ActionsContainer):
    ...
    def _parse_args(self, arg_strings, namespace):            
        ...
        for i, arg_string in enumerate(arg_strings_iter):
            ...
            # and note the index if it was an option
                ...
                if self._looks_like_option(arg_string):
    ...
    def _looks_like_option(self, arg_string):
        # if it doesn't start with a '-', it's meant to be positional
        if not arg_string.startswith('-'):
            return False

        # if it's just a single dash, it's meant to be positional
        if arg_string == '-':
            return False

        # if it starts with two dashes, it's meant to be an option
        if arg_string.startswith('--'):
            return True

        # if it's a valid short option, it's meant to be an option
        if arg_string[:2] in self._short_option_strings:
            return True

        # if it looks like a negative number, it's meant to be positional
        if self._negative_number_matcher.match(arg_string):
            return False

        # otherwise, it looks like an option (but it's not in the parser)
        return True
