...
$Id: enumeration.py 256 2006-05-31 04:36:35Z james $
Copyright (c) 2006 James Harris
"""
__revision__ = '$Revision: 256 $'
__location__ = '$URL: http://localhost/svn/raze/tags/raze-1.0.0/raze/enumeration.py $'
__all__ = [ 'Enumeration' ]


class MetaEnumeration(type):
    """ Enumeration meta class. """
    def __new__(metacls, name, bases, ctx):
        labels = ctx['labels']
        
        if isinstance(labels, dict):
            keys = sorted(labels.keys())
            ctx['labels'] = [ str(key) for key in keys ]
            ctx['values'] = [ int(labels[key]) for key in keys ]
        else:
            ctx['labels'] = [ str(key) for key in labels ]
            ctx['values'] = range(len(labels))

        return type.__new__(metacls, name, bases, ctx)

    def __init__(class_, name, bases, ctx):
        super(MetaEnumeration, class_).__init__(name, bases, ctx)
...
class Enumeration(object):
    """ Base class for generated enumeration types. """
