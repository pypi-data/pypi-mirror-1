...
class OSCServer(UDPServer):
	...
	def sendOSCerror(self, txt, client_address):
		...
		lines = txt.split('\n')
		if len(lines) == 1:
			msg = OSC.OSCMessage(self.error_prefix)
			msg.append(lines[0])
		elif len(lines) > 1:
			msg = OSC.OSCBundle(self.error_prefix)
			for line in lines:
				msg.append(line)
		else:
			return
	...
	def sendOSCinfo(self, txt, client_address):
		...
		lines = txt.split('\n')
		if len(lines) == 1:
			msg = OSC.OSCMessage(self.info_prefix)
			msg.append(lines[0])
		elif len(lines) > 1:
			msg = OSC.OSCBundle(self.info_prefix)
			for line in lines:
				msg.append(line)
		else:
			return
	...
	def noCallback_handler(self, addr, tags, data, client_address):
		...
		Returns None
		"""
		self.printErr("No callback registered to handle OSC-address '%s'" % addr)
	...
	def serverInfo_handler(self, addr, tags, data, client_address):
		...
		if cmd in ('help', 'info'):
			...
			self.printErr("unrecognized command '%s' in /info request from osc://%s. Try 'help'" % (cmd, getUrlStr(client_address)))
