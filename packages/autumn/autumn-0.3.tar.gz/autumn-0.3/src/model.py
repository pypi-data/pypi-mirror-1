...
class Model(object):
    ...
    Syntax::
        ...
        class MyModel(Model):
            class Meta:
                ...
                validations = {'field': lambda self, v: v > 0}
    ...
    def _update(self):
        ...
        query += ', '.join(['%s = %s' % (escape(f), db.placeholder) for f in self._changed])
        query += ' WHERE %s = %s ' % (escape(self.Meta.pk), db.placeholder)
    ...
    def _new_save(self):
        ...
        query = 'INSERT INTO %s (%s) VALUES (%s)' % (self.Meta.table_safe, 
                   ', '.join([escape(f) for f in self._fields if f != self.Meta.pk]),
                   ', '.join([db.placeholder] * (len(self._fields) - 1)))
        values = [getattr(self, f, None) for f in self._fields if f != self.Meta.pk]
        cursor = Query.raw_sql(query, values)        
    ...
    def delete(self):
        ...
        query = 'DELETE FROM %s WHERE %s = %s' % (self.Meta.table_safe, self.Meta.pk, db.placeholder)
