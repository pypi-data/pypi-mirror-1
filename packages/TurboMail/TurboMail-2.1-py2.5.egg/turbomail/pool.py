...
class Pool(Thread):
	"""A thread pool which checks regularly for new jobs and spawns processes
	as needed.
	...
	def __init__(self, interval=10, threads=4, jobs=10, timeout=60, polling=False, testmode=False, **kw):
		...
		@param polling: Enable or disable the periodic polling
		                mechanism.  Disabled, threads will be created,
		                as required, when work is enqueued.
		@type polling: bool

		@param testmode: Enable or disable the unit testing mode. 
		                Disabled, no mail is actually sent but only
		                stored in the queue for later retrieval.
		@type testmode: bool
		"""
	...
	def enqueue(self, work, block=True, timeout=None):
		...
		@param block: Block code execution until there is a free slot in
		              the queue.  If I{block} is True and I{timeout} is
		              None, block indefinitely.
		@type block: bool
		
		@param timeout: How long to block execution (in seconds).  If
		                the timeout expires (or block is false and the
		                queue is full) raise the Full exception.
		@type timeout: int
		"""
		...
		if self._testmode:
			log.debug("Testmode enabled, not sending mail.")
			return
		
		optimum_threads = min(self._threads, math.ceil(self._queue.qsize() / float(self._jobs)))

		if not self._polling and not self._queue.empty() and self._pool < optimum_threads:
			log.debug("Work enqueued.  Spawning %d threads." % (optimum_threads - self._pool))
			for i in range(int(optimum_threads - self._pool)):
				self.spawn()
	...
	def worker(self):
		...
		perform the work of the thread pool.
	...
	def worker(self):
		...
		while True:
			...
			try:
				...
				log.error("SMTP server refused delivery.  Not retrying.")
			
			except (socket.error, smtplib.SMTPException), exception:
				log.exception(exception)
				log.error("Exception occured when sending mail. Retrying...")
				time.sleep(1)
				self._queue.put(unit, block=False)
				log.debug("Worker death by exception.")
				break
