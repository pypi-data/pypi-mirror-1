...
class Dispatch(object):
	...
	def connect(self):
		...
		authenticates against the username and password previously
		provided.
		"""
	...
	def __call__(self, message):
		...
		@param message: This parameter must be a callable which returns
		...
		if isinstance(pack['sender'], tuple):
			packUpdate['addrfrom'] = '"%s" %s' % pack['sender']
			packUpdate['smtpfrom'] = pack['sender'][1]
		else:
			packUpdate['addrfrom'] = '- %s' % pack['sender']
			packUpdate['smtpfrom'] = pack['sender']
		
		if 'smtpfrom' in pack and pack['smtpfrom'] != None:
			if isinstance(pack['smtpfrom'], tuple):
				packUpdate['smtpfrom'] = pack['smtpfrom'][1]
			else:
				packUpdate['smtpfrom'] = pack['smtpfrom']
		
		pack.update(packUpdate)
		
		if not pack.has_key('to'):
			pack['to'] = pack['recipients']
