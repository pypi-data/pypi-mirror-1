...
class ControllerMessage(Message):
	"""A message that accepts a callable and arguments.

	Example usage::

		import turbomail
		message = turbomail.ControllerMessage(
				"from@host.com",
				"to@host.com",
				"Subject",
				controller.method,
	...
	Feel free to set a message.plain content as the controller only
	returns HTML.  If you wish to hand-produce content, use the
	Message class.
	
	This class can likely be improved to automatically generate the
	plain text content from processing the HTML, calling two methods,
	or calling a single method with special arguments.  Needs
	discussion.
	"""

	def __init__(self, sender, recipient, subject, method, variables={}, **kw):
		"""Store the additonal template and variable information.

		@param method: Any callable that returns HTML; usually an
		               exposed controller method.
		...
		super(ControllerMessage, self).__init__(sender, recipient, subject, **kw)

	def _process(self):
		"""Automatically generate the rich text content."""

		data = dict(sender=self.sender, recipient=self.recipient, subject=self.subject)

		for (i, j) in self._variables.iteritems():
			if callable(j): data[i] = j()
			else: data[i] = j

		self.rich = self._method(**data)

		return super(ControllerMessage, self)._process()
