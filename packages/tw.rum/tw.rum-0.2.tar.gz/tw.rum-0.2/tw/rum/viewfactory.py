...
class WidgetFactory(ViewFactory):
    ...
    input_field_map = {
        ...
        fields.Relation:    forms.SingleSelectField,
    ...
    def _gen_input_widgets(
        ...
        if 'fields' not in args:
            fields = app.fields_for_resource(resource)
            widgets = []
            for field in fields:
                try:
                    widgets.append(
    ...
    @staticmethod
    def _fields_for_display(resource):
        return  [f for f in app.fields_for_resource(resource)
    ...
    def _gen_fields(self, resource, parent, remote_name, attr, action, args):
        ...
        def to_column(field,widget):
            return Column(name=f.name,title=f.label,getter=widget, options=dict(field=field))
        if action=="index":
            converter=to_column
        else:
        ...
        if 'fields' not in args:
            ...
            args['fields'] = fields = []
            for f in self._fields_for_display(resource):
    ...
    def _add_args_for_relation(self, resource, parent, remote_name, attr,
        ...
        self.add_validator(v, args)
        def get_options():
            db_options=list(app.repositoryfactory(attr.other).select())
    ...
    get.when("isinstance(attr, fields.Relation) and "
              "attr.remote_name == remote_name and parent and "
              "issubclass(parent, attr.other) and action in new_actions",
              prio=-1)(NoWidget)
