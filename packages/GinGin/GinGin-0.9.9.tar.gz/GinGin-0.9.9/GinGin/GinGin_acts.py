...
def get_url_of_doc(doc_id):
    link = 'http://' + os.environ['SERVER_NAME']
    if os.environ['SERVER_PORT'] != '80':
	link = link + ':80'
    link = link + os.environ['SCRIPT_NAME']
    last = link.rfind('/')
...
class new_action(object):
	...
	user = GINGIN_USER.get_login_user()
	if not GINGIN_USER.authorize(user, self._act_name, doc_id):
...
# is a respository of variables from user clients.
...
class CGI_form(action_form):
	...
	try:
	    ...
	    pass
	try:
	    pinfo = split(os.environ['PATH_INFO'], '/')[2:]
	    i = 0
    ...
    ## Setup form variables from variables of CGI form and PATH_INFO.
...
class default_act(new_action):
    ...
    def __call__(self):
        ...
        try:
            ...
            try:
                func = split(os.environ['PATH_INFO'], '/')[1]
            except:
                func = ''
                pass
            pass
...
class show_id_doc(new_action):
	...
	kws, doc_id, title, doc_body, pdate, uid = GinGin_db.get_id_doc(doc_id)
	content = {}
	creator = GINGIN_USER.get_user_from_id(uid)
...
class update(new_action):
	...
	if len(doc_body) > MAX_DOC_SZ:
        ...
        uid = GINGIN_USER.get_id_from_user(user)
        doc_id = GinGin_db.update_doc(doc_id, title, kws, doc_body, uid, update_pdate)
        represent.show_update_ok(doc_id, title)
        return None
...
class show_kw_docs(new_action):
	...
	if len(docs) > 1 or len(urls) > 0:
	    ...
	    # why not redirect ?
	    base, spath = tools.get_server_url()
...
class get_afile(new_action):
	...
	afkey = form['afkey']
	afname, mtype = GinGin_db.get_afname_mtype_by_afkey(doc_id, afkey)
	if afname != None:
...
class add_comment(new_action):
	...
	represent.show_update_ok(doc_id, '')
	pass
    pass
...
class trackback(new_action):
	...
	if form.has_key('__mod'):
		...
		if form.has_key('blog_name'):
		    blog_name = form['blog_name'].value
		else:
		    blog_name = ''
		    pass
		if len(url) > 128:
		...
		pass
                ...
                rhost = os.environ['REMOTE_ADDR'] + ':' + os.environ['REMOTE_PORT']
...
def GinGin_run_action(form, octr):
	...
	try:
	    pinfo = split(os.environ['PATH_INFO'], '/')[1:]
	except KeyError:
	    mixin(show_id_doc, test_form)
	    mixin(show_id_doc, CGI_octr)
	    act = show_id_doc()
	    act.set_form({'doc_id': '1'})
	    act.set_octr(octr)
	    return act()
	func = pinfo[0]
	pass
    ...
    try:
        mixin(clazz, CGI_form)
        mixin(clazz, CGI_octr)
        act = clazz()
        act.set_form(form)
        act.set_octr(octr)
        return act()
