...
def get_normalized_parameters(request):
    ...
    oauth_param_keys = request.oauth_params.keys()
    items = [(k, v) for k, v in request.params.iteritems() if k not in oauth_param_keys]
    oauth_items = [(k, v) for k, v in request.oauth_params.iteritems() if k != 'oauth_signature']
    items += oauth_items
