jq(document).ready(function(){
   ...
   jq("#as-reset-form").click(function(){
      for (i = 1; i < 5; i++) {
            jq("as-text"+i).val("");
        }
//      if(jq('#as-categs2 option').attr("disabled")==false){
//          jq('#as-categs2 option').remove().appendTo('#as-categs1');
//          jq('#as-categs2').html("<option value=\"\" disabled>alle Kategorien suchen</option>");
//      }
       if(jq("#as-categs2 option").length>1){
           jq('#as-categs2 option:not(:disabled)').remove().appendTo('#as-categs1');
           jq('#as-categs2').html("<option value=\"\" disabled>alle Kategorien suchen</option>");
       }

   });
   jq("#as-addcat").click(function(){
       jq('#as-categs1 option:selected').remove().appendTo('#as-categs2');
       if(jq("#as-categs2 option").length>=2){
            jq('#as-categs2 option:disabled:first').text('nur diese Kategorien');
       }
   });
   jq("#as-removecat").click(function(){
       jq('#as-categs2 option:selected').remove().appendTo('#as-categs1');
       if(jq("#as-categs2 option").length==1){
           jq('#as-categs2').html("<option value=\"\" disabled>alle Kategorien suchen</option>")
       }
   });
   ...
   jq("#searchform").submit(function(){
       jq("#as-categs2 option:not(:selected)").attr("selected","selected");
   });
   ...
   jq(".as-info-hl").toggle(
        function(){
            jq("#"+this.id).html("Info verstecken");
            jq("."+this.id).show("slow");
        },
        function(){
            jq("#"+this.id).html("mehr");
...
function makeTooltip(id,x,y){
    ...
    switch (id) {
        case "as-tip0":
            msg = "<p>Um das Verzeichnis der Suche zu ändern, nevigieren Sie<br />\
            in das gewünschte Verzeichnis und öffnen Sie die Expertensuche erneut."
            break;
        case "as-tip1":
        ...
        case "as-tip6":
            msg = "<p>Wählen Sie <b>eine/n oder mehrere Autoren</b> für ihre Suche.<br />\
                    Falls Sie ausschließlich eine Zusammenarbeit der gewünschten<br />\
                     Autoren auflisten möchten, markieren Sie die 'Kooperation' Kontrollkästchen.<br />\
                     Um mehrere auszuwählen halten Sie die Strg-taste (Ctrl) gedrückt und klicken Sie<br />\
                     mit der Maustaste auf die gewünschte Kategorien.</p>"
            break;
        default:
            break;
