...
__version__ = tuple(map(int, "$Revision: 1.1 $"[11:-2].split(".")))
# $Source: /data/cvsroot/LivingLogic/Python/core/url.py,v $

import sys, os, os.path, urllib, urllib2, new, time, mimetypes, email.Utils, mimetools, cStringIO, warnings, datetime, cgi
...
def Filename(name, scheme="file"):
	return File(name, scheme)
...
def Dirname(name, scheme="file"):
	return Dir(name, scheme)
	...
	class lastmodified(misc.propclass):
		...
		def __get__(self):
			...
			if stats is not None:
				return datetime.datetime(*time.gmtime(stats.st_mtime)[:6])
			else:
				return None
