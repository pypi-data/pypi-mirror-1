...
try:
	from ll.xist import parsers, xsc
	from ll.xist.ns import doc, html
except ImportError:
	pass
...
try:
	lastchanges = parsers.parseFile("NEWS.xml", prefixes=xsc.DocPrefixes())
	lastchanges = xsc.Frag(e for e in lastchanges[doc.section][0] if not isinstance(e, doc.title))
	lastchanges = html.astext(lastchanges.conv(), encoding="iso-8859-1").strip()
	
	header = "New in version %s" % version
	
	description = "%s\n\n\n%s\n%s\n\n%s" % (DESCRIPTION.strip(), header, "-"*len(header), lastchanges)
