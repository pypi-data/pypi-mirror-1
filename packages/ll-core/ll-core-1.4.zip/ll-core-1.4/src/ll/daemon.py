...
class Daemon(object):
	...
	"""
	def __init__(self, stdin="/dev/null", stdout="/dev/null", stderr="/dev/null", pidfile=None, user=None):
		"""
	...
	def switchuser(self, user):
		"""
		Switch the effective user. If <arg>user</arg> is <lit>None</lit> nothing
		will be done. <arg>user</arg> can be an <class>int</class> or
		<class>str</class>.
		"""
		if user is not None:
