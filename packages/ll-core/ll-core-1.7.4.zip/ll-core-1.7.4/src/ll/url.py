...
__version__ = tuple(map(int, "$Revision: 1.55.2.2 $"[11:-2].split(".")))
...
class SshSchemeDefinition(SchemeDefinition):
	def connect(self, url, context=None, remotepython="python", identity=None):
	...
	def open(self, url, mode="rb", context=None, remotepython="python", identity=None):
		connection = self.connect(url, context, remotepython, identity)
