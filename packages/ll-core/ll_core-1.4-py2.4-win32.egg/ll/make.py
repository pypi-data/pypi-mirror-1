...
__version__ = "$Revision: 1.69 $"[11:-1]
				...
				if project.showdata:
					...
					if error is not None:
						...
						if dataclass.__module__ != "__builtin__":
							text = "%s.%s @ %x" % (dataclass.__module__, dataclass.__name__, id(data))
						else:
							text = "%s @ %x" % (dataclass.__name__, id(data))
...
class FileAction(PipeAction):
	...
	def __init__(self, key, input=None):
		...
		<arg>key</arg> must be an object that provides methods <method>openread</method> and
		<method>openwrite</method> for opening readable and writable streams to the file.
			...
			if infoonly:
				...
				for (i, input) in enumerate(self.inputs):
					if alldata[i] is nodata: # we didn't get data before, but we need it now
						alldata[i] = input.get(project, bigbang)
...
class ExternalAction(PipeAction):
	...
	except that <method>execute</method> will be called even if the <arg>infoonly</arg>
	is true.
	"""
	@misc.notimplemented
...
class XISTTextAction(PipeAction):
	...
	def execute(self, project, data):
		project.writestep(self, "Converting XIST node to text")
		from ll.xist import xsc
		return data.asText()
	...
	def get(self, project, since, infoonly):
		...
		# Are we newer then the specified date?
			...
			if not infoonly or project.showinfoonly:
				project.writestep(self, "Reusing cached module ", project.strkey(url.File(self.data.__file__)))
			if infoonly:
				return newdata
			return self.data
		return nodata
...
class OracleResource(object):
	"""
	An <class>OracleResource</class> (and its two subclasses <pyref class="OracleWriteResource"><class>OracleWriteResource</class></pyref>
	and <pyref class="OracleReadResource"><class>OracleReadResource</class></pyref>)
	wraps a function or procedure in an Oracle database in a file-like API.
	"""
...
class OracleWriteResource(OracleResource):
	"""
	Wraps an Oracle function or procedure in a writable stream.
	"""
	def __init__(self, loc):
...
class OracleReadResource(OracleResource):
	"""
	Wraps an Oracle function or procedure in a readable stream.
	"""
	def __init__(self, loc):
		OracleResource.__init__(self, loc)
		self.stream = None
...
class Project(dict):
	...
	def get(self, target):
		"""
		Get up to data output data from the target <arg>target</arg> (which must
