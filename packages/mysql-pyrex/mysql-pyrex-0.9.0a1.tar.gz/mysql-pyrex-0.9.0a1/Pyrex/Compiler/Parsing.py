...
def p_statement(s, level, cdef_flag = 0, visibility = 'private'):
    #print "p_statement: %s/%s level=%r cdef=%r" % (s.sy, s.systring, level, cdef_flag) ###
    if s.sy == 'ctypedef':
...
def build_compile_time_symbols():
    import os
    d = {'True': True, 'False': False}
    (d['UNAME_SYSNAME'], d['UNAME_NODENAME'], d['UNAME_RELEASE'],
     d['UNAME_VERSION'], d['UNAME_MACHINE']) = os.uname()
    return d

def p_compile_time_statement (s, level, cdef_flag, visibility):
    kind = s.systring
    if kind == '%if':
        return p_ct_if_statement (s, level, cdef_flag, visibility)
    elif kind == '%define':
        p_ct_define_statement (s)
        # statement only for side-effect
        return []
    elif kind == '%import':
        p_ct_import_statement (s)
        # statement only for side-effect
        return []
    else:
        s.error ('expected compile-time directive')

def p_ct_if_statement(s, level, cdef_flag, visibility):
    # s.sy == '%if'
    s.next()
    cond = p_ct_expr (s, ':')
    if_clause = p_suite (s, level, cdef_flag, visibility=visibility)
    if s.sy == 'DIRECTIVE' and s.systring == '%else':
        s.next()
        else_clause = p_suite(s, level, cdef_flag, visibility=visibility)
    else:
        else_clause = None

    if eval (cond, s.context.compile_time_symbols):
        body = if_clause
    else:
        body = else_clause
...
def p_ct_define_statement (s):
    # s.sy == '%define'
    s.next()
    name = p_ident(s)
    expr = p_ct_expr (s, 'NEWLINE')
    # skip newline
    s.next()
    if not expr:
        val = True
    else:
        val = eval (expr, s.context.compile_time_symbols)
    s.context.compile_time_symbols[name] = val

def p_ct_import_statement (s):
    # s.sy == '%import'
    s.next()
    name = p_ident (s)
    s.expect_newline ('Syntax error in %import statement')
    exec "import %s" % name in s.context.compile_time_symbols

def p_ct_expr(s, stop=':'):
    # read raw tokens until we see the stop token.
    expr = []
    while s.sy != stop:
        expr.append (s.systring)
        s.next()
    return ''.join (expr)
