...
class Microblog(object):
    ...
    def update(self):
        ...
        if not lastread:
            ...
            log.info ('last ' + self.__class__.__name__.lower() + ' update time: ' + str(lastread))
            try:
    ...
    def postIt(self, items):
        ...
        for it in list(items):
            ...
            try:
                ...
                log.info(status.text + " posted on " + self.__class__.__name__.lower())
                time.sleep(5)
...
class Twitter(Microblog):
    def __init__(self,  url, username, passwd, mode='title', items = 5, shortener='tinyurl', alias = ''):
	self.alias = alias
        self.mode = mode
        self.url = url
        self.items = items
        ...
        self.rss = readrss.parse(url,self.__class__.__name__.lower())
...
class Wordpress(Microblog):
    def __init__(self,  url, username, passwd, mode='title', items = 5, shortener='tinyurl', alias='', xmlrpc_url=None):
	self.alias = alias
        self.mode = mode
        self.url = url
        self.items = items
...
class service(object):
    def __init__(self,**kwargs):
        if kwargs['service'] == 'twitter':
            self.micro = Twitter(kwargs['url'],kwargs['username'],kwargs['password'],kwargs['mode'],kwargs['items'],kwargs['shortener'],kwargs['section'])
        elif kwargs['service'] == 'identica':
            self.micro = Identica(kwargs['url'],kwargs['username'],kwargs['password'],kwargs['mode'],kwargs['items'],kwargs['shortener'],kwargs['section'])
        elif kwargs['service'] == 'wordpress':
            self.micro = Wordpress(kwargs['url'],kwargs['username'],kwargs['password'],kwargs['mode'],kwargs['items'],kwargs['shortener'],kwargs['section'], kwargs['xmlrpc_url'])
