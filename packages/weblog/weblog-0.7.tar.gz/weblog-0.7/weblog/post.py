...
date '200008-101-10' (use YYYY-MM-DD format)
    ...
    def _parse_date(date_):
        ...
        >>> Post._parse_date('2008-4-05 12:35')
        ...
        """
        date_format_list = ('%Y-%m-%d', '%y-%m-%d', '%m/%d/%Y', '%m/%d/%y',
                            '%d/%m/%y', '%d/%m/%Y')
        datetime_format_list = ['%s %%H:%%M' % f for f in date_format_list] + \
                               ['%s %%H:%%M:%%S' % f for f in date_format_list]
        for date_fmt in date_format_list:
            try:
                return datetime.strptime(date_, date_fmt).date()
            except ValueError:
                continue
        for date_fmt in datetime_format_list:
            try:
                return datetime.strptime(date_, date_fmt)
            except ValueError:
                continue
        raise ValueError('unable to parse date %r (use YYYY-MM-DD format)' % \
                         (date_))
