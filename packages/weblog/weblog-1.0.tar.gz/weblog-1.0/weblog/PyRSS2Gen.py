...
class DateElement:
    """implements the 'publish' API for a datetime.date
...
class RSS2(WriteXmlMixin):
    ...
    def publish(self, handler):
        ...
        if isinstance(pubDate, (datetime.datetime, datetime.date)):
            pubDate = DateElement("pubDate", pubDate)
        _opt_element(handler, "pubDate", pubDate)

        lastBuildDate = self.lastBuildDate
        if isinstance(lastBuildDate, (datetime.datetime, datetime.date)):
