...
class TestSimpleLoad(unittest.TestCase):
    ...
    def test_load_post_list(self):
        post_list = load_post_list('test/simple/', 'ascii', '')
    ...
    def test_load_post_list_encoding_failure(self):
        self.assertRaises(PostError, load_post_list, 'test/encoding/', 'ascii',
                          '')

    def test_load_post_list_encoding(self):
        post_list = load_post_list('test/encoding/', 'UTF-8', '')
...
class TestConfiguration(unittest.TestCase):
    ...
    def test_empty(self):
        with self.__config() as f:
            self.assertEqual(configuration.read(f.name), dict())
    ...
    def test_encoding(self):
        with self.__config("encoding = 'latin-1'") as f:
            self.assertEqual(configuration.read(f.name),
