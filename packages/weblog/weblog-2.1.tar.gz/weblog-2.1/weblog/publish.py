...
def load_post_list(path, default_encoding, default_author,
    ...
    for post in post_list:
        ...
        params.update(dict(title=post.title,
                           ...
                           content=html_full_url(top_dir,
                                                 post.get_html()),
                           top_dir=top_dir))
        _write_file(os.path.join(dir, post.slug + '.html'),
                    config.get('encoding'),
...
def command_publish(args, options):
    ...
    try:
        post_list = list(load_post_list(source_dir,
                                        (config.get('encoding') or
                                         locale.getpreferredencoding()),
                                        (config.get('author') or u''),
                                        (config.get('filesystem_encoding') or
                                         locale.getpreferredencoding())))
        post_list.sort()
        post_list.reverse()
