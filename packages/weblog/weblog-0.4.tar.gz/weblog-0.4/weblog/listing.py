...
class PageIndex(Page):
    '''
    special case to have a page that returns 'index' as filename
    '''
    def filename(self):
        return 'index.html'
...
def generate_index_listing(limit, output_dir, template, post_list,
        template_params):
    '''
...
# list() needed since pages is subscripted later
    ...
    if pages:
        pages = [PageIndex('0', pages[0])] + [Page(str(k + 1), v) for k, v in
            enumerate(pages[1:])]
...
def generate_monthly_listing(output_dir, template, post_list, template_params):
    pages = slice_list_groupby(post_list, lambda x: str(x.date.year) + '-' +
            str(x.date.month))
