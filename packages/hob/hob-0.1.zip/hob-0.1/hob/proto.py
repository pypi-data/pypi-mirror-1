...
__all__ = ("ProtoError", "ConfigError",
           ...
           "Field", "Message", "defaultMessage", "Command", "Request", "Event", "Service",
           "assignInternalId", "iterTree",
           "ValidationError", "ErrorType", "Error", "Validator", "OperaValidator",
           "ServiceManager", "PackageManager", "iterProtoFiles", "loadService",
...
# Our options
# 5000-5049 = scope options
# 5050-5079 = cpp options
# 5080-5109 = java options
# 5110-5129 = python options
# 5130-5159 = javascript options
_options.setOptionValues("MessageOptions",
                         [])
#                    [(Option("cpp_send_message", 5050), OptionValue(False, raw="false")),
#                     ])
_options.setOptionValues("ServiceOptions",
                         [(Option("version",      5000), OptionValue("", raw="")),
                          (Option("core_release", 5001), OptionValue("", raw="")),
                          (Option("cpp_class",    5050), OptionValue("", raw="")),
                          (Option("cpp_hfile",    5051), OptionValue("", raw="")),
                          ])
...
_options.setOptionValues("MethodOptions",
                         [(Option("cpp_response", 5052), OptionValue("direct", raw="direct")),
                          ])
...
class Enum(object):
    def __init__(self, name, values=None, options=None, doc=None):
        self.name = name
        if values == None:
            values = []
        self.values = values
    ...
    def isCustomOption(self, option):
        return _options.isCustomOption("EnumOptions", option)

class EnumValue(object):
    def __init__(self, name, value, options=None, doc=None):
        self.name = name
        self.value = value
        self.options = Options(options or [])
        if doc != None and not isinstance(doc, DocBlock):
    ...
    def isCustomOption(self, option):
        return _options.isCustomOption("EnumValueOptions", option)
...
class Package(object):
    ...
    def find(self, path, current):
        ...
        def msg_find(msg, path):
            ...
            return msg
        def pkg_find(pkg, path):
            for msg in self.messages:
                if msg.name == path[0]:
                    if path[1:]:
                        msg = msg_find(msg, path[1:])
                    return msg
        ...
        if current:
            ...
            for child in current.children:
                if child.name == path[0]:
                    ...
                    if item:
                        return item
            for enum in current.enums:
                if enum.name == path[0]:
                    if path[1:]:
                        raise NameError("Invalid path '%s'" % ".".join(path))
                    return enum

        item = pkg_find(self, path)
        if item:
            return item
        for pkg in self.imports:
            if not pkg.name:
                item = pkg_find(pkg, path)
            else:
                ...
                if pkg_path != path[0:len(pkg_path)]:
                    continue
                item = pkg_find(pkg, path[len(pkg_path):])
            if item:
                return item
    ...
    def isCustomOption(self, option):
        return _options.isCustomOption("FileOptions", option)

class Field(object):
    def __init__(self, type_, name, number, q=None, default=None, message=None, options=None, doc=None):
        self.type = type_
        self.number = number
        self.name = name
        if not q:
    ...
    def isCustomOption(self, option):
        return _options.isCustomOption("FieldOptions", option)
...
class Message(object):
    ...
    def __init__(self, name, fields=None, doc=None, update=True, internal_id=None, parent=None, children=None, items=None, options=None, is_extension=False):
        self.name = name
        if fields == None:
            fields = []
        self.fields = fields
    ...
    def isCustomOption(self, option):
        return _options.isCustomOption("MessageOptions", option)
...
class Command(object):
    ...
    def isCustomOption(self, option):
        return _options.isCustomOption("MethodOptions", option)
...
def iterTree(messages, existing=None, with_global=True, with_inline=True):
    ...
    while messages:
        ...
        for field in message.fields:
            if field.type != Proto.Message or not field.message or field.message in existing:
                continue
            if with_global and field.message.parent:
                continue
...
class Service(object):
    ...
    def __init__(self, name, commands=None, options=None, doc=None):
        self.name = name
    ...
    def messages(self, only_global=True):
        ...
        def field_messages(message):
            ...
            while new_fields:
                ...
                new_fields = []
                for field in fields:
                    if field.type == Proto.Message and field.message and (not only_global or field.message.parent == None) and field.message not in messages:
                        yield field.message
    ...
    def isCustomOption(self, option):
        return _options.isCustomOption("ServiceOptions", option)
...
class ServiceManager(object):
    ...
    def loadFile(self, path, validate=True):
        if path.endswith(".py"):
            ...
            for item in g.itervalues():
                if issubclass(type(item), Service):
                    pkg.addService(item)
                elif issubclass(type(item), Message) and item.parent == None:
                    pkg.addMessage(item)
        ...
        for service in pkg.services:
            assignInternalId(service.messages(False))
...
class Target(object):
    ...
    def services(self):
        if "target." + self.name not in self.config:
            return
        for name, value in self.config["target." + self.name]:
            yield name
