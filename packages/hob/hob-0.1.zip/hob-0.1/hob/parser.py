...
def create_lexer_rules():
    ...
    rules = [(r'[ \t\r\n]+', ignore_WHITESPACE),
             ...
             (r'"[^"]*"', STRING),
             (r'[a-zA-Z][a-zA-Z0-9_]*([.][a-zA-Z][a-zA-Z0-9_]*)*', IDENTIFIER),
             (r'\[', L_SQUARE_BRACKET),
             (r'\]', R_SQUARE_BRACKET),
             (r'{', L_CURLY_BRACKET),
             (r'}', R_CURLY_BRACKET),
...
keywords = ("IDENTIFIER", "PACKAGE", "IMPORT", "MESSAGE", "ENUM",
...
#        self.debug("\n")
    ...
    def program(self):
        ...
        if self.require_syntax or t == "SYNTAX":
            self.match("SYNTAX")
            self.match("EQ")
            syntax = self.match("IDENTIFIER")
            self.match("SEMI")
            if syntax.value not in ("proto2", "scope"):
    ...
    def message(self, token):
        ...
        while True:
            ...
            if t in ("MESSAGE", "EXTEND", "ENUM", "OPTION"):
                ...
                t_type = self.match(("TYPE", "IDENTIFIER")) # Type or message-name
    ...
    def service(self):
        ...
        while True:
            ...
            if t == "OPTION":
                ...
                self.match("L_PAREN")
                t_in = self.matchIdentifier() # RPC input
                self.match("R_PAREN")

                self.match("RETURNS")
                self.match("L_PAREN")
                t_out = self.matchIdentifier() # RPC response
                self.match("R_PAREN")
...
class Symbol(object):
    def __init__(self, name, owner=None, attribute=None, kind=None, current=None):
        self.name = name
        self.owner = owner
...
#            raise BuildError("Message %s not found, cannot extend it" % name)
    ...
    def on_field_open(self, quantifier, symbol, name, tag):
        ...
        if type_ == Proto.Message:
            sym = field.message = Symbol(symbol, field, "message", (Message, Enum), self._item)
