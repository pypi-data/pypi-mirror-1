...
class AjaxPaginatedGrid(DataGrid):
    ...
    params = ['search_params', 'url', 'var_name', 'no_init']
    options = {}
    search_params = {}
    url = ''
    var_name = 'list'
...
def jsonify_datagrid(datagrid, list_name, *deco_args, **deco_kw):
    ...
    def entangle(func): 
        def decorated(func, *args, **kw): 
            output = func(*args, **kw)
            if output.get(list_name):
                if callable(datagrid):
                    datagrid_ = datagrid(args[0])
                else:
                    datagrid_ = datagrid
                data = output[list_name]
                prepared_data = list()
                for row in data:
                    data_row = dict()
                    for col in datagrid_.columns:
...
def is_Element_or_kidXML(data):
    ...
    if hasattr(data, 'tag') and hasattr(data, 'attrib'):
        return True
    try:
