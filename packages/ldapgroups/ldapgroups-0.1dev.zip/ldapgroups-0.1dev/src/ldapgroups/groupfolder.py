...
from zope.app.authentication.interfaces import IAuthenticatorPlugin, IQueriableAuthenticator, IQuerySchemaSearch
...
class LDAPGroupInformation(Persistent):
    implements(IReadGroupInformation)
...
class LDAPGroups(Persistent, Contained):
    ...
    def getGroupsForPrincipal(self, principalid):
        ...
        res = self._search({'member':internal_id, 'objectClass':'group'}, attrs=[self.groupIdAttribute], util=util)
        for dn, entry in res:
    ...
    def getPrincipalsForGroup(self, groupid):
        ...
        res = self._search( {'memberOf':internal_id, 'objectClass':'user'}, util.searchBase, util.searchScope, 
                           attrs=[util.idAttribute], util=util)
        for dn, entry in res:
            principal_id = entry[util.idAttribute][0]
...
#        raise NotImplementedError('%s object are readonly' % self.__class__.__name__)
#    
