...
from peak.rules import Method, Around, Before, always_overrides, \
                       AmbiguousMethods, Dispatching, rules_for, parse_rule
...
class PrioritizedMethod(Method):
    ...
    def make_decorator(cls, name, doc=None, default_prio=0):
        ...
        if cls is Method:
            ...
            maker = cls.make
        def decorate(f, pred=(), prio=default_prio, depth=2, frame=None):
...
always_overrides(Before, PrioritizedMethod)
always_overrides(PrioritizedAround, Around)

prioritized_when = PrioritizedMethod.make_decorator(
    'prioritized_when',
    """
...
prioritized_around = PrioritizedAround.make_decorator(
    'prioritized_around',
    """
