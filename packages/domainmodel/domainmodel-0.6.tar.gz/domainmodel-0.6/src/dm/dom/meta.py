...
class MarkdownText(Text):
    ...
    # Todo: Figure out how to render separately from read value?
    #     Ans: Implement this method:
    #def createLabelRepr(self, domainObject):
...
class Boolean(ValueObjectAttr):
    ...
    def makeValueFromMultiValueDict(self, multiValueDict):
        ...
        if boolVal.__class__ == False.__class__:
            ...
            attrValue = boolVal.lower() in self.trueStrings
        else:
            attrValue = False
        if moddebug and self.debug:
            msg = "Made %s value: %s" % (self.name, attrValue)
...
class Integer(ValueObjectAttr):
    ...
    def makeValueFromMultiValueDict(self, multiValueDict):
        integerValue = multiValueDict[self.name]
        if integerValue:
            attrValue = int(integerValue)
        else:
            if hasattr(self, 'default'):
...
class DomainObjectRef(DomainObjectAssociation):
    ...
    def createLabelRepr(self, domainObject):
        ...
        if value == None:
            ...
            return unicode(value.getLabelValue())
        else:
            return unicode(value)
