...
class ControlledAccessView(object):
    ...
    if AUTH_COOKIE_DOMAIN in dictionary:
        ...
        authCookieDomain = '.' + dictionary[DOMAIN_NAME]
    ...
    def __init__(self, request=None, masterView=None, **kwds):
        ...
        self.redirectPath = ''
        self.returnPath = ''
        if self.isSlaveView(): self.initFromMaster()
    ...
    def makeSessionKeyFromCookieString(self, cookieString):
        ...
        return validateCookieStrategy.validate()
...
class SessionView(ControlledAccessView):
    ...
    def readRequestSession(self):
        ...
        if not cookieString:
            if self.debug:
                msg = 'No authentication session cookie in request.'
                self.logger.debug(msg)
            return
        if cookieString == self.noAuthSessionCookieString:
            if self.debug:
                msg = 'Unauthenticated session cookie in request.'
                self.logger.debug(msg)
            return
        if self.debug:
            msg = 'Session cookie in request... (%s)' % (
        ...
        if self.session:
            ...
            if moddebug and self.debug:
                msg = 'No session for cookieString: %s' % cookieString
                self.logger.debug(msg)
    ...
    def getRequestParams(self):
        if self.requestParams == None:
            if moddebug and self.debug:
                msg = "Reading request params from request..."
                self.logger.debug(msg)
            if self.request.POST:
        ...
        if moddebug and self.debug:
            msg = "Request params: %s" % self.requestParams
            self.logger.debug(msg)
    ...
    def setContext(self, **kwds):
        ...
        pageCreated = dm.times.getLocalNowCWithZone()
    ...
    def setSessionCookie(self):
        ...
        if self.session:
            ...
            cookieString = self.noAuthSessionCookieString
        cookieName = self.authSessionCookieName
        ...
        if self.getRequestParam('rememberme'):
            ...
            maxAge = None
        if self.debug:
            msg = 'Setting cookie %s: %s' % (cookieName, cookieString)
            self.logger.debug(msg)
        webkitName = self.dictionary[WEBKIT_NAME]
        if webkitName == 'django':
            ...
            print "Todo: How to set the pylons cookie? Use AuthKit"
...
class AbstractInstanceView(AbstractClassView):
    ...
    def buildManipulator(self):
        ...
        if not issubclass(manipulatorClass, DomainObjectManipulator):
            msg = "Manipulator class isn't a DomainObjectManipulator: %s" % (
                manipulatorClass
            )
            raise Exception(msg)
        ...
        if moddebug and self.debug:
            msg = "Building manipulator %s for position %s" % (
    ...
    def manipulateDomainObject(self):
        ...
        if not self.getFormErrors():
            ...
            if not manipulator.domainObject:
                raise Exception("Create manipulator did not produce an object.")
