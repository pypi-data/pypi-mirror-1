...
class TestProtectionObject(TestCase):
    ...
    def test_makeProtectedNames(self):
        domainClassName = 'State'
        protectedObject = self.registry.getDomainClass(domainClassName)
        poClass = self.registry.getDomainClass('ProtectionObject')
        protectedNames = poClass.makeProtectedNames(protectedObject)
        self.failUnless(domainClassName in protectedNames)
