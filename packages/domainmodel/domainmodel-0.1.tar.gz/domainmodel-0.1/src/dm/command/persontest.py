...
class TestPersonCreate(TestCase):
    ...
    def tearDown(self):
        try:
            person = self.registry.persons.all[self.fixtureName]
            person.delete()
            person.purge()
        except:
            pass
    ...
    def testExecute(self):
        self.failIf(self.fixtureName in self.registry.persons)
        self.command.execute()
        self.failUnless(self.fixtureName in self.registry.persons)
...
class TestPersonUndelete(TestCase):
    ...
    def testExecute(self):
        self.failUnless(self.fixtureName in self.registry.persons.deleted)
        self.command.execute()
        self.failIf(self.fixtureName in self.registry.persons.deleted)
        self.failUnless(self.fixtureName in self.registry.persons)
...
class TestPersonPurge(TestCase):
    ...
    def testExecute(self):
        ...
        self.failIf(self.fixtureName in self.registry.persons.deleted)
        self.failIf(self.fixtureName in self.registry.persons)
        self.failIf(self.fixtureName in self.registry.persons.all)
...
class TestPersonRead(TestCase):
    ...
    def setUp(self):
        ...
        try:
            self.person = self.registry.persons.create(self.personName)
        except:
            person = self.registry.persons.all[self.personName]
            person.delete()
            person.purge()
            raise
...
class TestPersonList(TestCase):
    ...
    def testExecute(self):
        self.command.execute()
        self.command.persons[self.personName]
