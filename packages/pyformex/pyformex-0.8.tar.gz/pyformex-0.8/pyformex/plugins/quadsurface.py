...
##
"""Surface models using discrete quadrilaterals.

(c) 2009 Benedict Verhegghe
This module defines a class 'QuadSurface' that deals with surfaces modeled
by a collection of discrete quadrilaterals.
Most discrete models for surfaces use triangles as the base object.
In Finite Element Analysis however, quadrilaterals are often more
adequate than triangles, whenever the global geometry permits the use
of rather regular shaped ones.
"""
...
class QuadSurface(object):
    ...
    def __init__(self,*args):
        ...
        if len(args) == 1:
            ...
            a = asarray(args[1])
            if a.dtype.kind != 'i' or a.ndim != 2 or (
                len(args) == 2 and a.shape[1] != 2 ) or (
                len(args) == 3 and a.shape[1] != 4 ):
            ...
            if len(args) == 2:
                ...
                a = asarray(args[2])
                if not (a.dtype.kind == 'i' and a.ndim == 2 and a.shape[1] == 4):
