...
def is_sphinx_installed():
    """Check to see if Sphinx is installed both by setuptools require and simple
    import. Return True or False based on results.
    """
...
def list_docs_data_files(docs_location):
    ...
    for root, dirs, files in os.walk(TARGET_DIR, topdown=True):
        ...
        if len(files) == 0:
            continue
        install_dir = root.replace(TARGET_DIR, docs_location)
        return_list.append(
...
class BuildDocs(Command):
    ...
    def run(self):
        ...
        if self.distribution.docs_in_egg:
            ...
            self.distribution.data_files.extend(list_docs_data_files
    ...
    def finalize_options (self):
        ...
        if self.source_dirs is None:
            self.source_dirs = get_source_dirs()
