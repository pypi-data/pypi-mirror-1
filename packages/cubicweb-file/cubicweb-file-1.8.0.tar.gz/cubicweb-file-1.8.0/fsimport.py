...
def import_directory(cw, directory, mapfolders=True, bfss=False,
    ...
    if mapfolders:
        ...
        _alreadyimported = set(x.getvalue() for x, in cw.execute(
            "Any fspath(X, 'File', 'data') WHERE X is File, X filed_under F, F eid %(f)s",
            {'f': _parent.eid}, 'f'))
        _alreadyimported |= set(x.getvalue() for x, in cw.execute(
            "Any fspath(X, 'Image', 'data') WHERE X is Image, X filed_under F, F eid %(f)s",
            {'f': _parent.eid}, 'f'))
    elif _alreadyimported is None:
        _alreadyimported = set(x.getvalue() for x, in cw.execute(
            "Any fspath(X, 'File', 'data') WHERE X is File"))
        _alreadyimported |= set(x.getvalue() for x, in cw.execute(
            "Any fspath(X, 'Image', 'data') WHERE X is Image"))
    if not quiet:
