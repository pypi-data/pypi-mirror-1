...
import weakref
class MaybeWeakValueDict(object, DictMixin):
    '''
...
class PickleDict(object, DictMixin):
    ...
    '''
    special_character = '+'
    known_items = ['logger', 'log', 'dir', 'gzip', 'store_metadata', 'history_len', 'cache']
    
    def __init__(self, dir, gzip=True, store_metadata=True, log=True, context_history_len=20):
        self.logger = logging.getLogger('csc.divisi.util.PickleDict')
        self.log = log
    ...
    def __setattr__(self, key, val):
        if not key.startswith('_') and key not in self.known_items and self.log:
            self.logger.warn('Setting attributes on PickleDict instances isn\'t persistent. You probably intended to use pickle_dir.d.attribute instead.')
    ...
    def get_lazy(self, key, thunk, version=None):
        ...
        result = thunk()
        self[key] = result
        self.set_meta(key, 'version', version)
        return self[key]
    ...
    def lazy(self, name=None, version=None):
        ...
        def dec(thunk): # gets called with the actual thunk
            ...
            return wraps(thunk)(lambda: self.get_lazy(key, thunk, version))
        return dec
