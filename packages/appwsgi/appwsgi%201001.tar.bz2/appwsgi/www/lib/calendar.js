...
calendar=
{
 ...
 'overview':function()
 {
  ...
  {
   var i,v,t
   var c=xml.getElementsByTagName('calendar')
   var a=xml.getElementsByTagName('appointment')
   for (i=0;i<c.length;i++) //ie7
   {
    if (v=c[i].childNodes)
    {
     t=c[i].childNodes[0].nodeValue
     t=t.match(/-([^-]*?) /)[1]
     v=a[i].childNodes[0].nodeValue
     document.getElementById(t).innerHTML+='<p>'+v+'</p>'
  ...
  http.xml+='<root>\n'
  http.xml+=' <cmd>overview</cmd>\n'
  http.xml+=' <gid>'+document.getElementById('view').innerHTML+'</gid>\n'
  var m=calendar.currentMonth+1
  var y=calendar.currentYear
  http.xml+=' <from>'+calendar.currentYear+'-'+m+'-1 00:00:00</from>\n'
  m++;if(m>12){m=1;y++;}
  http.xml+=' <to>'+y+'-'+m+'-1 00:00:00</to>\n'
  http.xml+='</root>'
  http.send()
