...
def application(environ, response):
    ...
    if s.GID=='guest':
        if   v['cmd']=='find'  : db.execute("SELECT * FROM shop_products WHERE description LIKE ?",('%'+v['pna']+'%',))
             ...
             db.execute("INSERT INTO shop_orders (products,uid,bid,time) VALUES (?,?,?,DATETIME('NOW'))",(dumps(v['rec']),s.UID,3))
             oid=db.LASTROWID
             insert(db,oid)
             db.LASTROWID=oid
    ...
    if s.GID=='admin':
        if   v['cmd']=='find'  : db.execute("SELECT * FROM shop_products WHERE description LIKE ?",('%'+v['pna']+'%',))
             ...
             db.execute("INSERT INTO shop_orders (products,uid,bid,time) VALUES (?,?,?,DATETIME('NOW'))",(dumps(v['rec']),v['uid'],3))
             oid=db.LASTROWID
             insert(db,oid)
             db.LASTROWID=oid
...
def insert(db,oid):
    ...
    for i in p:
        ...
        qty = db.fetch()[0][0]
        if qty < 0: db.execute('UPDATE shop_orders SET bid=2 WHERE oid=?',(oid,))
