...
register=
{
 ...
 'session':function(cmd)
 {
  http.fml=function(xml)
  {
   ...
   obj.id='picture'
   obj.style.background='url(../lib/download.py?picture='+Math.random()+') no-repeat'
   obj.type="application/x-shockwave-flash"
   obj.data="../lib/upload.swf?url=../lib/upload.py"
   var param = document.createElement("param");
   param.setAttribute('name','movie');
   param.setAttribute('value','../lib/upload.swf?url=../lib/upload.py');
 ...
 'passwd':function()
 {
  ...
  http.xml ='<?xml version="1.0" encoding="UTF-8"?>\n'
  http.xml+='<root>\n'
  http.xml+=' <cmd>passwd</cmd>\n'
  http.xml+=' <gid>guest</gid>\n'
  http.xml+=' <pwd>'+sha1.msg(pwd)+'</pwd>\n'
  http.xml+='</root>'
  http.url='register.py'
  http.send()
 }
...
upload=
{
 ...
 'complete':function(n)
 {
  ...
  document.getElementById('picture').style.background='url(../lib/download.py?picture='+Math.random()+') no-repeat'
 },
