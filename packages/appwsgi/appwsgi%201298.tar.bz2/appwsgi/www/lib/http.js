  ...
  xmlHttp.onreadystatechange=function()
  {
   switch(xmlHttp.readyState)
   {
    ...
    case 4:
     ...
     try{v=JSON.parse(xmlHttp.responseText)}catch(e){http.fml(xmlHttp.responseText);break}
  ...
  xmlHttp.onreadystatechange=function()
  {
   switch(xmlHttp.readyState)
   {
    ...
    case 4:
     var v
     if(v=document.getElementById('status')){v.innerHTML=''}
     try{v=JSON.parse(xmlHttp.responseText)}catch(e){xmlHttp.fml(xmlHttp.responseText);break}
     if(v.error){alert(v.error)}else{xmlHttp.fml(v)}
    break
   }
  }
