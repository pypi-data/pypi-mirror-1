...
init=function()
{
 try{forum.topic=document.URL.match(/\?(.*)/)[1]}catch(e){forum.topic='-1'}
 if(forum.topic=='-1'){forum.find('topics')}else{forum.find('threads')}
 document.getElementById('session').style.background='url(../../wsgi/download.wsgi?'+session.sid+Math.random()+') no-repeat'
}
