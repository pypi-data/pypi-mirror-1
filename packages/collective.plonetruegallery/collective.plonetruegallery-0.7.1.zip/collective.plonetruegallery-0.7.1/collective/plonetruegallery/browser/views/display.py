...
class BaseDisplayType(object):
    ...
    def __init__(self, gallery, request):
        ...
        self.starter_image_index = 0
        if request.has_key('start_image'):
            si = request.get('start_image', '')
            images = self.adapter.get_all_cooked_images()
            for index in range(0, len(images)):
                if si == images[index]['title']:
                    self.starter_image_index = index
...
class ClassicDisplayType(BaseDisplayType):
    ...
    name = u"classic"
    description = _(u"label_classic_display_type",
        default=u"The original slideshow for plonetruegallery")
...
class SlideshowDisplayType(BaseDisplayType):
    ...
    description = _(u"label_slideshow_display_type",
        default=u"Slideshow 2 javascript gallery")
