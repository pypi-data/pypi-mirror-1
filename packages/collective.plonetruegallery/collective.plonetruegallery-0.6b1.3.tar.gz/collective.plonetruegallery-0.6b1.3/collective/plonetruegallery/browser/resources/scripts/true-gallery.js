...
TrueGallery = function(element, options){
    ...
    //to protect against click happy people
    tg.requestMoreImages = false;
    ...
    */
    ...
    tg.triggerLoadMoreImages = function(){
        if(tg.jcar.size() < (tg.jcar.last + tg.preloadImageOffset) && !tg.doneLoading && !tg.requestMoreImages){
            jq(tg.selectors.loadMoreImagesButtonSelector).trigger('click');
            tg.requestMoreImages = true;
        }
    };
    ...
    */
    ...
    tg.add = function(image){
        ...
        var carouselimg = jq('<img class="kssattr-index-' + index + '" src="' + image.thumb_url + 
        ...
        var imagehtml = '<div id="image-hash-' + image.hash + '" class="image kssattr-link-' + image.link + ' kssattr-index-' + index + '">' +
    ...
    tg.addAll = function(images){
        images.each(function(index, image){
            if(jq("div#image-hash-" + image.hash).size() == 0){//check if image already added first...
                tg.add(image);
            }
        });
