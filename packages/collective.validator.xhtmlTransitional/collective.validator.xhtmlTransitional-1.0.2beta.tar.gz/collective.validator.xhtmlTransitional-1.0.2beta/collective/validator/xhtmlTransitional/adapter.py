...
from collective.validator.xhtmlTransitional.interfaces import *

class W3cTransitional(Parser):

    security = ClassSecurityInfo()

    implements(IW3cTransitional)
    adapts(IVTPlone)

    val_url = "http://validator.w3.org/check"
    val_type = "XHTML 1.0 Transitional"
    ...
    def getValidationResults(self, xhtml):
        ...
        try:
            f = urlopen(W3cTransitional.val_url, params)
        except IOError:
            return 0
    ...
    def runValidation(self):
        ...
        for document in results:
            ...
            if shasattr(document,'view'):
                validation = self.getValidationResults(document.view().encode('utf-8'))
            else:
                try:
                    request = TestRequest()
                    validation=self.getValidationResults(getMultiAdapter((document,request),'view').encode('utf-8'))
                except:
                    try:
                        validation = self.getValidationResults(document().encode('utf-8'))
                    except:
                        validation= self.getValidationResults(document.unrestrictedTraverse('base_view').encode('utf-8'))
        ...
        str += self.translation_service.utranslate( \
                                           domain='collective.validator.xhtmlTransitional', \
                                           msgid='transitional_portal_message', \
                                           default="generated", \
    ...
    def runDebugValidation(self):
        ...
        if self.context.getSendReportDebug() and self.context.getEmailAddressesDebug():
            self.sendReportAsAttach(self.createReportMail(fileout,toterr, totwarn,len(portal_types)),("portal-validation-report",".html"))
        
        str = "<a href='portal_validationtool/%s'>report</a> debug %s " %(id_report,self.val_type)
        str += self.translation_service.utranslate( \
                                           domain='collective.validator.xhtmlTransitional', \
                                           msgid='transitional_portal_message', \
                                           default="generated", \
