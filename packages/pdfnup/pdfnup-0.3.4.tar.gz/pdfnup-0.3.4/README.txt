...
This release fixes a rotation buglet, adds a verbose option and allows
file objects for input documents.
...
interactive Python session::
    ...
    >>>
    >>> f = open("file.pdf")
    >>> generateNup(f, 8, outputPat="out-%(n)dup.pdf", verbose=True)
    written: out-8up.pdf
...
the file ``pdfnup-0.3.4.tar.gz`` and decompressing it with the following 
command::

  $ tar xfz pdfnup-0.3.4.tar.gz
...
initial PDF files for the test suite.
