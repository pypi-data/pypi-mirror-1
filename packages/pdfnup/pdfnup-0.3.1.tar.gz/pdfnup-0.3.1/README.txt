...
the file ``pdfnup-0.3.1.tar.gz`` and decompressing it with the following 
command::

  $ tar xfz pdfnup-0.3.1.tar.gz
...
Version 0.3.1 no longer needs the `ReportLab toolkit 
<http://www.reportlab.org/downloads.html>`_ to be installed, except 
for running the Python script `genpdf.py` generating the initial PDF 
