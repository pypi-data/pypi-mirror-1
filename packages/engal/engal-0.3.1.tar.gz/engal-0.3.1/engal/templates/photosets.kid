...
<div class="photoset_info">
  <div><span class="photoset_name">${photoset.title}</span> <span>(created by <a href="${tg.url('/gallery/%s' % (photoset.user.user_name))}">${photoset.user.user_name}</a>)</span></div>
...
<div class="thumb">
<A HREF="${tg.url('/gallery/%s/images/%s/set-%s' % (photoset.user.user_name, photo.name, photoset.name))}"><IMG SRC="${tg.url('/image/%d/thumbnailbox/100' % (photo.id))}" BORDER="0" /></A>
</div>
</div>
</div>
...
<div id="addphoto" class="invisible" py:content="photo_form(action=tg.url('/addPhoto'), value={'photoset_id': photoset.id, 'return_path': '/gallery/%s/sets/%s' % (user.user_name, photoset.name)})">Add photo</div>
