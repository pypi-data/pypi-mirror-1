...
package. We create a setup.py file that looks like this::
  ...
  #{{{cog include('started/oldway/setup.py')}}}
...
to build a source distribution::
  ...
  -rw-r--r--  1 admin  staff  806 May  7 22:35 TheOldWay-1.0.tar.gz
  # {{{end}}}
...
the users. We end up creating a shell script to do this::
  ...
  # {{{cog include("started/oldway/builddocs.sh")}}}
  cd docs
  make html
  cd ..
  rm -rf oldway/docs
...
function from our Old Way setup.py::
  ...
  # {{{cog include("started/oldway/setup.py", "setup")}}}
  setup(
      name="TheOldWay",
      packages=['oldway'],
      version="1.0",
...
options. Here's what the conversion looks like::
  ...
  # {{{cog include('started/newway/pavement.py', 'setup')}}}
  options(
      setup=Bunch(
          name="TheNewWay",
          packages=['newway'],
...
Want proof? How about looking at the output of ``paver help setup``::
  ...
  # {{{cog sh('cd docs/samples/started/newway; paver help setup')}}}
  ---> help
  Paver 0.7

  Usage: paver [global options] task [task options] [task...]
...
equivalent output file::
  ...
  # }}}
  total 8
  -rw-r--r--  1 admin  staff  2606 May  7 22:35 TheNewWay-1.0.tar.gz
  # {{{end}}}
...
Worried about bloating your package? The paver-minilib is not large::
  ...
  # }}}
  -rw-r--r--  1 admin  staff  24404 May  7 22:35 docs/samples/started/newway/paver-minilib.zip
  # {{{end}}}
...
pavement.py::
  ...
  # {{{cog include('started/newway/pavement.py', 'sdist')}}}
  @task
...
``paver help html`` will tell us how to use the html command::
  ...
  # {{{cog sh('paver help paver.doctools.html')}}}
  ---> help
  Paver 0.7
...
using a builddir called "_build". Let's add this to our pavement.py::
  ...
  # {{{cog include('started/newway/pavement.py', 'sphinx')}}}
  options(
      sphinx=Bunch(
          builddir="_build"
      )
...
here::
  ...
  # {{{cog include('started/newway/pavement.py', 'html')}}}
  @task
...
docs directory that we'll be moving::
  ...
  # {{{cog include('started/newway/pavement.py', 'html.builtdocs')}}}
  builtdocs = path("docs") / options.builddir / "html"
  # {{{end}}}
...
document file to see the first part::
  ...
  # {{{cog include("started/newway/docs/index.rst", "mainpart")}}}
...
at that file::
  ...
  # {{{cog sh("cat docs/samples/started/newway/newway/thecode.py") }}}
...
our deploy task::
  ...
  # {{{cog include('started/newway/pavement.py', 'deployoptions')}}}
  options(
      deploy = Bunch(
...
the options. Now for the deploy task itself::
  ...
  # {{{cog include("started/newway/pavement.py", "deploy")}}}
  @task
  @cmdopts([
...
what the commands will be::
  ...
  # {{{cog sh("cd docs/samples/started/newway; paver -n deploy -u kevin")}}}
  ---> deploy
