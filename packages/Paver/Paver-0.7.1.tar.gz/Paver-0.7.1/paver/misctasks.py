...
def minilib():
    ...
    """
    import tempfile
    tempdir = path(dry("Create tempdir", tempfile.mkdtemp))
    try:
        destdir = tempdir / "paver"
        destdir.mkdir()
        import paver
        ...
        def generate_zip():
            import zipfile
            destfile = zipfile.PyZipFile(output_file, "w", zipfile.ZIP_DEFLATED)
            destfile.writepy(destdir)
            destfile.close()
