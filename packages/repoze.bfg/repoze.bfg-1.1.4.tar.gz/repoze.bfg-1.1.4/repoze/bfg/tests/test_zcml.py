...
class TestViewDirective(unittest.TestCase):
    ...
    def test_add_view_with_accept_multiview_replaces_existing_view(self):
        ...
        def view2(context, request):
            return 'OK2'
        sm.registerAdapter(view, (Interface, IRequest), IView, name='')
        self._callFUT(context, 'repoze.view', Interface, view=view2,
                      accept='text/html')
        actions = context.actions
        self.assertEqual(len(actions), 1)
        action = actions[0]
        register = action['callable']
        register()
        wrapper = sm.adapters.lookup((Interface, IRequest), IView, name='')
    ...
    def test_add_view_multiview_context_superclass_then_subclass(self):
        ...
        context = DummyContext()
        sm = getSiteManager()
        sm.registerAdapter(view, (ISuper, IRequest), IView, name='')
        self._callFUT(context, 'repoze.view', ISub, view=view2)
        actions = context.actions
        self.assertEqual(len(actions), 1)
        action = actions[0]
        register = action['callable']
        register()
        wrapper = sm.adapters.lookup((ISuper, IRequest), IView, name='')
        self.failIf(IMultiView.providedBy(wrapper))
        self.assertEqual(wrapper(None, None), 'OK')
        wrapper = sm.adapters.lookup((ISub, IRequest), IView, name='')
        self.failIf(IMultiView.providedBy(wrapper))
        self.assertEqual(wrapper(None, None), 'OK2')
    ...
    def test_add_view_multiview_request_superclass_then_subclass(self):
        ...
        self.failIf(IMultiView.providedBy(wrapper))
        self.assertEqual(wrapper(None, None), 'OK')
        wrapper = sm.adapters.lookup((Interface, ISubRequest), IView, name='')
        self.failIf(IMultiView.providedBy(wrapper))
        self.assertEqual(wrapper(None, None), 'OK2')
