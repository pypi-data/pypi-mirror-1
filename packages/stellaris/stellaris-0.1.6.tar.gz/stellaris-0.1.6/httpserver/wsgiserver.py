...
class HTTPConnection(object):
    ...
    def __init__(self, sock, addr, server):
        ...
        if SSL and isinstance(sock, SSL.Connection):
            timeout = sock.socket.gettimeout()
            ...
            self.environ['SSL_CLIENT_S_DN'] = str(sock.get_peer_cert().get_subject())
        else:
...
#            self.socket.socket.settimeout(1)    
#        elif hasattr(self.socket, 'settimeout'):
#            self.socket.settimeout(1)
            
        #elif hasattr(self.socket, 'set_timeout'):
...
##        self.socket.setsockopt(socket.SOL_SOCKET, socket.TCP_NODELAY, 1)
        if self.ssl_key_cert and self.ssl_ca_certs:
            if SSL is None:
                raise ImportError("You must install M2Crypto to use HTTPS.")
            
            # See http://aspn.activestate.com/ASPN/Cookbook/Python/Recipe/442473
            ctx = SSL.Context('sslv23') #SSL.Context(SSL.SSLv23_METHOD)
            ctx.load_cert(self.ssl_key_cert)
    ...
    def populate_ssl_environ(self):
        ...
        cert = X509.load_cert(self.ssl_key_cert) #crypto.load_certificate(crypto.FILETYPE_PEM, cert)
        self.ssl_environ = {
