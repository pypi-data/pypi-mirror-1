...
import stellaris.index.backend
...
def _load_rdflib_memory(config, section, log):
    ...
    try:
        return MemoryIndex(baseuri=config['baseuri'], log=log)
    except KeyError, e:
...
def _load_rdflib_bdb(config, section, log):
    ...
    try:
        return BerkeleyDBIndex(config['db_path'], baseuri=config['baseuri'], log=log)
    except KeyError, e:
        raise ConfigAttributeMissing(section, e)
    except Exception,e:
        raise e
...
def _load_rdflib_mysql(config, section, log):
    ...
    try:
        return MySQLDBIndex(user=config['user'], password=config['password'], host=config['host'], port=config['port'], db=config['db_name'], baseuri=config['baseuri'], log=log)
    except KeyError, e:
        raise ConfigAttributeMissing(section, e)
    except Exception,e:
        raise e
...
class BackendIndex(Worker):
    ...
    def setup(self, config):
        ...
        self.__index = ConcurrentIndex(f(config, config['section'], log))
