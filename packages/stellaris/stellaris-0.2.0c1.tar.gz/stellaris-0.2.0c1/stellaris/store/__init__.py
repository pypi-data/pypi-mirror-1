...
class IntervalThread(Thread):
    def __init__(self, gc_func, interval=60.0):
        ...
        while True:
            try:
                ...
                try:
                    ...
                    log.info('Garbage collection failed: %s' % (str(e)))
...
class Store(object):
    ...
    def __init__(self, gc_interval=3600, backup_interval=86400, backup_path=None):
#        self.config = config
        if gc_interval > 0:
            self.__gc_t = IntervalThread(self.garbage_collection, interval=gc_interval)
            self.__gc = True
        else:
            self.__gc = False
