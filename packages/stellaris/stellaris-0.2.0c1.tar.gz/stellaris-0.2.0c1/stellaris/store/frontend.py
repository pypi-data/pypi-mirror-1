...
class FrontendStore(object):
    ...
    def __init__(self, env):
        self._env = env
        self.__im = IndexManager(env)

        try:
            self.__sm = StoreManager(env)
        except:
            self.__im.stop()
            raise
        ...
        try:
            self.graph_prefix = urljoin(self.__baseuri, env.config['service']['graphs_prefix'])
        except KeyError, e:
            self.graph_prefix = self.__baseuri

    def recover(self, index_name):
        """
        ...
        graphs = [self.retrieve(graph) for graph in all_graphs('/')]
        self.__im.recover(index_name, graphs)
    ...
    def create(self, name, content, content_size=-1):
        ...
        # print "creating graph: ", name, content, content_size
    ...
    def retrieve(self, name, version=None):
        ...
        store = self.__sm.responsible_store(name)
        return store.retrieve(name, version=version)
    ...
    def query(self, index_name, query, format='xml'):
        return self.__im.query(index_name, query, format=format)
