...
class RDBMSStore(Store):
    ...
    def __init__(self, db_uri, admin_user=ADMIN_GROUP, gc_interval=3600, backup_interval=86400, backup_path=None, log=None):
        ...
        Store.__init__(self, gc_interval=gc_interval, backup_interval=backup_interval, backup_path=backup_path)
    ...
    def create(self, graph, user=None):
        ...
        with session.begin():
            ...
            session.save(db_graph)
            session.save(db_graph_data)
    ...
    def retrieve(self, name, version=None):
        ...
        with session.begin():
            try:
                ...
                g = Graph(db_g.name, str(db_g_data.data), uri=db_g.uri, ttl=db_g.ttl, version=db_g_data.version)
                session.close()
                return g
            except InvalidRequestError, e:
