...
class Graphs(WSGICollection):
    ...
    def create(self, env, resp):
        ...
        # urljoin
        location = urljoin(urlunparse((env['wsgi.url_scheme'], env['HTTP_HOST'], self.__prefix, '','','')), graph_id[1:])
...
#        content_type = self._check_accept(env)
        ...
        try:
            ...
            resp('200 OK', [('Content-Type', mimetype['.rdf'])])
    ...
    def _error_update(graph_id, env):
    ...
    def update(self, env, resp):
        ...
        try:
            f = getattr(self, '_%s_update' % format, '_error_update')
            f(graph_id, env)                                           
        except GraphNotFound, e:
            raise HTTPNotFound('Graph: %s, not found' % str(graph_id))
