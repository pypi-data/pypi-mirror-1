...
getGVisuCells();
var chart = new google.visualization.OrgChart(document.getElementById('googlevisualization_div'));
chart.draw(gvisudata, {allowHtml:true});
}
""",
                      required=False)
...
class GooglevisualizationField(BaseField):
    ...
    def jscode(self, datatable):
        ...
        """
        js = self.jssettings + "\n"
        js = js + "function getGVisuCells() {\n"
        js = js + "gvisudata.addRows(" + str(len(datatable)) + ");\n"
        i = 0
        for row in datatable:
            j = 0
            for cell in row:
                js = js + "gvisudata.setCell(" + str(i) + ", " + str(j) + ", " + cell + ");\n"
                j = j + 1
            i = i + 1
        js = js + "}"
        return js
