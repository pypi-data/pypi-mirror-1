...
class PlominoDatabase(ATFolder, PlominoAccessControl, PlominoDesignManager, PlominoReplicationManager, PlominoScheduler):
    ...
    def deleteDocuments(self,ids=None):
        """massively delete documents from database
        onDelete formula and index updating are not performed (use refreshDB to update)
        """
        if ids is None:
