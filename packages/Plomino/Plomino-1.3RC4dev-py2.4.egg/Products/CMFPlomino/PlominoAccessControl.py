...
class PlominoAccessControl(Persistent):
	...
	def getCurrentUserRights(self):
		...
		try:
			...
			if len(rights)==0:
				# no specific rights for this user, we return AuthenticatedAccessRight
	...
	def hasReadPermission(self, obj=None):
		...
		if obj is None:
			...
			result = getSecurityManager().checkPermission('CMFPlomino: Read documents', obj)==True
		return result
	...
	def hasCreatePermission(self, obj=None):
		...
		if obj is None:
			obj = self
		return getSecurityManager().checkPermission('CMFPlomino: Create documents', obj)==True and getSecurityManager().checkPermission('CMFPlomino: Add Plomino content', obj)==True
	...
	def hasEditPermission(self, obj=None):
		...
		if obj is None:
			obj = self
		return getSecurityManager().checkPermission('CMFPlomino: Edit documents', obj)==True
	...
	def hasRemovePermission(self, obj=None):
		...
		if obj is None:
			obj = self
		return getSecurityManager().checkPermission('CMFPlomino: Remove documents', obj)==True
