...
class PlominoDesignManager(Persistent):
    ...
    def refreshDB(self, REQUEST=None):
        ...
        if not(hasattr(self, "plomino_version")):
            ...
            report.append(msg)
            logger.info(msg)
        elif self.plomino_version=="1.3.0":
...
#        
    ...
    def importDbSettingsFromXML(self, node):
        ...
        while child is not None:
            name = child.nodeName
            if name=='acl':
                self.AnomynousAccessRight = child.getAttribute('AnomynousAccessRight')
                self.AuthenticatedAccessRight = child.getAttribute('AuthenticatedAccessRight')
                subchild = child.firstChild
