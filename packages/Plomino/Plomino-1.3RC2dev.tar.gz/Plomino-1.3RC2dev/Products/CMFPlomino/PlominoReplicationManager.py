...
class PlominoReplicationManager(Persistent):
    ...
    def getRemoteDocuments(self, replication):
        ...
        remoteDocumentsIds = authenticateAndLoadURL(replication['remoteUrl'] + '/getDocumentsIds', replication['username'], replication['password']).read()
    ...
    def getDocumentsIds(self):
        """return the database documents ids in a string
        """
        docs = self.getAllDocuments()
        ids = REMOTE_DOC_IDS_HEADER+REMOTE_DOC_ID_SEPARATOR
        for d in docs:
            ids=ids+d.id+REMOTE_DOC_DATE_SEPARATOR+str(d.bobobase_modification_time().toZone('UTC'))+REMOTE_DOC_ID_SEPARATOR
        return ids
    ...
    def exportableDoc(self, doc, remoteDocuments, lastReplicationDate, whowins):
        ...
        if not lastEditRemoteDocumentDate:
            ...
            #TODO : UTC
            lastEditDocumentDate = self.bobobase_modification_time().toZone('UTC')
            #check dates
    ...
    def importableDoc(self, docId, lastEditRemoteDocumentDate, lastReplicationDate, whowins):
        ...
        if localDoc is None:
            ...
            #set while document modified thru script)
            #TODO : UTC
            lastEditDocumentDate = localDoc.bobobase_modification_time().toZone('UTC')
    ...
    def buidReplicationFromRequest(self, REQUEST = None):
        ...
        return self.buildReplication(id, name, remoteUrl, username, password, repType, whoWins, scheduled, cron, mode)
        
    security.declarePrivate('createReplication')
    def buildReplication(self, id, name, remoteUrl, username, password, repType, whoWins, scheduled, cron, mode):
        """return an empty replication
        """
    ...
    def processImport(self, REQUEST=None):
