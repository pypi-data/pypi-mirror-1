...
# Conversion between numpy/Numpy types and the DAP types.
...
def trim(dataset, constraints, copy=True):
    ...
    Arrays can be sliced. Here we have a ``(2,3)`` array:
        ...
        >>> data.shape = (2,3)
        >>> dataset['array'] = dtypes.ArrayType(data=data, name='array', shape=[2,3], type='Int32')
    ...
    The same is valid for grids:
        ...
        >>> data = array([1,2,3,4,5,6])
        >>> data.shape = (2,3)
        >>> dataset['grid'] = dtypes.GridType(data=data, name='grid', dimensions=['x', 'y'], shape=[2,3], type='Int32')
