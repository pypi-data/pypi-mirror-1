...
class DapUnpacker(object):
    ...
    def getvalue(self):
        ...
        # Check for Structure/Sequence.
        elif isinstance(self.var, dtypes.StructureType):
            out = []
            dapvar = self.var
        ...
        if isinstance(self.var, dtypes.ArrayType):
            # numarray can't handle arrays of strings.
            # TODO: Numeric can, just set type to 'O'.
            if self.var.type.lower() not in ['url', 'string']:
                # Convert to array and reshape.
                out = dap.array(out, type_)
                if self.outshape:
