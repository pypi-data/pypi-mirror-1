...
class StructureType(odict):
    ...
    def __deepcopy__(self, memo=None, _nil=[]):
        ...
        # Stored variables *are* deepcopied.
        for k, v in self.items():
...
class SequenceType(StructureType):
    ...
    def __iter__(self):
        ...
        # Make a copy so we can filter it without problem.
        ...
        for values in out.data:
            ...
            struct_ = StructureType(name=self.name, attributes=self.attributes)
            struct_._filters = out._filters[:]
            ...
            parent = self._id[:-len(self.name)-1]
