...
from dap.helper import buildfilter, parse_querystring, getslice
...
class Handler(BaseHandler):
    ...
    def _parseconstraints(self, constraints=None):
        ...
        if slice_:
            slice_ = getslice(slice_, [sys.maxint])[0]
            data = itertools.islice(data, slice_.start, slice_.stop, slice_.step)
        else:
            ...
            if slices:
                start, step, stop = zip(*[(s.start, s.step, s.stop) for s in slices])
