...
class Renderer(base.Renderer):
    ...
    def filter_list(self):
        ...
        reqform = None
        if self.request.has_key('filtering'):
            # Take the base from session
            reqform = self.request.SESSION.get('search-form')

        if reqform is None:
            reqform = dict(self.request.form)
            self.request.SESSION.set('search-form', reqform)

        brains = catalog.searchResults(**reqform)
        self.count = len(brains)
        filter_tags = self.data.filter_tags
        
        base_url = "%s?filtering&" % self.request.getURL()
        for key,val in reqform.items():
            if key == "filtering":
                continue
        ...
        for brain in brains:
            for tag in filter_tags:
                ...
                if str(val) == tag.value:
                    spec_url = base_url
                    if isinstance(val, bool):
                        if not (tag.index_name, True) in reqform.items():
                            spec_url = "%s%s:boolean=True" % (base_url, tag.index_name)
                    else:
                        if not (tag.index_name, tag.value) in reqform.items():
                    ...
                    if tag.show_unique:
                        ...
                        for meta in metas:
                            ...
                            if isinstance(meta, bool):
                                if not (tag.metadata_name, True) in reqform.items():
                                    meta_url = "&%s%s:boolean=True" % (spec_url, tag.metadata_name)
                            else:
                                if not (tag.metadata_name, meta) in reqform.items():
