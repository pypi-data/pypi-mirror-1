...
class Renderer(base.Renderer):
    ...
    def filter_list(self):
        ...
        if self.request.has_key('filtering'):
            ...
            form = self.request.SESSION.get('search-form')
        
        if form is None:
            form = self.request.form
            self.request.SESSION.set('search-form', form)

        #brains = self.context.queryCatalog(REQUEST = form)
        brains = catalog.searchResults(**form)
        ...
        for brain in brains:
            for tag in filter_tags:
                ...
                if str(val) == tag.value:
                    spec_url = base_url
                    if isinstance(val, bool):
                        if not (tag.index_name, True) in form.items():
                            spec_url = "%s%s:boolean=True" % (base_url, tag.index_name)
                    else:
                        if not (tag.index_name, tag.value) in form.items():
                    ...
                    if tag.show_unique:
                        ...
                        for meta in metas:
                            ...
                            if isinstance(meta, bool):
                                if not (tag.metadata_name, True) in form.items():
                                    meta_url = "&%s%s:boolean=True" % (spec_url, tag.metadata_name)
                            else:
                                if not (tag.metadata_name, meta) in form.items():
