...
#     anagram.py
# 
#     Copyright (c) 2009, 2010 Umang <umang.me@gmail.com>.
# 
...
class Anagram:
    ...
    def filter_wordlist(self, lines):
        ...
        return([x.strip() for x in lines
               if (len(x) > 3 and x.strip().isalpha())])
    ...
    def find_letters(self, length=7):
        ...
        for x in xrange(len(possible_words)):
    ...
    def start_new(self, letters=None, length=7, no_word_error=False):
        ...
        for word in self.wordlist:
            ...
            for x in xrange(0, len(word)):
                if word[x] in a_letters:
                    a_letters.remove(word[x])
                else:
                    is_anagram = False
                    break
            if is_anagram:
...
this anagram" % length)
        self.words = dict([(word, False) for word in words])
