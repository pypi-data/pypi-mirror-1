...
{
	document.getElementById('debug_text').innerHTML += str + "<br>";
}
...
function delete_concept(id, term)
{
	if(confirm('Are you sure to delete concept ' + term + ' from the Thesaurus?')) {
		raiseDeleteConceptEvent(id);
		deleteConceptFromBoxes(term);
		initDragDropScript();
	}
}

function raiseDeleteConceptEvent(id) {
	actionField = document.getElementById("delete_" + id);
...
function deleteConceptFromBoxes(aTerm) {
	var uls = dragDropTopContainer.getElementsByTagName('ul');
	for(var i=0;i<uls.length;i++){	// LOoping through all <ul>
		listItems = uls[i].getElementsByTagName('li');
		removeItem = false;
		for(var no=0;no<listItems.length;no++){
			if(!removeItem) {
				term = listItems[no].id.split("_")[0];
				if(term == aTerm) {
					uls[i].removeChild(listItems[no]);
					removeItem = true;
					break;
				}
			}
