...
class ScriptTask(BasicTask):
    """Task representing a script to run.

    This tends to have problems on windows if you want to impersonate users.
    The problem is that impersonation is not inhereited properly so even
    if you run this under ImpersonatingTask, the subprocess this task
    spawns won't be the right user and you will get 'Access is Denied'
    messages.
    """

    parameters = BasicTask.parameters + ['command','contents','sleepInterval',
                                         'logFile']

    def __init__(self,command,contents=None,shell=False,sleepInterval=60,
                 logFile='<default>',dirName=None,*args,**kw):
        ...
        self.dirName = dirName if dirName is not None else tempfile.mktemp()
        self.process = None
        
        if (logFile == '<default>'):
            self.logFile = os.path.join(self.dirName,'log.txt')
        elif (logFile in ['','None']):
            self.logFile = None

    def GetPids(self):
        "Return pids for process"
    ...
    def PrepareCommandContents(self):
        "Prepare self.command if we need to write contents to a script."

        if (not os.path.exists(self.dirName)):
            os.mkdir(self.dirName)                   
        if self.contents is not None:
            if self.command is None:
                self.command = os.path.join(self.dirName,'my_cmd.py')
            logging.debug('Writing contents of script to %s.' % self.command)
            f = None
            try:
                f = open(self.command, 'wb')
                f.write(self.contents)
            finally:
                if f is not None:f.close()
        if self.command is not None:
            #FIXME
            # this is a work around for ticket 577, this workaround
            # requires .py files to be associated with python.exe
            # on windows
            return [self.command]
            # ideally, we do not need such association, rather we should
            # simply use the following code, but it causes the regression
            # test to hang for some unknown reason
            #return (
            #    ['python', command]
            #    if command.endswith('.py') else [command])
        
        raise Exception("At least one of contents or command msut be given.")

    def Run(self):
        "Run the script"
        
        command = self.PrepareCommandContents()
        log = logging.debug
        log('Running task %s\ncommand is %s\n' % (self, command))
        output = None
        try:
            actualUser = os.getenv('USER', os.getenv('USERNAME', 'unknown'))
            if (self.logFile is not None):
                output = open(self.logFile,'wb')
                output.write('### Running task %s as user %s.\n' % (
                    self.Name(), actualUser))
                output.flush()

            self.process = subprocess.Popen(
                self.command,
                stdout=subprocess.PIPE if output is None else output,
                stdin=subprocess.PIPE,stderr=subprocess.STDOUT,shell=self.shell)
            while 1:
                if (self.die): self.Kill()
                self.exit_status = self.process.poll()
                log('Task %s: polled process to get %s' % (
                    self.Name(), self.exit_status))
                if (None == self.exit_status): # still running so wait a while
                    time.sleep(self.sleepInterval)
                else: break # process finished so exit the loop
        finally:
            if output is not None: output.close()
        log('Finished task %s. Output is ...' % self.Name())
    ...
    def _Result(self):
        """
        return content of self.logFile
        """
        f = None
        try:
            if (self.logFile is not None):
            ...
            result = 'Got Exception in Tasks.py: %s\n' % str(e)
            result += Process.MakeErrTrace(e)
        finally:
            if f is not None:f.close()                
        return str(result)
    ...
    def Kill(self):
        "Kill a task immediately."
        
        logging.debug('Attempting to kill task %s.' % self.Name())
        if (hasattr(os,'kill')):
            import signal
            kill = getattr(os, 'kill')
            kill(-self.process.pid, signal.SIGTERM) #kill whole group
            ...
            raise Exception("Do not know how to terminate a subprocess.")
