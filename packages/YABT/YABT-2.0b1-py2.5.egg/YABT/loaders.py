...
class BasicXMLFileLoader(BaseLoader):
    ...
    def _startElement(self, name, attrs):
        ...
        if name == "char":
            ...
            self._repl = ""
        elif self._path == ".transtable.YABTdata.decission":
            iclass = int(attrs["inputclass"])
            for state in attrs["states"].split(","):
                self._translator.addDecission(iclass, int(state))
    ...
    def _addRule(self):
        ...
        self._translator.addRule(int(self._iclass), self._focus, 
                                 self.contextFactory.getContextInstance(
                                      bftype, bfcontext, ContextFactory.BEFORE),
                                 self.contextFactory.getContextInstance(
                                      aftype, afcontext, ContextFactory.AFTER),
                                 self._trans, int(self._fstate))
