...
$Id: plush.py 50930 2007-01-30 10:30:36Z bdelbosc $
"""
import sys
import os
import re
...
class PlushBase:
    ...
    def openStore(self, store_dir):
        ...
        self.index_reader = IndexReader.open(self.store_path)
    ...
    def getTermFreqs(self, field, max_term=None, pattern=None):
...
Using PyLucene %s and Lucene %s.
  ...
  Type: \open [INDEX_PATH]    to open a lucene store
    ...
    def __init__(self, store_dir, verbose=False):
        ...
        self.cmd_encoding = self.guessEncoding()
        self.prompt = 'plush> '
        self.is_query = True
    ...
    def do_view(self, message):
        """\\v[iew] [DOC_NUM]
        View indexes and fields value for [DOC_NUM]
    ...
    def do_term(self, message):
        ...
        print "Top %d term occurences for field '%s' matching '%s'." % (
            self.limit, field, pattern and pattern or '*')
    ...
    def default(self, line):
        ...
        for i, doc in hits:
            ...
            for field in result_fields:
                print '  %s: %s' % (field, truncStr(doc.get(field), 150))
            print
            count += 1
        if count:
            print "  Found %s matching document(s) in %.3fs" % (nb_hits, delta)
