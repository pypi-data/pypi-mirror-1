...
class Site(object):
    ...
    def _parse(self, input_data):
        Article = namedtuple('Article', 'headers, body, input_path, output_path, media')
    ...
    def run(self):
        logging.info("project directory: %s", self.settings['project_dir']) 
        ...
        self._write()
        self._copy_media()
        logging.info("%s articles written", len(self.articles))
...
def main():
    ...
    parser.add_option('-s', '--serve', help='local webserver',
    ...
    if options.serve:
        import autoreload
        autoreload.main(runserver, (), {'directory': settings['input_dir']})
    else:
        site.run()
