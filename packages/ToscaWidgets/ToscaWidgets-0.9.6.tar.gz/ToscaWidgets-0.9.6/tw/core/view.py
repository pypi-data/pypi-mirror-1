...
def make_renderer(method, doc=None):
    def _renderer(self, renderable, **kw):
        ...
        if origin is None and renderable.available_engines:
            if tw.framework._default_engine in renderable.available_engines:
                origin = tw.framework._default_engine
            else:
                origin = renderable.available_engines[0]
        #print origin, renderable.__class__
...
class Renderable(object):
    ...
    def __new__(cls, *args, **kw):
        ...
        if obj.template is not None:
            engine_name, obj.template = choose_engine(obj)
        if engine_name:
