...
if __name__ == "__main__":
  ...
  for f in glob.glob('*.rst'):
  	...
  	open(f,'w').write(buffer)
  	
  	b = f.split('.')[0]
  	s = ['--no-xml-declaration','--stylesheet=default.css','--no-toc-backlinks']
  	s.append(f); s.append('%s.htm' % f[:-4])
	docutils.core.publish_cmdline(writer_name='html', argv=s)
