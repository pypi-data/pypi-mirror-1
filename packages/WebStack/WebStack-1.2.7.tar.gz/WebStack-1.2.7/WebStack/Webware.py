...
class Transaction(WebStack.Generic.Transaction):
    ...
    def get_content_type(self):
        ...
        if hasattr(request, "contentType"):
            ...
            content_type = self.trans.request()._environ.get('CONTENT_TYPE', None)
        return self.parse_content_type(content_type)

    def get_content_charsets(self):

        """
        Returns the character set preferences.
        NOTE: Requires enhancements to HTTPRequest in 0.8.1 or earlier.
        """

        request = self.trans.request()
        if hasattr(request, "contentCharsets"):
            return request.contentCharsets()
        else:
            return request.accept("charset")
    ...
    def get_content_languages(self):
        ...
        request = self.trans.request()
        if hasattr(request, "contentLanguages"):
            return request.contentLanguages()
        else:
    ...
    def get_path_info(self, encoding=None):
        ...
        # Make the context name resemble the start of a path if it doesn't
        # already do so.

        if not context_name.startswith("/"):
            context_name = "/" + context_name

        # Correct the path info if appropriate.
    ...
    def set_response_code(self, response_code):
        ...
        self.trans.response().setStatus(response_code, str(response_code))
