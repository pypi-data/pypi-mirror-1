...
class Stream:
    ...
    """
    Wrapper around java.io.InputStream.
    """
    ...
    def read(self, limit=None):
        ...
        bufsize = limit or self.bufsize
        characters = StringIO()
        a = jarray.zeros(bufsize, 'b')
        while 1:
            nread = self.stream.read(a, 0, bufsize)
            if nread != -1:
                self._copy(a, characters, nread)
            if nread != bufsize or nread == limit:
                return characters.getvalue()

    def readline(self, bufsize=None):

        "Read a line from the stream, returning it as a string."

        bufsize = bufsize or self.bufsize
        characters = StringIO()
        a = jarray.zeros(bufsize, 'b')
        while 1:
            nread = self.stream.readLine(a, 0, bufsize)
            if nread != -1:
                self._copy(a, characters, nread)
            if nread != bufsize:
                return characters.getvalue()
...
class Transaction(WebStack.Generic.Transaction):
    ...
    def __init__(self, request, response, servlet):

        """
        Initialise the transaction using the Java Servlet HTTP 'request' and
        'response', along with the deployment 'servlet'.
        """
    ...
    # Java-specific variants of the generic methods.
    ...
    def _get_fields_from_message(self, encoding):
        ...
        # Concatenate the headers with the rest of the stream.

        header_stream = java.io.StringBufferInputStream(str_buffer.toString())
        input_stream = self.request.getInputStream()
        message = javax.mail.internet.MimeMessage(session, java.io.SequenceInputStream(header_stream, input_stream))

        # Collect the fields by traversing the message.

        fields = {}
        self._get_fields_from_multipart(fields, message.getContent(), encoding)
        return fields

    def _get_fields_from_multipart(self, fields, content, encoding):

        "Get fields from multipart 'content'."

        for i in range(0, content.getCount()):
            part = content.getBodyPart(i)
            self._get_field_from_multipart(fields, part, encoding)

    def _get_field_from_multipart(self, fields, part, encoding):

        "Get a field from the given 'part'."

        if not part.getContentType().startswith("multipart"):
            ...
            if disposition.name is not None:
                ...
                # Test whether the part should be written to a temporary file.
                ...
                if part.getHeader("Content-Type") is not None:
                    ...
                    # Write to a temporary file and then open that file.

                    tempfile = java.io.File.createTempFile(str(id(self)), field_name)
                    temp_stream = java.io.FileOutputStream(tempfile)
                    try:
                        part.writeTo(temp_stream)
                    finally:
                        self.tempfiles.append(tempfile)

                    # The file must be treated like a message.

                    temp_part = javax.mail.internet.MimeBodyPart(java.io.FileInputStream(tempfile))
                    field_value = FileContent(Stream(temp_part.getRawInputStream()), headers)
...
class Session:
    ...
    session classes. This class can also be instantiated with a request object
    and used to access attributes on the request.
    """

    def __init__(self, session):

        """
        Initialise the session object with the framework 'session' object. If a
        ServletRequest object is given, the attributes on that will be
        accessible, as opposed to the attributes on an HttpSession object.
        """
