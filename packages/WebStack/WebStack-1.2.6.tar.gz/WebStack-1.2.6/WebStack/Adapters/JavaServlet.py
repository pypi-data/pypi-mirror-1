...
def deploy(resource, authenticator=None, address=None, handle_errors=1):
    ...
    class _Deploy(HttpServlet):
        def __init__(self):
            ...
            self.dispatcher = Dispatcher(self, self.resource, self.authenticator, self.handle_errors)
...
class Dispatcher:
    ...
    def __init__(self, servlet, resource, authenticator=None, handle_errors=1):

        """
        Initialise the dispatcher, providing the deployment 'servlet', the root
        application-specific 'resource', and an optional 'authenticator'. The
        optional 'handle_errors' parameter (if true) causes handlers to deal
        with uncaught exceptions cleanly.
        """
    ...
    def service(self, request, response):
        ...
        trans = WebStack.JavaServlet.Transaction(request, response, self.servlet)
        try:
