...
class PageMaker:
	...
	def load_package(self, packname):
		...
		for set in ("PARSERS", "MAKERS", "WRITERS"):
			try:
				exec "self.%(set)s['%(type)s'] = %(set)s['%(type)s']" \
					 % {'set': set, 'type': self.content_type}
			except:
				lib.prn("the %s package does not define %s for %s pages" \
						% (packname, set, self.content_type), "WARNING")

	def __read_data(self):
		lines = lib.read_lines(self.resource, localized=True) # exceptions are not handled
		header = lines[0].strip()
		lib.prn("header is: "+header)
		tokens = lib.tokens(header, ":")
		self.content_type = tokens[-1]
		lib.prn("page type is '%s'" % self.content_type, "INFO")
		if len(tokens) > 1:
			packname = tokens[0]
			self.load_package(packname)
		return lines[1:]
	...
	def make(self, resource):
		...
		# do not serve property elements
		if ((resource.find(".set") != -1) or 
			(resource.find(os.sep+"entries"+os.sep) != -1)):
