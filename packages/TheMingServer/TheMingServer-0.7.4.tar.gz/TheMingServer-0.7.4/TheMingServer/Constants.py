...
class Constants:
	...
	LOG_PASSWD = "truck"										  # Log passwd, used to reset log
	...
	if (IS_WIN32):
		...
		FILES_DIR = "..\\files"		 		 # incoming data handler's dir - must be read-write
		LOG_DIR = FILES_DIR					 # where your log will be written - "." for cgi-bin
	else:
		if os.path.exists(os.path.expanduser("~/.my_machine")):
			DATA = "../data"					 # root of all your data files - can be read-only
			FILES_DIR = "../files"			 # incoming data handler's dir - must be read-write-execute
		else:
			DATA =	os.path.expanduser("~/data") # root of all your data files - can be read-only
			FILES_DIR = os.path.expanduser("~/files") # incoming data handler's dir - must be read-write-execute
		LOG_DIR = FILES_DIR					 # where your log will be written - "." for cgi-bin
