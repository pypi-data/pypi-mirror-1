...
""" % NAME
	...
	def __blog(self, form):
		if "type" in form.keys():
			...
			from MingBlogServer import LibBlog
			lib = LibBlog()
			lib.give_headers()
			dir = lib.get_dir()
			author, email, title, tagline = lib.get_text(dir)
			intro = lib.get_lines(dir,"intro")
			footer = lib.get_lines(dir,"footer")
			links = lib.get_lines(dir,"links")
			content = lib.content(dir, author, email)
			page = lib.do_page(dir, title, tagline, intro, footer, links, content)
			print page
			sys.exit()
