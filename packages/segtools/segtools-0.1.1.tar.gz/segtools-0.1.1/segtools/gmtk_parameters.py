...
def save_graph(labels, probs, dirpath, q_thresh=Q_THRESH, p_thresh=P_THRESH,
    ...
    ps_dir = os.path.join(os.path.split(dirpath)[0], "pss")
    for node in G.nodes():
        node.attr["image"] = "%s/%s.ps" % (ps_dir, str(node))
        node.attr["label"] = " "
    G.write(dotfilename)

    return None #XXXXXXXXXX Need R code to create sub .ps images first
...
def parse_options(args):
    ...
    parser.add_option("--mnemonic-file", dest="mnemonic_file",
                      ...
                      " mnemonics found in this file. If not specified,"
                      " a mnemonic file will be generated automatically.")
...
def main(args=sys.argv[1:]):
    ...
    validate(paramsfile, options.outdir,
