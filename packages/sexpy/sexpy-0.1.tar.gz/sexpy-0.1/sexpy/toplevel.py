...
class Input(object):
    ...
    def get_line(self):
        ...
        if self.incomplete:
            ...
            prompt = 'SEXpy> '
        try:
            self.buffer.write(raw_input(prompt))
        except ValueError:
            raise EOFError
    ...
    while True:
        ...
        try:
            for statement in parse(i):
                i.incomplete = False
                try:
                    code = compile([statement], codegen = InteractiveCodeGenerator)
                except:
                    print '"<input>", line %i: invalid statement' % lineno(statement)
                    print ' -->', repr(statement)
                    continue
                try:
                    exec code in interactive_namespace
                except SystemExit:
                    return
                except:
                    traceback.print_exc()
