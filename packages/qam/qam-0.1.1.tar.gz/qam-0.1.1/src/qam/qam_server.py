...
The QAM Server executes remote method calls.
...
class QAMServer:
  ...
  *Class-variables of QAMServer:*
  ...
  def __init__(self,
    ...
    self.amqpconn = AMQPConnection(hostname=hostname,
                                   port=port,
                                   userid=username,
                                   password=password,
                                   host=vhost)
...
class ExecuteMethodThread(Thread):
  ...
  *Class-variables of ExecuteMethodThread:*
  ...
  def __init__ (self, result_queue, message_data, method_dict, instance_dict):
  ...
  def run(self):
    ...
    # external exception
      ...
      t,v =  sys.exc_info()[:2]
      logger.debug('catched exception: %s' % repr(v.args))
      sys.exc_clear()
...
class PublisherThread(Thread):
  ...
  *Class-variables of PublisherThread:*
  ...
  def __init__ (self, amqp_credentials, result_queue, server_id):
    Thread.__init__(self)
