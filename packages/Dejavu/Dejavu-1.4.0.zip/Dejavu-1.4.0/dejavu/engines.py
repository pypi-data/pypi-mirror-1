    ...
    def on_recall(self):
        if self.Expiration is not None:
            if self.Expiration <= datetime.datetime.now():
                self.forget()
                raise dejavu.UnrecallableError
            else:
...
class TemporarySweeper(recur.Worker):
    ...
    def work(self):
        ...
        f = logic.Expression(lambda x: x.Expiration != None
                                   and x.Expiration <= now)
        ...
        try:
            for i, eachID in enumerate(self.Members):
                if quota and i >= quota:
                    break
                unit = self.sandbox.unit(cls, **dict(zip(idnames, eachID)))
                if unit:
                    output.append(unit)
        finally:
            self.release()
...
class UnitEngine(TemporaryUnit):
    ...
    def snapshots(self):
        ...
        allSnap = self.sandbox.recall(UnitCollection, f)
    ...
    def visit_FILTER(self, setID, operand):
        ...
        try:
            ...
            if A.universal:
                ...
                idnames = [prop.key for prop in cls.identifiers]
                for id in mem:
                    unit = self.sandbox.unit(cls, **dict(zip(idnames, id)))
