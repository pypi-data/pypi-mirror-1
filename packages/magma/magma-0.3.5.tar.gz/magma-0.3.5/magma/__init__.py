...
====================
    ...
    >>> # from magma import *
    >>> mag = open_file("example-0.4.magma")

or 

    >>> mag = Magma(input_files=["input_file.txt", "input_file2.txt"])    
...
Create a list from input files. 
    >>> mag = create_from_input_files(["input_file.txt"])
...
Output the list, showing the empty entries, which could be filled with data. 
    ...
    list:
     - "magnet:?dn=input_file.txt&xt=urn%3Asha1%3A3UJCLAOIZVCNAIT7TQYFLAP7ZNFW6G2G"
    <BLANKLINE>
    
And save it to a file with the empty entries cleaned away. 
    >>> mag.save(path="example.magma")
