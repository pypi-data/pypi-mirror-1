...
class Magma(object):
    ...
    def __init__(self, magma_file=None, input_files=None, creator=None, data=None, yaml_data=None, *args, **kwds):
        ...
        @param yaml_data: Raw file data as read from a Magma file. 
        @type yaml_data: String
        
        @param data: A native Python dict with the data. 
        @type data: Dict
    ...
    def __str__(self): 
        ...
        # Then return its Magma representation (Magma Header + yaml data). 
        return self.dump()
    ...
    def remove_empty_entries(self): 
        ...
        # Just tell every file to clean itself. 
        for i in self.files: 
            i.remove_empty_entries()
    ...
    def magma_v0_2_filelist_representation(self):
        """@return: A list of files which can be read by MAGMAv0.2 compliant programs."""
        # First we get the list parameter. 
        data = "list:\n"
        
        # Then we append a magnet for each file. 
        for i in self.files: 
            data += ' - "' + i.magnet + '"\n'
        
        return data
    
    def magma_v0_2_data(self): 
        """@return: Data in Magma v0.2 format"""
        # First we begin with the MAGMAV0.2 header line. 
        data = "#MAGMAv0.2\n"
        
        # Then we append the file list. 
        data += magma_v0_2_filelist_representation()
        
        return data
        
        
    def save(self, path, magma_versions = [0.2, 0.4]):
        ...
        @param magma_versions: The versions of the magma specification to be supported. 
        @type magma_versions: List of Floats
    
        @return: None
        """
        # First remove empty entries, so we get a nice looking file. 
        self.remove_empty_entries()
        # First get the file handle for the magma file
        file_handle = open(path, "w")
        # Then write the data in yaml format. 
        file_handle.write(self.dump(magma_versions=magma_versions))
    ...
    def dump(self, magma_versions = [0.2, 0.4]):
        """Return the yaml data. 
        
        @return: A string representation of the Magma in yaml format.
        """
        # If we want 0.2 support, we add it. 
        if 0.2 in magma_versions and 0.4 in magma_versions: 
            # But first we remove the former 0.2 list. 
            if "list" in self.data.keys(): 
                del(self.data["list"])
            # Now we add a new list from the data we got. This can REMOVE DEPRECATED DATA. 
            return "#MAGMAv0.4\n" + self.get_yaml_data() + "\n# Magmav0.2 compatibility section\n" + self.magma_v0_2_filelist_representation()
        elif 0.4 in magma_versions: 
            return "#MAGMAv0.4\n" + self.get_yaml_data()
...
class MagmaFile(object):
    ...
    def parse_magnet(self): 
        ...
        # Now we get our url parameters. 
        params = [("xt", xt ), ("dn", dn )]
        
        # Additionally we want to add alt-locs. 
        gnet_alt_locs = self.readout_gnet_alt_locs()
        # They need to be transformed to the correct xs parameter, though. 
        for i in gnet_alt_locs: 
            params.append(("xs", "http://" + str(i) + "/uri-res/N2R?" + xt))
    ...
    magnet = property(fget=parse_magnet, fset=setattr)
    
    def remove_empty_entries(self): 
        """Remove empty entries from the file, for example empty alt-locs."""
        # Check each key, if it points to an empty dictionary, or only to empty subelements. 
        
        # First check for empty subkeys. 
        for i in self.data: 
            try: 
                # Iterate over the items inside the key
                for j in self.data[i].keys(): 
                    try: 
                        # if it has a length and the length is 0, we remove it. 
                        if len(self.data[i][j]) == 0: 
                            del(self.data[i][j])
                    except: pass
            except: pass
        
        # Now  check for empty keys. 
        for i in self.data.keys(): 
            try: 
                # If the length of the thing is 0, the whatever-it-is empty and we can remove it. 
                if len(self.data[i]) == 0: 
                    del(self.data[i])
            except: # If we can't get a length, we just ignore it. It seems to be data/content, not structure. 
                pass
        #TODO: Make this rekursive, so we can clean arbitrary deeply nested structures. 
