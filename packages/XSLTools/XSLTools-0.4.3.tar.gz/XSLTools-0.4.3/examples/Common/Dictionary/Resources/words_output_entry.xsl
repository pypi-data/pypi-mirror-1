...
<axsl:stylesheet xmlns:dyn="http://exslt.org/dynamic" xmlns:axsl="http://www.w3.org/1999/XSL/Transform" xmlns:template="http://www.boddie.org.uk/ns/xmltools/template" version="1.0" extension-element-prefixes="dyn">
  ...
  <axsl:template match="/">
    <axsl:for-each select="dyn:evaluate($element-path)">
      <axsl:apply-templates select="matches" mode="id2274353"/>
    </axsl:for-each>
  </axsl:template>
  <axsl:template match="matches" mode="id2274353">
    ...
    <axsl:choose><axsl:when test="@word"><axsl:variable name="this-name">word</axsl:variable><axsl:variable name="this-value" select="@word"/><select name="{template:this-attribute()}">
      <!-- onchange="requestUpdate(
        ...
        '/words/entry')" -->
      <axsl:apply-templates select="match-enum" mode="id2305876"/>
  ...
  <axsl:template match="match-enum" mode="id2305876">
