...
<axsl:stylesheet xmlns:dyn="http://exslt.org/dynamic" xmlns:axsl="http://www.w3.org/1999/XSL/Transform" xmlns:template="http://www.boddie.org.uk/ns/xmltools/template" version="1.0" extension-element-prefixes="dyn">
  ...
  <axsl:template match="/">
    <axsl:for-each select="dyn:evaluate($element-path)">
      <axsl:apply-templates select="accessories" mode="id2295197"/>
    </axsl:for-each>
  </axsl:template>
  <axsl:template match="accessories" mode="id2295197">
    ...
    <select multiple="multiple" name="{template:list-attribute('accessory-enum',&#10;            'value')}">
      <axsl:apply-templates select="accessory-enum" mode="id2295249"/>
    </select>
  </div>
  </axsl:template>
  <axsl:template match="accessory-enum" mode="id2295249">
