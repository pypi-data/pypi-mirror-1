...
<axsl:apply-templates select="words" mode="id2291225"/>
</html>
  </axsl:template>
  <axsl:template match="words" mode="id2291225">
    <body xmlns="http://www.w3.org/1999/xhtml">
<form action="" method="POST">

<axsl:apply-templates select="entry" mode="id2288434"/>
<p>
  ...
  <axsl:template match="entry" mode="id2288434">
  ...
  <axsl:apply-templates select="matches" mode="id2278922"/>
</div>
  </axsl:template>
  <axsl:template match="matches" mode="id2278922">
    ...
    <axsl:choose><axsl:when test="@word"><axsl:variable name="this-name">word</axsl:variable><axsl:variable name="this-value" select="@word"/><select name="{template:this-attribute()}">
      <!-- onchange="requestUpdate(
        ...
        '/words/entry')" -->
      <axsl:apply-templates select="match-enum" mode="id2304569"/>
  ...
  <axsl:template match="match-enum" mode="id2304569">
