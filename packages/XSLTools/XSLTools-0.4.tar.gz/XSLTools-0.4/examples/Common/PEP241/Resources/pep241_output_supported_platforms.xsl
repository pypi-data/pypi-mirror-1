...
<axsl:stylesheet xmlns:dyn="http://exslt.org/dynamic" xmlns:axsl="http://www.w3.org/1999/XSL/Transform" xmlns:template="http://www.boddie.org.uk/ns/xmltools/template" version="1.0" extension-element-prefixes="dyn">
  ...
  <axsl:template match="/">
    <axsl:for-each select="dyn:evaluate($element-path)">
      ...
      <axsl:apply-templates select="supported-platforms" mode="id2320594"/>
      <tr>
        <th/>
        <td>
  ...
  <axsl:template match="supported-platforms" mode="id2320594">
    <axsl:apply-templates select="supported-platform" mode="id2320594"/>
  </axsl:template>
  <axsl:template match="supported-platform" mode="id2320594">
