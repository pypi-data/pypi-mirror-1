...
<axsl:apply-templates select="structure" mode="id2335465"/>
</html>
  </axsl:template>
  <axsl:template match="structure" mode="id2335465">
    <body xmlns="http://www.w3.org/1999/xhtml">
<form action="" method="POST">

<!-- Template text between the start and the interesting part. -->

<axsl:apply-templates select="item" mode="id2335481"/>
<p>
...
<!-- Template text between the interesting part and the end. -->
  ...
  <axsl:template match="item" mode="id2335481">
  ...
  <p>
    Item type:
    <axsl:apply-templates select="type" mode="id2335523"/>
  </p>
  <axsl:apply-templates select="options" mode="id2335569"/>
  <p>
    Itself containing more items:
  </p>
  <axsl:apply-templates select="subitem" mode="id2335591"/>
  <p>
    <input type="submit" value="Add subitem" name="add2={template:this-element()}"/>
  </p>
</div>
  </axsl:template>
  <axsl:template match="type" mode="id2335523">
    <select xmlns="http://www.w3.org/1999/xhtml" multiple="multiple" onchange="requestUpdate(         'comments',         '{template:list-attribute('type-enum', 'value')}',         '{template:other-elements(../options)}',         '{template:child-attribute('value', template:child-element('comment', 1, template:other-elements(../options)))}',         '/structure/item')" name="{template:list-attribute('type-enum',&#10;            'value')}">
      <axsl:apply-templates select="type-enum" mode="id2335551"/>
    </select>
  </axsl:template>
  <axsl:template match="type-enum" mode="id2335551">
  ...
  <axsl:template match="options" mode="id2335569">
    <p xmlns="http://www.w3.org/1999/xhtml" template:id="comment-node" id="{template:this-element()}">
    <axsl:apply-templates select="comment" mode="id2335577"/>
  </p>
  </axsl:template>
  <axsl:template match="comment" mode="id2335577">
  ...
  <axsl:template match="subitem" mode="id2335591">
