...
<axsl:stylesheet xmlns:dyn="http://exslt.org/dynamic" xmlns:axsl="http://www.w3.org/1999/XSL/Transform" xmlns:template="http://www.boddie.org.uk/ns/xmltools/template" version="1.0" extension-element-prefixes="dyn">
  ...
  <axsl:template match="/">
    <axsl:for-each select="dyn:evaluate($element-path)">
      <axsl:apply-templates select="options" mode="id2337784"/>
    </axsl:for-each>
  </axsl:template>
  <axsl:template match="options" mode="id2337784">
    <p xmlns="http://www.w3.org/1999/xhtml" template:id="comment-node" id="{template:this-element()}">
    <axsl:apply-templates select="comment" mode="id2337796"/>
  </p>
  </axsl:template>
  <axsl:template match="comment" mode="id2337796">
