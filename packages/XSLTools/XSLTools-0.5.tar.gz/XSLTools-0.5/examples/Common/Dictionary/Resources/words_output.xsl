...
<axsl:apply-templates select="words" mode="id2287313"/>
</html>
  </axsl:template>
  <axsl:template match="words" mode="id2287313">
    <body xmlns="http://www.w3.org/1999/xhtml">
<form action="" method="POST">

<axsl:apply-templates select="entry" mode="id2283073"/>
<p>
  ...
  <axsl:template match="entry" mode="id2283073">
    <div xmlns="http://www.w3.org/1999/xhtml"> <!-- id="{template:this-element()}" template:section="word-node" -->
  <p>
  ...
  <axsl:apply-templates select="matches" mode="id2287245"/>
</div>
  </axsl:template>
  <axsl:template match="matches" mode="id2287245">
    <p xmlns="http://www.w3.org/1999/xhtml" id="{template:this-element()}" template:section="matches-node">
    Matches:
    <axsl:choose><axsl:when test="@word"><axsl:variable name="this-name">word</axsl:variable><axsl:variable name="this-value" select="@word"/><select name="{template:this-attribute()}">
      <!-- onchange="requestUpdate(
        ...
        '/words/entry')" -->
      <axsl:apply-templates select="match-enum" mode="id2327252"/>
  ...
  <axsl:template match="match-enum" mode="id2327252">
