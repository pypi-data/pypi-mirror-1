...
# TODO: Consider having a different alpha for each phi
    if no:#multicompartimental
        denslist=[]
        for i in xrange(no):
            denslist.append((array(q2phi[i]['y'])/array(q1est[i]['y']))**(1-alpha)) #pairwise pooling of the phis and q2phis
...
##==============================================================================
    if no:#multicompartimental
        ...
        for i in xrange(no):
            ...
            bin_bound.append(arange(ll,ul,(ul-ll)/512.)) # Bin boundaries of the weight vector
        ...
        bin_bound = arange(ll,ul,(ul-ll)/512.) # Bin boundaries of the weight vector
...
##===============================================================================
    ...
    while len(q)< L: # Extract L samples from q1theta_filt
        ...
        if random.random()<= wi[i]: 
            q.append(q1theta_filt[:,i])# retian the sample according with resampling prob.
