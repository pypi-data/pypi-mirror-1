...
import wx
import os,cPickle
from Bayes import flib,lhs, Melding as meld
...
class LHS(wxFrame):
    ...
    def genPriorList(self):
        ...
        for i in xrange(npar):
            self.priors[0].append(self.specs['P[%s]'%i]['dist'])
            self.priors[1].append(tuple([float(n) for n in self.specs['P[%s]'%i]['pars'].strip().split(' ')]))
        for i in xrange(neq):
            self.priors[0].append(self.specs['Y[%s]'%i]['dist'])
            self.priors[1].append(tuple([float(n) for n in self.specs['Y[%s]'%i]['pars'].strip().split(' ')]))
