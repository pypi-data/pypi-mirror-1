...
from ilrt.contentmigrator.ContentMigrator.config import FOLDERS, SEPARATOR
...
class SFWA(StructureFolderWalkingAdapter):
    ...
    def __init__(self, context):
        ...
        if MIGRATE_WORKFLOW:
            self.wfmt = WorkflowMigrationView(context,getattr(self,'REQUEST',{}))
            self.transmap = {}
        else:
            self.wfmt = None
            ...
            if meta:
                ...
                if data:
                    if portal_type == 'Image' or content_type.startswith('image'):
                        obj.setImage(data, content_type)
                    else:
                        obj.setFile(data)
            else:
    ...
    def do_transitions(self, obj, wf, from_state, to_state, import_context):
                    ...
                    for i, wf in enumerate(workflows):
                        if wf in chain: 
                            ...
                            if states[i] != from_state:
                                self.do_transitions(obj, wf, from_state,
                                                    states[i], import_context)
                                flag_done = 1
                if not flag_done:
                    wf = chain[0]
                    from_state = self.wfmt.workflow_tool.getInfoFor(obj,
    ...
    def at_demarshall(self, obj, metadata, import_context, subdir):
        ...
        for f in fields:
            ...
            if value:
                ...
                if not written:
                    self.out.append('Failed to update %s.%s = %s' % _
                                            (subdir, obj.getId(), name, value))
