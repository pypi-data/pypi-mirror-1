...
try:
    from experimental.catalogqueryplan.catalog import ADVANCEDTYPES
    from experimental.catalogqueryplan.catalog import VALUETYPES
except ImportError:
    ADVANCEDTYPES = []
    VALUETYPES = []
...
class BooleanIndex(UnIndex):
    """Index for booleans
       ...
       self._index = {True:[documentId1, documentId2]}
    ...
    def _apply_index(self, request, cid='', type=type, res=None):
        ...
        for key in record.keys:
            if key:
                # If True, check index
                return intersection(index, res), (self.id,)
            else:
...
ADVANCEDTYPES.append(BooleanIndex)
VALUETYPES.append(BooleanIndex)
