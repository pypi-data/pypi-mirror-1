...
def schedule():
    '''Check for signals and yield control of execution.

    In a preemptive execution model, this function does nothing
    except check for signals. In a cooperative model, this function
    also gives other actors a chance to run.
    '''
    _controller._current_actor().schedule()
...
def start_greenlet_controller(entry_point, *args, **kwargs):
    import parley.controllers.greenlet as model

    register_controller(model.GreenletController())
    spawn(entry_point, *args, **kwargs)
    _controller.run_forever()
...
def become(target, *args, **kwargs):
    '''Call the target function, keeping the same actor ID.

    This is currently equivalent to calling the target function directly,
    except that it transfers control without increasing the stack size.
    This is useful in cases of unbounded recursion.
    
    In the future, this function may also be used to change the mode or site
    of execution without changing the actor ID.
    '''
    
    from parley.actor import Become
    
    raise Become(target, args, kwargs)
