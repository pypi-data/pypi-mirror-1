'''
parley/proxy.py - reference and communicate with spawned actors.
'''

from parley import RPCException, recv
...
class ActorProxy(object):
    ...
    def send(self, msg_name, sender, *args, **kwargs):
        '''
        Send the message (msg_name, sender, args, kwargs) to this actor.
        '''
    ...
    # This function should only used by the controller
    def _send_signal(self, value, sender):
        self.controller.send_signal(self.id, (value, sender))

    def _with_link(self, sender, action):
        '''Make a one-way link to the given actor that exists for the
        duration of the call to action().'''
    ...
    def call(self, msg_name, sender, *args, **kwargs):
        ...
        def send_and_recv():
            ...
            # Note that recv() here receives as the currently running actor
            # (i.e. the calling actor)
            return recv(_is_rpc_reply)
    ...
    def __deepcopy__(self, memo):
        # The default behavior would be to try to deepcopy the controller
        # object, which is not what we want
        return ActorProxy(self.controller, self.id)
        
    def __getattr__(self, msg_name):
        '''
        Return a proxy that, when called, sends a message to this actor,
        using the method name as the name of the message.
        '''
        if msg_name[:2] == '__':
            # Make sure not to confuse python by defining magic methods
            raise AttributeError(msg_name)
