...
class W3cStrict(Parser):
    ...
    def getValidationResults(self, xhtml):
        ...
        try:
            ...
            if self.context.getProxy_address() and self.context.getProxy_password() and self.context.getProxy_userid() and self.context.getProxy_port():
                proxy_info = {
                    'user' : self.context.getProxy_userid(),
                    'pass' : self.context.getProxy_password(),
                    'host' : self.context.getProxy_address(),
                    'port' : int(self.context.getProxy_port())
                }
