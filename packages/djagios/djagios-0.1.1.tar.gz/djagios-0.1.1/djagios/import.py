...
'''
    ...
    def import_contact_to_db(self):
        filename = os.path.join(self.tmp_path, 'contact.cfg')
        s = self._get_file_content(filename)
        logging.debug('Cleaning out the contact objects from db.')
        self._delete_all_objects(Contact)
        logging.debug('Finished cleaning the %s objects from db.'%type)
        ...
        for key,val in mo:
            ...
            for line in val.strip().splitlines():
                ...
                m = re.search(attrib_pattern, line, re.I)
                rdict = m.groupdict()
                value = rdict['value'].split(';')[0]
                if hasattr(o,rdict['attrib']):
                    if rdict['attrib'] == 'contactgroups':
                        try:
                            ...
                            continue
                    if rdict['attrib'] in TP_KEYS:
                        try:
                            setattr(o, rdict['attrib'], TimePeriod.get(value))
                            continue
                        except TimePeriod.DoesNotExist:
                            self.write_queue(filename, key, val)
                            continue
                    if rdict['attrib'] == 'host_notification_commands':
                        try:
                            ...
                            self.write_queue(filename, key, val)
                            continue
                    if rdict['attrib'] == 'service_notification_commands':
                        try:
                            ...
                            self.write_queue(filename, key, val)
                            continue
                    if isinstance(o._meta.get_field_by_name(rdict['attrib'])[0],\
                        models.NullBooleanField):
                        if value == '1':
                            setattr(o, rdict['attrib'], True)
                        else:
                            setattr(o, rdict['attrib'], False)
                        continue
                    setattr(o, rdict['attrib'], value)
