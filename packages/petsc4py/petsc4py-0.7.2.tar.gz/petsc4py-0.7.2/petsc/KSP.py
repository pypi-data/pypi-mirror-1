...
respectively) or a sequential direct solver.

KSP users can set various Krylov subspace options at runtime via the
options database. KSP options can also be set programmatically by
directly calling specific KSP routines (e.g., 'KSP.setType()'). KSP
components can be used directly to create and destroy solvers; this is
normally not needed for users but is intended for library developers.
"""
...
class KSP(Object):
    ...
    def getConverged(self):
        ...
        reason_str = _petsc.KSPConvergedReasonString(reason)
    ...
    def setResidualHistory(self, length=None, reset=True):
        """
        Log residual history.
        """
        if length is None:
            length = _petsc.PETSC_DECIDE
        _petsc.KSPAllocResHistory(self, length, reset)

    def getResidualHistory(self):
        """
        Get logged residual history.
        """
    ...
    def computeEigenvalues(self, neig=None):
        """
        Compute the extreme eigenvalues for the preconditioned
        operator.
        """
    ...
    def computeEigenvaluesExplicitly(self, neig=None):
        """
        Compute all of the eigenvalues of the preconditioned operator
        using LAPACK.

        .. note:: This approach is very slow but will generally
           provide accurate eigenvalue estimates.  This routine
           explicitly forms a dense matrix representing the
           preconditioned operator, and thus will run only for
           relatively small problems, say n < 500.

        .. tip:: Many users may just want to use the monitoring
           routine KSPSingularValueMonitor() (which can be set with
           option -ksp_singmonitor) to print the singular values at
           each iteration of the linear solve.
        """
    ...
    def computeExtremeSingularValues(self):
        """
        Compute the extreme singular values for the preconditioned
        operator
        """
    ...
    pc_side = property(getPCSide, setPCSide)

    nullspace = property(getNullSpace, setNullSpace)

    norm_type = property(getNormType, setNormType,
    ...
    resnorm    = rnorm = property(getResidualNorm)
...
class PC(Object):
    ...
    def setDiagonalScale(self, vec):
        """
        Indicates the left scaling to use to apply an additional left
        and right scaling as needed by certain time-stepping codes.
        """
    ...
    def diagonalScaleRight(self, vec_in, vec_out):
        """
        Scales a vector by the right scaling as needed by certain
        time-stepping codes.
        """
