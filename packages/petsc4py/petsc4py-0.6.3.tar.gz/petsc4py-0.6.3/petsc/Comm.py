...
class Comm(_petsc.Comm):
    ...
    def duplicate(self):
        """
        Duplicates the communicator only if it is not already a PETSc
        communicator.
    ...
    def getSize(self):
        """
        Returns communicator size.
        """
        return _petsc.PetscCommGetSize(self)
    
    def getRank(self):
        """
        Returns processor rank in a communicator.
        """
        return _petsc.PetscCommGetRank(self)

    def barrier(self):
        """
        Blocks until this routine is executed by all processors.
        """
        _petsc.PetscCommBarrier(self)

    def globalMin(self, local):
        """
        Computes the minimum (real) value over several processors.
        """
        return _petsc.PetscGlobalMin(local, self)

    def globalMax(self, local):
        """
        Computes the maximum (real) value over several processors.
        """
        return _petsc.PetscGlobalMax(local, self)                

    def globalSum(self, local):
        """
        Computes the (scalar) sum over several processors.
        """
...
"""Rank of this processor on `COMM_WORLD`"""
