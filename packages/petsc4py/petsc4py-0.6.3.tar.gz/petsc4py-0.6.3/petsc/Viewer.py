...
class Viewer(Object):
    ...
    def createDraw(self, title=None, position=None, size=None,
        ...
        :Parameters:
          ...
          - `size`: tuple '(w, h)' specifying window width and height
            ...
            `ViewerDraw.WindowSize` for other possibilities.
        ...
        if size is None or size == _petsc.PETSC_DECIDE:
            ...
            size = _petsc.get_attr(ViewerDraw.WindowSize, size)
            if type(size) is int:
                w = h = size
            else:
                w, h = size
    ...
    ## def createString(self, buff, comm=None):
    ##     """
    ##     """
    ##     return _petsc.PetscViewerStringOpen(comm, buff, self)
