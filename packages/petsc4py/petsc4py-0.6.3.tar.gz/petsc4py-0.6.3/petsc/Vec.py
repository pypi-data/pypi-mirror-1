...
class Vec(Object):
    ...
    def createGhost(self, size, ghosts, bsize=None, comm=None):
       ...
       :Parameters:
        ...
        .. note:: `createGhost()` constructs the global vector
           representation (without ghost points as part of vector).
           To obtain de local vector representation, call
           `VecGhost.getLocalForm()`.

        .. hint:: 

           If ``vg`` is a `VecGhost` instance, use the following to
           update the ghost regions with correct values from the
           owning process::
    ...
    def setValues(self, indices, values, insert_mode=None):
        ...
        :Parameters:
          ...
          - `insert_mode`: either
            + `INSERT_VALUES` to replaces existing entries with new
              values.
            + `ADD_VALUES` to add values to any existing entries.
        """
    ...
    # Array Interface V2, Python side
    __array_shape__    = property(_petsc.Vec__array_shape__,
                                  doc='Array protocol: shape')
    __array_data__     = property(_petsc.Vec__array_data__,
                                  doc='Array protocol: data')
    __array_typestr__  = property(_petsc.Vec__array_typestr__,
                                  doc='Array protocol: typestr')
...
class VecShared(Vec):
    ...
    def __init__(self, *targs, **kwargs):
        """See `Vec.createShared()`"""
        super(VecShared, self).__init__(*targs, **kwargs)
        self.createShared(*targs, **kwargs)


class VecGhost(VecMPI):

    """
    Parallel vector with ghost padding on each processor.
    """

    def __init__(self, *targs, **kwargs):
        """See `Vec.createGhost()`"""
        super(VecMPI, self).__init__(*targs, **kwargs)
        self.createGhost(*targs, **kwargs)
...
class Scatter(Object):
    ...
    def create(self, vec_from, is_from, vec_to, is_to):
        ...
        :Parameters:
          ...
          - `is_from`: the indices of `vec_from` to scatter (or `None`
             to scatter all values).
