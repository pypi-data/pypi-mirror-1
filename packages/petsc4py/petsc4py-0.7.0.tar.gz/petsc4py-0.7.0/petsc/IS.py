...
class IS(Object):
    ...
    def createGeneral(self, indices, comm=None):
        """
        Creates an index set containing a general list of integers.
    ...
    def createBlock(self, indices, bsize, comm=None):
        """
        Creates an block index set containing a list of integers and a
    ...
    def createStride(self, size, first=None, step=None, comm=None):
        """
        Creates an stride index set containing a list of evenly spaced
        integers.
    ...
    def sum(self, iset):
        ...
        .. note:: If n1 and n2 are the sizes of the sets, this takes
           O(n1+n2) time; if `iset` is a subset of `self`, `self` is
           left unchanged, otherwise `self` is reallocated. Both index
           sets need to be sorted on input.
        """
...
class LGMapping(Object):
    ...
    """
    Mappings from an arbitrary local ordering from 0 to n-1 to a
