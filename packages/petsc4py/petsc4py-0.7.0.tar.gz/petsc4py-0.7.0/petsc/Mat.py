...
class Mat(Object):
    ...
    class InfoType:
        """
        Indicates if you want information about the local part of the
    ...
    class AssemblyType:
        """
        Indicates if the matrix is now to be used, or if you plan to
    ...
    class Structure:
        """
        Indicates if the matrix has the same nonzero structure.
        """
        # native
    ...
    def create(self, comm=None):
        """
        Creates a matrix object. The local and global row and column
        sizes can be set with `setSizes()` and the type can then be
        set with `setType()` (or perhaps `setFromOptions()`).
    ...
    def createIS(self, size, lgmap, comm=None):
        """
        Creates a *process unassmembled* matrix, it is assembled on
    ...
    def createNormal(self, matrix):
        """
        Creates a matrix that behaves like A'*A.
    ...
    def createShell(self, size, context, comm=None):
        """
        Creates a shell matrix to be used to define your own matrix
    ...
    def getLocalSize(self):
        """
        Returns the local number of rows and columns in a matrix.
        ...
        .. caution::
           This information may be implementation dependent.
        """
        return tuple(_petsc.MatGetLocalSize(self))

    def getSizes(self):
        """
        Returns the local and global number of rows and columns in a
        matrix.
    ...
    def setSizes(self, sizes, bsize=None):
        """
        Sets the local and global number of rows and columns, and
        checks to determine compatibility.
        """
    ...
    def getBlockSize(self):
        """
        Returns the matrix block size; useful especially for the block
    ...
    def setBlockSize(self, bsize):
        """
        Sets the matrix block size; for many matrix types you cannot
    ...
    def setOption(self, option):
        """
        Sets parameter options for a matrix. Some options may be
    ...
    def duplicate(self, copy_values=False):
        """
        Duplicates a matrix structure, and optionally copies its
        values.
        """
        if copy_values:
    ...
    def copy(self, mat=None, structure=None):
        """
        Copies a matrix structure and its values.
        """
        if mat is None:
    ...
    def Load(viewer, mat_type=None):
        """
        Loads a matrix that has been stored in binary format with
        `View()`. Generates a parallel MPI matrix if the
    ...
    def compress(self):
        """
        Tries to store the matrix in as little space as possible. May
    ...
    def getColumnVector(self, vec, col):
        ...
        .. notes:: Each processor for which this is called gets the
    ...
    def setLGMapping(self, lgmap):
        """
        Sets a local-to-global numbering for use by `setValuesLocal()`
    ...
    def setValueLocal(self, row, col, value, insert_mode=None):
        """
        Sets a single entry from a matrix using a local
        (per-processor) numbering..
        """
    ...
    def setValuesLocal(self, rows, cols, values, insert_mode=None):
        """
        Sets a block of values from a matrix using a local
        (per-processor) numbering.
        """
    ...
    def getVecLeft(self):
        """
        Gets a vector that the matrix vector product can be stored in.
        """
    ...
    def getDiagonal(self, diag):
        """
        Gets the diagonal of a matrix in a vector, A[i,i] -> diag[i]
    ...
    def getRowMax(self, vec):
        """
        Gets the maximum value (in absolute value) of each row of the
        matrix
        """
    ...
    def diagonalSet(self, D, insert_mode=None):
        """
        Computes Y <- Y + D (where D is a diagonal matrix that is
    ...
    def zeroRows(self, rows, diag=1.0):
        ...
        .. note::
           ...
           For the `AIJ` and `BAIJ` matrix formats this removes the
           old nonzero structure, but does not release memory.  For
           the dense and block diagonal formats this does not alter
    ...
    def zeroRowsLocal(self, rows, diag=1.0):
        ...
        .. note::
           ...
           Before calling `ZeroRowsLocal`(), the user must first set
           the local-to-global mapping by calling
           `setLGMapping()`.
    ...
    def getSubMatrix(self, isrow, iscol, submat=None, csize=None):
        """
        Gets a single submatrix on the same number of processors as
        the original matrix.
        ...
        .. note::
           ...
           The `iscol` argument MUST be the same on each
    ...
    def axpy(self, alpha, X, matstr=None):
        ...
        elif matstr != _petsc.DIFFERENT_NONZERO_PATTERN:
            if matstr != _petsc.SAME_NONZERO_PATTERN:
                raise ValueError('invalid value for `matstr`')
    ...
    def matMultSymbolic(self, mat, fill=1.0):
        """
        Performs construction, preallocation, and computes the ij
    ...
    def matMultNumeric(self, mat, result):
        """
        Performs the numeric matrix-matrix product.  Call this routine
    ...
    def interpolate(self, x, y):
        """
        Computes y <- A*x or A'*x depending on the shape of the matrix
        ...
        .. note::
           ...
           This allows one to use either the restriction or
    ...
    def interpolateAdd(self, x, y, w):
        """
        Computes w <- y + A*x or A'*x depending on the shape of the
        matrix
    ...
    def getOrdering(self, ord_type):
        """
        Gets a reordering for a matrix to reduce fill or to improve
    ...
    def factorCholesky(self, isperm, options=None):
        """
        Performs in-place Cholesky factorization of a symmetric
        matrix.
        ...
        .. note::
           ...
           See `LUFactor()` for the nonsymmetric case.

           Most users should employ the simplified KSP interface for
    ...
    def factorSymbolicCholesky(self, isperm, options=None):
        """
        Performs symbolic Cholesky factorization of a symmetric
    ...
    def factorNumericCholesky(self, mat, options=None):
        """
        Performs numeric Cholesky factorization of a symmetric
    ...
    def factorLU(self, isrow, iscol, options=None):
        ...
        .. note::
           ...
           This changes the state of the matrix to a factored matrix;
           it cannot be used for example with `setValues()` unless one
           first calls `setUnfactored()`.
        """
        if options is not None:
            factorinfo = self.FactorInfo(options)
        else:
    ...
    def factorSymbolicLU(self, isrow, iscol, options=None):
        """
        Performs symbolic LU factorization of matrix.  Call this
    ...
    def factorNumericLU(self, mat, options=None):
        """
        Performs numeric LU factorization of a matrix.  Call this
    ...
    def factorICC(self, isperm, options=None):
        ...
        .. note::
           ...
           otherwise allocates new space to store factored matrix and
           deletes previous memory.
    ...
    def factorSymbolicICC(self, isperm, options=None):
        """
        Performs symbolic incomplete Cholesky factorization for a
    ...
    def factorSymbolicILU(self, isrow, iscol, options=None):
        """
        Performs symbolic ILU factorization of a matrix.  Uses levels
    ...
    def forwardSolve(self, b, x):
        """
        Solves L x = b, given a factored matrix, A = LU.
        """
    ...
    def getShellContext(self):
        """
        Gets the user-provided context object associated with a shell
        matrix.
        """
    ...
    def setShellContext(self, context):
        """
        Sets the user-provided context object associated with a shell
        matrix.
        """
...
class MatSeqDense(Mat):
    ...
    def __init__(self, *targs, **kwargs):
        """
        Creates a sequential dense matrix
...
class MatSeqAIJ(Mat):
   ...
   .. note::
      ...
      compressed row storage), is fully compatible with standard
    ...
    def __init__(self, *targs, **kwargs):
        ...
        .. note::
           Specify the preallocated storage with `nz`. Set `nz` to
           `DEFAULT` or ``None`` for PETSc to control dynamic memory
           allocation.
        """
...
class MatMPIAIJ(Mat):
    ...
    def __init__(self, *targs, **kwargs):
        ...
        :Parameters:
           - `lsize`: tuple ``(m,n)`` with number of local rows
              ``m`` and columns ``n`` (or `DECIDE` to have calculated
              if `gsize` is given). This value should be the same as
              the local size used in creating the y vector for the
              matrix-vector product y = Ax. For square matrices ``m``
              is almost always ``n``.
           - `gsize`: tuple ``(M,N)`` with number of global rows
             ``M`` and columns ``N`` (or `DETERMINE` to have
             calculated if `lsize` is given). For square matrices
             ``M`` is almost always ``N``.
           - `d_nz`: number of nonzeros per row in DIAGONAL portion
              of local submatrix (same value is used for all local
              rows).
           - `d_nnz`: array containing the number of nonzeros in the
              ...
              ``None``, if `d_nz` is used to specify the nonzero
           ...
           - `o_nnz`: array containing the number of nonzeros in the
              ...
              submatrix (possibly different for each row) or
              ``None``, if `o_nz` is used to specify the nonzero
              structure. The size of this array is equal to the
...
class MatIS(Mat):
    ...
    def getLocalMat(self):
        """
        Gets the local matrix stored inside a IS matrix.

        .. note:: This can be called if you have precomputed the
        nonzero structure of the local matrix and want to provide it
        to the inner matrix object to improve the performance of the
        `setValues()` operation.
        """
...
class MatNormal(Mat):
    ...
    """
    Matrix that behaves like A'*A. See `Mat.CreateNormal()`.
    """
...
class NullSpace(Object):
    ...
    def create(self, vecs=None, has_const=False, comm=None):
        """
        Creates a data structure used to project vectors out of null
        spaces.
    ...
    def test(self, mat):
        """
        Tests if the claimed null space is really a null space of a
        matrix
        """
