...
%typemap(typecheck, precedence=SWIG_TYPECHECK_POINTER, noblock=1) PETSC_t OPTIONAL
{ $1 = ($input == Py_None) || Py##PETSc_t##_Check($input) ; }
%typemap(arginit)       PETSC_t OPTIONAL "$1 = PETSC_NULL;";
%typemap(in, noblock=1) PETSc_t OPTIONAL
{
  if ($input != Py_None) {
    $1 = Py##PETSc_t##_AsVal($input);
    if (PyErr_Occurred()) %argument_fail(SWIG_TypeError, "$type", $symname, $argnum);
...
%typemap(check, noblock=1)  PETSc_t OPTIONAL
{
  if ($1 != PETSC_NULL && !PETSC_chkobj($1, COOKIE)) SWIG_fail;
}
...
{
  if ($1 != NULL && !PETSC_chkobj(*$1, COOKIE)) SWIG_fail;
}
...
%typemap(argout)   PETSc_t* CREATE
{
  PyObject* robj;
  if ($input == Py_None) {
    robj = Py##PETSc_t##_##New(temp$argnum);
  } else {
    if (_obj_destroy((PetscObject)temp$argnum) != 0)
      if (PyErr_Warn(PyExc_RuntimeWarning, 
		     "trying to destroy a PETSc_t object") < 0) SWIG_fail;
    temp$argnum = ($*ltype) PyPetscObject_OBJ($input);
    _obj_clean(PyPetscObject($input));
    PyPetscObject_OBJ($input) = (PetscObject) temp$argnum;
    Py_INCREF($input);
    robj = $input;
  }
  %append_output(robj);
...
/* return value   */
/* -------------- */
%typemap(out, noblock=1) PETSc_t OUTNEW {
  if (!PETSC_chkobj($1, COOKIE)) SWIG_fail;
  %set_output(Py##PETSc_t##_##New($1)); 
}
%typemap(out, noblock=1) PETSc_t OUTREF {
  if (!PETSC_chkobj($1, COOKIE)) SWIG_fail;
...
%typemap(in, numinputs=0, noblock=1) 
Type*, Type* OUTPUT ($*ltype temp=0) { $1 = &temp; }
%typemap(argout,noblock=1,fragment=from_frag) 
Type*, Type* OUTPUT { %append_output(from_meth((*$1))); }
%enddef
...
PetscEnum* OUTPUT { %append_output(SWIG_From(PetscEnum)((*$1))); }
...
%define PETSC_ENUM_INIT_VALUE(EnumType, ENUM_INIT_VALUE)
%typemap(in, numinputs=0, noblock=1)
EnumType* OUTPUT ($*ltype temp=ENUM_INIT_VALUE) { $1 = &temp; }
%apply EnumType* OUTPUT { EnumType* }
%enddef /* PETSC_ENUM_INIT_VALUE */

%define PETSC_ENUM_DEFAULT_VALUE(EnumType, ENUM_DEFAULT_VALUE)
%typemap(arginit, noblock=1) EnumType { $1 = ENUM_DEFAULT_VALUE; }
