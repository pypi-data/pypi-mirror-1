...
from collective.lineage.content import ChildFolder

from collective.portlets.lineage import LineageNewsPortlet, LineageEventsPortlet, LineageRecentPortlet

from collective.portlets.lineage.tests.base import TestCase
...
class LineageEventsPortletTest(TestCase):

    def afterSetUp(self):
        setHooks()
        setSite(self.portal)
        self.addProfile('collective.lineage:default')
...
class LineageNewsPortletTest(TestCase):
...
class LineageRecentPortletTest(TestCase):
...
def test_suite():
    ...
    suite.addTest(makeSuite(LineageEventsPortletTest))
    suite.addTest(makeSuite(LineageNewsPortletTest))
    suite.addTest(makeSuite(LineageRecentPortletTest))
    return suite
