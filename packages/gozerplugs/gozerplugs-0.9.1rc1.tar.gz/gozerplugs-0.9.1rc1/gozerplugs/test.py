...
'gcalc', 'mail', 'part', 'cycle', 'exception', 'fleet']
...
def handle_testplugs(bot, msg):
    if bot.cfg['type'] == 'irc' and not msg.isdcc:
    ...
    for i in range(loop):
        ...
        for example in examplez:
            ...
            teller += 1
            if bot.jabber:
                from gozerbot.xmpp.message import Message
...
def handle_testsrun(bot, ievent):
    if bot.cfg['type'] == 'irc' and not ievent.isdcc:
    ...
    ievent.reply('%s tests run .. %s errors' % (teller, len(tests.err)))
    if err:
        ievent.reply('errors: ', tests.err)
    if toolate:
...
def handle_testrunthread(bot, ievent):
    if bot.cfg['type'] == 'irc' and not ievent.isdcc:
        ievent.reply('use this command in a /dcc chat with the bot')
        return
    toolate = []
    ...
    try:
        ...
        loop = 1
    teller = 0
    tests.dotests(bot, ievent, threaded=True)
    ievent.reply('%s tests run .. %s errors' % (teller, len(tests.err)))
    if err:
        ievent.reply('errors: ', tests.err)
    if toolate:
...
cmnds.add('test-runthread', handle_testrunthread, 'OPER')
