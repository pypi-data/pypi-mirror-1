...
from gozerbot.database.alchemy import Base, create_session, trans, create_all, engine
...
class KarmaDb(object):
    ...
    def getwhoup(self, item):
        ...
        item = item.lower()
        s = create_session()
        karma = s.query(WhoKarma).filter(WhoKarma.item==item).filter(WhoKarma.updown=='up').all()
        if karma:
    ...
    def getwhodown(self, item):
        ...
        item = item.lower()
        s = create_session()
        karma = s.query(WhoKarma).filter(WhoKarma.item==item).filter(WhoKarma.updown=='down').all()
        if karma:
            return [k.nick for k in karma]
