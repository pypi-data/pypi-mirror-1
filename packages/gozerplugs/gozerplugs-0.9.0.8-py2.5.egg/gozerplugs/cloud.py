...
result.error)
    ...
    def start(self, regname, regport, booturl=None):
        ...
        for name, url in self.startup['start'].iteritems():
            self.add(name, url)
        self.boot(regname, regport, booturl)
    ...
    def boot(self, regname, regport, url=None):
        rlog(10, 'cloud', 'booting')
        if url:
            self.sync(url)
        else:
            self.sync('http://gozerbot.org:10101')
...
def handle_cloudboot(bot, ievent):
    ...
    start = time.time()
    cloud.boot(cfg.get('name'), cfg.get('port'), url)
    time.sleep(5)
    teller = 0
    for node in cloud.nodes.values():
        if node.synced > start:
            teller += 1
    if teller:
        ievent.reply('cloud added %s nodes' % teller)
    else:
...
def handle_cloudfullboot(bot, ievent):
    ...
    if not cfg.get('enable'):
        ...
        return
    cloud.fullboot()
    ievent.reply('current %s nodes in list' % size())
    ievent.closequeue = False
