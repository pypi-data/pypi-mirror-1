...
"set this to 1 if you want to skip merge commits"}
...
'tinyurl'):
                        ...
                        resultstr += u"%s - " % striphtml(item).strip()
                except KeyError:
                    continue
...
response), fromm=rssitem.name)
    ...
    def feeds(self, botname, channel):
        ...
        result = []
        for j, z in self.data.iteritems():
            if jsonstring([botname, channel]) in z.watchchannels:
...
def handle_rssstart(bot, ievent):
    ...
    if not rssitem.running:
        ...
        return
    if jsonstring([bot.name, target]) in rssitem.watchchannels:
        ievent.reply('we are already monitoring %s on (%s,%s)' % \
(name, bot.name, target))
        return
    rssitem.watchchannels.append(jsonstring([bot.name, target]))
...
def handle_rssstop(bot, ievent):
    ...
    if not rssitem.running:
        ...
        return
    try:
        rssitem.watchchannels.remove(jsonstring([bot.name, target]))
    except ValueError:
        try:
            rssitem.watchchannels.remove(jsonstring([bot.name, target]))
        except ValueError:
...
(name, botname, channel))
        return
    rssitem.watchchannels.append(jsonstring([botname, channel]))
    watcher.save()
...
def handle_rssdelchannel(bot, ievent):
    ...
    if rssitem == None:
        ievent.reply("we don't have a %s rss object" % name)
        return
    if jsonstring([botname, channel]) not in rssitem.watchchannels:
        ievent.reply('we are not monitoring %s on (%s,%s)' % \
(name, botname, channel))
        return
...
cmnds.add('rss-url', handle_rssurl, ['RSS', 'USER'])
...
'rss-seturl gozerbot http://gozerbot.org/hg/gozerbot/?cmd=changelog;style=rss')
