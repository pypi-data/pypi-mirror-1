...
def handle_snarf(bot, ievent, direct=True):
    ...
    if not ievent.rest and (not urlcache.has_key(bot.name.lower()) or not \
urlcache[bot.name.lower()].has_key(ievent.printto.lower())):
        ievent.missing('<url>')
        return
    elif not ievent.rest:
        url = urlcache[bot.name.lower()][ievent.printto.lower()]
    else:
        url = ievent.rest
    try:
        url = valid_url(url)
