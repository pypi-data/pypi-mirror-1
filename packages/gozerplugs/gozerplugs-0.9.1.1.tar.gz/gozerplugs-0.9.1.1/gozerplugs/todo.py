...
from gozerbot.database.alchemy import Base, create_session, trans, create_all, byname
    ...
    try:
        ...
        for name, itemlist in oldpersist.data.iteritems():
            for item in itemlist:
                ...
                todo.add(name, item.descr, item.time, duration, item.warnsec, priority)
                teller += 1  
    except IOError, ex:
        ...
        rlog(10, 'todo', "upgraded %s items" % teller)
...
class TodoDb(object):
    ...
    def toolate(self, name):
        ...
        s = create_session()
        items = s.query(Todo).filter(Todo.name==name.lower).filter(Todo.time<datetime.now()).all()
        return items
    ...
    def withintime(self, name, time1, time2):
        ...
        name = name.lower()
        s = create_session()
        items = s.query(Todo).filter(Todo.name==name.lower).filter(Todo.time>datetime.fromtimestamp(time1)).filter(Todo.time<datetime.fromtimestamp(time2)).all()
        return items
...
tests.add('todo-chan program the bot', 'program the bot')
