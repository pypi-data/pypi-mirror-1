...
from gozerbot.database.alchemy import Base, create_session, trans, create_all, engine, query
...
class KarmaDb(object):
    ...
    def add(self, session, item, value):
        item = item.lower()
        if query(Karma, session).filter(Karma.item==item).count():
    ...
    def delete(self, session, item):
        """ delete karma item """
        item = item.lower()
        karma = query(Karma, session).filter(Karma.item==item).first()
        if karma:
