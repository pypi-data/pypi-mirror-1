...
from gozerbot.database.alchemy import Base, create_session, trans, transfunc, create_all, startmaindb, query
...
def handle_bdset(session, bot, ievent):
    ...
    if not name:
        ievent.reply("can't find a user for %s" % ievent.userhost)
        return
