...
class HTTPConnection(object):
    ...
    def proxy_complete2(self, proxy, complete=False):
        if complete:
            self.close()
        else:
            self.setup()
...
#        tbinfo()
    ...
    def read(self, data):
        ...
        if proxy_info:
            ...
            if self.proxy:
                ...
                if self.request.headers.get("connection", "Close") == "keep-alive" and self.keepalive_enabled:
                    self.debug("Keep-alive: %s" % self.request.headers.get("keep-alive", -1))
                    self.keepalive = int(self.request.headers.get("keep-alive", 300))
                self.proxy_id += 1
                id = str(self.id) + '.' + str(self.proxy_id) + '.' + '1'
                self.proxy = Proxy(self, addr, port, self.sock, buffer=a, id=id, keepalive=self.keepalive )
                self.set_keepalive_timer()
                self.revent.delete()
                self.revent = None
            return None
    ...
    def set_keepalive_timer(self):
        ...
        self.keepalive_timer = event.timeout(self.keepalive, self.keepalive_timeout)
    ...
    def state_completed(self):
        ...
        if target_addr:
            ...
            if ',' in self.session_id:
                self.session_id, self.transport_name = self.session_id.split(',')
            self.complete = True
            debug("completed")
