...
def search(tree, data, ns, nstree, remove_empty_nodes, escape):
    ...
    """
    # Search for all expressions inside a given namespace.
    regexp = re.compile(r"{%s[_\w\.]*}" % ns.rstrip('.'))
    ...
    for i, values in enumerate(data):
        ...
        repl = apply(clone(node), "{%s}" % ns.rstrip('.'),
                values, escape)
        ...
        for k in nstree:
            ...
            repl = search(repl, subdata, '%s%s.' % (ns, k),
                    nstree.get(k), remove_empty_nodes, escape)
...
def apply(tree, match, data, escape=False):
    ...
    if isinstance(data, (int, long, float, basestring)):
        ...
        data = u''
    if escape: data = cgi.escape(data)
    ...
    for node in nodes:
        if isinstance(node, NavigableString):
            ...
            if node.string:
                node.string = regexp.sub(data, node.string)
