...
def validate(form=None, validators=None,
    ...
    If you use simple validators, then you must pass a dictionnary with
    each value name to validate as a key of the dictionnary and the validator
    instance (eg: tg.validators.Int() for integer) as the value.
    @type validators: dictionnary or schema instance

    @param failsafe_schema: TODO: complete this docstring
    fail-safe schema.
    @type failsafe_schema: ???

    @param failsafe_values: TODO: complete this docstring
    replacements for erroneous inputs
    @type failsafe_values: ???

    @param state_factory: TODO: complete this docstring
    callable which returns the initial state instance for validation
    @type state_factory: ???
    """
    def entangle(func):
