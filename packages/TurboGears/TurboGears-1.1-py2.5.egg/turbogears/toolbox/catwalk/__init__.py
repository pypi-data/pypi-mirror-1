...
class CatWalk(turbogears.controllers.Controller):
        ...
        for col in cols:
            ...
            if values.has_key(column_name):
                if col['type'] == 'SODateTimeCol':
                    ...
                    try:
                        b = bool(int(values[column_name]))
                    except ValueError:
                        b = False
                    values[column_name] = b
                elif col['type'] == 'SOFloatCol':
                    try:
                        b = float(values[column_name])
                    except ValueError:
                        b = 0.0
                    values[column_name] = b
                elif col['type'] == 'SOIntCol':
                    try:
                        b = int(values[column_name])
                    except ValueError:
                        b = 0
                    values[column_name] = b
                elif col['type'] == 'SOForeignKey':
                    self.extract_foreign_key(values, column_name)
                elif col['type'] in ('SODecimalCol','SOCurrencyCol'):
    ...
    def extract_decimal_value(self, values, column_name):
        try:
            import decimal
            try:
                b = decimal.Decimal(values[column_name])
    ...
    def state_path(self):
        ...
        can not be determined, below the ccurrent directory.
    ...
    @expose(template='kid:turbogears.toolbox.catwalk.catwalk')
    def index(self):
        """Main CatWalk page.
