...
try:
    ...
    from genshi.input import XMLParser as genshi_parser
    genshi = True
except:
    genshi = False
...
def extract_genshi_strings(filename, options=None):
    ...
    stream = genshi_parser(tmpl, filename=filename)
    translator = Translator(None, **options)
    ...
    def extract_genshi_strings(self):
        ...
        if self.__curfile:
            ...
            for msg in extract_genshi_strings(self.__curfile):
                # XXX: We should split messages with inline Python code and add
                # the resulting sub-strings to the translatable strings.
                if msg[2] and not self.__contains_inline_python(msg[2]):
                    self.__addentry(msg[2], lineno=msg[0], istemplatestring=1)
