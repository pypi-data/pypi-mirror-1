...
"""
    ...
    def scan_genshi_files(self, potfile, files):
        ...
        extract_strings = turbogears.i18n.pygettext.pygettext.extract_genshi_strings
        messages = []
        options = {}
        ...
        for fname in files:
            ...
            for msg in extract_strings(fname, options or None):
                # Genshi 0.6 adds additional 4th element, so we take
                # care to stay compatible
                lineno, text = msg[0], msg[2]
                # Split into substrings around inline Python expressions.
                # XXX: strings with inline Python expressions only get
                # translated properly when the expression is at the start or
                # end of the string.
                for substring in _tmpl_expr_re.split(text):
                    messages.append((lineno, fname, substring.strip()))
    ...
    def scan_kid_files(self, potfile, files):
        ...
        #   turbogears.i18n.pygettext! This should be recfactored.
        messages = []
