...
from turbogears.util import setlike, to_unicode, copy_if_mutable, get_package_name
...
__all__ = ["load_widgets", "all_widgets", "Widget", "CompoundWidget",
           ...
           "Source", "CSSSource", "JSSource", "js_location", "mochikit", "jsi18nwidget",
...
class MochiKitLink(JSLink):
    """
    This is a specialized JSLink to allow for suppression of MochiKit-inclusion
    by core TG.

    The rationale for this is that MK it version 1.3.1 has some issues that are
    remedied by MK 1.4, but the latter is unfortunately unrleased.

    To remedy this, one can include MK 1.4 by oneself (using e.g. tgMochiKit) - but
    this will conflict with the contained version which is made use of in several
    TG widgets.
    ...
    the incusion of MK via built-in widgets will be suppressed. Of course then one
    has to include the proper version explicitly, or via tg.include_widgets.
    ...
    def retrieve_javascript(self):
        if config.get('tg.mochikit_suppress', False):
            if 'turbogears.mochikit' in config.get('tg.include_widgets', []):
                warnings.warn("""tg.mochikit_suppress == True, but 'turbogears.mochikit' is to be included via 'tg.mochikit_suppress'.
This is a contratiction, and mochikit_suppress overrides the inclusion to prevent subtle errors due to version mixing of MochkKit.""")
            return set()
...
class JSI18NWidget(Widget):
    ...
    parameters = ['locale_catalog_providers']
