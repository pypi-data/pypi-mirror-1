...
try:
    ...
    def create_session():
        ...
        #    bind_meta_data()
        return scoped_session(sessionmaker(autoflush=True, transactional=False))
...
[restart_transaction.when("_use_sa(args)")] # include "args" to avoid call being pre-cached
def sa_restart_transaction(args):
    log.debug("Restarting SA transaction")
    req = cherrypy.request
    if hasattr(req, 'sa_transaction') and req.sa_transaction == True:
        session.rollback()

    del session.context.current
    session.begin()
    req.sa_transaction = True
