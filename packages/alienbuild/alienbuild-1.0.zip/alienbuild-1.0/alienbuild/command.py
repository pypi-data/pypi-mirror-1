...
class COMMAND:
	...
	def __init__ ( self, filter, name, cmd, files, arguments, package, dependency, output, outputFilter=aliencommon.DefaultOutputFilter, addDependencyOutputsBackToPackage=None ):
		self.Filter       = filter
		self.Name         = name
		self.Cmd          = cmd
	...
	def __str__ ( self ):
		return 'cmd: %s\n\targs: %s\n\tfiles: %s\n\tdep: %s\n\toutput filter: %s\n\toutput: %s' % ( self.Cmd, self.Arguments, self.Files, self.Dependency, self.OutputFilter, self.Output )
	...
	def Execute ( self ):
		...
		def Replace( y, x ): return y.replace( self.Package.Symbols.Get(x).lower() + os.sep, '' )
		...
		aliencommon.MakeDirectoryForFile( self.Package.Symbols.Get('packagebin') + os.sep + "x" )
	...
	def _HandleDone ( self ):
		...
		self.OutputFilter( self.StdOut, self.StdErr, '[%s] ' % self.Name )
