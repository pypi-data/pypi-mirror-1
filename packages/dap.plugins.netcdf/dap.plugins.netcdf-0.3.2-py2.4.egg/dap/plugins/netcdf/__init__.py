...
try:
    ...
    attrs = lambda obj: obj.__dict__
except ImportError:
    from pupynere import NetCDFFile
    attrs = lambda obj: obj.attributes
...
class Handler(BaseHandler):
    def __init__(self, filepath, environ):
        ...
        # Add description.
        self.description =attrs(self._file).get('title', self.filename)
    ...
    def _parseconstraints(self, constraints=None):
        ...
        dataset.attributes['NC_GLOBAL'] = attrs(self._file).copy()
        ...
        for name in grids:
            if name in fields or not fields:
                ...
                if not grid.dimensions:
                    dataset[name] = dtypes.BaseType(name=name, data=data.getValue(), type=grid.typecode(), attributes=attrs(grid).copy())
                else:
                    a = dataset[name] = dtypes.ArrayType(name=name, data=data, shape=data.shape, dimensions=grid.dimensions, type=grid.typecode())
                    g = dataset[name] = dtypes.GridType(name=name, array=a, attributes=attrs(grid).copy())
            ...
            # Partial grids -- returned as structures.
                ...
                for varname in [var.split('.')[-1] for var in fields if var.split('.')[0] == name]:
                    ...
                    struct_[varname] = dtypes.ArrayType(name=varname, data=data, shape=data.shape, type=var.typecode(), attributes=attrs(var).copy())
        ...
        for name in self._file.dimensions:
            if name in fields or not fields:
                if name not in self._file.variables:
                    ...
                    if slice_: data = data[slice_]
                    dataset[name] = dtypes.ArrayType(name=name, data=data, shape=data.shape, type=array_.typecode(), attributes=attrs(array_).copy())
