...
import math, numpy
from approximatecomparisons import *
import types
...
def _define_and_test_interface(self):
    ...
    see the Piquant.unitsafefunctions module for a way round this. It is better
    ...
    self.assert_(some_scaled_units==sorted_scaled_units)
    # some powered units
    ...
    def inconsistent_operation(a,b,op):
        return op(a,b)
    for op in tryops:
        self.assertRaises(DimensionMismatchError,inconsistent_operation,a,b,op)
    ...
    self.assert_((-a).has_same_dimensions(a))
    self.assert_(abs(a).has_same_dimensions(a))
    self.assert_(-abs(a)<abs(a)) # well why not, this should be true
    ...
    a = 1 * kilogram
    b = 1 * second
    self.assertRaises(DimensionMismatchError,inconsistent_power,a,b)
    
    # check casting rule 1
    a = 1 * kilogram
    b = 2 * kilogram
    for op in [add,sub,mul,div]: self.assert_(isinstance(op(a,b),Quantity))
    
    # check casting rule 2
    a = 1
    b = 1 * kilogram
    for op in [mul, div]: self.assert_(isinstance(op(a,b),Quantity) and isinstance(op(b,a),Quantity))
    for op in [add, sub]:
        self.assertRaises(DimensionMismatchError,inconsistent_operation,a,b,op)
    for op in [add, sub, mul, div]: self.assert_(isinstance(op(a,b/b),Quantity) and isinstance(op(b/b,a),Quantity))

    # check casting rule 3
    self.assert_(isinstance(numpy.array([1,2])*(1*kilogram),numpy.ndarray))
    self.assert_(isinstance((1*kilogram)*numpy.array([1,2]),numpy.ndarray))
    ...
    self.assertRaises(DimensionMismatchError,find_resistance,1,1)
...
def is_dimensionless(obj):
    ...
    Note that the syntax may change in later releases of Piquant, with tighter
...
class Quantity(numpy.float64):
    ...
    change in future releases of Piquant, as we plan to reimplement it
