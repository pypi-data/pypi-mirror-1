...
#                     if dt.uriref() == "http://www.w3.org/1999/02/22-rdf-syntax-ns#XMLLiteral":
                    if dt.value == "http://www.w3.org/1999/02/22-rdf-syntax-ns#XMLLiteral":
                        try:
...
#    def __init__(self, s, dtype, lang): 
#       self.value = s
#       self.datatype = dtype
#       self.language = lang
# 
#    def __str__(self): 
#       value = str(self.value.encode('unicode-escape'))
#       value = value.replace('"', '\\"')
#       if self.datatype: 
#          return '"' + value + '"^^<' + self.datatype + '>'
#       if self.language: 
...
class Formula(object): 
   ...
   def newUniversal(self, uri, why=None): 
      return Var(uri.value.split('#').pop())
   ...
   def close(self): 
      # print 'okay'
      b = bNode('_:Formula%s' % self.number)
      b.formula = True
      return b
...
class RDFSink(object): 
   ...
   def newLiteral(self, s, dt, lang): 
      if dt: return TypedLiteral(s, dt)
      else: return PlainLiteral(s, lang)
   ...
   def normalise(self, f, n): 
      ...
      if isinstance(n, bool): 
         ...
         s = TypedLiteral(str(n).lower(), boolean)
         return s

      if isinstance(n, int) or isinstance(n, long): 
         integer = URI(u'http://www.w3.org/2001/XMLSchema#integer')
         s = TypedLiteral(unicode(n), integer)
         return s
      ...
      if isinstance(n, Decimal): 
         ...
         if value == '-0': 
            value = '0'
         s = TypedLiteral(value, decimal)
         return s

      if isinstance(n, float): 
         double = 'http://www.w3.org/2001/XMLSchema#double'
         s = TypedLiteral(str(n), double)
         return s
