...
__id__ = '$Id: __init__.py 3 2009-02-06 04:05:07Z chris $'
...
class SchemaBot(object):
    def __init__(self, metadata, create_table=True, initial_version=0, log=None):
        ...
        `metadata` : an initialise sqlalchemy.MetaData object that is bound to an
        sqlalchemy engine.
    ...
    def model_version(self, version=None, lowest=False):
        if version is None:
            ...
            if not versions:
                raise SchemaBotError("No schema versions are defined.")
                
            if lowest:
                version = min(versions)     # smallest version number
            else:
    ...
    def version_check(self, version=None):
        ...
        # Returns an int between -1 and 1:
        # 
        # -1 : DB version < version => schema upgrade required
        #  0 : DB version == version => no action required
        #  1 : DB version > version => downgrade required
        """
    ...
    def schema_update(self, version=None):
        ...
        NOTE: some databases do not support CREATE/DROP/etc within a transaction,
        so those operations cannot be rolled back.  In those cases, the database
        may be left in an inconsistent state if an error occurs during upgrade
        or downgrade.
