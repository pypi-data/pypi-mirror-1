...
class TestSchemaBot(unittest.TestCase):
    ...
    def test_schema_update_sa_sequence_upgrade(self):
    ...
    def test_schema_update_sa_sequence_downgrade(self):
        test1_seq = SA.Sequence("test1seq", self.metadata)
        schemabot = SchemaBot(self.metadata)
        schemabot.define_schema(1, upgrade=[test1_seq], downgrade=[test1_seq])
        schemabot.schema_update()
        ...
        # If Sequences aren't supported by engine then skip the test
        if result is None:
            raise nose.SkipTest
        ...
        self.failUnlessRaises(
            (SA.exceptions.OperationalError, SA.exceptions.ProgrammingError),
            self.engine.execute, test1_seq
        )
    
    def test_schema_update_ddl_upgrade(self):
        test1_ddl = SA.DDL("CREATE TABLE foo (val INTEGER)")
        schemabot = SchemaBot(self.metadata)
        schemabot.define_schema(1, upgrade=[test1_ddl])
    ...
    def test_schema_update_ddl_downgrade(self):
        test1_ddl_create = SA.DDL("CREATE TABLE foo (val INTEGER)")
        test1_ddl_drop = SA.DDL("DROP TABLE foo")
        schemabot = SchemaBot(self.metadata)
        schemabot.define_schema(1, upgrade=[test1_ddl_create], downgrade=[test1_ddl_drop])
