...
class ForumAddPost(BrowserContentView):
    ...
    def update(self):
        ...
        if getattr(self.request, 'unsubscribe', None):
            ...
            title = getattr(self.request, 'post_title', None)
            text = getattr(self.request, 'post_text', None)

            now = datetime.today()
            post_id = self.context.invokeFactory('ForumPost', 
                      id=now.strftime('forumpost_%Y-%m-%d.')+str(now.microsecond),
                      title=title, 
                      text=text)
            post = getattr(self.context, post_id)
            post._renameAfterCreation()
            ...
            portal_from = str(portal.getProperty('email_from_address')) + \
                              '<' + str(portal.getProperty('email_from_name ')) + '>'
            ...
            mail['Subject'] = translate('notify', 
                domain = 'garbas.forum',
                target_language = localization_code,
                context = self.context).encode(mail_encoding)
            mail.attach( MIMEText( translate( 'mail_add_new_member_text',
                domain = 'garbas.forum',
                mapping = dict(),
                target_language = localization_code,
                context = self.context).encode(mail_encoding)))
