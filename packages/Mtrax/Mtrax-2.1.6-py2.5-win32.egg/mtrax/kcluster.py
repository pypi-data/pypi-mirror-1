...
def gmmmemberships(mu,S,priors,x,weights=1):
    # number of data points
    n = x.shape[0]
...
def gmmem(x,mu,S,priors,weights=None,niters=100,thresh=.001,mincov=.01):
    ...
    if mincov > 0:
        for i in range(S.shape[2]):
            ...
            #print 'D = ' + str(D)
            D[D<mincov] = mincov
            S[:,:,i] = num.dot(num.dot(U,num.diag(D)),V.transpose())
            #print 'S is now: '
            #print S[:,:,i]
    initcovars = S.copy()
    #print 'initcovars: '
    ...
    for iter in range(niters):
        ...
        fixsmallpriors(x,mu,S,priors,initcovars)

        # E-step: compute memberships
        [gamma,newe] = gmmmemberships(mu,S,priors,x,weights)
    ...
    [gamma,e] = gmmmemberships(mu,S,priors,x,weights)

    return (mu,S,priors,gamma,e)

def fixsmallpriors(x,mu,S,priors,initcovars):
