...
class BackgroundCalculator:
    ...
    def meanstd( self ):
        ...
        # main computation
        nframesused = 0
        for i in range(0,params.n_frames,nframesskip):
            # read in the data
    ...
    def medmad( self ):
        ...
        headersize = params.movie.h_mov.chunk_start+params.movie.h_mov.timestamp_len
        framesize = nr*nc
    ...
    def DoSub( self ):
        ...
        if self.show_img_type == params.SHOW_BACKGROUND:
            (self.img_wind.bitmap,self.resize,self.img_size) = imagesk.draw_image(self.center,windowsize=self.windowsize)
            #self.img = self.center.copy()
        elif self.show_img_type == params.SHOW_DISTANCE:
            #self.img = self.dfore.copy()
            (self.img_wind.bitmap,self.resize,self.img_size) = imagesk.draw_image(self.dfore,windowsize=self.windowsize)
        elif self.show_img_type == params.SHOW_THRESH:
            #self.img = self.bw.copy()
            (self.img_wind.bitmap,self.resize,self.img_size) = imagesk.draw_image(self.bw,windowsize=self.windowsize)
        elif self.show_img_type == params.SHOW_NONFORE:
            #self.img = self.isnotarena.copy()
            (self.img_wind.bitmap,self.resize,self.img_size) = imagesk.draw_image(self.isnotarena,windowsize=self.windowsize)
            ...
            n2 = mu + 3.*sig
            img = self.dev.copy()
            (self.img_wind.bitmap,self.resize,self.img_size) = imagesk.draw_image(img.clip(n1,n2),windowsize=self.windowsize)
        elif self.show_img_type == params.SHOW_CC:
            (L,ncc) = meas.label(self.bw)
            #self.img = colormap_image(L)
            (self.img_wind.bitmap,self.resize,self.img_size) = imagesk.draw_image(colormap_image(L),windowsize=self.windowsize)
            ...
            im = imagesk.mono8_to_rgb8( im )
            # plot on dfore
            (self.img_wind.bitmap,self.resize,self.img_size) = draw_ellipses(im,obs,windowsize=self.windowsize)
            #self.img = draw_targets(im, obs, 1)
    ...
    def DoDraw( self ):
        ...
        #self.scale_x = params.movie.get_width() / self.img_wind.GetRect().GetWidth()
        #self.scale_y = params.movie.get_height() / self.img_wind.GetRect().GetHeight()
        #if self.img.ndim <= 2:
        #    scale_img = self.img[::int(self.scale_y)+1,::int(self.scale_x)+1]
        #    scale_img = scale_img - num.min(scale_img)
        #    scale_img = 255. * scale_img / num.max(scale_img)
        #    self.img_wind.update_image_and_drawings( "img",
        #                                             scale_img.round().astype( num.uint8 ),
        #                                             'MONO8' )
        #else:
        #    scale_img = self.img[::int(self.scale_y)+1,::int(self.scale_x)+1,:]
        #    self.img_wind.update_image_and_drawings( "img",
        #                                             scale_img.astype(num.uint8),
        #                                             'RGB8' )
