...
class MtraxApp( algorithm.MtraxAlgorithm ): # eventually inherits from wx.App
	...
	"""
        print "mtrax:\n\
              ...
              Example:\n\
              mtrax --Interactive=True --Input=movie.1fmf \n\
              --Output=movie1.ann \n\
	...
	# if output ann name is not specified, set it to moviename + .ann
        if (not hasattr(self,'outfilename')) or (self.outfilename is None):
            self.outfilename = self.filename + '.ann'
            self.outdir = self.dir
        if (not hasattr(self,'settingsfilename')) or (self.settingsfilename is None):
	    self.settingsfilename = self.outfilename.copy()
    ...
    def CheckAnnotation( self ):
        ...
        try:
            ...
            print "Failed when reading annotation file"
            self.ann_file = None
            self.ann_data = None
            return False
    ...
    def OpenMovie( self ):
        ...
        try:
            ...
            # get a pointer to the "mtraxmain" child
            img = num.zeros((self.img_size[0],self.img_size[1]),dtype=num.uint8)
            self.img_wind.update_image_and_drawings("mtraxmain",
                                                    img,
                                                    format="MONO8")
	...
	# draw_frame is the frame number relative to when tracking was started
        ...
        if self.menu.IsChecked( xrc.XRCID("menu_playback_show_ann") ) \
            ...
            self.img_wind.update_image_and_drawings('mtraxmain',frame8,
                                                    format='MONO8',
                                                    linesegs=linesegs,
