...
# matplotlib was not being installed correctly on ubuntu, 
# possibly a disagreement between apt and setuptools?
try:
    import matplotlib
    if version.LooseVersion(matplotlib.__version__) < \
            version.LooseVersion('0.91.2'):
...
mtrax_package_data = [ f[6:] for f in glob.glob('mtrax/*.xrc')]+\
                     ...
                     [ f[6:] for f in glob.glob('mtrax/mtraxicon.ico')]
eager_resources = [ f for f in glob.glob('mtrax/*.xrc') ] + \
    [ f for f in glob.glob('mtrax/*.bmp')] + \
    [ f for f in glob.glob('mtrax/mtraxicon.ico)')]
