...
class AppWithSettings( wx.App ):
    ...
    def InitGUI( self ):
        ...
        # default settings
        self.dir = os.environ['HOME'] # Linux only
    ...
    def ReadUserfile( self ):
        ...
        Called on startup."""
        try:
            self.userfile_name = os.path.join( os.environ['HOME'], '.mtraxrc' ) # Linux only
            userfile = open( self.userfile_name, "r" )
            # line 1: last version used
            last_version = userfile.readline().rstrip()
            if last_version != __version__:
                wx.MessageBox( "Welcome to Mtrax version %s"%__version__,
                               "Mtrax updated", wx.ICON_INFORMATION )
    ...
    def WriteUserfile( self ):
        ...
        try:
            ...
            print >> userfile, self.menu.IsChecked( xrc.XRCID("menu_settings_use_colorblind") )
    ...
    def ZoomTarget(self,targ,window):
        ...
        if window is None:
            ...
            if nwindows < maxnwindows:
                ...
                self.zoom_window.n_ell_spinner.SetValue(nwindows+1)
                self.zoom_window.OnNEllSpinner(None)
                window = nwindows
            else:
    ...
    def OnCheckColorblind( self, evt ):
        ...
        if self.menu.IsChecked( xrc.XRCID("menu_settings_use_colorblind") ):
            ...
            paramse.colors = params.normal_palette
