...
def draw_targets( img, targets, thickness, step=10*num.pi/180., colors=params.colors ):
    ...
    for i, target in targetiterator:
        ...
        for tt in range( thickness ): # draw thicker line
            use_width = target.size.width + tt
            if use_width == 0: use_width = 5 # backwards compatibility
            use_height = target.size.height + tt
            if use_height == 0: use_height = 5
            use_angle = target.angle
            # range shouldn't matter if sin() and cos() are any good...
            #while use_angle < 0.: use_angle += 2.*num.pi
            #while use_angle > 2.*num.pi: use_angle -= 2.*num.pi

            alpha = num.sin( use_angle )
            beta = num.cos( use_angle )
            for aa in num.arange( 0., 2.*num.pi, step ):
                ax = use_width * num.cos( aa )
                ay = use_height * num.sin( aa )
                xx = int(num.round( target.center.x + ax*alpha - ay*beta ))
                yy = int(num.round( target.center.y - ax*beta - ay*alpha ))
                if xx < 0 or xx >= s[1] or yy < 0 or yy >= s[0]: continue
                for c in range( 3 ):
                    # TODO: could speed up slightly by checking for duplication first
                    img[yy,xx,c] = colors[i % len(colors)][c]
                #endfor: all color channels
            # endfor: entire arc
        # endfor: each arc (different radii)
    # endfor: all targets
...
class EllipseWindow:
    ...
    def redraw(self):
        ...
        windowsize = [self.window.GetRect().GetHeight(),self.window.GetRect().GetWidth()]
        ...
        if not self.ellipse.hasItem(ind):
            ...
            (self.window.bitmap,resize,img_size) = imagesk.draw_image(blank,windowsize=windowsize)
        else:
            ...
            (self.window.bitmap,resize,img_size) = \
                 draw_ellipses(self.data, [self.ellipse[ind],], colors=[params.colors[ind%len(params.colors)],],
                               windowsize=windowsize,zoomaxes=zoomaxes)
        
        ## scale up ellipse
        #offset_ellipse = self.ellipse.copy()
        #offset_ellipse.center.x -= self.offset[0] # center in frame
        #offset_ellipse.center.y -= self.offset[1]
        #offset_ellipse.center.x *= scale_factor # scale
        #offset_ellipse.center.y *= scale_factor
        #offset_ellipse.size.width *= scale_factor
        #offset_ellipse.size.height *= scale_factor
        #offset_ellipse.center.x += scale_factor/2 # re-center
        #offset_ellipse.center.y += scale_factor/2

        ## scale up image
        #frame = num.zeros( (params.zoom_window_pix*scale_factor,
        #                    params.zoom_window_pix*scale_factor), num.uint8 )
        #for s1 in range( scale_factor ):
        #    for s2 in range( scale_factor ):
        #        frame[s1::scale_factor,s2::scale_factor] = self.data[:]
        #
        ## draw ellipse on image
        #frame_clr = imagesk.to_rgb8( 'MONO8',frame )
        #if self.ellipse:
        #    ell_list = TargetList()
        #    ell_list.append( offset_ellipse )
        #    draw_targets( frame_clr, ell_list,
        #                   thickness=int(num.ceil( scale_factor/2. )),
        #                   step=num.pi/180. )
        #
        ## draw image on window
        #self.window.update_image_and_drawings( "frame", frame_clr, 'RGB8' )
    ...
    def OnNEllSpinner( self, evt ):
        ...
        if new_n_ell > self.n_ell:
            ...
            while use_id in id_list and use_id >= 0:
                use_id -= 1
            # add it
            self.ellipse_windows.append( EllipseWindow( self.img_panel, use_id ) )
            ...
            # remove a window
            self.img_box.Detach( len(self.ellipse_windows)-1 )
            self.ellipse_windows.pop()
...
def draw_ellipses( img, targets, thickness=1, step=10*num.pi/180., colors=params.colors, windowsize=None, zoomaxes=None ):
    """Draw ellipses on a color image (MxNx3 numpy array).
    Refuses to draw empty ellipses (center==size==area==0)."""
...
def add_ellipses( img, bmp, targets, resize, thickness=1, step=10*num.pi/180., colors=params.colors, zoomaxes=None ):
    """Draw ellipses on a color image (MxNx3 numpy array).
    Refuses to draw empty ellipses (center==size==area==0)."""
    ...
    bmp = imagesk.add_annotations(bmp,resize,linelists=linelists,zoomaxes=zoomaxes,linecolors=linecolors)
