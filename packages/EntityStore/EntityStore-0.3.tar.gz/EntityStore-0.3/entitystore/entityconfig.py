...
class Configuration(object):
    ...
    def generate_default(self, filehandle=None, conf='basic', store_base_url="http://localhost/",
        ...
        add_service(default_cfg, "Solr service", "services/bin/solr.sh")
        add_service(default_cfg, "4Store backend", "services/bin/4store.sh")
        ...
        for store in stores:
            ...
            add_worker(default_cfg, 'accesslogger_%s' % store, 'workers/logger.py', binding='%s.r' % store,
                       logfile='logs/%saccess.log' % store)

            add_worker(default_cfg, 'changelogger_%s' % store, 'workers/logger.py', binding='%s.w' % store,
                       logfile='logs/%schange.log' % store)
