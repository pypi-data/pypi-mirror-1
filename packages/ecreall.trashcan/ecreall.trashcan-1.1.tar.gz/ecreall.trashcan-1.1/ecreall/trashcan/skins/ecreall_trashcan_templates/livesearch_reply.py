...
show_trashcan = context.isTrashcanOpened()
if show_trashcan:
    results = catalog(SearchableText=r, portal_type=friendly_types, path=path,
              object_provides='ecreall.trashcan.interfaces.ITrashed')
else:
    contents_normal = catalog(SearchableText=r, portal_type=friendly_types, path=path)
    contents_trash = catalog(SearchableText=r, portal_type=friendly_types, path=path,
                             object_provides='ecreall.trashcan.interfaces.ITrashed')
    contents_trash = [c.UID for c in contents_trash]
    results = [c for c in contents_normal if c.UID not in contents_trash]
