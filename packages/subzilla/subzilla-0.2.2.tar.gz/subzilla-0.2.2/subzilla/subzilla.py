...
def login():
    links = (l for l in state.links() if login_pat.match(l.url))
    try:
        browser.follow_link(links.next())
    except StopIteration:
        pass
    login_forms = [f for f in state.forms() if f.name == 'login']
    if len(login_forms) == 0:
        return
    promptForSettings(login=True)
    state.form = login_forms[0]
...
def close_bug():
    ...
    state.select_form('changeform')

    # this won't work because of a bug in twill that won't
    # let multiple controls have the same name
...
login_pat = re.compile('show_bug\.cgi.*GoAheadAndLogIn=1')
...
def getOptions():
    ...
    global bug_number, description, cfg, cfg_section_used
    ...
    parser.add_option("-c", "--close", dest="close", action="store_true",
                      help="close the bug, this doesn't work with recent Twill versions because of a bug in form selection")
    ...
    for section in cfg.sections():
        if project is not None:
            ...
            if os.path.commonprefix([root, os.getcwd()]) == root:
                cfg_section_used = section
        if cfg_section_used is not None:
...
def promptForSettings(login=False):
    global cfg_section_used
    ...
    if options.prompt or not hasattr(options, 'subversion_root'):
        current = os.getcwd()
        print "subzilla version %s" % version
        prompted.subversion_root = raw_input(
            "\nNo project setting found for this Subversion tree.\n\n"
            "What directory do you want to use as the root for creating and\n"
            "applying patches?  Enter a path or press enter to accept the\n"
            "default [%s]: "% current)
    ...
    if options.prompt or not hasattr(options, 'url'):
        prompted.url = raw_input(
            "\nWhat is the URL for the Bugzilla repository associated with \n"
            "this Subversion tree?  You can press enter to accept\n"
            "the default [%s]:" % options.defaulturl)
        if prompted.url == '':
            prompted.url = options.defaulturl
            
    if login:
        if options.prompt or not hasattr(options, 'username'):
            prompted.username = raw_input(
                "\nBugzilla needs a username and password to continue.\n\n"
                "Enter a Bugzilla username (this is usually an email\n"
                "address): ")
    ...
    if options.prompt or len(prompted.__dict__) > 0:
        if login:
            msg ="\nSave project to %s [y]/n (password will be stored in plaintext)? "
        else:
            msg = "\nSave project to %s [y]/n? "
        should_save = raw_input(msg % options.ini_file)
        if should_save.lower() in ("", "y"):
            if cfg_section_used is None:
                cfg_section_used = raw_input("Project name: ").lower().strip()
            if not cfg.has_section(cfg_section_used):
                cfg.add_section(cfg_section_used)

            for key, val in prompted.__dict__.iteritems():
                cfg.set(cfg_section_used, key, val)
                f = file(os.path.expanduser(options.ini_file), 'w')
                cfg.write(f)
                f.close()
