...
class HTTPConnectionNoBaseURITests(unittest.TestCase):
    ...
    def test_request_get(self):
        ...
        self.failUnlessEqual(r['status'], 200)
...
class HTTPConnectionWithBaseURITests(unittest.TestCase):
    ...
    def test_request_get_with_params(self):
        r = self.conn.request('get', '/foo/bar?param1=value1&param2=value2')
        self.failUnlessEqual(r['status'], 200)
        self.failUnlessEqual(r['body'].split()[2], '/foo/bar?param1=value1&param2=value2')
    


class HTTPConnectionLogger(unittest.TestCase):
    def test_logger_not_callable(self):
        self.failUnlessRaises(AssertionError, HTTPConnection, logger=5)
    
    def test_log(self):
        def dummy_logger(msg):
            self.msg_received = msg
        
        conn = HTTPConnection(logger=dummy_logger)
        conn.log("test message")
        self.failUnlessEqual(self.msg_received, "test message")
    


class HTTPConnectionUserAgent(unittest.TestCase):
    def setUp(self):
        self.httpd_pid, self.httpd_uri = fork_http_server()
        self.conn = HTTPConnection(user_agent="Unittester/1.0")
    
    def tearDown(self):
        os.kill(self.httpd_pid, signal.SIGTERM)
    
    def test_request_get(self):
        r = self.conn.request('get', self.httpd_uri + "user_agent")
        self.failUnlessEqual(r['status'], 200)
        self.failUnlessEqual(r['body'], "Unittester/1.0")
    


class HTTPConnectionBasicAuth(unittest.TestCase):
    def setUp(self):
        self.httpd_pid, self.httpd_uri = fork_http_server()
        self.conn = HTTPConnection(username="testuser", password="testpass")
    
    def tearDown(self):
        os.kill(self.httpd_pid, signal.SIGTERM)
    
    def test_request_get(self):
        r = self.conn.request('get', self.httpd_uri + "test_auth")
        print r
        self.failUnlessEqual(r['status'], 200)
        self.failUnlessEqual(r['body'], "Basic testuser:testpass")
