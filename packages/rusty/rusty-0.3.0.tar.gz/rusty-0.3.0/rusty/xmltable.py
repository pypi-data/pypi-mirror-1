...
from rusty.util import Messenger
msgr = Messenger()
...
class XMLTableDirective(ListTable):
  ...
  def run(self):
    ...
    if not file_path:
      error = msgr.error(self, 'file_path -option missing')
      return [error]

    if not query:
      error = msgr.error(self, 'query -option missing')
      return [error]
    ...
    if not table:
      msgr.error(self, 'The table generated from queries is empty')
    ...
    try:
      ...
      if len(table_data) > 1 and len(table_data[0]) != len(table_data[1]):
        error = msgr.error(self, 'Data amount mismatch: check the directive data and params')
        return [error]
...
class BeautifulTable(object):
  ...
  def create_table(self, query, columns):
    ...
    msgr.debug('ITERATOR: ', query)
    for elem in self._query_path(query):
      row = []
      for column_query in columns:

        msgr.debug('#'*50)
        #msgr.debug('## ITERM ELEM: ', elem.encode(), type(elem))
  ...
  def _query_path(self, query, root=None):
    ...
    msgr.debug('RETURN ROOT: ', root, type(root))
    return listing
  ...
  def _query_value(self, query, root=None):
    ...
    for part in root_parts:
      ...
      msgr.debug('----------> new root: %s, %s' % (new_root, type(new_root)))
    ...
    if u'self' != query:
      msgr.debug('## QROOT: ', root, type(root))
      msgr.debug('## QUERY: ', query)
      ...
      if getattr(root, query, None):
        ...
        if new_root:
          msgr.debug('## NEW QROOT:', root, type(new_root))
          root = new_root
    ...
    msgr.debug('## FINAL ROOT: ', root, type(root))
