...
from util import Messenger, DirectiveTemplate

msgr = Messenger('rusty.exceltable')
...
class ExcelTableDirective(ListTable, DirectiveTemplate):
  ...
  csv-table (docutils) and xmltable (:mod:`rusty.xmltable`).
...
class ExcelTable(object):
  ...
  def create_table(self, fromcell=None, tocell=None, nheader=0, sheet=0):
    ...
    if type(fromcell) in [str, unicode]:
      match = re.match(r'(?P<chars>[A-Z]+)(?P<nums>[1-9]+)', fromcell)
      if match:
    ...
    if type(tocell) in [str, unicode]:
      match = re.match(r'(?P<chars>[A-Z]+)(?P<nums>[1-9]+)', tocell)
      if match:
        parts = (match.group('chars'), int(match.group('nums')))
...
def _unittests():
  ...
  etable = ExcelTable(open(path, 'r+b'))
