...
# www.cafepress.com site class for mechanoid
...
class CafePress:
	HOST = "https://www.cafepress.com/cp/members/login.aspx?passthru=yes"
	RESPONSE = os.path.expandvars("$HOME/cafepress_debug.html")
	RETRIES = 5
	...
	def __log(self, msg):
		...
		f = open(os.path.expandvars("$HOME/cafepress.error"),'a')
	...
	def log_in(self):
		n = netrc.netrc()
		try:
			login = n.authenticators("cafepress")
		except:
			self.usage()
		self.b.select_form(nr=0)
		self.b["txtEmail"] = login[0]
		self.b["txtPassword"] = login[2]
