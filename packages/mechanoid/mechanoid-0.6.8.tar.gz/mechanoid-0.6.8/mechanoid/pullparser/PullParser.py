...
class PullParser(HTMLParser):
	...
	def __init__(self, response, textify={"img": "alt", "applet": "alt"}, encoding="ascii"):
		"""
		response: file-like object (only a .read() method is required)
		from which to read HTML to be parsed
	...
	# a way to vary the method of iteration
	...
	def get_links(self):
		...
		for token in self.tags(*(self.URLTAGS.keys()+["base"])):
			if token.data == "base":
			...
			if tag == "a":
				if token.type != "startendtag":
					text = self.get_compressed_text(("endtag", tag))
			if not url:
				continue
