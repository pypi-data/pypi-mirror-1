...
# Copyright 2003-2004 John J. Lee (mechanize based on Perl's WWW::Mechanize)
# Copyright 2003 Andy Lester (original Perl code)
#

import os, re, urlparse
from clientform.ClientForm import ClientForm
from http_processors import HTTPRequestUpgradeProcessor
...
class Browser(UserAgent):
	...
	def __open(self, url, data=None, update_history=True):
		...
		try:
			...
			scheme, netloc, action = urlparse.urlparse(url)[:3]
...
##		self.request.add_header("accept-encoding","gzip") # XXX
	...
	def click_link(self, link=None, **kwds):
		...
		if link.absolute_url[0] == "/":
			link.absolute_url = link.absolute_url[1:]
		if ((link.absolute_url.find("cgi-bin") == -1) and
