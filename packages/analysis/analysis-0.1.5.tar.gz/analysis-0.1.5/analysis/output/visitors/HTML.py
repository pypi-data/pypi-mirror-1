...
class HTMLVisitor(Visitor):
    ...
    def _name_ref(self, name, qualified_name):
    ...
    def _doc(self, doc):
        if doc is not None:
            ...
            self.generator.indent()
            self.generator.writeln(self._text(repr(doc)))
            self.generator.dedent()
            self.generator.writeln("</pre>")
    ...
    def _usage_specialisation(self, node):
        if hasattr(node, "_specialisation"):
            self.generator.writeln(" usage-1'>")
        else:
            self.generator.writeln(" usage-0'>")
    ...
    def _targets(self, node):
        if len(node._targets) == 0:
            return
        self.generator.writeln("<div class='targets'>")
        self.generator.indent()
        for target in node._targets:
            self._target(target)
        self.generator.dedent()
        self.generator.writeln("</div>")
    ...
    def _target(self, target):
        ...
        self.generator.indent()
        self.generator.writeln(self._text(target._qualified_name))
        self.generator.dedent()
        self.generator.writeln("</a>")
    ...
    def visitModule(self, node):
        ...
        self.generator.writeln(
            ...
            "   position: absolute; top: 0em; left: -5em;"
        ...
        self.generator.writeln(
            ...
            " }")
        self.generator.writeln(".targets a { display: block; color: white; text-decoration: none; }")
        self.generator.writeln(
    ...
    def visitImport(self, node):
        ...
        for _name in node._names:
            ...
            self._name_ref(_name.module_name, _name._qualified_name)
    ...
    def visitClass(self, node):
        ...
        if node.bases:
            ...
            for base in node.bases:
                if not first:
                    self.generator.write(", ")
                self._name_ref(base.name, base._qualified_name)
                first = 0
            self.generator.write(")")
        self.generator.writeln(":")
        self._doc(node.doc)
