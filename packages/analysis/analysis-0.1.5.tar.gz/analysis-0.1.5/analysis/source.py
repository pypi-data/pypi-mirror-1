...
class AnalysisVisitor(ASTVisitor):
    ...
    def visitImport(self, import_, handler):
        ...
        for module_name, as_name in import_.names:
            # NOTE: Complete this!
            module = process_file("tests/%s.py" % module_name)
        ...
        for child_node in child_nodes:
            try:
                ...
                if self.debug:
                    ...
                    print >>sys.stderr, get_line(from_node),
                    print >>sys.stderr, "for"
                    print >>sys.stderr, get_line(to_node),
...
# Functions which use the above AnalysisVisitor class.
...
def process_file(filename, debug=0):
    ...
    visitor = AnalysisVisitor(builtins._namespace, get_module_name(filename), debug=debug)
    compiler.walk(module, visitor, visitor)

    current_filenames.pop()
