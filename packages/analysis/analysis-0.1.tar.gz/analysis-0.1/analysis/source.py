...
class AnalysisVisitor(ASTVisitor):
    ...
    def visitModule(self, module, handler=None):
        ...
        handler = NamespaceRegister(self.globals, self.globals, self.builtins, name_qualifier=self.module_name)
    ...
    def visitAssAttr(self, assattr, handler):
        ...
        if isinstance(assattr._parent, compiler.ast.AssTuple) or isinstance(assattr._parent, compiler.ast.AssList):
    ...
    def visitAssName(self, assname, handler):
        ...
        if isinstance(assname._parent, compiler.ast.AssTuple) or isinstance(assname._parent, compiler.ast.AssList):
    ...
    def visitAssTuple(self, asstuple, handler):
        ...
        if isinstance(asstuple._parent, compiler.ast.AssTuple) or isinstance(asstuple._parent, compiler.ast.AssList):
        ...
        self.next_specialisations(asstuple, "expr")
        for obj in lobj(asstuple, strict=1):

            # Access an iterator on the expression.
            # ie. iter = expr.__iter__()

            if not obj._namespace.has_key("__iter__"):
                if self.debug:
            ...
            for method in methods:
                self.call_specialisation(asstuple, method, [obj], handler)
    ...
    def visitCallFunc(self, callfunc, handler):
        ...
        self.next_specialisations(callfunc, "context")
        ...
        for obj, attributes in context_items:
            for attribute in attributes:
                ...
                if isinstance(attribute, compiler.ast.Function):
                    ...
                    self.call_specialisation(callfunc, attribute, args, handler,
                    ...
                    if attribute._namespace.has_key("__init__"):
                        ...
                        for method in methods:
                            ...
                            self.call_specialisation(callfunc, method, args,
                                handler, callfunc.star_args, callfunc.dstar_args)
    ...
    def visitList(self, list, handler):
        ...
        for definition in handler.builtins["list"]:
            ref = analysis.reference.instantiate_class(definition, list)
            ...
            for node in list.nodes:
                ref._namespace["fake"].append(node)
    ...
    def visitSlice(self, slice, handler):
        ...
        self.next_specialisations(slice, "expr")
        ...
        for expr_input in lobj(slice.expr, strict=1):
            ...
            if slice.lower is not None:
                ...
                method_args.append(handler.builtins["None"][0])
            if slice.upper is not None:
                method_args.append(slice.upper)
            else:
                method_args.append(handler.builtins["None"][0])
            for method in methods:
                self.call_specialisation(slice, method, method_args, handler)
    ...
    def visitSubscript(self, subscript, handler):
        ...
        self.next_specialisations(subscript, "expr")
        ...
        for expr_input in lobj(subscript.expr, strict=1):
            ...
            for method in methods:
                self.call_specialisation(subscript, method, method_args, handler)
    ...
    def attach_next_iteration(self, node, handler):
        ...
        self.next_specialisations(node, "iter")
        ...
        for iter_obj in lobj(node._parent, strict=1):
            ...
            if not iter_obj._namespace.has_key("next"):
                if self.debug:
            ...
            for next_method in next_methods:
                self.call_specialisation(node, next_method, [iter_obj], handler)
    ...
    def _process_binary_operator(self, node, left, right, left_method_name, right_method_name, handler):
        ...
        self.next_specialisations(node, "expr")
        ...
        for left_input in left_inputs:
            ...
            if left_input._namespace.has_key(left_method_name):
                ...
                for right_input in right_inputs:
                    method_args = [left, right]

                    # Make a specialisation for the operation method.

                    for method in methods:
                        try:
                            self.call_specialisation(node, method, method_args, handler)
    ...
    def next_specialisations(self, caller, refcontext):
    ...
    def call_specialisation(self, caller, method, args, handler, star_args=None, dstar_args=None):

        """
        On the given 'caller' node, invoke the given 'method' with the given
        'args', using the specified 'handler' to provide namespace information.
        ...
        analysis.node.link(caller, spec, blocking=1)
    ...
    def specialise_function(self, function, locals, handler, add_to_module=1):
        ...
        while 1:
            ...
            qualified_name = handler.get_qualified_name(specialisation.name)

            # Prepare a new namespace for the specialisation.

            new_handler = NamespaceRegister(locals, function._globals, handler.builtins, name_qualifier=qualified_name)
        ...
        if specialisation.doc is not None and specialisation.doc.strip().startswith("NAME:"):
            ...
            specialisation._qualified_name = handler.get_qualified_name(specialisation.name)
