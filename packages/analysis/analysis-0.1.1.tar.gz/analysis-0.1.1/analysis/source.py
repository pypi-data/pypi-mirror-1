...
class AnalysisVisitor(ASTVisitor):
    ...
    def visitAssTuple(self, asstuple, handler):
        ...
        self.next_specialisations(asstuple)
        for obj in lobj(asstuple, strict=1):
            ...
            for method in methods:
                self.call_specialisation(asstuple, method, [obj], refcontext, handler)
    ...
    def visitClass(self, class_, handler):
        ...
        # NOTE: Hack to permit recursive class name usage.

        handler.globals[class_.name] = [class_]
    ...
    def visitSlice(self, slice, handler):
        ...
        for expr_input in lobj(slice.expr, strict=1):
            ...
            for method in methods:
                self.call_specialisation(slice, method, method_args, None,
                    handler)
    ...
    def visitSubscript(self, subscript, handler):
        ...
        for expr_input in lobj(subscript.expr, strict=1):
            ...
            method_args = [subscript.expr] + subscript.subs
            for method in methods:
                self.call_specialisation(subscript, method, method_args, None,
                    handler)
