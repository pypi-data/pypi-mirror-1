...
class AnalysisVisitor(ASTVisitor):
    ...
    def visitAnd(self, and_, handler):
        ...
        for node in and_.nodes:
            ...
            self.attach_true(helper, lobj(node, strict=1), "expr", handler)
        ...
        for compare, block in blocks:
            if compare is not None:
                ...
                # Additional processing to produce a boolean result.
                    ...
                    self.attach_true(helper, lobj(compare, strict=1), "expr", handler)
    ...
    def visitNot(self, not_, handler):
        ...
        self.attach_true(not_, lobj(not_.expr, strict=1), "expr", handler)

        # Add the negation operation.

        self.next_specialisations(not_)
        method = handler.builtins["__not__"][0]
        for obj in lobj(not_, strict=1):
            method_args = [obj]
            self.call_specialisation(not_, method, method_args, "pop-next", handler)
    ...
    def visitWhile(self, while_, handler):
        ...
        while signature not in signatures:
            ...
            self.attach_true(helper, lobj(while_.test, strict=1), "expr", handler)
            while_._test = helper
    ...
    def attach_true(self, node, objects, refcontext, handler):

        """
        On the given 'node', attach invocations of the __true__ method for each
        of the 'objects', defining 'refcontext' for the invocations, and using
        the given 'handler'.
        """
