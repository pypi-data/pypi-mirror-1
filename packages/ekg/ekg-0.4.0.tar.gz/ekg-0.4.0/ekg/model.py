...
class SqlBase(object):
    ...
    @classmethod
    def create_or_update(cls, core, **remote):
...
class Stream(Base):
    ...
    def update(self, remote):
        ...
        with pwd(self.core.opts['cache_dir']):
            try:
                makedirs(self.cache)
            except OSError, e:
                print e
...
#                 try:
                self.source_class.create_or_update(self.core, **remote)
#                 except UpdateError, e:
#                     print 'failed with remote source %s for stream %s' % (remote['identifier'], self.name)
...
class RHMailman(Mailman):
    ...
    def remote_iter(self):
        try:
            conn = self.core.new_grabber(prefix='http://www.redhat.com')
            ...
            print 'error fetching attribute %s' % e.message
        except Exception, e:
            print 'fail doing something big, not sure what\'s going on'
            traceback.print_exc()
            raise UpdateError('unknown error', e)
    ...
    def update(self, remote):
        try:
            ...
            for email in iter_in_transaction(session, mbox):
                ...
                self.fact.create_or_update(self.core, **email)
        except KeyError, e:
            print 'error fetching attribute %s' % e.message
        except Exception, e:
            print 'fail doing something big, not sure what\'s going on'
