...
def getFSStoragePath(product='p01.fsfile', confKey='fsStorage',
...
class TMPFileConsumerBase(object):
    ...
    def _storeFSFileData(self, tmpFile, fsFile):
        ...
        try:
            ...
            shutil.move(tmpFile.tmpPath, fsFile.fsID)
...
class FSPathStorageBase(persistent.Persistent, contained.Contained,
    FSStorageBase):
    ...
    def __init__(self, path):
        super(FSPathStorageBase, self).__init__()
    ...
    def _getStorageDir(self, fsNameSpace=None):
        """Get the storage directory."""
        raise NotImplementedError('Sub class must implement _getStorageDir')
    ...
    def _getFSID(self, fsFile, fileName=None, fsNameSpace=None):
        ...
        dirPath = self._getStorageDir(fsNameSpace)
    ...
    def getOpenFile(self, fsFile):
        ...
        if not os.path.exists(fsFile.fsID):
            raise NotFound(self, fsFile.fsID)
        return open(fsFile.fsID, 'rb')

    def getSize(self, fsFile):
        """Set meta data based on data dict. Normal only contentType get set.
        """
        # our fsID in this storage is the real file system file path
        return os.path.getsize(fsFile.fsID)
    ...
    def abortStoreTransaction(self, fsFile):
        """Knows how to abort a store transaction."""
        if fsFile.fsID is not None and os.path.isfile(fsFile.fsID):
            try:
                os.remove(fsFile.fsID)
            except OSError, e:
    ...
    def removeGhostFiles(self):
        ...
        for key, gFile in self._ghostFiles.items():
            if os.path.exists(gFile.fsID):
                try:
                    os.remove(gFile.fsID)
                    append(key)
                except OSError, e:
...
class FlatPathFSStorageBase(FSPathStorageBase):
    ...
    def getFilenameForOID(self, oid):
        ...
        path = self._getStorageDir()
        oidPath = os.path.join(path, fname)
        return oidPath
    ...
    def _getFSID(self, fsFile, fileName=None, fsNameSpace=None):
        ...
        fileName = '%s.%3d' % (now.strftime("%Y%m%d-%H-%M-%S"),
            now.microsecond/1000)
        oidPath = self.getFilenameForOID(oid)
        return "%s-%s" % (oidPath, fileName)
...
class UserPathFSStorageBase(FSPathStorageBase):
    ...
    def __init__(self, path):
        super(UserPathFSStorageBase, self).___init__(path)
        # setup share dir
...
class UserPathFSStorage(UserPathFSStorageBase, TMPFileConsumerBase):
    """User based file system storage."""

    zope.interface.implements(interfaces.IUserPathFSStorage)


# TODO: implement better filename, e.g. use a version
class BushyFSStorageBase(FSPathStorageBase):
    ...
    def getOIDForPath(self, oidPath):
