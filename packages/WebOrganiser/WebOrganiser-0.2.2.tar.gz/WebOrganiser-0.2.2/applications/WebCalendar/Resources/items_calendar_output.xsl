...
<axsl:apply-templates select="items" mode="id2283577"/>
</html>
  </axsl:template>
  <axsl:template match="items" mode="id2283577">
  ...
  <form action="" method="POST">
    <div class="controls">
      <div class="buttons">
        ...
        <axsl:apply-templates select="new-type" mode="id2283633"/>
      </div>
      <axsl:apply-templates select="item-types" mode="id2283676"/>
    </div>
    <axsl:apply-templates select="month" mode="id2283721"/>
  </form>
</body>
  </axsl:template>
  <axsl:template match="new-type" mode="id2283633">
    <axsl:choose>
      <axsl:when test="@value">
        ...
        <select xmlns="http://www.w3.org/1999/xhtml" name="{template:this-attribute()}">
          <axsl:apply-templates select="new-type-enum" mode="id2283655"/>
        </select>
      </axsl:when>
      <axsl:otherwise>
        <axsl:variable name="this-name">value</axsl:variable>
  ...
  <axsl:template match="new-type-enum" mode="id2283655">
  ...
  <axsl:template match="item-types" mode="id2283676">
    <div xmlns="http://www.w3.org/1999/xhtml" class="types" xml:space="preserve">
        <axsl:apply-templates select="item-type" mode="id2283685"/>
  ...
  <axsl:template match="item-type" mode="id2283685">
  ...
  <axsl:template match="month" mode="id2283721">
    <div xmlns="http://www.w3.org/1999/xhtml" class="view">
      <table align="center" border="0" cellpadding="5" cellspacing="0" width="100%">
        ...
        <axsl:apply-templates select="week" mode="id2283887"/>
      </table>
    </div>
  </axsl:template>
  <axsl:template match="week" mode="id2283887">
    <tbody xmlns="http://www.w3.org/1999/xhtml">
          <tr>
            <axsl:apply-templates select="day" mode="id2283895"/>
          </tr>
          <tr>
            <axsl:apply-templates select="day" mode="id2283919"/>
          </tr>
        </tbody>
  </axsl:template>
  <axsl:template match="day" mode="id2283895">
  ...
  <axsl:template match="day" mode="id2283919">
    <td xmlns="http://www.w3.org/1999/xhtml" class="{template:choice(@date, 'day-minimum', 'day-unused')} {template:choice(@marked = 'true', 'day-region')}">
              ...
              <axsl:apply-templates select="item" mode="id2283928"/>
            </td>
  </axsl:template>
  <axsl:template match="item" mode="id2283928">
    <div xmlns="http://www.w3.org/1999/xhtml" xml:space="preserve">
                ...
                <a href="{template:choice(/items/@filtered, '../../')}../{@item-type}/{template:url-encode(template:url-encode(@item-value))}/">
                  <axsl:apply-templates select="summary" mode="id2283669"/>
                </a>
                ...
                <axsl:apply-templates select="dtstart" mode="id2284014"/>

              </div>
  </axsl:template>
  <axsl:template match="summary" mode="id2283669">
    <axsl:choose>
  ...
  <axsl:template match="dtstart" mode="id2284014">
    <axsl:choose>
