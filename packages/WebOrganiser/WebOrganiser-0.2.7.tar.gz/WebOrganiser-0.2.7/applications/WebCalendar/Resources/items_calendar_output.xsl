...
<axsl:apply-templates select="items" mode="id2288546"/>
</html>
  </axsl:template>
  <axsl:template match="items" mode="id2288546">
  ...
  <form action="" method="POST">
    <div class="controls">
      ...
      <div class="buttons">
        ...
        <axsl:apply-templates select="new-type" mode="id2288625"/>
      </div>

      <!-- Item type "tabs". -->

      <axsl:apply-templates select="item-types" mode="id2288686"/>
    </div>

    <!-- Item sorting controls. -->

    <axsl:apply-templates select="month" mode="id2288750"/>
  </form>
</body>
  </axsl:template>
  <axsl:template match="new-type" mode="id2288625">
    <axsl:choose>
      <axsl:when test="@value">
        ...
        <select xmlns="http://www.w3.org/1999/xhtml" name="{template:this-attribute()}">
          <axsl:apply-templates select="new-type-enum" mode="id2288659"/>
        </select>
      </axsl:when>
      <axsl:otherwise>
        <axsl:variable name="this-name">value</axsl:variable>
  ...
  <axsl:template match="new-type-enum" mode="id2288659">
  ...
  <axsl:template match="item-types" mode="id2288686">
    <div xmlns="http://www.w3.org/1999/xhtml" class="types" xml:space="preserve">
        ...
        <axsl:apply-templates select="item-type" mode="id2288702"/>
  ...
  <axsl:template match="item-type" mode="id2288702">
  ...
  <axsl:template match="month" mode="id2288750">
    <div xmlns="http://www.w3.org/1999/xhtml" class="view">
      <table align="center" border="0" cellpadding="5" cellspacing="0" width="100%">
        ...
        <axsl:apply-templates select="week" mode="id2288961"/>
      </table>
    </div>
  </axsl:template>
  <axsl:template match="week" mode="id2288961">
    <tbody xmlns="http://www.w3.org/1999/xhtml">
          <tr>
            <axsl:apply-templates select="day" mode="id2288972"/>
          </tr>
          <tr>
            <axsl:apply-templates select="day" mode="id2289002"/>
          </tr>
        </tbody>
  </axsl:template>
  <axsl:template match="day" mode="id2288972">
  ...
  <axsl:template match="day" mode="id2289002">
    <td xmlns="http://www.w3.org/1999/xhtml" class="{template:choice(@date, 'day-minimum', 'day-unused')} {template:choice(@marked = 'true', 'day-region')}">
              ...
              <axsl:apply-templates select="item" mode="id2289041"/>
            </td>
  </axsl:template>
  <axsl:template match="item" mode="id2289041">
    <div xmlns="http://www.w3.org/1999/xhtml" xml:space="preserve">
                ...
                <axsl:if test="$metadata = 'true' or                                    substring(dtstart/@datetime, 1, 8) = ../@date or                                    dtstart/@datetime and not(substring(dtstart/@datetime, 1, 8) = ../@date) and                                    not(substring(created/@datetime, 1, 8) = ../@date) and                                    not(substring(last-modified/@datetime, 1, 8) = ../@date)"><span>
                  <a href="{template:choice(/items/@filtered, '../../')}../{@item-type}/{template:url-encode(template:url-encode(@item-value))}/">
                    <axsl:apply-templates select="summary" mode="id2289160"/>
                  </a>
                  ...
                  <axsl:apply-templates select="dtstart" mode="id2289197"/>
                </span></axsl:if>

              </div>
  </axsl:template>
  <axsl:template match="summary" mode="id2289160">
    <axsl:choose>
  ...
  <axsl:template match="dtstart" mode="id2289197">
    <axsl:choose>
