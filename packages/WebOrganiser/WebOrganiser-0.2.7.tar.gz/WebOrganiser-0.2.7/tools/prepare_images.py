...
CRYSTAL_CLEAR = "crystalclear"
TANGO = "Tango" # originally "tango-icon-theme-0.7.1"

icon_set_preferences = [CRYSTAL_CLEAR, CRYSTAL_SVG, TANGO]
...
icon_sets = {
    ...
    CRYSTAL_CLEAR : [
        ...
        ("actions", "edit", "updated"),
        ("apps", "xclock", "appointment"),
        ("actions", "redo", "continued"),
        ("filesystems", "folder", None),
        ("actions", "exec", "details")
        ],
    TANGO : [
...
if __name__ == "__main__":
    ...
    if args.has_key("icon-set"):
        icon_sets_to_try = [args["icon-set"]]
    else:
        icon_sets_to_try = icon_set_preferences

    icon_base = args.get("icon-base", "/usr/share/icons")
    icon_size = args.get("icon-size", "32x32")

    found = 0
    for icon_set in icon_sets_to_try:
    ...
    try:
        ...
        for section, icon, target in icon_sets[icon_set]:
            ...
            if args.has_key("convert"):
                ...
                # Recover by trying .svgz files.
                if not os.system(cmd):
                    cmd = "ksvgtopng %s %s %sz %s" % (width, height, image, target_image)
                    os.system(cmd)
            else:
