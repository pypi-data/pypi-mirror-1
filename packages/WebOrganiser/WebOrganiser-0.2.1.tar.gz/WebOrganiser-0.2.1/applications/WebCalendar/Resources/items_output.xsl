...
<axsl:apply-templates select="items" mode="id2280217"/>
</html>
  </axsl:template>
  <axsl:template match="items" mode="id2280217">
  ...
  <form action="" method="POST">
    ...
    <axsl:apply-templates select="table" mode="id2280538"/>
  </form>
</body>
  </axsl:template>
    ...
    <axsl:choose>
      <axsl:when test="@value">
        ...
        <select xmlns="http://www.w3.org/1999/xhtml" name="{template:this-attribute()}">
          <axsl:apply-templates select="new-type-enum" mode="id2280304"/>
        </select>
      </axsl:when>
      <axsl:otherwise>
        <axsl:variable name="this-name">value</axsl:variable>
  ...
  <axsl:template match="new-type-enum" mode="id2280304">
  ...
  <axsl:template match="table" mode="id2280538">
    <div xmlns="http://www.w3.org/1999/xhtml" class="view">
      <table align="center" border="0" cellpadding="5" cellspacing="0" width="100%">
        ...
        <axsl:apply-templates select="item" mode="id2280589"/>
      </table>
    </div>
  </axsl:template>
  <axsl:template match="item" mode="id2280589">
          ...
          <axsl:if test="not(related-to) or /items/@related-to"><tr>
            ...
            <td xml:space="preserve">
              ...
              <a href="../{@item-type}/{template:url-encode(template:url-encode(@item-value))}/">
                <axsl:apply-templates select="summary" mode="id2280677"/></a>
            ...
            <td>
              <axsl:apply-templates select="created" mode="id2280728"/>
            </td>
          </tr></axsl:if>

        </tbody>
  </axsl:template>
  <axsl:template match="summary" mode="id2280677">
    <axsl:choose>
  ...
  <axsl:template match="created" mode="id2280728">
    <axsl:choose>
