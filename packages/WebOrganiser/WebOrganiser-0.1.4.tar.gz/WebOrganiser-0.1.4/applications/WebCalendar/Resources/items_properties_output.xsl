...
<axsl:stylesheet xmlns:dyn="http://exslt.org/dynamic" xmlns:axsl="http://www.w3.org/1999/XSL/Transform" xmlns:template="http://www.boddie.org.uk/ns/xmltools/template" version="1.0" extension-element-prefixes="dyn">
  ...
  <axsl:template match="/">
    <axsl:apply-templates select="items" mode="id2288852"/>
  </axsl:template>
  <axsl:template match="items" mode="id2288852">
    <axsl:apply-templates select="table" mode="id2288852"/>
  </axsl:template>
  <axsl:template match="table" mode="id2288852">
  ...
  <axsl:apply-templates select="item" mode="id2288923"/>
</D:multistatus>
  </axsl:template>
  <axsl:template match="item" mode="id2288923">
    <D:response xmlns:D="DAV:">
    <D:href><axsl:value-of select="concat($path, template:url-encode(template:url-encode(@item-value)))"/></D:href>
    <D:propstat>
      <D:prop>
