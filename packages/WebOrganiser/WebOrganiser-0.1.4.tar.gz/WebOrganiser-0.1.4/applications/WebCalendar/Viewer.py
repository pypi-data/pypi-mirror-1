...
from WebCalendar.Common import default_item_types
import os
...
class CalendarViewerResource(XSLForms.Resources.WebResources.XSLFormsResource):
    ...
    def respond_to_form(self, trans, form):
        ...
        found_items = self.store.get_items_using_attributes(attributes, default_item_types)
        ...
        elif parameters.has_key("new-journal"):
            new_path = trans.update_path(pvpath, "../journal/new/")
            trans.redirect(trans.encode_path(new_path, self.path_encoding))
            # Response ended!
...
class RootViewerResource(SimpleViewerResource):
    ...
    def get_directories(self, trans, pvpath):
        d = ["all", "event", "journal", "message", "to-do"]
        if pvpath == "/":
            d.append("calendar")
            d.append("card")
