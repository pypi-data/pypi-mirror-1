...
class TopSelectorResource:
    ...
    def respond(self, trans):
        ...
        if trans.get_request_method() == "GET":
            ...
            vpath_parts = vpath.split("/")
            pvpath = trans.get_processed_virtual_path_info(self.path_encoding)

            # If there is nothing more on the path, redirect to the target.

            if len(vpath_parts) < 3:
                trans.redirect(
                    trans.encode_path(
            ...
            # Otherwise, eliminate the "empty" path component.

            else:
                trans.redirect(
                    trans.encode_path(
                        pvpath + "/".join(vpath_parts[1:]),
                        self.path_encoding
                        )
                    )
...
class DateSelectorResource:
    ...
    def respond(self, trans):
        ...
        try:
            ...
            if len(vpath_fields) == 2 or vpath_fields[2] == "":
...
class ItemSelectorResource:
    ...
    def respond(self, trans):
        ...
        # Get the item identifier, fully decoding the component.
        # Override filtering if more than one item value has been encountered.
        ...
        if len(vpath_fields) == 1 or len(vpath_fields) == 2 and (
            ...
            if trans.get_request_method() == "GET":
                ...
                elif attributes.has_key("filter-type"):
...
class FilterSelectorResource:
    ...
    def __init__(self, filter_type, viewer, default_item_type):
