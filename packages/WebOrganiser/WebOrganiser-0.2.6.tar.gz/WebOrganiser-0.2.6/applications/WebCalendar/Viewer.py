...
class CalendarViewerResource(XSLForms.Resources.WebResources.XSLFormsResource):
    ...
    def respond_to_form(self, trans, form):
        ...
        ppath = trans.get_path_without_info() + pvpath
        ...
        if parameters.has_key("download-all"):
            ...
            trans.redirect(trans.encode_path(new_path))
            # Response ended!
        ...
        stylesheet_parameters["metadata"] = parameters.get("metadata", ["false"])[0]
...
class ListViewerResource(XSLForms.Resources.WebResources.XSLFormsResource):
    ...
    def respond_to_form(self, trans, form):
        ...
        if parameters.has_key("download-all"):
            ...
            trans.set_content_type(ContentType("text/calendar"))
            stream = trans.get_response_stream()
            export_items(selected_items, self.store, stream, 1)
            raise EndOfResponse
...
class PropertyViewerResource(XSLForms.Resources.WebResources.XSLFormsResource):
    ...
    def respond_to_form(self, trans, form):
        ...
        pvpath = trans.get_processed_virtual_path_info()
        ppath = trans.get_path_without_info() + pvpath
        encoded_pvpath = "/".join([trans.encode_path(part) for part in pvpath.split("/")])
        encoded_ppath = trans.get_path_without_info() + encoded_pvpath
