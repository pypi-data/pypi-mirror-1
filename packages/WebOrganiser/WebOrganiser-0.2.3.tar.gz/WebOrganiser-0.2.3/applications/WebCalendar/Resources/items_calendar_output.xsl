...
<axsl:apply-templates select="items" mode="id2283574"/>
</html>
  </axsl:template>
  <axsl:template match="items" mode="id2283574">
  ...
  <form action="" method="POST">
    <div class="controls">
      <div class="buttons">
        ...
        <axsl:apply-templates select="new-type" mode="id2283630"/>
      </div>
      <axsl:apply-templates select="item-types" mode="id2283673"/>
    </div>
    <axsl:apply-templates select="month" mode="id2283717"/>
  </form>
</body>
  </axsl:template>
  <axsl:template match="new-type" mode="id2283630">
    <axsl:choose>
      <axsl:when test="@value">
        ...
        <select xmlns="http://www.w3.org/1999/xhtml" name="{template:this-attribute()}">
          <axsl:apply-templates select="new-type-enum" mode="id2283652"/>
        </select>
      </axsl:when>
      <axsl:otherwise>
        <axsl:variable name="this-name">value</axsl:variable>
  ...
  <axsl:template match="new-type-enum" mode="id2283652">
  ...
  <axsl:template match="item-types" mode="id2283673">
    <div xmlns="http://www.w3.org/1999/xhtml" class="types" xml:space="preserve">
        <axsl:apply-templates select="item-type" mode="id2283681"/>
  ...
  <axsl:template match="item-type" mode="id2283681">
  ...
  <axsl:template match="month" mode="id2283717">
    <div xmlns="http://www.w3.org/1999/xhtml" class="view">
      <table align="center" border="0" cellpadding="5" cellspacing="0" width="100%">
        ...
        <axsl:apply-templates select="week" mode="id2283884"/>
      </table>
    </div>
  </axsl:template>
  <axsl:template match="week" mode="id2283884">
    <tbody xmlns="http://www.w3.org/1999/xhtml">
          <tr>
            <axsl:apply-templates select="day" mode="id2283892"/>
          </tr>
          <tr>
            <axsl:apply-templates select="day" mode="id2283915"/>
          </tr>
        </tbody>
  </axsl:template>
  <axsl:template match="day" mode="id2283892">
  ...
  <axsl:template match="day" mode="id2283915">
    <td xmlns="http://www.w3.org/1999/xhtml" class="{template:choice(@date, 'day-minimum', 'day-unused')} {template:choice(@marked = 'true', 'day-region')}">
              ...
              <axsl:apply-templates select="item" mode="id2283924"/>
            </td>
  </axsl:template>
  <axsl:template match="item" mode="id2283924">
    <div xmlns="http://www.w3.org/1999/xhtml" xml:space="preserve">
                ...
                <axsl:if test="dtstart/@datetime and not(substring(dtstart/@datetime, 1, 8) = ../@date) and                                    not(substring(created/@datetime, 1, 8) = ../@date)"><span>
                ...
                <a href="{template:choice(/items/@filtered, '../../')}../{@item-type}/{template:url-encode(template:url-encode(@item-value))}/">
                  <axsl:apply-templates select="summary" mode="id2283665"/>
                </a>
                <a href="{template:choice(/items/@filtered, '../../')}../related-to/{template:url-encode(template:url-encode(@item-value))}/"><axsl:variable name="translation" select="$translations/translations/locale[code/@value=$locale]/translation[@value='(Related)']/text()"/><axsl:choose><axsl:when test="$translation"><axsl:value-of select="$translation"/></axsl:when><axsl:otherwise>(Related)</axsl:otherwise></axsl:choose></a>
                <axsl:apply-templates select="dtstart" mode="id2284011"/>

              </div>
  </axsl:template>
  <axsl:template match="summary" mode="id2283665">
    <axsl:choose>
  ...
  <axsl:template match="dtstart" mode="id2284011">
    <axsl:choose>
