...
def test_clone_command_simple_ssh_auth():
    ...
    assert not result.writes_remote
    assert str(result) == "clone --ssh ssh -i /tmp/id.rsa -o StrictHostKeyChecking=no ssh://hg.mozilla.org/bar bar"
...
def test_clone_command_ssh_username_auth():
    ...
    generic_clone = commands.clone(context, ["ssh://hg.mozilla.org/bar"])
    result = dialect.convert(generic_clone)
    assert str(result) == "clone --ssh ssh -i /tmp/id.rsa -o StrictHostKeyChecking=no ssh://someone_else@hg.mozilla.org/bar bar"
...
def test_push_command_ssh_username_auth():
    ...
    assert result.reads_remote
    assert str(result) == "push --ssh ssh -i /tmp/id.rsa -o StrictHostKeyChecking=no ssh://someone_else@hg.mozilla.org/bar"
