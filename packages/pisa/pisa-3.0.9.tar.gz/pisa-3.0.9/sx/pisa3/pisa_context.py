...
class pisaCSSBuilder(css.CSSBuilder):
    ...
    def _pisaDimensions(self, data):
        ...
        height = -getSize(data.get("bottom", 0))
        width = -getSize(data.get("right", 0))
        if (not top) and height:
            top = height
            height = 0            
        if (not left) and width:
            left = right
            width = 0            
        top += getSize(data.get("margin-top", 0))
        left += getSize(data.get("margin-left", 0))
        height -= getSize(data.get("margin-bottom", 0))
        width -= getSize(data.get("margin-right", 0))
        width = getSize(data.get("width", 0)) or width
        height = getSize(data.get("height", 0)) or height
        # box = getCoords(left, top, width, height, self.c.pageSize)
        # print "BOX", box
        return left, top, width, height
    
    def _pisaAddFrame(self, name, data, first=False):
        c = self.c            
        if not name:
            name = "-pdf-frame-%d" % c.UID()                
        x, y, w, h = self._pisaDimensions(data)
        if first:
            c.frameList.insert(0, (
                name, 
                None, 
                data.get("-pdf-frame-border", None),
                x, y, w, h))
        else:
            c.frameList.append((
                name, 
            ...
            if declarations:             
                ...
                for prop in [
                    ...
                    if data.has_key(prop):
                        self._pisaAddFrame(name, data, first=True)
                        break
    ...
    def atFrame(self, name, declarations):
        result = self.ruleset([self.selector('*')], declarations)
        # print "@BOX", name, declarations
        try:
            data = result[0].values()[0]
            self._pisaAddFrame(name, data)
        except Exception, e:
            self.c.warning(ErrorMsg())            
        return {}, {}
...
class pisaContext:
    ...
    def __init__(self, path, debug=0):
        ...
        self.pathDocument = path        
        self.pathDirectory = getDirName(path)
    ...
    def addPara(self):
        ...
        if self.text.strip() and self.fragList:
    ...
    def loadFont(self, names, src, encoding="WinAnsiEncoding"):
        ...
        # print fontAlias, fontName, baseName
        ...
        try:
            ...
            if suffix == "ttf":
                ...
                if fontName in self.fontList:
                    ...
                    addMapping(fontName, 1, 1, fontName) #italic and bold
                ...
                if fontName in self.fontList:
                    ...
                    justFont = pdfmetrics.Font(fontName, fontName, encoding)
