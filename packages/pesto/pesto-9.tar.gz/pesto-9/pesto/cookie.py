# Copyright (c) 2007-2009 Oliver Cope. All rights reserved.
# See LICENCE.TXT for terms of redistribution and use.

import cgi
import copy
import urllib
import datetime
import time
from email.utils import formatdate

class Cookie(object):
    """
    Represents an HTTP cookie.

    See rfc2109, HTTP State Management Mechanism

    >>> c = Cookie('session_id', 'abc123')
    >>> c.path = '/cgi-bin'
    >>> c.domain = '.ucl.ac.uk'
    >>> c.path
    '/cgi-bin'
    >>> print str(c)
    session_id=abc123;Path=/cgi-bin;Domain=.ucl.ac.uk;Version=1
    """

    def __init__(self, name, value, maxage=None, expires=None, path=None, secure=None, domain=None, comment=None, version=1):
        self.name = name
        self.value = value
        self.maxage = maxage
        self.path = path
        self.secure = secure
        self.domain = domain
        self.comment = comment
        self.version = version
        self.expires = expires

        self.atts = (
            ("Max-Age", "maxage"),
            ("Path",    "path"),
            ("Secure",  "secure"),
            ("Domain",  "domain"),
            ("Comment", "comment"),
            ("Version", "version"),
            ("Expires", "expires"),
        )

    def __str__(self):
        """
        Returns a string representation of the cookie in the format
        ``session_id=abc123;Path=/cgi-bin;Domain=.ucl.ac.uk;Version=1``
        """
        cookie = [ '%s=%s' % (self.name, urllib.quote(str(self.value))) ]
        for cookie_name, att_name in self.atts:
            value = getattr(self, att_name, None)
            if value is not None:
                cookie.append('%s=%s' % (cookie_name, str(value)))
        return ';'.join(cookie)

    def set_expires(self, dt):
        self._expires = dt

    def get_expires(self):
        if self._expires is None and self.maxage is not None:
            if self.maxage == 0:
                # Make sure immediately expiring cookies get a date firmly in
                # the past.
                self._expires = datetime.datetime(1980, 1, 1)
            else:
                self._expires = datetime.datetime.now() + datetime.timedelta(seconds = self.maxage)

        if isinstance(self._expires, datetime.datetime):
            return formatdate(time.mktime(self._expires.utctimetuple()))

        else:
            return self._expires

    expires = property(get_expires, set_expires)

def expire_cookie(cookie_or_name, *args, **kwargs):
    """
    Synopsis::

        >>> from pesto.wsgiutils import MockWSGI
        >>> from pesto.response import Response
        >>> mock = MockWSGI(
        ...     HTTP_COOKIE='''$Version="1";
        ...     Customer="WILE_E_COYOTE";
        ...     Part="Rocket_0001";
        ...     Part="catapult_0032"
        ... ''')
        >>> def handler(request):
        ...     return Response(set_cookie = expire_cookie('Customer', path='/'))
        >>> mock.run_pesto(handler).get_header('Set-Cookie')
        'Customer=;Max-Age=0;Path=/;Version=1;Expires=Mon, 31 Dec 1979 23:00:00 -0000'
    """
    if isinstance(cookie_or_name, Cookie):
        expire = cookie_or_name
    else:
        expire = Cookie(name=cookie_or_name, value='', *args, **kwargs)
    return Cookie(
        name=expire.name,
        value='',
        expires=datetime.datetime(1980, 1, 1),
        maxage=0,
        domain=kwargs.get('domain', expire.domain),
        path=kwargs.get('path', expire.path)
    )


def parse_cookie_header(cookie_string, unquote=urllib.unquote):
    """
    Return a list of Cookie objects read from the request headers.

    cookie_string
        The cookie, eg ``CUSTOMER=FRED; path=/;``

    unquote
        A function to decode quoted values. If set to ``None``, values will be
        left as-is.

    See rfc2109, section 4.4

    The Cookie header should be a ';' separated list of name value pairs.
    If a name is prefixed by a '$', then that name is an attribute
    of the most recently (left to right) encountered cookie.  If no
    cookie has yet been parsed then the value applies to the cookie
    mechanism as a whole.
    """

    if unquote is None:
        unquote = lambda v: v

    if not cookie_string:
        return []
    cookies = []

    # Here we put the $ prefixed attributes that appear *before* a
    # named cookie, to use as a template for other cookies
    cookie_template = Cookie(None, None)

    for part in [ part.strip() for part in cookie_string.split(";")]:

        if not '=' in part:
            continue

        (k, v) = part.split("=", 1)

        # Unquote quoted values ('"..."' => '...')
        if v and '"' == v[0] == v[-1] and len(v) > 1:
            v = v[1:-1]

        if k[0] == '$':
            # Value pertains to most recently read cookie,
            # or cookie_template
            k = k[1:]
            if len(cookies) == 0:
                cookie = copy.copy(cookie_template)
            else:
                cookie = cookies[-1]

            if k in cookie.atts:
                setattr(cookie, cookie.atts[k], v)
        else:
            cookies.append(copy.copy(cookie_template))
            cookies[-1].name = unquote(k)
            cookies[-1].value = unquote(v)
    return cookies



