...
def get_session_id_from_querystring(environ):
    ...
    """
    pattern = re.escape(Session.COOKIE_NAME) + '=([0-9a-z]{%d})' % ID_LENGTH
    try:
...
def get_session_id_from_cookie(environ):
    ...
    """
    cookie = Request(environ).cookies.get(Session.COOKIE_NAME)
...
class SessionManagerBase(object):
    ...
    def __init__(self, persist='cookie', secret=None):
        ...
        if persist == 'cookie':
            self.get_session_id = get_session_id_from_cookie
        if persist == 'querystring':
            self.get_session_id = get_session_id_from_querystring
    ...
    def wsgi_middleware(self, app, auto_purge_every=0, auto_purge_olderthan=1800):
        ...
        def sessionmanager_middleware(environ, start_response):
            ...
            elif self.persist == 'querystring':
                request = Request(environ)
                if session.is_new and environ['REQUEST_METHOD'] == 'GET':
                    query_session_id = request.query.get(Session.COOKIE_NAME)
                    if query_session_id != session.session_id:
                        new_query = request.query.copy()
                        new_query[Session.COOKIE_NAME] = session.session_id
...
def session_middleware(
    ...
    def middleware(fn):
        ...
        return session_manager.wsgi_middleware(
            fn,
            auto_purge_every=auto_purge_every,
            auto_purge_olderthan=auto_purge_olderthan
        )
