...
def entity_tag_matches(tag, taglist):
    """
    Return True if ``tag`` matches an entry in ``taglist``
    """
    for item in taglist:
        pass
...
def with_etag(etag_func, weak=False):
    ...
    Synopsis::
        ...
        ...
        >>> print MockWSGI().run(to_wsgi(view)).raw_response
        200 OK\r
...
def etag_middleware(app):
    ...
    def etag_middleware(environ, start_response):
