...
def prepare(name, main_path, sys_path, curdir, new_console):
    ...
    if new_console:
        try:
            from processing import _processing, ProcessExit
            import signal
        except ImportError:
            pass
        else:
            if new_console and hasattr(_processing, 'InheritCtrlC'):
...
def run(preparation_data, self_data):
    ...
    if type(self) is str:
        # `self` is the address from which we can get the data
        from processing.connection import Client
        conn = Client(self)
        self = conn.recvbytes()
        self = cPickle.loads(self)
        conn.close()
