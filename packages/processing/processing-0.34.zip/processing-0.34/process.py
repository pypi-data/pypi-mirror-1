...
def debug(s):
    if VERBOSE:
        print >>sys.stderr, 'DEBUG [%s] %s' % (currentProcess().getName(),s)
...
class Process(object):
    ...
    def getExitCode(self):
        ...
        if self._popen is None:
            return self._popen
        if self._popen.poll() == 0x10000 - signal.SIGTERM:
    ...
    class Popen(subprocess.Popen):
        ...
        if '_processing' in globals():
            ...
            def terminate(self):
                if self.returncode is None:
                    try:
                        _processing.TerminateProcess(self._handle,
                                                     0x10000-signal.SIGTERM)
                    except WindowsError, e:
                        if self.returncode is not None:
                            raise
...
def _exit_func():
    try:
        debug('running all "atexit" finalizers')
        Finalize._run_all_finalizers()
        
    finally:
        for p in activeChildren():
            if p._daemonic:
                debug('calling `stop()` for daemon %s' % p.getName())
                p._popen.stop()
        ...
        for p in activeChildren():
            if p._daemonic:
                debug('calling `join(timeout)` for daemon %s' % p.getName())
                p.join(deadline - time.time())

        for p in activeChildren():
            if p._daemonic:
                debug('calling `terminate()` for daemon %s' % p.getName())
                p._popen.terminate()

        for p in activeChildren():
            if not p._daemonic:
                debug('calling `join()` for normal process %s' % p.getName())
                p.join()
