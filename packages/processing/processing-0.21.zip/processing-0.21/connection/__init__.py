...
def deliver_challenge(connection, authkey):
    ...
    connection.send_string('#CHALLENGE:' + message)
    digest = hmac.new(authkey, message, sha).digest()
    #print "deliver: %r" % [authkey, message, digest]
    response = connection.recv_string()
    if response == digest:
        connection.send_string('#WELCOME')
    else:
        connection.send_string('#AUTHENTICATION_FAILED')
        raise AuthenticationError('Digest received was wrong')
    
def answer_challenge(connection, authkey):
    message = connection.recv_string()
    assert message[:11] == '#CHALLENGE:', 'message = %r' % message
    message = message[11:]
    digest = hmac.new(authkey, message, sha).digest()
    #print "answer: %r" % [authkey, message, digest]
    connection.send_string(digest)
    response = connection.recv_string()
    if response != '#WELCOME':
...
def _test(address):
    ...
    c = Client(address, authkey=authkey)
...
if __name__ == '__main__':
    ...
    for f in families:
        ...
        l = Listener(family=f, authkey=key)
