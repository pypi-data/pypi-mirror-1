...
class BaseProxy(object):
    ...
    def __init__(self, token):
        self._token = token
        self._key = (os.getpid(), threading._get_ident(), token.address)
        self._id = token.id
        self._reconnect()
    ...
    def _apply(self, id, funcname, *args, **kwds):
        # this only gets called once before being overwritten
