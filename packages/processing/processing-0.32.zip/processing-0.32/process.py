...
def freezeSupport():
    ...
    '''
    global _done_freeze_support
    
    if (_IS_WINDOWS_EXE and not _done_freeze_support and len(sys.argv) > 1
            and sys.argv[1] == '--processing-fork'):
        assert len(sys.argv) == 4
        _done_freeze_support = True
        _nonforking.run(sys.argv[2], sys.argv[3])
        sys.exit(0)

#
# Support for frozen executables
#

def _is_windows_exe():
    '''
    Return whether the program is a windows frozen executable
    '''
    ...
    try:
        ...
        base1, ext1 = os.path.splitext(os.path.abspath(sys.argv[0].lower()))
        base2, ext2 = os.path.splitext(os.path.abspath(sys.executable.lower()))
        return base1 == base2 and ext1 in ('.exe', '') and ext2 in ('.exe', '')

_done_freeze_support = False
_IS_WINDOWS_EXE = _is_windows_exe()

if _IS_WINDOWS_EXE:
    _fork_commandline = [sys.executable, '--processing-fork']
else:
    _fork_commandline = [sys.executable, _nonforking.__file__]
...
if sys.platform != 'win32':
    ...
    class Popen(subprocess.Popen):
        ...
        def __init__(self, process_obj):
            if sys.argv[0] not in ('', '-c') and not _IS_WINDOWS_EXE:
                mainpath = os.path.abspath(sys.argv[0])
            else:
                mainpath = None

            preparation_data = [mainpath, sys.path, os.getcwd()]
        ...
        if '_processing' in globals():
            def stop(self):
                try:
                    CTRL_BREAK_EVENT = 1
                    _processing.GenerateConsoleCtrlEvent(CTRL_BREAK_EVENT,
                                                         self.pid)
                except WindowsError:
                    pass
        else:
