...
#### pseudomethod parser
@_import("ast tempfile")
class parser(ast.NodeVisitor):
  @staticmethod
  def compile_exec(ss, fpath): node = parser().parse(ss, fpath, "exec", sugar_pseudomethod = True); return compile(node, fpath, "exec")

  # @staticmethod
  # def exec(ss, globals, locals = None, fpath = ""): exec(parser.compile_exec(ss, fpath), globals, locals)

  @staticmethod
  def exec(ss, globals, locals = None, fpath = ""):
    if not fpath:
      with tempfile.NamedTemporaryFile() as ff: return parser.exec(ss, globals, locals, ff.name)
    exec(parser.compile_exec(ss, fpath), globals, locals)
    ...
    for dpath in path or sys.path:
      ...
      if os.path.exists(fpath):
        ...
        with open(fpath, "r") as f:
          ss = f.read(1024)
          if self.sugar in ss:
            ss = ss.replace(self.sugar, "\n#{}".format(self.sugar[1:], 1)); self.found = fpath, ss + f.read(); return self
