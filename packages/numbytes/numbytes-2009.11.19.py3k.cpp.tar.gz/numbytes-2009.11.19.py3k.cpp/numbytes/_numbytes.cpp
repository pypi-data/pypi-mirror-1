...
typedef long long i64; typedef ssize_t ssz; typedef unsigned char uch; typedef unsigned int uint; int ERROR = -1;
...
namespace IMG2TXT {
  ...
  uch *ansi_check(PyObject *barr, int *TROWS, int *TCOLS) {
    ...
    if (PyObject_Size(barr) < HEADER_SIZE) throw pyerr(PyExc_IndexError, "bytearray size must be at least %i bytes", HEADER_SIZE);
    ...
    if (PyObject_Size(barr) != HEADER_SIZE + *TROWS * *TCOLS * 2) throw pyerr(PyExc_IndexError, "bytearray size does not match image dimensions %i x %i", *TROWS, *TCOLS);
...
namespace NUMBYTES {
  enum code_TT {code_CC='c',code_II='i',code_FF='f',};
  typedef char CC; typedef long long II; typedef double FF;
  ...
  inline void str_TT(CC aa, char *&ss) { *ss = aa < 0 ? '-' : ' '; ss ++; ss += sprintf(ss, "%.2x ", abs(aa)); }
  ...
  inline void math_mod_TT(CC *aa, CC *bb) { *aa %= *bb; }; inline void math_mod_TT(II *aa, II *bb) { *aa %= *bb; }; inline void math_mod_TT(FF *aa, FF *bb) { *aa = fmod(*aa, *bb); }
  inline void math_lshift_TT(CC *aa, CC *bb) { *aa <<= *bb; }; inline void math_lshift_TT(II *aa, II *bb) { *aa <<= *bb; }; inline void math_lshift_TT(FF *aa, FF *bb) { throw pyerr(PyExc_ArithmeticError, "cannot <double <<= double>"); }
  inline void math_rshift_TT(CC *aa, CC *bb) { *aa >>= *bb; }; inline void math_rshift_TT(II *aa, II *bb) { *aa >>= *bb; }; inline void math_rshift_TT(FF *aa, FF *bb) { throw pyerr(PyExc_ArithmeticError, "cannot <double >>= double>"); }
  inline void math_and_TT(CC *aa, CC *bb) { *aa &= *bb; }; inline void math_and_TT(II *aa, II *bb) { *aa &= *bb; }; inline void math_and_TT(FF *aa, FF *bb) { throw pyerr(PyExc_ArithmeticError, "cannot <double &= double>"); }
  inline void math_or_TT(CC *aa, CC *bb) { *aa |= *bb; }; inline void math_or_TT(II *aa, II *bb) { *aa |= *bb; }; inline void math_or_TT(FF *aa, FF *bb) { throw pyerr(PyExc_ArithmeticError, "cannot <double |= double>"); }
  inline void math_xor_TT(CC *aa, CC *bb) { *aa ^= *bb; }; inline void math_xor_TT(II *aa, II *bb) { *aa ^= *bb; }; inline void math_xor_TT(FF *aa, FF *bb) { throw pyerr(PyExc_ArithmeticError, "cannot <double ^= double>"); }
  ...
  template <typename TT> class view; class pytype { public: PyObject_HEAD; void *tself; };
  inline int numbytes_CHECK(PyObject *self) { return PyObject_IsInstance(self, (PyObject *) &TYPE); }
  PyObject *numbytes_new(PyTypeObject *type, PyObject *args, PyObject *kwds);



  //// template
  template <typename TT> class view {
  public:
    PyObject *self; int tcode; PyObject *bytes; TT *words; int shape0, shape1, strides0, strides1;
#define ITERATE1() int i0, j0; for (i0 = 0; i0 < shape0; i0 ++) for (j0 = 0; j0 < shape1; j0 ++)
#define ITERATE2(tthem1) int i0, j0, i1, j1, di1 = tthem1 -> shape0 == 1 ? 0 : 1, dj1 = tthem1 -> shape1 == 1 ? 0 : 1; for (i0 = i1 = 0; i0 < shape0; i0 ++, i1 += di1) for (j0 = j1 = 0; j0 < shape1; j0 ++, j1 += dj1)
#define ITERATE3(tthem1, tthem2) int i0, j0, i1, j1, i2, j2, di1 = tthem1 -> shape0 == 1 ? 0 : 1, dj1 = tthem1 -> shape1 == 1 ? 0 : 1, di2 = tthem2 -> shape0 == 1 ? 0 : 1, dj2 = tthem2 -> shape1 == 1 ? 0 : 1; for (i0 = i1 = i2 = 0; i0 < shape0; i0 ++, i1 += di1, i2 += di2) for (j0 = j1 = j2 = 0; j0 < shape1; j0 ++, j1 += dj1, j2 += dj2)
#define Py_RETURN_SELF Py_INCREF(self); return self;
#define PROTOTYPE
#define VIEW(TT, self) ((view<TT> *) ((pytype *) self) -> tself)
    inline int CHECK_IDX0(int ii) { if ((ii < 0 ? ii += shape0 : ii) < 0 || shape0 <= ii) throw pyerr(PyExc_IndexError, "index[0]<%i> out of range", ii); return ii; }
    inline int CHECK_IDX1(int ii) { if ((ii < 0 ? ii += shape1 : ii) < 0 || shape1 <= ii) throw pyerr(PyExc_IndexError, "index[1]<%i> out of range", ii); return ii; }
    inline void CHECK_MEM() { if (words < WORDS(bytes) || shape0 * shape1 + (words - WORDS(bytes)) > PyObject_Size(bytes)) throw pyerr(PyExc_MemoryError, "numbytes memory invalidated"); }
    inline view<TT> *CHECK_TCODE(PyObject *them) { if (VIEW(TT, them) -> tcode != tcode) throw pyerr(PyExc_TypeError, "inconsistent tcode self<%c> vs them<%c>", tcode, (VIEW(TT, them)) -> tcode); return VIEW(TT, them); }
    inline void CHECK_SHAPE(view<TT> *tthem) {
      if ((tthem -> shape0 != 1 && tthem -> shape0 != shape0) || (tthem -> shape1 != 1 && tthem -> shape1 != shape1)) throw pyerr(PyExc_IndexError, "inconsistent shape0 - self<%i %i> vs them<%i %i>", shape0, shape1, tthem -> shape0, tthem -> shape1);
    }
    inline int CONTIGUOUS() { return strides1 == 1 && WORDS(bytes) == words; }
    inline TT *GETITEM(int ii, int jj) { return words + ii * strides0 + jj * strides1; }
    inline int LENGTH() { return shape0 * shape1; }
    inline int TRANSPOSED() { return strides1 != 1; }
    inline static int TSIZE() { return sizeof(TT); }
    inline static TT *WORDS(PyObject *bytes) { return (TT *) PyByteArray_AS_STRING(bytes); }



    //// constructor
    inline ~view() { Py_DECREF(bytes); }
    inline view(PyTypeObject *type, int _tcode, PyObject *_bytes, TT *_words, int _shape0, int _shape1, int _strides0, int _strides1) {
      self = type -> tp_alloc(type, 0); if (! self) throw ERROR;
      ((pytype *) self) -> tself = this;
      tcode = _tcode; bytes = _bytes; words = _words; shape0 = _shape0; shape1 = _shape1; strides0 = _strides0; strides1 = _strides1; Py_INCREF(bytes);
    }
    PROTOTYPE inline void numbytes_dealloc() { delete this; }
    inline static PyObject *numbytes_new(PyTypeObject *type, int tcode, PyObject *bytes, TT *words, int shape0, int shape1, int strides0, int strides1) {
      try { return (new view(type, tcode, bytes, words, shape0, shape1, strides0, strides1)) -> self; }
      catch (bad_alloc&) { throw pyerr(PyExc_MemoryError, "fail new numbytes"); }
    }
    PROTOTYPE static PyObject *numbytes_new(PyTypeObject *type, PyObject *args) {
      int tcode; PyObject *bytes; if (! PyArg_ParseTuple(args, "iO:numbytes_new", &tcode, &bytes)) throw ERROR;
      if (! PyByteArray_Check(bytes)) throw pyerr(PyExc_TypeError, "not bytearray object");
      int ll = PyObject_Size(bytes); if (ll % TSIZE()) throw pyerr(PyExc_IndexError, "bytearray size not multiple of tsize");
      ll /= TSIZE(); return numbytes_new(type, tcode, bytes, WORDS(bytes), 1, ll, ll, 1);
    }



    //// static method
    PROTOTYPE static PyObject *static_tsize_from_tcode(PyObject *args) { return PyLong_FromLong(TSIZE()); }
    PROTOTYPE static PyObject *static_bytes_from_itr(PyObject *args) {
      int tcode, ll; PyObject *itr, *bytes, *aa; if (! PyArg_ParseTuple(args, "iiO:static_bytes_from_itr", &tcode, &ll, &itr)) throw ERROR;
      if (! PyIter_Check(itr)) throw pyerr(PyExc_TypeError, "not iterator object");
      bytes = PyByteArray_FromStringAndSize(NULL, ll * TSIZE()); if (! bytes) throw ERROR;
      try { TT *words = WORDS(bytes); for (int ii = ll; ii -- && (aa = PyIter_Next(itr)); words ++) { as_TT(aa, words); Py_DECREF(aa); } return bytes; }
      catch (...) { if (aa) { Py_DECREF(aa); } Py_DECREF(bytes); throw; }
    }



    //// sequence
    PROTOTYPE int sq_length() { return LENGTH(); }
    PROTOTYPE int sq_contains(PyObject *aa) { TT bb; as_TT(aa, &bb); ITERATE1() if (*GETITEM(i0, j0) == bb) return true; return false; }
    PROTOTYPE PyObject *sq_item(int ii) { if (ii >= LENGTH()) throw ERROR; return from_TT(*GETITEM(ii / shape1, ii % shape1)); }



    //// getset
    PROTOTYPE PyObject *get_base(PyObject *args) { return numbytes_new(&TYPE, tcode, bytes, words, shape0, shape1, strides0, strides1); }
    PROTOTYPE PyObject *get_bytes(PyObject *args) { Py_INCREF(bytes); return bytes; }
    PROTOTYPE PyObject *get_contiguous(PyObject *args) { return PyLong_FromLong(CONTIGUOUS()); }
    PROTOTYPE PyObject *get_shape0(PyObject *args) { return PyLong_FromLong(shape0); }
    PROTOTYPE PyObject *get_shape1(PyObject *args) { return PyLong_FromLong(shape1); }
    PROTOTYPE PyObject *get_strides0(PyObject *args) { return PyLong_FromLong(strides0); }
    PROTOTYPE PyObject *get_strides1(PyObject *args) { return PyLong_FromLong(strides1); }
    PROTOTYPE PyObject *get_T(PyObject *args) { return numbytes_new(Py_TYPE(self), tcode, bytes, words, shape1, shape0, strides1, strides0); }
    // PROTOTYPE PyObject *get_tcode(PyObject *args) { return PyUnicode_FromFormat("%c", tcode); }
    PROTOTYPE PyObject *get_tcode(PyObject *args) { return PyLong_FromLong(tcode); }
    PROTOTYPE PyObject *get_transposed(PyObject *args) { return PyLong_FromLong(TRANSPOSED()); }
    PROTOTYPE PyObject *get_tsize(PyObject *args) { return PyLong_FromLong(TSIZE()); }



    //// method
    // template <typename UU> inline PyObject *recast(view<UU> *tthem) { ITERATE2(tthem) { *tthem -> GETITEM(i1, j1) = (UU) *GETITEM(i0, j0); } return tthem -> self; }
    template <typename UU> inline PyObject *recast(view<UU> *tthem) { UU *aa = tthem -> words; ITERATE1() { *aa = (UU) *GETITEM(i0,j0); aa ++; } return tthem -> self; }
    PROTOTYPE PyObject *recast(PyObject *args) {
      PyObject *them = NUMBYTES::numbytes_new(Py_TYPE(self), args, NULL); if (! them) throw ERROR;
      view<TT> *tthem = VIEW(TT, them);
      if (tthem -> shape0 * tthem -> shape1 != LENGTH()) throw pyerr(PyExc_IndexError, "invalid length <%i> -> <%i>", LENGTH(), tthem -> shape0 * tthem -> shape1);
      // tthem -> shape0 = shape0; tthem -> shape1 = shape1;
      try { switch (tthem -> tcode) {
        case code_CC: return recast(VIEW(CC, them));
        case code_II: return recast(VIEW(II, them));
        case code_FF: return recast(VIEW(FF, them));
        default: throw pyerr(PyExc_ValueError, "invalid tcode <%c>", tthem -> tcode);
        }
      }
      catch (...) { Py_DECREF(them); throw; }
    }
    PROTOTYPE PyObject *reshape(PyObject *args) {
      int ll, mm; if (! PyArg_ParseTuple(args, "ii:reshape", &ll, &mm)) throw ERROR;
      if (! CONTIGUOUS()) throw pyerr(PyExc_IndexError, "cannot reshape non-contiguous numbytes");
      if (ll == -1) ll = LENGTH() / mm; else if (mm == -1) mm = LENGTH() / ll;
      if (ll < 0 || ll * mm != LENGTH()) throw pyerr(PyExc_IndexError, "invalid reshape <%i %i> -> <%i %i>", shape0, shape1, ll, mm);
      return numbytes_new(Py_TYPE(self), tcode, bytes, words, ll, mm, mm, strides1);
    }
    PROTOTYPE PyObject *retype(PyObject *args) {
      PyObject *type; if (! PyArg_ParseTuple(args, "O:retype", &type)) throw ERROR;
      return numbytes_new((PyTypeObject *) type, tcode, bytes, words, shape0, shape1, strides0, strides1);
    }
    inline void parse_slice(int ii, PyObject *slice, ssz &start, ssz &stop, ssz &step, ssz &slicelength) {
      if (PyLong_Check(slice)) { start = ii ? CHECK_IDX1(PyLong_AsLong(slice)) : CHECK_IDX0(PyLong_AsLong(slice)); stop = start + 1; step = 1; slicelength = 1; }
    ...
    PROTOTYPE PyObject *__getitem(PyObject *args) {
      int ii, jj; if (PyArg_ParseTuple(args, "ii:__getitem", &ii, &jj)) { return from_TT(*GETITEM(CHECK_IDX0(ii), CHECK_IDX1(jj))); } else { PyErr_Clear(); }
      PyObject *slice0 = NULL, *slice1 = NULL; if (! PyArg_ParseTuple(args, "OO:__getitem", &slice0, &slice1)) throw ERROR;
      ...
      return numbytes_new(Py_TYPE(self), tcode, bytes, GETITEM(start0, start1), slicelength0, slicelength1, step0 * strides0, step1 * strides1);
    }
    inline void setslice(PyObject *aa) {
      if (! numbytes_CHECK(aa)) { TT bb; as_TT(aa, &bb); ITERATE1() *GETITEM(i0, j0) = bb; return; } // number
      view<TT> *tthem = CHECK_TCODE(aa); CHECK_SHAPE(tthem); ITERATE2(tthem) *GETITEM(i0, j0) = *tthem -> GETITEM(i1, j1);
    }
    PROTOTYPE PyObject *__setitem__(PyObject *args) {
      PyObject *slices, *aa; if (! PyArg_ParseTuple(args, "OO:__setitem__", &slices, &aa)) throw ERROR;
      int ii, jj; if (PyArg_ParseTuple(slices, "ii:__setitem__", &ii, &jj)) { as_TT(aa, GETITEM(ii, jj)); Py_RETURN_NONE; } else { PyErr_Clear(); }
      PyObject *_self = __getitem(slices);
      try { VIEW(TT, _self) -> setslice(aa); Py_DECREF(_self); Py_RETURN_NONE; }
      catch (...) { Py_DECREF(_self); throw; }
    }
    PROTOTYPE PyObject *__str__(PyObject *args) {
      char ss[LENGTH() * 16 + 1]; char *tt = ss; ITERATE1() str_TT(*GETITEM(i0, j0), tt);
      return PyUnicode_FromStringAndSize(ss, tt - ss);
    }
#define CASE1(ii, op) case ii: for (j0 = 0; j0 < shape1; j0 ++) { op; } break;
#define CASE2(ii, op) case ii: for (i0 = i1 = 0; i0 < shape0; i0 ++, i1 += di1) for (j0 = j1 = 0; j0 < shape1; j0 ++, j1 += dj1) { op; } break;
#define CASE3(ii, op) for (i0 = i1 = i2 = 0; i0 < shape0; i0 ++, i1 += di1, i2 += di2) for (j0 = j1 = j2 = 0; j0 < shape1; j0 ++, j1 += dj1, j2 += dj2) { op; } break;
    enum MATH_OP {MATH_ABS1='a',MATH_ACOS1='C',MATH_ASIN1='S',MATH_ATAN1='T',MATH_CEIL1='~',MATH_COS1='c',MATH_COSH1='h',MATH_FLOOR1='f',MATH_SIN1='s',MATH_SINH1='i',MATH_SQRT1='2',MATH_TAN1='t',MATH_TANH1='j',
                  MATH_ADD2='+',MATH_AND2='&',MATH_ATAN2='U',MATH_DIV2='/',MATH_EXP2='e',MATH_LOG2='l',MATH_LSHIFT2='<',MATH_MOD2='%',MATH_MUL2='*',MATH_OR2='|',MATH_POW2='p',MATH_RSHIFT2='>',MATH_SUB2='-',MATH_XOR2='^',
                  MATH_FMA3='n',};
    PROTOTYPE PyObject *math_iop3(PyObject *args) {
      int op; PyObject *them1, *them2; if (! PyArg_ParseTuple(args, "iOO:math_iop3", &op, &them1, &them2)) throw ERROR;
      if (! numbytes_CHECK(them1) || ! numbytes_CHECK(them2)) throw pyerr(PyExc_TypeError, "them1 and them2 must be numbytes");
      view *tthem1 = CHECK_TCODE(them1), *tthem2 = CHECK_TCODE(them2); CHECK_SHAPE(tthem1); CHECK_SHAPE(tthem2);
      int i0, j0, i1, j1, i2, j2, di1 = tthem1 -> shape0 == 1 ? 0 : 1, dj1 = tthem1 -> shape1 == 1 ? 0 : 1, di2 = tthem2 -> shape0 == 1 ? 0 : 1, dj2 = tthem2 -> shape1 == 1 ? 0 : 1;
      switch (op) {
        CASE1(MATH_ABS1, *GETITEM(i0, j0) = abs(*GETITEM(i0, j0)); );
        CASE1(MATH_ACOS1, *GETITEM(i0, j0) = acos(*GETITEM(i0, j0)); );
        CASE1(MATH_ASIN1, *GETITEM(i0, j0) = asin(*GETITEM(i0, j0)); );
        CASE1(MATH_ATAN1, *GETITEM(i0, j0) = atan(*GETITEM(i0, j0)); );
        CASE1(MATH_CEIL1, *GETITEM(i0, j0) = ceil(*GETITEM(i0, j0)); );
        CASE1(MATH_COS1, *GETITEM(i0, j0) = cos(*GETITEM(i0, j0)); );
        CASE1(MATH_COSH1, *GETITEM(i0, j0) = cosh(*GETITEM(i0, j0)); );
        CASE1(MATH_FLOOR1, *GETITEM(i0, j0) = floor(*GETITEM(i0, j0)); );
        CASE1(MATH_SIN1, *GETITEM(i0, j0) = sin(*GETITEM(i0, j0)); );
        CASE1(MATH_SINH1, *GETITEM(i0, j0) = sinh(*GETITEM(i0, j0)); );
        CASE1(MATH_SQRT1, *GETITEM(i0, j0) = sqrt(*GETITEM(i0, j0)); );
        CASE1(MATH_TAN1, *GETITEM(i0, j0) = tan(*GETITEM(i0, j0)); );
        CASE1(MATH_TANH1, *GETITEM(i0, j0) = tanh(*GETITEM(i0, j0)); );
        // CASE2(MATH_ADD2, *GETITEM(i0, j0) += *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_AND2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_ATAN2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_DIV2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_EXP2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_LOG2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_LSHIFT2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_MOD2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_MUL2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_OR2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_POW2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_RSHIFT2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_SUB2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );
        // CASE2(MATH_XOR2, *GETITEM(i0, j0) &= *tthem1 -> GETITEM(i1, j1); );

        CASE2('+', *GETITEM(i0, j0) += *tthem1 -> GETITEM(i1, j1); );
        CASE2('-', *GETITEM(i0, j0) -= *tthem1 -> GETITEM(i1, j1); );
        CASE2('*', *GETITEM(i0, j0) *= *tthem1 -> GETITEM(i1, j1); );
        CASE2('/', *GETITEM(i0, j0) /= *tthem1 -> GETITEM(i1, j1); );
        // CASE2('p', *GETITEM(i0, j0) = pow(*GETITEM(i0, j0), tthem1 -> GETITEM(i1, j1)));
        CASE2('%', math_mod_TT(GETITEM(i0, j0), tthem1 -> GETITEM(i1, j1)));
        CASE2('<', math_lshift_TT(GETITEM(i0, j0), tthem1 -> GETITEM(i1, j1)));
        CASE2('>', math_rshift_TT(GETITEM(i0, j0), tthem1 -> GETITEM(i1, j1)));
        CASE2('&', math_and_TT(GETITEM(i0, j0), tthem1 -> GETITEM(i1, j1)));
        CASE2('|', math_or_TT(GETITEM(i0, j0), tthem1 -> GETITEM(i1, j1)));
        CASE2('^', math_xor_TT(GETITEM(i0, j0), tthem1 -> GETITEM(i1, j1)));
        CASE3(MATH_FMA3, *GETITEM(i0, j0) = fma(*tthem1 -> GETITEM(i1, j1), *tthem2 -> GETITEM(i2, j2), *GETITEM(i0, j0)); );
      default: throw pyerr(PyExc_ValueError, "invalid op<%c>", op);
      }
      Py_RETURN_SELF;
    }
  };
  ...
  //// init
#define METH_TT(call, err) { try { switch (VIEW(CC, self) -> tcode) {   \
      case code_CC: VIEW(CC, self) -> CHECK_MEM(); return VIEW(CC, self) -> call; \
      case code_II: VIEW(II, self) -> CHECK_MEM(); return VIEW(II, self) -> call; \
      case code_FF: VIEW(FF, self) -> CHECK_MEM(); return VIEW(FF, self) -> call; \
      default: throw pyerr(PyExc_ValueError, "invalid tcode <%c>", VIEW(CC, self) -> tcode); } } catch (...) { return err; } }
#define STATMETH_TT(call, err)  { try { PyObject *_; int tcode; if (! PyArg_ParseTuple(args, "i|OOO", &tcode, &_, &_, &_)) throw ERROR; switch (tcode) { \
      case code_CC: return view<CC>::call;                              \
      case code_II: return view<II>::call;                              \
      case code_FF: return view<FF>::call;                              \
      default: throw pyerr(PyExc_ValueError, "invalid tcode <%c>", tcode); } } catch (...) { return err; } }
#include "_numbytes.hpp"
  PySequenceMethods PYSEQUENCE = {NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL};
  PyObject *numbytes_new(PyTypeObject *type, PyObject *args, PyObject *kwds) { STATMETH_TT(numbytes_new(type, args), NULL); }
  int sq_length(PyObject *self) { METH_TT(sq_length(), ERROR); }
  int sq_contains(PyObject *self, PyObject *aa) { METH_TT(sq_contains(aa), ERROR); }
  PyObject *sq_item(PyObject *self, int ii) { METH_TT(sq_item(ii), NULL); }
  void numbytes_dealloc(PyObject *self) { METH_TT(numbytes_dealloc(), ); }
  PyObject *numbytes_init(PyObject *module) {
    TYPE.tp_dealloc = numbytes_dealloc; TYPE.tp_new = numbytes_new; TYPE.tp_str = (reprfunc) __str__; TYPE.tp_basicsize = sizeof(pytype); TYPE.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE; // constructor
    TYPE.tp_as_sequence = &PYSEQUENCE; PYSEQUENCE.sq_length = sq_length; PYSEQUENCE.sq_contains = sq_contains; PYSEQUENCE.sq_item = sq_item; // sequence
    TYPE.tp_getset = PYGETSET; TYPE.tp_methods = PYMETHOD; // method
...
namespace numbytes {
  PROTOTYPE PyObject *is_itr(PyObject *self, PyObject *aa) { return PyLong_FromLong(    PyIter_Check(aa)); }
  PROTOTYPE PyObject *is_map(PyObject *self, PyObject *aa) { return PyLong_FromLong( PyMapping_Check(aa)); }
  PROTOTYPE PyObject *is_num(PyObject *self, PyObject *aa) { return PyLong_FromLong(  PyNumber_Check(aa)); }
  PROTOTYPE PyObject *is_numbytes(PyObject *self, PyObject *aa) { return PyLong_FromLong(NUMBYTES::numbytes_CHECK(aa)); }
  PROTOTYPE PyObject *is_seq(PyObject *self, PyObject *aa) { return PyLong_FromLong(PySequence_Check(aa)); }
  PROTOTYPE PyObject *refcnt(PyObject *self, PyObject *aa) { return PyLong_FromSsize_t(Py_REFCNT(aa)); }
  ...
  extern "C" {
    PyObject *PyInit__numbytes(void) {
      ...
      if (! NUMBYTES::numbytes_init(module)) return NULL;
      return module;
    }
  }
}
