...
class Circle(Shape):
    ...
    def scanLines(this, xmin=None, xmax=None, ymin=None, ymax=None):
        ...
        for y in xrange(ymin, ymax+1):
            dy = y-cy
            dx2 = radius2 - dy*dy
            #print "(dy,dx2,radius2)", (y,dy,dx2, radius2)
            if dx2>0:
                dx = sqrt(dx2)
                x1 = int(round(cx-dx))
...
class Rectangle(Shape):
    ...
    def __init__(this, lowerLeft, width, height):
        #print "rect", (lowerLeft, width, height)
        ...
        this.wx = llx+width
        this.hy = lly+height
        #print "rect end", (this.wx, this.hy)
    def pixelTest(this,point):
        eps = this.epsilon
        (x,y) = point
        x1 = x+1
        ...
        lly = this.lly
        hy = this.hy
        if x1<llx-eps or y1<lly-eps or x>wx+eps or y>hy+eps:
            #print "out", (x,y)
            #print x1<llx, y1<lly, x>wx, y>hy
            return -1
        if y1<hy-eps and y>lly+eps and x1<wx-eps and x>llx+eps:
            #print "inside", (x,y)
            #print y1<hy, y>lly, x1<wx, x>llx
            return 1
        #print "border", (x,y)
        #print x1<llx, y1<lly, x>wx, y>hy
        #print y1<hy, y>lly, x1<wx, x>llx
    ...
    def bbox(this):
        llx = this.llx
        lly = this.lly
        bbox = ( (llx, lly), (llx+this.width+1, lly+this.height+1) )
        #print "bbox", bbox
        #raise ValueError
        return bbox

def LineSegment(startPoint, endPoint, width, StartCapSize=None, EndCapSize=None):
    #print "ls", (startPoint, endPoint, width, StartCapSize, EndCapSize)
    import math
    ...
    d = math.sqrt(d2)
    unitvector = (dx/d, dy/d)
    cd = d+EndCapSize+StartCapSize
    Rect = Rectangle( (-StartCapSize, -halfwidth), cd, width )
    RotateRect = RotateV( Rect, unitvector )
    ShiftRect = Translate( RotateRect, startPoint )
    return ShiftRect
...
class Canvas:
    ...
    def addLines(this, points, closed=False):
        ...
        lastpoint = points[-1]
        L = LineSegment(recentpoint, lastpoint, width, 0, cap)
        this.plot(L)
