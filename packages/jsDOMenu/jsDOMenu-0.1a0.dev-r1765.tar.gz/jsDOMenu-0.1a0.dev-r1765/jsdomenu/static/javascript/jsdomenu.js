  ...
  if (this.enabled && this.actionOnClick) {
    ...
    if (action.indexOf("link:") == 0) {
      ...
      if (action.indexOf("code:") == 0) {
        ...
        if (action.indexOf("iframe:") == 0) {
            action = action.substr(7);
            separator_pos = action.indexOf(':');
            iframe = action.substr(0, separator_pos);
            action = action.substr(separator_pos + 1);
...
function addMenuItem(menuItemObj) { // Public method
  if (menuItemObj.displayText == "-") {
    ...
    if (itemElm.actionOnClick) {
        var linkNode = createElm("a");
        var imgNode = createElm("img");
        imgNode.src = "/tg_widgets/jsdomenu/images/external.png";
        imgNode.border = 0;
        linkNode.appendChild(imgNode);
        
        linkNode.className = "jsdomenuitemexternallink";
        linkNode.target = "_blank";
        linkNode.href = itemElm.actionOnClick;
        ...
        linkNode.actionOnClick = itemElm.actionOnClick;
        itemElm.appendChild(linkNode);
    }
