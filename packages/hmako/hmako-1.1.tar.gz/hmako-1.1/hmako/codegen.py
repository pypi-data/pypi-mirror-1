...
import time
import re
from htmlmako.pygen import PythonPrinter
from htmlmako import util, ast, parsetree, filters
...
class _GenerateRenderMethod(object):
    ...
    def write_toplevel(self):
        ...
        self.printer.writeline("from htmlmako import runtime, filters, cache")
    ...
    def write_inherit(self, node):
        ...
        self.printer.writelines(
            "def _htmlmako_inherit(template, context):",
                "_htmlmako_generate_namespaces(context)",
    ...
    def write_namespaces(self, namespaces):
        ...
        self.printer.writelines(
            "def _htmlmako_get_namespace(context, name):",
                "try:",
        ...
        self.printer.writeline("def _htmlmako_generate_namespaces(context):")
    ...
    def write_variable_declares(self, identifiers, toplevel=False, limit=None):
        ...
        if toplevel and getattr(self.compiler, 'has_ns_imports', False):
            ...
            for ident, ns in self.compiler.namespaces.iteritems():
                if ns.attributes.has_key('import'):
                    self.printer.writeline("_htmlmako_get_namespace(context, %s)._populate(_import_ns, %s)" % (repr(ident),  repr(re.split(r'\s*,\s*', ns.attributes['import']))))
        ...
        for ident in to_write:
            if ident in comp_idents:
                ...
                self.printer.writeline("%s = _htmlmako_get_namespace(context, %s)" % (ident, repr(ident)))
            else:
