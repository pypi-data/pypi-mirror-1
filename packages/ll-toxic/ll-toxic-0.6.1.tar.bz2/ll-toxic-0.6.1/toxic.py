...
from ll.xist import xsc, errors, publishers
...
@classmethod
def stringify(cls, string):
	"""
	Format <arg>string</arg> as a PL/SQL expression.
	"""
	result = []
	current = []
...
def xml2ora(cls, string, encoding):
	...
	class Writer(object):
		...
		def __str__(self):
			if isclob:
				...
				return "\tc_out := c_out || %s;\n" % self.text

	for (t, s) in cls.tokenize(string, encoding):
		if t is str:
			foundplsql.append(Writer(cls.stringify(s)))
		elif issubclass(t, code):
			foundplsql.append(s)
			foundplsql.append("\n")
		elif issubclass(t, expr):
			foundplsql.append(Writer(s))
	...
	isclob = plaintype in ("clob", "nclob")
...
def prettify(cls, string):
	"""
	Try to fix the indentaion of the PL/SQL snippet passed in.
	"""
	...
	for line in lines:
		if not line:
			...
			if line.endswith("("):
				...
				# as followed by begin have same indentation
				pre = -1
