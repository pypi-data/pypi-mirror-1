...
class FileUploadWidget(widget.SimpleInputWidget):
    ...
    def __call__(self):
        kwargs = {'name': self.name, 'filename': ''}
        ...
        if self._data is not None:
            ...
            options = """
            <div><input type="radio" name="_modify_%(name)s" value="0" checked="checked" /> Keep existing file '%(filename)s'</div>
            <div><input type="radio" name="_modify_%(name)s" value="1" /> Replace with new file:</div>
            """ % kwargs
        elif self.context.required:
            options = """
            <input type="hidden" name="_modify_%(name)s" value="1" />
            """ % kwargs
        else:
            options = """
            <div><input type="radio" name="_modify_%(name)s" value="0" checked="checked" /> No file </div>
            <div><input type="radio" name="_modify_%(name)s" value="1" /> New file:</div>
            """ % kwargs           
