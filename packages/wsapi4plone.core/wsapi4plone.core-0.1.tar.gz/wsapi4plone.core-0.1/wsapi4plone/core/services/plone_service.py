    ...
    def set_properties(self, params):
        for par in params:
            if isinstance(params[par], xmlrpclib.DateTime):
                # ISO 8601 formatted dates are required!
                # DateTime raises a SyntaxError if it is ill formated.
                # Python 2.4 is horrible at converting these values
                # Python 2.5 version:
                # datetime.datetime.strptime(params[par].value, "%Y%m%dT%H:%M:%S").strftime("%d.%m.%Y, %H:%M")
                # TODO pumazi: The format of the xmlrpclib.DateTime seems to change between 2.4 and 2.5
                iso8601_str = datetime.datetime(*time.strptime(params[par].value, "%Y%m%dT%H:%M:%S")[:5]).strftime("%Y/%m/%d %H:%M:%S")
                params[par] = DateTime(iso8601_str)
...
class PloneServiceContainer(PloneService, ServiceContainer):
    ...
    def get_misc(self, as_callback=False):
        results = {}
        query_line = {'path': {'query': '/'.join(self.context.getPhysicalPath()), 'depth': 1}}
        if as_callback:
            results['contents.callback'] = {'function': 'query', 'args': (query_line,)}
        else:
            formatter = getUtility(IFormatQueryResults)
            contents = formatter(self.context.portal_catalog(query_line))
            if contents: results['contents'] = contents
        return results
