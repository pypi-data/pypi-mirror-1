...
#from lib import color_markup_string as cms
...
def run_commands(options, printer):
    ...
    options.add and tasks.add_task(options.add)
    
    if options.remove:
        tasks.list_all_tasks(printer=printer)
        task_index = raw_input('\nEnter index of task to be moved to the trash: ')
        tasks.move_to_trash(int(task_index) - 1)
    
    if options.done:
        tasks.list_active_tasks(printer=printer, with_index=True)
        task_index = raw_input('\nEnter index of task that is done: ')
        tasks.mark_as_completed(int(task_index) - 1)    
    
    if options.un_done:
        tasks.list_all_tasks(printer=printer)
        task_index = raw_input('\nEnter index of task to set active: ')
    ...
    if not options.show_all and not options.show_completed and not options.show_trash:
        tasks.list_active_tasks(printer=printer, with_index=options.indexed)
...
def run(): 
    ...
    parser = OptionParser('%prog: [-n TASK] ', version=get_version().current_release())
    ...
    basic_group.add_option('-n', '--add', dest='add', 
                           help="""add a new task""", default=None)
    ...
    list_group.add_option('-i', '--indexed', action="store_true", dest='indexed', 
                          help="""show active tasks with index""", default=False)
