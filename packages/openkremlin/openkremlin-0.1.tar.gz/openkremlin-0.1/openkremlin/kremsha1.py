...
"""An implementation of SHA-1 in pure Python, from the PyPy project.
   This file is licensed under the MIT license; see
   http://codespeak.net/pypy/dist/LICENSE

   Framework adapted from Dinu Gherman's MD5 implementation by
   J. Hallén and L. Creighton. SHA-1 implementation based directly on
   the text of the NIST standard FIPS PUB 180-1.

   Modified to replicate the endianness bug in Kremlin Encrypt's SHA-1
   implementation.
"""
...
class kremsha1:
    ...
    def digest(self):
        ...
        H4 = self.H4
        input = [] + self.input
        count = map(_byteswap, [] + self.count)
        ...
        bits = _bytelist2longLittleEndian(self.input[:56]) + count
