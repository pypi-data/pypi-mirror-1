...
static PyObject* Stemmer_call(Stemmer *self, PyObject *args, PyObject *kw)
{
    ...
    res = PyArg_ParseTuple(args, "O", &from_stem);
    if (!res) {
        PyErr_BadArgument();
        return NULL;
    }
    ...
    if (unicode) {
        ...
        to_stem = PyUnicode_AsUTF8String(from_stem);
        if (!to_stem) {
            return NULL;
        }
    } else {
        ...
        /* Make copy. Is this correct way? */
        to_stem = PyString_FromString(PyString_AsString(from_stem));
    }

    res = stem(&self->stem,
        PyString_AsString(to_stem), PyString_Size(to_stem)-1);
...
static const char *module_doc = "Porter Stemmer Algorithm\n\n"
"\"The Porter stemming algorithm (or ‘Porter stemmer’) is a process f\n"
...
"Stemmer is callable object without any method/member.\n"
"Stemmer.__call__(self, word)\n"
"word must be str or unicode object.\n"
"returns stemmed str or unicode object.\n";
