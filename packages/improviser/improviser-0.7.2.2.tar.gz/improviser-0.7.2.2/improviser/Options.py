...
def save_options(filename, options):
	...
	f.write("# Improviser configuration\n")
	f.write("conf_version = 2\n")
			...
			if len(parts) == 2:
				...
				if attr == "conf_version":
					if int(val) > 2 or int(val) < 0:
						raise OptionError, "Cannot handle version %d of this file." % int(val)
...
def set_progressions(options, movement):
	for prog in options.progression.split(","):
		b = check_options(prog, get_available_progressions,
			"progression", Progressions)
		if b != False:
			movement.add_progression(b)
		else:
			if prog != "":
				raise OptionError, "Unknown progression '%s'" % prog
...
def parse_instrument_params(params):
	...
	if params[0] != "{" or params[-1] != "}":
		raise OptionError, "Syntax error in instrument parameters. Need to open and close with a parenthesis."
...
def set_misc_options(options, block):
	...
	block.wildness = int(options.wild)
...
def load_fluidsynth(options):
	if not options.no_fluidsynth:
		try:
			...
			raise OptionError, "Couldn't load the music library mingus. A download is available at http://mingus.googlecode.com/"
