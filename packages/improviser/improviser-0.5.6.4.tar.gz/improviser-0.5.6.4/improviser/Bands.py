...
metal = [
	...
	SoloInstrument({'channel':6, 'midi_instr': 32}),
	]

jazz = [
	SoloInstrument({'channel': 11, 'start': 2, 'step': 4}),
	SoloInstrument({'channel': 15, 'start': 6, 'stop': 8, 'step': 4, 'max_velocity': 80}),
	ChordInstrument({'channel': 4, 'max_velocity': 70, 'chance': 0.5, 'start': 4}),
	BassInstrument({'channel': 6, 'start': 3}),
	SlowStridePianist({'channel': 3, 'start':0, 'step': 4, 'max_velocity': 70}),
	FastStridePianist({'channel': 3, 'start':4, 'step': 4}),
