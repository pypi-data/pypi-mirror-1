...
class ProgressionDialog(QtGui.QDialog):
	...
	def add_chord(self):
		...
		self.ui.progression.addItem("%s %s" % (duration, chord))
	...
	def add_progression(self):
		...
		for x in range(self.ui.progression.count()):
			...
			res += "%s-" % text
		if res != "":
			r = str(self.ui.repeat.value())
			self.ui.progressionsequence.addItem(r + " " + res[:-1])
	...
	def load_progression(self, prog_str):
		...
		if len(parts) > 3:
			...
			for x in params:
				r = x.split("*")
				if len(r) == 2:
					self.ui.progressionsequence.addItem("%d %s" % (int(r[0]) - 1, r[1]))
				if len(r) == 1 and x != "":
					self.ui.progressionsequence.addItem("0 %s" % r[0])

	def save_progression(self):
		res = ""
		name = str(self.ui.progressionname.text())
		if name == "":
			return
