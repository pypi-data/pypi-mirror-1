...
class Engine(object):
    ...
    When a new message is received from the transport layer, it is
    passed on to the react() method which will unpack the raw message 
    and look up the correct handler. The handler may opt to return
    stomper.NO_RESPONSE_NEEDED or a valid STOMP message.
    
    Internally the STOMP messages are mapped as follows to the 
    ...
    It is the intention that this class be a framework on which more 
    complex message handling may be built. Derive from this class 
    and override to change the default behaviour. 
