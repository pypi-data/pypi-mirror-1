...
The module contains the following public classes:
    ...
    ArgumentParser
        The main entry point for command-line parsing. As the
    ...
    ArgumentError 
        The exception raised by ArgumentParser objects when
        there are errors with the parser's actions. Errors raised while
        parsing the command-line are caught by ArgumentParser and emitted
        as command-line messages.

    FileType 
        A factory for defining types of files to be created. As the
...
class Action(_AttributeHolder):
    ...
    const 
        The value to be produced if the option is specified and the
        option uses an action that takes no values.
    default
        The value to be produced if the option is not specified.
    type 
        The type which the command-line arguments should be converted
    ...
    choices
        ...
        collection.
    required 
        True if the action must always be specified at the command
...
class FileType(object):
    ...
    -----------------
    mode 
        A string indicating how the file is to be opened. Accepts the
        same values as the builtin open() function.
    bufsize 
        The file's desired buffer size. Accepts the same values as
