...
class BDist(BaseSubcommand):
    ...
    def __init__(self, subparsers):
        desc = ('Preform a bdist command on the projects within a checkout. '
            'This always builds eggs by appending a "--formats egg" but you '
    ...
    def main(self, args, cfg):
        ...
        if args.release:
            ...
            release = ''
        command = 'cd %%s && %s setup.py %s %sbdist %s %s' % (
            sys.executable, args.setup_args, release,
            '-d %s --formats egg' % args.dest,
            args.command_args)
