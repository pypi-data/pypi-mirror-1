...
class ProxyHTTPSConnection(ProxyHTTPConnection):
    ...
    def connect(self):
        ...
        if sys.version_info[:2] < (2, 6):
            import socket
            ssl_wrap = socket.ssl(self.sock, self.key_file, self.cert_file)
            self.sock = httplib.FakeSocket(self.sock, ssl_wrap)
        else:
            import ssl
            self.sock = ssl.wrap_socket(self.sock, certfile=self.cert_file,
