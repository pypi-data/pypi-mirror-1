...
#        if 'trunk' == project:
#            project = 'ets'
#            self._status('Interpreting "trunk" as "ets" project specification.',
#                ignore_verbose=True)
        ...
        # Otherwise, treat it as a setuptools requirement.
            ...
            try:
                ...
                if raise_missing:
                    raise ProjectNotFound('Unable to locate source for project '
                        '"%s" due to error: %s' % (project, str(e)))

                # Or, record the missing project and inform the user latter.
                else:
                    if parent is not None:
                        msg = '%s -- Required by parent project %s' % (e, parent)
                    else:
                        msg = str(e)
                ...
                try:
                    ...
                    for name, info in self._projects.items():
                        ...
                        try:
                            ...
                            for dep in info['install_requires']:
    ...
    def checkout(self, dir, dry_run=True):
        ...
        # Ensure the target directory doesn't already exist as either a file or
        # a directory as we don't want to overwrite anything or need to prompt
        # the user for any action.
        target = dir
        count = 0
        while os.path.exists(target):
            self._status('The directory name "%s" is already in ' % target + \
                'use, bumping the suffix count')
            count += 1
            target = '%s.%s' % (dir, count)
        ...
        try:
            ...
            for name, info in self._projects.items():
                ...
                if not dry_run:
                    ...
                    errcode = os.system('svn co "%s" "%s"' % (url, target_dir))
                    if errcode != 0:
                        break
    ...
    def inspect_repositories(self, repositories, ignore_repo_map=False):
        ...
        for repo in repositories:
            ...
            try:
                ...
                # map.
                for info in self._parse_svn_dir(repo):
    ...
    def _get_project_info_from_svn(self, url):
        ...
        try:
            f = urllib2.urlopen(setup_data_url)
            setup_data = f.read()
            f.close()
        ...
        if info is None:
            ...
            try:
                f = urllib2.urlopen(setup_url)
                setup = f.read()
                f.close()
            except Exception, e:
                raise ProjectParseError('Unable to find a setup.py for project URL '
                    '"%s".  Exception was: %s' % (setup_url, str(e)))
    ...
    def _parse_svn_dir(self, url):
        ...
        for link in re.findall(pattern, svn_content):
            ...
            try:
                self._status_indent += 1
                try:
                    f = urllib2.urlopen('%s/trunk/setup.py' % (link_url))
                    dummy = f.read()
                    f.close()
                    project_infos.extend(self._parse_svn_dir(link_url))
                    continue
        ...
        try:
            ...
            for link in re.findall(pattern, index):
                ...
                if '/' not in link[-1:]:
                    ...
                    remote_file = urllib2.urlopen(child_url)
