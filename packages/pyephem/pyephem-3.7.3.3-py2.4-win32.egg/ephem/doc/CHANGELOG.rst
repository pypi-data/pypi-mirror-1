...
- Added ``cmsI`` and ``cmsII`` attributes to ``Jupiter`` to provide the
  central meridian longitude in both System I and System II, for people
  trying to observe the Great Red Spot and other cloud features.

- **Bugfix**: Saturn was returning the wrong values for both its
  earthward and sunward angle tilt.
