...
def Node_attributes(node):
    ...
    while node is not None:
        ...
        if ns is not None:
            attributes[(libxml2mod.xmlNodeGetContent(ns), libxml2mod.name(node))] = node
        else:
...
def Node_namespaceURI(node):
    ns = libxml2mod.xmlNodeGetNs(node)
    if ns is not None:
        return to_unicode(libxml2mod.xmlNodeGetContent(ns))
    else:
        return None
...
def Node_setAttributeNS(node, ns, name, value):
    # NOTE: Need to convert from Unicode.
    ns, name, value = map(from_unicode, [ns, name, value])

    prefix, localName = _get_prefix_and_localName(name)

    # NOTE: Might need to be xmlSetNsProp.
    if ns is not None and ns == libxml2mod.xmlNodeGetContent(libxml2mod.xmlNodeGetNs(node)):
        libxml2mod.xmlNewNsProp(node, libxml2mod.xmlNodeGetNs(node), localName, value)
    elif prefix is not None:
        new_ns = libxml2mod.xmlNewNs(node, ns, prefix)
        libxml2mod.xmlNewNsProp(node, new_ns, localName, value)
    else:
        ...
        libxml2mod.xmlNewNsProp(node, None, localName, value)
...
def Node_createElementNS(node, ns, name):
    ...
    if ns is not None:
        new_ns = libxml2mod.xmlNewNs(new_node, ns, prefix)
