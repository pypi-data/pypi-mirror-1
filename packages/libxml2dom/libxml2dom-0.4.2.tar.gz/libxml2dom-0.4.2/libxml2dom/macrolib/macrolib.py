...
def check_document(node, tmp):
    if Node_ownerDocument(tmp) is not None and Node_ownerDocument(node) is not Node_ownerDocument(tmp):
...
def parse(stream_or_string, html=0, htmlencoding=None):
    if hasattr(stream_or_string, "read"):
        ...
        return parseString(stream.read(), html, htmlencoding)
    else:
        return parseFile(stream_or_string, html, htmlencoding)

def parseFile(s, html=0, htmlencoding=None):
