...
class Node(object):
    ...
    def _doctype(self):
        return self.impl.get_node(Node_doctype(self._node), self)
    ...
    def __eq__(self, other):
        return isinstance(other, Node) and libxml2mod.xmlXPathCmpNodes(self._node, other._node) == 0
...
def parse(stream_or_string, html=0, htmlencoding=None, impl=None):
    ...
    if hasattr(stream_or_string, "read"):
        ...
        return parseString(stream.read(), html, htmlencoding, impl)
    else:
        return parseFile(stream_or_string, html, htmlencoding, impl)

def parseFile(filename, html=0, htmlencoding=None, impl=None):
    ...
    impl = impl or default_impl
    return impl.adoptDocument(Node_parseFile(filename, html, htmlencoding))

def parseString(s, html=0, htmlencoding=None, impl=None):
    ...
    impl = impl or default_impl
    return impl.adoptDocument(Node_parseString(s, html, htmlencoding))

def parseURI(uri, html=0, htmlencoding=None, impl=None):
    ...
    if html:
        f = urllib.urlopen(uri)
        try:
            return parse(f, html, htmlencoding, impl)
        finally:
            f.close()
    else:
        impl = impl or default_impl
        return impl.adoptDocument(Node_parseURI(uri, html, htmlencoding))
