...
from dbf.exceptions import Bof, Eof, DbfError, DataOverflow, FieldMissing
...
class _DbfRecord(object):
    ...
    def _retrieveFieldValue(yo, record_data, fielddef):
        ...
        return yo._layout.fieldtypes[fielddef['type']]['Retrieve'](record_data, fielddef, yo._layout.memo)
    def _updateFieldValue(yo, fielddef, value):
        ...
        if len(update) > size:
            raise DataOverflow("tried to store %d bytes in %d byte field" % (len(update), size))
        blank = array('c', ' ' * size)
        start = fielddef['start']
        end = start + size
        new_data = array('c', update)
        blank[:len(update)] = new_data[:]
...
class DbfTable(object):
    ...
    _fieldtypes = {
            'D' : { 'Type':'Date',
                    ...
                    'Update':io.updateDate,
                  },
            'L' : { 
                    'Type':'Logical',
                    'Init':io.addLogical,
                    'Blank':bool,
                    'Retrieve':io.retrieveLogical,
                    'Update':io.updateLogical,
                  },
            'M' : { 
                    'Type':'Memo',
                    'Init':io.addMemo,
                    'Blank':str,
                    'Retrieve':io.retrieveMemo,
                    'Update':io.updateMemo,
                  } }
    _memoext = ''
    _memotypes = tuple()
    _memoClass = _DbfMemo
    ...
    _noMemoMask = ''
    _fixedFields = tuple()
    _decimalFields = tuple()
    _variableFields = tuple()
    _numericFields = tuple()
    ...
    def _loadtable(yo):
        ...
        for i in range(header.recordcount()):
            try:
                ...
                print
                print yo.filename(), yo.field_names()
                print "record length: %d" % length
                print "data length: %d" % len(record_data)
                print "data <%s>" % record_data
                print
                raise
            yo._index.append(i)
        dfd.seek(0)
    ...
    def __init__(yo, filename=':memory:', field_spec=None, memo_size=128, ignore_memos=False, 
                 read_only=False, keep_memos=False, meta_only=False, codepage='cp1252'):
        """open/create dbf file
        filename should include path if needed
        field_spec can be either a ;-delimited string or a list of strings
    ...
    def __str__(yo):
        ...
        """ % (yo.filename, version_map.get(yo._meta.header.version(), 'unknown - ' + hex(ord(yo._meta.header.version()))),
                status, yo.last_update(), len(yo), yo.field_count(), yo.record_length())
        str += "\n        --Fields--\n"
    ...
    def add_fields(yo, field_spec):
        ...
        if isinstance(field_spec, str):
            fields = field_spec.replace('; ',';').split(';')
        else:
            fields = list(field_spec)
        for field in fields:
            try:
    ...
    def append(yo, kamikaze='', drop=False, multiple=1):
        ...
        if not isinstance(kamikaze, _DbfRecord):
            if isinstance(kamikaze, dict):
                ...
                tupledata = kamikaze
                kamikaze = ''
        yo._table.append(_DbfRecord(recnum=yo._meta.header.recordcount(), layout=yo._meta, kamikaze=kamikaze))
    ...
    def eof(yo):
        ...
        while yo._meta.current < yo._meta.header.recordcount() - 1:
            ...
            return True
        return False
    def export(yo, records=None, filename=None, field_spec=None, format='csv', header=True):
        ...
        if field_spec is None:
            field_spec = yo.field_names()
        else:
            field_spec = field_spec.replace(', ',',').split(',')
        if records is None:
            records = yo
        format = format.lower()
        ...
        try:
            if format == 'csv':
                ...
                if header:
                    csvfile.writerow(field_spec)
                for record in records:
                    fields = []
                    for fieldname in field_spec:
                        fields.append(record[fieldname])
                    csvfile.writerow(fields)
            else:
                if header:
                    fd.write('\t'.join(field_spec) + '\n')
                for record in records:
                    fields = []
                    for fieldname in field_spec:
    ...
    def new(yo, filename, _field_spec=None):
        "returns a new table of the same type"
        if _field_spec is None:
            _field_spec = yo.structure()
        if filename != ':memory:':
        ...
        return yo.__class__(filename, _field_spec)
    def next(yo):
    ...
    def structure(yo, fields=None):
        ...
        try:
            if fields is None:
                for i in range(len(yo._meta.fields)):
                    field_spec.append(yo._fieldLayout(i))
            else:
                for name in fields.replace(' ','').split(';'):
                    field_spec.append(yo._fieldLayout(yo.field_names().index(name)))
        except ValueError:
...
class DbfList(object):
    ...
    def __init__(yo, new_records=None, desc=None):
        if new_records is not None:
            yo._list_of_records = list(new_records)
    ...
    def extend(yo, new_records):
        yo._list_of_records.extend(list(new_records))
        yo._current_record = len(yo._list_of_records) - 1
    def goto(yo, index_number):
        if yo._list_of_records:
            if 0 <= index_number <= len(yo._list_of_records):
                record = yo[index_number]
                yo._current_record = index_number
                return yo._list_of_records[yo._current_record]
            raise DbfError("index %d not in DbfList of %d records" % (index_number, len(yo._list_of_records)))
        raise DbfError("DbfList is empty")
