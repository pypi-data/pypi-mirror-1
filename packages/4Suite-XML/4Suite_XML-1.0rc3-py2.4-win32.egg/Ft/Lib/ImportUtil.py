########################################################################
# $Header: /var/local/cvsroot/4Suite/Ft/Lib/ImportUtil.py,v 1.1 2006-08-11 15:43:24 jkloth Exp $
"""
...
def GetLastModified(fullname):
    ...
    barename = os.sep + fullname.replace('.', os.sep)
    if loader.is_package(fullname):
        barename += os.sep + '__init__'
    for suffix in suffixes:
        pathname = barename + suffix
        try:
            timestamp = GetResourceLastModified(fullname, pathname)
