########################################################################
# $Header: /var/local/cvsroot/4Suite/Ft/Lib/DistExt/__init__.py,v 1.19.2.1 2006-08-24 19:24:42 jkloth Exp $
"""
...
__all__ = ['EnsureVersion', 'Extension']
...
if sys.platform.startswith('darwin') and util.get_platform().endswith('-ppc'):
    ...
    for name in ('LDFLAGS', 'LDSHARED', 'BLDSHARED'):
        if cfgvars[name].find('-arch i386') != -1:
            cfgvars[name] = vars[name].replace('-arch i386', '')
    del name, cfgvars
