import sys, os, base64, struct, time
from types import TupleType
import distutils
from distutils.command.bdist_wininst import bdist_wininst
from distutils.dir_util import create_tree, remove_tree

# (display, metadata_attr) or (display, [attr1, attr2...])
_metadata_names = [('Name', 'name'),
                   ('Version', 'version'),
                   ('Summary', 'description'),
                   ('Homepage', 'url'),
                   ('Author', ('maintainer', 'author')),
                   ('Email', ('maintainer_email', 'author_email')),
                   ('License', ('license', 'licence')),
                   ]

# This is, for the most part, the wininst command from Python 2.1.2 and 2.2
# modified to handle our installation scheme and run with Python 2.0 and 2.1

class BDistWininst(bdist_wininst):

    command_name = 'bdist_wininst'

    def initialize_options(self):
        # Features missing form distutils prior Python 2.1.2
        self.title = None
        self.bitmap = None
        return bdist_wininst.initialize_options(self)

    def finalize_options(self):
        if self.bdist_dir is None:
            bdist_base = self.get_finalized_command('bdist').bdist_base
            self.bdist_dir = os.path.join(bdist_base,
                                          'wininst-%s' % sys.version[:3])
        return bdist_wininst.finalize_options(self)
    ...
    def run(self):
        ...
        install.install_platlib = install.install_purelib = 'PURELIB'
        install.install_headers = 'HEADERS/Include/$dist_name'
    ...
    def create_exe (self, arcname, fullname, bitmap=None):
        ...
        file.write(base64.decodestring(EXEDATA))
        if bitmap:
    ...
    def get_inidata(self):
        ...
        for display, names in _metadata_names:
            if isinstance(names, TupleType):
                data = None
                for name in names:
