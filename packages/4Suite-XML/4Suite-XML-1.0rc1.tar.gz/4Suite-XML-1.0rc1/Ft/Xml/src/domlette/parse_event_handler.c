...
{
  Context *context = self->context;
  PyNodeObject **children = context->children;
  int newsize = context->children_size + 1;
                                 ...
                                 ExpatAttribute atts[], int atts_len)
{
  ...
  elem = Document_CreateElementNS(state->owner_document, 
...
static void builder_EndElement(void *userState, ExpatExpandedName *name)
{
  ...
  new_element = context->node;

  /* Set the element's children */
  if (_Node_SetChildren(new_element, context->children,
                        context->children_size) == -1) {
    Expat_ParserStop(state->parser);
    return;
  }
  ...
  if (ParserState_AddNode(state, new_element) == 0) {
    Expat_ParserStop(state->parser);
    /* just fall through */
  }
}
...
static void builder_CharacterData(void *userState, PyObject *data)
{
  ...
  new_text = (PyNodeObject *)
    Document_CreateTextNode(state->owner_document, data);

  /* ParserState_AddNode steals the reference to the new node */
  if (ParserState_AddNode(state, new_text) == 0) {
    Expat_ParserStop(state->parser);
    /* just fall through */
  }
}
                                          ...
                                          PyObject *data)
{
  ...
  new_pi =(PyNodeObject *)
    Document_CreateProcessingInstruction(state->owner_document, target, data);

  /* ParserState_AddNode steals the reference to the new node */
  if (ParserState_AddNode(state, new_pi) == 0) {
    Expat_ParserStop(state->parser);
    /* just fall through */
  }
}
...
static void builder_Comment(void *userState, PyObject *data)
{
  ...
  new_comment = (PyNodeObject *)
    Document_CreateComment(state->owner_document, data);

  /* ParserState_AddNode steals the reference to the new node */
  if (ParserState_AddNode(state, new_comment) == 0) {
    Expat_ParserStop(state->parser);
    /* just fall through */
  }
}
                                       ...
                                       PyObject *notationName)
{
  ...
  PyDict_SetItem(state->owner_document->unparsedEntities, name, systemId);
}
                               ...
                               int asEntity, PyObject *namespaces)
{
  ...
  if (asEntity) {
    rootNode = (PyNodeObject *) Document_CreateDocumentFragment(document);
    if (rootNode == NULL) {
      Py_DECREF(document);
      return NULL;
    }
