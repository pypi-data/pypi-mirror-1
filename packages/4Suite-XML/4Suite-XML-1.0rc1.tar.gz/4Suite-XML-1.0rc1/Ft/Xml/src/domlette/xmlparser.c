...
static ExpatStatus parser_Error(void *userData, PyObject *exception)
{
  ...
  if (handler != NULL) {
    ...
    PyObject *type = (PyObject *)((PyInstanceObject *) exception)->in_class;
    PyErr_SetObject(type, exception);
    Py_DECREF(exception);
    Expat_ParserStop(self->parser);
    return EXPAT_STATUS_ERROR;
  }
...
static PyMappingMethods attributes_as_mapping = {
  /* mp_length        */ (lenfunc) attributes_length,
...
static PyTypeObject Attributes_Type = {
  ...
  /* tp_clear          */ (lenfunc) attributes_clear,
  /* tp_richcompare    */ (richcmpfunc) 0,
  /* tp_weaklistoffset */ 0,
