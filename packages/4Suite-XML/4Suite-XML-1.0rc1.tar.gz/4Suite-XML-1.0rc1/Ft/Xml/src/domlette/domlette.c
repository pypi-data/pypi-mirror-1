/***********************************************************************
 * $Header: /var/local/cvsroot/4Suite/Ft/Xml/src/domlette/domlette.c,v 1.53 2006/08/13 22:07:04 uogbuji Exp $
 ***********************************************************************/
...
#define CHECK_DOCUMENT_REFS(num, where) if (doc->ob_refcnt != (num)) {  \
    PyErr_Format(PyExc_MemoryError, "%s expected %d ref count, found %d", \
...
static PyObject *PyTestTree(PyObject *self, PyObject *args)
{
  ...
  pi = Document_CreateProcessingInstruction(doc, target, data);
  Py_DECREF(data);
  Py_DECREF(target);
  ...
  text = Document_CreateTextNode(doc, data);
  Py_DECREF(data);
  ...
  localName = qualifiedName;
  element = Document_CreateElementNS(doc, namespaceURI, qualifiedName,
                                     localName);
  Py_DECREF(qualifiedName);
  ...
  comment = Document_CreateComment(doc, data);
  Py_DECREF(data);
...
static PyObject *PyTestRefCounts(PyObject *self, PyObject *args)
{
  ...
  if (!PyArg_ParseTuple(args, "OO!:TestRefCounts", &tester, 
                        &DomletteDocument_Type, &document))
    return NULL;

  if (test_refcounts(tester, document) == 0) {
    return NULL;
  }
