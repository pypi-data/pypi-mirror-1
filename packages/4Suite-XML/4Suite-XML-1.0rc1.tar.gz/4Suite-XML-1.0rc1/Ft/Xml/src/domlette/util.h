...
#endif
#define PyType_TRAVERSE_BASE(op)                           \
  do {                                                     \
    PyTypeObject *base = (op)->ob_type->tp_base;           \
    if (base && PyType_IS_GC(base) && base->tp_traverse) { \
      int vret = base->tp_traverse((op), visit, arg);      \
      if (vret) return vret;                               \
    }                                                      \
  } while (0)
...
#endif
#define PyType_CLEAR_BASE(op)                              \
  do {                                                     \
    PyTypeObject *base = (op)->ob_type->tp_base;           \
    if (base && PyType_IS_GC(base) && base->tp_clear) {    \
      int vret = base->tp_clear((op));                     \
      if (vret) return vret;                               \
    }                                                      \
  } while (0)
