                        ...
                        long flags)
{
  ...
  if (node != NULL) {
    ...
    node->parentNode = (PyNodeObject *) Py_None;
  ...
  if (self == NULL) {
    ...
    if (Node_HasFlag(self, Node_FLAGS_SHARED_ATTRIBUTES)) {
      if (add_sep)
        ...
        add_sep++;
      fprintf(stderr, " Node_FLAGS_SHARED_NAMEDNODEMAP");
    }
    if (!add_sep) {
      fprintf(stderr, " (none)");
    }
...
static PyObject *node_cloneNode(PyNodeObject *self, PyObject *args)
{
  ...
  result = Node_CloneNode((PyObject *) self, deep, 
                          (PyNodeObject *) Node_GET_DOCUMENT(self));
...
static int node_clear(PyNodeObject *self)
{
#ifdef DEBUG_NODE_CREATION
  printf("Clearing %s node at 0x%p\n", self->ob_type->tp_name, self);
#endif

  Py_CLEAR(self->ownerDocument);
  if (self->flags & Node_FLAGS_CONTAINER) {
...
static PyObject *node_richcompare(PyNodeObject *a, PyNodeObject *b, int op)
{
  ...
  doc_a = PyDocument_Check(a) ? (PyObject *) a : PyNode_OWNER_DOCUMENT(a);
  doc_b = PyDocument_Check(b) ? (PyObject *) b : PyNode_OWNER_DOCUMENT(b);
  if (doc_a != doc_b) {
...
static int nodeiter_length(NodeIterObject *iter)
{
  int len;
  if (iter->node) {
    len = ContainerNode_GET_COUNT(iter->node) - iter->index;
    if (len >= 0)
      return len;
  }
  return 0;
...
static PySequenceMethods nodeiter_as_sequence = {
  /* sq_length */ (lenfunc) nodeiter_length,
};
