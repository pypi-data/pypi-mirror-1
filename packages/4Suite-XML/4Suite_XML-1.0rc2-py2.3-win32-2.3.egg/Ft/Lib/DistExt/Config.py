...
class Config(Command):
    ...
    user_options.extend([
        ...
        ('plat-name=', 'p',
         ...
         " (default: %s)" % Util.GetPlatformSpecifier()),
        ])
    ...
    def finalize_options(self):
        ...
        if self.plat_name is None:
            self.plat_name = Util.GetPlatformSpecifier()
        ...
        changed = False
        if not (parser.has_section(self.plat_name) and
            ...
            if not parser.has_section(self.plat_name):
                parser.add_section(self.plat_name)

            # string values
            for key in ('prefix', 'exec_prefix', 'compiler'):
                value = getattr(self, key) or ''
                parser.set(self.plat_name, key, value)
        ...
        # string values
        for key in ('prefix', 'exec_prefix', 'compiler'):
            if parser.has_option(self.plat_name, key):
                value = parser.get(self.plat_name, key) or None
        ...
        for key in ('debug',):
            if parser.has_option(self.plat_name, key):
                value = eval(parser.get(self.plat_name, key))
                setattr(self, key, value)
            else:
                value = str(getattr(self, key))
                parser.set(self.plat_name, key, value)
        ...
        if self.cache_filename and changed:
            if DEBUG:
                ...
                print '    [%s]' % self.plat_name
                for name in CONFIG_KEYS:
                    value = parser.get(self.plat_name, name)
    ...
    def write_config_vars(self, filename):
        """
        Write the configuration variables to the given filename.
        """
        # Create the configuration filename
        self.announce('writing %s' % filename, 2)
    ...
    def write_config_file(self, file):
        ...
        """
        file.write('# Configuration variables\n')
        maxlen = max(map(len, EXPORTED_CONFIG_KEYS))
        for name in EXPORTED_CONFIG_KEYS:
            exported_name = name.upper()
            value = self.config_vars[name]
            file.write('%-*s = %r\n' % (maxlen, exported_name, value))
        return
