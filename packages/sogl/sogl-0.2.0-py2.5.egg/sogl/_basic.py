...
#
# Created:      26-10-2006
# SVN-ID:       $Id: _basic.py 1875 2006-12-15 14:30:52Z obi $
...
class Shape(ShapeEvtHandler):
    ...
    def AddSensitivityFilter(self, op, recursive = False):
        ...
        if recursive:
            for shape in self._children:
                shape.AddSensitivityFilter(op, True)

    def RemoveSensitivityFilter(self, op, recursive = False):
        self._sensitivity &= ~op
        if recursive:
            for shape in self._children:
                shape.RemoveSensitivityFilter(op, True)
...
##                shape.SetDraggable(drag, True)
    ...
    def OnLeftClick(self, x, y, keys = 0, attachment = 0):
        if not self.IsSensitiveTo(OP_CLICK_LEFT):
            if self._parent:
    ...
    def OnDragRight(self, draw, x, y, keys = 0, attachment = 0):
        if not self.IsSensitiveTo(OP_DRAG_RIGHT):
            if self._parent:
                attachment, dist = self._parent.HitTest(x, y)
