...
pointMatrix = NurbsMatrix.Matrix_Point3Dd(maxPoints,maxPoints)
pointMatrixNew = NurbsMatrix.Matrix_Point3Dd(maxPoints,maxPoints)
matrix = NurbsMatrix.Matrix_Point3Dd(1,1)

for j in range(11):
	for k in range(11):
		matrix.reset(NurbsPoint.Point3Dd(coordinates[0][j][k],coordinates[1][j][k],coordinates[2][j][k]))
		pointMatrix.as_func(j,k,matrix)
		pointMatrixNew.setelem(j,k,NurbsPoint.Point3Dd(coordinates[0][j][k],coordinates[1][j][k],coordinates[2][j][k]))


pointMatrix.write("Test.txt")
pointMatrix.writeRaw("TestRaw.txt")

surface = NurbsSurface.NurbsSurfaced()
surface.globalInterp(pointMatrix,degree,degree)

surface.writePS("test.ps",1,1,NurbsPoint.Point3Dd(50.0,50.0,0.0),NurbsPoint.Point3Dd(0,0,0))

red = NurbsColor.Color(255, 0, 0)
...
surface2 = NurbsSurface.NurbsSurfaced()
surface2.globalInterp(pointMatrixNew,degree,degree)

knotsU = surface2.knotU()
knotsV = surface2.knotV()

for i in range(knotsU.size()):
	print knotsU[i]

for i in range(knotsV.size()):
	print knotsV[i]

uWert = NurbsCurve.doublep()
vWert = NurbsCurve.doublep()

minDistance = surface2.minDist2(NurbsPoint.Point3Dd(50.0,50.0,0.0),uWert,vWert)
print uWert.value(), vWert.value()

print surface2.findSpanU(0.2), surface2.findSpanV(0.2)

print "\nYou can visualise the 97er VRML objects with ParaView, openvrml or freewrl"
