...
__all__ = [
    'MPManager', 'tree_recursive_iterator',
...
def _iter_current_next(sequence):
    ...
    while current_item != _nonexistent:
        next_item = next(iterator, _nonexistent)
...
class DeclarativeMeta(BaseDeclarativeMeta):
    ...
    def __init__(cls, name, bases, dct):
        ...
        for opt, default in [
            ...
            opts[opt] = getattr(cls, '__mp_%s__' % opt, default)
        for field, ftype in [
            ...
            if not hasattr(cls, opts[field]):
                dct[opts[field]] = sqlalchemy.Column(ftype(), nullable=False)
        super(DeclarativeMeta, cls).__init__(name, bases, dct)
        ...
        cls.__mapper__.extension.append(mp_class_manager.mapper_extension)
