...
class MPClassManager(object):
    ...
    def rebuild_all_trees(self, order_by=None):
        ...
        for index in opts.indices:
            index.drop()
        # resetting the path for roots:
        opts.table.update().where(opts.parent_id_field == None) \
                           .values({opts.path_field: ''}) \
                           .execute()
    ...
    def query_all_trees(self, session):
        ...
        :returns:
            `Query` object whith all nodes of all trees sorted as usual
            by `(tree_id, path)`.
        """
