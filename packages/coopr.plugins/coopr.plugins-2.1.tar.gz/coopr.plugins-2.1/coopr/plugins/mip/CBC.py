...
#        self._valid_problem_formats=[ProblemFormat.nl, ProblemFormat.cpxlp, ProblemFormat.mps]
    ...
    def executable(self):
        ...
        if executable is None:
            pyutilib.plugin.core.PluginGlobals.env().log.error("Could not locate the 'cbc' executable, which is required for solver %s" % self.name)
            self.enable = False
            return None
    ...
    def process_soln_file(self,results):
        ...
        for line in INPUT:
          ...
          if processing_constraints is True:
             constraint = tokens[1]
