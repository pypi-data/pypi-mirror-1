...
'use_hyphens' has been added to the __init__ method of the TextWrapper class which
...
With Python 2.4 and 2.5, PyHyphen should work out of the box. There are binaries
for win32. On other platforms you will need a build environment such as gcc, make
and so on. PyHyphen is likely to work also with Python 2.3, but this has not been
tested.

3.2 Compiling and installing from source

Download the source distribution (eather a zip archive or a tarball) from
...
of hnjmodule for your platform. If there is a binary that looks ok, this version is installed. Otherwise,
hnjmodule is compiled from source. On Windows you will need MSVC 2003
or whatever fits to your Python version.
If the distribution comes with a binary of 'hnjmodule'
...
the documentation of hyphen-2.3.1 is missing.
...
Distributions of PyHyphen are available at the Python Package Index at
