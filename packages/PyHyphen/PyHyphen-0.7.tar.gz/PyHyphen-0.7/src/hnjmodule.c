...
/* mode:
  ...
  bit2 == 1: entire word must be uppered before returning  */
    ...
    if (hnj_hyphen_hyphenate2(self->dict, word_str, wd_size, hyphens,
...
HyDict_init(HyDictobject *self, PyObject *args) {
    char* fn;
    int fn_size;
    if (!PyArg_ParseTuple(args, "s#", &fn, &fn_size))
  		return -1;
...
static struct PyMethodDef hnjmodule_methods[] = {
...
static char hnjmodule_module_documentation[] =
...
if (PyType_Ready(&HyDict_type) < 0)
	...
	m = Py_InitModule3("hnjmodule", hnjmodule_methods,
		hnjmodule_module_documentation);
  if (m == NULL)
    return;

	/* Add some symbolic constants to the module */
	d = PyModule_GetDict(m);
	ErrorObject = PyString_FromString("hnjmodule.error");
	...
	/* Check for errors */
	if (PyErr_Occurred())
		Py_FatalError("cannot initialize module hnjmodule");
}
