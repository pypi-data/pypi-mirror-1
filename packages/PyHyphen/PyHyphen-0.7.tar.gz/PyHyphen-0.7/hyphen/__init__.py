...
1.1 The hyphen package contains:
    ...
    - 'hnjmodule' is the C extension module that does all the ground work. It
      contains the C library hyphen-2.3.1 (or higher). I cannot think of a reason
...
the C extension module 'hnjmodule' used by the hyphenator class defined in
__init__.py contains the C library hyphen-2.3.1 which is used by OpenOffice.org, Mozilla
...
For further information on the hyphenation library and available dictionaris visit
...
import hnjmodule, config

__all__ = ['dictools', 'config']
...
class hyphenator:
    ...
    'hyphen-2.3-1'.
    '''
    def __init__(self, language = 'en_US', directory = config.default_dic_path):
        '''
        ...
        self.hyph = hnjmodule.hyphenator_(file_path)
    ...
    def inserted(self, word):
        '''
        Hyphenate 'word' and return a unicode object of the form u'hy=phen=ation'.
        Return 'word' if len(word) < 4 or word could not be hyphenated (= insert 0 '=')

        Note that when used with languages with non-standard hyphenation, 'inserted
        may return a string with replacements at more than one position. This is
        normally not desired in text processing environments. Consider using 'pairs'
        instead.
        '''
        if isinstance(word, str): word = word.decode("utf-8", "strict")
        mode = 0
        if (len(word) < 4) or ('=' in word): return word
        if not word.islower():
            if (word.isupper()):
                mode += 4
                word = word.lower()
        ...
        # Now call the hyphenator
        return self.hyph.apply(word, mode)


    def pairs(self, word):
        '''
        Hyphenate 'word' and return a list of lists of the form [[u'hy', u'phenation'], [u'hyphen', u'ation']].

        Return [], if len(word) < 4 or if word could not be hyphenated.
        '''
        if isinstance(word, str): word = word.decode("utf-8", "strict")
        mode = 1
