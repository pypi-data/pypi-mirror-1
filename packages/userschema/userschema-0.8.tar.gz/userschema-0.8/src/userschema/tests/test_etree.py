...
class FieldHandlerTests(unittest.TestCase):
    ...
    def test_ChoiceHandler(self):
        ...
        self.assertEqual(
            [term.value for term in field.vocabulary], options)
    ...
    def test_ChoiceSetHandlerNonASCII(self):
        ...
        node = XML('''<ChoiceSet name="test">
            ...
            <required>True</required>
            <readonly>False</readonly>
            <option>copyright sign (\xc2\xa9)</option>
            </ChoiceSet>''')
        field = ChoiceSetHandler(node)
        # Mind you: by the the vocabulary is created, the strings
        # have been converted to unicode. The unicode escape sequence
        # for the copyright sign is \xa9.
        tokens = [str(hash(u'copyright sign (\xa9)'))]
        self.assertEqual(
            [term.token for term in field.vocabulary], tokens)
