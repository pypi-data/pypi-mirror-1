...
def group_alternate_file (file_chooser_config_hook=None):
    ...
    chooser = gtk.FileChooserButton("Select a File") # 2.6
    on_box.pack_start(chooser, True, True)

    def set_alternate_file (value):
        chooser.set_filename(os.path.expanduser(value))

    alt_file_item = ConfigItem(on_box, 'alternate_file',
                               chooser.get_filename, set_alternate_file)
    alt_file_item.connect('file-activated', chooser)
...
def on_daemon_found (locator, sender):
    _gui_data['daemon-apply-button'].set_sensitive(True)

def on_daemon_lost (locator):
    _gui_data['daemon-apply-button'].set_sensitive(False)

def start_ipc_client ():
    return ipc.create_client(on_daemon_found, on_daemon_lost)


def on_win_delete (win, *args):
    quit_gui(win)
...
def create_main_window (title, border_width=5, quit_on_delete=True,
    ...
    if quit_on_delete:
        _gui_data['main-handler'] = win.connect("delete-event", on_win_delete)
