...
# TODO:  automatically handle encoding/decoding of unicode strings?
# TODO:  is_filelike() and to_filelike() functions
...
def is_filelike(obj,mode="rw"):
    ...
    """
    if mode == "" or not isinstance(mode,basestring):
        raise ValueError("Invalid mode spec: %s" % (mode,))
    for c in mode:
        if c not in ("r","w","a","+"):
            raise ValueError("Invalid mode spec: %s" % (mode,))    
...
##  TODO:  maybe move these into a sub-package, filelike.wrappers ?
