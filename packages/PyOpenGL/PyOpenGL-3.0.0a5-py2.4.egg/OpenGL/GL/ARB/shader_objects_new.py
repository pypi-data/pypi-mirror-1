...
def glInitShaderObjectsARB():
	...
	finalisewrappers.finalise( globals() )
	return extensions.hasGLExtension( 'GL_ARB_shader_objects' )
### END AUTOGENERATED SECTION
GL_INFO_LOG_LENGTH_ARB = constant.Constant( 'GL_INFO_LOG_LENGTH', 0x8B84 )
...
def glShaderSourceARB( shader, source ):
	...
	if not isinstance( source, str ):
		source = '\n'.join( source )
	# simple case...
	data = ctypes.c_char_p( source )
	arraySize = len(source)
	length = ctypes.c_int(arraySize)
	return base_glShaderSourceARB(
		shader, 1, 
		ctypes.cast( ctypes.POINTER( constants.GLcharARB ), data,  ),
		ctypes.byref( length) 
	)
...
def glGetObjectParameterivARB( shader, pname ):
	...
	status = constants.GLint( -1 )
	glGetObjectParameterivARB(
		shader, GL_OBJECT_COMPILE_STATUS_ARB,ctypes.byref(status)
	)
	return status.value
base_glGetObjectParameterivARB = glGetObjectParameterivARB
def glGetObjectParameterfvARB( shader, pname ):
	"""Retrieve the float parameter for the given shader"""
	status = constants.GLfloat( -1.0 )
	glGetObjectParameterivARB(
		shader, GL_OBJECT_COMPILE_STATUS_ARB,ctypes.byref(status)
	)
	return status.value
base_glCompileShaderARB = glCompileShaderARB
def glCompileShaderARB( shader ):
	"""Compile the current source-code for the given shader"""
	base_glCompileShaderARB(shader)
	status = glGetObjectParameterivARB(
		shader, GL_OBJECT_COMPILE_STATUS_ARB
	)
	if (not status):
		from OpenGL import error
		raise error.GLError( description= glGetInfoLogARB( shader ))
	return status
...
def glGetInfoLogARB( obj ):
	...
	"""
	length = glGetObjectParameterivARB(obj, GL_INFO_LOG_LENGTH)
	if length > 0:
		log = ctypes.create_string_buffer(length.value)
		glGetInfoLogARB(obj, length, None, log)
		return log.value[:-1] # strip null termination
	return ''
