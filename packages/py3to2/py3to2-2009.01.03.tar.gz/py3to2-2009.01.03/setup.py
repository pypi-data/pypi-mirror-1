...
except: missing_dependency("Python-2.6.1", "http://www.python.org/download")
try: system("python3.0 -V")
except: missing_dependency("Python-3.0", "http://www.python.org/download/releases/3.0/")
if sys.version_info[0] is not 2 or sys.version_info[1] < 6: raise Exception("setup needs to b run w/ Python-2.6.1")
...
class dev(core.Command):
  ...
  user_options = [
    ("diff", None, "create 'py3to2.diff' patch against Python-2.6.1 src"),
    ("doc", None, "print doc"),
    ("maketest", None, "make test - run regression tests"),
    ("pkginfo", None, "create pkg-info"),
    ("py2to3test", None, "test importing python2.6.1 standard library generated from 2to3"),
  ...
  def run(self):
    ...
    if self.diff: diff()
      ...
      CWD = os.getcwd(); os.chdir("Python-2.6.1")
      ...
      if not os.path.exists("py3k_Lib"): system("cp -a Python-2.6.1/Lib py3k_Lib")
...
class Distribution(dist.Distribution):
  ...
  def run_command(self, command):
    ...
    if command == "build":
      ...
      if not os.path.lexists("py3to2"): system("ln -s Python-2.6.1/py3to2 .") ## create link to built binary
      if not os.path.exists("Python-2.6.1"):
        try: system("tar -xzvf Python-2.6.1.tgz") ## untar src
        except: raise Exception("""\n
py3to2 requires Python-2.6.1 src files in order to b built.
please first download src Python-2.6.1.tgz from:

  http://www.python.org/ftp/python/2.6.1/Python-2.6.1.tgz

into CURRENT directory.  then run setup again
""")
      self.patch("Python-2.6.1") ## patch src
