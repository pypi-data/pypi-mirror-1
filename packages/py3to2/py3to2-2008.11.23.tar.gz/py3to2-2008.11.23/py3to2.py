...
def uniquestring(s, pre = "q", suf = ""): # create a unique alphanumeric keyword in s
  # keyword = hex(id(s))[2:]
  # while keyword in s: keyword += hex(hash(keyword))[2:]
  keyword = str(id(s))
  ...
  if ISPY3K:
    ...
    def op3to2(self):
      ...
      for i, x in enumerate(s): # map opcodes from 3k to 2x
        ...
        # if x == 122:
          # pass
          # arg = s[i + 1] + (s[i + 2] << 8) + 3; assert arg < 65536
          # s[i + 1] = arg & 7; s[i + 2] = arg >> 8
...
if not ISPY3K:
  ...
  # s = open("py3k_Lib/pickletools.py").read()
  # s = compiler().preparse(s)
  # print( s )
  ...
  class _py2to3(object):
    ...
    # skip = "BaseHTTPServer mimetools".split(" ") # add to here anything that segfaults
    skip = ()

    def test_stdlib(self, dirpath, stdlib = stdlib, skip = skip):
