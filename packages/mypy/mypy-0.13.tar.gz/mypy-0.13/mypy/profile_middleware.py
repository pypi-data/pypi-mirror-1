    ...
    def _application(request):
        ...
        if profile or cookie_profile:
            if profile=="0":
                ...
                if result is not None and result.startswith("<"):#disable output for json
                    stats = load_stats()
                    ...
                    splitor = "\n"+"=*"*60+"\n\n\n"
                    so = escape(splitor.join(so))
                    so = """<pre style="clear:both;padding:1em;background:#ffffde;color:#033">%s</pre>"""%so
