...
class _img2txt(object):
  ...
  clrmap = clrmap = clrmap.split("  ") ..ravel()

  if 1 and DEBUG:
    print( clrmap[256:768] ..strjoin() )
    # for x in clrmap.reshape(216, -1)[:4]: print( x[self.ford] ..strjoin() )
  ...
  def __init__(self, plaintxt = True):
    ...
    # fpop[-1] = fres.prod() # popcount of fbmp

    if not plaintxt: # include inverted color
      fbmp = (fbmp, fbmp ^ True) ..concatenate(axis = 0)
      ford = r_[ford, ford + 0x80]; fpop = r_[fpop, fres.prod() - fpop]
    ...
    fbmp, ford, fpop = (x[cll] for x in (fbmp, ford, fpop))
    self.level = level = fpop.mean() * 0xff / fres.prod()
    ...
    chrmap = [chr(x) for x in range(0x80)] # chrmap
    if not plaintxt: chrmap += ["\33[7m%s\33[0m" % x for x in chrmap] # include inverted color
    self.chrmap = chrmap = asarray(chrmap)
    if 1 and DEBUG: print( chrmap[ford] ..strjoin() ) # print chrmap in ascending density

    self.fbmp = fbmp; self.ford = ford; self.fpop = fpop; self.fbin = fbin; self.fmap = fmap

  for x in "__len__ reshape".split(" "): # add arr-like methods from self.buf
    exec("def %s(self, *args): return self.buf.%s(*args)" % (x, x))
  ...
  def fill(self, arr): # fill buf w/ data
    ...
    for i, x in enumerate(arr != 0): buf |= int64(x) << i # fill buf
    return self

  def itp(self, whitespace = True, wmatch = 2, wmismatch = 1): # interpolate block -> ascii chr
  ...
  def test(self):
    import img2txt_mario; reload(img2txt_mario)
    img = img2txt_mario.img() # simulated image file "mario.jpg"
    txt = self(
      img,
      scale = 0.3, # scale image size (0.5 = half, 1 = original, 2 = double)
...
class dataNFT(ndarray):
  def __new__(self, ft = None, f = None, t = None):
    ...
    self = _asarray.__new__(self, ft.copy())
...
################################################################
# this is all old code which will eventually b deprecated
# kept for the sake of the 3d plotter

######## img2txt
class _img2txt_old(object):
  debug = 0
  ...
  def __init__(self):
    ...
    hst = _histogram(fpop); hst = [x for x in enumerate(hst) if x[1]] # cll blanks in histogram
    ...
    for x in dir(ndarray): # add arr-like methods - from self.buf
      if not hasattr(_img2txt_old, x) and getattr(ndarray, x) ..callable() and "set" not in x: setattr(_img2txt_old, x, method(x)) # disallow setters - have bugs
      # if not hasattr(_img2txt_old, x) and getattr(ndarray, x) ..callable(): setattr(_img2txt_old, x, method(x))
  ...
  def load(self, file, scale = 1, color = True): # must be a file or filename
  ...
  def test():
    ...
    if 1:
      ...
      scale = 0.5
      color = 1
      arr = self.load(img, color = color, scale = scale)
      print( self.tostr() )
    self.debug = None

img2txtold = _img2txt_old()

class _tplot3d(_img2txt_old):

  def __init__(self):
    _img2txt_old.__init__(self)
    i = self.fres.prod() // 2
