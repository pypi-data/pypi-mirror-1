...
<body>
    <div style="width: 40%">
    <p>This form will simulate a validation error so you can see how the image is 
    stored on the server side cache (on a secure temp file) allowing the user
    to resubmit without resending the image (only an id on a hidden field).</p>
    <p>The image links (thumbnail and full size) only work per session, that 
    means that the user can link it to other person and they won't be able to 
    open them</p>
