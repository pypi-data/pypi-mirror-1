...
from storage_exceptions import ObjectNotFoundException, ObjectAlreadyExistsException, NotAPairtreeStoreException
...
class FileStorageObject(object):
    ...
    def add_part(self, partid, bytestream, buffer_size):
        return self.fs._put_part(self.id, partid, bytestream, buffer_size)

    def get_part(self, partid, stream=False):
        return self.fs._get_part(self.id, partid, stream)
...
class FileStorageClient(object):
    ...
    def _id_to_dirpath(self, id):
        ...
        while id:
            dirpath = os.path.join(dirpath, id[:self.shorty_length])
            id = id[self.shorty_length:]
        return dirpath
...
#            leaf = False
#            for filename in [x for x in filenames if not x.startswith('pairtree')]:
#                if len(filename) > self.shorty_length:
#                    leaf = True
#            if leaf:
...
#                self.id_parts[this_id] = dirnames
    ...
    def _create(self, id):
        ...
        if not os.path.exists(dirpath):
            os.makedirs(dirpath)
            os.mkdir(os.path.join(dirpath, u'ROOT'))
            os.mkdir(os.path.join(dirpath, u'MANIFEST'))
    ...
    def _put_part(self, id, part_id, bytestream, buffer_size):
        dirpath = os.path.join(self._id_to_dirpath(id), "%s" % (part_id))
        if not os.path.exists(dirpath):
            os.mkdir(dirpath)
        ...
        try:
            ...
            if hasattr(bytestream, 'read'):
                if not buffer_size or buffer_size<1024:
                    buffer_size = 1024 * 8
    ...
    def _get_part(self, id, part_id, stream):
        dirpath = os.path.join(self._id_to_dirpath(id), "%s" % (part_id))
        if not os.path.exists(dirpath):
