...
class FileStorageClient(object):
    ...
    def list_ids(self):
        ...
        for dirpath, dirnames, filenames in os.walk(self.store_dir):
            leaf = False
            dir_contents = dirnames + filenames
            for name in [x for x in dir_contents if not x.startswith('pairtree')]:
                if len(name) > self.shorty_length:
                    leaf = True
            if leaf:
                objects.append(self._get_id_from_dirpath(dirpath))
                # # Much more detailed info than was needed
