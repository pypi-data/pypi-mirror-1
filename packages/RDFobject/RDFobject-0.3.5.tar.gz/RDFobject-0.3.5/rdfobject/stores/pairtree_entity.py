...
class FileStorageObject(object):
    ...
    def add_part(self, partid, bytestream, version=None):
        return self.fs._put_part(self.id, partid, bytestream, version)
...
class FileStorageClient(object):
    ...
    def _put_part(self, id, part_id, bytestream, version=False, buffer_size = 1024 * 8):
        ...
        if self.queue != None:
            if version == 1:
                self._log(id, 'c', 'Creating a part', part_id=part_id, version=version, checksum=hexhash)
            else:
                self._log(id, 'w', 'Updating a part', part_id=part_id, version=version, checksum=hexhash)
