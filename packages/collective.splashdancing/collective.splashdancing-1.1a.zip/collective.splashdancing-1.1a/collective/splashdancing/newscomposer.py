...
def add_related(item,html):
    r"""Adds related items to items sent as newsletters"""
    elements = [(item.Title(),item.Description(),item.absolute_url()) for item in item.getRelatedItems()]
    #combine the related items
    all_related_items = ""
    relatedItems = ""
    if len(elements) > 0:
        for element in elements:
            related_item = """<div class="related-item">
                              <a href="%(url)s">
                              <h3>%(title)s</h3>
                              </a>
                              <p>%(description)s</p>
                              <a href="%(url)s">read more ...</a>
                           </div> """ % dict(title = element[0],description = element[1], url = element[2])
                           
            all_related_items = all_related_items + related_item

        relatedItems = """<div id="relatedItems"><h2>Related Items</h2>%s</div>""" % all_related_items
    soup = BeautifulSoup(html)
    target_tag = soup.find('span',attrs={'id':'contentBottomLeft'})
    target_tag.replaceWith(relatedItems) 
...
def plone_html_strip(html, not_likey=plone_html_strip_not_likey):
    ...
    output = content.renderContents(encoding=None) # encoding=None results in  unicode
...
class SplashDancingNewsItemFormatter(object):
    """Custom Formatter for News Items and derivatives of News Items
    ...
    def __call__(self):
        ...
        if 'kss' in html:
            return add_related(self.item,plone_html_strip(html))
        else:
            return html
