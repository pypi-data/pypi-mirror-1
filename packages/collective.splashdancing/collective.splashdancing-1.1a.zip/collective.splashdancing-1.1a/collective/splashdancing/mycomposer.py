...
class PloneCallHTMLFormatter(object):
    ...
    @property
    def settings(self):
        return  IHTMLClassIDStripper(self.site.portal_newsletters) 

    @property
    def stripping_rules(self):
        """ return the stripping_rules in a format suitable for BeautifulSoup"""
        _stripping_rules =  self.settings['stripping_rules'].split('\r\n') # review this approach XXX
        print _stripping_rules
        return  [dict([ tuple(item.split('|')) ]) for item in _stripping_rules]
    ...
    def __call__(self):
        ...
        if 'kss' in html:
            if self.settings['enabled']: 
                return plone_html_strip(html,not_likey=self.stripping_rules)
            else:
