...
class world:
    ...
    def render(self, filename, limit):
        ...
        y = 0
        while y < self.height or nproc > 0:
            if nproc < limit:
                channel = pprocess.start(self.render_row, y)
            ...
            for channel in exchange.ready():
                nproc -= 1
                sy, row = channel.receive()
                sx = 0
                for col in row:
                    draw.point((sx,sy),fill=(col.x*255,col.y*255,col.z*255))
                    #depth=depth/self.backplane
                    #draw.point((sx,sy),fill=(depth*255,depth*255,depth*255))
                    count=count+1
                    sx += 1
