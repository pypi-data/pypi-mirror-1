...
def test_04_authorization():
    ...
    assert admin1 in [i.user for i in mod_history[1].approves]
    assert admin2 not in [i.user for i in mod_history[1].approves]
...
def test_07_multiple_authorization():
    ...
    assert admin1 in [i.user for i in mod_history[1].approves]
    assert admin2 in [i.user for i in mod_history[1].approves]
    assert admin3 in [i.user for i in mod_history[1].approves]
    assert len(mod_history[1].rejects) == 0
...
def test_20_pending_creation_authorization_test():
    ...
    assert mod_history[0].user == user
...
def test_23_modify_after_creation():
    """
    Pending modfications after a pending creations are identified as creations
    """
    set_user(user)
