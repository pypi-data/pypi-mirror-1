...
class SOOSTBase:
    ...
    def get_by_revision(self, name, revision, closest=True,
                        default=NoDefault):
        """
        ...
        @param closest: When True it will behave like SVN, if you ask for the
                        value in revision 10 but the value wasn't set in that
                        revision, but on a revision earlier, it will return
                        that value. When set to False it strictly checks if the
                        attribute was set in the specified revision.
        ...
        s = OST.select(AND(OST.q.model_name == self.__class__.__name__,
                           OST.q.instance_id == self.id,
                           OST.q.pending == False,
                           OST.q.rejected == False,
                           closest and OST.q.id <= revision or 
                                       OST.q.id == revision,
                           TraceData.q.name == name),
                           join=join)
        id = s.max('ost_main.id')
        try:
            if not id:
                # SQLObject raises an AssertionError if .get() receives a None.
