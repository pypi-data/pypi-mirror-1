...
from locale import gettext_ost
...
def report_error(func, *args, **kw):
    try:
        ...
        log.error('Error while calling %s. Exception: %s. Exception Args: %s.'\
                  ' Auth Args: %s, %s' \
                    %  (func.__name__, sys.exc_info()[0], sys.exc_info()[1],
                        args[1:], kw))
        rollback_all()
...
class HistoryController(Controller):
    ...
    def state_detail(self, state_id=None):
        ...
        if state.user != identity.current.user and not state.can_authorize():
