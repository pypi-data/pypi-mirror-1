...
def create_polygon(obj): # Must take a polygon object
    ...
    height = (max(y_values) - min(y_values)) + obj.thickness
    width = (max(x_values) - min(x_values)) + obj.thickness
    dx = min(x_values)  
    dy = min(y_values)
    relative_points = []
    for point in obj.points:
        relative_points.append((point[0] - dx, point[1] - dy))
...
def create_arc(obj):
    check_graphics()
    sprite_create(obj, radius*2, radius*2)
    sprite_move(obj, (obj.center.x - radius, obj.center.y - obj.radius))
    if obj.filled == True:
        obj.thickness = obj.radius

    pygame.draw.arc(obj.sprite.image, obj.color, [0, 0, obj.width, obj.height], 
                     math.radians(obj.start_angle), math.radians(obj.end_angle), obj.radius)
    update()
...
def wait():
    ...
    while wait:
        event = check_events()
        if event.type == KEYDOWN and event.unicode == u"\r":
            wait = False
