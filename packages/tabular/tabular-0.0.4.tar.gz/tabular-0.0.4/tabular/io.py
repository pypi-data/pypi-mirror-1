...
def loadSV(fname, comments=None, delimiter=None, delimiter_regex=None, linebreak=None, skiprows=0, usecols=None, toload=None, metadatadict=None, namesinheader=True, headerlines=None, valuefixer=None, linefixer=None, returnasrecords=False):
	"""
	...
	**Parameters**
		...
		**comments** :  string, optional
			...
			that if not supplied and the first line of the input file begins
			with a '#', then it is assumed to be '#'.  Lines at the top of 
			the file beginning with `comments` are assumed to be the 
			header of the file where metadata can be found (e.g. 
			column names).  
	...
	**See Also:**  
	...
	if comments is None:
		line = delimiter.join(list(csv.reader([F[0]], delimiter=delimiter))[0])
		if line[0] == '#':
			comments = '#'
		else:
			comments = ''
...
def saveSV(fname, X, comments=None, delimiter=None, linebreak=None, metadata=None, printmetadict=None, strongmetadata=False):
	"""
	...
	**Parameters**
		...
		**metadata** :  list of strings or Boolean, optional
			...
			*	If `metadata` is not specified, then defaults to ['names'].

			*	If `True`, this is the same as `metadata = ['coloring', 'types', 'names']`. 
		...
		**strongmetadata** :  Boolean, optional
			...
			`metadatadict` if `printmetadict == True`. 
	...
	**See Also:**  
	...
	if 'coloring' in metakeys:
		if strongmetadata is False and X.coloring == {}:
			metakeys = [key for key in metakeys if key != 'coloring']
		else:
	...
	if printmetadict is True:
		metadatadict = dict( zip( metakeys, range(1, 1+len(metadata.keys())) ) )
