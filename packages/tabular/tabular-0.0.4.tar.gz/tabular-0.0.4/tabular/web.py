...
def tabular2html(fname, X=None, fin=None, title=None, printheader=False, split = True, usecss=None, writecss=None, SERVERNAME=None, SERVER_FROM_CURDIR='../', ROWS_PER_PAGE=1000, **kwargs):
	"""
	...
	**Parameters**
	...
	#if i specify usecss and its not false, it must be a string and I want to put that file name in the link and not write anything out
	#if i specify writecss i want it to write out file to that name and use it in he link
	...
	if usecss != None:
		if isinstance(usecss,str):
			cssfile = usecss
			CSSLINK = '<link rel="stylesheet" type="text/css" href="' + '/'  + cssfile[len(SERVER_FROM_CURDIR):] + '"</link>'
		else:
			assert usecss == False
			CSSLINK = ''
	else:
	...
	for section in range( numSections ):   #for each section
		...
		with open( sectionfname, 'w' ) as f:     #open the output file for the section to fileobject 'f' and do: 
			...
			f.write( '<html><META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=utf-8" /><head><title>' + title + '</title>' + CSSLINK + '</head><body>\n' )   
			if printheader:
