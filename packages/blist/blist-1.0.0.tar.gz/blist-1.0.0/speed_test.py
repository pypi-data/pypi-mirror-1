...
CFLAGS='-c -DNDEBUG -g -O3 -Wall -Wstrict-prototypes -I/usr/local/include/python3.0 --std=gnu99 -Werror'
CC = 'gcc'
PYTHON='python3.0'
LD = CC
LDFLAGS='-g -shared'
LOADLIBES='-lpython3.0'
TIMEIT='/usr/local/lib/python3.0/timeit.py'
...
def timeit(python, stmt, setup, rep):
    assert rep >= MIN_REPS
    key = (python, stmt, setup, rep, current_limit)
...
""" % (d, label, safe_label)
    ...
    for limit in limits:
        print >>f, ('    "dat/%d-%s.dat" using 1:(%f*$%d):(%f*$%d):(%f*$%d) title "blist()" with yerrorlines '
                    % (limit, label, m, k+2, m, k+3, m, k+4))
    print >>f
    f.flush()
    f.close()
