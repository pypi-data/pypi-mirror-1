...
class Reinstaller:
    ...
    def handleMigration(self):
        ...
        if (self.desiredPloneVersion and
            ploneVersion != self.desiredPloneVersion):
            # migration needed.
        ...
        atct_tool = getattr(self.ploneSite, 'portal_atct', None)
        if (self.desiredAtctVersion and 
            atct_tool and hasattr(atct_tool, 'getVersion')):
            # In ATCT as delivered with Plone 3, there is no
            # getVersino and actually no migration is necessary
            # anymore; the Migrate tab has even disappeared.
            ...
            if atctVersion != self.desiredAtctVersion:
                print "ATContentTypes needs upgrading! (%s to %s)"%\
                    (atctVersion, self.desiredAtctVersion)
