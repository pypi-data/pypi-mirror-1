...
def parseArguments():
    ...
    if len(args) > 0:
        ...
        if firstArgument in projects:
            ...
            project = firstArgument
            log.debug("Bootstrapping, so taking the first argument "
                      "as projectname: %s.", project)
    if len(args) > 1:
    ...
    if not project and options.bootstrap is not None:
        # Try to construct project name out of bootstrap file:
        # .../projectname.py => projectname
        try:
            project = options.bootstrap.split('/')[-1][:-3]
            log.debug("Extracted project name out of bootstrap "
                      "filename: %s.", project)
        except:
...
def main():
    ...
    if project == 'ALL':
        ...
        if options.bootstrap:
            # Special case
            projectConfig = configuration.BootstrapConfiguration(
                project,
