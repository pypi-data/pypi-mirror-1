...
from nmapparser._nmapxmlparser import parse_nmap_xml
...
class _NmapParsed(dict):
    """
    A especial dict used to facilitate attribute access and listing 
    ...
    def _cleanup(self):
        """
        Clear current dict content and other attributes created when
        setting currenct dict.
        """
        self.clear()
...
class NmapParser(object):
    """
    A wrapper over core nmap xml parser. After parsing a xml file
    you can access its dict keys like object attribute access, because they
    are turned into attributes for you. You can also list the options
    available by doing myinstance.options, where myinstance is a NmapParser
    instance. If myinstance.someoption is a dict, it will contain .options 
    too (myinstance.someoption.options), otherwise you iterate over it 
    where it's items may be a dict, and if it is, it will contain .options 
    too.
    """
    ...
    def __getattribute__(self, attr):
        try:
            ...
            if self.parsed:
                if attr == DICTKEYS:
                    return object.__getattribute__(self.parsed, "keys")()
                else:
                    return object.__getattribute__(self.parsed, attr)

            # if it is still here, raise that error
            raise
    ...
    discard = property(_get_discard, _set_discard, 
                       doc="Set/Get elements to be discarded. Elements that \
may be discarded \n\
