...
class CodeIO(BufferIO):
    ...
    def out(self, value):
        ...
        elif isinstance(value, unicode) and self.encoding:
    ...
    def flush(self):
        if self.queue:
            ...
            while self.queue:
                ...
                if isinstance(current, basestring) and isinstance(next, basestring):
    ...
    def escape2(self, variable):
        self.write("if basestring not in type(%s).__bases__ and hasattr(%s, '__html__'):" % (variable, variable))
        self.indent()
        self.write("%s = %s.__html__()" % (variable, variable))
        self.outdent()
        self.write("elif isinstance(%s, basestring):" % variable)
        self.indent()
