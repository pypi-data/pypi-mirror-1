...
class Node(object):
    ...
    def ns_attributes(self):
        ...
        attrs = dict(
            ((prefix and "xmlns:%s" % prefix or "xmlns", ns) for (prefix, ns) in \
             ...
             ns not in ns_omit and prefix not in prefix_omit))
        ...
        for prefix, ns in self.element.nsmap.items():
            if ns not in self.ns_omit:
                ...
                for tag, value in attrs.items():
                    ...
                    if prefix and self.element.prefix != prefix:
    ...
    def serialize(self):
        ...
        if self.define_slot:
            ...
            if name in scope:
                scope_args = tuple(
                "%s=%s" % (variable, variable) for variable in scope \
                    if variable != self.symbols.scope)
