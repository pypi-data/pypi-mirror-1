...
class Node(object):
    ...
    def serialize(self):
        ...
        # use macro
            ...
            kwargs = []
            scope = set(itertools.chain(*self.stream.scope[1:]))
            exclude = self.stream.scope[0]
            callbacks = set()
            undefined = set()

            if self.use_macro:
                # when using a macro, slots that are not used in this
                # template (by a slot definition), are marked as
                # undefined; note that a filled slot takes precedence
                # over an undefined slot---the logic is a little
                # convoluted, the reason being that macros essentially
                # behave like a black box
                defined = set()
                element = self.element
                while element is not None:
                    for define in utils.elements_with_attribute(
                        element, config.METAL_NS, 'define-slot'):
                        defined.add(define.node.define_slot)
                    element = element.getparent()

                for variable in scope:
                    if variable.startswith(self.symbols.slot):
                        name = variable[len(self.symbols.slot):]
                        if name not in defined:
                            undefined.add(variable)

            for element in utils.elements_with_attribute(
            ...
            elif self.extend_macro:
                _.append(clauses.Assign(self.extend_macro, self.symbols.metal))
            ...
            for key in undefined:
                self.stream.symbol_mapping[self.symbols.raise_exc] = utils.raise_exc
                raise_method_name = "raise%s" % key
                
                _.append(clauses.Assign(
                    types.value("lambda *args, **kwargs: %s(KeyError(%s))" % (
                        self.symbols.raise_exc, repr(key.lstrip(self.symbols.slot)))),
                    raise_method_name))
                
                arguments[key] = raise_method_name
            ...
            if 'xmlns' in self.element.attrib:
                arguments['include_ns_attribute'] = repr(True)

            arguments = ", ".join("%s=%s" % kwarg for kwarg in arguments.items())
            value = types.value(
            ...
            if self.use_macro:
                ...
                value.label = self.extend_macro.label
...
class Compiler(object):
    ...
    def __init__(self, tree, explicit_doctype=None,
        ...
        macros = self.macros = []
        for element in utils.elements_with_attribute(
            self.tree.getroot(), config.METAL_NS, 'define-macro'):
            macro = element.attrib.get('define-macro')
            if macro is None:
                macro = element.attrib.get('{%s}define-macro' % config.METAL_NS)
            macros.append(macro)
    ...
    def __call__(self, macro=None, global_scope=True, parameters=()):
        ...
        if macro:
            elements = tuple(utils.elements_with_attribute(
            ...
            if 'define-macro' in element.attrib:
                del element.attrib['define-macro']
            elif utils.metal_attr('define-macro') in element.attrib:
