    ...
    def begin(self, stream):
        ...
        for attribute, value in self.attributes.items():
            if isinstance(value, types.expression):
                ...
                if isinstance(value, unicode) and stream.encoding:
                    value = value.encode(stream.encoding, 'xmlcharrefreplace')
...
class Write(object):
    ...
    def write(self, stream):
        ...
        write("if type(%(tmp)s) not in (str, unicode) and hasattr(%(tmp)s, '__html__'):")
        stream.indent()
...
class Method(object):
    ...
    def __init__(self, name, args, ret=None, decorators=()):
        self.name = name
        self.args = args
