...
class TagBase(object):    
    ...
    def get_tag_contents(self):
        ...
        contents = "".join([unicode(element) for element in content_elements if isinstance(element, StringToken)])
...
class PostMarkup(object):
    ...
    def render_to_html(self, post_markup, max_length = None, continuation_text = None, max_continuous = None):
        """
        ...
        for tag_type, tag_token in PostMarkup.tokenize(post_markup):
            ...
            if tag_type == PostMarkup.TOKEN_TEXT:
                redo_break_stack()                                    
                if max_continuous:
                    orig_tag_token = tag_token
                    words = tag_token.split(" ")
                    out_words = []
                    for word in words:
                        while len(word) > max_continuous:
                            out_words.append(word[:max_continuous])
                            word = word[max_continuous:]
                        out_words.append(word)
                    tag_token = " ".join(out_words)
                        
                if max_length:
                   if len(tag_token) > remaining_length:
                       post.append(StringToken(tag_token[:remaining_length]))
                       continued = True
                       break
                   remaining_length -= len(tag_token)
        ...
        if continued and continuation_text:
            post.append(continuation_text)
                     
        return "".join(unicode(p) for p in post)
