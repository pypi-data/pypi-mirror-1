kukit.actionsGlobalRegistry.register("autocomplete", function(oper) {
	oper.evaluateParameters(['vocab_list'], 
	    {'timeout':60000, 'limit':20, 'first_text':1, 'expand_onfocus':1, 'complete_on_tab':1},
	    'autocomplete action');
	/*
	if (confirm(oper.parms.text)) {
        oper.binderInstance.__continueEvent__('selected', oper.node, {});
	};
	*/
	vocab_array = eval(oper.parms.vocab_list);
	actb(oper.node, oper.browserevent, vocab_array, oper.parms.timeout,oper.parms.limit,oper.parms.first_text,
	     oper.parms.expand_onfocus, oper.parms.complete_on_tab, oper)
	     
	//actb(obj,evt,ca, time_out, limit, first_text, expand_onfocus, complete_on_tab)
});
kukit.commandsGlobalRegistry.registerFromAction('autocomplete', 
                                                kukit.cr.makeSelectorCommand);

kukit.eventsGlobalRegistry.register(null, 'selected', kukit.pl.NativeEventBinder, null, null);
...
function actb(obj,evt,ca, time_out, limit, first_text, expand_onfocus, complete_on_tab, oper){
    /* ---- Variables ---- */
    var actb_oper = oper; // The oper object, so i can run the event
    ...
    obj.onkeypress = actb_noenter;
    
    if (!document.getElementById('tat_table') && actb_expand_onfocus) actb_onfocusid = setTimeout(function(){actb_tocomplete(188)},150);

    function actb_noenter(evt) {
    ...
    function actb_mouse_down(){
        ...
        document.getElementById('tat_tr'+actb_pos).className = 'actb_active';
        //document.getElementById('tat_tr'+actb_pos).style.backgroundColor = actb_hColor;
        //if (actb_onfocusid) clearTimeout(actb_onfocusid);
        actb_curr.focus();
        //if (actb_expand_onfocus) actb_onfocusid = setTimeout(function(){actb_tocomplete(188)},150);
...
//        actb_caretmove = 1;
    ...
    function actb_insertword(a){
        ...
        //actb_curr.binderInstance.__continueEvent__('confirmed', oper.node, {});
        actb_mouse_on_list = 0;
        actb_removedisp();
