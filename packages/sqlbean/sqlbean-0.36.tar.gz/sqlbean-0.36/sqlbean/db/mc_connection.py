...
class SimpleCached:
    ...
    def set_marshal(self,key,val,time=0):
        val = marshal.dumps(val)
        self.mc.set(key,val,time)
  
    def get_marshal(self,key):
        r = self.mc.get(key)
        if r is not None:
            r = marshal.loads(r)
        return r
    
    def get_multi_marshal(self,keys):
        rs = self.mc.get_multi(keys)
        rs = dict((k,marshal.loads(v)) for k,v in rs.iteritems())
        return rs
    
    def get_list_marshal(self, keys):
        rs = self.get_multi_marshal(keys)
...
class LocalCached:
    ...
    def start_log(self):
        from zku.debug_util import CallLogger
...
def init_mc(memcached_addr,disable_memcache=False,disable_local_cached=False):
   
    global mc

    if disable_memcache or not memcached_addr:
