...
def _check_fop():
    if not fop_home:
        LOG.error('$FOP_HOME not set')
        return False

    if not os.path.exists(fop_home):
        LOG.error('$FOP_HOME=%s does not exist' % fop_home)
        return False
...
class FOPPDFConverter(object):
    ...
    def convert(self, fo_filename, output_filename=None):
        ...
        if sys.platform == 'win32':
            cmd = '%s\\fop.exe -fo "%s" -pdf "%s"' % (fop_home, fo_filename, output_filename)
        else:
            cmd = '"%s/fop" -fo "%s" -pdf "%s"' % (fop_home, fo_filename, output_filename)
