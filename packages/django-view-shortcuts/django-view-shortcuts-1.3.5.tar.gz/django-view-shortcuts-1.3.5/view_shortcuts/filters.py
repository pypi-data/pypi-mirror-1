...
class RelationFilter(Filter):
    ...
    def generate_choices(self):
        ...
        for c in choices:
            ...
            yield FilterChoice(self, unicode(c), value, c.items_count)
...
class AlphabeticFilter(Filter):
    def generate_choices(self):
        ...
        for choice in choices:
            char = unicode(choice.get(self.lookup))[0].lower()
...
class FilterChoice(object):
    ...
    @cached_property
    def urlencode(self):
        value = self.value.encode('utf-8') if isinstance(self.value, unicode) else self.value
