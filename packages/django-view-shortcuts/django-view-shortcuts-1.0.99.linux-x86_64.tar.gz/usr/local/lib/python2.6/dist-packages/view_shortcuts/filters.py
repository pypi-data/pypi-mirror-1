...
class facet(dict):
    ...
    >>> facets = (
    ...     facet('category'),
    ...     facet('author', 'author__pk', AlphabetRelationFilter),
    ... )
...
class FilterList(list):
    ...
    views.py:
        ...
        def my_entry_list(request):
            ...
            filters = FilterList(qs, request, ('foo','bar'))
...
class Filter(object):
    ...
    def register(cls, test, factory):
        cls._filter_specs.append((test, factory))
    ...
    def create(cls, param, qs, lookup, value, active=False, sort_by_usage=True):
        # chosen by user
        if not cls == Filter:
            return cls(param, qs, lookup, active, value, sort_by_usage)
        # autoselect
        field = cls.resolve_field(qs,lookup)
        for test, factory in cls._filter_specs:
            if test(field):
                return factory(param, qs, lookup, active, value, sort_by_usage)
