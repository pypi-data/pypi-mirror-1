...
logger = logging.getLogger('iwm.recipe.bebopinstance')
...
class Recipe:
    ...
    def __init__(self, buildout, name, options):
        ...
        # Enable or disable bebop specific options
        self.bebopmode = False
        if options.get('bebop-mode', '').strip() == 'on':
            self.bebopmode = True
    ...
    def install(self):
        ...
        subprogram_dir = os.path.join(dest, 'bin')
        config_dir = os.path.join(dest, 'etc')
        log_dir = os.path.join(dest, 'log')
        run_dir = os.path.join(dest, 'var')
        requirements, ws = self.egg.working_set()

        for directory in (dest, subprogram_dir, config_dir, log_dir, run_dir):
            os.mkdir(directory)
        ...
        requirements, ws = self.egg.working_set(['iwm.recipe.bebopinstance'])
        zc.buildout.easy_install.scripts(
            [('debugzope', 'iwm.recipe.bebopinstance.zope3scripts', 'debug'),
             ('scriptzope', 'iwm.recipe.bebopinstance.zope3scripts', 'script'),
             ],
            ws, options['executable'], subprogram_dir,
            extra_paths = options['extra-paths'].split(),
            arguments = ('\n        ["-C", %r]'
        ...
        zc.buildout.easy_install.scripts(
            [('zopectl', 'iwm.recipe.bebopinstance.ctl', 'main')],
            ws, options['executable'], subprogram_dir,
            extra_paths = options['extra-paths'].split(),
            arguments = ('\n        %r,'
                         '\n        %r,'
    ...
    def _zcml(self, src, dest, options):
        ...
        # Write a (Bebop specific) securitypolicy.zcml
        securitypolicy_zcml_path = os.path.join(dest, 'securitypolicy.zcml')
        if self.bebopmode:
            open(securitypolicy_zcml_path, 'w').write(securitypolicy_zcml_bebop)
        else:
            open(securitypolicy_zcml_path, 'w').write(securitypolicy_zcml)
