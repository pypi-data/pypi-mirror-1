...
class MenuDisplay(framework.CompoundElement):
    ...
    def refresh(self, menu=None):
        ...
        if self.hidable:
            ...
            img = self.font.render(self.app, menu.name, False, self.titleColour)
            r = img.get_rect()
            ...
            img = self.font.render(self.app, menu.name, False, self.hvrColour)
            hvrImage.blit(img, r)
    ...
    def processEvent(self, event):
        ...
        return super(MenuDisplay, self).processEvent(event)
    ...
    def addMenuItem(self, item, x, y, width, itemHeight):
        ...
        if item.name == '---':
            ...
            img = self.font.render(self.app, item.name, False, self.stdColour)
            r = img.get_rect()
            r.midleft = (2, itemHeight // 2)
            stdImage.blit(img, r)

            # Put caption on hover canvas.        
            img = self.font.render(self.app, item.name, False, self.hvrColour)
            hvrImage.blit(img, r)
        ...
        if item.action is not None and self.keymapping is not None:
            try:
                ...
                img = self.font.render(self.app, hkStr, False, self.stdColour)
                r = img.get_rect()
                r.midright = (width-2, itemHeight // 2)
                stdImage.blit(img, r)

                # Put hotkey on hover canvas.        
                img = self.font.render(self.app, hkStr, False, self.hvrColour)
                hvrImage.blit(img, r)
