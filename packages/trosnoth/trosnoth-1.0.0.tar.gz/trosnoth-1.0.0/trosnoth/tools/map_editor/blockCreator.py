...
from trosnoth.src.universe import Universe, ForwardInterfaceMapBlock, InterfaceMapBlock, \
     BackwardInterfaceMapBlock, TopBodyMapBlock, BottomBodyMapBlock, BodyMapBlock, MapLayout
...
from trosnoth.data import getPath, blocks
...
class MapBlockCreator(app.Application):
    ...
    scaleIndex = 3
    def __init__(self):
        super(MapBlockCreator, self).__init__((600,800), 0, "hello", interface.Interface, False)
    ...
    def getMapBlockString(self):
        ...
        finalString = 'blockType = ' + self.currentType.__name__
        finalString += ',\nblocked = ' + repr(self.currentBlockage)
        finalString += ",\nsymmetrical = %s," % (self.symmetrical)
        finalString += "\ngraphic = '%s'," % (self.graphicName)
        finalString += "\nobstacles = %s" % (repr(allObstacles))
        if allPlatforms != []:
            finalString += ",\nplatforms = %s" % (repr(allPlatforms))
        return finalString
    ...
    def saveInternal(self):
        try:
            graphicPath = getPath(blocks, self.graphicName)
            self.interface.graphify(graphicPath)
            blockPath = getPath(blocks, self.blockName)
    ...
    def load(self):
        try:
            ...
            try:
                code = compile('loaded("%s", %s)' % (filename, contents), \
                               filename, 'exec')
            except SyntaxError:
                raise IOError, 'invalid file format'
            
            namespace = blockFileNamespace.copy()
            namespace.update({'loaded': self.loaded})
    ...
    def loaded(self, filename, blockType=TopBodyMapBlock, blocked=False, \
