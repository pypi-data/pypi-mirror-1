...
class InternetTab(Tab, framework.TabFriendlyCompoundElement):
    def __init__(self, app, tabContainer, startupInterface):
        super(InternetTab, self).__init__(app, "Internet")
        ...
        self.elements = [self.serverNameBox,
                         ...
                         self.button('clear',  self.clear,               (-50, 160), 'topright'),
                         TextElement(self.app, 'Server name:',
        ...
        self.previousServerData = self.restore()
        if self.previousServerData != None:
            self.serverNameBox.value = str(self.previousServerData["host"])
            self.portBox.value = str(self.previousServerData["port"])
