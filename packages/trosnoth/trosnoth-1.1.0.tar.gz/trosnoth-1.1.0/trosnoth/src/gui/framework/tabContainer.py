...
# TODO: Stop Key repeats in prompts when tabs change

from trosnoth.src.gui.framework.framework import Element, CompoundElement
from trosnoth.src.gui.framework.hotkey import Hotkey
from trosnoth.src.gui.framework.tab import Tab
from trosnoth.src.gui.common import translateEvent, Location, AttachedPoint, ScaledScalar
...
class TabHeader(Element):
    ...
    def _getColour(self):
        # TODO: Put these const colours somewhere else?
        #       or actually calculate thems.
        if self.hasFocus:
    ...
    def _redraw(self):
        ...
        newSurface = self.font.render(self.app, self.tab.caption, True, self._getColour())
        newRect = newSurface.get_rect(centerx=self.size[0]/2)
        
        x = self.tabContainer._getBorderWidth() + 2
        fullWidth = self.size[0] - 2 * x
        rect = pygame.Rect(0,0,newSurface.get_height(), 20)
        self.surface.blit(newSurface, (max(x, newRect.left),x))#, rect)
    ...
    def processEvent(self, event):
        if (not self.hasFocus and \
           event.type == pygame.locals.MOUSEBUTTONDOWN and \
           pygame.Rect(self.pos, self.size).collidepoint(event.pos)): 
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(screen, self._getColour(), pygame.Rect(self.pos, self.size), self.tabContainer._getBorderWidth())
...
class TabHeaderArrow(Element):
    ...
    def _redraw(self):
        ...
        # TODO: background colour?
        newSurface = self.font.render(self.app, self.text, True, self._getColour())
        newRect = newSurface.get_rect(centerx=self._getSize()[0]/2)
        
        x = self.tabContainer._getBorderWidth() + 2
        fullWidth = self._getSize()[0] - 2 * x
        rect = pygame.Rect(0,0,newSurface.get_height(), 20)
        self.surface.blit(newSurface, (max(x, newRect.left),x))#, rect)
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(screen, self._getColour(), self._getRect(), self.tabContainer._getBorderWidth())
...
class TabHeaderRow(CompoundElement):
    def __init__(self, app, tabContainer):
        ...
        canClickRight = lambda: self.firstVisible + self.numToDisplay < len(self.headers)
        self.tabContainer = tabContainer
        ...
        self.leftArrow = TabHeaderArrow(app, tabContainer, "<", tabContainer.font, onLeft, canClickLeft)
        leftLocation = Location(AttachedPoint((0,0), tabContainer._getRect, 'topleft'), 'topleft')
        leftSize = ArrowSize("<")
        self.leftArrow.setDimensions(leftLocation, leftSize)

        
        self.rightArrow = TabHeaderArrow(app, tabContainer, ">", tabContainer.font, onRight, canClickRight)
        rightLocation = Location(AttachedPoint((0,0), tabContainer._getRect, 'topright'), 'topright')
        rightSize = ArrowSize(">")
    ...
    def _getMaxHeaderWidth(self):
        return min(self._getHeight() * 5, self._getWidth() / 2)
    ...
    def _moveDirection(self, left):
        if left:
            ...
            self.firstVisible = min(self.firstVisible + self.skipOnMove, len(self.headers)-self.numToDisplay)
    ...
    def _orderElements(self):
        ...
        if self.selectedIndex == -1:
            self.elements = []
        elif self.firstVisible <= self.selectedIndex < self.firstVisible+self.numToDisplay:
            self.elements = self.headers[self.firstVisible:self.selectedIndex] + self.headers[self.selectedIndex+1:self.firstVisible+self.numToDisplay] + [self.headers[self.selectedIndex]]
        else:
            self.elements = self.headers[self.firstVisible:self.firstVisible+self.numToDisplay]
    ...
    def _setFirstVisible(self):
        if self.firstVisible <= self.selectedIndex < self.firstVisible + self.numToDisplay:
            pass
    ...
    def _update(self):
        if len(self.headers) > 0:
            ...
            for x in range (self.firstVisible, self.firstVisible+self.numToDisplay):
                header = self.headers[x]
                thisWidth = width
                # Do a bit of balancing for tab header widths
                if x - self.firstVisible < remainder:
                    thisWidth += 1
                header.setDimensions((currentX, self._getPt()[1]), (thisWidth, self._getHeight()))
                currentX += thisWidth
...
class TabContainer(CompoundElement):
    def __init__(self, app, area, font, borderColour = (0,0,0)):
        ...
        nextTab = lambda: self._tabSelected((self.selectedIndex + 1) % len(self.tabs))
        lastTab = lambda: self._tabSelected((self.selectedIndex - 1) % len(self.tabs))
        self.nextTabKey = Hotkey(app, pygame.locals.K_TAB, pygame.locals.KMOD_CTRL)
        self.nextTabKey.onTriggered.addListener(nextTab)
        self.lastTabKey = Hotkey(app, pygame.locals.K_TAB, pygame.locals.KMOD_CTRL|pygame.locals.KMOD_SHIFT)
    ...
    def draw(self, screen):
        ...
        assert len(self.tabs) > 0, "Need to have at least one tab before drawing a tabContainer"
        # Draw a border
        pygame.draw.rect(screen, self.borderColour, self._getTabRect(), self._getBorderWidth())
