...
class LeaderBoard(framework.CompoundElement):
    def __init__(self, app, world, gameViewer):
        ...
        self.xPos = 4
        self.yPos = self.gameViewer.miniMap.sRect.bottom + self.gameViewer.zoneBarHeight + 5
        ...
        self.playerTable.style.foreColour = colours.leaderboardNormal
        ...
        self.colourLookup = {"A": colours.leaderboardBlue, "B": colours.leaderboardRed}
    ...
    def update(self):
        ...
        xPos = 4
        yPos = self.gameViewer.miniMap.sRect.bottom + self.gameViewer.zoneBarHeight + 5 + random.randint(0, 200)
        position = Location(FullScreenAttachedPoint((xPos, yPos), 'topright'), 'topright')

        #self.playerTable.pos = position
        
        self.callDef = reactor.callLater(1, self.update)
            ...
            if len(players) > 0:
                ...
                if self.world.gameState == GameState.PreGame:
                    if team.ready:
                        ...
                        self.playerTable[2][count].style.foreColour = colours.leaderboardNormal
                    else:
                        self.playerTable[2][count].setText("not ready")
                        self.playerTable[2][count].style.foreColour = colours.leaderboardDead
            ...
            for player in players:
                ...
                if self.world.gameState == GameState.PreGame:
                    ...
                    if player["upgrade"] is None:
                        ...
                        # Only show enemy upgrades if they are in the whitelist
                        if team is self.enemyTeam and player["upgrade"] not in visibleUpgrades:
                            upgradeText = ""
                        else:
                ...
                if player["dead"]:
                    self.playerTable[0][count].style.foreColour = colours.leaderboardDead
                    self.playerTable[1][count].style.foreColour = colours.leaderboardDead
                else:
                    self.playerTable[0][count].style.foreColour = self.colourLookup[team.id]
                    self.playerTable[1][count].style.foreColour = colours.leaderboardNormal

            # Add the "star total" row
            if len(players) > 1 and (team is self.friendlyTeam or self.spectator) \
            and self.world.gameState != GameState.PreGame:
                count += 1
                self.playerTable.addRow()
                self.playerTable.getRow(count).setHeight(ScaledScalar(self.rowHeight))
                self.playerTable[0][count].setText("Total:")
                self.playerTable[1][count].setText(str(team.teamStars))
                if team.teamStars == 1:
                    noun = "star"
                else:
                    noun = "stars"
    ...
    def draw(self, surface):
        ...
        for name, index in self.indexes.iteritems():
            ...
            if name[:5] == 'total':
                ...
                pointC, pointD = shift(pointA, pointB)
                pygame.draw.line(surface, colours.leaderboardNormal, pointA, pointB, 1)
                pygame.draw.line(surface, (0, 0, 0), pointC, pointD, 1)
