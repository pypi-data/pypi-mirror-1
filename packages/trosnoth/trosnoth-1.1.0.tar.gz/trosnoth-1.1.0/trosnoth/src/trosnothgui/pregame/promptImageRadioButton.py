...
from trosnoth.src.trosnothgui.pregame.colours import *

class CustomPrompt(DialogBox, framework.TabFriendlyCompoundElement):
    def __init__(self, app, width=2, height=1):
        super(CustomPrompt, self).__init__(app, ScaledSize(400,230), "Custom Size")

        # Half width input and label
        widthText = TextElement(app, "Half Width:", app.screenManager.fonts.bigMenuFont,
                                Location(DialogBoxAttachedPoint(self, ScaledSize(260,10)), 'topright'), radioSelected)
        self.width = InputBox(app, Area(DialogBoxAttachedPoint(self, ScaledSize(280,10)), ScaledSize(80,50)),
                              str(width), font = app.screenManager.fonts.bigMenuFont, validator = intValidator, maxLength = 2)
        ...
        heightText = TextElement(app, "Height:", app.screenManager.fonts.bigMenuFont,
                                 Location(DialogBoxAttachedPoint(self, ScaledSize(260,75)), 'topright'), radioSelected)
        self.height = InputBox(app, Area(DialogBoxAttachedPoint(self, ScaledSize(280,75)), ScaledSize(80,50)),
                               str(height), font = app.screenManager.fonts.bigMenuFont, validator = intValidator, maxLength = 2)
        ...
        # OK Button
        okButton = TextButton(app,  Location(DialogBoxAttachedPoint(self, ScaledSize(115,160))), "OK", font, dialogButtonColour, radioMouseover)
        okButton.onClick.addListener(lambda sender: self.ok())

        # Cancel Button
        cancelButton = TextButton(app,  Location(DialogBoxAttachedPoint(self, ScaledSize(195,160))), "Cancel", font, dialogButtonColour, radioMouseover)
        cancelButton.onClick.addListener(lambda sender: self.cancel())

        # Add elements to screen
        self.elements = [self.height, self.width, widthText, heightText, okButton, cancelButton]
        self.tabOrder = [self.width, self.height]
        self.setFocus(self.width)
        self.setColours(dialogBoxEdgeColour, dialogBoxTextColour, dialogBoxTextColour)
...
class PromptImageRadioButton(ImageRadioButton):
    def __init__(self, app, text, area):
        customFont = ScaledFont("KLEPTOCR.TTF", 100)
        self.imageCustom= TextImage("?", customFont, (0,0,0))
        
        super(PromptImageRadioButton, self).__init__(app, text, area, self.imageCustom)
        self.value = None
