...
from trosnoth.src.gui.framework.elements import PictureElement, TextElement
...
class RadioButtonGroup(framework.CompoundElement):
    ...
    def selected(self, radioButton):
        assert radioButton in self.elements, "Cannot select a radiobutton that isn't in this group"
...
class ImageRadioButton(framework.CompoundElement):
    ...
    class IRB_background(framework.Element):
        ...
        def draw(self, surface):
            if self.imageRadioButton._selected:
                colour = radioSelected
            elif self.mouseOver:
                colour = radioMouseover
            else:
                colour = radioUnselected
    ...
    def __init__(self, app, text, area, image):
        super(ImageRadioButton, self).__init__(app)
        textLoc = Location(AttachedPoint((0,0), self._getRect, 'midbottom'), 'midbottom')
        font = app.screenManager.fonts.smallNoteFont
        self.text = TextElement(app, text, font, textLoc, (0,0,0))
        picLoc = Location(AttachedPoint((0,0), self._getRect, 'midtop'), 'midtop')
        ...
        self.elements = [self.bg, self.image, self.text]

    def _beforeSelected(self):
        return True

    def _clicked(self):
        assert self.group, "Image Radio Buttons are required to belong to a RadioButtonGroup"
