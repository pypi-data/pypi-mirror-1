...
class ConnectionFailedDialog(DialogBox):
    def __init__(self, app, reason):
        ...
        elements = []
        for text in lines:
            elements.append(TextElement(app, text, font, Location(DialogBoxAttachedPoint(self, (0, x), 'midtop'), 'midtop'), colour=colours.errorColour))
        ...
        self.elements.append(TextButton(app, Location(DialogBoxAttachedPoint(self, (0, -boundary), 'midbottom'), 'midbottom'), "OK", font, colours.dialogButtonColour, colours.radioMouseover, onClick = lambda sender: self.Close()))
        self.onEnter = Event()
