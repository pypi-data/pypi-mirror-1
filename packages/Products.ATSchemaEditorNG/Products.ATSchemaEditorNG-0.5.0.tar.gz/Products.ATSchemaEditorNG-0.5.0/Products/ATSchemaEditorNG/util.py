...
$Id: util.py 60770 2008-03-16 12:29:11Z spamsch $
"""
...
def redirect(REQUEST, dest, msg=None,**kw):
    """ redirect() helper method """
    
    # first parameter was RESPONSE until recently
   
    if hasattr(REQUEST,'redirect'):
        # old style with RESPONSE given instead of REQUEST
            url = dest + "?"
            if msg:
    ...
    # newstyle
    if (REQUEST is not None) and hasattr(REQUEST,'RESPONSE'):    
        url = dest + "?"
        if msg:
            IStatusMessage(REQUEST).addStatusMessage(msg)
        if kw:
            url += '&'.join(['%s=%s' % (k, urllib.quote(v)) for k,v in kw.items()])
