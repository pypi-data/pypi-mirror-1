...
$Id: ParentManagedSchema.py 60770 2008-03-16 12:29:11Z spamsch $
"""
...
class ManagedSchemaBase:
    ...
    def Schema(self, schema_id=None):
        ...
        # Versioning in plone3
        try:
            self.lookup_provider()
        except AttributeError:
            if hasattr(self,'_schema_protected_from_fs_changes'):
               return self._wrap_schema(self._schema_protected_from_fs_changes)
            else:
...
class ParentManagedSchema(ManagedSchemaBase):
    ...
    def lookup_provider(self):
        ...
        # could be wrapped by ColumnPortletManagerRenderer
...
class ParentOrToolManagedSchema(ManagedSchemaBase):
    ...
    def lookup_provider(self):
        ...
        provider = None
        try:
            provider = self.aq_inner.aq_parent
            if not ISchemaEditor.isImplementedBy(provider):
                if provider.__class__.__name__ == 'FactoryTool':
                    return provider.aq_inner.aq_parent
                raise SchemaEditorError, ''

        # We redirec lookup per default to the tool
