...
try:
    ...
    print "Usage: %s ini_file path_to_watch [delay]" % os.path.basename(sys.argv[0])
    sys.exit(1)

file(os.path.join(WORK_DIR, "%s.pid"%win32process.GetCurrentProcessId()), 'w').writelines((ini_file, path_to_watch, delay))
delay = int(delay)
    ...
    for id, path in events:
        ...
        if id == winnt.FILE_ACTION_ADDED:
            if os.path.isdir(cpath):
                time.sleep(delay)
                command = u'cmd /c start /min "%spython.exe" "%s" "%s" %s' % (PYTHON_PATH, os.path.join(WORK_DIR, 'UploadFu_Upload.py'), ini_file, cpath)
