...
__version__ = "UploadFu/0.4.2-r%s" % "$Revision: 74 $".split(':')[1].strip(' $')
...
def match_section_stuff(logger, matchs, section):
    ...
    for k,v in result:
        try:
            ex = re.compile(k, re.I)
        except:
            logger.exception("Bad regexp in %s: %r" % (section.name, k))
            continue
        m = ex.search(data, re.I)
        if m:
            return v % m.groupdict()

def match_stuff(logger, defaults_dict, regex_dict, text):
    v = {}
            ...
            if os.path.exists(dir_to_upload):
                while 1:
                    try:
                        ...
                        logger.info("Using nfo_regex result: %r" % v)
                        ...
                        logger.info("Using page_regex result: %r" % v)
