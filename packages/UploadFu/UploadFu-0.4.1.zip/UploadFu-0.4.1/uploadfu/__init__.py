...
__version__ = "UploadFu/0.4.1-r%s" % "$Revision: 73 $".split(':')[1].strip(' $')
...
def match_section_stuff(matchs, section):
    data = section['data'] % matchs
    result = section['result']
    
    for k,v in result:
        m = re.search(k, data, re.I)
        if m:
...
def match_stuff(defaults_dict, regex_dict, text):
    ...
    for exp in regex_dict:
        if isinstance(regex_dict[exp], Section):
            v[exp] = match_section_stuff(v, regex_dict[exp])
        else:
