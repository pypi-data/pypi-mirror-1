            ...
            if os.path.exists(dir_to_upload):
                while 1:
                    try:
                        ...
                        print 'Making:', target
                        make_meta_files(args[0], args[1:], piece_len_pow2=int(config['piece_size']), progressfunc=prog, filefunc=dc, comment='made with %s'%__version__, target=target, filesystem_encoding='')
