...
__revision__  = "$Revision: 77666 $"
__version__   = '$Revision: 77666 $'[11:-2]
...
class TestPloneArticle(GSXMLTestCase):

    def afterSetUp(self):
        self.setRoles(('Manager',))

    def test_plonearticle_export(self):
        from collective.plone.gsxml.context import DummyContext
        from collective.plone.gsxml.content import XMLContentFSExporter

        self.folder.invokeFactory("Article", "article")

        ec = DummyContext()
        exporter = XMLContentFSExporter(self.folder)
        exporter.export(ec)
