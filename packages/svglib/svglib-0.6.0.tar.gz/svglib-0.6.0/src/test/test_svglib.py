...
Some tests try using a tool called uniconv to convert 
SVG files into PDF for comparision with svglib.
"""
...
def testit(func, mapping):
    "Call 'func' on pais in mapping and return list of failed tests."
...
class SVGSamplesTestCase(unittest.TestCase):
    ...
    def test0(self):
        ...
        paths = glob.glob("samples/*")
        paths = [p for p in paths 
            if os.path.splitext(p.lower())[1] in [".svg", ".svgz"]]
...
class SVGSamplesFromWikipediaTestCase(unittest.TestCase):
    "Tests on sample SVG files from wikipedia.org."
...
/wikipedia/commons/9/97/Tudor_rose.svg""".split()
    ...
    def test1(self):
        "Test converting Wikipedia SVG files to PDF using uniconverter."
...
class W3CTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.folderPath = splitext(tarPath)[0]

        if not exists(self.folderPath):
            if not exists(tarPath):
                if not exists(archivePath):
                    ...
                    try:
                        ...
                        return
                    archivePath = basename(url)
                    open(archivePath, "wb").write(data)
                print "unpacking %s" % archivePath
                tarData = gzip.open(archivePath, "rb").read()    
                open(tarPath, "wb").write(tarData)
            print "extracting into %s" % self.folderPath
            os.mkdir(self.folderPath)
            tarFile = tarfile.TarFile(tarPath)
    ...
    def test0(self):
        ...
        for i, path in enumerate(paths):
            ...
            if os.path.basename(path) in excludeList:
    ...
    def _test1(self):
        "Test converting W3C SVG files to PDF using uniconverter."
        # outcommented, because all SVG samples seem to generate errors
