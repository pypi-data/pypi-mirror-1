...
"""
<p>This module provides a class <pyref class="Call"><class>Call</class></pyref>
that allows you to use Oracle PL/SQL procedures/functions as
<a href="http://www.cherrypy.org/">CherryPy</a> response handlers.
...
from  <pyref module="ll.orasql"><mod>ll.orasql</mod></pyref>.</p>
<p>For example, you might have the following PL/SQL function:</p>
<prog><![CDATA[
...
<p>Using this function as a CherryPy response handler can be done like this:</p>
<prog>
import cherrypy
...
def httpdate(dt):
	...
	<p><arg>dt</arg> is a <class>datetime.datetime</class> object.
	...
	different from the one used by <arg>dt</arg>).</p>
	"""
...
class cache(object):
	"""
	<p>Decorator that adds caching to a CherryPy handler.</p>
	
	<p>Calling a <class>cache</class> object will cache return values
...
def conditional(func):
	"""
	<p>Decorator that adds handling of conditional <lit>GET</lit>s to a CherryPy handler.</p>
	
	<p>The decorated function will correctly handle the <lit>If-Modified-Since</lit>
...
class Connect(object):
	"""
	<p><class>Connect</class> objects can be used as decorators that wraps
	a function that needs a database connection.</p>
	
	<p>If calling the wrapped function results in a database exception
	that has been caused by a lost connection to the database or similar problems,
	the function is retried with a new database connection.</p>
	"""
	...
	def __init__(self, connectstring=None, pool=None, retry=3, **kwargs):
		"""
		Create a new parameterized <class>Connect</class> decorator.
		Either <arg>connectstring</arg> or <arg>pool</arg> (a database pool
		object) must be specified. <arg>retry</arg> specifies how often to retry
		calling the wrapped function after a database exception. <arg>kwargs</arg>
		will be passed on the the <func>connect</func> call.
		"""
...
class Call(object):
	"""
	<p>Wrap an Oracle procedure or function in a CherryPy handler.</p>

	<p><class>Call</class> object wraps a procedure or function object from
	<pyref module="ll.orasql"><mod>ll.orasql</mod></pyref> and makes it
	...
	def __call__(self, *args, **kwargs):
		"""
		<p>Call the procedure/function with the arguments <arg>args</arg> and
		<arg>kwargs</arg> mapping Python function arguments to Oracle procedure/function
		arguments. On return from the procedure the <lit>c_out</lit> parameter is
		mapped to the CherryPy response body, and the parameters <lit>p_expires</lit>
		(the number of days from now), <lit>p_lastmodified</lit> (a date in UTC),
		<lit>p_mimetype</lit> (a string), <lit>p_encoding</lit> (a string),
		<lit>p_etag</lit> (a string) and <lit>p_cachecontrol</lit> (a string)
		are mapped to the appropriate CherryPy response headers. If
		<lit>p_etag</lit> is not specified a value is calculated.</p>
		<p>If the procedure/function raised a PL/SQL exception with a code between
		20200 and 20599, 20000 will be substracted from this value and the resulting
		value will be used as the HTTP response code, i.e. 20404 will give a
		"Not Found" response.
		"""
