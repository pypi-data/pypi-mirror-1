...
class cache(object):
	"""
	Decorator that adds caching to a CherryPy handler.
	
	Calling a :class:`cache` object will cache return values of the decorated
	function for a certain amount of time. You can pass the timespan either via
	:var:`timedelta` (which must be a :class:`datetime.timedelta` object), or
	via :var:`timedeltaargs` (which will be used as keyword arguments for
...
def conditional(func):
	"""
	Decorator that adds handling of conditional ``GET`` requests to a CherryPy
	handler.
	
	The decorated function will correctly handle the ``If-Modified-Since``
	and ``If-None-Match`` request headers. For this to work properly the
	decorated function should set the ``Last-Modified`` and/or the ``ETag``
	header.
	"""
	def wrapper(*args, **kwargs):
