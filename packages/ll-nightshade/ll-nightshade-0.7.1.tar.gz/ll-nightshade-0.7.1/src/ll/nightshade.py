...
class withconnection(object):
	"""
	<par>Decorator that wraps a function that needs a database connection.</par>
	
	<par>If calling the wrapped function results in a database exception
	that has been caused by a lost connection to the database or similar problems,
	the function is retried with a new database connection.</par>
	"""
	...
	def __init__(self, connectstring=None, pool=None, retry=3):
		"""
		Create a new parameterized <class>withconnection</class> decorator.
		...
		calling the wrapped function after a database exception.
		"""
	...
	def connection(self):
		...
		if self.pool is not None:
			...
			self._connection = orasql.connect(self.connectstring, threaded=True)
			...
			if self.pool is not None:
				...
				for i in xrange(self.retry):
					try:
						...
						if self._isbadoracleexception(exc):
							self._connection = None # retry with new connection
						else:
							raise
					else:
						break
...
class Call(object):
	...
	"""
	def __init__(self, callable, pool=None, connectstring=None, retry=3):
		"""
		Create a <class>Call</class> object wrapping the function or procedure
		<arg>callable</arg>.
		"""
		self.callable = callable
		wc = withconnection(connectstring=connectstring, pool=pool, retry=retry)
		...
		self.realcallable = wc(self._call)

	def _call(self, connection, *args, **kwargs):
	...
	def __call__(self, *args, **kwargs):
		...
		<lit>p_mimetype</lit> (a string), <lit>p_encoding</lit> (a string) and
		...
		now = getnow()
		try:
			(body, result) = self.realcallable(*args, **kwargs)
		except cx_Oracle.DatabaseError, exc:
			if exc.args:
				code = getattr(exc[0], "code", 0)
				if 20200 <= code <= 20599:
