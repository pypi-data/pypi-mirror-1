...
class RepeatPattern:
	...
	def __init__ (self,
		...
		max_stdv_ptrn - Optional maximum normalized standard deviation of
			the distances between the patterns. This is a number between 0 and 1.
			0 means that the patterns are in sequence (no other items in
			between). 1 means that there is no importance to the position of the
			patterns in the list. In a normal HTML page you should keep this
			number low, but maybe bigger than zero. The default is zero.
		max_stdv_indx - Optional maximum normalized standard deviation of
			the distances between the returned indexes. This has the same
	...
	def __GetGapsInfo (self, lst):
		"""
		This method returns the minimum, maximum and normalized standard
		deviation of the gaps between the indexes of "lst".
		"""
		gaps = [lst[i+1] - lst[i] for i in range (len (lst) - 1)]
		min_val = min (gaps)
		max_val = max (gaps)
		stdv = self.__NStdv (gaps)
		return min_val, max_val, stdv
	...
	def __FilterOutOnStdv (self, lst, max_stdv):
		...
		min_val, max_val, stdv = self.__GetGapsInfo (lst)
		#print lst, stdv
	...
	def __FindInterfereItems (self, lsts, look_ahead, match_len):
		...
		It also returns the new lists after filtering.
		Return: set, list
		"""
		if not lsts or len (lsts) < 2: return set(), lsts	# Not enough data
		...
		#		print "<<<<", lst
		#	print "----------------------------"
		return result_filters, new_lsts
	...
	def FindRepeatPattern (self, trie = None, cur_lst = None):
		...
		by the number of repeats and by inverse on the STDV, is the biggest.

		In addition it test that the distances between the patterns is within
		some limit (this is the STDV, see __init__), and it will not return a
		patten that is overlapping.
		...
		if len (indexes) >= self.min_repeat and len (cur_lst) >= self.min_len:
			min_gap, max_gap, stdv_gap = self.__GetGapsInfo (indexes)
			#print '>>>', min_gap, len (cur_lst), len (indexes), stdv_gap, self.max_stdv_ptrn
			# Second Criteria - Pattern doesn't overlap and under STDV maximum
			if min_gap >= len (cur_lst) and stdv_gap <= self.max_stdv_ptrn:
				factor = len (cur_lst) * len (indexes) * (1 - stdv_gap)
	...
	def DoAllAtOnce (self, lst, lazy = True):
		"""
	...
	def FindFilter (self, look_ahead = 2, match_len = 2):
		...
		fltr, lst = self.__FindFilter (self._trie, look_ahead, match_len)
