...
class SesameConnector(object):
    ...
    def __init__(self, dburi):
        ...
        self._url = 'http://%s:%s/openrdf-sesame' % (self._host, self._port)
        http = httplib2.Http()
        try:
            ...
            raise ConnectionError(
                'Can not connect to Sesame repository, is it running?')
        for repo in SPARQLXMLResult(content).results():
            if repo['id']['value'] == self._name:
                break
        else:
            raise ConnectionError(
                'Sesame server has no repository: %s' % self._name)

    def connect(self):
        return SesameDatabase(self._url, self._name)
    
    def disconnect(self, db):
        del db._http

class SesameDatabase(object):
    implements(IDatabase)
    
    def __init__(self, sesame_url, db_name):
        self._url = sesame_url
        self._name = db_name
        self._http = httplib2.Http()
        
    def formats(self):
        return ['ntriples', 'rdfxml', 'turtle', 'n3', 'trix', 'trig']
    ...
    def contexts(self):
        ...
        return [c['contextID']['value'].split(':', 1)[1] for c in
                SPARQLXMLResult(content).results()]
    ...
    def count(self, context=None):
        if context is None:
            ...
            context = '?context='+quote('<context:%s>' % context)
        resp, content = self._http.request(
    ...
    def select(self, sparql):
        ...
        return SPARQLXMLResult(content)
    ...
    def ask(self, sparql):
        ...
        if content == 'true':
            ...
            raise DatabaseError('ASK query got unexpected result: %s' % content)
