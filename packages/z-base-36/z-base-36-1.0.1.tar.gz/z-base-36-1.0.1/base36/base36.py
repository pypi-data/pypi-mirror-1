...
vals = ''.join([chr(i) for i in range(36)])
...
def b2a(os):
    ...
    @return the contents of os in base-36 encoded form
    """
...
def b2a_l(os, lengthinbits):
    ...
    b2a_l() will generate a base-36 encoded string big enough to encode
    lengthinbits bits.  So for example if os is 3 bytes long and lengthinbits is
    17, then b2a_l() will generate a 4-character- long base-36 encoded string
    (since 4 chars is sufficient to encode all 2^17 values).  If os is 3 bytes
    long and lengthinbits is 21 (or None), then b2a_l() will generate a
    5-character string (since 5 chars are required to hold 2^21 values or 2^24
    values).  Note that if os is 3 bytes long and lengthinbits is 17, the least
    significant 7 bits of os are ignored.

    Warning: if you generate a base-36 encoded string with b2a_l(), and then
    someone else tries to decode it by calling a2b() instead of a2b_l(), then
    they will (potentially) get a different string than the one you encoded!  So
    use b2a_l() only when you are sure that the encoding and decoding sides know
    exactly which lengthinbits to use.  If you do not have a way for the encoder
    and the decoder to agree upon the lengthinbits, then it is best to use b2a()
    and a2b().  The only drawback to using b2a() over b2a_l() is that when you
    have a number of bits to encode that is not a multiple of 8, b2a() can
    sometimes generate a base-36 encoded string that is one or two characters
...
def num_octets_that_encode_to_this_many_chars(numcs):
    return log_floor(36**numcs, 256)

def num_chars_that_this_many_octets_encode_to(numos):
    return log_ceil(256**numos, 36)

def a2b(cs):
    """
    @param cs the base-36 encoded data (a string)
    """
