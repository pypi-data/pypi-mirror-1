...
import anydbm, errno, md5, os, stat, struct, sys
...
def dffem(VERBOSITY, IGNOREDIRS, HARDLINKEM, DELETEEM, MINSIZE, MAXSIZE, PROFILES, NOFOLLOWSYMLINKS, DIRS, WINDOWS):
    ...
    d1 = anydbm.open(os.path.join(tempd.name, TMPDB1FNAME), "n")
    tempd.register_file(d1)
    d2 = anydbm.open(os.path.join(tempd.name, TMPDB2FNAME), "n")
    tempd.register_file(d2)
    try:
        return _dffem(VERBOSITY, IGNOREDIRS, HARDLINKEM, DELETEEM, MINSIZE, MAXSIZE, PROFILES, NOFOLLOWSYMLINKS, DIRS, WINDOWS, d1, d2)
    finally:
        tempd.shutdown()

DUMPSEMPTYLIST = dumps([])
def _dffem(VERBOSITY, IGNOREDIRS, HARDLINKEM, DELETEEM, MINSIZE, MAXSIZE, PROFILES, NOFOLLOWSYMLINKS, DIRS, WINDOWS, d1, d2, next_power_of_k=mathutil.next_power_of_k):
    ...
    for (sizestr, thed1setstr,) in d1.iteritems():
        ...
        if (len(thed1set) > 1) or (len(thed1set) == 1 and PROFILES):
            ...
            # d3 = { k: md5hash, v: { k: idx, v: [ fname ] } }
            # on other platforms:
            # d3 = { k: md5hash, v: { k: devino, v: [ fname ] } }
            d3 = {}
            if WINDOWS:
                idx = 0
            ...
            for fname_or_devino in thed1set:
                ...
                if PROFILES:
                    print "%s:%s" % (md5h[:4], next_power_of_k(size, 2))
                ...
                if len(d4) == 2:
                    ...
                    for (iterdevino, iterfnames) in d4.iteritems():
                        if PROFILES:
                            print "identical; md5: %s, size: %s" % (md5h[:4], next_power_of_k(size, 2),)
                        else:
                            print "md5: %s, size: %s, fnames: %s" % (md5h, size, iterfnames,)
