...
def dffem(VERBOSITY, IGNOREDIRS, HARDLINKEM, DELETEEM, MINSIZE, PROFILES, DIRS):
    ...
    # First pass: build map from size, dev, inode number to filenames, and if PROFILES then print out the first 2 chars of the md5sum and size of each file.
            ...
            try:
                ...
                if not stat.S_ISLNK(thestat.st_mode) and thestat.st_size >= MINSIZE:
                    ...
                    if d2.has_key((thestat.st_dev, thestat.st_ino,)):
                        ...
                        if PROFILES:
                            print "%s:%s" % (md5it(open(fullfname, 'r'))[:2], thestat.st_size)
                    l.append(fullfname)
            ...
            for ((dev, ino,), fnames,) in d2.iteritems():
                ...
                if len(d4) == 2:
                    ...
                    for ((iterdev, iterino,), iterfnames,) in d4.iteritems():
                        if PROFILES:
                            print "identical; md5: %s, dev: %s, ino: %s, size: %s" % (md5it(open(onename, 'r'))[:2], iterdev, iterino, size,)
                        else:
                    ...
                    if PROFILES:
                        print "identical; md5: %s, dev: %s, ino: %s, size: %s" % (md5it(open(fnames[0], 'r'))[:2], dev, ino, size,)
                    else:
