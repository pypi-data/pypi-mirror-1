...
from swan_gui import Rhythm_path
...
class Swan:
    ...
    def setup_colormaps(self):
        ...
        self.known_cmaps = ('autumn', 'bone', 'cool', 'copper', 'flag',\
    ...
    def setup_axes(self):
        ...
        self.fig.clear()
        self.ax = [self.fig.add_axes((0.04, 0.2, 0.9,0.78))]
                       #self.fig.add_axes([0.80,0.76,0.17,0.20]),\
        self.ax.append(self.fig.add_axes((0.04,0.04,0.9, 0.12), sharex=self.ax[0]))
        self.ax[0].hold(True)
    ...
    def set_freqs(self):
        if self.f_pars['log?']:
            ...
            return arange(self.f_pars['start'], self.f_pars['stop'], self.f_pars['step'])
        ...
        if i > -1:
            if event.inaxes == self.ax[0]:
                ...
                if event.button == 1:                      # left click
                    crps.append(event.xdata,event.ydata)
    ...
    def path_2_rhythm0(self):
        i = self.rind                   # index of current rhythm path
        crps = self.rps[i]              # current rhythm path
        while len(self.rps[i].x) >0:
            self.rps[i].x.pop()
            self.rps[i].y.pop()
            self.rps[i].v.pop()
        L = self.get_xind(crps.px[-1]) - self.get_xind(crps.px[0]) + 1
        for k in range(len(crps.px) -1):
            p = polyfit(crps.px[k:k+2],crps.py[k:k+2],1)  # Linear approximation between two path points
            sy = arange(self.cwt.shape[0])            # eds surface y-indices
            jstart = self.get_xind(crps.px[k])
            jstop = self.get_xind(crps.px[k+1])
            self.ax[0].hold(True)
            for j in range(jstart,jstop):
                ...
                self.rps[i].x.append(self.time[j])
                self.rps[i].v.append(yslice[yl+tmp])
                self.rps[i].y.append(self.freqs[yl+tmp])
                if self.verbose:
                    if not j % 10:
