...
In some cases, when using version 1.1 or higher you can get a
UnicodeEncodeError when changing an issue or adding a response to it.
This is due to a change in how the description of an issue ends up in
the portal catalog.  I (Maurits) have seen this myself but noone else
reported it, so probably not many people are affected.  But there is a
migration for this that is safe for everyone to use.  For this and
other migrations visit the form at:

    http://your-plone-site/migrate-poi.html

If unsure, just run all steps there; they are safe to run more than
once.
