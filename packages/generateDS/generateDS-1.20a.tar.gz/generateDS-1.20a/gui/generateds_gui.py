...
class GeneratedsGui(object):
    ...
    def on_generate_menuitem_activate(self, menuitem, data=None):
        ...
        if result:
            ...
            cmd = self.create_command_line(params_dict)
            #print 'cmd: %s' % (cmd, )
    ...
    def trans_gui_2_obj(self):
        for item in ParamNameList:
            if item.get_name() == 'member_specs':
                ui_obj = self.ui_obj_dict['member_specs_combobox']
                ...
                ui_obj = self.ui_obj_dict[s2]
                method = getattr(ui_obj, 'get_%s' % item.get_access_action())
                value = method()
                setattr(self.params, s1, value)
    ...
    def trans_obj_2_gui(self):
        for item in ParamNameList:
            ...
            if ui_name == 'member_specs_combobox':
    ...
    def dump_params(self):
        print 'Params:'
        self.params.export(sys.stdout, 0, name_='session')
    ...
    def trans_params_2_dict(self):
        ...
        self.transform_1_param(params, pd, 'search_path', 'search-path', True)
    ...
    def create_command_line(self, params_dict):
        params_dict['exec_path'] = self.options.exec_path
        cmd = CmdTemplate % params_dict
        return cmd
    ...
    def run_command(self, cmd):
        spobj = subprocess.Popen(cmd, shell=True,
    ...
    def on_open_session_menuitem_activate(self, menuitem, data=None):
        self.session_filename = self.choose_filename(
    ...
    def on_output_superclass_chooser_button_clicked(self, button, data=None):
    ...
    def choose_filename(self, action=gtk.FILE_CHOOSER_ACTION_SAVE,
        patterns=(), confirm_overwrite=False):
        filename = None
...
def main():
    ...
    parser.add_option("--exec-path",
        ...
        help="path to python executable generated in command line."
            "  default: python",
        )
##     parser.add_option("--impl-schema",
##         type="string", action="store",
##         dest="impl_schema",
##         help="name of schema that defines implementation names "
##             "if not embedded.",
##         )
