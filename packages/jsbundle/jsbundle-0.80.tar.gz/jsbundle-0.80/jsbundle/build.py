...
class DocumentNotRight(Exception):
    pass
...
class DocScanner(object):
    ...
    def next(self):
        for line in self.lines:
            if line.strip() == '':
                yield ''
            elif not line.startswith(self.indent.rstrip(' ')):
                print "%r - %r" % (line, self.indent.rstrip(' '))
                raise DocumentNotRight("Indent doesn't match previous line in %s" % self.filename)
            yield line[len(self.indent):]
...
class Builder(object):
    """
    Builder instances scan() for javascript files, and then allows an interface
    for compiling source, documentation, and tests.
    
    see: build() at the bottom for an example on how to use a Builder.
    """
    ...
    def get_modules(self, require=None):
        """
        Returns a list of the modules, and if *modules* is specified, it will
        return a list of only those modules and the ones they require.
        """
        include = []
        complete = set()
        if require:
        ...
        while require:
            ...
            if r in self.modules:
                ...
                for req in m.get_requirements():
                    if req not in complete:
                        require.add(req)
                
            elif r in self.entries:
                req = self.entries[r].module.name
                if req not in complete:
                    require.add(req)
    ...
    def compile_tests(self, out, require=None):
        ...
        for module in self.get_modules(require):
            if module.name not in self.tests:
                print module.name, "is not tested."
                continue
    ...
    def process_section(self, module, section):
        ...
        for line in DocScanner(lines, section):
            if line.strip().startswith('@'):
                directive, rest = line[line.find('@'):].split(None, 1)
                if directive == '@requires':
                    entry.requires.update( rest.split(None) )
                    continue
            entry.append(line)
...
def build( name,
           ...
           scan = 'src' ):
    """
    Builds javascript files found, recursively, at the path *src*, and
    searches *test_src* for tests.  If modules is specified, it can be a list
    of modules to build.
    ...
    *tests* - the path to the tests build.
    ...
    builder = Builder(src_match=src_match, test_match=test_match)
    ...
    hash = builder.hash(modules)
    context = {'name': name, 'hash': hash[-8:], 'fullhash': hash}
    
    if build:
        builder.compile(build % context, require=modules)
    if min:
        builder.compile_min(min % context, require=modules)
    if docs:
        builder.compile_docs(docs % context, require=modules)
    if tests:
        builder.compile_tests(tests % context, require=modules)
