...
class BzrRecipe(object):
    ...
    def __init__(self, buildout, name, options):
        ...
        if shared_repo not in ('True', 'False', 'true', 'false'):
            self.logger.error(
    ...
    def install(self):
        ...
        if not os.path.isdir(root_dir):
            if self.options.get('shared-repo', 'false') in ('True', 'true'):
                cmd = BZR + ' init-repo'
    ...
    def bzr_get(self, branch):
        ...
        if not stderr_lines or not stderr_lines[-1].startswith('Branched'):
            self.logger.error(
                'Error calling bazaar in "%s", detailed bzr output follows:\n"""\n%% %s\n\n%s\n%s"""',
                branch['egg'], cmd, stdout, stderr)
            raise zc.buildout.UserError('Error calling bzr')

        # Now, let's go to the working dir for pulling
        ...
        try:
            ...
            if branch.get('revspec') is not None:
                cmd += ' -r%(revspec)s' % branch
            success = self.bzr_pull(cmd, branch)
    ...
    def bzr_update(self, branch):
        ...
        if stderr or match is None:
            ...
            self.logger.error(
                'Update failed in "%s" (not a bzr branch, or no pull location?), detailed bzr output follows:\n"""\n%% %s\n\n%s\n%s"""',
                branch['egg'], cmd, stdout, stderr)
            return
    ...
    def bzr_pull(self, cmd, branch):
        ...
        # an error about the case, to notify the user.
        self.logger.error(
            'Update failed in "%s", detailed bzr output follows:\n"""\n%% %s\n\n%s\n%s"""',
            branch['egg'], cmd, stdout, stderr)
        return
