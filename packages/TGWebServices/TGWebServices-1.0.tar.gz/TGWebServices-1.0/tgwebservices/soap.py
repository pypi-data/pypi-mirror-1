...
from tgwebservices.runtime import primitives, ctvalues, typedproperty, unsigned

namespace_expr = re.compile(r'^\{.*\}')

_simplevalues = set([int, long, basestring, str, unicode, float])
...
@soap_value.when("isinstance(value, list)")
...
class SoapController(controllers.Controller):
    ...
    @tgexpose("genshi:tgwebservices.templates.soap",
              content_type="text/xml", format="xml")
    def index(self):
        ...
        params = {}
        for paramelem in body.getchildren():
            paramname = namespace_expr.sub("", paramelem.tag)
            params[paramname] = paramelem.text
    ...
    def api_wsdl(self, xmlsoap=False):
        ...
        ct.sort(key=lambda item: item._type_dependents, reverse=True)
