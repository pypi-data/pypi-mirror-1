...
def test_wsdl_with_ints():
    ...
    output = cherrypy.response.body[0]
    print output
    assert cherrypy.response.headers["Content-Type"] == "text/xml"
    root = et.fromstring(output)
...
def test_call_to_nested_controller():
    ...
    print output
    assert output == """<soapenv:Envelope xmlns="http://foo.bar.bazsoap/types" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <soapenv:Body>
        <innerFooGetFancyResponse><result xsi:type="FancyValue"><age xsi:type="xsd:int">42</age><computed xsi:type="xsd:string">Hello!</computed><name xsi:type="xsd:string">Mr. Bean</name></result></innerFooGetFancyResponse>
    </soapenv:Body>
</soapenv:Envelope>"""
