...
static PyObject* 
PyObjC_recoverPickleArchive(
	PyObject* self __attribute__((__unused__)),
	PyObject* args)
{
	OC_PickleUnarchiver* unarchiver = nil;
	NSObject* root = nil;
	PyObject* result = NULL;

	PyObjC_DURING
		unarchiver = [[OC_PickleUnarchiver alloc] initWithState:args];
		root = [unarchiver rootObject];
	PyObjC_HANDLER
		PyObjCErr_FromObjC(localException);
		root = nil;
	PyObjC_ENDHANDLER

	if (root == nil && PyErr_Occurred()) {
		if (unarchiver) {
			[unarchiver release];
		}
		return NULL;
...
static PyMethodDef mod_methods[] = {
	...
	{ "_recoverPickleArchive", (PyCFunction)PyObjC_recoverPickleArchive,
		METH_VARARGS,
		"used by the implementation of pickling for Objective-C "
		"objects. Don't call directly.",
	},
...
init_objc(void)
{
	...
	PyObjC_recoverArchiveFunc = PyObject_GetAttrString(m, 
			"_recoverPickleArchive");
}
