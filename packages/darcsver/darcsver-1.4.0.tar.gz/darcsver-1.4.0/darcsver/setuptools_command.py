...
class DarcsVer(setuptools.Command):
    ...
    def run(self):
        if self.loud and not self.quiet:
            ...
            loud = False
        (rc, verstr) = darcsvermodule.update(self.project_name, self.version_file, self.count_all_patches, abort_if_snapshot=self.abort_if_snapshot, loud=loud, EXE_NAME="setup.py darcsver")
