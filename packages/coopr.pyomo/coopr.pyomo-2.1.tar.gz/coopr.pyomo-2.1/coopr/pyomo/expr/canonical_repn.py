...
__all__ = ['generate_canonical_repn']
...
def as_expr(rep, model, ignore_other=False):
...
def repn_mult(r1, r2, model, coef=1.0):
    rep = {}
    for d1 in r1:
        for d2 in r2:
    ...
    #
    if None in r1:
        rep[None] = as_expr(r2, model, ignore_other=True) * copy.deepcopy(r1[None])
        #print "YY"
        #r1[None].pprint()
        #as_expr(r2, model, ignore_other=True).pprint()
        #print "YY"
        if None in r2:
            rep[None] += copy.deepcopy(r1[None])*copy.deepcopy(r2[None])
    if None in r2:
        if None in rep:
            rep[None] += as_expr(r1, model, ignore_other=True) * copy.deepcopy(r2[None])
        else:
            rep[None]  = as_expr(r1, model, ignore_other=True) * copy.deepcopy(r2[None])
    #
...
def collect_canonical_repn(exp, model):
    #ans = do_collect_canonical_repn(exp, model)
    #exp.pprint()
    #print "HERE",ans
    #return ans
    #def do_collect_canonical_repn(exp, model):
    ...
    if isinstance(exp,expr.Expression):
        ...
        if isinstance(exp,expr._SumExpression):
            ...
            for i in range(len(exp._args)):
                repn = repn_add(repn, collect_canonical_repn(exp._args[i], model), coef=exp._coef[i] )
            return repn
        #
        # Product
        #
            ...
            for e in exp._numerator:
                repn = repn_mult(repn, collect_canonical_repn(e, model), model)
            return repn
        #
        # ERROR
        #
    ...
    #
        ...
        temp_var = { 1: {frozendict({exp.id:1}):1.0} }    
        return temp_var
    #
    # ERROR
    #
...
#
# This function generates a canonical representation for a Pyomo expression
#
def generate_canonical_repn(expr, model):
