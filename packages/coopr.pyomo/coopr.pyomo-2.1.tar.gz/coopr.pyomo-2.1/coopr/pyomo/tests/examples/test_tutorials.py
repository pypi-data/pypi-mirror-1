...
class PyomoTutorials(pyutilib.th.TestCase):
    ...
    def test_data(self):
        ...
        self.failUnlessFileEqualsBaseline(currdir+"data.log", tutorial_dir+"data.out")

    def test_excel(self):
        if not _win32com:
           return
        pyutilib.misc.run_file(tutorial_dir+"excel.py", logfile=currdir+"excel.log", execdir=tutorial_dir)
        self.failUnlessFileEqualsBaseline(currdir+"excel.log", tutorial_dir+"excel.out")

    def test_set(self):
        pyutilib.misc.run_file(tutorial_dir+"set.py", logfile=currdir+"set.log", execdir=tutorial_dir)
        self.failUnlessFileEqualsBaseline(currdir+"set.log", tutorial_dir+"set.out")

    def test_table(self):
        pyutilib.misc.run_file(tutorial_dir+"table.py", logfile=currdir+"table.log", execdir=tutorial_dir)
        self.failUnlessFileEqualsBaseline(currdir+"table.log", tutorial_dir+"table.out")

    def test_param(self):
        pyutilib.misc.run_file(tutorial_dir+"param.py", logfile=currdir+"param.log", execdir=tutorial_dir)
        self.failUnlessFileEqualsBaseline(currdir+"param.log", tutorial_dir+"param.out")
