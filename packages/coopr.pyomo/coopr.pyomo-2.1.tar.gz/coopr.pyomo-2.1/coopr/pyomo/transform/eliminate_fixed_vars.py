...
from coopr.pyomo.base.var import _VarBase, _VarValue


class EliminateFixedVars(pyutilib.plugin.core.SingletonPlugin):
    """
    This plugin relaxes integrality in a Pyomo model.
    """

    pyutilib.plugin.core.implements(IModelTransformation)
    ...
    def apply(self, model, **kwds):
        ...
        for i in range(len(M.variables())):
            ...
            del M._var[ i ]
            if var.fixed:
                del M._name_varmap[ var.label ]
                del var.var._varval[ var.index ]
            else:
                M._var[ ctr ] = var
                var._old_id = var.id
                var.id = ctr
    ...
    def _fix_vars(self, expr, model):
        ...
        for i in range(len(expr._args)):
            ...
            elif (isinstance(expr._args[i],_VarBase) or isinstance(expr._args[i],_VarValue)) and expr._args[i].fixed:
                if expr._args[i].value != 0.0:
                    _args.append( NumericConstant(value=expr._args[i].value) )
            else:
