...
class RichImageField(BlobField):
    ...
    def getAvailableCrops(self, instance):
        """ """
        #FIX-ME make it more complete...like getAvailableSizes
        return self.crops
    ...
    def editCrop(self, instance, id, x1, y1, x2, y2, scale=1):
        """creates the crops and save them
        """
        self._createCrop(instance, id, x1, y1, x2, y2, scale=scale)

    def _createCrop(self, instance, id, x1, y1, x2, y2, scale=None):
        #Calculate scale
        selectedWidth = float(int(x2)-int(x1))
        cropWidth = self.crops[id][2]
