...
class ProjectEnvironment(AnyEntity):
    ...
    @property
    def project(self):
        if 'has_apycot_environment' in self.req.vreg.schema:
            return self.reverse_has_apycot_environment[0]
...
class TestConfig(TestConfigGroup):
    ...
    def all_check_results(self):
        rset = self.req.execute('Any MAX(X), XN GROUPBY XN, EXB ORDERBY XN '
...
class TestExecution(AnyEntity):
    ...
    def dc_title(self):
        return self.req._('Execution of %(config)s on %(date)s') % {
    ...
    def previous_entity(self):
        rql = ('Any X,C ORDERBY X DESC LIMIT 1 '
               ...
               'X eid < %(x)s')
        rset = self.req.execute(rql, {'c': self.configuration.eid, 'x': self.eid,
        ...
        if result is not None:
            rql = '''DELETE CheckResultInfo CRI
                     WHERE CRI for_check TE,
                           ...
                           CRI type "archive",
                           CRI value %(name)s'''
            self.req.execute(rql, {'eid': self.eid, 'name': result})
        return result
