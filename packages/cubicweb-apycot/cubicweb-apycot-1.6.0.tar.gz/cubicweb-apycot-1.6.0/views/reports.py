...
class ProjectEnvironmentLatestExecutionSummary(EntityView):
    ...
    def call(self, title=None):
        configs = []
        map(configs.extend, (e.reverse_use_environment for e in self.rset.entities()))
    ...
    def multiple_configs_report(self, configs, title, incontext):
        ...
        self.w(u'<h3>%s</h3>\n' % (title or self.req._(self.title)))
        if not configs:
            self.w(self.req._('no test execution results to display'))
        else:
            self.req.add_css('cubes.apycot.css')
            self.w(multiple_configs_table(self.req._, configs,
                                          incontext=incontext))
        self.w(u'</div>')
...
class TestConfigLatestExecutionSummary(ProjectEnvironmentLatestExecutionSummary):
    ...
    def call(self, title=None):
        self.multiple_configs_report(list(self.rset.entities()),
                                     title, incontext=False)

    def cell_call(self, row, col, title=None):
        self.multiple_configs_report([self.rset.get_entity(row, col)],
                                      title, incontext=False)
...
class TestExecutionSummary(EntityView):
    ...
    def cell_call(self, row, col):
        ...
        self.w(u'<tr><th>%s</th><th>%s</th></tr>'
               % (self.req._('name'), self.req._('status')))
