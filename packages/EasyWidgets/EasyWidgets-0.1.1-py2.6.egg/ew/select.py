...
class SelectField(InputField):
    ...
    def option_object_for(self, value, option):
        ...
        if option.html_value is None:
            option.html_value = self.from_python(option.py_value, None)
        return option
...
class Option(Widget):
    ...
    def __init__(self, **kw):
        ...
        if self.html_value is None and self.py_value:
            self.html_value = unicode(self.py_value)
