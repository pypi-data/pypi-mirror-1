...
def cartesian_product(seqs):
    """Returns Cartesian product of seqs.

    WARNING: creates the whole list of combinations in memory at once.
    
    Adapted from Byran Olson's recipe:

    http://groups.google.com/groups?hl=en&lr=&selm=75el96%246va%241%40news.eecs.umich.edu&rnum=10    

    Combination of this and _cartesian_product_inner (below) gives best overall
    performance for many, small lists or few, large lists.
    """
    if not seqs:
        return [()]
    prod = _cartesian_product_inner(seqs[:-1])
    return [x + (y,) for x in prod for y in seqs[-1]]

def _cartesian_product_inner(seqs):
    """Returns Cartesian product of seqs.

    WARNING: creates the whole list of combinations in memory at once.
    
    Adapted from Evan Simpson's recipe:

    http://groups.google.com/groups?hl=en&lr=&selm=75el96%246va%241%40news.eecs.umich.edu&rnum=10    

    Combination of this recipe and cartesian_product (above) gives best overall
    performance for many, small lists or few, large lists.
    """
    listlens = map(len, seqs)
    num_lists = len(seqs)
    ilist = [0] * num_lists
    productlen = reduce(mul, listlens, 1)
    product = [None] * productlen
    for i in range(productlen):
        product[i] = tuple(map(getitem, seqs, ilist))
        incr = num_lists -1
        while incr >= 0:
            n = ilist[incr] + 1
            if n == listlens[incr]:
