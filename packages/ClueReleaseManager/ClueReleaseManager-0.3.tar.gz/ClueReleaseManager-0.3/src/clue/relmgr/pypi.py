...
class PyPi(object):
    ...
    def update_metadata(self, name, **kwargs):
        ...
        if len(distro) == 0:
            if not self.has_role(None, ADD_DISTRO_ROLE):
    ...
    def upload_files(self, name, content_items, **kwargs):
        distro_id = utils.make_distro_id(name)
    ...
    def get_distros(self):
        ses = self.sessionmaker()
        distros = [x for x in ses.query(model.SQLDistro)
                   if self.has_role(x.distro_id,
                                    READER_ROLE, MANAGER_ROLE,
                                    model.OWNER_ROLE)]
        distros.sort(lambda x, y: cmp(x.name, y.name))
        return distros
