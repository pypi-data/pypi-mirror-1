...
class SecurityManager(object):
    ...
    def update_users_groups(self, username, groups):
        ses = self.sessionmaker()
        user = ses.query(SQLUser).filter_by(username=username).first()
        groupdict = {}
        for g in user.groups:
            groupdict[g.groupname] = g

        if set(groupdict.keys()) == set(groups):
            self.logger.info('Groups for user "%s" unchanged' % username)
            return
...
class IndexManager(object):
    ...
    def get_indexes(self, distro_id):
        ...
        for x in indexq.filter_by(distro_id=distro_id):
            ...
            if d is None:
                raise ValueError('Cannot find target distro: %s'
                                 % x.target_distro_id)
