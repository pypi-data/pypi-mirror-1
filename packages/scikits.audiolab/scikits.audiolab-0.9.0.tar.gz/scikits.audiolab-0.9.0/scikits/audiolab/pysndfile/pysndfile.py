...
out of luck, because its implementation of POSIX open is broken)"""
    ...
    def read_frames(self, nframes, dtype = N.float64):
        ...
        warnings.warn("Output of read_frames will change after 0.9: instead "
                "of one channel/column, read_frames will return an array with "
                "one row per channel convention, to fit more with numpy "
                "convention (algorithm applied along last axis by default)",
                DeprecationWarning)
    ...
    def write_frames(self, input, nframes = -1):
        ...
        warnings.warn("Input format of write_frames will change after 0.9: instead "
                "of one channel/column, write_frames will take an array with "
                "one row per channel, to fit more with numpy "
                "convention (algorithm applied along last axis by default)",
                DeprecationWarning)
