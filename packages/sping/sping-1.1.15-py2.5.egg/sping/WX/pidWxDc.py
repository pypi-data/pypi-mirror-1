...
class SpingWxDc(sping.pid.Canvas):
    ...
    def _getWXcolor(self, color, default = None):
        ...
        if color is not None:
            if color == sping.pid.transparent:
                ...
                # could use color.toRGBHex() -cwl
                return wx.Color(color.red * 255,
                                color.green * 255,
                                color.blue * 255)
    ...
    def _getWXfont(self, font):
        ...
        #  Sping fonts are matched to wx.Font families.  While it is
        #  possible to match them to individual fonts, this is
        #  difficult to do in a platform independant way
    ...
    def drawString(self, s, x,y, font=None, color=None, angle=None):
        ...
        if angle is not None:
          ...
          for l in range(0,len(lines)):
            self.dc.DrawText(lines[l], x,
                             y - self.fontAscent(font) + l*line_height)


    def _drawRotatedString(self, lines, x, y,
                           font = None, color = None, angle=0):
    ...
    def drawPolygon(self, pointlist, edgeColor=None,
        ...
        # drawPolygon: For fillable shapes,
        #   edgeColor defaults to self.defaultLineColor,
        #   edgeWidth defaults to self.defaultLineWidth,
        #   and fillColor defaults to self.defaultFillColor.
        # Specify "don't fill" by passin fillColor=transparent.
        ...
        # Because wxPython automatically closes polygons, the polygon
        # fill and the border are drawn seperately, so open polygons
        # will display correctly
        self.dc.SetPen(wx.TRANSPARENT_PEN)
        self.dc.SetBrush(self._getWXbrush(fillColor, self.defaultFillColor))

        #  Workaround : Sping will pass mixed lists of lists and
        #  2-tuples instead of just 2-tuples.  Therefore, pointlist
        #  must be re-created as only 2-tuples
