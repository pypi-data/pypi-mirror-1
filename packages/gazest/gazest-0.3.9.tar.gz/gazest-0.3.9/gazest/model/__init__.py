...
def full_commit():
    if c.site_readonly:
        # TODO: a better message would be nice
        abort(403)
    db_sess.flush()
    db_sess.commit()
...
revnodes_table = Table('revnodes', meta,
                       ...
                       Column("creat_ip", String, nullable=False),
                       Column("user_id", Integer, ForeignKey("users.id"), 
                              nullable=True),
                       Column("creat_date", DateTime, nullable=False),
...
class RevNode(object):
    def __init__(self, creat_ip=None, creat_date=None, **kwargs):
        self.creat_ip = creat_ip or get_client_ip()
        self.creat_date = creat_date or datetime.utcnow()
...
mapper(RevNode, revnodes_table,
              properties={
    'user':relation(User)
    })
mapper(RevArc, revarcs_table, properties={
...
mapper(Namespace, namespaces_table,
...
mapper(Confirmation, confirmations_table,
              properties=dict(user=relation(User)))


mapper(Gazmail, gazmails_table, 
...
mapper(AbuseReport, abuse_reports_table,
              properties=dict(reporter=relation(User),
                              rev=relation(RevNode)))

__all__ = ["db_sess", "User", "Page", "Namespace", "RevNode",
           "Boycott", "AbuseReport"]
