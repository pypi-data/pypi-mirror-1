...
static PyObject *
bitarray_multiply(bitarrayobject *self, PyObject *v)
{
    idx_t vi;
    
    if (!ISINDEX(v)) {
	PyErr_SetString(PyExc_TypeError,
			"integer value expected for multiplication");
	return NULL;
    }
    getIndex(v, &vi);
    
    if (vi <= 0) {
	delete(self, 0, self->nbits);
	Py_RETURN_NONE;
    }
    if (vi == 1)
	Py_RETURN_NONE;
    ...
    newbitsize = self->nbits * vi;
    check_overflow(newbitsize);
    ...
    for (j = 1; j < vi; j++)
	bitcopy(self, j * self->nbits, self, 0, self->nbits);
    
    self->nbits = newbitsize;
    Py_RETURN_NONE;
}

PyDoc_STRVAR(multiply_doc,
"_multiply(value)\n\
\n\
Perform in-place multiplication (repetition) of bitarray, value must be\n\
and integer.  This method is not meant to be called directly, but from\n\
...
directly, but from __and__, __iand__, etc");
...
bitarray_encode(bitarrayobject *self, PyObject *args)
{
    ...
    while ((symbol = PyIter_Next(iter)) != NULL) {
	    ...
	    sprintf(buff, "no symbol %s in prefix code",
		    PyString_AsString(PyObject_Repr(symbol)));
	    PyErr_SetString(PyExc_ValueError, strdup(buff));
	    Py_DECREF(symbol);
	    Py_DECREF(iter);
            return NULL;
...
btree_new(void)
{
    ...
    list = PyList_New(2);
    PyList_SetItem(list, 0, PyList_New(0));
...
btree_insert(PyObject *tree, PyObject *symbol, PyObject *b)
{
	...
	if (IS_EMPTY_LIST(PyList_GetItem(tree, vi)))
	    PyList_SetItem(tree, vi, btree_new());
	
	/* a = b[1:], and insert symbol */
	a = bitarray_copy(bb);
...
bitarray_decode(bitarrayobject *self, PyObject *codedict)
{
    ...
    res = PyList_New(0);
    while (1) {
	symbol = btree_traverse(iter, tree);
	if (symbol == NULL)
	    break;
	...
	if (IS_EMPTY_LIST(symbol)) {
	    ...
	    Py_DECREF(res);
	    res = NULL;
	    /* XXX res sould contain symbols read so far, but then
	       res is not NULL an no exception is raised.
	    */
	    goto finish;
	}
	PyList_Append(res, symbol);
	Py_INCREF(symbol);
    }
finish:
    Py_XDECREF(tree);
...
bitarray_methods[] = {
    ...
    {"length",       (PyCFunction) bitarray_length,      METH_NOARGS,
     length_doc},
    {"_multiply",    (PyCFunction) bitarray_multiply,    METH_O,
     multiply_doc},
