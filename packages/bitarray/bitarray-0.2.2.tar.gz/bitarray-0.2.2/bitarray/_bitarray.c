...
/* Reverse the order of bite in each byte of the buffer */
static void
...
static inline int
set_item(bitarrayobject *self, idx_t i, PyObject *item)
{
	long vi;
...
{
	if (self->nbits + 1 > BITS(self->ob_size) &&
...
extend_bitarray(bitarrayobject *self, bitarrayobject *b)
{
	...
	/* When the bitarray has no unused bits, and the extending bitarray 
           has the same endianness, simply copy the memory */
	if (self->nbits % 8 == 0 &&
	    self->endian == b->endian) {
		memcpy(self->ob_item + self->ob_size, /* first new byte */
		       b->ob_item,
		       b->ob_size);
	}
	/* Do it the slow way */
...
extend_list(bitarrayobject *self, PyObject *list)
{
	...
	self->nbits += addbits;
	if (resize(self, BYTES(self->nbits)) == -1)
		return -1;
...
"index(x)\n\
\n\
Return index of first occurrence of x in the bitarray.  It is an error\n\
when x does not occur in the bitarray");
...
bitarray_buffer_info(bitarrayobject *self)
{
	...
	PyTuple_SET_ITEM(res, 3,
		PyInt_FromLong((long) (BITS(self->ob_size) - self->nbits)));
	return res;
}
...
"buffer_info()\n\
\n\
Return a tuple (address, size, endianness, unused) giving the current\n\
...
/* ----------- "private" methods used in the Python layer -------------- */
...
bitarray_sscr3(bitarrayobject *self, PyObject *args)
{
	...
	size_t newsize = BYTES(newbitsize);
	char *item = self->ob_item;
...
bitarray_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	...
	idx_t len;                            /* only used in int, long */
...
bits2bytes(PyObject *self, PyObject *v)
{
	...
	return PyLong_FromLongLong((idx_t) BYTES(n));
}
