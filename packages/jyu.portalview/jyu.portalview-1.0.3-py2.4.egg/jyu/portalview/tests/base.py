...
class ContentTypeTestCase(TestCase):
    ...
    name = None # override with content type's name
    schema = None # override with content type's archetype schema
    ...
    def assertAction(self, action_id, params):
        actions = [action for action in self.types.get(self.name).listActions()
                   if action.id == action_id]
        self.assertEquals(1, len(actions), "Content type doesn't have action ``%(action_id)s``." % vars())
        action = actions[0]

        for key in params:
            {'title':          lambda x, y: x.assertEquals(y, action.title),
             'category':       lambda x, y: x.assertEquals(y, action.category),
             'condition_expr': lambda x, y: x.assertEquals(y, action.condition
                                                           and action.condition.text or ''),
             'url_expr':       lambda x, y: x.assertEquals(y, action.action
                                                           and action.action.text) or '',
             'visible':        lambda x, y: x.assertEquals(y, action.visible),
             'permission':     lambda x, y: x.assertEquals(y, action.getPermissions()),
             }[key](self, params[key])
