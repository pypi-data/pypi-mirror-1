...
class Recipe(object):
    ...
    def compileMySQLPython(self):
        ...
        if not os.path.exists(dest):
            os.makedirs(dest)
        assert subprocess.call([self.python, "setup.py", "build"],
    ...
    def addScriptWrappers(self):
        ...
        assert subprocess.call([os.path.join(dir, "mysql_install_db")]) == 0
        # starting shouldn't be done in a subprocess, need to find another way
        #self.logger.info("Starting mysqld_safe")
        #assert subprocess.call([os.path.join(dir, "mysqld_safe")]) == 0
