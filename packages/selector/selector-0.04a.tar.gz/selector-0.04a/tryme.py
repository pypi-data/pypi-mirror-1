...
from selector import Selector, status_app, SimpleSyntaxParser
...
structure2 = [('/yo/{name}', {'GET': say_hello}),
              ('/whatup/{name}', {'GET': say_hello})]

s = Selector(structure, prefix="^/fluptest")
s.prefix = '/fluptest'
s.slurp(structure2, parser=SimpleSyntaxParser())
s.prefix = '^/fluptest'
#s.add(r'/hello/(?P<name>[a-zA-Z]+)$', {'GET': say_hello}, GET=status_app('200 OK', 'new hello'))
