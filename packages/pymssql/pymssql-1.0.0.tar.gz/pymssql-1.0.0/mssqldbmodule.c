...
PyObject *_quote_or_flatten(PyObject *data) {
	...
	if (PyList_Check(data)) {
		PyObject *str;
		int i, len = PyList_GET_SIZE(data);
	...
	if (PyTuple_Check(data)) {
		PyObject *str;
		int i, len = PyTuple_GET_SIZE(data);
...
static PyObject *_mssql_nextresult(_mssql_connection *self, PyObject *args) {
	...
	self->last_dbresults = 0;  // force call to dbresults()
...
charset  - character set name to set for the connection. [XXX BROKEN]\n\
...
static PyObject *_mssql_connect(_mssql_connection *self, PyObject *args,
	...
	static char *kwlist[] = { "server", "user", "password", "trusted",
			"charset", "database", NULL };

	if(!PyArg_ParseTupleAndKeywords(args, kwargs, "s|ssiss:connect", kwlist,
...
static PyObject *_mssql_connection_repr(_mssql_connection *self) {
	...
	if(self->connected)
		snprintf(buf, 100, "<Open mssql connection at %lx>", (long) self);
	else
		snprintf(buf, 100, "<Closed mssql connection at %lx>", (long) self);
...
static PyMemberDef _mssql_connection_members[] = {
	...
	{ "debug_queries", T_INT, _MSSQL_OFF(debug_queries), WRITE_RESTRICTED,
...
static PyObject *_mssql_row_iterator_repr(_mssql_row_iterator *self) {
	...
	snprintf(buf, 100, "<_mssql.MssqlRowIterator at %lx>", (long) self);
	return PyString_FromString(buf);
}
...
/* This is a helper function, which does most work needed by any
   ...
   Actually it returns Py_None, but there's no need to DECREF it. Also,
   we can't use ints here because of assert_connected and other macros. */

PyObject *format_and_run_query(_mssql_connection *self, PyObject *args) {
	RETCODE rtc;
	char *query;
	PyObject *format = NULL, *params = NULL, *formatted, *tuple;
	...
	if (params != NULL) {
		...
		query = PyString_AsString(formatted);
		Py_DECREF(formatted);
	} else {
...
PyObject *get_row(_mssql_connection *conn, int rowinfo) {
	...
	for (col = 1; col <= conn->num_columns; col++) {     // do for all columns
		...
		switch (coltype) {                        // else we have data
			...
			case SQLVARCHAR: case SQLCHAR: case SQLTEXT:
				// on *nix with FreeTDS it's FreeTDS who converts data
				// into UTF-8. On Windows we must convert them ourselves, using
				// encoding provided by the user during connect()
#ifdef MS_WINDOWS
				if (*conn->charset)  // if charset given then decode
...
#else
				PyTuple_SET_ITEM(record,col-1,PyUnicode_Decode((char *) data,
						GET_LEN(dbproc,rowinfo,col), "utf8", NULL));
#endif
				break;
