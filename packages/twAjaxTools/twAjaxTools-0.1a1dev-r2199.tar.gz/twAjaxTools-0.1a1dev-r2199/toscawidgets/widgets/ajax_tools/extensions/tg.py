...
def exception_handler(func, args, kw):
    '''
    This decorator is used to handle exceptions raised by the validation
    decorator or the controller's method.
    
    It was mainly developed to catch some global exceptions
    and display some useful output to the user.
    
    Never forget these points:
        1. Remember to rollback!
        2. Remember to rollback!
        3. Remember to rollback!
    
    Oh, and don't forget to rollback.
    
    You only need to rollback when you handle an exception.
    The ones that aren't caught should trigger the rollback 
    on the expose decorator.
    '''
...
def ajax_form_connector(func, *args, **kw):
    ...
    _exception_handler = config.get('ajaxtools.exception_handler', 
                                   exception_handler)
    
    output = None
    redirect = None
    try:
        output = _exception_handler(func, args, kw)
