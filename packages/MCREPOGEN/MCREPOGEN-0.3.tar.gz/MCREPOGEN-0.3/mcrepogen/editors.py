...
class Patcher(Editor):
    ...
    Zero files are changed with a probability p_no_change.

    If some files are changed, then the number changed is a geometric random
    variable with mean 1/p.
    ...
    equal to the number of lines in the hunk.

    The replacement lines have length uniformly distributed between 0 and
    max_line_length
    """

    default_parameters = {'p_no_change': 0.013,
                          'p': 0.46,
                          'p_line_changed': .01,  #no data yet
                          'max_line_length': 72,
                         }
    ...
    def edit(self, input_tree):
        ...
        if mtrand.random() < self.parameters['p_no_change']:
            return input_tree
        # select files that have changed
        n_changed = mtrand.geometric(self.parameters['p'])
