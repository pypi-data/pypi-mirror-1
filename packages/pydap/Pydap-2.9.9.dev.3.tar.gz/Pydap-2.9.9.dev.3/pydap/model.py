...
from pydap.lib import quote, walk
...
class SequenceType(StructureType):
    ...
    def __setitem__(self, key, item):
        ...
        # Increment ``nest_level`` in nested sequences.
        for var in walk(item, SequenceType):
            item.nest_level += 1
    ...
    def _set_data(self, data):
        self._data = data
        for var in self.walk():
            # Try to acess child data using recarray syntax, otherwise
            # fallback to simple column access.
            try:
                var.data = data[var.name]
            except TypeError:
                i = self._keys.index(var.name)
                var.data = get_rows(data, itemgetter(i), self.nest_level)
    def _get_data(self):
        return self._data
    data = property(_get_data, _set_data)
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, basestring) and key in self:
            ...
            try:
                ...
                func = lambda line: tuple([line[k] for k in out._keys])
                out.data = get_rows(self.data, func, self.nest_level)
            return out
        else:
            out = copy.deepcopy(self)
...
def get_rows(data, func, level):
    if level == 1:
        return map(func, data)
    else:
        return [get_rows(value, func, level-1) for value in data]
