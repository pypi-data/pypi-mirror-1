...
class StructureType(odict, DapType):
    ...
    def __getitem__(self, key):
        return odict.__getitem__(self, quote(key))

    def __setitem__(self, key, item):
        odict.__setitem__(self, quote(key), item)
...
class SequenceType(StructureType):
    ...
    _nesting_level = 1
    def __setitem__(self, key, item):
        StructureType.__setitem__(self, quote(key), item)

        # Increment ``_nesting_level`` in nested sequences.
        for var in walk(item, SequenceType):
            item._nesting_level += 1

        if self._data is not None:
            self.data = self.data[ tuple(self.keys()) ]
...
def get_row(data, col, level):
    if level == 1:
        return [ line[col] for line in data ]
    else:
