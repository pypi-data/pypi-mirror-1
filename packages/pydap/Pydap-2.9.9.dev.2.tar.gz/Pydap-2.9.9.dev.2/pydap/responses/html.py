    ...
    def __call__(self, environ, start_response):
        ...
        def serialize(dataset):
            context = {
                ...
                'title': 'DODS server: %s' % dataset.name,
                'responses': [r.name for r in iter_entry_points('dap.response')
...
def redirect(environ):
    ...
    if not query.rstrip('?&'):
        raise HTTPBadRequest('You did not select any variables.')
