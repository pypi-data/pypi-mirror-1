...
class SequenceProxy(VariableProxy):
    """
    ...
    """
    def __iter__(self):
        # This hack is necessary for Structures inside Sequences. In this case,
        # the Structure iters over the SequenceProxy trying to unzip the data
        # for each of its children. We detect this here, and return a list of
        # SequenceProxy objects for each children in the Structure.
        frame = sys._getframe(1)
        parent = frame.f_locals.get('self')
        if type(parent) is StructureType:
            data = frame.f_locals.get('data')
            return iter([data[var.name] for var in parent])
