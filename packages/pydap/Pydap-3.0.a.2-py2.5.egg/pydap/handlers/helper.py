...
def constrain(dataset, ce):
    ...
    A constraint expression applier.
        ...
        ...     print struct_.data
        (10, 17.199999999999999, 'Diamont_St')
        (11, 15.1, 'Blacktail_Loop')
    ...
    Instead, inner sequences have to be filtered inside a loop::
        ...
        ...         print profile.data
        (20.5, 34.899999999999999, 200.0)
        (19.0, 33.0, 300.0)
        (21.0, 35.399999999999999, 200.0)
        (20.0, 33.5, 300.0)
        (19.0, 33.0, 500.0)
    ...
    for var in projection:
        ...
        while var:
            ...
            if slice_: slice_var(candidate, slice_)

            if isinstance(candidate, StructureType):
                if var:
                    # Convert degenerate grids into structures.
                    if isinstance(candidate, GridType): candidate = StructureType(name)
...
def slice_var(var, slice_):
    """
    Apply a tuple of slices to a Pydap variable.

    """
    if isinstance(var, GridType):
        # Apply the slice on the first variable (the array), and 
        # then each subslice on each map.
        for var, s in zip(var.walk(), [slice_] + list(slice_)):
            var.data = var.data[s]
            var.shape = var.data.shape
    elif isinstance(var, SequenceType):
        # For sequences slice shouldn't be a tuple, but a slice
        # object.
        var.data = var.data[slice_[0]]
    else:
