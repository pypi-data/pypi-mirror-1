...
You can install the latest version (|release|) using `EasyInstall <http://peak.telecommunity.com/DevCenter/EasyInstall>`_. Simply download and run the script `ez_setup.py <http://peak.telecommunity.com/dist/ez_setup.py>`_ (you need root permission, unless you create a `sandbox <http://wiki.pylonshq.com/display/pylonscookbook/Using+a+Virtualenv+Sandbox>`_), and then proceed to install Pydap with the command:

.. code-block:: bash

    $ easy_install Paste Pydap
