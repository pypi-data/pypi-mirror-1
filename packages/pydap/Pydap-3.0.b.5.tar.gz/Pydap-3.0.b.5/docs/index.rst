...
This is the homepage for the 3.0 branch of Pydap, which is still in beta. If you want you can still use the old `2.x branch <http://pydap.org/2.x/>`_. Keep in mind that Pydap beta is like Google beta -- it's a well tested code base, used daily by the developer for actual work -- so don't be afraid to give it a try. 

Quickstart
----------

You can install the latest version (|release|) using `EasyInstall <http://peak.telecommunity.com/DevCenter/EasyInstall>`_. Simply download and run the script `ez_setup.py <http://peak.telecommunity.com/dist/ez_setup.py>`_ (you will need root permission, unless you create a `sandbox <http://wiki.pylonshq.com/display/pylonscookbook/Using+a+Virtualenv+Sandbox>`_), and then proceed to install Pydap with the command:

.. code-block:: bash

    $ easy_install Paste  # sometimes Paste is problematic, so let's install it manually
    $ easy_install Pydap
...
This will start a standalone server running on http://localhost:8001/, serving netCDF files from ``./myserver/data/``. Since the server uses the `WSGI <http://wsgi.org/>`_ standard, it can easily be run behind Apache. The `server documentation <server.html>`_ has more information on how to better deploy Pydap.
