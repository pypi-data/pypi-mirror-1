...
from pydap.lib import hyperslab, combine_slices, fix_slice, walk, isiterable
...
class SequenceProxy(VariableProxy, SequenceData):
    ...
    def __iter__(self):
        ...
        url = urlunsplit((
                scheme, netloc, path + '.dods',
                quote(self.id) + hyperslab(self._slice) + '&' + query,
                fragment))
        ...
        for var in walk(dataset):
            if type(var) in (StructureType, DatasetType):
                ...
                # request for the whole sequence, return data.
                return iter(data)
            elif (isinstance(var, SequenceType) and
                    self.id.startswith(var.id)):
                # request for children, prepare data.
                data = remove_tuples(data)
                reqs = re.sub('\[.*?\]', '', self.id).split(',')
                data = reorder(var, data, reqs)
                return iter(data)
...
def remove_tuples(data):
    """
    Remove tuples from Sequence data when a children is requested directly.
...
def reorder(var, data, reqs):
    """
    Reorder Sequence data according to the request.

    TODO: not sure how this works with Structures inside Sequences.

    """
    if not isiterable(data): return data
    reqs = [req[len(var.name)+1:] for req in reqs]  # remove own id from requested vars
    order = [req.split('.', 1)[0] for req in reqs if req]
    order = [(var.keys().index(name), name) for name in order]
    order = uniqify(order)
    data = [
            tuple([
                reorder(var[name], row[col], reqs) for (col, name) in order
            ]) for row in data] \
        if len(reqs) > 1 else [
            tuple([
                reorder(var[name], row, reqs) for (col, name) in order
            ]) for row in data]
    return data
