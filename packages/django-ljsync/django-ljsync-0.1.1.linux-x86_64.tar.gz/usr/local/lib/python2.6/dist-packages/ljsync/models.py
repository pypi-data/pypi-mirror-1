...
class LJPost(Model):
    ...
    # security
    security  = CharField("Security", choices=security_choices, max_length=8, blank=True, null=True)
...
#    post = ForeignKey(Post, blank=True, null=True, verbose_name=_('Post'), related_name='lj_post',
#                             help_text=_('A post from which this LJ entry was exported or to which it was imported.'))
    ...
    # for admin
    get_local_post = lambda self: '<a href="%s">%s</a>' % (
                            self.local_post.get_absolute_url(), self.local_post)
