...
if not results:
    ...
    write('''<li class="LSRow">''')
    write( '<a href="%s/search_form" style="font-weight:normal">%s</a>' % (context.portal_url(), ts.translate(label_advanced_search)))
    write('''</li>''')

    if len(results)>limit:
        # add a more... row
        write('''<li class="LSRow">''')
        write( '<a href="%s/%s" style="font-weight:normal">%s</a>' % (context.portal_url() , 'search?SearchableText=' + searchterms, ts.translate(label_show_all)))
        write('''</li>''')
    write('''</ul>''')
    write('''</div>''')
    write('''</fieldset>''')
