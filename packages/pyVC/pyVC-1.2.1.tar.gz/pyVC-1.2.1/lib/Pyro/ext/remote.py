#############################################################################
#
#     $Id: remote.py,v 1.3 2004/02/22 20:50:55 irmen Exp $
...
def tb_info(tb):
	...
	lineno   = tb.tb_lineno
	if not (codename == '<string>' or string.find(codename, ".py") > 0):
		lineno = lineno - 2
...
def format_exception():
	return canonize(sys.exc_type, sys.exc_value, sys.exc_traceback)
...
def find_nameserver(hostname = None, portnum = None):
	if hostname and string.find(hostname, '://') > 0:
...
class ObjBase(Pyro.core.ObjBase):
	...
	def Pyro_dyncall(self, method, flags, args):
		try:
			...
			result = Error(string.join(format_exception(), '\n'))
		return result
...
class DynamicProxy(Pyro.core.DynamicProxyWithAttrs):
	...
	def __invokePYRO__(self, *vargs, **kargs):
		result = unwrap(apply(Pyro.core.DynamicProxyWithAttrs.__invokePYRO__,
		...
		if type(result) is types.InstanceType and \
		   ...
		   isinstance(result, ProtocolError):
			msg = str(result)
			type_name = msg[: string.find(msg, ' ')]

			if type_name == 'exceptions.IndexError':
				try:
					real_type = eval(type_name)
					msg       = string.split(msg, '\n')[0]
					result    = real_type(msg[string.find(msg, ':') + 2 :])
				except:
					pass
...
def unwrap(value):
	...
	if t is types.InstanceType and isinstance(value, DynamicProxy):
		if pyro_daemon:
			id = value.objectID
			if pyro_daemon.implementations.has_key(id):
				return pyro_daemon.implementations[id][0]
		return value
...
def provide_local_object(object, name = None, hostname = None, portnum = None):
	...
	if not isinstance(object, Pyro.core.ObjBase):
		if provided_objects.has_key(object):
			object = provided_objects[object]
		else:
			slave = ObjBase()
			slave.delegateTo(object)
			provided_objects[object] = slave
			object = slave

	URI = pyro_daemon.connect(object, name)
	daemon_objects.append(object)
...
def unregister_object(object):
	if pyro_daemon is not None:
		try: pyro_daemon.disconnect(object)
		except: pass
		global daemon_objects
		if object in daemon_objects:
			daemon_objects.remove(object)
