...
# mx.ODBC.zopeda version
VERSION = '1.0.10'

BASE_URL = 'http://downloads.egenix.com/python/egenix-mxodbc-zopeda-' + VERSION + '.'
EXTENSION = '.zip'
...
_urls = {
    ...
    (2, 4, 'Darwin', 'i386'): 'darwin-8.9.0-Power_Macintosh-py2.4',
    (2, 5, 'Darwin', 'i386'): 'macosx-10.3-fat-py2.5',
    (2, 3, 'Darwin', 'Power Macintosh'): 'darwin-8.9.0-Power_Macintosh-py2.3',
    (2, 4, 'Darwin', 'Power Macintosh'): 'darwin-8.9.0-Power_Macintosh-py2.4',
    (2, 5, 'Darwin', 'Power Macintosh'): 'macosx-10.3-fat-py2.5',
...
class Recipe(object):
    def __init__(self, buildout, name, options):
        ...
        self.options.setdefault('licenses-archive', 'mxzopeda-licenses.zip')
    ...
    def install(self):
        ...
        self._install_mxodbczopeda(location)
    ...
    def _install_mxodbczopeda(self, location):
        ...
        here = os.getcwd()
        try:
            self.logger.debug('Extracting mx.ODBC.ZopeDA archive')
            setuptools.archive_util.unpack_archive(fname, location)
            # remove existing files
            # move the mx and product to the main dir of the part
            zopeda = os.path.join(location, 'lib', 'python',
                                  'Products',)
            new_zopeda = os.path.join(location, 'Products')
            if os.path.exists(new_zopeda):
                shutil.rmtree(new_zopeda)
            shutil.move(zopeda,new_zopeda)

            mxlib =  os.path.join(location, 'lib', 'python', 'mx')
            new_mxlib =  os.path.join(location, 'mx')
            if os.path.exists(new_mxlib):
                shutil.rmtree(new_mxlib)
            shutil.move(mxlib, new_mxlib)            
        finally:
