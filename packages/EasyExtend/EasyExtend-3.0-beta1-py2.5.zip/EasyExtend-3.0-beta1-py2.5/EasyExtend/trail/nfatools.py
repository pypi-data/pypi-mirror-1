...
class NFAMultiTrace:
    ...
    def commit(self):
        ...
        self.new_traces = {}
        # print "DEBUG - traces", self.traces.keys()
...
class NFACursor:
    ...
    def move(self, nid, *other):
        ...
        for state in self.state:
            if state[0] == nid:
                statelist.extend(self.get_next_state(state))
            for n in other:
                if state[0] == n:
                    statelist.extend(self.get_next_state(state))
        self.state = statelist
        self.mtrace.commit()
        selection = list(set(s[0] for s in statelist))
    ...
    def get_next_state(self, state):
        ...
        for label in labels:
            ...
            if label[1] == SKIP:
                next_states.extend(self.get_next_state(label))
            else:
                next_states.append(label)
        if delayed:
            self.mtrace.update(delayed, delayed)
            if delayed[1] == SKIP:
                next_states.extend(self.get_next_state(delayed))
            else:
                next_states.append(delayed)
        return next_states
