...
#
#  TODO:  recursive transformations need a better implicit treatment
#
...
def transform_dbg(spec, **kwd):
    '''
    Decorator used to display and check properties of input and output nodes
    that are passed into a node handler.

    Arguments:
        spec --  specifier string. A specifier string is a list of commands specifiers
                 e.g. 'ni no co gc'. Each command spec has two letters.
        ...
        kwd  --  a dictionary specifying user defined actions. The keys of the dict are
                 command
    ...
    def transform(f):
        ...
        def call_dbg(self, node):
            ...
            if 'fi' in spec:
                action('fi', node)
            if 'ni' in spec:
                print "[ni -- plain node (input) -- %s>\n"%name
                print node
                action('ni', node)
                print "<ni -- plain node (input)-- %s]\n"%name
            if 'si' in spec:
                print "[si -- python source (input) -- %s>\n"%name
                print self.langlet.unparse(node)
                action('si', node)
                print "<si -- python source (input) -- %s]\n"%name
            if 'ci' in spec:
                print "[ci -- cst (input) -- %s>\n"%name
            ...
            if res:
                ...
                if 'fo' in spec:
                    action('fo', rlist)
                if 'no' in spec:
                    print "[no -- plain node (output) -- %s>\n"%name
                    print(res)
                    action('no', res)
                    print "<no -- plain node (output) -- %s]\n"%name

                if 'rs' in spec:
                    print "[rs -- single node return? -- %s>\n"%name
                    print len(rlist) == 1
                    action('rs', len(rlist))
                    print "<rs -- single node return? -- %s]\n"%name
                elif 'r>' in spec:
                    print "[r> -- multiple node return? -- %s>\n"%name
                    print len(rlist) > 1
                    action('r>', len(rlist))
                    print "<r> -- multiple node return? -- %s]\n"%name
                if 'r=' in spec:
                    print "[r= -- nid(return_node) == nid(input_node)? -- %s>\n"%name
                ...
                if 'r!' in spec:
                    ...
                    print "<r! -- nid(return_node) == nid(input_node)? -- %s]\n"%name
                if 'so' in spec:
                    print "[so -- python source (output) -- %s>"%name
                    for i, nd in enumerate(rlist):
                        if i:
                            print "\n---------------- %s ----------------\n"%(i+2)
                        print self.langlet.unparse(nd)
                        action('so', nd)
                    print "<so -- python source (output) -- %s]\n"%name
                if 'co' in spec:
                    print "[co -- cst (output) -- %s>"%name
                    for i, nd in enumerate(rlist):
                        if i:
                            print "\n---------------- %s ----------------\n"%(i+2)
                        self.langlet.pprint(nd)
                        action('co', nd)
                    print "<co -- cst (output) -- %s]\n"%name
                if 'cv' in spec:
                    import EasyExtend.langlets.zero.langlet as zero
                    print "[cv -- cst validation test -- %s>"%name
                    for i, nd in enumerate(rlist):
                        if i:
                            print "\n---------------- %s ----------------\n"%(i+2)
                        check_node(nd, zero)
                        action('gc', nd)
                    print "<cv -- cst validation test -- %s]\n"%name
            return res
...
class FSTransformer:
    ...
    def call_node_transformer(self, tree, handler, locals = None):
        ...
        for i, this_handler in enumerate(handler):
            ...
            if locals:
                try:
                    nodes = this_handler(new_tree)
                except TypeError:
                    nodes = this_handler(new_tree, locals)
            else:
                nodes = this_handler(new_tree)
            break
        else:
    ...
    def transform_more(self, tree, node_stack, locals):
        def find_transformable(tree, node_stack):
            res = []
            for sub in tree[1:]:
                if isinstance(sub, cstnode):
                    if sub.transformable:
                        res.append((sub, node_stack+[tree]))
                        continue
                if isinstance(sub, list):
                    res+=find_transformable(sub, node_stack = node_stack+[tree])
            return res

        for (sub,c) in find_transformable(tree, node_stack):
            if sub.transformable:
                self.run(sub, node_stack = c, locals = locals)
    ...
    def substitute(self, tree, nodes, repl_id, node_stack):
        ...
        S = "Substitution failure. Node %s must be a subnode of %s in:\n\n%s"%(
    ...
    def run(self, tree, node_stack = [], locals = None):
        '''
        ...
        @param locals: dictionary that contains local context of the langlet transformation to be
                       evaluated.
        ...
        try:
            ...
            return self.transform_more(tree, node_stack, locals)
...
class Transformer(object):
    ...
    def unmark_node(self, tree):
        fs_transformer._unmark_node(tree)
    ...
    def node_stack(self, node):
        nid = id(node)
        _node_stack =  fs_transformer._node_stack.get(nid)
        if _node_stack:
    ...
    def run(self, tree, locals = None):
        ...
        fs_transformer.run(tree, locals = locals)
    ...
    def general_transform(self, tree):
        ...
        def import_langlet_trans(node):
            if no_future(node):
                ...
                init_langlet     = any_node(CST_CallFunc("EasyExtend.eecommon.init_langlet", ["__langlet__"]), py_symbol.small_stmt)
