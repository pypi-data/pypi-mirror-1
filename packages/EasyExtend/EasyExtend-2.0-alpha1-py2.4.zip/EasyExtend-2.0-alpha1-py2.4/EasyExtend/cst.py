...
MAX_PY_SYMBOL = 333        # maximal node_id of Python 2.4 symbols. Change this for other releases!
FIBER_OFFSET_SCALE = 512   # used to discriminate fibers
PY_NT_OFFSET = 256
...
def py_cst(f):
    '''
    Decorator used to filter arguments of cst module functions on fiber-specific nodes.
    If a node was found that could be projected into Pythons node range apply projection
    by FIBER_OFFSET_SCALE.
    '''
    def cstfunc(*args):
        for arg in args:
            if isinstance(arg, list):
                if arg[0]>FIBER_OFFSET_SCALE:
                    rest = arg[0]%FIBER_OFFSET_SCALE
...
def import_from(*args):
    ...
    if args[1] == "*":
        ...
        assert args[2][0] == py_symbol.import_as_names, toText(args[2][0])
        nodelist.append(args[2])
        nodelist.append([py_token.RPAR,")"])
    else:
        assert args[1][0] == py_symbol.import_as_names, toText(args[1][0])
        nodelist.append(args[1])
    return nodelist
...
def dotted_as_name(*args):
    ...
    if len(args) == 3:
        nodelist.append(Symbol(py_token.NAME,args[1]))
        nodelist.append(Symbol(py_token.NAME,args[2]))
    return nodelist
...
def test(*args):
    ...
    assert (args[0][0] == py_symbol.lambdef) or (args[0][0] == py_symbol.and_test)
    nodelist = [py_symbol.test]
    nodelist.append(args[0])
    for arg in args[1:]:
        nodelist.append(Symbol(py_token.NAME, 'or'))
        nodelist.append(arg)
    return nodelist
...
def comp_op(cmp):
    ...
    if cmp == "<":
        ...
        nodelist.append(Symbol(py_token.NOTEQUAL, '!='))
    elif cmp in ("in","not","is"):
        nodelist.append(Symbol(py_token.NAME, cmp))
    elif cmp in ("not in","is not"):
        for c in cmp.split():
...
def atom(*args):
    ...
    if args[0] == '(':
        nodelist.append(Symbol(py_token.LPAR, "("))
        if len(args)>2:
            assert args[1][0] == py_symbol.testlist_gexp, toText(args[1][0])
            nodelist.append(args[1])
        nodelist.append(Symbol(py_token.RPAR, ")"))
    elif args[0] == '[':
...
def testlist_safe(*args):
    ...
    nodelist[0] = py_symbol.testlist_safe
    return nodelist
...
def gen_if(*args):
    ...
    assert args[0] == py_symbol.test, toText(args[0])
    nodelist.append(Symbol(py_token.NAME,"if"))
    nodelist.append(args[0])
    if len(args)>=2:
