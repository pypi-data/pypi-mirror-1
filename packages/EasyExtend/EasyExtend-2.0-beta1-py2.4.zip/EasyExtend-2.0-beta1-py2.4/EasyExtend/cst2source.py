...
from EasyExtend.parser.DFAParser import cstnode, add_info
from csttools import find_node
import keyword
...
class Unparser(object):
    def __init__(self, symbol=py_symbol, token=py_token, offset = 0):
        self.token = token
        self.offset= offset
        self.symbol= symbol
    ...
    def __call__(self, tree):
        ...
        handler = NodeHandler_Exact(output = out,
                                    symbol = self.symbol,
                                    token = self.token,
                                    offset = self.offset)
        handler.handle(tree)
...
if __name__ == '__main__':
    import eecompiler
    import EasyExtend.fibers.Py25Lite.fiber as fiber
    #import EasyExtend.fibers.hex_fiber.fiber as fiber
    cc = eecompiler.EECompiler(fiber)
    tree = cc.eeparse_file(r"C:\lang\Python24\Lib\site-packages\EasyExtend\fibers\Py25Lite\fibertest\test_condexpr.py")
    #tree = cc.eeparse_file(r"C:\lang\Python24\Lib\site-packages\EasyExtend\fibers\hex_fiber\_test.py")
    cc.eetransform(tree)
    print fiber.unparse(tree)
