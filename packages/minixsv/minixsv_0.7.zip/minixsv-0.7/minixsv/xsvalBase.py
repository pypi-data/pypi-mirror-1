...
import string
from minixsv             import *
from genxmlif.xmlifUtils import collapseString, convertToAbsUrl, NsNameTupleFactory
...
class XsValBase:
    ...
    def validate (self, inputTree, xsdTree):
        ...
        self.xsdNsURI    = self.xsdRoot.getNamespaceURI()
        ...
        self.xsdTargetNamespace      = self.xsdRoot.getAttributeOrDefault("targetNamespace", None)
        self.xsdElementFormDefault   = self.xsdRoot.getAttributeOrDefault("elementFormDefault", "unqualified")
        self.xsdAttributeFormDefault = self.xsdRoot.getAttributeOrDefault("attributeFormDefault", "unqualified")
        ...
        self._updateLookupTables(self.xsdRoot, self.xsdLookupDict)

        self.xsdIncludeDict = {self.xsdRoot.getAbsUrl():1,}
        self._includeAndImport (self.xsdTree, self.xsdTree, self.xsdIncludeDict, self.xsdLookupDict)
        ...
        if self.xsdElementDict.has_key(inputRootNsName):
            ...
            self._raiseError ("Used root tag %s not found in schema file %s!"
                              %(str(inputRootNsName), self.xsdRoot.getFilePath()), self.inputRoot)
    ...
    def _includeAndImport (self, baseTree, tree, includeDict, lookupDict):
        tree = self._expandIncludes (baseTree, tree, includeDict, lookupDict)
        tree = self._expandImports (baseTree, tree, includeDict, lookupDict)
        # import well-known schema files
        tree = self._importWellknownSchemas (baseTree, tree, includeDict, lookupDict)
        return tree
    ...
    def _expandIncludes (self, baseTree, tree, includeDict, lookupDict):
        ...
        for includeNode in rootNode.getChildrenNS(namespaceURI, "include"):
            ...
            tree = self._includeSchemaFile (baseTree, tree, includeNode, expNamespace, includeUrl, includeNode.getBaseUrl(), includeDict, lookupDict)
    ...
    def _importWellknownSchemas (self, baseTree, tree, includeDict, lookupDict):
        ...
        for namespace, schemaFile in ((XML_NAMESPACE, "xml.xsd"),
            ...
            tree = self._includeSchemaFile (baseTree, tree, nextSibling, namespace, file, None, includeDict, lookupDict)
        return tree
    ...
    def _includeSchemaFile (self, baseTree, tree, nextSibling, expNamespace, includeUrl, baseUrl, includeDict, lookupDict):
        ...
        if expNamespace != stRootNode.getAttributeOrDefault("targetNamespace", None):
        ...
        subTree = self._expandIncludes (baseTree, subTree, includeDict, lookupDict)
    ...
    def _updateLookupTables (self, rootNode, lookupDict):
        tns = rootNode.getAttributeOrDefault("targetNamespace", None)
...
#            self._checkMixed (xsdNode, inputNode)
        ...
        if inputNode[(XSI_NAMESPACE, "nil")] == "true":
            ...
            if usedAsBaseType in (None, "extension"):
                ...
                if usedAsBaseType == None and inputChildIndex < len (childTags):
                    ...
                    self._addError ("Unexpected child tag '%s' found in tag '%s'!"
    ...
    def _checkSimpleTypeContent (self, xsdParentNode, xsdNode, inputNode, inputChildIndex, usedAsBaseType, baseTypeAttributes):
        if inputNode.getChildren() != []:
            raise TagException ("No child tags allowed for element %s!" %(str(inputNode.getNsName())), inputNode)
    ...
    def _checkElementSimpleType (self, xsdNode, xsdTypeAttr, inputNode, attrName, attrValue, checkAttribute=0):
        if inputNode.hasAttribute((XSI_NAMESPACE, "nil")):
    ...
    def _checkAttributeTags (self, parentNode, xsdNode, inputNode, validAttrDict):
        ...
        for qAttrName, validAttrEntry in validAttrDict.items():
            ...
            if self.xsdAttributeDict.has_key(qAttrName) and self.xsdAttributeDict[qAttrName] == attrRefNode:
                ...
                attributeForm = attrRefNode.getAttributeOrDefault ("form", self.xsdAttributeFormDefault)
            ...
            if inputAttribute == (XSI_NAMESPACE, "type"):
                ...
                try:
                    ...
                    if inputAttribute[0] == None and xsdNode.getAttribute("form") == "unqualified":
                        inputNamespace = self.xsdTargetNamespace
    ...
    def _checkAttributeTag (self, qAttrName, xsdAttrNode, xsdAttrRefNode, inputNode, inputAttrDict):
        if qAttrName[0] == self.xsdTargetNamespace and xsdAttrRefNode.getAttribute("form") == "unqualified":
        ...
        elif inputAttrDict.has_key((self.xsdTargetNamespace, qAttrName[1])):
            self._addError ("Local attribute '%s' must be unqualified!" %(qAttrName[1]), inputNode)
            del inputAttrDict[(self.xsdTargetNamespace, qAttrName[1])]
        elif inputAttrDict.has_key((None, qAttrName[1])) and qAttrName[0] == self.xsdTargetNamespace:
    ...
    def _updateAttributeDict (self, xsdNode, validAttrDict, checkForDuplicateAttr=0, recursionKeys=None):
        ...
        for validAttributeNode in validAttributeNodes:
            if validAttributeNode.hasAttribute("ref"):
                ...
                if checkForDuplicateAttr:
                    targetNamespace = self.inputRoot.getAttributeOrDefault("targetNamespace", None)
                    attrKey = (targetNamespace, validAttributeNode.getAttribute("name"))
                else:
                    attrKey = (self.xsdTargetNamespace, validAttributeNode.getAttribute("name"))
    ...
    def _checkInputNamespace (self, xsdNode, inputNode, inputNamespace):
        ...
        if namespaces == "##any":
            ...
            if inputNamespace == self.xsdTargetNamespace:
            ...
            for namespace in string.split(collapseString(namespaces), " "):
                if namespace == "##local" and inputNamespace == None:
                    break
                elif namespace == "##targetNamespace" and inputNamespace == self.xsdTargetNamespace:
                    break
    ...
    def _checkIdentityConstraint (self, identityConstrNode, inputNode):
        ...
        identConstrNsLocalName = (self.xsdTargetNamespace, identConstrName)
                ...
                if not self.xsdIdentityConstrDict[keyReference]["ValueDict"].has_key (keyValue):
                    ...
                    for fieldXPathNode, baseTypes in zip(fieldXPathNodeList, baseTypesList):
                        ...
                        if attrNodeList == None:
                            ...
                            inputChild = attrNodeList[0]
                            refBaseTypes = self._setBaseTypes(attrNodeList[0].getXsdAttrNode(attrName))
    ...
    def _getXPath (self, node, xPathNode, identityConstraint=None):
        ...
        try:
            childList, attrNodeList, attrName = node.getXPathList (xPath, namespaceRef=xPathNode)
