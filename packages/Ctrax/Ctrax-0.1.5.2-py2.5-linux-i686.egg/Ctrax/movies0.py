...
class CompressedAvi:
    ...
    def __init__(self,filename):
        ...
        self.issbfmf = False
        self.file = media.load(filename)
        self.player = media.Player()
        self.player.queue(self.file)
        self.ZERO = 0.00001
        self.player.seek(self.ZERO)
        self.start_time = self.file.get_next_video_timestamp()

        # estimate properties of the video that would be nice 
        # to be able to read in -- i have no idea how accurate 
        # these estimates are
        self.duration_seconds = self.file.duration
        self._estimate_fps()
        self.frame_delay_us = 1e6 / self.fps
        self._estimate_keyframe_period()
        self.n_frames = self.duration_seconds * self.fps
        
        # added to help masquerade as FMF file:
        self.filename = filename

        # read in the width and height of each frame
        self.width = self.file.video_format.width
        self.height = self.file.video_format.height
        ...
        self.player.seek(self.ZERO)
        im = self.file.get_next_video_frame()
    ...
    def get_frame(self,framenumber):
        ...
        if framenumber >= self.bufferframe0 and framenumber < self.bufferframe1:
            off = framenumber - self.bufferframe0 + self.bufferoff0
            print "frame %d is in buffer at offset %d"%(framenumber,off)
            return (self.buffer[:,:,off],self.bufferts[off])
        ...
        print "keyframe before frame %d is %d (ts = %f)"%(framenumber,lastkeyframe,lastkeyframe_s)
        ...
        if framenumber > self.currframe and self.currframe >= lastkeyframe:
            ...
            print "the best thing to do is just to read in frames until we get to desired %d (currframe = %d)"%(framenumber,self.currframe)
        ...
        print "frame %d is equivalent to time stamp %f"%(framenumber,wantts)

        while True:
            # try seeking
            print "trying to seek to %f"%lastkeyframe_s
            self.player.seek(lastkeyframe_s-self.start_time)
            tscurr = self.file.get_next_video_timestamp()
            print "ended up seeking to %f"%tscurr

            # are we in the right ballpark?
            if wantts < tscurr:

                lastkeyframe_s = max(self.ZERO,lastkeyframe_s-self.keyframe_period_s)
                print "still not back far enough, trying to seek to %f"%lastkeyframe_s
                dirmoved = -1
                continue
                ...
                if dirmoved == -1:
                    ...
                    # so we're screwed up
                    # let's just stay
                    print "uh oh -- we've already gone backward, and now we think we need to go forward, so we're just going to stay"
                    break
    ...
    def _get_next_frame_helper(self):
        ts = self.file.get_next_video_timestamp()
        im = self.file.get_next_video_frame()
    ...
    def get_next_frame(self):
        ...
        self.buffer[:,:,self.bufferoff] = frame
    ...
    def _estimate_fps(self):
        ...
        # initial time stamp
        ts0 = self.file.get_next_video_timestamp()
        ts1 = ts0
    ...
    def _estimate_keyframe_period(self):
        ...
        self.player.seek(self.start_time)
        ts0 = self.file.get_next_video_timestamp()
        ...
        while True:
            ...
            self.player.seek(float(i)/self.fps)
