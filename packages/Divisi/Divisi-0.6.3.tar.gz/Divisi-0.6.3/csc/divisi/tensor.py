    ...
    *Obscure terminology*: For higher-dimensional tensors, mode 2 has sometimes
    ...
    def extremes(self, n=10, top_only=False):
        ...
        *n* lowest values
        ...
        """
        import operator
        pairs = sorted(self.iteritems(), key=operator.itemgetter(1))
        if top_only:
            return pairs[:n]
        else:
    ...
    def isvd(self, k=50, niter=100, lrate=.001, *a, **kw):
...
class View(Tensor):
    ...
    def __getattr__(self, name):
        ...
        if name.startswith('__') or name in attribute_blacklist:
...
def wrap_unless_scalar(result):
    if numpy.isscalar(result): return result
...
class DenseTensor(Tensor):
    ...
    def k_means_inner(self, k=20):
        ...
        while error > numpy.linalg.norm(means)/1000:
