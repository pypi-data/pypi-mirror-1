...
from divisi.normalized_view import NormalizedView, TfIdfView
...
def get_shape_for_dict(expected):
    return tuple(idx+1 for idx in max(expected.keys()))
...
class TensorTest(unittest.TestCase):
    ...
    def assertTensorEqual(self, actual, expected):
        '''Takes an n-dimensional array 'expected' and runs the following
        tests on the tensor 'actual'
...
#        self.assertEqual(len(self.tensor), 0)
 ...
 #       self.assertEqual(len(self.tensor), 2)
        self.assertEqual(len(self.tensor.keys()), 2)
        self.assertEqual(self.tensor.ndim, 2)
        self.assertEqual(self.tensor.shape, (2, 2))
        self.assertEqual(self.tensor['banana', 'yellow'], 1)
        self.assertEqual(self.tensor['apple', 'red'], 2)
    ...
    def test_iter_dim_keys(self):
        ...
        i = 0
        exp = {'a' : False, 'b' : False, 'c' : False}
        for key in self.tensor.iter_dim_keys(0):
            self.assertTrue(exp.has_key(key))
            exp[key] = True
            i += 1
        self.assertEqual(i, 3)

        for key in exp:
            self.assertTrue(exp[key])
...
normalize_testcase = [[1, 0],
                      [3, 4]]

normalize_expected_result = [[1, 0],
                             [3/5., 4/5.]]

class NormalizedViewTest(TensorTest):
    def setUp(self):
        self.raw = DictTensor(2)
        self.raw.update(nested_list_to_dict(normalize_testcase))
