...
class Tensor(object, MyDictMixin):
    ...
    *Obscure terminology*: For higher-dimensional tensors, mode 2 has sometimes
    ...
    def normalized_copy(self):
        '''
        Return a normalized copy of this tensor. For a vector, this is
    ...
    def extremes(self, n=10, top_only=False):
        ...
        magnitude. *extremes* returns the *n* highest and
        *n* lowest values
        ...
        top = [(-maxint,None)] * n
        bottom = [(-maxint,None)] * n # maintains the negative of the bottom part

        for k, v in self.iteritems():
            if v > top[0][0]: heapreplace(top, (v, k))
            if -v > bottom[0][0]: heapreplace(bottom, (-v, k))

        top.sort()
        bottom.sort(reverse=True)
        top = [(k, v) for v, k in top]
        bottom = [(k, -v) for v, k in bottom]
...
# List of attributes never to try to delegate to the tensor.
attribute_blacklist = set(('_getAttributeNames', 'trait_names'))
...
class View(Tensor):
    ...
    def __getattr__(self, name):
        ...
        if name.startswith('_') or name in attribute_blacklist:
...
try:
    ...
    def outer_tuple_iterator(shape):
        ...
        >>> sorted(outer_tuple_iterator((3,)))
        [(0,), (1,), (2,)]
        >>> sorted(outer_tuple_iterator((2,2)))
        [(0, 0), (0, 1), (1, 0), (1, 1)]
        >>> sorted(outer_tuple_iterator(()))
        []
        '''
...
def data(tensor):
    ...
    """
    return getattr(tensor, '_data', tensor)
