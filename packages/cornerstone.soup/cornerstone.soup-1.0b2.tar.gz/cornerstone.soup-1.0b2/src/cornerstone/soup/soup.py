...
class NameChooser(object):
    ...
    def checkName(self, name, object): pass
...
class Soup(BTreeFolder2, Location):
    ...
    def query(self, **kw):
        ...
        intids = getUtility(IIntIds, name=self.id)
        for id in ids:
            yield intids.getObject(id)
    
    def lazy(self, **kw):
        ids = self._query(**kw)
        intids = getUtility(IIntIds, name=self.id)
        for id in ids:
            yield LazyRecord(intids, id)
...
class LazyRecord(object):
    ...
    def __init__(self, intids, id):
        self.intids = intids
        self.id = id
    
    def __call__(self):
        return self.intids.getObject(self.id)
