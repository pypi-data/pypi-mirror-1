...
class PMainWindowBase(app.PTopWindowBase):
    ...
    The main window generates a tool bar (see PToolBar) and manages the
    enable state of buttons based on internal flags and file state. If
    the window is the main widget of the application (see PApplication),
    ...
    pendinglist = [ACTION_OK, ACTION_CANCEL]
    ...
    def acceptclose(self):
        ...
        if self.app.mainwin == self:
            return accept and ((not self.queryonexit) \
                or (self.queryexit() == answerOK))
        else:
            return accept
