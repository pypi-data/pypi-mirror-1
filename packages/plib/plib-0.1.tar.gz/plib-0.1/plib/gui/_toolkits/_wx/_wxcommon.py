...
_wxeventmap = {
    ...
    #SIGNAL_LISTVIEWCHANGED: wx.EVT_XXXX,
...
def wx_plain_wrapper(target):
    def wrapper(event):
        target()
    return wrapper

def wx_tablechanged_wrapper(target):
    def wrapper(event):
    ...
    def setup_notify(self, signal, target, wrap=True):
        if signal in _wxeventmap:
            ...
            if wrap and (signal in _wxwrappermap):
                handler = _wxwrappermap[signal](target)
            else:
                handler = target
...
class _PWxWidget(_PWxCommunicator):
