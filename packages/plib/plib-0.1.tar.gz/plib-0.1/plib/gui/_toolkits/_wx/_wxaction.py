...
class _PWxUIObject(object):
    """
    Mixin class to provide 'standard' wxWidget methods for menu and toolbar.
    """
...
class PWxMenu(wx.MenuBar, action.PMenuBase, _PWxUIObject):
    """
...
class PWxToolBar(wx.ToolBar, action.PToolBarBase, _PWxUIObject):
    """
    ...
    def add_action(self, act):
        if self.mainwin.show_labels:
            self.AddLabelTool(act.id, act.toolbarstr,
                scaled_bitmap(act.image, act.toolbarfactor),
                shortHelp=act.toolbarstr)
        else:
            self.AddTool(act.id, scaled_bitmap(act.image, act.toolbarfactor),
                shortHelpString=act.toolbarstr)
