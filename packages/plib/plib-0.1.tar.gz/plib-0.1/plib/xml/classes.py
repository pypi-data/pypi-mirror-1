...
def _getprop(propname, filter=None):
    if filter is not None:
        def _getfunc(self):
            return filter(self.get(propname))
    else:
        def _getfunc(self):
            return self.get(propname)
    return _getfunc

def _setprop(propname, filter=None):
    if filter is not None:
        def _setfunc(self, value):
            self.set(propname, filter(value))
    else:
        def _setfunc(self, value):
...
def attrib_prop(propname):
    ...
    return property(_getprop(propname), _setprop(propname))
...
def attrib_prop_int(propname):
    ...
    return property(_getprop(propname, _strint), _setprop(propname, str))
...
def attrib_prop_bool(propname):
    ...
    return property(_getprop(propname, _strbool), _setprop(propname, _boolstr))

# Dictionary of functions vs. type constants
attrfuncs = {ATTR_STR: attrib_prop, ATTR_INT: attrib_prop_int, ATTR_BOOL: attrib_prop_bool}

class AttrsMeta(type):
    """
    Metaclass to automatically add XML 'attribute properties'
    using the above attrib_prop functions. Looks in the 'attrs' class
        ...
        if 'attrs' in cls.__dict__:
            ...
            for name, value in attrs.iteritems():
                ...
                if isinstance(value, tuple):
                    ...
                    attrtype = value
                # Allow for type entries to be lists of allowed strings; note
                # that since list objects aren't hashable we need the extra first
                # isinstance test to avoid a TypeError
                if not (isinstance(attrtype, int) and (attrtype in attrfuncs)):
                    attrtype = ATTR_STR
    ...
    def xmlattrs(cls):
        ...
        return utils.recursedict(cls, 'attrs')
...
dtdattrs = {ATTR_STR: "CDATA", ATTR_INT: ("CDATA", 0), ATTR_BOOL: (["yes", "no"], False)}

class TagMeta(AttrsMeta):
    """
    Metaclass to automatically construct more useful class fields for
    XML tag classes, and to provide DTD generation functionality based
    on class fields.
    ...
    def xmlsubtags(cls):
        ...
        return utils.recurselist(cls, 'subtags')
    ...
    def dtdattr(cls, attrname, attrentry):
        ...
        if not (isinstance(attrdefault, basestring) and attrdefault.startswith("#")):
    ...
    def dtdsubtag(cls, tagname):
        """
        Return the appropriate subtag string for tagname, using
        the maybe, some, and any class fields to determine if
        a suffix should be added to the tag name.
        """
        
        if tagname in utils.recurselist(cls, 'any'):
            return "%s*" % tagname
        if tagname in utils.recurselist(cls, 'maybe'):
            return "%s?" % tagname
        if tagname in utils.recurselist(cls, 'some'):
            return "%s+" % tagname
        return tagname
    
    def subtaglist(cls):
        """
        Return the DTD subtag list for this tag, and/or appropriate
        string values for other types of content. Note that the
        substr class field, if non-empty, overrides all other ways
        of specifying the element content. The pcdata class field
        determines if character data is acceptable as well as
        ...
        if subtags:
            return (cls.sep.join(pre + [cls.dtdsubtag(tagname) for tagname, _ in subtags]), cls.end)
        if pre:
            return (pre[0], "")
        return "EMPTY"
    ...
    def dtdentry(cls, tagname, done):
        ...
        recursively including subtag entries before this tag's
...
class Tag(BaseElement):
    """
    Base class for deriving custom element classes with more
...
class RootMeta(TagMeta):
    """
    Metaclass to add some useful class methods for DTD
    generation.
    ...
    def dtdgen(cls):
        ...
        return "\n".join([cls.dtdheader(), cls.dtdstart(), cls.dtdimports(), cls.dtdentry(cls.rootname, []), cls.dtdend()])
...
class TreeMeta(type):
    """
    Metaclass for XML tree encapsulation. This metaclass
...
class Tree(object):
    """
    Base class encapsulating XML tree, including parsing,
    serialization, and dtd generation.
    """
    ...
    def _set_tree(self, fn, arg):
        ...
        with our lookup (defaults to our tagdict) and whitespace stripping.
        """
    ...
    def serialize(self):
        ...
        return io.toString(self.tree, self.encoding, self.indent)
