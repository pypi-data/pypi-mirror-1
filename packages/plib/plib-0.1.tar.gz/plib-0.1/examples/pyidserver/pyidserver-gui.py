...
outputwidth = (3 * checkboxwidth) + combowidth + editwidth + (6 * offset)
combostart = (2 * checkboxwidth) + (3 * offset)
...
button_geometry = (offset, offset, buttonwidth, widgetheight)
combo_geometry = (combostart, offset, combowidth, widgetheight)
edit_geometry = (offset, offset, editwidth, editheight)
url_geometry = (offset, offset, urlwidth, editheight)
output_geometry = (offset, offset, outputwidth, outputheight)

IDServerPadding = [] # equates to an empty panel when processed

IDServerTopLeft = [ (gui.PEditBox, (), {'geometry': url_geometry}, 'edit_url') ]
...
IDServerMainEdit = [ (gui.PEditBox, (), {'geometry': edit_geometry, 'expand': False}, 'edit_portnum') ]
IDServerMainHeader = [
...
IDServerMainBody = [ (gui.PEditControl, (), {'geometry': output_geometry, 'scrolling': True}, 'text_output') ]
IDServerMainPanel = [ 
...
class IDServerFrame(gui.PAutoPanel):
    ...
    def _createpanels(self):
        # Monkey patch button creation parameters with our events
