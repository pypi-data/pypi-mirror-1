...
# Mixin classes to enable automatic GUI generation from specifications stored
# in Python lists, tuples, and dicts
...
class _PAutoBase(object):
    ...
    def widget_from_spec(self, spec, fn=None):
        """
        Constructs widget from spec, putting self as first constructor argument
        (i.e., assumes that widget constructor takes 'parent' as first arg).
        """
...
class PPanelMixin(_PAutoBase):
    ...
    def _widget_from_klass(self, klass, args, kwds, attrname):
        ...
        return _PAutoBase._widget_from_klass(self, klass, args, kwds, attrname)
...
class _PAutoWithPanels(_PAutoBase):
    """
    An auto-constructing class that groks having a PAutoPanel as a parent (it
    allows the parent panel to 'pass through' attribute access to this object's
    children) and having PAutoPanel specs in its spec list.
    """
    ...
    def _widget_from_klass(self, klass, args, kwds, attrname):
        if not isinstance(klass, (type, types.ClassType)):
            ...
            klass = type(base)("%s_%s" % (basename, attrname), (base,), {'childlist': klass})
        return _PAutoBase._widget_from_klass(self, klass, args, kwds, attrname)
    
    def _process_control(self, control, attrname):
        if isinstance(self._parent, self.panelclass):
            if attrname is not None:
                setattr(self._parent, attrname, control) # 'pass-through' this control
            if isinstance(control, self.panelclass):
                self._parent._panels.append(control) # so finding controls by attr name works
...
# Mixin class for group boxes to let them create their contents from specs
