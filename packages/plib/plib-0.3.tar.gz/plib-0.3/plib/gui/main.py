...
from plib.stdlib import dotted_import
...
def toolkit_helper(toolkit, modname, klassname):
    def f():
        return getattr(
            dotted_import('plib.gui._toolkits._%s._%s%s' % (toolkit.lower(), toolkit.lower(), modname)),
...
def widget_helper(klassname, modname):
    def f():
        return getattr(dotted_import('plib.gui._widgets.%s' % modname), klassname)
...
def class_helper(modname, klassname):
    def f():
        return getattr(dotted_import('plib.gui.%s' % modname), klassname)
    f.__name__ = '%s_%s' % (modname.lstrip('_'), klassname)
    return f

class_list = [
    ('_classes', ['PHeaderLabel']),
...
def mixin_helper(klassname, bases):
    def f():
        baselist = [ getattr(dotted_import('plib.gui.%s' % modname), basename)
...
del ModuleProxy, toolkit_helper, widget_helper, class_helper, mixin_helper, \
    toolkit_dict, toolkit_list, widget_list, class_list, mixin_list, \
