...
Defines "helper" classes that implement basic functionality used
by various widgets.
"""

from plib.stdlib import abstractlist, basesequence, normalize, SortMixin
...
class _PHelperData(object):
    ...
    def _update(self, data):
        """ Override to populate widget from data. """
...
class _PShift(object):
    ...
    def _shiftright(self, index):
        ...
        if index < n:
            ...
            # List will be longer by one when insert/append occurs
            # in _add method below (so this statement isn't really
            # needed, it's there for symmetry with _shiftleft)
    ...
    def _shiftleft(self, index):
        ...
        if index < n:
            ...
            # (note that unlike the del in _shiftright above, this
            # statement really is necessary to shorten the list)
            del self._items[n]
...
class _PHelper(_PShift, SortMixin, abstractlist):
    ...
    Note also that all of the sequence emulation methods work with the
    ...
    def _set(self, index, value):
        ...
        index will always be in the items dict); adding a new item should always
        be done using the _add method.
        """
    ...
    def __setitem__(self, index, value):
        ...
        elif isinstance(index, slice) and (index.stop == index.start):
            # We're inserting an item or extending by a sequence
            # (e.g., from the constructor)
            for i, item in enumerate(value):
                self._add(index.start + i, item)
        else:
            raise TypeError, "List index must be an int."
...
class _PRowHelperItem(_PColsMixin, _PHelperItem):
    """
    Helper item that exposes a sequence of columns, intended
    to map to a row in a multi-column widget which does *not*
...
class _PRowHelper(_PHelper):
    """
    Helper class for list or table-type widgets where each
    row has a helper item, but no nested tree structure.
    """
...
class _PTreeHelper(_PHelper):
    """
    Helper class for tree-type widgets, provides methods to
    easily manage the entire tree with this object as root.
    """
    
    def current_item(self):
        """ Derived classes must override to return the currently selected helper item. """
        raise NotImplementedError
...
class _PTreeHelperItemCols(_PColsMixin, _PHelperData):
    """
    Class to expose columns of a tree view node as a
    sequence.
    """
