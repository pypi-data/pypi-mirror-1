...
This module contains the GUI objects for the table widgets.
"""
...
from _kdecommon import _PKDECommunicator, _kdecolormap
...
class PKDETableLabels(table.PTableLabelsBase):
    ...
    def _set_align(self, index, align):
        ...
        #self.setColumnAlignment(index, _kdealignmap[label.align]) # doesn't work
        pass
    
    def _set_readonly(self, index, readonly):
        self.table.setColumnReadOnly(index, readonly)

class PKDETable(qttable.QTable, table.PTableBase, _PKDECommunicator):
    ...
    def _set_color(self, mapping, row, col, color):
        if color is None:
            if mapping.has_key((row, col)):
                del mapping[(row, col)]
        else:
            mapping[(row, col)] = _kdecolormap[color]
    ...
    def paintCell(self, painter, row, col, rect, selected, colorgroup=None):
        ...
        if colorgroup is not None:
            if self.textcolors.has_key((row, col)):
                colorgroup.setColor(qt.QColorGroup.Text, self.textcolors[(row, col)])
            if self.cellcolors.has_key((row, col)):
                colorgroup.setColor(qt.QColorGroup.Base, self.cellcolors[(row, col)])
            qttable.QTable.paintCell(self, painter, row, col, rect, selected, colorgroup)
        else:
