...
version 1.1 (PEP 314), except for the (very limited) support for the
requires keyword (which is not included in the metadata that are
registered with PyPI) unless you are using setuptools.
...
SETUPHELPER_VERSION = (0, 2)

def version_tuple(ver_s, dev=False):
    """
    Convert version string to tuple, correctly extracting
    an alphabetic suffix into a separate tuple item, if dev_s
    is None; if dev_s is not None, returns a 2-tuple of
    <version-tuple>, <dev-tuple>
    """
    
    ver_t = ver_s.split('.')
    try:
        result = tuple(map(int, ver_t))
        if dev:
            return (result, ())
        else:
            return result
...
def distutils_default():
    ...
    class RequiresHelper(object):
        ...
        def process_req(self, req, link_data, clean=False):
            # Find latest version and url by looking in link_data
            ver_tuple, index, url, ext, filename, version = \
                self.get_latest_version(link_data, req)
                ...
                for req in self._requires:
                    ...
                    if findstr in pypi_data:
                        ...
                        if link_data is not None:
                            ...
                            self.process_req(req, link_data, clean)
                            continue
...
class SetupHelper(object):
    ...
    def __init__(self, vars, **kwargs):
        ...
        setup_opts = " ".join([opt for opt in sys.argv if opt not in ("python", "setup.py")])
        ...
        if self.__requires_class__ is not None:
            ...
            if do_requires and hasattr(self, '__requires__'):
                self.__requires_class__(self.__requires__, setup_opts).process_reqs()
        ...
        # the download URL is determined)
        ...
        # Do this after classifiers have been converted to list so we don't
        # overwrite if a status classifier is already present
        if hasattr(self, '__dev_status__'):
            has_status = False
            for item in self.__classifiers__:
                if 'Development Status ::' in item:
                    has_status = True
                    break
            if not has_status:
                ...
                for trove in devstatus_trove:
                    if (devstatus in trove.lower()):
                        # Status is usually up front
                        self.__classifiers__.insert(0, trove)
                        break
        ...
        # TODO: Add automatic generation of license value from classifiers if not given;
        # question: why doesn't PyPI automatically fill this in itself?
