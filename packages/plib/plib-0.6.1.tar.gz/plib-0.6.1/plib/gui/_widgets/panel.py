...
class PPanelBase(object):
    """
    Base class for 'panel' widget, a container for
    laying out other widgets. The layout parameter
    is used to determine whether child widgets are
    laid out horizontally or vertically; the align
    parameter determines how this panel takes up
    space in its parent widget. Panels should be
    nestable to any depth desired, to allow complex
    layouts to be achieved.
    ...
    def __init__(self, parent, layout=LAYOUT_NONE, style=PANEL_NONE, align=ALIGN_JUST,
        ...
        if (width > -1) or (height > -1): self.set_min_size(width, height)
