...
are functions that read and write the app variable for the option; or, alternatively,
attrname is a string giving the name of the attribute on the INI file object to
be used to store the value of the option. If neither of these is present, the
default attribute name <sectionname>_<optionname> is used to store the value.
...
class PIniInterface(object):
    ...
    def writevalue(self, oname, vtype, avalue):
        if self.parser is not None:
            # Dunno why ConfigParser doesn't have setint, setboolean corresponding
            # to getint, getboolean above, but oh well...
            if vtype == INI_BOOLEAN:
                avalue = {False: self.falsestr, True: self.truestr}[avalue]
            else:
                avalue = str(avalue)
