...
def get_parser(look=None, elem=None, strip=None, val=None):
    ...
    - Specify a default element class: set the default_element global to the
      ...
      Element and ElementTree factories, will use the global lxml default, which
      can be changed by calling etree.setDefaultElementClass. The parser-specific
    ...
    - Stripping blank whitespace: the strip parameter can be passed to control
      this, or the strip_blanktext global can be set to define global behavior.
      Note that if this is turned on (which is the default), lxml will only strip
      blank whitespace if it is certain that it isn't actual text. See the lxml
      documentation.
    """
