...
from plib.classes import SerialPseudoSocket

class SerialTelnet(telnetlib.Telnet):
    """
    Telnet client that can talk over a serial port as well as
    a socket. Use the ``openserial`` method to open a serial
    device, and ``open`` to open a socket. Does not allow any
