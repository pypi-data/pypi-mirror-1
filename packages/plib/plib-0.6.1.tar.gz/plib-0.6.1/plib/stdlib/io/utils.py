...
- Class ``IOModuleProxy``: implements the machinery for
  ...
  handling on the fly. See the ``plib.stdlib.io``
...
def mixin_helper(pkgname, klassname, bases, doc):
    def f():
        baselist = [ dotted_from_import(pkgname, basename)
...
def _update_module_dict(pkgname, module_dict, add_persistent):
    ...
    module_dict.update([ (klassname, mixin_helper(pkgname, klassname, bases, doc))
...
def io_class(pkgname, name, rwbase, add_persistent=False, endstr="", meta=None):
    ...
    for klassname, bases, doc in _get_names(add_persistent):
        if klassname == name:
            base1, base2 = [ dotted_from_import(pkgname, base)
                for base in bases ]
            if not endstr:
            ...
            if meta is None:
                meta = io_meta
            return meta(klassname + endstr, (base1, rwbase, base2), {'__doc__': format_doc(doc + rwdoc, pkgname)})
...
class IOModuleProxy(ModuleProxy):
    ...
    def __init__(self, pkgname, names, add_persistent=False, meta=None):
        _update_module_dict(pkgname, names, add_persistent)
        ModuleProxy.__init__(self, pkgname, names)
        ...
        try:
            ...
            for endstr in _rwbases.iterkeys():
                if name.endswith(endstr):
                    rwbase = dotted_from_import('plib.stdlib.io.data', _rwbases[endstr])
                    result = self.get_readwrite_class(name[:-len(endstr)], rwbase, endstr)
    ...
    def get_readwrite_class(self, basename, rwklass, endstr=""):
        ...
        return io_class(self._mod.__name__, basename, rwklass, self.add_persistent, endstr, self.meta)
