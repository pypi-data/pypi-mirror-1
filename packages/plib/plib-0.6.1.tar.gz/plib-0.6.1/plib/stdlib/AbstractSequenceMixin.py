...
class AbstractSequenceMixin(AbstractContainerMixin):
    ...
    def __setitem__(self, index, value):
        # NOTE: the treatment of extended slices matches Python 2.6 list semantics;
        # that means that, unlike in 2.5 and earlier, extended slices with step == 1
        # are treated exactly the same as non-extended slices
        extended = isinstance(index, slice) and (index.step is not None) and (index.step != 1)
        ...
        if isinstance(index, tuple):
            ...
            try:
                ...
                try:
                    # NOTE: there's no way to avoid realizing generators/iterators
                    # here; we use tuple because it's faster to construct than list.
                    # As far as I can tell, the Python interpreter does the same
                    # thing for comparing the lengths of values and slices; the
                    # difference is that the interpreter only does it for extended
                    # slices, but we have to do it here even for a standard slice.
                    # This means, of course, that for fully mutable sequences (like
                    # AbstractMixin) and standard slices, we make an extra copy of
                    # the value when we don't really have to. For the use cases
                    # I've had thus far, this is not a major issue, because I've
                    # almost never had to use slicing, and when I do it's always
                    # small slices; also, typically the overhead of the individual
                    # item inserts (e.g., if each item is a row in a GUI table or
                    # a node in a GUI tree view) is much higher than the overhead of
                    # copying the object pointer into a tuple. Your mileage may vary.
                    value = tuple(value)
                    vlength = len(value)
                except TypeError:
