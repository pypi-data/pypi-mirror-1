...
class AbstractContainerMixin(object):
    """
    Mixin class for abstract container classes to support slice operations
    and normalization of indexes. This class is immutable; increasing levels
    of mutability are provided by AbstractSequenceMixin and AbstractListMixin.
    ...
    the slice_class class field is provided to allow derived classes to specify
    ...
    def _index_ok(self, index):
        """
        Validate, and if necessary normalize, index as follows:
        
        If index is an int, it must be within the valid sequence
        range (0 to len - 1), after correcting negative indexes relative
        to the end of the sequence. If so, return it; otherwise throw
        IndexError. (The normalize function is used to accomplish this.)
        ...
        if isinstance(index, (int, long)):
            return normalize(self.__len__(), index)
        elif isinstance(index, slice):
            n = normalize_slice(self.__len__(), index)
