...
from plib.stdlib import any, first, inrange
...
class abstractcontainer(object):
    ...
    *not* mean the underlying data structure must be immutable, only
    ...
    if sys.version_info < (2, 4):
        ...
        def __ne__(self, other):
            return (len(other) != len(self)) or any(x != y for x, y in izip(self, other))
    ...
    if sys.version_info < (2, 4):
        ...
        def index(self, value, start=None, end=None):
            ...
            result = first(i for i in xrange(start, end) if self[i] == value)
