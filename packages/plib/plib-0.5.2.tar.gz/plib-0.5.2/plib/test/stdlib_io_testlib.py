...
def forking_client_test(client_class, test_data, server_addr):
    def f(sock):
    ...
    def r():
        ...
        if (exitcode >> 8) != 0:
            raise IOError, "forking_client_test aborted!"
        return result
    return r
...
class ConcurrentRequestTest(ClientServerTest):
    ...
    def test_io(self):
        results = []
        for i in xrange(self.request_num):
            results.append(forking_client_test(self.client_class, self.test_data, self.server_addr))
