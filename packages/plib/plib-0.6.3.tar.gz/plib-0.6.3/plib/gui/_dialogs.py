...
PrefsButtonPanel = [
    ...
    specs.get_action_button(ACTION_APPLY, 'apply'),
    specs.get_action_button(ACTION_OK, 'ok'),
    specs.get_action_button(ACTION_CANCEL, 'cancel') ]
...
class PPrefsPanel(gui.PAutoPanel):
    ...
    childlist = [
        specs.get_toplevel_panel(PrefsButtonPanel, ALIGN_BOTTOM, LAYOUT_HORIZONTAL, 'buttons') ]
...
controltypes = {
    INI_INTEGER: [PPrefsNumEditBox, (), {'expand': False, 'geometry': specs.numeditgeometry()}],
    INI_BOOLEAN: [PPrefsCheckBox, (), {}],
    INI_STRING: [PPrefsEditBox, (), {'geometry': specs.editgeometry()}] }
...
def get_ini_panel(otype, label, name, target):
    ...
    return specs.get_interior_panel(result, ALIGN_TOP, LAYOUT_HORIZONTAL, name)
...
class PPrefsDialog(PDialogBase):
    ...
    def __init__(self, parent, inifile, labels, style=SECTION_TAB):
        ...
        self.panelclass.childlist.insert(0,
            specs.get_toplevel_panel(PrefsMainPanel, ALIGN_JUST, LAYOUT_VERTICAL, 'main'))
        ...
        for sname, opts in inifile._optionlist:
            ...
            for opt in opts:
                ...
                if ocallback is not None:
                    ocallback(main, inifile, labels, attrname, otype, getter, setter)
            if scallback is not None:
    ...
    def _spec_section(self, groups, inifile, labels, sname, done=False):
        if done:
            ...
            if self._style == SECTION_GROUPBOX:
                self._currentgroup = specs.get_groupbox([], labels[sname], sname)
            else:
                self._currentgroup = (labels[sname],
                    specs.get_tab_panel([], ALIGN_TOP, LAYOUT_VERTICAL, sname))
    
    def _spec_option(self, groups, inifile, labels, attrname, otype, getter, setter):
        self._current().append(get_ini_panel(otype, labels[attrname], attrname, self.data_changed))
    ...
    def _populate_option(self, mainpanel, inifile, labels, attrname, otype, getter, setter):
    ...
    def _retrieve_option(self, mainpanel, inifile, labels, attrname, otype, getter, setter):
        control = getattr(mainpanel, attrname)
