...
class TagLookup(etree.CustomElementClassLookup):
    """
    Customized element class lookup for XML tags. Initialize
    with a dictionary mapping tag names to element classes.
...
def get_parser(look=None, elem=None, strip=None, val=None):
    ...
    - Specify a parser object: set pxml.default_parser to the desired object.
      The object must be an instance of etree.XMLParser or a subclass. If
      this option is used, pxml.default_parser is returned and all other
      parameters are ignored.
    
    - Specify a lookup object: set pxml.default_lookup to the desired object
      (if you want it to apply to all parsers) or pass an object as the
      look parameter. The object must be an instance of a subclass of
    ...
    - Specify a default element class: set pxml.default_element to the desired
      class or pass the class as the elem parameter. The class must be a
      subclass of lxml.etree.ElementBase. The parser returned will use the
      ...
      set the default element class; pxml.default_element or the elem parameter
      only applies to elements generated by the parsers returned by this
      function. Elements generated by other means, such as the default Element
      and ElementTree factories, will use the global lxml default, which can
      be changed by calling etree.setDefaultElementClass. The parser-specific
      scheme implemented here was considered preferable because the anticipated
      use case will have multiple possible mappings of tag names to element
      classes, rather than a single global one.)
    
    - Stripping blank whitespace: the strip parameter can be passed to control
      this, or pxml.strip_blanktext can be set to define global behavior. Note that
      if this is turned on (which is the default), lxml will only strip blank
      whitespace if it is certain that it isn't actual text. See the lxml
      documentation.
    """
    ...
    if default_parser is not None:
        ...
        if isinstance(look, dict) and look:
            ...
            result.setElementClassLookup(etree.ElementDefaultClassLookup(element=elem))
    return result
...
def _xmlheader(tree, encoding):
    ...
    return '<?xml version="%s" encoding="%s"?>' % (tree.docinfo.xml_version, encoding)
...
def toString(tree, default=None, indent=None, xmldecl=None, doctype=None):
    """
    Return lxml tree serialized as string with given encoding and indentation. Also
    handles the XML declaration according to the xmldecl parameter: if True, uses
    the built-in XML support, otherwise includes it manually, and also checks the
    doctype argument, and includes it instead of the built-in LXML doctype for the
    tree if it is not None (this is useful if you want to do tricks with the decl
    or doctype that LXML doesn't support).
    """
    ...
    result = etree.tostring(tree, encoding=enc, pretty_print=indent, xml_declaration=xmldecl)
    if not xmldecl:
        # Can't override a doctype that's already present
        # TODO: figure out a way to strip the old doctype and insert a new one
        ...
        if indent:
            ...
            sep = ""
        result = sep.join(x for x in (_xmlheader(tree, enc), doctype, result) if len(x) > 0)
    return result

def toFile(tree, f, default=None, indent=None):
    """
    Serialize lxml tree to open file object f with given encoding and indentation.
    """
...
def toFilename(tree, filename, default=None, indent=None):
    """
    Serialize lxml tree to file filename with given encoding and indentation.
    """
...
def applyFiles(filelist, tree, **kwargs):
    return _apply([parseStylesheetFile(filename) for filename in filelist], tree, **kwargs)

# XSLT output file and string serialization functions,

def resultToString(transform, tree, **kwargs):
    """
    Output result to string of applying xslt transform to lxml tree with given arguments. Note
    that transform may also be an iterable yielding xslt stylesheets, in which case the output is the
    result of applying them all in sequence to the tree.
    """
...
def resultToFile(transform, tree, f, **kwargs):
    """
    Output result to open file object f of applying xslt transform to lxml tree with given arguments. Note
    that transform may also be an iterable yielding xslt stylesheets, in which case the output is the
    result of applying them all in sequence to the tree.
    """
...
def resultToFilename(transform, tree, filename, **kwargs):
    """
    Output result to file filename of applying xslt transform to lxml tree with given arguments. Note
    that transform may also be an iterable yielding xslt stylesheets, in which case the output is the
    result of applying them all in sequence to the tree.
    """
