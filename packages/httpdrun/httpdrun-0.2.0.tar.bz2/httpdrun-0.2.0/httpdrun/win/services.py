...
class ServiceStartupFailed(Exception): pass
class ServiceDoesNotExist(Exception): pass
class ServiceTimeout(Exception): pass
class ServiceStateChangeTimeout(Exception): pass
...
class ServiceControl(ServiceInfo):
    ...
    def wait_for_state_change(self, state, timeout):
        self.update_status()
        while self.status.current_state == state:
            time.sleep(1)
            timeout -= 1
            if timeout == 0:
                raise ServiceStateChangeTimeout
            self.update_status()

    def start(self,timeout=0):
        win32service.StartService(self.handle, None)
        self.wait_for_state_change(win32service.SERVICE_START_PENDING, 5)        # wait five seconds to leave start pending state
        self.update_status()
        if self.status.win32_exit_code == ERROR_SERVICE_SPECIFIC_ERROR:
            raise ServiceStartupFailed, 'service "%s" exited with error code %i' % (self.name, self.status.service_specific_exit_code)
        if timeout > 0:
