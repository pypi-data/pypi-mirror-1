...
class MyActivities(startup.IndexView):
    ...
    def call(self):
        req = self._cw
        _ = req._
        req.add_js( ('cubicweb.ajax.js', 'cubicweb.edition.js', 'cubes.gingouz.js') )
        req.add_css('cubicweb.form.css')
        resource = self._cw.user.default_resource
        if resource:
            self.w(u'Go to your <a href="%s">resource page (%s)</a>'
                   % (resource.absolute_url(), resource.title))
        #req.user.view('euser-activities-submitform', startup=True, w=self.w)
        # orders
        self.w(u'<h1>%s</h1>' % _('Open workorders'))
        rset = req.execute('Any W,O ORDERBY T,TT WHERE O title T, O split_into W, '
                           'W is WorkOrder, W title TT, W in_state S, S name "in progress"')
