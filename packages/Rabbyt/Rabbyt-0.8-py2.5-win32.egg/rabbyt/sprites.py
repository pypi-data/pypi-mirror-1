...
class BaseSprite(cBaseSprite, Animable):
    ...
    * bounding_radius (for collision detection)
    ...
    (``x``, ``y``, ``xy``, etc.)
    """
    x = anim_slot(default=0, index=0)
    y = anim_slot(default=0, index=1)
    rot = anim_slot(default=0, index=2)
    red = anim_slot(default=1, index=3)
    green = anim_slot(default=1, index=4)
    blue = anim_slot(default=1, index=5)
    alpha = anim_slot(default=1, index=6)
    scale_x = anim_slot(default=1, index=7)
    scale_y = anim_slot(default=1, index=8)
...
class Sprite(cSprite, BaseSprite):
    u = anim_slot(default=0, index=9)
    v = anim_slot(default=0, index=10)
