...
<%def name="show_field(col)">
    ...
    % elif isinstance(tbl_col.type, types.Date):
        ...
        <input id="${col}" name="${col}" readonly="readonly"
            value="${value}" size="10" />
        <img src="/date_chooser/calendar.gif"
            onclick="showChooser(this, '${col}', '${col}_chooser', 1960, 2020, 'Y-m-d', false);" />
        <div id="${col}_chooser" class="dateChooser select-free"
    ...
    % elif isinstance(tbl_col.type, types.DateTime):
        ...
        value="${value}" size="18" />
        <img src="/date_chooser/calendar.gif"
            onclick="showChooser(this, '${col}', '${col}_chooser', 1950, 2010, Date.patterns.ISO8601LongPattern, true);" />
        <div id="${col}_chooser" class="dateChooser select-free"
            style="display: none; visibility: hidden; width: 160px;">
        </div>
