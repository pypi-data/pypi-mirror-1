...
class DeliveryReceiptController(TransactionController):
    ...
    def dr_print(self, id):
        self._dbg('details', request.params)
        self._details(request.params['id'])
        return render('/delivery_receipt/print_details.mako')
        ...
        if 'trans_item.item' in params.dict_of_lists():
            ...
            for item in params.dict_of_lists()['trans_item.item']:
                args.append({
                        ...
                        'price': params.dict_of_lists()['trans_item.price'][cnt],
                    })
                cnt +=1
            for arg in args:
    ...
    def _save_for_stocks_dr(self, id, params):
        _stocks_out = StocksOutController()
        params['type'] = 2
        # Add kwargs for trans_item child
        cnt = 0
        args = []
        for item in params.dict_of_lists()['trans_item.item']:
            args.append({
            ...
            cnt +=1
        # Add kwargs for transaction entry
        for arg in args:
            stocks_out_params = dict(
                    item=arg['item'],
                    area=params['area'],
                    qty=arg['qty'],
                    dr=id,
