...
import re, copy, sys, types
from Parser import NotMatched, ParseError

__all__ = ('AbstractToken', 'TokenWrapper', 'Token', 'RawToken', 'CompositeToken', 'TokenSequence', 'TakeToken', 'TokenSeries', 'EmptyToken', 'Default', 'Skip', 'Omit', 'Optional', 'Defer', 'Lookahead', 'Negative', 'EOF')
...
class AbstractToken:
    ...
    def _call_preprocess(self, parser, origCursor):
        ...
        if self.callback is not None:
            try: data = self.callback(data)
                ...
                if sys.exc_info()[2].tb_next: raise
    ...
    def __rshift__(self, callback):
        ...
        if new.__call__ is not new._call_preprocess:
            new._call_orig = new.__call__
            new.__call__ = new._call_preprocess
        return new
...
class Default (AbstractToken):
    ...
    '''
    def __call__(self, parser, origCursor): return self.desc
...
class TokenSequence (AbstractToken):
    ...
    def __call__(self, parser, origCursor):
        ...
        for g in self.desc:
            r = yield g
            if not isinstance(g, Skip) and parser.last is None:
                raise NotMatched
            if not isinstance(g, (Skip, Omit)):
                o.append(r)
    ...
    def __call__(self, parser, origCursor):
        ...
        while i != self.max:
            ...
            if i != 0 and self.delimiter is not None:
                d = yield self.delimiter
                if parser.last is None: break
                if self.skip: yield self.skip
            if self.prefix:
                yield self.prefix
                if not parser.last:
                    break
...
class Defer (AbstractToken):
    ...
    def __call__(self, parser, origCursor):
        if not self.realToken:
            self.realToken = self.desc()
        t = yield self.realToken
...
class Lookahead (AbstractToken):
    '''
    Scans for another token, but doesn't actually advance the parser's cursor.
    '''
    def __call__(self, parser, origCursor):
        t = yield self.desc
...
class Negative (AbstractToken):
    '''
    Scans for another token, and only matches (returning True) if that token does not match.
    '''
    def __call__(self, parser, origCursor):
        yield self.desc
        parser.cursor = origCursor
