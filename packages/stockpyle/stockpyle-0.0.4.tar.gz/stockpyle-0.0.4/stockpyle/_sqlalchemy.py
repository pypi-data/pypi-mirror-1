...
if _has_sqlalchemy:
    ...
    class SqlAlchemyStoreExtension(MapperExtension):
        ...
        def __get_store(self, instance):
            store = instance.__stockpyle_store__
            if callable(store):
                return store()
            else:
                return store
        ...
        def after_update(self, mapper, connection, instance):
            _do_sqlalchemy_operation(objs=[instance], operation=lambda: self.__get_store(instance).put(instance))
            return EXT_CONTINUE

        def after_delete(self, mapper, connection, instance):
            _do_sqlalchemy_operation(objs=[instance], operation=lambda: self.__get_store(instance).delete(instance))
            return EXT_CONTINUE
...
def _do_sqlalchemy_operation(objs, operation):
    """performs a sqlalchemy operation on an object, but
    doesn't allow more than one operation on an object
    in a single thread (this avoids the potential for a commit
    to cascade to another commit and recurse infinitely)"""
...
class SqlAlchemyStore(BaseStore):
    ...
    def delete(self, obj):
        ...
        if _is_sqlalchemy_object(obj):
            ...
            # TODO: why do we need to declare a local variable for the closure?
            local_obj = [obj]
