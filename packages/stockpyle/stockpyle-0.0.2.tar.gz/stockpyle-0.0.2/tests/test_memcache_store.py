...
from stockpyle._procmem import ProcessMemoryMemcacheClient
...
class MemcacheStoreTestCase(StoreFixture, ExpiringStoreFixture, TestCase):
    ...
    def setUp(self):
        self.__mc = MemcacheStore(client=ProcessMemoryMemcacheClient())
    
    def test_must_specify_client_or_servers(self):
        self.assertRaises(ValueError, lambda: MemcacheStore())
        self.assertRaises(ValueError, lambda: MemcacheStore(client=ProcessMemoryMemcacheClient(), servers=["localhost:8080"]))
        try:
