...
def first():
    ...
    menubar.add('cascade', men=filemenu, label='test bar')
    #win['menu'] = menubar
    #root['menu'] = menubar
...
def propsheet(config=None, master=None, **kwargs):
    ...
    l1.pack({'side': 'left'})
    e = Entry(f1, width=5, textvariable=var_for('display', 'ttl'))
    e.pack({'side': 'left'})
    f1.pack()
    ...
    sl = Scale(win.datacube, orient='horizontal', label='Spin rate (degrees/sec.)', length=150, variable=var_for('display', 'spin'))
    sl['from'] = 0
    sl['to'] = 360
    sl.pack()
    ...
    f1.frame.range.minentry['textvariable'] = var_for('display', 'xmin')
    f1.frame.range.maxentry['textvariable'] = var_for('display', 'xmax')
    f1.frame.cidr.ip['textvariable'] = var_for('display', 'xcidrip')
    f1.frame.cidr.mask['textvariable'] = var_for('display', 'xcidrmask')

    v = var_for('display', 'xbounds')
    ...
    Entry(f2, width=5, textvariable=var_for('display', 'ymin')).pack({'side': 'left'}, anchor='w')
    Label(f2, text="max").pack({'side': 'left'}, anchor='w')
    Entry(f2, width=5, textvariable=var_for('display', 'ymax')).pack({'side': 'left'}, anchor='w')
    f2.pack()
    ef1.pack()
    ...
    af1.frame.range.minentry['textvariable'] = var_for('display', 'zmin')
    af1.frame.range.maxentry['textvariable'] = var_for('display', 'zmax')
    af1.frame.cidr.ip['textvariable'] = var_for('display', 'zcidrip')
    af1.frame.cidr.mask['textvariable'] = var_for('display', 'zcidrmask')

    v = var_for('display', 'zbounds')
...
if __name__ == '__main__':
    ...
    root.withdraw()
    ps = propsheet(master=root, width=327)
