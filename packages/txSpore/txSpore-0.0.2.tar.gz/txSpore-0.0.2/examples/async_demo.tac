...
class UserInfo(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        d = self.client.rest.getProfileInfo(user)
...
class UserAssets(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        [user] = request.args.get("user")
        d = self.client.rest.getAssetsForUser(user, 0, 100)
        d.addCallback(writeData)
        return NOT_DONE_YET
...
class SiteRoot(resource.Resource):
    ...
    def __init__(self, client):
        ...
        # we set the client here so we can take advantage of caching
        self.client = client

    def putChild(self, location, instance):
        # we override this method so that we can set the "client" attribute on
        # all child resources
        resource.Resource.putChild(self, location, instance)
        instance.client = self.client


# application code
application = service.Application("Demo txSpore Application")
sporeClient = client.AsyncClient()
root = SiteRoot(sporeClient)
