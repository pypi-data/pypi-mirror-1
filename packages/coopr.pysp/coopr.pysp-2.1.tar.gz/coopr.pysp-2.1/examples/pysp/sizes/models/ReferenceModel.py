...
# TBD: once Pyomo allows parameters as domain bounds, add in upper bounds for production capacity.
...
# TBD: once Pyomo allows parameters as domain bounds, impose upper bound.
...
def enforce_inventory_first_stage_rule(i, model):
   return (None, sum(model.NumUnitsCutFirstStage[i,j] for j in model.ProductSizes if j <= i) - model.NumProducedFirstStage[i], 0.0)

def enforce_inventory_second_stage_rule(i, model):
   return (None, sum(model.NumUnitsCutFirstStage[i,j] for j in model.ProductSizes if j <= i) + sum(model.NumUnitsCutSecondStage[i,j] for j in model.ProductSizes if j <= i) - model.NumProducedFirstStage[i] - model.NumProducedSecondStage[i], 0.0)
...
def first_stage_cost_rule(model):
   production_costs = sum(model.SetupCosts[i] * model.ProduceSizeFirstStage[i] + model.UnitProductionCosts[i] * model.NumProducedFirstStage[i] for i in model.ProductSizes)
   cut_costs = sum(model.UnitReductionCost * model.NumUnitsCutFirstStage[i,j] for (i,j) in model.NumUnitsCutDomain if i != j)
...
def second_stage_cost_rule(model):
   production_costs = sum(model.SetupCosts[i] * model.ProduceSizeSecondStage[i] + model.UnitProductionCosts[i] * model.NumProducedSecondStage[i] for i in model.ProductSizes)
   cut_costs = sum(model.UnitReductionCost * model.NumUnitsCutSecondStage[i,j] for (i,j) in model.NumUnitsCutDomain if i != j)
