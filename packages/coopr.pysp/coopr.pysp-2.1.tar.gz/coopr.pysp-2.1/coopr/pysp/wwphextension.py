...
def slam_priority_descend_compare(a, b):
   ...
   value_a = getattr(a, "SlammingPriority")
   value_b = getattr(b, "SlammingPriority")
...
#=========================
   def process_suffix_file(self, ph):
      ...
      if os.path.exists(self._suffix_filename) is False:
         raise RuntimeError, "Suffix file="+self._suffix_filename+" specified for WW PH extension either does not exist or cannot be read"
         ...
         if len(line) > 0 and line[0] != '#':
            ...
            if isVariableNameIndexed(variable_string) is True:
               ...
               for index in match_indices:
                  ...
                  # if the suffix already exists, we don't care - we're stomping it.
                  if hasattr(variable_value, suffix_name) is True:
                     delattr(variable_value, suffix_name)
 
                  # set the suffix on the variable value.
                  setattr(variable_value, suffix_name, converted_value)
               ...
               # if the suffix already exists, we don't care - we're stomping it.
               if hasattr(variable, suffix_name) is True:
                  delattr(variable, suffix_name)
 
               # set the suffix on the variable.
               setattr(variable, suffix_name, converted_value)

               # place the variable in the suffix->variable map, for easy searching elsewhere in this plugin.
               if suffix_name not in self._suffix_to_variable_map.keys():
                  self._suffix_to_variable_map[suffix_name] = []
               self._suffix_to_variable_map[suffix_name].append(variable)
...
#==================================================
   def post_ph_initialization(self, ph):
      ...
      self.fix_discrete_variables_at_exit = False
      ...
      # TBD: the user might also want to give slamming priorities
      ...
      for stage in ph._scenario_tree._stages:
         ...
         if stage != ph._scenario_tree._stages[-1]:
...
##
            for tree_node in stage._tree_nodes:
               ...
               # next, create parameters for each variable in the corresponding tree node.
...
#                     print "dlw debug create stage association ",variable_name, index, stage._name                     
                     variable[index]._stage = stage

                     new_stat_index = variable._index
                     new_stat_parameter_name = "NODESTAT_NUM_ITERS_CONVERGED_"+variable.name
                     new_stat_parameter = Param(new_stat_index,name=new_stat_parameter_name)
                     for newindex in new_stat_index:
                        new_stat_parameter[newindex] = 0
                     tree_node._num_iters_converged[variable.name] = new_stat_parameter
                     
                     # need to know to what we have most recently converged
                     new_conv_index = variable._index
                     new_conv_parameter_name = "NODESTAT_LAST_CONVERGED_VAL_"+variable.name
                     new_conv_parameter = Param(new_conv_index,name=new_conv_parameter_name)
                     for newindex in new_conv_index:
                        new_conv_parameter[newindex] = 0.5 # not an int, so harmless
                     tree_node._last_converged_val[variable.name] = new_conv_parameter
                     
                     # need to know to what has been fixed
                     new_fix_index = variable._index
                     new_fix_parameter_name = "NODESTAT_FIXED_FLAG_VAL_"+variable.name
                     new_fix_parameter = Param(new_fix_index,name=new_fix_parameter_name)
                     for newindex in new_fix_index:
                        new_fix_parameter[newindex] = False
                     tree_node._fixed_var_flag[variable.name] = new_fix_parameter
                     
                     # now make the w hash value storage array
                     new_hash_index = variable._index
                     new_hash_parameter_name = "W_HASH_STORAGE_"+variable.name
                     new_hash_parameter = Param(new_hash_index, ph._iteration_index_set, name=new_hash_parameter_name)
                     for newindex in new_hash_index:
                        for i in range(0, ph._max_iterations+1):
                           new_hash_parameter[newindex,i] = 0
                     tree_node._w_hash[variable.name] = new_hash_parameter

      # store the total variable counts for future reporting.
      (self.total_discrete_vars,self.total_continuous_vars) = ph.compute_variable_counts()
      ...
      if self._configuration_filename is not None:
         if os.path.exists(self._configuration_filename) is True:
            ...
            raise RuntimeError, "***WW PH Extension: Failed to load user-specified configuration from file="+self._configuration_filename
      else:
         print "WW PH Extension: No user-specified configuration file supplied - using defaults"

      # process any suffix information, if it exists.
...
#==================================================
   def post_iteration_0_solves(self, ph):
      ...
      for stage in ph._scenario_tree._stages:
         ...
         if stage != ph._scenario_tree._stages[-1]: # no blending over the final stage
                     ...
                     if is_used is True:
                        ...
                        if instance_fixed_count == 0:
                           ...
                           if isinstance(variable_type, IntegerSet) or isinstance(variable_type, BooleanSet):                           
                              ...
                              attrvariable = ph._model_instance.active_components(Var)[variable_name][index]
                              if hasattr(attrvariable, 'Iter0FixIfConvergedAtLB'):
                                 lb = getattr(attrvariable, 'Iter0FixIfConvergedAtLB')
                              else:
                                 lb = self.Iter0FixIfConvergedAtLB
                              if hasattr(attrvariable, 'Iter0FixIfConvergedatUB'):
                                 ub = getattr(attrvariable, 'Iter0FixIfConvergedAtUB')
                              else:
                                 ub = self.Iter0FixIfConvergedAtUB
                              if hasattr(attrvariable, 'Iter0FixIfConvergedAtNB'):
                                 nb = getattr(attrvariable, 'Iter0FixIfConvergedAtNB')
                              else:
                              ...
                              if self._should_fix_discrete_due_to_conv(tree_node, variable, index, lb, ub, nb):
                                 ...
                                 self._w_hash_acct(ph, tree_node, variable_name, index) # obviously not checking for cycles at iteration 0!
...
#                                 self._w_hash_acct(ph, tree_node, variable_name, index)
                     ...
                     if is_used is True:                        
                        ...
                        if instance_fixed_count == 0:
                           ...
                           if isinstance(variable_type, IntegerSet) or isinstance(variable_type, BooleanSet):                           
                              ...
                              attrvariable = ph._model_instance.active_components(Var)[variable_name][index]
                              if hasattr(attrvariable, 'FixWhenItersConvergedAtLB'):
                                 lb = getattr(attrvariable, 'FixWhenItersConvergedAtLB')
                              else:
                                 lb = self.FixWhenItersConvergedAtLB
                              if hasattr(attrvariable, 'FixWhenItersConvergedAtUB'):
                                 ub = getattr(attrvariable, 'FixWhenItersConvergedAtUB')
                              else:
                                 ub = self.FixWhenItersConvergedAtUB
                              if hasattr(attrvariable, 'FixWhenItersConvergedAtNB'):
                                 nb = getattr(attrvariable, 'FixWhenItersConvergedAtNB')
                              else:
                              ...
                              if self._should_fix_discrete_due_to_conv(tree_node, variable, index, lb, ub, nb):
                                 ...
                                 if self.W_hash_history_len > 0:   
                                    self._w_hash_acct(ph, tree_node, variable_name, index)
                                    computed_cycle_length = self.hash_hit_len(ph, tree_node, variable_name, index)
                                    if (computed_cycle_length >= self.CycleLengthSlamThreshold) and ((ph._current_iteration - self._last_slam_iter) > self.PH_Iters_Between_Cycle_Slams):
                                       ...
                                       # note: i am not slamming the offending variable, but a selected variable
                                       print "Cycle length exceeds slam threshold="+str(self.CycleLengthSlamThreshold)+"; choosing variable to slam"
                              ...
                              if self._should_fix_continuous_due_to_conv(tree_node, variable, index):
                                 ...
                                 # note: we currently don't clam continuous variables!

      # TBD: the 1 might need to be parameterized
      if (ph._current_iteration > self.SlamAfterIter) and ((ph._current_iteration - self._last_slam_iter) > 1) and (ph._converger.isImproving(1)):
         self._pick_one_and_slam_it(ph)
...
#==================================================
   def post_ph_execution(self, ph):
      ...
      if self.fix_discrete_variables_at_exit is True:
   ...
   def _int_convergence_tracking(self, ph, tree_node, variable_name, index, node_min, node_max):
       ...
       if (node_min == node_max) and (type(node_min) is types.IntType): 
          if node_min == tree_node._last_converged_val[variable_name][index]():
             tree_node._num_iters_converged[variable_name][index].value = tree_node._num_iters_converged[variable_name][index].value + 1
          else:
             tree_node._num_iters_converged[variable_name][index].value = 1
             tree_node._last_converged_val[variable_name][index] = node_min
       else:
          tree_node._num_iters_converged[variable_name][index].value = 0
          tree_node._last_converged_val[variable_name][index].value = 0.5  
...
#=========================          
   def _w_hash_acct(self, ph, tree_node, variable_name, index):
      ...
      for scenario in tree_node._scenarios:
         ...
         tree_node._w_hash[variable_name][index,ph._current_iteration].value += getattr(instance,weight_parameter_name)[index].value * self.W_hash_rand_val
...
#=========================
   def hash_hit_len(self, ph, tree_node, variable_name, index):
      ...
      if (tree_node._num_iters_converged[variable_name][index].value == 0) and (tree_node._fixed_var_flag[variable_name][index].value is False):
         ...
         for i in range(max(ph._current_iteration - self.W_hash_history_len - 1, 1), ph._current_iteration - 1, 1):
             ...
             if abs(this_hash_value - current_hash_value) <= ph._integer_tolerance:
                ...
                print "Current hash value="+str(current_hash_value)+" matched (within tolerance) hash value="+str(this_hash_value)+" found at PH iteration="+str(i)+"; cycle length="+str(ph._current_iteration - i)
                return ph._current_iteration - i
      return 0
...
#=========================
   def _fix_var(self, ph, tree_node, variable, index, fix_value):
       ...
       for scenario in tree_node._scenarios:
          ...
          tree_node._fixed_var_flag[variable.name][index].value = True
          ...
          if fixing_reported is False:
             ...
             if index is None:
                print "Fixing variable="+variable.name+" at tree node="+tree_node._name+" to value="+str(fix_value)+"; converged for "+str(tree_node._num_iters_converged[variable.name][index]())+" iterations"
             else:
...
#=========================
   def _should_fix_discrete_due_to_conv(self, tree_node, variable, index, lb_iters, ub_iters, nb_iters):
      ...
      if nb_iters > 0 and tree_node._num_iters_converged[variable_name][index]() >= nb_iters:
         ...
         if lb_iters > 0 and tree_node._num_iters_converged[variable_name][index]() >= lb_iters and conval == lb:
            return True
         elif ub_iters > 0 and tree_node._num_iters_converged[variable_name][index]() >= ub_iters and conval == ub:
            return True
...
#=========================
   def _slam(self, ph, tree_node, variable, index):
      ...
      if isinstance(variable_type, IntegerSet) or isinstance(variable_type, BooleanSet):
         node_min = self.Int_If_Close_Enough(ph, tree_node._minimums[variable_name][index]())
         node_max = self.Int_If_Close_Enough(ph, tree_node._maximums[variable_name][index]())
         anywhere = round(tree_node._averages[variable.name][index].value)
      else:   
         node_min = tree_node._minimums[variable_name][index]()
         node_max = tree_node._maximums[variable_name][index]()
         anywhere = tree_node._averages[variable.name][index].value

      if self.CanSlamToLB is True: fix_value = variable[index].lb()
      if self.CanSlamToMin is True: fix_value = node_min
      if self.CanSlamToUB is True: fix_value = variable[index].ub()
      if self.CanSlamToMax is True: fix_value = node_max
      if self.CanSlamToAnywhere is True: fix_value = anywhere
      if fix_value is False:
         print "Warning: Not allowed to slam variable="+variable.name+str(index)+" at tree node="+tree_node._name
         return False
      else:
         if index is None:
            print "Slamming variable="+variable.name+" at tree node="+tree_node._name+" to value="+str(fix_value)
         else:
            print "Slamming variable="+variable.name+indexToString(index)+" at tree node="+tree_node._name+" to value="+str(fix_value)
...
#=========================
   def _pick_one_and_slam_it(self, ph):
      ...
      for vl in self.slam_list:
         ...
         if hasattr(variable[full_index],'_stage') is True:
            for tree_node in variable[full_index]._stage._tree_nodes:
               # determine if the variable is already fixed (the trusting version...).
               if tree_node._fixed_var_flag[variable_name][full_index].value is False:
