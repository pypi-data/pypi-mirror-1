...
#                     print "dlw debug create stage association ",variable_name, index, stage._name                     
                     ...
                     if (len(new_stat_index) is 1) and (None in new_stat_index):
                        new_stat_parameter = Param(name=new_stat_parameter_name)
                     else:
                     ...
                     if (len(new_conv_index) is 1) and (None in new_conv_index):
                        new_conv_parameter = Param(name=new_conv_parameter_name)
                     else:
                     ...
                     if (len(new_fix_index) is 1) and (None in new_fix_index):
                        new_fix_parameter = Param(name=new_fix_parameter_name)
                     else:
                     ...
                     if (len(new_hash_index) is 1) and (None in new_hash_index):
                        new_hash_parameter = Param(ph._iteration_index_set, name=new_hash_parameter_name)
                     else:
...
#=========================          
   def _w_hash_acct(self, ph, tree_node, variable_name, index):
      ...
      for scenario in tree_node._scenarios:
         ...
         if index is None:
            tree_node._w_hash[variable_name][ph._current_iteration].value += getattr(instance,weight_parameter_name)[index].value * self.W_hash_rand_val
         else:
      ...
      for (variable, index_template, variable_indices) in stage._variables:
          ...
          if isinstance(variable_type, IntegerSet) or isinstance(variable_type, BooleanSet):
             for index in variable_index:
                if index is None:
                   print "%4d        %50ls %20.5f" % (ph._current_iteration, tree_node._w_hash[variable_name][ph._current_iteration], tree_node._w_hash[variable_name][ph._current_iteration]())
                else:
...
#=========================
   def hash_hit_len(self, ph, tree_node, variable_name, index, report_possible_cycles):
      ...
      if (tree_node._num_iters_converged[variable_name][index].value == 0) and (tree_node._fixed_var_flag[variable_name][index].value is False):
         ...
         if index is None:
            current_hash_value = tree_node._w_hash[variable_name][ph._current_iteration]()
         else:
         ...
         for i in range(max(ph._current_iteration - self.W_hash_history_len - 1, 1), ph._current_iteration - 1, 1):
             ...
             if index is None:
                this_hash_value = tree_node._w_hash[variable_name][i]()
             else:
