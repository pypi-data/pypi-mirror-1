...
BINDING_PATTERN = re.compile(r'([0-9.]+|\*):(\d+)')
...
class Recipe(object):
    ...
    def __init__(self, buildout, name, options):
        ...
        self.options['squid_executable'] = self.options.get('squid_executable',
                                                           '/usr/sbin/squid')
        self.options['squid_extra_conf'] = self.options.get('squid_extra_conf',
                                                           '')
        self._get_zope_conf()
    ...
    def _get_zope_conf(self):
        ...
        for cache_key in self.options.get('zope_cache_key','').split(os.linesep):
            ...
            if zope_cache_key:
                ...
                key.update(zope_cache_key)

        self.options['md5_key'] = key.hexdigest()
    ...
    def _make_squid_conf(self):
        ...
        generate squid.conf
        """
        conf = self._generate_cheetah_template('squid.conf_tmpl', self.options)
        ## write conf
    ...
    def _generate_cheetah_template(self, select_tpl, conf):
        """
        get chetah template and return result
        """
        curdir = os.path.dirname(__file__)
        fd = open(os.path.join(curdir,'templates',select_tpl))
        c = ''
        try:
            c = fd.read()
        finally:
        ...
        tp  =  Template.compile(c , baseclass = dict)
        conf = dict(conf)
        return str(tp(**conf))
    ...
    def _make_apacheconf(self):
        ...
        for (proto, binding) in self.front:
            first = True
            #import pdb;pdb.set_trace()
            m = BINDING_PATTERN.match(str(binding))
            port = binding
            ip = '127.0.0.1'
            if m:
                ip = m.groups()[0]
            ...
            for zconf in self.zope_confs:
                ...
                if self.cache_key:
                    for zope_cache_key in self.cache_key:
                        traversal_pattern += ":%%{ENV:%s}" % (zope_cache_key,)
                traversal_pattern = self.options['md5_key'] +  traversal_pattern

                conf['zope_cache_keys'] = self.cache_key
                conf['traversal'] = traversal_pattern
                r = self._generate_cheetah_template('vhost_zcache_tmpl',conf)
                vhost_conf.update(conf)
                ...
                vhost_conf['visible_hostname'] = \
                    conf['host_name']
                vhost_conf['port'] = int(port)
                vhost_conf['ip'] = ip
                vhost_conf['binding'] = binding
                ...
                vhost_conf['debug_apache_rewrite_rules'] = self.options.get('debug_apache_rewrite_rules',0)
                ...
                if first:
                    ...
                    if (vhost_conf['port'] != 80 and vhost_conf['port'] != 443) \
                        or ('*' not in vhost_conf['binding']):
                        ## add listen part if port is not in listen
                        r = "Listen %(binding)s\nNameVirtualHost %(binding)s\n" % vhost_conf + r
                    else:
