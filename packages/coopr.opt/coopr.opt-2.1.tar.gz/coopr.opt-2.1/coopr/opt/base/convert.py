...
import copy
import os
from formats import ProblemFormat
from error import *
...
def convert_problem(args, target_problem_type, valid_problem_types):
    ...
    if not (target_problem_type is None or \
       ...
       raise ConverterError, "Problem type "+str(target_problem_type)+" is not valid"
    if len(args) == 0:
    ...
    if isinstance(tmp,basestring):
       ...
       try:
          if suffix == "lp":
             source_ptype = [ProblemFormat.cpxlp]
          else:
             source_ptype = [getattr(ProblemFormat,suffix)]
      ...
      for s_ptype in source_ptype:
        ...
        for converter in ExtensionPoint(IProblemConverter):
            ...
            if converter.can_convert(s_ptype,ptype):
                ...
                tmp = tuple(tmp)
                problem_files, symbol_map = converter.apply(*tmp)
                return problem_files, ptype, symbol_map
    raise ConverterError, "No conversion possible.  Source problem type: %s.  Valid target types: %s" % (str(source_ptype[0]),map(str,valid_ptypes))
