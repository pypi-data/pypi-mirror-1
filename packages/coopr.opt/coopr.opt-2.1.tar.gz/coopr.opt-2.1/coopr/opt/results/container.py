...
class UndefinedData(object):
    ...
    def __str__(self):
        return "<undefined>"
        #raise TypeError, "Cannot print undefined data"
...
class ScalarData(object):
    ...
    def pprint(self, ostream, option, prefix="", repn=None):
        ...
        if option.ignore_time and str(self.scalar_type) == str(ScalarType.time):
            return ignore
        if type(self.value) is EnumValue:
            value = self.yaml_fix(str(self.value))
        elif type(self.value) is UndefinedData:
            value = '<undefined>'
        else:
            value = self.yaml_fix(self.value)
        if not option.schema and self.description is None and self.units is None:
            print >>ostream, value
        else:
            print >>ostream, ""
            print >>ostream, prefix+'Value:',value
            if not option.schema:
                if not self.description is None:
                    print >>ostream, prefix+'Description:',self.yaml_fix(self.description)
                if not self.units is None:
                    print >>ostream, prefix+'Units:',self.units
            else:
                if not self.scalar_description is None:
                    print >>ostream, prefix+'Description:',self.yaml_fix(self.scalar_description)
                if not self.scalar_type is ScalarType.undefined:
                    print >>ostream, prefix+'Type:',self.yaml_fix(self.scalar_type)
    ...
    def __setattr__(self,name,val):
        ...
        if tmp not in self:
            if strict:
                raise AttributeError, "Unknown attribute `"+str(name)+"' for object with type "+str(type(self))
            #else:
                #print "WARNING: creating attribute '"+str(name)+"' for object with type "+str(type(self))
            #print "DECL1"
            self.declare(tmp)
