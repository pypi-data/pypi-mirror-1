...
default_print_options = Bunch(schema=False, sparse=True, num_solutions=None, ignore_time=False)

SolutionStatus = Enum('globallyOptimal', 'locallyOptimal',
                            'optimal', 'bestSoFar', 'feasible', 'infeasible', 'stoppedByLimit',
                            'unknown', 'unsure', 'error', 'other')
...
class SolutionMap(MapContainer):
    ...
    def _set_value(self, name, val):
        #print "_set_value",name,val
        if isinstance(name,basestring) or type(name) is int:
            self.declare_item(name)
        if type(name) is int:
            name = self._names[name]
        dict.__getitem__(self, name).value = as_number(val)
        #print "_set_value",dict.__getitem__(self, name).value
        ...
        if isinstance(name, basestring):
            if '[' in name:
                ...
                if len(tokens) == 1:
                    ...
                    #print "SETTING",varname,tokens,tuple(tokens)
...
class Solution(MapContainer):
    ...
    def __init__(self):
        ...
        self.declare('variable', value=SolutionMap(), active=False)
        self.declare('constraint', value=SolutionMap(), active=False)
