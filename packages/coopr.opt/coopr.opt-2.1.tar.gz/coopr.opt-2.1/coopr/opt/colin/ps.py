...
from coopr.opt import SolverResults, ProblemSense, SolverStatus, SolutionStatus, TerminationStatus
import solver
import problem
...
class PatternSearch(solver.COLINSolver):
    ...
    def _apply_solver(self):
        ...
        ndim=len(x)
        curr_iter=0
        tstatus = TerminationStatus.maxIterations
        tmsg=None
        while curr_iter < self.max_iterations:
            ...
            if best <= self.min_function_value:
                tstatus = TerminationStatus.minFunctionValue
                tmsg = "%s < %s" % (best,self.min_function_value)
                break
            if self.delta <= self.min_delta:
                tstatus = TerminationStatus.minStepLength
