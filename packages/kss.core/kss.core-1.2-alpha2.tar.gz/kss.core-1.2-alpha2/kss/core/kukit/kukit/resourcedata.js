...
kukit.rd.KssTextValue.prototype.evaluate = function(parms, key) {
    parms[key] = this.txt;
};
...
kukit.rd.KssMethodValue.prototype.check = function() {
    // Check syntax
    this.pprovider = new (kukit.pprovidersGlobalRegistry.get(this.methodname))();
...
kukit.rd.Action.prototype.getParms = function(node, defaultparms) {
    // Return the completed action parameters, based on the node
    ...
    for (var key in this.parms) {
        ...
        kssvalue.evaluate(parms, key, node, defaultparms); 
    }
    return parms;
};
...
kukit.rd.Action.prototype.execute = function(oper) {
    oper = oper.clone({
            'parms': this.getParms(oper.node, oper.parms),
            'action': this
        });
    switch (this.type) {
        case 'D': {
