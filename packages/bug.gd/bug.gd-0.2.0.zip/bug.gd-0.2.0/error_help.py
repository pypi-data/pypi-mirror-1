...
def error_search(error_text):
    ...
    # Print the top (0, 5) errors
    errors = min(len(_error_list), 5)
    if errors > 0:
        for i in range(0, errors):
            e = _buggd.getErrorInfo(_error_list[i])
            print_error(i + 1, len(_error_list), e)            
    else:
        print 'Our database contained no errors like that.'
...
def error_help(item_number = None):
    ...
    try:
            ...
            if type(item_number) == str:
                ...
                if type(item_number) == int:    
                    ...
                    #Lookup and print the error_info
                        if item_number - 1 < len(_error_list):
                            e = _buggd.getErrorInfo(_error_list[item_number - 1])
                            print_error(item_number, len(_error_list), e)
                        else:
                            print "The requested error was out of range for the search.  Pick an error number less than " + str(len(_error_list) + 1) + "."
                    else:
                        print "No error cached yet.  Type 'error_help()' to lookup the last error."
        else:
            if sys.__dict__.has_key("last_value"):
                # Format the traceback
                tbString = StringIO.StringIO()
                traceback.print_exception(sys.last_type, sys.last_value, sys.last_traceback, file=tbString)
                search_text = tbString.getvalue()
                # Do search on bug 
                error_search(search_text)
            else:
                # No error to lookup
        ...
        print "There was an error trying to access the server."
        print str(p.errcode) + " : " + str(p.errmsg)
    except xmlrpclib.Fault, f:
        print "The requested data was not available on the server."
        print "xmlrpclib.Fault " + repr(f)
    except Exception, e:
        print "Unexpected exception using error help."
        print repr(e)
