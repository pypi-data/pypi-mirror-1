...
class Rect(object):
    ...
    def fits(self, other):
        """
        Return True if other fits inside self
        """
    ...
    def split(self, other):
        """
        Substracts other from self, return the two rects left over.
        """
        nw = self.width - other.width
        nh = self.height - other.height
        if nh < 0 or nw < 0: 
            raise ValueError('Rect is too small to split with other')
        return Rect((0,other.height,self.width, nh)), Rect((other.width, 0, nw, self.height))
