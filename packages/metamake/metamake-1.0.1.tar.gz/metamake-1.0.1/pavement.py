...
the 'metamake' tool or the bootstrapped Makefile::
    ...
    # determine if the flag was given on the commandline
...
more parameters to help you out here::
    ...
    Flag("debuglevel").explain("this flag sets the debug level for the build", convert=int, allow=[1,2,3,4])
    
    # determine the value of the flag, which will
    # automatically be converted to an integer
    if Flag("debuglevel").value == 3:
        print "flag was the integer 3"

You can also grab the Flag instance instead of always referring to it by it's
string name::
