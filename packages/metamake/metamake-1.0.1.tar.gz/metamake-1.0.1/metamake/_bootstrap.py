...
__default:
	@python -c "$(BOOTSTRAPMETAMAKE);import Makefile;import metamake;metamake.run_default_task();"

ls:
	@python -c "$(BOOTSTRAPMETAMAKE);import Makefile;import metamake;metamake.list_tasks()"

%%:
	@python -c "$(BOOTSTRAPMETAMAKE);import Makefile;import metamake;metamake.run_task('$@')"
''' % locals())
...
def standalone_deserialize_metamake_from_makefile(makefile_filename):
    ...
    makefile_lines = []
    try:
        makefile_lines = [line.replace("# ","") for line in makefile_file.readlines() if line.startswith("#")]
    finally:
        makefile_file.close()
    ...
    sys.path.append(temp_metamake_import_path)
