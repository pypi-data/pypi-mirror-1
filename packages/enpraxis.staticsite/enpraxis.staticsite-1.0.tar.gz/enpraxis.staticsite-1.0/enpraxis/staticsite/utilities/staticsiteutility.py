...
class StaticSiteUtility(SimpleItem):
    ...
    def deploySiteStructure(self, context, ssprops, dpath):
        ...
        # Deploy CSS Images into the appropriate skin folder
    ...
    def runDocumentFilters(self, portal, current, soup, ssprops):
        ...
        self.filterDocActionImages(soup, portal.portal_url(), current)
        self.filterCSSLinks(soup, current)
        self.filterIEFixesCSS(soup, current)
        self.filterJSLinks(soup, current)
    ...
    def filterDocActionImages(self, soup, portal_url, current):
        tags = soup.findAll('li')
        for x in tags:
            if x.has_key('id'):
                id = x['id']
                if 'document-action' in id:                    
                    for z in x.findAll('img'):
                        src = z['src']
                        surl = '%s/%s' % (portal_url, src)
                        url = self._convertLinkToRelative(surl, current)                        
                        z['src'] = url
    ...
    def filterJSLinks(self, soup, current):
        scripts = soup.findAll('script', type="text/javascript")
        for x in scripts:
            if x.has_key('src') == True:
                url = x['src']
                url = url.split(');')[0]
                url = self._convertLinkToRelative(url, current)
                x['src'] = url                
