...
class schedule_to_rss( cmdline ):
    ...
    def __init__(self, configfilepath ):
        ...
        self._channelname = self._config.get('feed','title')
    ...
    def getcalendaritems(self):
        ...
        for day in range( 0, self.DAYS_IN_SCHEDULE, self.DAYS_PER_READ ):
            log.info("Loading schedule for day: %i" % day )
            start = now + periods.oneday * day
            end = now + periods.oneday * ( day + self.DAYS_PER_READ )
            cal = CalQuery( *self._calargs )
            eventinstances.extend( [ scheduleevent(a) for a in cal.getEventInstances( start , end ) ] )
