...
class pinger( object ):
    ...
    def getEvents(self, evqueue, includeCurrent=False ):
        ...
        scheduleEventList = [ scheduleevent(a, default_artist=self._default_artist, default_website=self._default_website, default_logo=self._default_logo ) for a in eventList ]
        eventListWithGaps = self.insertGaps( now, scheduleEventList )
        
        for ev in eventListWithGaps:
            log.info("Enquing event: %s" % repr(ev) )
            evqueue.put( ev )
