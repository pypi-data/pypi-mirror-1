/* Based on Thickbox Plus Modified by Christian Montoya (http://www.christianmontoya.com)
 * For information, visit http://lab.christianmontoya.com/thickbox-plus/
 * Adapted for last Jquery + minor bugfixes 
 */
	...
	try {
	  ...
	  hideSelectBoxes();
		...
		if (urlIsImage) {//show images
			...
			jQuery("#TB_Image").click(TB_remove); // close when image clicked
			TB_position();
		...
		if(urlType == '.htm' || urlType == '.html' || !urlIsImage){//show html pages
...
function TB_remove() {
  showSelectBoxes();
	// #solves bug when overlay is closed before image loads
...
// fix problem with select boxes zindex
