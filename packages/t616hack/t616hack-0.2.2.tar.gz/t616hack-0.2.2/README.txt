...
The code has since been updated for the retrieval, writing and deletion of
text messages, and has been tested to work over a Bluetooth link using either
Bluetooth networking sockets or a serial port or device (such as COM1 on
Windows or /dev/rfcomm0 on Linux). The Sony Ericsson T610 is equivalent to the
T616 and has also been tested with this software.

Choosing a Communications Mechanism
-----------------------------------

Where available, Bluetooth networking is easier to use. First, discover the
address of your device; then supply that address as the bdaddr parameter with
the various tools and APIs. Discovering addresses can be done by making your
device visible and then using the Linux hcitool program as follows:
...
parameter in the various tools and APIs. Sometimes, reliability may be
improved by setting a different transmission rate to the default, supplying
the baudrate parameter (such as 57600) to the various tools and APIs.

Setting up a Serial Device with Linux
-------------------------------------

In order to make a Bluetooth device available through a device file, first
discover the device address (as described above); then, issue a command like
the following:

rfcomm bind /dev/rfcomm0 01:23:45:67:89:AB

Given a real address (instead of that provided above) and a suitable device
file (such as that provided above), it should then be possible to specify that
device (ie. /dev/rfcomm0) in the port parameter of the T616 constructor
function.
...
New in t616hack 0.2.2 (Changes since t616hack 0.2.1)
----------------------------------------------------

  * Fixed message writing support; added message sending support.
  * Relicensed under the LGPL version 3 or later.
  * Added Ubuntu Feisty (7.04) package support.

New in t616hack 0.2.1 (Changes since t616hack 0.2)
--------------------------------------------------

  * Fixed writeMessage to use the correct timeout and to
    obtain the previous charset before trying to reset it.
  * Added a tool which writes messages to the telephone.

New in t616hack 0.2 (Changes since t616hack 0.1)
------------------------------------------------

  * Introduced improved support for Unicode, meaning that
    text may be supplied as Unicode objects to API methods;
    conversions to and from the telephone's active character
    set will happen without requiring any intervention from
    API clients.
  * Added support for writing new messages.
  * Updated the tools to make use of CMDsyntax-supported
    arguments, and to work with Unicode (where appropriate).
