from silme.core import L10nPackage, L10nObject, Comment, EntityList, Entity
from silme.diff import L10nObjectDiff, EntityListDiff, BlobDiff
...
class TextSerializer():
    ...
    def serialize(cls, element, fallback=None):
        if isinstance(element, L10nPackage):
            return cls.dump_l10npackage(element)
        else:
            return cls.dump_l10nobject(element)
    ...
    @classmethod
    def dump_l10nobject (cls, l10nobject, indent=0, content=True):
    ...
    @classmethod
    def dump_l10npackage (cls, l10npack, indent=0, content=True):
        string = u''
        string += (u'    '*indent)+u'=== L10nPackage: ' + l10npack.id + u' ===\n'
        for key, package in l10npack.packages.items():
            string += cls.dump_l10npackage(package, indent+1, content)
        for key, object in l10npack.objects.items():
            if isinstance(object, L10nObject):
                string += cls.dump_l10nobject(object, indent+1, content)
...
# diff part
    ...
    @classmethod
    def dump_objectdiff (cls, objectdiff, indent=0):
        string = u''
        if objectdiff.diff:
    ...
    def dump_entitylistdiff (cls, entitylistdiff, indent=0):
        string = u''
        added = entitylistdiff.get_entities('added')
        if len(added):
            string += u'     '*indent + u'added entites:\n'
            for entity in added.values():
                string += u'     '*(indent+1) + entity.id + u'\n'
        removed = entitylistdiff.get_entities('removed')
        if len(removed):
            string += u'     '*indent + u'removed entities:\n' 
            for entity in removed.values():
                string += u'     '*(indent+1) + entity.id + u'\n'
        modified = entitylistdiff.get_entities('modified')
        if len(modified):
            string += u'     '*indent + u'modified entities:\n'
            for entitydiff in modified.values():
                string += u'     '*(indent+1) + entitydiff.id + u"(value '"+entitydiff.get_value()[0]+u"' -> '"+entitydiff.get_value()[1]+u"')\n"
        return string

    @classmethod
    def dump_l10nobjectdiff (cls, l10nobjectdiff, indent=0):
        return cls.dump_entitylistdiff(l10nobjectdiff.get_entitylistdiff(), indent=indent)

    @classmethod
    def dump_l10npackagediff (cls, l10npackagediff, indent=0):
        string = u''
        added = l10npackagediff.get_objects('added')
        if len(added):
            string += u'     '*indent + u'\033[1mnew in the latter package:\033[0m\n'
            for object in added:
                string += u'     '*(indent+1) + os.path.join('.', object.id) + u'\n'
        removed = l10npackagediff.get_objects('removed')
        if len(removed):
            string += u'     '*indent + u'\033[1mremoved in the latter package:\033[0m\n'
            for object in removed:
                string += u'     '*(indent+1) + os.path.join('.', object.id) + u'\n'
        modified = l10npackagediff.get_objects('modified')
        if len(modified):
            string += u'     '*indent + u'\033[1mmodified in the latter package:\033[0m\n'
            for object in modified:
                string += u'     '*(indent+1) + os.path.join('.', object.id) + u'\n'
                if isinstance(object, L10nObjectDiff):
                    string += cls.dump_l10nobjectdiff(l10npackagediff.objects[object.id]['object'], indent+2)
                elif isinstance(object, EntityListDiff):
                    string += cls.dump_entitylistdiff(l10npackagediff.objects[object.id]['object'], indent+2)
                elif isinstance(object, BlobDiff):
                    string += cls.dump_objectdiff(l10npackagediff.objects[object.id]['object'], indent+2)

        added = l10npackagediff.get_packages('added')
        if len(added):
            string += u'     '*indent + u'\033[1mnew in the latter package:\033[0m\n'
            for package in added:
                string += u'     '*(indent+1) + os.path.join('.',package.id) + u'\n'
        removed = l10npackagediff.get_packages('removed')
        if len(removed):
            string += u'     '*indent + '\033[1mremoved from latter package:\033[0m\n'
            for package in removed:
                string += u'     '*(indent+1) + os.path.join('.',package.id) + u'\n'
        modified = l10npackagediff.get_packages('modified')
        if len(modified):
            string += u'     '*indent + u'\033[1mmodified in the latter package:\033[0m\n'
            for package in modified:
                string += u'     '*(indent+1) + os.path.join('.',package.id) + u'\n'
                string += cls.dump_l10npackagediff(l10npackagediff.packages[package.id]['package'], indent+2)
        if not len(string):
