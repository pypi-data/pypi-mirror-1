...
class OneToOnePropertyLoader(PropertyLoader):
    ...
    def do_init(self):
        ...
        for prop_key, prop in self.mapper._Mapper__props.iteritems():
            # Skip anything that's not a ColumnProperty (that's all we know how to do)
            # Also skip foreign keys (they are already mapped)
            if not isinstance(prop, ColumnProperty) or prop.columns[0] in self.remote_side:
                continue
