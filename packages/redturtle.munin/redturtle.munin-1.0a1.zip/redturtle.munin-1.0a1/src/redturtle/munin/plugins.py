...
from redturtle.munin.memory import vmkeys
...
def install():
    """ set up plugin symlinks using the current hostname and directory """
    script, cmd, path = argv
    ...
    for name, value in globals().items():
        if isclass(value) and value.__module__ == __name__:
            plugin = '_'.join((hostname, name, project))
...
def run(ip_address='localhost', http_address=8080, port_base=0, user=None):
    ...
    if not 'URL' in environ:
        ...
        environ['URL'] = 'http://%s/@@redturtle.munin.plugins/%%s' % host
