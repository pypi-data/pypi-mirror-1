    ...
    def __call__( self ):
        ...
        for op in self.operations():
            ...
            try:
                ...
                # allow for operations which can't resolve context to be retried once
##                 if result == interfaces.OP_REQUEUE:
##                     if op.requeue:
##                         raise KeyError("Could not Resolve %s"%(op.document_id) )
##                     op.requeue = True
##                     log.info("Sleeping For Requeue")
##                     time.sleep(2)
##                     index_queue.put( op )
##                     continue
            except:
                # we don't want the indexing thread to die from an exception, log it and move on
