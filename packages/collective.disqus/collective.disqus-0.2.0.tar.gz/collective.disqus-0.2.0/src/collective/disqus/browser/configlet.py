...
class IDisqusSettings(Interface):
    ...
    forum_id = TextLine(
        ...
        description = _(u'This short name is used to uniquely identify your website on DISQUS.'),
        required = True
        )
    
    dev_mode = Bool(
        title = _(u'Developer mode'),
        description = _(u'Enables developer mode, which allows testing to be done on local, protected, or otherwise inaccessible servers. '),
        required = True
        )
...
class DisqusControlPanel(ControlPanelForm):
    ...
    description = _("Allow to configure options required to integrate DISQUS comment system with Plone.")
