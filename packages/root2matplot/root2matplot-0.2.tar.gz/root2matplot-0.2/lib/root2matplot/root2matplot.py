...
class HistStack(object):
    ...
    def __init__(self, hists=None, title="", xlabel=None):
        self.hists  = []
        self.kwargs = []
        self.title  = title
        self.xlabel = xlabel
        if hists:
            for hist in hists:
                self.add(hist)
    def add(self, hist, label="_notitle_", **kwargs):
        """
        Add a Hist object to this stack.

        Any additional keyword arguments will be applied to this Hist only
    ...
    def plot_bar(self, replace=None, **kwargs):
        ...
        """
        for i, hist in enumerate(self.hists):
            hist.label = _parse_latex(hist.title, replace)
            hist.title = self.title
            if self.xlabel is not None:
                hist.xlabel = self.xlabel
            all_kwargs = copy.copy(kwargs)
            all_kwargs.update(self.kwargs[i])
...
class RootFile:
    ...
    def get(self, object_name):
        """Return a Hist object from the given ROOT path."""
