...
class Hist(object):
    ...
    def __init__(self, hist, label="__nolabel__", title=None,
        ...
        binlabels = [hist.GetXaxis().GetBinLabel(i) for i in range(1, n + 1)]
        # can do the following with ``any`` starting in 2.5
        has_labels = False
        ...
        if has_labels:
            ...
            self.nbins = n = binlabels.index('')
            self.binlabels = binlabels[:n]
        else:
            self.binlabels = None
...
##             axes.add_line(line)
    def breakmarks(axes, spacing, height):
        plt.axes(axes)
        # Height of gap between axes in data coordinates
        gap_height = spacing / (height + spacing) * (1.0 - plt.ylim()[0])
        segment_height = gap_height / 3
        # Parameters for the breakmarks in data coordinates
        xwidth = 0.008 * (plt.xlim()[1] - plt.xlim()[0])
        xoffsets = [0, +xwidth, -xwidth, 0]
        yvalues  = [plt.ylim()[0] - i * segment_height for i in range(4)]
        for i in range(2):
            line = matplotlib.lines.Line2D(
    ...
    lower_axes.set_xlabel(axes.get_xlabel())
    axes.set_xlabel("")
    breakmarks(axes, breakmark_spacing, axes_height)
    return axes, lower_axes
