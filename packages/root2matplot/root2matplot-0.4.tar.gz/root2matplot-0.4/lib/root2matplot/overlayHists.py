...
#             print 'Hist ', hist.GetFillColor()
    ...
    if options.legend == 1:
        legend_loc = [0.65, 0.89 - legend_height, 0.87, 0.89]
...
def process_hist_matplotlib(path, new_path, files, obj):
    ...
    for i, file in enumerate(files):
        ...
        if options.title:
            if options.title[0] == "+":
                hist.title += options.title[1:]
            else:
                hist.title = options.title
        if options.xlabel:
            if options.xlabel[0] == "+":
                hist.xlabel += options.xlabel[1:]
            else:
                hist.xlabel = options.xlabel
        if options.ylabel:
            if options.ylabel[0] == "+":
                hist.ylabel += options.ylabel[1:]
            else:
    ...
    if (options.errorbar):
        plots = stack.plot_errorbar(replace=options.replace, yerr=True)
    if options.stack:
        stack.plot_barstack(replace=options.replace)
    if options.bar:
        stack.plot_bar(replace=options.replace, alpha=0.5)
...
def add_options_from_configuration(options, nfiles):
    ...
    def append_to_options(config, options):
        ...
        return options
    try:
        from root2matplot import overlayHists_config
        options = append_to_options(overlayHists_config, options)
    except: pass
    try:
        import overlayHists_config
        options = append_to_options(overlayHists_config, options)
    except: pass
    return options

def process_options(options, nfiles):
    options.use_matplotlib = (options.stack or options.errorbar or options.bar)
    try:
        global matplotlib
        import matplotlib
        if options.ext == "png":
...
Note: By default, this script will work completely in ROOT.  If you use the
  options 'bar', 'errorbar', or 'stack', output will be produced in matplotlib.
  More complex style attributes like colors and markers used can be configured
  by placing an overlayHists_config.py in your current directory."""
...
#                       help="Fill histograms with a color")
    ...
    parser.add_option('--ncolors', type=int, default=None,
                      ...
                      "colormap")
    parser.add_option('--legend', type=int, default=1, metavar="LOC",
...
##                       help="Use the given style configuration file; default "
##                       "styles are taken from overlayHists_config.py in the "
##                       "root2matplot distribution or the current directory")
    parser.add_option('--efficiency-from', default=None, metavar="DENOM",
                      help="Divide all plots by DENOM")
    ...
    options = add_options_from_configuration(options, len(files))
    options = process_options(options, len(files))
    global canvas
    canvas = ROOT.TCanvas()
