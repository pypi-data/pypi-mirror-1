...
class Clockin(CalendarConsumer):
    ...
    def formatDate(self, dt):
        ...
        minute = (dt.minute - (dt.minute % 5) + (dt.minute % 5 >= 3 and 5 or 0))
        if minute == 60:
        ...
        if lastEntry is not None:
            if lastEntry.when[0].end_time == lastEntry.when[0].start_time:
                ...
                if self.parseDate(stopTime) > self.parseDate(lastEntry.when[0].end_time):
                    ...
                    hookHandler.process("before_stop", lastEntry)
                    self.service.UpdateEvent(lastEntry.GetEditLink().href, lastEntry)
                    hookHandler.process("after_stop", lastEntry)
                    ...
                    hookHandler.process("before_delete", lastEntry)
                    self.service.DeleteEvent(lastEntry.GetEditLink().href)
                    hookHandler.process("after_delete", lastEntry)
...
#                 if options.description.split(" ")[0].isdigit():
#                     options.description = '#' + options.description
            ...
            hookHandler.process("before_edit", entry)
            self.service.UpdateEvent(entry.GetEditLink().href, entry)
            hookHandler.process("after_edit", entry)
    ...
    def continueLastEntry(self):
        ...
        if lastEntry is not None:
            if lastEntry.when[0].end_time != lastEntry.when[0].start_time:
                ...
                hookHandler.process("before_continue", entry)
                newEntry = self.service.InsertEvent(entry, insert_url)
                hookHandler.process("after_continue", entry)
...
#                 options.description = '#' + options.description
        ...
        if not hasConflict:
            ...
            hookHandler.process("before_new", entry)
            newEntry = self.service.InsertEvent(entry, insert_url)
            hookHandler.process("after_new", entry)
