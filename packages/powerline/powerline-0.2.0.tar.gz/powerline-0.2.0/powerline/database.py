...
class user_model(dbwrap.table_ref):
	...
	def get(self, username = None):
		...
		results = self.rows(username = username).select_one('*', 'allowed_after >= CURRENT_DATE AS banned')
	...
	def get_line_position(self, username):
		...
		try:
			...
			for i, row in enumerate(session.get_unstarted()):
				...
				raise RuntimeError('fatal contradiction in get_line_position')

			return max(0, i + 1 - system.get_num_open())
		except KeyError:
			raise RuntimeError('user does not have a reservation')
...
class session_model(dbwrap.table_ref):
	...
	def create(self, user):
		"""Creates a session from the specified `user`.

		Note: `user` must be a return value from user_model.get().
		"""
		# A little housekeeping...
		self.rows('date(timestamp) < current_date AND start_time IS NULL').delete()

		if not self.hours.open_now():
			raise RuntimeError('Currently closed')

		if user.banned:
			raise RuntimeError('You are currently banned')

		if self.rows(user = user.username, end_time = None).exist():
			raise RuntimeError('You have already reserved a computer')
		...
		self.insert(user = user.username, daily_id = daily_id)
		self.con.commit()
	...
	def get_open_time(self):
		...
		return self.rows('start_time IS NOT NULL ORDER BY start_time DESC LIMIT 1').select_value('start_time') + datetime.timedelta(minutes = settings['session_time'])
