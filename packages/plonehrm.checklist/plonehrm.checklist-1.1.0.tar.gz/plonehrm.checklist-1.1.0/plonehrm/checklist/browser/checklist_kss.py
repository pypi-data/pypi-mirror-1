...
class ChecklistKss(KSSView):
    ...
    def add_or_edit_item(self):
        ...
        # First, we check that all the form was submitted
        # to avoid key errors.
        keys = ['item_id', 'item_text', 'item_due_date',
                'item_link_url', 'item_link_title',
                'item_due_date_year']
        for k in keys:
            if not k in form:
                return
        ...
        # It seems that when we manually reset the date widget
        # to None, this is not applied to the 'item_due_date' field,
        # so we manually check if the year field has been reseted.
        if form['item_due_date_year'] == '0000':
            date = None

        link_url = form['item_link_url']
        link_title = form['item_link_title']
