...
class Recipe(object):
    ...
    def install(self):
        ...
        for instance, sites in instances.items():
            ...
            recipe_egg_path = os.path.dirname(__file__)[:-len(self.options['recipe'])]
            template_file = os.path.join(os.path.dirname(__file__), 'script.py_tmpl')
            ...
            if zeo:
                os.system("%s start" % zeo)

            #run the script
            os.system("%s run %s" % (instance_ctl, tmp_file))

            #stop the zeo server if we have a zeo setup
            if zeo:
                os.system("%s stop" % zeo)
