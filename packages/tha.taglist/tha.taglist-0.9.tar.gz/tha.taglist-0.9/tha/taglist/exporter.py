...
def main(defaults_file=None, specific_outfile=None):
    ...
    # New: prepare a section with latest versions.  Ideally this needs some
    # refactoring to use a proper Jinja template, but I've got another thingy
    # that needs the current behaviour...
    latest_part = ['<h2><a name="latest">Latest versions</a></h2>', '<dl>']
    packages = latest.keys()
    packages.sort()
    for package in packages:
        version = latest[package]
        latest_part.append('<dt>%s</dt><dd>%s</dd>' % (package, version))
    latest_part.append('</dl>')
    outfile.write(HTMLEND % {'latest_versions': '\n'.join(latest_part)})
    outfile.close()
