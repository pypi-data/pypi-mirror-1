    ...
    def popup_javascript(self, context, request, site_url, fieldname, multiValued=False):
        ...
        if multiValued == False:
            ...
            multi_select = '''
                ...
                jq('#archetypes-fieldname-%(fieldname)s ul.searchwidget-selected > li').appendTo("#searchwidget-%(fieldname)s-popup ul.searchwidget-multiselect");
        ...
        for formname in getattr(self, 'searchforms', self._properties['searchforms']):
            ...
            searchwidget_forms[2] += '''
                jq("#searchwidget-%(fieldname)s-%(formname)s-form").load(
                    ...
                    jq("#searchwidget-%(fieldname)s-popup .searchwidget-multiselect").height(jq("#searchwidget-%(fieldname)s-popup .searchwidget-forms").height()-50);
                });
                ''' % {'fieldname':     fieldname,
                       'formname':      formname,
                       'site_url':      site_url,
        ...
        searchwidget_forms = '''
            %(multi_select)s
            jq('#searchwidget-%(fieldname)s-popup .searchwidget-forms').replaceWith(jq('<div class="searchwidget-forms"><ul class="ui-dialog-titlebar ui-widget-header" style="height: 30px;">%(tabs)s<a class="ui-dialog-titlebar-close ui-corner-all" style="-moz-user-select: none;" unselectable="on" role="button" href="#"><span class="ui-icon ui-icon-closethick" style="-moz-user-select: none;" unselectable="on">close</span></a></ul>%(forms)s</div>'));
            %(forms_js)s
            jq("#searchwidget-%(fieldname)s-popup .ui-dialog-titlebar-close").click(function() {
                jq("#searchwidget-%(fieldname)s-popup").dialog("close");
                return false;
            });
        ...
        return '''
            jq("#searchwidget-%(fieldname)s-button").click(function() {
                jq("#searchwidget-%(fieldname)s-popup").dialog({
                    ...
                    width:      %(popup_height)s,
                    resizeStop:     function() {
                                    jq("#searchwidget-%(fieldname)s-popup .searchResults").height(jq("#searchwidget-%(fieldname)s-popup").height()-jq("#searchwidget-%(fieldname)s-popup .searchwidget-forms").height()-jq("#searchwidget-%(fieldname)s-popup .searchwidget-results h1").height()-11); },
                    close:      function(event, options) {
                                    jq(this).dialog("destroy").remove();
                                    jq('<div class="searchwidget-popup" style="display: none;" id="searchwidget-%(fieldname)s-popup"><div class="searchwidget-forms"></div><div class="searchwidget-results"></div></div>').appendTo(jq("#archetypes-fieldname-%(fieldname)s"));
                    },
...
registerPropertyType('popup_width', 'int', SearchWidget)
registerPropertyType('popup_height', 'int', SearchWidget)
