...
from py2app.filters import \
    not_site_packages_filter, not_stdlib_filter, not_system_filter, has_filename_filter
from py2app import recipes
from py2app import eggsupport
from py2app.simpleio import fsinter
...
class py2app(Command):
    ...
    user_options = [
        ...
        ("exclude-site-packages", None,
         "exclude all packages in the site-packages directories"),
        ("dylib-excludes=", 'E',
        ...
        ("no-include-whole-eggs", None,
         "do not include entire eggs, copy only the files detected by dependency analysis"),
        ("semi-standalone", 's',
         "depend on an existing installation of Python " + installation_info()),
        ("applet", None,
         "Shorthand for --semi-standalone --site-packages --exclude-site-packages"),
        ("alias", 'A',
    ...
    def initialize_options (self):
        ...
        self.include_whole_eggs = False # XXX should be true
        self.applet = False
        self.exclude_site_packages = False
    ...
    def finalize_options (self):
        if self.applet:
            ...
            self.semi_standalone = True
            self.exclude_site_packages = True
        ...
        if self.exclude_site_packages:
            self.filters.append(not_site_packages_filter)
    ...
    def get_plist_options(self):
        """
        Return the "PyOptions" dictionary for the
        Info.plist file.
        """
        return dict(
            PyOptions=dict(
    ...
    def get_default_plist(self):
        """
        Return default plist with name, version
        attributes that are extracted from the
        setup.py file and/or main script
        """
        plist = {}
    ...
    def create_binaries(self, py_files, pkgdirs, extensions, loader_files):
        ...
        resourceset = eggsupport.ResourceSet(self.eggs)
        ...
        print "pkgdirs", pkgdirs
        if pkgdirs:
            py_files = filter(packagefilter, py_files)
            print "py_files:", py_files

        extension_list = []
        def prepare_extension(pkgname, filename):
            mod = SourceModule(pkgname, filename)
            py_file = self.create_loader(mod)
            extension_list.append(filename)
            return fsinter.read(py_file.filename)

        os.mkdir('foobar')
        resourceset.finalize(None)
        resourceset.copy_eggs('foobar', prepare_extension, self.compressed)
        resourceset.copy_objects('foobar', self.compressed)

        # XXX: is code below still needed?
    ...
    def copy_versioned_framework(self, info, dst):
        """
        Copy only the required version of a framework.

        We don't copy the entire .framework directory because
        a developer may have several versions of a framework installed
        (such as Tk 8.4 and Tk 8.5).
        """
        version = info['version']
        if version is None:
            # We don't know which version to copy,
            # copy everthing
    ...
    def copy_framework(self, info, dst):
        """
        Copy a framework, but copy only the required
        version of the framework. 

        The Python.framework is treated specially because
        we know exactly what should be copied for this
        framework.

        XXX: Does this mean we have to add per-framework
        recepies? Don't worry about that until someone
        starts complaining.
        """
        force, self.force = self.force, True
        if info['shortname'] == sysconfig.get_config_var.get('PYTHONFRAMEWORK', 'Python'):
    ...
    def raw_copy_framework(self, info, dst):
        """
        Copy an entire framework, all versions without
        leaving out files.
        """
        short = info['shortname'] + '.framework'
        infile = os.path.join(info['location'], short)
    ...
    def copy_python_framework(self, info, dst):
        """
        Copy the Python.framework into the bundle.
        """
    ...
    def build_alias_executable(self, target, script):
        ...
        self.build_bootfile(resdir, target, script, is_alias=True)

        target.appdir = appdir
        return appdir


    def build_bootfile(self, resdir, target, script, is_alias=False):
        """
        Create the bootstrap file inside the Resource
        directory. 

        The bootstrap file for an alias build is slightly different
        to ensure that we load the aliased script file instead of
        one copied into the bundle.
        """
        bootfn = '__boot__'
    ...
    def build_executable(self, target, arcname, pkgexts, copyexts, script):
        ...
        self.build_bootfile(resdir, target, script, is_alias=False)
    ...
    def create_loader(self, item):
        """
        Create a python loader script for an extension.

        The loader script will be added to the zipfile, the
        loader then locates the extension outside of the zipfile
        and manually loads it.
        """
        slashname = item.identifier.replace('.', os.sep)
        pathname = os.path.join(self.temp_dir, "%s.py" % slashname)
        if os.path.exists(pathname):
            # Assume that the contents of the loader script won't
            # change for a given item.
            if self.verbose:
    ...
    #
    # Support for CoreData mapping models and CoreData data models
    #
    # NOTE: These are for deprecated py2app options and will be removed
    # in the future.
    #
