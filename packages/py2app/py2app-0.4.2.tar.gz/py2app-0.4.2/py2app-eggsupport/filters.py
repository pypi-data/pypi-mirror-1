...
def not_site_packages_filter(module, prefix=None):
    """
    Return False if the module is included in a site-packages
    directory.
    """
    if prefix is None:
        prefix = sys.prefix
    ...
    if rp.startswith(prefix):
        ...
        if '/site-python' in rest:
            return False
        if '/site-packages' in rest:
            return False

        return True

    prefix = '/Library/Python/%d.%d/site-packages'%(
            sys.version_info[:2])
    if rp.startswith(prefix):
        return False
