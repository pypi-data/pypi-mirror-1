...
"""
__all__ = ('FileIO', 'ReadOnlyIO', 'fsinter')

import os, time, zipfile

BLACKLIST=('.DS_Store', '.svn', 'CVS')
...
class ReadOnlyIO (object):
    ...
    def is_file(self, path):
        ...
        """
        try:
            zf, zp = self._zippath(path, strict=True)
    ...
    def listdir(self, path):
        ...
        NOTE2: Ignores BLACKLIST-ed files.
        """
        zf, zp = self._zippath(path, strict=False)

        if zf is None:
            return [fn for fn in os.listdir(path) if fn not in BLACKLIST]
