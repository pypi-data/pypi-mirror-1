    ...
    def _load_video_metadata(self):
        ...
        if accessor is not None:
            filename = fileutils.write_ofsfile_to_tempfile(
                # Let the AT field get the edit accessor to support
                # both ATFile which has getRawFile and ATBlob which
                # does not
                self.context.getField(
                    'file').getEditAccessor(self.context)(),
                self.context.getId())
