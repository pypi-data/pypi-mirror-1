...
def abbrev_bare(id, b2a_l=idlib.b2a_l, trimnpad=idlib.trimnpad, abbrevsize=6):
    return b2a_l(trimnpad(id[:abbrevsize], 25), 25)

def abbrev(id, abbrevsize=6):
    return '<' + abbrev_bare(id, abbrevsize=abbrevsize) + '>'

def to_base32(id, b2a_l=idlib.b2a_l):
    return b2a_l(id, 160)

def could_be_id(id, could_be_base32_encoded_l=idlib.could_be_base32_encoded_l):
    return could_be_base32_encoded_l(id, 160)

def could_be_abbrev(id, could_be_base32_encoded_l=idlib.could_be_base32_encoded_l):
...
class AbbrevRepr(humanreadable.BetterRepr):
    ...
    def repr_str(self, obj, level, could_be_base32_encoded_l=idlib.could_be_base32_encoded_l, nulltrans=nulltrans, printableascii=printableascii, abbrev=abbrev):
        if len(obj) == 32:
            # But maybe it was just a 32-character human-readable string, so this is an attempt to detect that case.
            if len(obj.translate(nulltrans, printableascii)) == 0:
                if self.maxstring >= 34:
                    return `obj`
            ...
            return abbrev(obj, abbrevsize=7)
        elif len(obj) == 20:
...
def install_repr_into(mod):
    # You should call this and pass "humanreadable" as the mod.
    # use base32id repr to pretty-print ids embedded in structures
    mod.brepr = AbbrevRepr()
