...
Suppose we want to adapt on requests that have an "Accept-Encoding"
...
Let's now craft a request that will match this interface.
  ...
  >>> request = TestRequest({'Accept-Encoding': 'compress, gzip'})
...
German.
  ...
  >>> IGermanLanguageRequest = IRequest({'Accept-Language': 'de'})

We'll create a new request, which, besides asking a gzipped response, also wants the content in German.

  >>> request = TestRequest(
  ...     {'Accept-Encoding': 'compress, gzip', 'Accept-Language': 'de'})
...
combines these two environments.
  ...
  >>> IZippedGerman = IRequest({'Accept-Encoding': 'gzip', 'Accept-Language': 'de'})
  >>> notify(NewRequest(request))
...
Let's try a request that doesn't match.
  ...
  >>> IZippedFrench = IRequest({'Accept-Encoding': 'gzip', 'Accept-Language': 'fr'})
  >>> notify(NewRequest(request))
...
Adapted requests are global with respect to the environment:
  ...
  >>> IRequest({'Accept-Encoding': 'gzip'}) is IRequest({'Accept-Encoding': 'gzip'})
  True
...
We'll reinitialize the interfaces module.
  ...
  >>> loads(p)
  <IHTTPRequest Accept-Encoding=gzip>
