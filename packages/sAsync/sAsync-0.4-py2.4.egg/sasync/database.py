...
from twisted_goodies import misc, taskqueue
...
def transact(f):
    ...
    @keyword session: Set this option to C{True} to set a I{session} attribute
...
class AccessBroker(object):
    ...
    def getSession(self):
        ...
        """
        def gotConnection(connection):
            d = self.q.call(SA.create_session, bind_to=connection, doNext=True)
