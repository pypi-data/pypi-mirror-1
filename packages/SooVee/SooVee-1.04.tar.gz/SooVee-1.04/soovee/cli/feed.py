...
from main import CACHEOBJ, CACHEDIR, Service #: Get current environs from Main.
...
def update(opmlpath):
    ...
    @rtype: None
    """
            from ..lib.sv_check import Feed
        try:
            #
            #{ Update Rss data
            #
            update = Feed(cache=CACHEOBJ, opml=OPMLPATH, store=CACHEDIR, service=Service)
            #
            #{ Update Rss data
            #
            for index, title in update:
...
#                    filename=Service.toRss(feed['title']), 
...
def manage(action, idnum):
    ...
    if Query(cache=CACHEOBJ, service=Service, action=action, idnum=idnum):
