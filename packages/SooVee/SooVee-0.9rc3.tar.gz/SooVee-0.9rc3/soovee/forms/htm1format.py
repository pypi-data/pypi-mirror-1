...
the index page.in the parent folder.
    ...
    - C{Format.write(filename:str, workstore:str, data:list, cache=None:obj, 
        rx=None:obj) -> None}
    - C{Format.item(item:dict, feed=None:str, rx=None:obj) -> None}
...
class Format():
    ...
    page, description, and cover as well as rss episode title, author, category,
    file location and length in MB.
    ...
    def write(self, filename, workstore, data, cache=None, rx=None):
        ...
        for item in data:
            ...
            fileobj_page = open(
                setName(filename=os.path.join("books", self._pagename), 
                workstore=workstore), "w"
                )
            fileobj_page.write(
            ...
            fileobj_index.write("<li><a href=\"%s\">%s</a><li>\n" % (
                filename, PRETITLE.sub("", item['title'])))
            fileobj_page.close()
    ...
    def item(self, item, feed={}, rx=None):
        ...
        #{ Set page's filename
        #
        self._pagename = "%(itunes:author)s-%(title)s.html"%(feed['sum'])
        feed['sum']['category'] = feed['sum']['category'].split("; ")[2] #:get 3rd
        feed['sum']['imageurl'] = feed['sum']['image']['url']
        awards = rx.IMGSOURCE.findall(item['description'])
        if awards:
            feed['sum']['award'] = "".join(
