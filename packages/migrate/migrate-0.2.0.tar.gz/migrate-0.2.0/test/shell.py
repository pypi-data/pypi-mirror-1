...
class ShellDatabase(Shell,fixture.DB):
    ...
    def test_changes(self):
        """Changes to an outside file shouldn't affect a migration's SQL"""
        repos_name = 'repos_name'
        repos_path = self.tmp()
        script_path = self.tmp_py()
        model_path = self.tmp_named('my_model.py')

        self.assertSuccess(self.cmd('create',repos_path,repos_name))
        self.exitcode(self.cmd('drop_version_control',self.url,repos_path))
        self.assertSuccess(self.cmd('version_control',self.url,repos_path))

        # my_model contains the text model_text_1, which declares the table 
        # my_table. 
        # The change script script_text creates this table in a database.
        model_text_1 = """
        from sqlalchemy import *
        ...
        my_model.meta.connect(migrate_engine)
        
        def upgrade():
            my_model.table.create()
            my_model.table.insert().execute(id=1)
        
        def downgrade():
            my_model.table.drop()
        """.replace('\n        ','\n')
        fd = open(model_path,'w')
        fd.write(model_text_1)
        fd.close()
        ...
        try:
            self.engine.text('drop table my_table').execute()
        except:
            pass
        ...
        self.assertSuccess(self.cmd('upgrade',self.url,repos_path))
        self.assertEquals(self.cmd_db_version(self.url,repos_path),1)
        # Column 'stuff' does not exist
        self.assertRaises(Exception,self.engine.text('select stuff from my_table').execute)
        self.assertSuccess(self.cmd('downgrade',self.url,repos_path,0))
        self.assertEquals(self.cmd_db_version(self.url,repos_path),0)

        # Now a new column is added to my_model.table. (This would need a change
        # script, but that's irrelevant to this test.)
        # This changes the original definition; what our change script is 
        # dependent on. Even so, it shouldn't change the sql we generate
        model_text_2 = """
        from sqlalchemy import *
        ...
        """.replace('\n        ','\n')
        fd = open(model_path,'w')
        fd.write(model_text_2)
        fd.close()

        self.assertEquals(self.cmd_db_version(self.url,repos_path),0)
        self.assertSuccess(self.cmd('upgrade',self.url,repos_path))
        self.assertEquals(self.cmd_db_version(self.url,repos_path),1)
        # The line below means the generated SQL didn't change when the outside
        # file changed: no 'stuff' column
        self.assertRaises(Exception,self.engine.text('select stuff from my_table').execute)
        self.assertSuccess(self.cmd('downgrade',self.url,repos_path,0))
        self.assertEquals(self.cmd_db_version(self.url,repos_path),0)

        # Can preview upgrades: sql
        fd=self.execute(self.cmd('upgrade',self.url,repos_path,'--preview_py'))
        # no upgrade executed
        self.assertEquals(self.cmd_db_version(self.url,repos_path),0,
            "--preview_py changed the DB") 
        # reasonable output
        result = fd.read()
        self.assert_(result.lower().count('my_model.table.create') > 0)
        self.assertSuccess(fd)

        # Can preview upgrades: py
        fd=self.execute(self.cmd('upgrade',self.url,repos_path,'--preview_sql'))
        # no upgrade executed
        self.assertEquals(self.cmd_db_version(self.url,repos_path),0,
            "--preview_sql changed the DB") 
        # reasonable output
        result = fd.read()
        self.assert_(result.lower().count('create table') > 0)
        self.assertSuccess(fd)
    
    @fixture.supported('postgres')
    # Above just ensures the syntax is correct. DB shouldn't have an effect
    # on success here, otherwise. 
