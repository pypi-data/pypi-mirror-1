...
class WorkflowMigrationView(BrowserView):
    ...
    def getTransitionStateMap(self,wf):
        """ Return a dictionary of the destination state ids 
            to transitions ids for a workflow.
            FIXME: Assumes one-step transition workflow,
            ie. for a manager any state has at least one single
            transition that can be used to reach any other state 
        """
        workflow_folder = getattr(self.workflow_tool, wf, None)
        transitions = {}
        if workflow_folder:
            ...
            if trans_folder is not None:
                for trans in trans_folder.objectValues():
                    transitions[getattr(trans,'new_state_id','')] = trans.getId() 
        return transitions
