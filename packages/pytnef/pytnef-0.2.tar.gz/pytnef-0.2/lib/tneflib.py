...
def decode_mapi(data):
   ...
   #debug("%i properties" % num_properties)
   ...
   for i in range(num_properties):
      ...
      if attr_type == SZMAPI_SHORT:
         ...
         for j in range(num_vals):
            ...
            #  debug attr_data[j]
            #else
            attr_data[j] = data[offset:offset+length]; offset += length
            #end

      #elif attr_type == SZMAPI_PT_SYSTIME:
      #   pass
      ...
      attr = TNEFMAPI_Attribute(attr_type, attr_name, attr_data)
      attrs.append(attr)
