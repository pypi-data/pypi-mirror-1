...
class Node(object):
    ...
    def decompose(self):
        ...
        if self.level < self.maxlevel:
            a, d = pywt.dwt(self.data, self.wavelet, self.mode)
    ...
    def reconstruct(self, update=False):
        ...
        if self.isZeroTree:
            ...
            if data_a is None and data_d is None:
                ...
                rec = pywt.idwt(data_a, data_d, self.wavelet, self.mode, correct_size=True)
                if update:                    
                    self.data = rec
                return rec
...
class WaveletPacket(Node):
    ...
    mode - signal extension mode - see pywt.MODES
    maxlevel - maximum level of decomposition
    """
    def __init__(self, data, wavelet, mode=pywt.MODES.sp1, maxlevel=None):
        Node.__init__(self, None, data, "")

        if type(wavelet) is not pywt.Wavelet:
        ...
        if data is not None:
            ...
            if maxlevel is None:
                maxlevel = pywt.dwt_max_level(self.data_size, self.wavelet.dec_len)
        else:
            self.data_size = None
