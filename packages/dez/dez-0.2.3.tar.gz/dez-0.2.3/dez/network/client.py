...
class SocketClient(object):
    ...
    def get_connection(self, host, port, cb, args=[], eb=None, ebargs=None, timeout=60):
        addr = host, port
        if addr not in self.pools:
            self.pools[addr] = ConnectionPool(*addr)
        self.pools[addr].get_connection(cb, args, timeout)

    def start_connections(self, host, port, num, cb, args=[], timeout=None):
        addr = host, port
        if addr not in self.pools:
            self.pools[addr] = ConnectionPool(*addr)
...
#            print 'open a new connection'
    ...
    def __start_connection(self):
            ...
            conn = Connection(self.addr, sock, self)
            conn.connect()
