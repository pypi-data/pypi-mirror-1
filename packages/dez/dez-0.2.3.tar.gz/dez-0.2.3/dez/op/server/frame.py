...
class SendFrame(Frame):
    def success(self, succ, body="", cb=None, cbarg=[]):
        self.conn.callback("success",{"id":self.request_id,"recipients":succ},body,cb,cbarg)

    def failure(self, fail, body="", cb=None, cbarg=[]):
        self.conn.callback("failure",{"id":self.request_id,"recipients":fail},body,cb,cbarg)
