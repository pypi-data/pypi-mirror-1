...
if not n:
    print >>sys.stderr, "WARNING: No user config found in %r, this will be pretty useless." % CONFIG_FILENAME
...
def timeout_update(n):
    ...
    def updatep(now, last_update):
        ...
        for i in xrange(last_update, now):
            if i % n == 0:
                return True
        else:
            return False
...
def print_line(f):
    ...
    for part in order:
        if part in results and results[part]:
            f.write(results[part].encode('utf-8'))
    f.write("\n")
    f.flush()
...
def load_plugins():
    ...
    if errors:
        print >>sys.stderr, "Had errors loading plugins: ", errors
    ...
    class LBNBFDReader(object):
        ...
        def __call__(self):
            try:
                ...
                if t:
                    self.line_buf += t
                    self.line_buf = self.line_buf.splitlines()[-1] + '\n'
            except OSError, e:
            ...
            if self.line_limit and len(self.line_buf) > self.line_limit:
                self.line_buf  = self.line_buf[:self.line_limit] + '...\n'

            return self.line_buf.strip()
...
def plugin_battery(config):
    ...
    '''
    def do_battery():
        battery_dir = '/sys/class/power_supply/CMB1/'
