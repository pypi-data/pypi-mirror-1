Translations for iw.memberreplace
=================================
...
internationalisation and localisation, as well as the gettext standard
(.po files, ...) otherwise, ask Google.
...
templates, you may need to run::
  ...
  (i18ndude)$ cd .../iw.memberreplace/locales
  (i18ndude)$ python updatelocales.py

If some msgids are missing from the synched ".po" files, you may need
to add these msgids to "iw.memberreplace-manual.pot" (for
'iw.memberreplace' domain). Then re-run the above commands.

Having iw.memberreplace in your language
-----------------------------------
...
Easy::
  ...
  $ cd .../iw.memberreplace/iw/sitestat/locales # Here ;)
  $ mkdir -p xx/LC_MESSAGES
  $ cp iw.memberreplace.pot xx/LC_MESSAGES/iw.memberreplace.po
  $ $EDITOR xx/LC_MESSAGES/iw.memberreplace.po
...
Don't forget to add that new directory to the subversion reposo if you
got iw.memberreplace with a subversion checkout.
