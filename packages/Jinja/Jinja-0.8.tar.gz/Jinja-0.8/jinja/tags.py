...
class ForLoopTag(Node):
    ...
    def render(self, context, iterable=None):
        ...
        nodelist = NodeList()
        if iterable is None:
            if len(self._iterable) == 1 and isinstance(self._iterable[0], VariableNode):
        ...
        try:
            ...
            counted = enumerate(iterable)
            iterable = []
            for len_values, item in counted:
...
class RecurseTag(Node):
    ...
    rules = {
        ...
        'in': [
            KeywordNode('recurse'), VariableNode(), KeywordNode('in'), VariableNode()]
    }
...
class FilterTag(Node):
    ...
    def render(self, context):
        ...
        for f, args in self._filters:
            parsed = f(parsed, *[arg.resolve(context) for arg in args])
        return parsed
...
class CaptureTag(Node):
    ...
    def render(self, context):
        var = self._variable.define(context, self._body.render(context))
        return ''
