        ...
        for pos, c in enumerate(data):
            ...
            if self._state == STATE_SEP_WS:
                ...
                if c in '01234567':
                    ...
                    self._invalid = True
                    raise TagLexerError('Expected octal digit or separator', pos,
                                        self._databuffer)
                ...
                if c.isdigit():
                    ...
                    self._invalid = True
                    raise TagLexerError('Expected decimal digit or separator', pos,
                                        self._databuffer)
    ...
    def finish(self):
        ...
        if self._state == STATE_NONE_BOOL_NAME:
            ...
            if self._state == STATE_START:
                raise TagLexerError('No data or dangling separator', self._curpos,
                                    self._databuffer)
            else:
                raise TagLexerError('String not closed', self._curpos,
                                    self._databuffer)
...
class Parser(object):
    ...
    def __init__(self, rules=None):
        if rules is None:
            rules = {}
        self._lexer = Lexer()
        self._chains = dict([(name,(state,chain[:])) for name, (state, chain) in
                            rules.iteritems()])
