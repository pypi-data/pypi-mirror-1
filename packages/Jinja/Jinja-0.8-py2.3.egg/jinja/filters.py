...
def stringfilter(filter):
    ...
    """
    def newfilter(s, context, *vars):
        if not isinstance(s, unicode):
            s = str(s).decode(context.charset, 'replace')
        for idx, var in enumerate(vars):
            if isinstance(var, str):
                vars[idx] = str(var).decode(context.charset, 'replace')
        return filter(s, *vars)
    try:
        newfilter.__name__ = filter.__name__
        newfilter.__doc__ = filter.__doc__
    except:
        pass #requires python2.4
    return newfilter
...
def do_str(obj, context):
    ...
    if isinstance(obj, unicode):
        # this is already unicode, so do nothing
        return obj
    return str(obj).decode(encoding, 'replace')
