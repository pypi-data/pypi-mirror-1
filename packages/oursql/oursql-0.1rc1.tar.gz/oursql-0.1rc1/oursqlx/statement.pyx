...
cdef class _Statement:
    ...
    cdef int buffered, data_waiting, show_table, prepared
    ...
    def __cinit__(self, Connection connection not None, 
        ...
        self.warnings = []
        self.buffered = self.data_waiting = self.prepared = False
    ...
    def prepare(self, statement):
        ...
        cdef MYSQL_BIND *b, *bind = NULL
        cdef column_output *c, *result_data = NULL
        ...
        if not res:
            ...
            return
        try:
            # Otherwise, we (try to) set up the result buffers.
            ...
            for 0 <= i < fields:
                ...
                if field.flags & UNSIGNED_NUM_FLAG == UNSIGNED_NUM_FLAG:
                    b.is_unsigned = True
            if mysql_stmt_bind_result(self.stmt, bind):
                self._raise_error()
        except:
            ...
            # But in any case, we don't need either of these things.
    ...
    def execute(self, *parameters):
        ...
        if not self.prepared:
            raise ProgrammingError('no statement has been prepared to execute')
        if self.data_waiting:
        ...
        try:
            ...
            if self.conn.warning_count():
                q = _Query(self.conn, 'SHOW WARNINGS')
                self.warnings.extend(_exceptions_from_warnings_query(q))
        finally:
            PyMem_Free(buf)
    ...
    def fetchone(self):
        ...
        for 0 <= i < self.result_fields:
            ...
            if c.is_null:
                ...
                # endian format. Align it and then convert it to an int 
                # instead of passing back a string that people would probably
                # be converting to ints anyway.
                memset(b.c, 0, 8)
                memcpy(b.c + (8 - c.length), &d.c, c.length)
                val = ntohll(b.u64)
            else:
                val = PyString_FromStringAndSize(&d.c, c.length)
