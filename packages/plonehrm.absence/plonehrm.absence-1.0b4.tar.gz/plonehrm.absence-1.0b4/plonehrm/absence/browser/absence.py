...
def simple_close_absence(context, end_date = None):
    ...
    if end_date is None:
        end_date = date.today()
    try:
        absencelist.close_absence(end_date)
        message = None
    except AbsenceDateError, exc:
        message = exc
    return message
...
class AbsenceCloseView(PloneKSSView):
    ...
    @kssaction
    def close_absence(self, end_date, end_date_year,
        ...
        if end_date:
            ...
            end_date = date.today()
        message = simple_close_absence(self.context, end_date)
        view = self.context.restrictedTraverse('@@plonehrm.absence')
        if message:
            view.error_message = message
