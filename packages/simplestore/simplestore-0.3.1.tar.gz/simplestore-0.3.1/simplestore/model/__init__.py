...
class KeyProperty(Property):
    ...
    def validate(self, value):
        ...
        if value is not None and not isinstance(value, str):
            # < 0.3.1 there was a bug here for some reason. I'm not sure
            # why I didn't catch it before publishing it. The bug was just
            # type(value).__name instead of type(value).__name__
...
class FloatProperty(Property):
    ...
    def validate(self, value):
        ...
        if value is not None and not isinstance(value, float):
            raise Exception('Property %s must be a float, not a %s' %
                            (self.name, type(value).__name__))
        return value
