...
class DataStore(object):
    ...
    def choose_shard(self, id):
        i = long(md5(id).hexdigest(), 16) % len(self.shards)
    ...
    def index_entity(self, entity):
        ...
        for shard, sql_list in shard_sql.iteritems():
            for sql in sql_list:
                self.execute_on_shard(shard, sql)
    ...
    def put(self, entity):
        ...
        self.execute_on_shard(shard, sql, id, fields)
    ...
    def update(self, entity, **kwargs):
        ...
        shard = self.choose_shard(id)
        self.execute_on_shard(shard, sql, fields, id)
    ...
    def deindex_entity(self, entity):
        ...
        for index in self.indexes.values():
            ...
            self.execute_on_shard(shard, sql, id)
    ...
    def query_on_shard(self, shard, sql, *parameters):
        conn = self.get_connection(shard)
        return conn.query(sql, *parameters)

    def execute_on_shard(self, shard, sql, *parameters):
        """execute sql on the given shard"""
        conn = self.get_connection(shard)
    ...
    def global_execute(self, sql, *parameters):
        ...
        for shard in self.shards:
            self.execute_on_shard(shard, sql, *parameters)
