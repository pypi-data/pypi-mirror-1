...
# uses pg8000 library written in native Python.
#
...
import pg8000; from pg8000 import DBAPI; DBAPI.paramstyle = "qmark"
import pg8000.types; from pg8000.types import Bytea
...
import typecheck; from typecheck import typecheck, list_of, tuple_of
...
class Resource(SQLResource): # PostgreSQL resource
    ...
    def __str__(self):
        return "pgsql://{0[0]:s}:{0[1]:d}/{1:s}".\
               format(self._server_address, self._database)
    ...
    def _execute_sql(self, sql, **params):
        ...
        try:
            ...
            param_list = ", ".join(map(lambda x: isinstance(x, str) and
                                                 "'{0:s}'".format(x) or str(x),
                                       params))
            if param_list:
        ...
        if rowcount < 0:
            pmnc.log.info("{0:s} << OK".format(self))
        else:
            pmnc.log.info("{0:s} << OK, {1:d} record(s) affected".format(self, rowcount))
...
def self_test():
    ...
    def test_supported_types():
        ...
        def test_value(typename, value):
            ...
            xa = pmnc.transaction.create()
            t = "table_{0:s}".format(random_string(8))
            xa.postgresql_1.execute("SELECT ({{value}})::{typename} AS value".format(typename = typename),
        ...
        def test_int():
            ...
            with expected(ProgrammingError):
        ...
        def test_datetime():
            ...
            with expected(TypeError("type timedelta is not supported")):
               test_value("", td)
    ...
    def test_errors():
        ...
        fake_request(10.0)
        xa = pmnc.transaction.create()
        xa.postgresql_1.execute("SELECT 1/0")
        with expected(ProgrammingError("(b'ERROR', b'22012', b'division by zero')")):
            xa.execute()
        ...
        fake_request(10.0)
        xa = pmnc.transaction.create()
        xa.postgresql_1.execute("CREATE TABLE {t} (id int PRIMARY KEY)".format(t = t),
                                "INSERT INTO {t} VALUES ({{id}})".format(t = t),
                                "INSERT INTO {t} VALUES ({{id}})".format(t = t),
                                id = 123)
        with expected(ProgrammingError("(b'ERROR', b'23505', b'duplicate key")):
            xa.execute()

        fake_request(10.0)
        xa = pmnc.transaction.create()
        xa.postgresql_1.execute("DROP TABLE {t}".format(t = t))
        with expected(ProgrammingError("(b'ERROR', b'42P01', b'table \"{0:s}\" does not exist')".format(t))):
            xa.execute()
    ...
    def test_unsupported_types():
        ...
        with expected(TypeError("type time cannot be converted")):
    ...
    def test_stress():
        ...
        fake_request(10.0)
        xa = pmnc.transaction.create()
        xa.postgresql_1.execute("CREATE TABLE {t} (id int PRIMARY KEY, key char(8) UNIQUE NOT NULL, "
        ...
        for i in range(10):
            ...
            for j in range(100):
                ...
                sqls.append(sql)
            xa.postgresql_1.execute(*sqls, **params)
            xa.execute()
        ...
        def th_proc():
            while not stop.expired:
                ...
                key = randint(0, 999)
                xa = pmnc.transaction.create()
                xa.postgresql_1.execute("SELECT id FROM {t} WHERE key = {{key}}".format(t = t), key = str(key))
                assert xa.execute()[0][0][0]["id"] == key
                count.next()
