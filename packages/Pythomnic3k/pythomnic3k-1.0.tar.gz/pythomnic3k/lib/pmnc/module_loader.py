...
import typecheck; from typecheck import typecheck, by_regex, callable, list_of
...
valid_cage_name_suffix = by_regex("^([A-Za-z0-9_-]{1,32}|[A-Za-z0-9_-]{0,32}:retry|[A-Za-z0-9_-]{1,32}:reverse)$")
...
class ModuleLoader:
            ...
            try:
                if priority is None:
                    ...
                    if self._src_module:
                        ...
                        s += " # {0:s}.py:{1:d} in {2:s}()".format(self._src_module, line, func)
    ...
    def _set_log_priority(self, log_priority):
    ...
    # this method handles syntax for RPC calls
    # pmnc("other_cage", option = value).module.method(*args, **kwargs)
    #                                  ^ the method is invoked here

    def __call__(self, cage_name, **options):
        return RemoteModuleLoader(self, cage_name, options)
...
class ModuleLoaderProxy:
    ...
    def __call__(self, cage_name, **options):
        return self._loader.__call__(cage_name, **options)
...
class RemoteModuleLoader:
    ...
    @typecheck
    def __init__(self, module_loader: ModuleLoader, cage_name: valid_cage_name_suffix, options: dict):
        self._module_loader, self._cage_name, self._options = module_loader, cage_name, options

    def __getattr__(self, name):
        return RemoteModuleProxy(self._module_loader, self._cage_name, self._options, name)
...
class RemoteModuleProxy:
    ...
    def __init__(self, module_loader, cage_name, options, module_name):
        self._module_loader, self._cage_name = module_loader, cage_name
        self._options, self._module_name = options, module_name

    def __getattr__(self, name):
        return RemoteMethodProxy(self._module_loader, self._cage_name,
...
class RemoteMethodProxy:
    ...
    def __call__(self, *args, **kwargs):
        return self._module_loader.remote_call.execute(
                        self._cage_name, self._module_name,
...
if __name__ == "__main__":
    ...
    print("remote cage call: ", end = "")

    fake_request(30.0)

    write_module("remote_call.py",
                 "__all__ = ['execute', 'test']\n"
                 "def execute(cage, module, method, args, kwargs, **options):\n"
                 "    return cage, module, method, args, kwargs, options\n"
                 "def test():\n"
                 "    return pmnc('another_cage', opt_1 = 'aaa').foo.bar(1, '2', biz = 'baz')\n"
                 "# EOF")

    assert pmnc.remote_call.test() == \
           ("another_cage", "foo", "bar", (1, "2"), {"biz": "baz"}, {"opt_1": "aaa"})
    ...
    pmnc.logging.test()
    assert log_lines[-6:] == \
           [ "ERROR # logging.py:5 in test()",
             "MESSAGE # logging.py:6 in test()",
             "WARNING # logging.py:7 in test()",
             "DEFAULT # logging.py:8 in test()",
             "INFO # logging.py:9 in test()",
             "DEBUG # logging.py:10 in test()" ]

    pmnc._set_log_priority(1)

    del log_lines[:]
    pmnc.logging.test()
    assert log_lines == [ "ERROR # logging.py:5 in test()" ]

    pmnc._set_log_priority(6)
    ...
    sleep(1.0)
    for f in listdir(default_dir):
        remove(os_path.join(default_dir, f))

    sleep(1.0)
    for f in listdir(cage_dir):
        remove(os_path.join(cage_dir, f))
