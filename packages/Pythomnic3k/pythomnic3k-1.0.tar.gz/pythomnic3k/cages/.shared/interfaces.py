...
def start():
    ...
    _start_interfaces(pmnc.config.get("interfaces"))
...
def begin_request(**kwargs) -> Request:
    ...
    request = _request_factory.create(**kwargs)
    if request is not None:
        pmnc.log.debug("request {0:s} begins, {1:d} request(s) are now active".\
                       format(request.unique_id, _request_factory.count))
        return request
    else:
...
def end_request(request: Request):

    _request_factory.destroyed() # we don't care exactly which request is being destroyed

    pmnc.log.debug("request {0:s} has ended, {1:d} request(s) remain active".\
                   format(request.unique_id, _request_factory.count))
...
###############################################################################
# this hook is used by other modules to access special interfaces with predefined
# names, such as "rpc" or "retry"
