...
class ModuleState:
    ...
    def __init__(self, dir: os_path.isdir):
        ...
        env.set_lg_bsize(1048576)
        env.set_flags(bsddb.DB_AUTO_COMMIT | bsddb.DB_TXN_NOWAIT, 1)
        env.log_set_config(bsddb.DB_LOG_AUTO_REMOVE, 1)
...
def _get_module_state(module_name: str) -> ModuleState:
    ...
    if not _checkpoint_thread.is_alive():
...
###############################################################################
# this method is used by the heavy checkpoint thread to acquire exclusive access
# to state shared lock, it couldn't just use pmnc.request.acquire() because it
# has no implicit waiting timeout like a worker thread and would lock infinitely

def _acquire_state_lock():

    # the exponential backoff is used here because the caller needs the lock
    # for checkpointing or shutting down the state, nothing urgent, can wait

    lockout_interval = 0.05
    max_lockout_interval = pmnc.config_interfaces.get("request_timeout")
    while not _state_lock.acquire(lockout_interval): # needs to access lock directly
        lockout_interval = min(lockout_interval * 1.2, max_lockout_interval)

###############################################################################

def _checkpoint_state():

    _acquire_state_lock() # requires exclusive access
    try:
        with _state_cache_lock:
...
def _checkpoint_thread_proc():

    while not current_thread().stopped(pmnc.config.get("checkpoint_interval")):
        try:
            _checkpoint_state()
        except:
            pmnc.log.error(exc_string()) # log and ignore
...
def start():
    ...
    if not _no_bssdb3:
        _checkpoint_thread = HeavyThread(target = _checkpoint_thread_proc,
                                         name = "checkpoint")
...
def implicit_transaction(content: callable, *args, __source_module_name):
    ...
    if pmnc.request.acquire_shared_fast(_state_lock):
        try:
            module_state = _get_module_state(__source_module_name)
...
def start_transaction(source_module_name: str):
    ...
    if pmnc.request.acquire_shared(_state_lock):
        try:
            module_state = _get_module_state(source_module_name)
            txn = module_state.start_transaction()
        except:
...
def self_test():
    ...
    def test_log_truncation():
        ...
        for i in range(32):
            pmnc.state.set(str(i), urandom(4096))

        logs_before = current_logs()

        for i in range(64):
            pmnc.state.set(str(randint(0, 127)), urandom(65536))

        logs_after = current_logs()
