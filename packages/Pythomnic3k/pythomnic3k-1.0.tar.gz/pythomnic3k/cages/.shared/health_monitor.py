...
class HealthMonitor:
    ...
    # note that only one health monitor is expected to run,
    # therefore there is no name nor any other arguments
            ...
            try:
                ...
                for cage, nodes in probe_cages.items():
                    ...
                    for node, cage_info in nodes.items():
                        ...
                        # therefore entitled to be terminated without warning at shutdown,
                        # this is ok, because they do no useful work for the clients

                        request = Request(timeout = pmnc.config_interfaces.get("request_timeout"),
                                          interface = "health_monitor", protocol = "n/a",
                                          parameters = dict(auth_tokens = {}))

                        self._probe_thread_pool.enqueue(request, self.wu_probe_cage,
                                                        (node, cage,
                                                         cage_info["location"],
                                                         cage_info["probe_result"]),
                                                        {})
    ...
    def wu_probe_cage(self, node, cage, location, prev_probe_result):
        try:
            ...
            cage_desc = "cage {0:s}.{1:s} at {2:s}".format(node, cage, location)

            pmnc.log.info("probing {0:s}".format(cage_desc))
            try:
                probe_result = self._probe_cage(node, cage, location)
            except:
                pmnc.log.error("probing {0:s} failed: {1:s}".format(cage_desc, exc_string()))
                self._up_down_queue.push((node, cage, "down"))
            else:
                pmnc.log.info("probing {0:s} succeeded".format(cage_desc))
                if prev_probe_result == "restarted":               # if the cage has restarted
                    self._up_down_queue.push((node, cage, "down")) # we push a down event first
    ...
    def _probe_cage(self, node, cage, location) -> dict:
        ...
        try:
            rpc.begin_transaction("", source_module_name = __name__, transaction_options = {},
                                  resource_args = (), resource_kwargs = {}) # note empty transaction id
            try:
    ...
    def _schedule_up_down_event(self, node, cage, up_down, probe_result = None):
        ...
        pmnc.log.info("health monitor introduces request {0:s} for registering "
                      "cage up/down event, deadline in {1:.01f} second(s)".\
                      format(request.unique_id, request.remain))

        pmnc.performance.event("interface.health_monitor.request_rate")

        pmnc.interfaces.enqueue(request, self.wu_up_down_event, (node, cage, up_down, probe_result))
    ...
    @typecheck
    def wu_up_down_event(self, node: str, cage: str, up_down: one_of("up", "down"),
        ...
        try:
            ...
            pmnc.performance.sample("interface.health_monitor.pending_time", pending_ms)

            try:

                try:

                    with pmnc.performance.timing("interface.health_monitor.processing_time"):
                        try:
                            if pmnc.request.self_test == __name__: # self-test harness
                                self._rpc_interface.push_test_event(node, cage, up_down, probe_result)
                            elif up_down == "up":
                                pmnc.health_monitor_event.cage_up(node, cage, probe_result)
                            elif up_down == "down":
                                pmnc.health_monitor_event.cage_down(node, cage)
                        finally:
                            elapsed_sec = pmnc.request.elapsed
                            elapsed_ms = int(elapsed_sec * 1000)

                except:
                    pmnc.performance.sample("interface.health_monitor.response_time.failure", elapsed_ms)
                    pmnc.performance.event("interface.health_monitor.response_rate.failure")
                    raise
                else:
                    pmnc.performance.sample("interface.health_monitor.response_time.success", elapsed_ms)
                    pmnc.performance.event("interface.health_monitor.response_rate.success")

            except:
                pmnc.log.error("request {0:s} has failed in {1:.01f} second(s)".\
                               format(pmnc.request.unique_id, elapsed_sec))
                raise
            else:
                pmnc.log.info("request {0:s} has succeeded in {1:.01f} second(s)".\
                              format(pmnc.request.unique_id, elapsed_sec))
...
def self_test():
    ...
    class FakeRpcInterface:
        ...
        def push_test_event(self, node, cage, up_down, probe_result):
    ...
    def start_health_monitor(cages_list, responses_list):
        ...
        hm = pmnc.health_monitor.HealthMonitor()
        hm._probe_cage = _probe_cage
