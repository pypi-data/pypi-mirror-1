...
class TcpConnection:
    ...
    def _go_active(self):
        self._idle = False
        self._timeout = Timeout(pmnc.config_interfaces.get("request_timeout"))
    ...
    def _request_parameters(self):                              # this is made a separate method to allow
        return dict(auth_tokens = dict(peer_ip = self._peer_ip, # overriding it in SslConnection class
                                       encrypted = False))

    def _begin_request(self):
        protocol = self._handler.protocol
        self._request = \
            pmnc.interfaces.begin_request(
                    timeout = self._timeout.remain, interface = self._interface_name,
                    protocol = protocol, parameters = self._request_parameters())
        pmnc.performance.event("interface.{0:s}.request_rate".format(self._interface_name))
        pmnc.log.info("{0:s} via interface {1:s} introduces {2:s} request {3:s}, deadline "
                      "in {4:.01f} second(s)".format(self._cf, self._interface_name,
                                                     protocol, self._request.unique_id,
                                                     self._request.remain))

    def _end_request(self, reason = None):
        if self._request:
            elapsed_sec = self._request.elapsed
            elapsed_ms = int(elapsed_sec * 1000)
            if reason is None:
                pmnc.performance.sample("interface.{0:s}.response_time.success".\
                                        format(self._interface_name), elapsed_ms)
                pmnc.performance.event("interface.{0:s}.response_rate.success".\
                                       format(self._interface_name))
                pmnc.log.info("request {0:s} has delivered a response in {1:.01f} "
                              "second(s)".format(self._request.unique_id, elapsed_sec))
            else:
                pmnc.performance.sample("interface.{0:s}.response_time.failure".\
                                        format(self._interface_name), elapsed_ms)
                pmnc.performance.event("interface.{0:s}.response_rate.failure".\
                                       format(self._interface_name))
                pmnc.log.info("request {0:s} has been aborted: {1:s}".\
                              format(self._request.unique_id, reason))
            pmnc.interfaces.end_request(self._request)
            self._request = None
...
class SslConnection(TcpConnection):
    ...
    def __init__(self, interface_name: str, socket, handler_factory: callable,
                 ...
                 required_auth_level: one_of(CERT_REQUIRED, CERT_OPTIONAL)):
        TcpConnection.__init__(self, interface_name, socket, handler_factory)
...
class TcpInterface: # called such so as not to be confused with "real" Interface's
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 required_auth_level: optional(one_of(CERT_REQUIRED, CERT_OPTIONAL)) = CERT_OPTIONAL):
        ...
        if self._use_ssl:
            self._connection_factory = \
                lambda socket: SslConnection(self._name, socket, self._handler_factory,
                                             self._ssl_key_cert_file, self._ssl_ca_cert_file,
                                             required_auth_level)
        else:
            self._connection_factory = \
                lambda socket: TcpConnection(self._name, socket, self._handler_factory)
    ...
    def wu_process_tcp_request(self, socket, connection):
        try:
            ...
            # the connection has its response ready, is pushed back to the I/O queue
    ...
    # by default requests are enqueued to the interfaces thread pool,
    # but this method can be overridden to use a different one
...
class TcpResource: # called such so as not to be confused with "real" Resource's
    ...
    def __init__(self, name: str, *,
        ...
        self._peer_cn = None
        self._peer_info = "{0:s} {1:s}://{2[0]:s}:{2[1]:d}/".\
    ...
    peer_info = property(lambda self: self._peer_info)
    ...
    def _tcp_send(self, data):
        if not select([], [self._socket], [], pmnc.request.remain)[1]:
            raise Exception("request deadline writing data to {0:s}".format(self._peer_info))
        return self._socket.send(data)

    def _tcp_recv(self, n):
        if not select([self._socket], [], [], pmnc.request.remain)[0]:
            raise Exception("request deadline reading data from {0:s}".format(self._peer_info))
    ...
    def connect(self):
        ...
        pmnc.log.debug("connecting to {0:s}".format(self._peer_info))
        try:
            ...
            try:
                ...
                if self._use_ssl: # perform SSL handshake
                    ...
                    self._peer_info = "{0:s} ssl://{1[0]:s}:{1[1]:d}/cn={2:s}".\
            ...
            pmnc.log.error("connection to {0:s} failed: {1:s}".\
                           format(self._peer_info, exc_string()))
            raise
        else:
            pmnc.log.debug("connected to {0:s}".format(self._peer_info))
    ...
    def send_request(self, request: bytes, response_handler: optional(callable) = None):
        ...
        if request:
            pmnc.log.debug("sending {0:d} byte(s) to {1:s}".format(len(request), self._peer_info))

        # send request, occassionally timing out
        ...
        while data:
            sent = self._send(data)
            if sent == 0:
                raise Exception("unexpected eof writing data to {0:s}".format(self._peer_info))
            unsent = len(data) - sent
            if unsent > 0:
        ...
        pmnc.log.debug("waiting for response from {0:s}".format(self._peer_info))

        # receive response, occassionally timing out
        ...
        while True:
            ...
            if response is not None:          # because eof may be legitimate at this time
                break
            if not data:
                raise Exception("unexpected eof reading data from {0:s}".format(self._peer_info))

        pmnc.log.debug("received {0:d} byte(s) from {1:s}".format(response_length, self._peer_info))
    ...
    def disconnect(self):
        try:
            pmnc.log.debug("disconnecting from {0:s}".format(self._peer_info))
            self._socket.close()
        except:
            pmnc.log.error(exc_string()) # log and ignore
...
def self_test():
    ...
    def test_interface(ssl_key_cert_file, ssl_ca_cert_file):
        ...
        def start_interface(handler_factory, **kwargs):
            ...
            ifc = TcpInterface("test", handler_factory, **kwargs)
            ifc.start()
            return ifc
        ...
        def test_dos():
            ifc = start_interface(SlowLineEchoParser, max_connections = 100)
            try:
                ...
                ths = [ Thread(target = th_proc) for i in range(100) ]
                for th in ths: th.start()
                for th in ths: th.join()
