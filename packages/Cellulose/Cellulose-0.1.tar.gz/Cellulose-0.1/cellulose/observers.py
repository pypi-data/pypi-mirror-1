...
from cellulose.cells import DependantCell, dependant_cell_stack
...
class ObserverBank(set):
    ...
    def flush(self, force=False):
        ...
        """
        while len(self):
            if not force and dependant_cell_stack._call_stack:
                dependant_cell_stack.call_when_empty.add(lambda: self.flush())
                return
            self.pop().check()


# The ObserverBank instance that Observers default to.
