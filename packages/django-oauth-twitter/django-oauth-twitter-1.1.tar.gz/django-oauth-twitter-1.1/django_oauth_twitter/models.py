...
class TwitterUser(models.Model):
    ...
    def get_friends(self):
        friends = TwitterApi(self.access_token).GetFriends(cursor=None)
        return self.objects.filter(twitter_id__in=[f.id for f in friends])

    def get_followers(self):
        followers = TwitterApi(self.access_token).GetFollowers(cursor=None)
        return self.objects.filter(twitter_id__in=[f.id for f in followers])
