...
def kiln(dirname, verbose=False):
	from kiln import FileKiln, DataElement
	b = FileKiln(os.path.join(dirname, 'base.html'))
	t = FileKiln(os.path.join(dirname, 'template.html'))
	def render():
		base = b.clone()
		template = t.clone()
		c_cont = base.get_element_by_id('center_content')
		if c_cont:
			c_cont.replace(template)
		del template
		base.get_element_by_tag('title').children= [DataElement('Just a test')]
		base.get_element_by_id('greeting').children = [DataElement('Hello joe')]
		item = base.get_element_by_id('item')
		il = item.parent
		for a in ['Number %d' % num for num in range(1, 15)]:
			it = item.clone()
			it.children = [DataElement(a, it)]
			il.children.append(it)
		return str(base)
