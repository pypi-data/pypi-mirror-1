        ...
        if filename is not None:
            ...
            if fr != gsamplerate:
                ...
                if nc == 2:
                    ...
                    ndata = self.data.reshape(2, len(self.data)/2, order='FORTRAN')
                    left, right = ndata[0], ndata[1]
                    nleft = resample(left, scale)
                    nright = resample(right, scale)
                    ndata2 = numpy.vstack((nleft, nright))
                    ndata3 = numpy.ravel(ndata2, order='F')
                    self.data = ndata3
            if nc != gchannels:
                ...
                if nc == 1:
                    ...
                    ndata = numpy.vstack((self.data, self.data))
                    ndata2 = numpy.ravel(ndata, order='F')
                    self.data = ndata2
                if nc == 2:
                    ...
                    ndata = self.data.reshape(2, len(self.data)/2, order='FORTRAN')
                    left, right = ndata[0], ndata[1]
                    self.data = (0.5 * left + 0.5 * right).astype(numpy.int16)
            return
        if data is not None:
            self.data = numpy.fromstring(data, dtype=numpy.int16)
            return
        assert False
...
def init(samplerate=22050, chunksize=1024, stereo=True, microphone=False):
    """Initialize mixer
