...
#                    fc.name = make_name(self.name, fc.name)
    ...
    def funccalllist(self):
        dot = []
        for fn in self.funcalls:
            for fc in fn.calls:
                dot.append('%s -> %s;' % (fn.name, fc.name))
        for cl in self.classes:
            dot.extend(cl.funccalllist())
        return dot

    def header(self):
        dot = []
        dot.append('subgraph cluster_%s {' % self.name)
        dot.append('label = "%s";' % self.name)
        dot.append('style = dashed;')
        for fn in self.funcalls:
            dot.append('%s;' % fn.name)
        for cl in self.classes:
            dot.extend(cl.header())
        dot.append('}')
        return dot
    ...
    def todotlist(self):
        ...
        dot.extend(self.header())
        # fun calls outside the cluster
        dot.extend(self.funccalllist()) 
        return dot 
...
class FileInput(Element, HashableName):
    ...
    def todot(self, subgraph=False):
        dot = []
        if subgraph:
            dot.append('subgraph cluster_%s {' % self.name)
            dot.append('style = filled; fillcolor = lightgrey;')
        else:
            dot.append('digraph %s {' % self.name)
            dot.append('clusterrank=min;')
        dot.append('label = "%s";' % self.name)
        for cl in self.classes:
            dot.extend(cl.todotlist())
        for fn in self.funcalls:
            if len(fn.calls) == 0:
                dot.append('%s;' % fn.name)
            for fc in fn.calls:
        ...
        for imp in self.imports:
            dot.append(imp.todot(subgraph=True))
        dot.append('}')
        return '\n'.join(dot) 
