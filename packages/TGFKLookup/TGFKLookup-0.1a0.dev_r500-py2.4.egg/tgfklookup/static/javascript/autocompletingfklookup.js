...
function(field_id, options) {
	...
	this.searchController = options.search_controller;
    ...
    // don't try to use .observe
    // won't be able to prevent some browsers from submitting the form
...
AutoCompletingFKLookupManager.prototype.keyPress = function(event) {
    ...
    window.status = !(window.event && window.event.keyCode == 13);    
    ...
    switch(key) {
        case Event.KEY_RETURN:
            if (this.lastResults && this.lastResults[this.selectedResultRow]) {
                ...
                this.idField.value = result[this.options.id_result_attr];
                this.textField.value = result[this.options.text_result_attr];
                this.lastSelectedID = this.idField.value;
                this.lastSelectedText = this.textField.value;
                this.clearResults();
            }
            break;
...
function(request, focus, value) {
    ...
    if (focus == this.idField) {
        // If it's an ID written in the id field, we only set the text 
        // if the first id in the result and the written one are equal.
        var result = this.lastResults[0];
        if (result[this.options.id_result_attr] == value) {
            this.idField.value = result[this.options.id_result_attr];
            this.textField.value = result[this.options.text_result_attr];
            this.lastSelectedID = this.idField.value;
            this.lastSelectedText = this.textField.value;
