...
from plone.app.contentrules.actions.copy import ICopyAction
...
class ILinguaMoveAction(IMoveAction):
...
class LinguaMoveAction(SimpleItem):
    ...
    """
    implements(ILinguaMoveAction, IMoveAction, IRuleElementData)
...
class ILinguaCopyAction(ICopyAction):
    """ Interface for the configurable aspects of the linguatarget action.
    """
...
class LinguaCopyAction(SimpleItem):
    """ Note that we must mix in SimpleItem to keep Zope 2 security happy.
    """
    implements(ILinguaCopyAction, ICopyAction, IRuleElementData)
...
class LinguaMoveActionExecutor(MoveActionExecutor):
    ...
    adapts(Interface, IMoveAction, IObjectEvent)
    ...
    def __call__(self):
        if not self.element.move_to_translated_parent:
            return super(MoveActionExecutor, self)()
...
class LinguaCopyActionExecutor(CopyActionExecutor):
    ...
    def __call__(self):
        if not self.element.copy_to_translated_parent:
            return super(CopyActionExecutor, self)()
