...
class Subtitle(dict):
  def __init__(self, srtFile=None, srt=None, subFile=None, sub=None, fps=None):
    if srt:
      self.loadSrt(srt)
    elif srtFile:
      self.loadSrtFile(srtFile)
    elif sub and fps:
      self.loadSub(sub, fps)
    elif subFile and fps:
      self.loadSubFile(subFile, fps)
  ...
  def loadSubFile(self, fname, fps):
    data = self.loadFile(fname)
    self.loadSub(data, fps)

  def loadSub(self, sub, fps):
    sub = re.sub(r'\r\n|\r|\n', '\n', sub)
    next = 0
    def frame2time(frame):
      return ms2time(int((float(frame)/float(fps))*1000))
    for subtitle in re.compile('\{(\d*?)\}\{(\d*?)\}(.*)').findall(sub):
      self["%s" % next] = {
        'in':frame2time(subtitle[0]),
        'out':frame2time(subtitle[1]),
        'text': subtitle[2]
      }
      next +=1
