...
FILTERS= {
    "python":   ".vim .diff .patch .py .txt",
    "plone":    ".vim .diff .patch .py .pt .txt .zcml .xml .cpt .cpy .vpy .metadata .dtml",
    "c":        ".vim .diff .patch .c .cpp .h .m .dbg .mk .in",
    "objc":     ".vim .diff .patch .c .cpp .h .m .dbg .mk .in .nib .applescript .plist .strings",
    "none":     "",
}
...
class DirEntry(object):
    ...
    def write( self, f, level ):
        ...
        print >>f, "%s%s=\"%s\" CD=. %s filter=\"%s\" {" % (
                self.indent(level),
                name, root,
                "in.vim" in self.files and 'in="in.vim"' or "",
                self.getFilter(), )
