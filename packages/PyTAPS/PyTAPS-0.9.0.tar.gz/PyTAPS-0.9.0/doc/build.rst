...
`/usr/local/iMesh/lib`.  This option may also be specified in the `setup.cfg`
file:
...
A similar process is used for importing the `iGeom-Defs.inc` file, using the
option ``--igeom-dir=PATH``.
...
(``--enable-shared``) or with position-independent code (``--with-pic``).
