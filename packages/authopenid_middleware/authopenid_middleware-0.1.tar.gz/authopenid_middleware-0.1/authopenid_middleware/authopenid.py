...
class AuthOpenIdHandler(object):
    ...
    def __init__(self, app, app_conf, authopenid_conf):
        ...
        self.rememberme = authopenid_conf.get('rememberme', True)
        ...
        self.cookie_expires = authopenid_conf.get('cookieexpires', 3600.0)
    ...
    def _get_store(self, app_conf):
        ...
        if 'database' in app_conf:
            ...
            url = make_url(app_conf['database'])
            p = {}
            if url.username:
                p['user'] = url.username
            if url.password:
            ...
            if url.drivername == 'mysql':
                ...
                conn = sqlite.connect(url.database, **p)
