...
# We search only on types interesting for the Digesto
#friendly_types = ploneUtils.getUserFriendlyTypes()
friendly_types = ['Area', 'Attachment', 'Normativa']
...
if not results:
    ...
    write('''<li class="LSRow">''')
    write( '<a href="normativa_search" style="font-weight:normal">%s</a>' % ts.translate(label_advanced_search))
    write('''</li>''')

    if len(results)>limit:
        # add a more... row
        write('''<li class="LSRow">''')
        write( '<a href="%s" style="font-weight:normal">%s</a>' % ('normativa_search?SearchableText=' + searchterms, ts.translate(label_show_all)))
        write('''</li>''')
    write('''</ul>''')
    write('''</div>''')
    write('''</fieldset>''')
