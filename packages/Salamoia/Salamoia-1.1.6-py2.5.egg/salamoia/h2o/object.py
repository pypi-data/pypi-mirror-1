...
from salamoia.h2o.acl       import AllowACE, ReadAction, WriteAction, AdminAction
...
class BasicObject(object):
        ...
        if name == 'id':
            if value:
                if hasattr(self, 'setIdHook'):
                    self.setIdHook()
                if hasattr(self, 'acl'):
                    self.acl.insert(0, AllowACE(self.id, [ReadAction(), WriteAction()]))
    ...
    def inspectableDict(self):
        ...
        res = {}
        for i in [a for a in self.attributeNames() if not a in specials]:            
            res[i] = self._attributes[i].transportRead()
        return res
