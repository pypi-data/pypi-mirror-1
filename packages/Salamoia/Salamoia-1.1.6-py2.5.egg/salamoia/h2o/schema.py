...
from salamoia.h2o.acl           import AllowACE, DenyACE, Action, ReadAction, WriteAction, AdminAction

from protocols import (protocolForURI, advise, declareImplementation, declareAdapter,
                       Interface, Attribute, declareMultiAdapter)

from salamoia.h2o.generics   import Any, Each, EachAttr
...
class AttributeDescription(Element):
    ...
    def init(self):
        ...
        self.hooks = instancesOf(AttributeHookDescription, self.children)
...
class ObjectClass(Element):
    attributes = ['name', 'optional', 'attributes']
...
class ObjectClasses(Element):
    ...
    def init(self):
        ...
        # python-ldap doesn't tolerate unicode
        self.objectClasses = [str(x.name) for x in self.children if not x.optional]
        optionalClasses = [x for x in self.children if x.optional]
        self.optionalObjectClasses = [str(x.name) for x in optionalClasses]

        self.attributeClassMap = {}
        for i in optionalClasses:
            for a in i.attributes.split():
                self.attributeClassMap[str(a)] = str(i.name)
        
        if len(self.objectClasses) == 0:
            self.objectClasses.append("top") # object class top is assumed
            self.objectClasses.append(str(self.structural))
...
class Ldap(Element):
    ...
    def init(self):
        ...
        # this is preferred than have the parent do a "findChild" after its super().init
        # because this way static tal expressions can refer to attributes in siblings
        self.parent.ldap = self
        
        self.objectClasses = instancesOf(ObjectClasses, self.children)
        if self.objectClasses:
            self.objectClasses = self.objectClasses[0]
        else:
            raise "Required", "objectClasses"

        self.mappings = instancesOf(Mapping, self.children)
...
class AceDescription(Element):
    attributes = ['to', 'actions']

    def ace(self):        
        return self.aceClass(self.to, map(Action, self.actions))
        
class AllowAceDescription(AceDescription):
    aceClass = AllowACE        

class DenyAceDescription(AceDescription):
    aceClass = DenyACE

class AccessDescription(Element):
    childClasses = {'allow': AllowAceDescription, 'deny': DenyAceDescription}

    @lazy
    def acl(self):
        return Each(instancesOf(AceDescription, self.children)).ace()
...
class Definition(Element):
    ...
    def init(self):
        ...
        self.attributes = instancesOf(AttributeDescription, self.children)
        self.hooks = instancesOf(EntityHookDescription, self.children)

        self.access = self.findChild(AccessDescription)
...
class SchemaDescription(Description):
    ...
    def registerDefinition(self, definition):
        if definition.name in EachAttr(self.registeredDefinitions).name:
            Ione.warning("double registration of definition", definition.name)
        assert not definition.name in EachAttr(self.registeredDefinitions).name
