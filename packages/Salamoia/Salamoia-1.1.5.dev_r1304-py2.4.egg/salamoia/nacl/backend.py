...
from salamoia.h2o.exception  import FetchError, WritePermissionDeniedException
...
class BackendService(Service):
    ...
    def store(self, object, mode="auto"):
        ...
        if not object.checkAcl(self.authTokens.byName(self._currentUser).id, WriteAction()):
            Ione.error("ACCESS FAILED (%s -> %s)", self.authTokens.byName(self._currentUser).id, 
                       object.id, object.acl)
            raise WritePermissionDeniedException
