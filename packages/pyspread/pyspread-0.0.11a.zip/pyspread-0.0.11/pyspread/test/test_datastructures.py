...
class TestPyspreadGrid(object):
    ...
    def test_init(self):
        ...
        assert hasattr (self.grid, 'name')
        assert hasattr (self.grid, 'sgrid')
        assert hasattr (self.grid, 'fgrid')
    ...
    def test_getitem(self):
        ...
        assert self.grid[:, :, :].shape == self.grid.fgrid.shape
        
        empty_grid = _datastructures.PyspreadGrid(dimensions=(0, 0, 0))
        assert empty_grid[:, :, :].shape == (0, 0, 0)
        ...
        for i in v.getints(gmpy.mpz(-2**99), gmpy.mpz(2**99), 10):
            ...
            filled_grid.eval_function_grid()
            for j in xrange(gridsize):
    ...
    def test_len(self):
        ...
        assert len(self.grid) == len(self.grid.sgrid) == len(self.grid.fgrid)
    ...
    def test_remove(self):
        ...
        for currdim in xrange(len(dim)):
            ...
            for rem, norem in zip(cells_2b_removed, cells_2b_leftalone):
                ...
                #print dim, rem, norem
                assert self.grid.sgrid.shape == self.grid.fgrid.shape
    ...
    def test_nodegraph_append(self):
        """Tests creation of cycle detection graph"""
        self.grid[0, 1, 0] = '5'
        self.grid[1, 1, 0] = 'S[:10, 1, 0]'
        self.grid.eval_function_grid()
        assert self.grid[1, 1, 0][0] == 5
        assert self.grid[1, 1, 0][1] == 'Recursion detected!'
    ...
    def test_findnextmatch(self):
        ...
        self.grid[:100, 0, 0] = map(str, xrange(100))
        self.grid.eval_function_grid()
    ...
    def test_get_function_cell_indices(self):
        ...
        self.grid[:10:2, 1, 0] = map(str, xrange(5))
        self.grid.eval_function_grid()
        indices = self.grid.get_function_cell_indices()
    ...
    def test_eval_function_grid(self):
        """Tests grid evaluation to pass w/o errors"""
        
        for teststring in v.getstrings(number=1000, maxlength=1000):
            self.grid[0, 0, 0] = teststring
            self.grid.eval_function_grid()
...
class TestUnRedo(object):
    ...
    def test_undo(self):
        ...
        assert "MARK" not in self.unredo.redolist
