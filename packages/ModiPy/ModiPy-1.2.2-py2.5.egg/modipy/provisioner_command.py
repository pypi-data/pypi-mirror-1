#
# $Id: provisioner_command.py 99 2009-08-25 22:15:25Z daedalus $
#
#
...
##        self.cmd.append('%s' % str(device))
    ...
    def run_commands(self, ignored, device, expectset, namespace):
        ...
        for (expr, cmdstring, timeout) in expectset.commands:
            ...
            try:
                # This only runs a single command at a time,
                # which is a combination of the 'connect' command
                # (usually an ssh or sudo or something similar)
                # followed by the command we want to run remotely
