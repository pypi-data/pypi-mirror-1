#!/usr/bin/python
# A twisted protocol class that provides an interface to the
# NetApp ZAPI protocol
#
...
class ZAPIResult:
    ...
    def __repr__(self):
        return '<%s: %s, %s>' % (self.__class__, self.status, etree.tostring(self.tree) )
...
"""
    ...
    def zapi_request(self, device, command, username='root', password='netapp1', scheme='https', realm='Administrator', timeout=30):
        ...
        # FIXME: Use unique request_ids
        zapi_key = (device, command)
