...
class ConfigLoader:
    ...
    def load_config(self, options, devices=[]):
        ...
        # skip changes marked to be skipped on commandline
        ...
        # If only-changes were marked, mark all other changes as skipped
        for change in self.changes.values():
    ...
    def set_prereqs(self, ignored):
        ...
        # FIXME: This does nothing. Remove it.
    ...
    def add_provisioner(self, ignored, node):
        ...
        if self.options.sessionlog is not None:
            ...
            # always create a session log, named after the xml config file
            filename = "%s.log" % os.path.splitext(self.options.configfile)[0]
            sessionlog = open(filename, "w")
            #sessionlog = None
        
        try:
            provisioner = klass(prov_name,
                                authoritarian=self.options.authoritarian,
        ...
        # Make sure all the required nodes exist
    ...
    def create_change_object(self, node):
        ...
        # Check to see if the change name already exists:
    ...
    def get_available_changes(self):
        ...
        # Am I in backout mode?
        if self.options.backout:
            log.info("In backout mode. Doing changes in reverse.")
            # Find all changes that don't have children,
            # or that have children that have already been run
            for change in self.pending_changes:
                ...
                # check all my children have backed out ok
                all_children = True
                for child_change in change.children:
                    if child_change.state() not in ['backout_ok',
                                                    'skipped',
                                                    'success',
                                                    ]:
                        all_children = False
            ...
            # FIXME: This looks too complicated. Can it can be simplified?
            for change in self.pending_changes:
...
class ChangeController:
    ...
    def print_stats(self, ignored):
        ...
        # We return a failure count that is the sum of
        # all the changes that failed, but the ones
        # that didn't run because they couldn't be.
        # This ensure we have a useful exit code.
        return (success_count, failure_count + pending_count)
