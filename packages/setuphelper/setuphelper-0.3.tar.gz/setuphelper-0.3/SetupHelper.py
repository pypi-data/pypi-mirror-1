...
if sys.version < '2.5':
    ...
    class DistributionMetadata(_DistributionMetadata):
        ...
        if sys.version < '2.2.3':
            _METHOD_BASENAMES = _METHOD_BASENAMES + (
        ...
        def __init__ (self):
            ...
            # Hack to catch setuptools' different keyword
        ...
        def get_requires(self):
            # Includes setuptools hack
            return self.install_requires or self.requires or []
...
def distutils_default():
    ...
    class RequiresHelper(object):
        ...
        def get_latest_local_version(self, req, clean=False):
            result = None
        ...
        def process_req(self, req, latest_local, link_data=None, clean=False):
            if (link_data is not None) and (not clean):
                # Find latest online version and url by looking in link_data
                latest = self.get_latest_version(link_data, req)
            else:
                latest = None
            ...
            if os.path.isdir("deps"):
                ...
                if not clean:
                    ...
                    unpack_dir = [dname for dname in os.listdir(".") if os.path.isdir(dname)
                        and dname.startswith(req)][0]
        ...
        def process_reqs(self):
            ...
            if pypi_data is not None:
                ...
                for req in self._requires:
                    ...
                    latest_local = self.get_latest_local_version(req, clean)
                    
                    if (link_data is not None) or (latest_local is not None):
                        print "Processing requirement %s..." % req
                        self.process_req(req, latest_local, link_data, clean)
                        continue
...
class SetupHelper(object):
    ...
    def __init__(self, vars, **kwargs):
        ...
        internal_vars.extend(['setup_main', 'SetupHelper',
            '__setup_func__', '__setup_args__', '__post_install__'])
        ...
        for varname, value in vars.iteritems():
            ...
            if varname not in ('__builtins__', '__doc__', '__file__', '__name__', '__package__'):
        ...
        if self.__requires_class__ is not None:
            if hasattr(self, '__install_requires__'):
                ...
                # We need to change the variable name (we can't use __requires__ in the
                # actual setup script because that breaks setuptools)
