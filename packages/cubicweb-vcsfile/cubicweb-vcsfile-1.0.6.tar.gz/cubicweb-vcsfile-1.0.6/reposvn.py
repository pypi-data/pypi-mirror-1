...
class SVNRepository(repo.VCSRepository):
    ...
    def import_content(self, repoentity):
        ...
        execute = repoentity.req.execute
        while rev < actualrev:
            base_root = fs.revision_root(fsrep, rev)
            rev += 1
            if execute('Any X WHERE X from_repository R, R eid %(repo)s, X revision %(rev)s',
            ...
            self.imported_revision = rev # for hooks
