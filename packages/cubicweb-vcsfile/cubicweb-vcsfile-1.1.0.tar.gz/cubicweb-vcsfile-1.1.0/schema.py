...
class Revision(EntityType):
    ...
    ancestor_revision = SubjectRelation('Revision')
...
class VersionContent(DeletedVersionContent):
    ...
    # not required to avoid yams validation error since it's not actually stored
    # in the database
    data = Bytes(fulltextindexed=True, description=_("file's data"))
