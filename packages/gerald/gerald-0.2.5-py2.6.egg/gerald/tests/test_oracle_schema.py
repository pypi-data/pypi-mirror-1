...
class TestSchemaNew:
    ...
    def test_empty(self):
        ...
        self.empty_schema = OracleSchema('empty test')
...
class TestViewNew:
    ...
    def test_empty(self):
        ...
        self.empty_view = View('test_view')
...
class TestTableComments(TestTableParent):
    ...
    def test_get_table_comment(self):
        "Does the comment we have just added get included in the round trip DDL?"
        table = Table(self.table_name, self.cursor)
        ddl = table.get_ddl()
    ...
    def test_get_long_column_comment(self):
        "Does the comment we have just added get included in the round trip DDL?"
        table = Table(self.table_name, self.cursor)
        ddl = table.get_ddl()
        assert ddl.find(self.long_column_comment) != -1
    ...
    def test_get_short_column_comment(self):
        "Does the comment we have just added get included in the round trip DDL?"
        table = Table(self.table_name, self.cursor)
        ddl = table.get_ddl()
        assert ddl.find(self.short_column_comment) != -1
