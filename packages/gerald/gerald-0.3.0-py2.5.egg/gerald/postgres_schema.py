...
class Table(schema.Table, PostgresCalcPrecisionMixin):
    ...
    def _get_table(self, cursor):
        ...
        for row in cursor.fetchall():
            ...
            cursor.execute(cons_details_stmt, {'table':table, 'constraint_name':constraint['name']})
            cons_details = cursor.fetchall()
            if constraint['type'] in ('Primary', 'Foreign'):
                constraint['columns'] = [x[0] for x in cons_details]
                if constraint['type'] == 'Foreign':
                    ...
                    constraint['refcolumns'] = [x[4] for x in cons_details]
    ...
    def get_named_constraint_ddl(self, constraint_name):
        ...
        if constraint_name in self.constraints:
            ...
            if cons_type == 'Check':
                ddl_strings.append(' CHECK ( %s )' % cons_details['columns'])
