...
class BitsyAuthInnerWare(object):
    ...
    def join_form(self):
        ...
        if CAPTCHA:
            ...
            post_html = StringIO()
            print >> post_html, markup.p("Please type the word below so I know you're not a computer:<br/> %s" % markup.i("(refresh the page if its unreadable)"))
            print >> post_html, skimpyAPI.Pre(word).data()
            print >> post_html, markup.p(markup.input(None, **dict(name='captcha', type='text')))
            post_html = post_html.getvalue()

        form = Form(action=self.urls['join'], submit='Join', post_html=post_html)
    ...
    def join_post(self):
        ...
        if CAPTCHA:
            ...
            if not word:
                errors['CAPTCHA'] = 'Please type the funky looking word'
            if word != self.request.POST.get('captcha'):
