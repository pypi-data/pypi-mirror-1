...
from rum.wsgiutil import RumResponse, RumRequest,\
                         ...
                         status_map, HTTPException, HTTPRedirection
...
class Controller(object):
    ...
    @handle_exception.when((HTTPRedirection,))
    def _copy_cookies_from_response(self, e, routes):
        """
        ...
        for k,v in self.response.headerlist:
            if k.lower() == 'set-cookie':
                inst.headerlist.append((k,v))
        raise typ, inst, tb


    @handle_exception.when((HTTPException,))
    def _let_thru(self, e, routes):
        raise self.exc_info[0], self.exc_info[1], self.exc_info[2]
...
class CRUDController(Controller):
    ...
    def index(self):
        ...
        if query:
            if not query.limit:
                #TODO: Make defaults configurable on a per-controller basis
                query = query.clone(
                    limit = self.app.config.get('default_page_size', 15)
                    )
