...
class HeartBeat(Base):
    ...
    #filename = '/etc/ha.d/ha.cf'
    conf = HeartBeatConf(filename)

    def copy(self):
        #cmd = 'sudo scp %s root@fslave:/etc/ha.d/haresources > /dev/null 2>&1 &' % (self.filename)
        cmd = 'sudo scp %(file)s root@fslave:%(file)s > /dev/null 2>&1 &' % {'file': self.filename}
        stat = os.system(cmd)
