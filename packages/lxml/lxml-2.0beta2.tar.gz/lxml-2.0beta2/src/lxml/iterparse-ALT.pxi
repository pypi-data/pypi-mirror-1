...
cdef class iterparse_ALT(_BaseParser):
...
def iterparse(source, events=None, tag=None, _BaseParser parser=None):
    """Incremental parser.  Parses XML into a tree and generates tuples
    (event, element) in a SAX-like fashion. ``event`` is any of 'start',
    'end', 'start-ns', 'end-ns'.
    ...
    if events is not None:
        for event in events:
            ...
            events = None
    if events is None:
        # special iterator for 'end' events only
        return _IterParseEndIterator(source, tag, parser)
    return _IterParseIterator(source, events, tag, parser)

cdef class _IterParseEndIterator(TreeBuilder):
    cdef readonly object root
    cdef object _source
    cdef object _tag
    ...
    def __init__(self, source, tag, _BaseParser parser):
        if parser is None:
            parser = get_default_parser()
        parser = parser._copy()
        TreeBuilder.__init__(self, parser=parser)
        parser._target = self
        self._sax_event_propagate |= SAX_EVENT_DOCTYPE
    ...
    cdef _handleSaxEnd(self, tag):
        ...
        element = TreeBuilder._handleSaxEnd(self, tag)
        if self._tag is None or self._tag == tag:
            python.PyList_Append(self._event_queue, ("end", element))
        return element
    ...
    def __next__(self):
        if python.PyList_GET_SIZE(self._event_queue) > self._event_index:
            item = python.PyList_GET_ITEM(self._event_queue, self._event_index)
            python.Py_INCREF(item) # 'borrowed reference' from PyList_GET_ITEM
        ...
        while python.PyList_GET_SIZE(self._event_queue) == 0:
            data = self._source.read(__ITERPARSE_CHUNK_SIZE)
            if data:
                ...
                if python.PyList_GET_SIZE(self._event_queue) == 0:
                    raise StopIteration

        self._event_index = 1
        event = python.PyList_GET_ITEM(self._event_queue, 0)
        python.Py_INCREF(event) # 'borrowed reference' from PyList_GET_ITEM
        return event

cdef class _IterParseIterator(_IterParseEndIterator):
    cdef int _event_filter
    cdef object _nsevent_stack
    def __init__(self, source, events, tag, _BaseParser parser):
        _IterParseEndIterator.__init__(self, source, tag, parser)
        self._nsevent_stack = []
        if events is None:
            self._event_filter = ITERPARSE_FILTER_END
        else:
            ...
            for event in events:
                if event == "start":
                    ...
                    self._event_filter |= ITERPARSE_FILTER_START_NS
                elif event == "end":
                    self._event_filter |= ITERPARSE_FILTER_END
                elif event == "end-ns":
                    self._event_filter |= ITERPARSE_FILTER_END_NS

    cdef _handleSaxStart(self, tag, attrib, nsmap):
        cdef _Element element
        element = _IterParseEndIterator._handleSaxStart(
            self, tag, attrib, nsmap)
        if self._event_filter & ITERPARSE_FILTER_START_NS:
            if nsmap is not None and len(nsmap) > 0:
                ns_events = []
                for namespace in nsmap.iteritems():
                    if namespace[0] is None:
                        namespace = ('', namespace[1])
                    python.PyList_Append(
                        self._event_queue, ("start-ns", namespace))
                    python.PyList_Append(
                        ns_events, ("end-ns", None))
            else:
                ns_events = ()
            python.PyList_Append(self._nsevent_stack, ns_events)
        if self._event_filter & ITERPARSE_FILTER_START:
            if self._tag is None or self._tag == tag:
                python.PyList_Append(self._event_queue, ("start", element))
        return element

    cdef _handleSaxEnd(self, tag):
        cdef _Element element
        if self._event_filter & ITERPARSE_FILTER_END:
            element = _IterParseEndIterator._handleSaxEnd(self, tag)
        else:
            element = TreeBuilder._handleSaxEnd(self, tag)
        if self._event_filter & ITERPARSE_FILTER_END_NS:
            for event in self._nsevent_stack.pop():
                python.PyList_Append(self._event_queue, event)
        return element
...
def iterwalk(element_or_tree, events=("end",), tag=None):
    """Returns a tree walker that generates events from an existing tree
    as if it was parsing XML data with ``iterparse()``.
    """
    return _IterWalkIterator(element_or_tree, events, tag)

cdef class _IterWalkIterator:
    ...
    def __init__(self, element_or_tree, events, tag):
        cdef _Element root
        cdef int ns_count
