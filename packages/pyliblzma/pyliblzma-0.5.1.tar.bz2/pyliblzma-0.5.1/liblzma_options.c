...
init_lzma_options(const char *funcName, PyObject *kwargs, lzma_filter *filters){
	const char *argtypes = "|iiiiiiOOOO:";
	PyObject *args = NULL, *myFormat = NULL, *levelString = NULL,
		 *extremeString = NULL, *myCheck = NULL, *mf_key = NULL,
		 *mode_key = NULL;
	...
	if(myFormat){
		if(PyOS_stricmp("alone", PyString_AsString(myFormat)) == 0)
			filters[0].id = LZMA_FILTER_LZMA1;
		else if(PyOS_stricmp("xz", PyString_AsString(myFormat)) == 0)
	...
	if(myCheck){
		if(PyOS_stricmp("crc32", PyString_AsString(myCheck)) == 0)
			filters[LZMA_FILTERS_MAX + 1].id = LZMA_CHECK_CRC32;
		else if(PyOS_stricmp("crc64", PyString_AsString(myCheck)) == 0)
			filters[LZMA_FILTERS_MAX + 1].id = LZMA_CHECK_CRC64;
		else if(PyOS_stricmp("sha256", PyString_AsString(myCheck)) == 0)
