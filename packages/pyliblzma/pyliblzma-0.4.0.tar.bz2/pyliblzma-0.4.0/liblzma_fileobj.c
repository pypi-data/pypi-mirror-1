...
LZMAFile_read(LZMAFileObject *self, PyObject *args)
{
	...
	// FIXME: No idea how much sense there is to this, blindly following bz2.c..
	if(self->fp->encoding)
...
LZMAFile_seek(LZMAFileObject *self, PyObject *args)
{
	...
	if (offset >= self->pos) {
		...
		lzma_open_real(&lzuerror, NULL, PyFile_AsFile(self->file));
		if (lzuerror != LZMA_OK) {
			Util_CatchLZMAError(lzuerror, &self->fp->strm);
			goto cleanup;
		}
		...
		ret = NULL;
		self->pos = 0;
		self->fp = lzma_open_real(&lzuerror, NULL, PyFile_AsFile(self->file));
		if (lzuerror != LZMA_OK) {
			Util_CatchLZMAError(lzuerror, &self->fp->strm);
			goto cleanup;
		}
...
LZMAFile_close(LZMAFileObject *self)
{
	...
	if (lzuerror != LZMA_OK && lzuerror != LZMA_STREAM_END) {
		Util_CatchLZMAError(lzuerror, NULL);
		Py_XDECREF(ret);
		ret = NULL;
	}
...
static PyObject *
LZMAFile_get_newlines(LZMAFileObject *self, void *closure)
{
...
{
	return PyInt_FromLong(!self->open);
}

static PyObject *
LZMAFile_get_mode(LZMAFileObject *self, void *closure)
{
	return PyObject_GetAttrString(self->file, "mode");
}

static PyObject *
LZMAFile_get_name(LZMAFileObject *self, void *closure)
{
...
LZMAFile_init(LZMAFileObject *self, PyObject *args, PyObject *kwargs)
{
	...
	/* This is a bit cumbersome, but we don't use PyArg_ParseTupleAndKeywords
	 * since init_lzma_options() uses this later on.
	 */
	if (!PyArg_ParseTuple(args, "O|si:LZMAFile",
					&name, &mode, &buffering))
		return -1;
	if(kwargs != NULL){
		PyObject* modeString = PyString_FromString("mode");
		if(PyDict_Contains(kwargs, modeString)){
			mode = PyString_AsString(PyDict_GetItem(kwargs, modeString));
			PyDict_DelItem(kwargs, modeString);
		}
		PyObject* bufferingString = PyString_FromString("buffering");
		if(PyDict_Contains(kwargs, bufferingString)){
			buffering = PyInt_AsLong(PyDict_GetItem(kwargs, bufferingString));
			PyDict_DelItem(kwargs, bufferingString);
		}
	}
	...
	for (;;) {
		...
		switch (*mode) {
			case 'w':
				if(!options)
				{
					lzma_options_lzma _options = lzma_preset_lzma[LZMA_DEFAULT_COMPRESSION-1];
					if(!init_lzma_options("LZMAFile", kwargs, &_options))
						return -1;
					options = &_options;
				}
				break;
	...
	self->fp = lzma_open_real(&lzuerror, options, PyFile_AsFile(self->file));
...
PyDoc_STR(
"LZMAFile(name [, mode='r', buffering=0, format=alone, eopm=False, level=7,\n\
dictionary_size=23, literal_contextbits=3 literal_pos_bits=0, pos_bits=2,\n\
mode=2, fast_bytes=128, match_finder='bt4', match_finder_cycles=0])\n\
-> file object\n\
\n\
