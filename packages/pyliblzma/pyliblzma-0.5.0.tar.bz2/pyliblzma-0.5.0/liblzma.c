...
{
	PyObject *RetVal = NULL, *options_dict = NULL;
	uint8_t *input;
	...
	*lzus = tmp;
	bufsize = lzma_stream_buffer_bound(length);
	...
	if(filters[0].id == LZMA_FILTER_LZMA2)
	{
		...
		lzuerror = lzma_stream_buffer_encode(filters, filters[LZMA_FILTERS_MAX + 1].id,
				NULL, input, (size_t)length,
 				(uint8_t *)PyString_AS_STRING(RetVal), &loc, (size_t)bufsize);
		Py_END_ALLOW_THREADS
		_PyString_Resize(&RetVal, (Py_ssize_t)loc);
...
LZMA_decompress(__attribute__((unused)) PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s#|ll:decompress", kwlist,
			  &input, &length, &r_strlen, &memlimit))
		return NULL;
