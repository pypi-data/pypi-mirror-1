...
"decompress(data[, max_length=0, memlimit=-1]) -> string\n\
\n\
...
LZMADecomp_decompress(LZMADecompObject *self, PyObject *args, PyObject *kwargs)
{
	...
	INITCHECK
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "s#|ll:decompress", kwlist,
			  &input, &inplen, &self->max_length, &self->memlimit))
		return NULL;
...
"flush( [flushmode=LZMA_FINISH, length] ) -> string\n\
\n\
Return a string containing any remaining decompressed data.\n\
'length', if given, is the initial size of the output buffer.\n\
\n\
...
static PyObject * LZMADecomp_flush(LZMADecompObject *self, PyObject *args, PyObject *kwargs)
{
	...
	static char *kwlist[] = {"flushmode", "length", NULL};
...
LZMADecomp_reset(LZMADecompObject *self, PyObject *args, PyObject *kwargs)
{
	...
	INITCHECK
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "|ll:reset", kwlist,
...
LZMADecomp_init(LZMADecompObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "|ll:LZMADecompressor", kwlist,
				&self->max_length, &self->memlimit))
		return -1;
