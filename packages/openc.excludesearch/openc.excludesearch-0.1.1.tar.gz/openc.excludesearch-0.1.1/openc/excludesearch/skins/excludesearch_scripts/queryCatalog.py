...
if show_query:
    try:
        ...
        truequ = catalog.makeAdvancedQuery(query)   # Convert the existing query to an Advanced Query object
        tq = truequ & ~Eq("object_provides","openc.excludesearch.interfaces.IExcludeFromSearch") # Add a requirement not to implement IExcludeFromSearch
        results = catalog.evalAdvancedQuery(tq)     # Save the AdvancedQuery results where the normal results should be
    except ParseError:
        pass
