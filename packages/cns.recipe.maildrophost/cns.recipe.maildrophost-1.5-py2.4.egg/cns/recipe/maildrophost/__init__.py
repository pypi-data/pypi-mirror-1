...
class Recipe:
    ...
    def install(self):
        ...
        # 'config' is used in MDH 1.22+
        # 'config.py' in MDH up to 1.21
        config_name = os.path.join(result, 'config.py')
        if not os.path.isfile(config_name):
            config_name = os.path.join(result, 'config')
        config = open(config_name, 'rU')
