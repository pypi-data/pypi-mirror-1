...
# $Id: fileinput.py 13 2010-01-20 17:21:59Z ggenellina $
...
class MultiFileIter(_multifile.MultiFileIter):
    ...
    def _prepare_next_file(self, file_or_name, mode):
        ...
        if file_or_name == '-':
            return file_or_name
        if isinstance(file_or_name, string_types):
...
class _delegate_output:
    ...
    def write(self, obj):
        return self._fileinput.output.write(obj)
    def flush(self):
        self._fileinput.output.flush()
    ...
    def _open_file(self, filename, mode):
        ...
        if self.inplace:
            ...
            try:
                ...
                if sys.version_info >= (3,) and hasattr(os, 'O_BINARY'):
                    mode |= os.O_BINARY
                fd = os.open(filename, mode, perm)
