...
An iterator over all lines of the given files. Like module :mod:`fileinput` in 
the standard library, but faster, and written in C.

Unlike :mod:`fileinput`, input files are *not* completely read into memory;
...
This package implements a basic multiple file iterator written in C, a thin 
Python wrapper on top of it, and a replacement for the standard 
