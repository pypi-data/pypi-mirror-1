...
from webdav.NameCheck import validateResourceName
...
class ResourceStorer(object):
    ...
    __slots__ = ('path', 'connection', 'versionHandler', 'defaultNamespace',
                 'aclSupportAvailable', 'daslBasicsearchSupportAvailable', 'name')
    ...
    def __init__(self, url, connection=None):
        ...
        @raise WrongNameError: L{datafinder.common.NameCheck.WrongNameError} if 
            validation of URL fails.
        """
    ...
    def getAcl(self):
        ...
        @rtype: L{ACL<datafinder.webdav.acp.Acl.ACL>}
        """
    ...
    def getCurrentUserPrivileges(self):
        ...
        @rtype: list of L{Privilege<datafinder.webdav.acp.Privilege.Privilege>}
        """
...
class CollectionStorer(ResourceStorer):
    ...
    def __init__(self, url, connection=None):
        ...
        @see: L{datafinder.webdav.WebdavClient.ResourceStorer.__init__}
        @param url: unique resource location for this storer
        ...
        if  url[-1] != '/':     # Collection URL must end with slash
            url += '/'
        super(CollectionStorer, self).__init__(url, connection)
    ...
    def getResourceStorer(self, name):
        ...
        return ResourceStorer(self.url + name, self.connection)
    ...
    def addCollection(self, name, lockToken=None):
        ...
        return CollectionStorer(self.url + name, self.connection) 
    ...
    def addResource(self, name, content=None, properties=None, lockToken=None):
        ...
        resource = ResourceStorer(self.url + name, self.connection)
        resource.uploadContent(content, lockToken)
        if properties:
            resource.writeProperties(properties, lockToken)
        return resource
    ...
    def getCollectionContents(self):
        ...
        for url, properties in result.items():
            if not self.path == url:
                ...
                collectionContents.append((myWebDavStorer, properties))
...
if __name__ == "__main__":
    ...
    webdavConnection = CollectionStorer(webdavUrl)
