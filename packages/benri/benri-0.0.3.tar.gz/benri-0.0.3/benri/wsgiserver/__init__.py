    ...
    def communicate(self):
        ...
        try:
            while True:
                ...
                # the SSL handshake is performed transparently on the
                # first read on the socket. When SSL verification is
                # used, this means that the peer certificate is not
                # available until after the first read, that is when
                # the request has been parsed.
                if SSL and isinstance(self.socket, SSL.ConnectionType):
                    # fill the environment with the DN subject string
                    peer_cert = self.socket.get_peer_certificate()
                    if peer_cert:
                        req.environ["SSL_CLIENT_S_DN"] = str(peer_cert.get_subject())[18:-2]
...
class CherryPyWSGIServer(object):
    ...
    def bind(self, family, type, proto=0):
        ...
        if self.ssl_certificate and self.ssl_private_key:
            ...
            def verify_cb(conn, x509, errnum, errdepth, preverify):
                return preverify

            # enable client verification
            if self.ssl_cacert_path:
                ctx.load_verify_locations(self.ssl_cacert_path)
                ctx.set_verify(SSL.VERIFY_PEER | SSL.VERIFY_FAIL_IF_NO_PEER_CERT, verify_cb)
                ctx.set_verify_depth(self.ssl_verify_depth)
