...
class postfeedsView(BrowserView):
    ...
    def lookfornew(self):
        ...
        ####
        ## apply filters to the post list
        if self.context.enable_filters:
            for post in toPost: 
                
                search_text = post['summary'] + ' ' + post['title']

                try:
                    search_text = unicodedata.normalize('NFKD', search_text).encode('ascii','ignore')
                except:
                    pass
                ...
                # AND include filter
                remove = False
                for inc in self.context.filter_inc:
                    if search_text.find(inc) == -1:
                        remove = True
                        print 'or removal - ', inc

                if remove:
                    #print '*******(AND )******', search_text[:50]
                    remove_post = True
                
                
                # OR include filter
                if len(self.context.filter_inc_or) != 0:
                    remove = True
                    for inc_or in self.context.filter_inc_or:
                        if search_text.find(inc_or) != -1:
                            remove = False
                            print 'yes ', inc_or
                        else:
                            print 'no ', inc_or
                    if remove:
                        #print '*******(OR )******', search_text[:50]
                        remove_post = True
                            
                #exclude filter
                remove = False
                for excl in self.context.filter_excl:
                    #print '---excl: ',excl
                    if search_text.find(excl) != -1:
                        remove = True
                        print 'or removal - ', excl
                if remove:
                    remove_post = True

                #now perform the actual removal
                timenow = str(time.strftime('%d/%m/%y %H:%M',datetime.datetime.utcnow().timetuple()))
                if remove_post:                                        
                    print timenow, '-post removed-', search_text[:100]
                    toPost.remove(post)
                else:
                    print timenow, '-posting-', search_text[:100]
        ...
        for post in toPost:
            ...
            if self.context.news_onoff:
                ...
                create_body +='<p><a href="%s" target="_blank" title="... read the original here ...">%s</a></p>' % (str(post['link']),str(post['title']))
