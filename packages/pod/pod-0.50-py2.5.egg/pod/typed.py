...
class Dynamic(DynamicCondition):
    ...
    def dump(self, value):
        return self.cls_pod.pickle_dump(value = value)
...
class Object(Typed):
    ...
    def dump(self, value, inst = None):
        return self.cls_pod.pickle_dump(value = value, inst = inst, attr = self.name, col = self)
    
    def load(self, value, inst = None):
        return self.cls_pod.pickle_load(value = value, inst = inst, attr = self.name, col = self)
