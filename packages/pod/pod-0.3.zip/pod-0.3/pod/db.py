...
class Db(object):
    ...
    def __init__(self, file, remove = False, clear = False, connect = True, attach = None, chatty = False, very_chatty = False, use_full_table_name = True):
    ...
    def attach_to_object(self, obj):
        ...
        for cls in classes:
            if object.__getattribute__(type.__getattribute__(cls, 'pod'), 'db') is None:
    ...
    def commit(self, clear_cache = False, close = False):
        ...
        for inst,value in self.dirty.iteritems():
            object.__getattribute__(inst[0],'__setattr__')(inst[1], value)
...
class Cache(object):
    ...
    def get_inst(self, cls_id, inst_id, exists = None): 
        ...
        if cls_pod is None:
            ...
            return cls_pod.inst_get_inst_by_id(inst_id = inst_id, exists = exists)
