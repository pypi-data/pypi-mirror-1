    ...
    class Pod(object):
        ...
        def attach_to_db(self, db):
            ...
            if self.db is None:
                ...
                table_state = self.sql_table_state_get()
    
                if table_state is None:   
                    self.table_create()   # Note, this calls 'activate_on_set_id' at the right place . . . 
                else:
        ...
        def table_create(self):
            ...
            self.id = self.sql_create_class_tables()
            if self.index_kv:
                self.sql_index_kv_add()
            ...
            if len(self.typed_group) == 0:
                ...
                self.sql_table_state_set()

        def table_drop(self, child_classes = True):
            if self.id:
                self.sql_drop_class_tables()
            ...
            if copy_dropped_to_dict and len(list_drop_pod_types) > 0:
                ...
                for row in self.cursor.fetchall():
                    for i, type_name in enumerate(list_drop_pod_types):
                        ...
                        args.append((row[0], type_name,self.sql_pickle_dump(value = new_value),))  # Don't need to provide inst,attr for mutables check because this is a copy
        ...
        def type_create(self, name, pod_type, is_new):
            ...
            if is_new is False:
                ...
                self.cursor.executemany("UPDATE " + self.table + " SET " + name + "=? WHERE id=?", [(pod_type.dump(value = self.sql_pickle_load(row[1])), row[0]) for row in self.cursor.fetchall()])
            ...
            if kwargs is not None:
                ...
                for key, value in kwargs.iteritems():
                    if key in self.typed_group:
                        ...
                        dynamic_values[key] = self.sql_pickle_dump(value = value, inst = inst, attr = key)
    
            inst_id = self.sql_insert_object(values = typed_values)
            self.sql_add_many_kvargs(values = dynamic_values, inst_id = inst_id)
        ...
        def inst_update_dict(self, inst, kwargs):
            ...
            for key, value in kwargs.items():
                ...
                if key in self.typed_group:
                    ...
                    nom_values.append((id, key,self.sql_pickle_dump(value = value, inst = inst, attr = key),))
        ...
        def inst_load_attr_from_db(self, inst, attr):
            ...
            if attr in self.typed_group:
                ...
                if row:
                    self.zombies.discard(inst)
                    object.__setattr__(inst, attr, self.sql_pickle_load(value = row[0], inst = inst, attr = attr))
                else:
        ...
        def inst_save_attr_to_db(self, inst, attr, value):
            if attr in self.typed_group:
                ...
                self.cursor.execute('INSERT OR REPLACE INTO ' + self.table_dict + ' (fid,key,value) VALUES (?,?,?)', (inst.id, attr, self.sql_pickle_dump(value = value, inst = inst, attr = attr),))                    
                ...
                if row:
                    ...
                    for kv in rows:
                        if kv[0] not in inst_dict:
                            object.__setattr__(inst, kv[0], self.sql_pickle_load(value = kv[1], inst = inst, attr = kv[0]))               
                else:
                    raise PodObjectDeleted, "The object " + str(inst) + " with id " + inst.get_full_id() + " had been deleted . .  ."
        ...
        """ pickle commands """
        def sql_pickle_dump(self, value, inst = None, attr = None, col = None):
            if value is None:
                return value
        ...
        def sql_pickle_load(self, value, inst = None, attr = None, col = None):
        ...
        def sql_get_old_pod_types(self):
            return self.db.pickler.load(self.cursor.execute("SELECT pod_types FROM pod_classes WHERE id=(?)", (self.id,)).fetchone()[0])

        """ sql commands create/alter local tables """ 
        def sql_create_class_tables(self):
        ...
        def sql_table_state_set(self):
            self.cursor.execute("UPDATE pod_classes SET mtime=?,index_kv=?,pod_types=? WHERE id=?", (self.mtime, self.index_kv, db.Pickler.dump(self.typed_group), self.id,))      
            self.db.connection.commit()

        """ sql commands insert/update/delete object """
        def sql_insert_object(self, values):
            type_names = "("
            type_quest = "("
            type_values = []
        ...
        def sql_add_many_kvargs(self, values, inst_id):
            if len(values) > 0:
