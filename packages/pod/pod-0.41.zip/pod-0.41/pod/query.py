            ...
            if len(pod_types) > 0:
                ...
                for i,cls_pod in enumerate(query.class_pod_list):
                    ...
                    for j,col in enumerate(pod_types):
                        if col in cls_pod.type_group:
                            self.returned_pod_types[i].append((j+2,col, cls_pod.type_group[col]))   # Duck typing: both pod_types and pickler support 'load' method
                        else:
...
class RawQueryIterator(object):
    ...
    def __init__(self, raw_query):
        ...
        if isinstance(Row.cls, core.Meta):
            Row.pod_type_fns = Row.cls.pod.type_group
        else:
            Row.pod_type_fns = {}
