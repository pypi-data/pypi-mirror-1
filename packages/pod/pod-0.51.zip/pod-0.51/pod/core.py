...
class Meta(type):
    ...
    def execute(cls, query, args=()):
        return cls.pod.cursor.execute(query.replace("cls_table", cls.pod.table).replace("cls_table_kvdict", cls.pod.table_dict), args)
    ...
    class Pod(object):
        ...
        def activate_on_set_id(self):
            ...
            for pod_type in self.typed_group.itervalues():
                pod_type.on_db_activate(db=self.db)
        ...
        def get_mtime(self, source_file=None):
            try:
                ...
                if source_file[-13:] == 'collection.py':
                    return 0
                else:
        ...
        def table_create(self, inst=None, kwargs=None, create_inst=True):
            ...
            self.mtime = self.get_mtime(source_file=inspect.getsourcefile(self.cls))
            self.index_kv = type.__getattribute__(self.cls, '__dict__').get('POD_DYNAMIC_INDEX', self.db.index)

            # if row is None, means a new class and a new table needs to be made  
            self.cursor.execute("INSERT INTO pod_classes (cls_name, ctime, mtime,index_kv) VALUES (?,?,?,?)", (self.table, self.mtime, self.mtime,int(self.index_kv)))
        ...
        def table_index_kv_add(self):
            self.cursor.execute("CREATE INDEX IF NOT EXISTS " + self.table_dict + "_pin_value ON " + self.table_dict + " (value)")

        def table_index_kv_drop(self):
            self.cursor.execute("DROP INDEX IF EXISTS " + self.table_dict + "_pin_value")

        def table_check_for_changes(self, copy_dropped_to_dict=True):

            # First, check if you need to drop dynamic indicies . . . 
            index_kv_current = type.__getattribute__(self.cls, '__dict__').get('POD_DYNAMIC_INDEX', self.db.index)

            if self.index_kv is True and index_kv_current is False:
                self.table_index_kv_add()
                self.table_check_for_changes_msg(msg='Adding index for dynamic attributes . . . ')
            elif self.index_kv is False and index_kv_current is True:
                self.table_index_kv_drop()
                self.table_check_for_changes_msg(msg='Dropping index for dynamic attributes . . . ')
        ...
        def table_check_for_changes_msg(self, msg=None):
        ...
        def type_drop_old(self, old_pod_types, set_old_pod_types, set_drop_pod_types, copy_dropped_to_dict):
            ...
            self.table_check_for_changes_msg(msg="dropping pod_types " + ",".join(set_drop_pod_types) + "  . . . ")
                ...
                for row in self.cursor.fetchall():
                    for i, type_name in enumerate(list_drop_pod_types):
                        ...
                        if value is None:
                            ...
                            new_value = Undefined(id = value.split(":"), cursor = self.cursor)
                        else:
                            new_value = old_pod_type.load(value)                             
                        args.append((row[0], type_name,self.pickle_dump(value = new_value),))  # Don't need to provide inst,attr for mutables check because this is a copy
        ...
        def type_drop_and_delete_forever(self, pod_type):
            ...
            self.table_check_for_changes(copy_dropped_to_dict=False)
        ...
        def type_create(self, name, pod_type, is_new):
            ...
            self.table_check_for_changes_msg(msg="adding pod_type '" + name + "'  . . . ")
            ...
            if is_new is False:
                ...
                self.cursor.executemany("UPDATE " + self.table + " SET " + name + "=? WHERE id=?", [(pod_type.dump(value = self.pickle_load(row[1])), row[0]) for row in self.cursor.fetchall()])
        ...
        def type_add_index(self, name, unique=False):
            ...
            self.table_check_for_changes_msg(msg="adding " + unique.lower() + "index '" + name + "'  . . . ")
        ...
        def type_drop_index(self, name):        
            self.table_check_for_changes_msg(msg="dropping index '" + name + "'  . . . ")
        ...
        def type_commit_to_db(self, mtime=None):
            ...
            self.cursor.execute("UPDATE pod_classes SET mtime=?,index_kv=?,pod_types=? WHERE id=?", (mtime, self.index_kv,db.Pickler.dump(self.typed_group), self.id,))      
            ...
            if kwargs is not None:
                ...
                for key, value in kwargs.iteritems():
                    if key in self.typed_group:
                        type_names += key + ','
                        type_quest += '?,'
                        pod_type = self.typed_group[key]
                        type_values.append(pod_type.dump(value = value, inst = inst))
                        if pod_type.index == typed.unique:
                            insert_cmd = 'INSERT OR REPLACE'
                    else:
                        nom_values.append([None, key, self.pickle_dump(value = value, inst = inst, attr = key)])
        ...
        def inst_update_dict(self, inst, kwargs):
            ...
            for key, value in kwargs.items():
                ...
                if key in self.typed_group:
                    ...
                    nom_values.append((id, key,self.pickle_dump(value = value, inst = inst, attr = key),))
        ...
        def inst_load_attr_from_db(self, inst, attr):
            ...
            if attr in self.typed_group:
                ...
                if row:
                    self.zombies.discard(inst)
                    object.__setattr__(inst, attr, self.pickle_load(value = row[0], inst = inst, attr = attr))
                else:
        ...
        def inst_save_attr_to_db(self, inst, attr, value):
            if attr in self.typed_group:
                ...
                self.cursor.execute('INSERT OR REPLACE INTO ' + self.table_dict + ' (fid,key,value) VALUES (?,?,?)', (inst.id, attr, self.pickle_dump(value = value, inst = inst, attr = attr),))                    
                ...
                if row:
                    ...
                    for kv in rows:
                        if kv[0] not in inst_dict:
                            object.__setattr__(inst, kv[0], self.pickle_load(value = kv[1], inst = inst, attr = kv[0]))               
                else:
                    raise PodObjectDeleted, "The object " + str(inst) + " with id " + inst.get_full_id() + " had been deleted . .  ."
            ...
            if value is None or isinstance(value, (int, long, float)):
                ...
                if isinstance(value, Object):
                    ...
                    if vclass is str:
                        ...
                        return '0'
                    elif isinstance(value, NoSave):
                        return None
                    else:            
                        value = 'x' + db.Pickler.dump(value)            
                        if inst is not None:           
