...
class Meta(type):
    ...
    class Pod(object):
        ...
        def activate(self, row=None):
            ...
            if self.id is None:
                ...
                self.id = int(row[0])    
                index_kv_old = bool(row[2])
                
                self.index_kv = getattr(self.cls, 'POD_DYNAMIC_INDEX', self.db.index)                        
                self.mtime = self.get_mtime(source_file=inspect.getsourcefile(self.cls))
    
                self.activate_on_set_id()
                        
                if (int(row[1]) != self.mtime) or (self.index_kv != index_kv_old):
        ...
        def get_mtime(self, source_file=None):
            try:
                ...
                if source_file[-13:] == 'collection.py':  # pragma: no cover
                    return 0
        ...
        def table_check_for_changes(self, index_kv_old, copy_dropped_to_dict=True):
            ...
            if set(self.type_get_current_db_pod_types()) != set_old_pod_types:  # pragma: no cover
        ...
        def type_drop_and_delete_forever(self, pod_type):
            ...
            if self.db.chatty:  # pragma: no cover
    ...
    class PodZombie(object):
        ...
        def __getattr__(self, attr):
            raise PodMetaError, "You cannot get an attribute on this object . . . "
...
class Undefined(Object):
    ...
    def try_to_activate_class(self, key):
        id = object.__getattribute__(self, 'id')
        row = object.__getattribute__(self, 'cursor').execute("SELECT cls_name,mtime,index_kv FROM pod_classes WHERE id=(?)", (id[0],)).fetchone()                
        cls_name, mtime, index_kv = row[0], row[1], row[2]
        if cls_name in register.class_pods:
            cls_pod = register.class_pods[cls_name]
            cls_pod.activate(row=[id[0], mtime, index_kv])
            inst_id = id[1]
            ...
            raise PodObjectUndefined, "You tried to access attr '" + key + "' on a pod.Undefined of class type '" + cls_name + "' -- this means the class was not loaded at import time. . . "
