...
class Time(Int):
    ...
    def __init__(self, index = False, format_string = None, local_time = True, cls = None, db = None, name = None):
    ...
    def dump(self, value):
        if self.format_string:
            calendar.timegm(time.strptime(value, self.format_string))
        else:
            return Int.dump(self, value)
    
    def load(self, value):
        if self.format_string:
            return datetime.datetime.utcfromtimestamp(value).strftime(self.format_string)
        else:
            return Int.load(self, value)
    
    """ COMPARISON OPERATORS """
    def __eq__(self, value):
        return Int.__eq__(self, self.parse_date(value))
    def __ne__(self, value):
        return Int.__ne__(self, self.parse_date(value))
    def __gt__(self, value):
        return Int.__gt__(self, self.parse_date(value))
    def __ge__(self, value):
        return Int.__ge__(self, self.parse_date(value))
    def __lt__(self, value):
        return Int.__lt__(self, self.parse_date(value))
    def __le__(self, value):
        return Int.__le__(self, self.parse_date(value))
    ...
    def parse_date(self, value):
        if isinstance(value, str):
            ...
            if len(ymd) > 2:
                d = int(ymd[2])
            value = self.args_to_epoch(y, m, d, None,None,None,None,True)
        return value
    ...
    def older_than(self, years = 0, weeks = 0, days = 0, hours = 0, minutes = 0, seconds = 0):
        ...
        return self <= int(1000*(Time.utc_timestamp()-ctime))
    
    def younger_than(self, years = 0, weeks = 0, days = 0, hours = 0, minutes = 0, seconds = 0):
        ctime = years*365*24*3600 + weeks*7*24*3600 + days*24*3600 + hours*3600 + minutes*60 + seconds
        return self >= int(1000*(Time.utc_timestamp()-ctime))
    
    def pre_dates(self, year = None, month = None, day = None, hour = None, minute = None, second = None, time_stamp = None, local_time = True):
        return self <= int(1000*self.args_to_epoch(year,month,day,hour,minute,second,time_stamp,local_time))
        
    def post_dates(self, year = None, month = None, day = None, hour = None, minute = None, second = None, time_stamp = None, local_time = True):
        return self >= int(1000*self.args_to_epoch(year,month,day,hour,minute,second,time_stamp,local_time))
    
    def is_date(self, year = None, month = None, day = None, local_time = True):
        if year:
    ...
    def args_to_epoch(self,y,m,d,h,n,s,time_stamp,local_time):        
        if time_stamp:
            ctime = time.mktime(time.strptime(time_stamp, self.format_string))
        else:
        ...
        if local_time is True:
            ...
            return time.mktime(datetime.datetime.utcfromtimestamp(ctime).timetuple())
        else:
            return ctime
...
class TimeStamp(object):
    ...
    def set_time(self):
        self.time = int(1000*Time.utc_timestamp())
