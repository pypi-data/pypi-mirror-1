...
class Recipe(gocept.cmmi.Recipe):
    ...
    def __init__(self, buildout, name, options):
        ...
        egg = pkg_resources.load_entry_point(
            "zc.recipe.egg", "zc.buildout", "eggs")(buildout, name, options)
        ignored, self.working_set = list(egg.working_set(()))
        ...
        options["path-list"] = repr(
            [os.path.join(location, str(i))
             for i, ignored in enumerate(self.working_set)] +
    ...
    def install(self):
        ...
        for i, spec in enumerate(self.working_set):
