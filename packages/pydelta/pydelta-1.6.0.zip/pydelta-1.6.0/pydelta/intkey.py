...
It returns a pydelta.tDelta object, which can be used like a loaded DELTA file.
...
class IKCharDescr(pydelta.tCharDescr):
...
Otherwise, the filenames will be opened as regular files.
	"""
	# Set up structure.
	delta = pydelta.tDelta(charsfn, itemsfn, parse=False)
	...
	for i, rnCdata in enumerate(rnsCdata):
		...
		if delta.chars[i+1].type == pydelta.CT['TE']:
			...
			for j, value in enumerate(values):
				if value[1] and not value[0]:
					delta.items[j+1].attributes[i+1] = value[1]
					
		# Multistate data.
		elif (delta.chars[i+1].type == pydelta.CT['UM']) or \
				(delta.chars[i+1].type == pydelta.CT['OM']):
				
			NS = len(delta.chars[i+1].states)
			nbits = (NS + 1) * NI
		...
		# Real numeric data.
		elif delta.chars[i+1].type == pydelta.CT['RN']:
		...
		elif delta.chars[i+1].type == pydelta.CT['IN']:
			min = delta.chars[i+1].min
			NS = delta.chars[i+1].max - min
