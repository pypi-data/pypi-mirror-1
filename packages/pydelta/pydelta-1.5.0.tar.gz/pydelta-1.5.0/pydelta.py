...
#                                                                          #
#   REVISION HISTORY:                                                      #
#     Version 1.00, Aug 2008 - Initial implementation (Mauro Cavalcanti)   #
#     Version 1.50, Feb 2010 - Substantial reworking (Thomas Kluyver)      #
#==========================================================================#
"""
...
class tDelta(object):
	...
	def __init__(self, chars_fname, items_fname,\
			specs_fname = None, cnotes_fname=None):
		self.chars = tDeltaCharList(chars_fname, cnotes_fname)
		self.items = tDeltaItemList(items_fname)
		if specs_fname:
...
if __name__ == '__main__':
	...
	while not stop:
		...
		if opt == 1 : #----- Displays all characters features --------------------
			...
			for i, char in enumerate(Example.chars,start=1):
				print i, ": ", char.feature
			...
			if (opt < 1) or (opt > len(Example.chars)):
				...
				if Example.chars[opt].char_type in (CT['UM'], CT['OM']):
					for j,state in enumerate(Example.chars[opt].states,start=1):
						print "State",j,": ",state
				else:
