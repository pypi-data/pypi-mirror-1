...
if req.get('reply_come_from'):
    target = req.get('reply_come_from')
else:
    target = view

query = 'anc=%s' % anchor

if req.get('blogbatch.b_page'):
    query = '%s&blogbatch.b_page=%s' % (query, req.get('blogbatch.b_page'))

target = '%s?%s#%s' % (target, query, anchor)
