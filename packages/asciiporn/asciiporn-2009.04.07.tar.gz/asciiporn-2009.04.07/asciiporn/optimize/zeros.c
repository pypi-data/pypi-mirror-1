...
static double asciiporn_zeros_rtol=0;

double
asciiporn_zeros_functions_func(double x, void *params)
{
    asciiporn_zeros_parameters *myparams = params;
...
call_solver(solver_type solver, PyObject *self, PyObject *args)
{
    ...
    asciiporn_zeros_parameters params;
    jmp_buf env;
    ...
    if (!setjmp(env)) {
        ...
        params.error_num = 0;
        zero = solver(asciiporn_zeros_functions_func, a, b,
                xtol, asciiporn_zeros_rtol, iter, (default_parameters*)&params);
        Py_DECREF(fargs);
