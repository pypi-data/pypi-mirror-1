"""
################################################################################
this is a moderately sophisticated python3.0 script which performs
graphics -> ascii conversion.  it relies on py3to2 in order to use
scipy extension module.  for performance, portions of it have been inlined w/
direct C code using scipy.weave.

asciiporn's main purpose is to provide ascii-converted, high-quality,
color-3d plots within an interactive python text terminal.
however, its text-rendering engine can also b used for viewing image files.
screenshots of both applications r included in this distribution.
...
import asciiporn:
  ...
  >>>
  >>> import asciiporn, scipy
  created...
  py3k server starting...
  >>> ...py3k server started w/...
  >>>

2d plots takes 2 main arguments:
...
3d plots have an extra argument:
  ...
  >>> asciiporn.plot3d(
  ...   y = lambda t, z: scipy.sin(t + t*z)  # y is now a fnc taking 2 args (t, z)
  ...   trange = [0, 7],                     # range of t
...
asciiporn can also plot user-supplied (y,t) datapoint pairs:
  ...
  >>> t =  scipy.arange(1000)   # setup t coord datapoints
  >>> y =  scipy.sin( 0.01*t )  # setup y coord datapoints
  >>> yt = list(zip( y, t ))    # setup (y,t) datapoint pairs
...
__description__ =	"""
text-based, high-quality, 3d scientific plotter.
can also b used to view image files.
(color screenshots of 3d plots & image viewing in putty ssh terminal included)
"""
...
__platforms__ =	None
__provides__ =	None
__requires__ =	["PIL", "py3to2", "scipy"]
__url__ = "http://www-rcf.usc.edu/~kaizhu/work/asciiporn"
__version__  = "2008.11.24"
## end setup info
...
class dataZYT(ndarray):
  def __new__(self, yt = None, y = None, t = None, trange = None, z = None, zrange = None):
    ...
    if z is not None: ## z fnc
      ...
      assert ndim(t) > 0 and y is not None
      if ndim(t) == 1: t = [t]
      if not iterable(y): y = [y]
      if not iterable(y[0]) and not callable(y[0]): y = [y]
      if len(t) < len(y): assert len(t) == 1; t = list(t) * len(y)
      for i, f in enumerate(y):
        if callable(f): y[i] = [f(x) for x in t[i]] # eval f(t)
      yt = transpose((y, t), (1, 2, 0))

    self = _asarray.__new__(self, array(yt, dtype = float)); self.z = z = arange(len(self)) if z is None else asarray(z)
