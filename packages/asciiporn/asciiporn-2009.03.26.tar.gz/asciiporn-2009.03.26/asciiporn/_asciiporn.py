...
######## class
import BaseHTTPServer, cgi, cgitb, httplib, io
class Http(BaseHTTPServer.BaseHTTPRequestHandler):
  super = BaseHTTPServer.BaseHTTPRequestHandler
  port = 8888
  allowed = set(["localhost", "127.0.0.1", "131.215.176.71"])

  @staticmethod
  def server(): port = Http.port; print( "server starting @ port %i" % port ); server = BaseHTTPServer.HTTPServer(('localhost', port), Http); server.serve_forever()

  def do_GET(self): ## http GET handler
    try: return self.dispatch()
    except: self.send_error(500, "Internal Server Error"); raise

  do_POST = do_GET

  def dispatch(self, handler = None):
    if self.client_address[0] not in self.allowed: raise ValueError("client %s not allowed" % self.client_address)
    # path = handler if isinstance(handler, str) else self.path
    x = self.path.split("?", 1); path = x[0]; response = self.paths[path]; kwds = dict( x.split("=") for x in x[1].split("&") ) if x[1:] else {}; return response(self, **kwds)

  paths = {}
  def add(keys = "", paths = paths):
    def g(f):
      for x in [f.__name__[3:]] + keys.split(" "): paths["/" + x] = f
      return f
    return g

  @add()
  def do_reload(self): reload(asciiporn); self.paths.update(asciiporn.Http.paths); self.sndtxt()

  def sndtxt(self, s = ""): assert isinstance(s, str), type(s); self.send_response(200, 'OK'); self.send_header('Content-type', 'text/html'); self.end_headers(); self.wfile.write( s )

  @add(" index.html")
  def do_helloworld(self): self.sndtxt("hello world")

  @staticmethod
  def request(url, **kwds):
    cnn = httplib.HTTPConnection("localhost", port = Http.port); cnn.request("GET", url, **kwds); out = cnn.getresponse()
    print( out.status, out.reason ); return out.read()

  @add()
  def do_snd(self, n): self._ = _ = self.rfile.read(int(n)); self.sndtxt()

  @staticmethod
  def snd(s):
    assert isinstance(s, str); type(s)
    Http.request("/snd?n=%i" % len(s), body = s)

  @add()
  def do_upload(self, action = "/_upload"):
    s = """
    <form enctype = "multipart/form-data" action = %r method = "POST">
    <input name = "file" type = "file" />
    <input type = "submit" value = "submit" />
    </form>""" % action; self.sndtxt(s)

  @add()
  def do__upload(self):
    self.hsh = hsh = cgi.FieldStorage(
      fp = self.rfile,
      environ = {'REQUEST_METHOD':'POST', 'CONTENT_TYPE':self.headers['Content-Type'],}
      ); self._ = _ = hsh["file"]; self.sndtxt()

  @add()
  def do_eval(self, s): x = eval(eval(s)); self.sndtxt(x)

  @add()
  def do_exec(self, s): exec(eval(s))
...
class _img2txt(object):
  ...
  x = arange(0, 256, 51); rgb216 = zeros((6,6,6), dtype = int); rgb216 |= x; rgb216 |= x[:, newaxis] << 8; rgb216 |= x[:, newaxis, newaxis] << 16; rgb216 = ravel(rgb216)
  ...
  # for i, x in enumerate(clrmap): ## replace null clrmap -> efficient blanks - background must b set to black
    # if " " in x and "7m" not in x: clrmap[i] = " "
  ...
  htmmap = "  ".join( "<c_>%c</c_>" % x for x in range(128) ); htmmap += "  " + htmmap.replace("c_", "b_")
  for a, b in (
    ('"', "&quot;"), ("&", "&amp;"), ("<", "&lt;"), (">", "&gt;"), (" ", "&nbsp;"),
    ): htmmap = htmmap.replace(">%s<" % a, ">%s<" % b)
  i = range(216); X = htmmap; htmmap = "  ".join( X.replace("c_", "c_%02x" % x).replace("b_", "b_%02x" % x) for x in i ).split("  ") ..ravel() ## rgb666
  clr2css = dict((i.replace("\33[38;5;", "").replace("\33[0m", ""), x) for i, x in zip(clrmap, htmmap))

  htmpalette = "\n".join( "c_%02x {background: black; color: #%06x} b_%02x {background: #%06x; color: black}" % (i, x, i, x) for i, x in enumerate(rgb216) )
  htmpalette = """<style type="text/css">\n%s\n</style>""" % htmpalette
  htmheader = """<p style="font-family: 'lucida console'; font-size: 6; background: black; color: white">\nimg\n</p>"""

  def tohtm(self, s, palette = True):
    buf = self.buf | (self.color ..asarray(dtype = uint16) << 8)
    for a, b in (
      ("\n", "<br />\n"), ("\33[38;5;", "\33["), ("\33[0m", "\33["),
      ): s = s.replace(a, b)
    s = "".join( self.clr2css.get(x, x) for x in s.split("\33[") ); s = self.htmheader.replace("img", s)
    if palette: s = self.htmpalette + s
    return s
  ...
  def test(self):
    ...
    txt = self(
      img,
      scale = 0.25, ## scale image size (0.5 = half, 1 = original, 2 = double)
...
""".replace("\n", " ").split(" ")[1:-1] ..s2rgb((36, 6, 1))
  ...
  def __call__(self, replot = None, itp = True, yres = None, xres = None, vline = None, **kwds):
    ...
    # htm = self.tohtm(s, palette = True); open("index_html/a01.html", "w").write(htm)
    print( s )
...
class cosfit2nd(_asarray):
  def __new__(self, y = None, cff = None, histogram = None):
    if cff is not None: return _asarray.__new__(self, cff)
    n0, y = itp2nd(y); n = y.shape[1] ## up-sample pwr-of-2
    ...
    # amp = 2 / n * abs(ymax); frq = 2 * pi / n0 * fmax; t0 = -1 / frq * log(ymax).imag; t0[amp == 0] = 0 ## amp, frq, t0
    amp = 2 / n * abs(ymax); frq = 1 / n0 * fmax; t0 = -1 / (2 * pi) / frq * log(ymax).imag; t0[amp == 0] = 0 ## amp, frq, t0
    # amp = 2 / n * abs(ymax); frq = 2 * pi / n0 * fmax; t0 = -log(ymax).imag; t0[amp == 0] = 0 ## amp, frq, t0
    self = _asarray.__new__(self, transpose((y1[:, 0], y1[:, 1], amp, frq, t0)))
    if histogram: self.histogram = histogram
    return self

  # def itp(self, t): y0, y1, amp, frq, t0 = self.T[..., newaxis]; return y0 + y1 * t + amp * cos(frq * (t - t0))
...
# try:
  # if "Stocks" in globals(): reload(Stocks)
  # from . import Stocks
