...
if 1: ######## math
  ...
  def inline(code, **vars): weave.inline(code = code, arg_names = tuple(vars.keys()), local_dict = vars); return vars
...
""".replace("\n", " ").split(" ")[1:-1] ..s2rgb((36, 6, 1))
  ...
  def __call__(self, replot = None, itp = True, yres = None, xres = None, **kwds):
    ...
    s = self.plot().tostr(); print( self.header() ); print( s )
...
class cosfit2nd(_asarray):
  def __new__(self, y = None, cff = None, histogram = None):
    if cff is not None: return _asarray.__new__(self, cff)
    y = as2d(array(y), nonempty = True); y1 = polyfit2nd(y, nth = 1); y -= y1.itp(arange(y.shape[1])) ## linear regression baseline
    n0, y = itp2nd(y); y -= y.mean(axis = 1)[:, newaxis]; n = y.shape[1] ## up-sample pwr-of-2
