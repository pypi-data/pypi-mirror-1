...
import asciiporn, os, sys



import collections, functools, itertools, numpy; from itertools import *; from numpy import *; from asciiporn import weave
# try: import scipy.weave
# except ImportError: sys.modules["scipy"] = asciiporn
...
if 1: ######## math
  ...
  # def sequential_orequal(arr, i, x):
  ...
  def sequential_assign(arr, i, x, assign = "="):
    arr = ravel(arr); dtype = arr.dtype; i = ravel(i); x = ravel(x)
    assert isint(i), i.dtype
    assert i.shape == x.shape and x.dtype == dtype and 0 <= i.min() <= i.max() <= len(arr), (i.shape, x.shape, arr.dtype, x.dtype, i.min(), i.max(), len(arr))
    if assign not in ("=", "&=", "|="): raise ValueError("unknown assign type %r" % assign)
    weave.inline("""const int N = Ni[0]; for (int j = 0; j < N; j++) {arr[*i] %s *x; i++; x++;}""" % assign, ("arr", "i", "x")); return arr
...
if 1: ######## simd
  def dot2nd(arr, x):
    arr = as2d(arr, contiguous = True); dtype = arr.dtype; x = as2d(x, contiguous = True); assert x.dtype == dtype, (x.dtype, dtype); out = zeros(len(arr), dtype = dtype); ptr = ravel(dtype.type())
    if len(x) <= 1: assert arr.shape[1] == x.shape[1], shapes(arr, x); dx = 0
    else: assert arr.shape == x.shape, shapes(arr, x); dx = x.shape[1]
    weave.inline("""
    int i; const int Ni = Narr[0]; int j; const int Nj = Narr[1];
    for (i = 0; i < Ni; i++, out++, x += dx) {
    for (j = 0, ptr = x; j < Nj; *out += *arr * *ptr, j++, arr++, ptr++);
    }""", ("arr", "x", "out", "dx", "ptr")); return out
  ...
  def histogram2nd(arr):
    ...
    h = zeros((len(arr), n), int); weave.inline("""
    int i; const int Ni = Narr[0]; int j; const int Nj = Narr[1]; const int Nh1 = Nh[1];
...
""".replace("\n", " ").split(" ")[1:-1] ..s2rgb((36, 6, 1))
  ...
  def __call__(self, replot = None, itp = True, yres = None, xres = None, **kwds):
  ...
  def plot(self):
    ...
    for i, yt in enumerate(arr):
      ...
      # sequential_orequal(buf, a, b)
      sequential_assign(buf, a, b, assign = "|=")
  ...
  def plot(self, **kwds):
    ...
    for z, yt, t0 in zip_longest(z, yt, data.roots()):
      if z != z0:
        ...
        t0 = self.scaleyt(transpose((t0, t0)), inverty = None)[:, 1]; buf[z, t0] = self.block; color[z, t0] = self.rooti ## plot y roots
      z0 = z
    return self
...
class polyfit2nd(_asarray):
  def __new__(self, y = None, t = None, nth = None, wgt = None, cff = None):
    ...
    y = as2d(y, transpose = True)
    if t is None: t = arange(len(y))
  ...
  def test():
    ...
    f = polyfit2nd(y = y, t = t, nth = 8); z = f.itp(t); plot(y = [y, z], t = t)
...
class fft2(_asarray):
  ...
  def dot(self, w):
    arr = asarray(self); dtype = arr.dtype; w = ravel(w); assert w.dtype == dtype, (w.dtype, dtype)
    out = zeros((len(arr), len(w)), dtype = dtype); wx = ravel(dtype.type()); arrx = wx.copy(); arry = wx.copy(); outx = wx.copy(); weave.inline("""
  ...
  def sort(self):
    arr = asarray(self); dtype = arr.dtype; x = ravel(dtype.type()); ptr = x.copy(); weave.inline("""
  ...
  def dif(self, w = None):
    arr = asarray(self); dtype = arr.dtype; n = arr.shape[1]; w = exp(-2j * pi / n) ** arange(n) if w is None else w.copy()
    x = ravel(dtype.type()); wptr = x.copy(); e = x.copy(); o = x.copy(); weave.inline("""
...
class cosfit2nd(_asarray):
  def __new__(self, y = None, cff = None, histogram = None):
    if cff is not None: return _asarray.__new__(self, cff)
    y = as2d(array(y)); y1 = polyfit2nd(y, nth = 1); y -= y1.itp(arange(y.shape[1])) ## linear regression baseline
    n0, y = itp2nd(y); y -= y.mean(axis = 1); n = y.shape[1] ## up-sample pwr-of-2

    y = fft2(y)
    if histogram: yh, fh = histogram = y.histogram(); ymax, fmax = y.fmax(yh, fh); fh *= n0 / n
    else: ymax, fmax = y.fmax()

    amp = 2 / n * abs(ymax); frq = 2 * pi / n0 * fmax; t0 = 1 / frq * log(ymax).imag ## amp, frq, t0
    self = _asarray.__new__(self, transpose((y1[:, 0], y1[:, 1], amp, frq, t0)))
    if histogram: self.histogram = histogram
    return self

  def itp(self, t): y0, y1, amp, frq, t0 = self.T[..., newaxis]; return y0 + y1 * t + amp * cos(frq * (t0 + t))

  # @staticmethod
  # def histogram(self, y): return fft2(

  @staticmethod
  def test():
    n = 60; t = arange(n); t0 = 3.2
    frq = 0.5; y0 = 1 / n * t + cos(frq * (t + t0))
    y0 += rng(n) * 2
    y0 = [y0]
    # y0 += [rng(n) - 0.5]
