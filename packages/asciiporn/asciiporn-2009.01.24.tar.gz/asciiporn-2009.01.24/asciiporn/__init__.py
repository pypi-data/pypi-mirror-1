...
& contains integrated c code for efficent graphics rendering.
asciiporn can easily render 32bit megapixel graphics on a 1ghz 500mb machine

screenshots of both plotting and image viewing r included w/ this distribution.
...
for basic usage, remotely run python in an ssh terminal:

  >>> ## print image file as colorized ascii art
  >>> from asciiporn import img2txt
  >>> print img2txt("mario.jpg")

  >>> ## plot several math functions
...
3d plots have an extra argument:
  ...
  >>> import asciiporn, numpy
  >>> asciiporn.plot3d(
  ...   ftz = lambda t, z: numpy.sin(t + t*z), ## 3d function
  ...   tmin = 0.0, tmax = 7.0,                ## range of t
  ...   zmin = 0.0, zmax = 0.25,               ## range of z
  ... )
...
to 0.5 (or less):
  ...
  >>> plaintxt = asciiporn.img2plaintxt("mario.jpg", scale = 0.75)
  >>> print( plaintxt )
...
__description__ =	"""
view color images & scientific plots from ssh terminal
...
class importer(object):
  ...
  def find_module(self, mname, path = None):
    ...
    s = s.replace(self.magic, "\nfrom __future__ import with_statement, division, print_function; from py3k_syntax import *\n", 1)
