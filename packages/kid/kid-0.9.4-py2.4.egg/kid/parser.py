...
__date__ = "$Date: 2006-10-26 09:24:33 -0400 (Thu, 26 Oct 2006) $"
__author__ = "Ryan Tomayko (rtomayko@gmail.com)"
__copyright__ = "Copyright 2004-2005, Ryan Tomayko"
__license__ = "MIT <http://www.opensource.org/licenses/mit-license.php>"

from kid.et import *  # ElementTree
from kid.util import open_resource, QuickTextReader, show_error
...
def document(file, encoding=None, filename=None, entity_map=None):
    if not hasattr(file, 'read'):
        ...
        if filename is None:
            filename = '<string>'
    p = Parser(file, encoding, entity_map)
...
def _coalesce(stream, encoding, extended=True):
...
def Parser(source, encoding=None, entity_map=None):
    return ExpatParser(source, encoding, entity_map)
...
class ExpatParser(object):
    ...
    def __init__(self, source, encoding=None, entity_map=None):
