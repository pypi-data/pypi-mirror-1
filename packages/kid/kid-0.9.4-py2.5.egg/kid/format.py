...
class Format(object):
    ...
    def __repr__(self):
        ...
        for f in self.text_filters + self.context_filters:
            ...
            if attr == 'indent_lines':
                if 'indent' not in parms:
                    args['indent'] = True
            elif attr == 'wrap_lines':
                if 'wrap' not in parms:
                    args['wrap'] = True
            else:
                args[attr] = True
    ...
    def educate_quotes_filter(self, s, last_char=None, next_char=None):
        """Educate quotes filter (needs context characters)."""
        return educate_quotes(s, last_char, next_char,
            self.apostrophe, self.squotes, self.dquotes,
            self.educate_backticks)
