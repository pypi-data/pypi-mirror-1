...
from kid.serialization import HTMLSerializer, XMLSerializer
...
def test_escape():
    for serializer in XMLSerializer, HTMLSerializer:
        for escape in serializer.escape_cdata, serializer.escape_attrib:
            assert escape('str') == 'str'
            assert escape('k\204se') == 'k\204se'
            assert escape('str', 'utf-8') == 'str'
            assert escape('k\204se', 'utf-8') == 'k&#132;se'
            assert escape('&') == '&amp;'
            t = '<'
            s = escape(t)
            if serializer != HTMLSerializer or escape != serializer.escape_attrib:
                t = '&lt;'
            assert s == t
            t = '"'
            s = escape(t)
            if escape == serializer.escape_attrib:
                t = '&quot;'
            assert s == t
            try:
                assert escape(1)
