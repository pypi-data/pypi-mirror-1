...
$Id: productman.py 49133 2006-09-18 08:29:22Z bdelbosc $
"""
import sys
import os
import logging
...
"""  % (self.version_new, self.tag_url,
    ...
    def init(self):
        ...
        if files_to_add:
            ...
            command('svn add ' + files_to_add)
            command('svn commit -m"bundleman init" ' + files_to_add)
...
""" % (prod_name, prod_version, os.getenv('USER'),
    ...
    def buildArchive(self, archive_dir):
        ...
        if self.status not in ('use_tag'):
            ...
            self.showAction()
            return -1
        product_name = self.path
        if self.version[1]:
            version = self.version[1] + '-' + self.version[2]
        else:
            # not a bundleman product
            version = 'r' + self.revision
        archive_name = '%s-%s.tgz' % (product_name, version)
        archive_path = os.path.join(archive_dir, archive_name)
        logger.info('Creating archive: %s' % archive_name)

        # extract tag
        tmpdir = mkdtemp()
        product_path = os.path.join(tmpdir, product_name)
        command('svn -q export %s %s' % (self.tag_url, product_path))
        ...
        command('cd %s; find . -type f -not -name MD5SUMS | xargs md5sum'
                '> MD5SUMS' % product_path)

        # tarball
        command('cd %s; tar czvf %s %s' % (tmpdir, archive_path, product_name))
...
"""
    def __init__(self, argv=None):
        if argv is None:
            argv = sys.argv[1:]
        self.product_path = os.path.abspath(os.path.curdir)
