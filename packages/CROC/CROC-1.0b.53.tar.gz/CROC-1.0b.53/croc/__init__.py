...
def SampleCurves(sample, N=500):
    ...
    SUM = Curve([(0,0), (1,1)])
    AREA = []
    for i in xrange(options.samples):
...
class ScoredData:
    ...
    def sweep_threshold(self, tie_mode = "smooth"):
        ...
        * sample - randomly shuffle the ties.
        ...
        TP = FP = Rational(0, 1)
        TN = Rational(self.num_neg, 1)
        FN = Rational(self.num_pos, 1)

        yield TP.evalf(), TN.evalf(), FP.evalf(), FN.evalf()
...
class Curve:
    ...
    def append(self, x, y):
        ...
        x = float(x); y = float(y)
        if len(self) >= 1 :
            assert( x >= self[-1][0] ) #assert that x monotonically increases.
            assert( y >= self[-1][1] ) #assert that y monotonically increases.
