...
class RequestQueuer(object):
    ...
    def _checkActive(self):
        while self.getActive() < self.max_simul_reqs and self.getPending() > 0:     
            ...
            if not dispatched_requests:
                LOGGER.critical("Done with Check active. (1) Active: %s, Max Simul: %s, Pending: %s" % (
                    self.getActive(),
                    self.max_simul_reqs,
                    self.getPending()))
                return
        LOGGER.critical("Done with Check active. (2) Active: %s, Max Simul: %s, Pending: %s" % (
            self.getActive(),
            self.max_simul_reqs,
            self.getPending()))
                            
    def _requestComplete(self, response, deferred, host):
        LOGGER.critical("Request complete for %s. (2) Active: %s, Max Simul: %s, Pending: %s" % (
            host,
            self.getActive(),
            self.max_simul_reqs,
            self.getPending()))
    ...
    def _requestError(self, error, deferred, host):
        LOGGER.critical("Request error for %s. (2) Active: %s, Max Simul: %s, Pending: %s" % (
            host,
            self.getActive(),
            self.max_simul_reqs,
            self.getPending()))        
