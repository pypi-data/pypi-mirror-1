...
# Remove pytz/req from pagegetter
...
class AWSpider:
    ...
    def _checkPeersCallback( self, discovered ):
        ...
        failed_peers = set( self.failed_peers.keys() )
        discovered_peers = set( discovered.keys() )
        new_peers = discovered_peers - ( existing_peers | failed_peers )
        old_peers = ( existing_peers | failed_peers ) - discovered_peers
        ...
        for uuid in old_peers:
            ...
            if uuid in self.failed_peers:
                del self.failed_peers[ uuid ]
...
#        print self.peers 
        ...
        logger.debug( "Failed peers updated to: %s" % self.failed_peers )
    ...
    def verifyPeer( self, uuid, peer ):
        ...
        if len(deferreds) > 0:
            d = DeferredList( deferreds, consumeErrors=True )
            d.addCallback( self._verifyPeerCallback, uuid )
            return d
        else:
            return None
    
    def _verifyPeerCallback( self, data, uuid ):

        for row in data:
            if row[0] == True:
                return None
        self.failed_peers[ uuid ] = True
    ...
    def _verifyPeerLocalIPCallback( self, data, uuid, local_ip, port ):
        ...
        self.peers[ uuid ]["uri"] = "http://%s:%s" % (local_ip, port )
    ...
    def _verifyPeerPublicIPCallback( self, data, uuid, public_ip, port ):
        ...
        self.peers[ uuid ]["port"] = port
        self.peers[ uuid ]["uri"] = "http://%s:%s" % (public_ip, port )
