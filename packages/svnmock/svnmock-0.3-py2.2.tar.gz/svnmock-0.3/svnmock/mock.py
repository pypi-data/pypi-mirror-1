...
class State(object):
    ...
    _incomplete = classmethod(_incomplete)
...
# Operations on the global active_session

def add_command(func, args=(), return_val=None):
    return active_session.add_command(func, args, return_val)
    
def add_sequence(state_list=[]):
    return active_session.add_sequence(state_list)
    
def add_any_order(state_list=[]):
    return active_session.add_any_order(state_list)
    
def add_error(func, args, exc):
    return active_session.add_error(func, args, exc)
