...
class Synchronization(grok.Annotation, grok.Model):
    ...
    def sync(self):
        ...
        try:
            remote_urls = set(self.client.remote_urls())
        except Exception, e:
            log.exception(e)
        ...
        # Filter URLs against our policies
        for url_string in list(remote_urls):
            handler = gocept.lms.interfaces.ISchemeHandler(url_string, None)
            if not (handler and handler.allow(url_string)):
                remote_urls.discard(url_string)
        
        url_provider = zope.component.getUtility(gocept.lms.interfaces.IURLProvider)
        remote_urls = set(url_provider.add(u)
