...
class Client(grok.Model):
    ...
    def notify(self, urls):
        ...
        log.info('Notifying client %s about %s URLs' % (self.id, len(urls)))
...
class Notifications(grok.Annotation, grok.Model):
    ...
    def _p_resolveConflict(self, oldState, savedState, newState):
        ...
        # gratious about the other values.
        savedState['enabled'] = savedState['enabled'] or newState['enabled']
        savedState['last'] = newState['last']
        savedState['failed'] = min(savedState['failed'], newState['failed'])
        return savedState
