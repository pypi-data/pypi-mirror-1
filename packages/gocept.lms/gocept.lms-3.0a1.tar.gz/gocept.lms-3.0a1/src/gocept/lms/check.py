...
def check():
    ...
    while True:
        ...
        if handler is None:
            log.info('%s: ignored due to unknown scheme' % url.url)
            continue
...
def process_finished():
    ...
    for thread in finished:
        ...
        url.last_check = now
        url.reason = thread.reason
        if url.state != thread.state:
            url.state = thread.state
            url.last_state_change = url.last_check
