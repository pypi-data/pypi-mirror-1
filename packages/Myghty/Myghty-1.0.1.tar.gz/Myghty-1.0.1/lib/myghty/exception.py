# $Id: exception.py 2039 2006-02-05 06:58:40Z zzzeek $
...
import string, sys, re, traceback, StringIO, os
import posixpath as unixpath
from myghty.util import *

# get the directory of myghty, to filter it out of
# friendly stack traces
import imp
(f, MYGHTY_DIR, desc) = imp.find_module("myghty")
MYGHTY_DIR = os.path.normcase(MYGHTY_DIR)
        ...
        def get_file_lines(self, f, linenum, span):
            ...
            while True:
                ...
                if i in range(linenum - span, linenum + span + 1):
                    lines.append(Error.CodeLine(i, l))
    ...
    class SimpleTraceLine(TraceLine):
        ...
        def get_lines(self, span = 3):
            if self.compiled_record is not None:
                ...
                return [Error.CodeLine(self.line, self.__code)]
            else:
                f = file(self.file)
...
class HTMLErrorFormatter(ErrorFormatter):
    def do_format(self):
        ...
        try:            
            self.interp.execute(self.comp, out_buffer = buffer, request_args = argdict(
...
class Syntax(Error):
    ...
    def __init__(self, error, comp_name, source_line, line_number, source = None, file = None):
        self.error = error
        Error.__init__(self, error)
        self.lead_rec = Error.SyntaxTraceLine(source_line, line_number, comp_name, source, file)
