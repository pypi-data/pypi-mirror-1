# $Id: lexer.py 2013 2005-12-31 03:19:39Z zzzeek $
...
class Lexer:
    ...
    def start(self):
        ...
        if self.current.in_def or self.current.in_method:
            ...
            if not self.current.ending.match(end):
                block_name = self.current.block_name
                self.raise_syntax_error("no </%%%s> tag for <%%%s %s> block" % (type, type, block_name))
    ...
    def raise_syntax_error(self, error):
        raise exception.Syntax(error = error, comp_name = self.get_name(), source_line = self._current_line(), line_number = self.line_number(), source = self.current.source, file = self.input_file)
