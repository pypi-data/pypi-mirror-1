...
"""Protocol handlers for the different versions of Google 
Visualization API.
...
class GViz_0_5(BaseGVizHandler):
    """Implementation of the Google Visualization API data source
    ...
    def get_std_params(self):
        """Return a mapping containing the following values :
    ...
    def _error(self, req, error, reqId, version, responseHandler):
        ...
        if isinstance(error, tuple):
            ...
            tb = None
        if isinstance(error, PermissionError) and req.remote_user is None:
