...
from iterpipes import (bincmd, cmd, linecmd, run, format, each, Fun,
    join, strip, call, check_call, CalledProcessError)
from subprocess import STDOUT

STDOUT_FILENO = 1

def test_basic_single():
    eq_(run(cmd('ls -d /') | join), '/\n')

def test_basic_many():
    eq_(run(cmd('echo foo | wc -l') | join), '1\n')
...
def test_dont_touch_stdout():
    eq_(run(cmd('ls -d /', stdout=STDOUT_FILENO) | join), '')
...
def test_stdin_iterable_excpt():
    ...
    def g():
        raise E('foo')
        yield 'bar'
    assert_raises(E, lambda: run(cmd('grep foo') | join, g()))
...
def test_run_output():
    ...
    eq_(run(cmd('echo foo') | join), 'foo\n')
    eq_(join(run(cmd('echo foo'))), 'foo\n')
    eq_(call(cmd('echo foo')), 0)

def test_or_operator():
    eq_(run(cmd('echo foo') | (lambda x: x) | join), 'foo\n')
    eq_(run(Fun(lambda _: ['a']) | cmd('tr a-z A-Z') | join), 'A')

def test_each():
    upper = lambda x: x.upper()
    eq_(run(linecmd('echo "a\nb\nc"') | each(upper) | strip('\n') | join), 'ABC')
