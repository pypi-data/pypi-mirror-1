...
or (num + 1 == len(self.ordres) and self.script[-1] != self.entetel2):
            if self.commande == False:
                self.script.append('interact +++ return\n')
            else:
                self.script.append('expect eof\n')
            resultat = abspath(join(self.destination + self.machinefinale
                                                 + self.extfichier))
            try:
    ...
    def interprete_ordres(self, num):
        ...
        if ligne != linesep:
            if ligne.split()[0] == '-c':
                print 'Erreur de syntaxe : -c est seul sur la ligne, il vous \
manque le nom de la machine à contacter'
                sys.exit(1)
            if self.commande == True:
                self.traite_commande(ligne)
            else:
                machine = ligne.split()[0]
                self.machinefinale = ligne.split()[0].split('@')[-1]
                if self.premiereligne == True:
