...
user or a tag::
    ...
    >>> tagger.getItemsFor('bongo')
    set(['item1_id'])
    
    >>> tagger.getItemsFor('user1')
    set(['item1_id'])

    >>> tagger.getTagsFor('user1') == set(['bongo', 'java', 'smoothie'])
    True

    >>> tagger.getTagsFor('item1_id') == set(['bongo', 'java', 'smoothie'])
    True

    >>> tagger.getUsersFor('item1_id')
    set(['user1'])

    >>> tagger.getUsersFor('smoothie')
    set(['user1'])

    >>> tagger.delete_tag(item, user1, 'java')

    >>> tagger.getTagsFor('user1') == set(['smoothie', 'bongo'])
    True

Now lookups for java should return nothing::

    >>> tagger.getUsersFor('java')
    set([])

In addition, the tag should have been reaped from the RDF store::
...
for this item (all that are currently stored in fact)::
    ...
    >>> tagger.delete(item)
    >>> tagger.getUsersFor(item)
    set([])

## @@ reap not reaping foaf entries
...
Now let's try something fancier::
    ...
    >>> tagger.getUsersFor('wow') == set((user2, user3))
    True

    >>> tagger.getUsersFor('wow') == tagger.getUsersFor(item2)
    True
...
Let's makes sure delete only deletes what it is suppose to::
    ...
    >>> tagger.delete(item2)
    >>> tagger.getTagsFor(item2)
    set([])

    >>> tagger.delete(item)
    >>> tagger.getTagsFor(item)
    set([])
