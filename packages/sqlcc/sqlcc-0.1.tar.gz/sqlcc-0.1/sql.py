...
class SQL_field_type(operand):
    ...
    @property
    def schema(self):
	return '%s %s' % (self.field_name, self.type_name)
    pass
...
class vview(object):
	...
	if self.__fields:
	    ...
	    pass
	if self.__cond:
	    conds = self.__cond.get_sql_name()
	else:
	    conds = ''
	    pass
	if restrict:
	    if conds:
		conds = '(' + conds + ') ADN (' + restrict.get_sql_name() + ')'
	    else:
	...
	if conds:
	    conds = ' where ' + conds
	    pass
	return 'select ' + fds + ' from ' + tbs + conds
