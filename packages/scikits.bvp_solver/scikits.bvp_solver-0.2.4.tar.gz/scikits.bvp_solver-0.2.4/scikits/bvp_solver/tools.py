...
def preparg(arg):
    ...
    """
    if not (isinstance(arg, numpy.ndarray ) or arg is None):
        try:
            # if the argument was not a numpy array, try to cast it to one
            array = numpy.array(arg, dtype = numpy.float)
            if has_nans(array):
        ...
        #if it was a numpy array jsut pass it through
        return arg
...
def argShapeTest(arg, shape, argName , recommendation):
    ...
    if not isinstance(arg, numpy.ndarray):
        raise ValueError(argName + " is not a numpy array")
