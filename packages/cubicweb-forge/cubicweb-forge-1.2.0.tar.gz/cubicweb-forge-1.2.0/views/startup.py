        ...
        if not user.matching_groups(('managers', 'staff', 'users')):
            ...
            for i, project in enumerate(projects):
                ...
                if i == chcol:
                    w(u'</td><td>')
            w(u'</td></tr></table>')
            w(u'<p><a href="%s">%s</a></p>' % (self.req.build_url('project'),
                                               _('view all active projects')))
            w(u'</div>')
            w(u'</div>')

    def _right_section(self):
        user = self.req.user; _ = self.req._; w = self.w
        if not user.matching_groups(('managers', 'staff', 'users')):
            w(u'<div>')
            self.folders()
            w(u'</div>')
