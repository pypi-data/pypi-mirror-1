...
class LinksViewlet(ViewletBase):
    ...
    def update(self):
        ...
        #this is the new way:
        # XXX check if local links are allowed in the configuration (planned for future relase),
        # otherwhise we can just return links for the plonesite
        ...
        if nextProvider != linkSource and checkPermission(MANAGE_LINKS_PERMISSION_Z3, nextProvider):
            self.actions.append(dict(title = _(u"Set links for ${context}",
                                             mapping={'context': nextProvider.title}),
