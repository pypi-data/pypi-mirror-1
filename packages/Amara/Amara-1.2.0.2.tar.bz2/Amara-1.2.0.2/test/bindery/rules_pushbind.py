...
class TestRules(unittest.TestCase):
    ...
    def testSimpleStringRules(self):
        ...
        rule = binderytools.simple_string_element_rule(u'bookmark/title')
        elems = amara.pushbind(XBEL, u"folder", rules=[rule])
        for folder in elems:
            if hasattr(folder, 'bookmark'):
                #print "="*10
                #print folder.xml()
                #print "="*10
                self.assert_(isinstance(folder.bookmark.title, unicode))
        return

    def XtestMultipleOmitElementRules(self):
    ...
    def XtestSimpleAndOmitRules(self):
        rules = [
    ...
    def XtestTypeInference(self):
        ...
        self.assertEqual(doc.a.c.e, datetime.datetime(2003, 1, 30, 17, 48, 7, 848769))
        #self.assertEqual(len(list(doc.labels)), 1)
        #self.assertEqual(doc.xml_xpath(u"count(//@id)"), 4.0)
        return

    def XtestWhitespaceStripping1(self):
    ...
    def XtestWhitespaceStripping4(self):
        rules = [ binderytools.ws_strip_element_rule(u"*") ]
