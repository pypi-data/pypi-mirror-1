...
<xsl:stylesheet
  ...
  <xsl:param name="ignore-namespaces" select="/.."/>
  ...
  <xsl:template match="articleinfo" mode="HEADER">
    ...
    <xsl:variable name="authors" select="author|authorgroup/author"/>
    <xsl:variable name="others" select="othercredit|authorgroup/othercredit"/>
    <xsl:if test="$history or $authors or $others">
      <dl>
        ...
        <xsl:if test="$others">
          ...
          <xsl:apply-templates select="$others" mode="HEADER"/>
        </xsl:if>
      </dl>
    </xsl:if>
