...
// Lo que sigue es código de Pato Valarezo (c) patovala@pupilabox.net.ec Viva el Software Libre!

if (window.addEventListener) {
    window.addEventListener("load",initSGAWidgets,false);
} else if (window.attachEvent) {
    window.attachEvent("onload",initSGAWidgets);
} else {
	window.onload = function() {initSGAWidgets();}
}


function initSGAWidgets(){
           var anchors = (this.isFrame) ? window.parent.frames[window.name].document.getElementsByTagName('a') : document.getElementsByTagName('a');
           for (var i = 0; i < anchors.length; i++) {
       	        var anchor = anchors[i];
       	        var relAttribute = String(anchor.getAttribute('rel'));
       	        if (anchor.getAttribute('href')) {
       	        	if (relAttribute.toLowerCase().match('popesperar')) {
       	        		anchor.onclick = function () { ModalPopupsEsperar(anchor.getAttribute('href')); }
       	        	}
       	        }
           }
}
