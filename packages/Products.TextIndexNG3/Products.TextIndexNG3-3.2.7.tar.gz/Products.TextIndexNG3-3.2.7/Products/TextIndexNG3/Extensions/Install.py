...
def uninstall(self):
    ...
    indexes = [idx for idx in indexes if idx.meta_type in ('TextIndexNG3',)]

    for idx in indexes:
        catalog.manage_delIndex(idx.getId())

    import transaction 
    transaction.commit()
    setup_tool = getToolByName(self, 'portal_setup')
    setup_tool.runImportStepFromProfile('profile-Products.CMFPlone:plone','catalog', run_dependencies=False)
    transaction.commit()

    catalog.manage_reindexIndex([idx.getId() for idx in indexes], self.REQUEST)
