...
def validate_file(filename):
    ...
    fre = re.compile(r'\s*def\s+(t_[a-zA-Z_0-9]*)\(', re.UNICODE)
    sre = re.compile(r'\s*(t_[a-zA-Z_0-9]*)\s*=', re.UNICODE)
    counthash = { }
    linen = 1
    noerror = 1
    for l in lines:
...
def _read_lextab(lexer, fdict, module):
    ...
    lexer.lexre = re.compile(lextab._lexre, re.VERBOSE|re.UNICODE)
...
def lex(module=None,debug=0,optimize=0,lextab="lextab"):
    ...
    for f in fsymbols:
        ...
        if f.__doc__:
            if not optimize:
                try:
                    c = re.compile(f.__doc__, re.VERBOSE|re.UNICODE)
                except re.error,e:
    ...
    for name,r in ssymbols:
        ...
        if not optimize:
            ...
            if not lexer.lextokens.has_key(name[2:]):
                ...
                continue
            try:
                c = re.compile(r,re.VERBOSE|re.UNICODE)
            except re.error,e:
    ...
    try:
        ...
        lexer.lexre = re.compile(regex, re.VERBOSE|re.UNICODE)
