...
class DoapFiend(object):
    ...
    def print_doap(self, doap_xml):
        ...
        if len(plugins) == 0:
            ...
            serializer = plugins[0].serialize
        if self.options.write:
            filename = self.options.write
        else:
            filename = None
        print_doap(doap_xml, serializer=serializer, filename=filename,
    ...
    def setup_opt_parser(self):
        ...
        group_output.add_option('--debug', action='store_true', dest=
                              'debug', default=False, help=
                              'Show debugging information')

        group_output.add_option('-w', '--write', action='store', dest=
                              'write', default=False, help=
                              'Write DOAP to a file instead of displaying it.',
                              metavar='FILENAME')

        group_output.add_option('-b', '--brief', action='store_true', dest=
                              'brief', default=False, help=
                              'Show less output.')

        group_output.add_option('-C', '--no-color', action='store_true', dest=
