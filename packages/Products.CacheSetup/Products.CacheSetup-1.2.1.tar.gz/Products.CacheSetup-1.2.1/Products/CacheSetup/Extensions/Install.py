...
def uninstall(self, reinstall=False):
    ...
    # We bypass 'setEnabled' here since 'getEnabled' will always return
    # False while the filesystem and installed versions don't match.
    if not reinstall and config.PAGE_CACHE_MANAGER_ID in self.objectIds():
        utils._updateChangedSchema(cache_tool, None)  # for version 1.0x
        cache_tool.getField('enabled').set(cache_tool, False)
