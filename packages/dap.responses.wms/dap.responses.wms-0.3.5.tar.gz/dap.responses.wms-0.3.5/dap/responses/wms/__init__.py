...
def build(self, constraints=None):
    ...
    im = _dispatch(dataset, query, self.environ)
    buf = StringIO()
    im.save(buf, format)
    output = buf.getvalue()
...
def _islayer(dapvar):
    if isinstance(dapvar, GridType):
        ...
        for var in dapvar.maps.values():
            if re.match("degrees?_n", var.attributes.get('units', ''), re.IGNORECASE): lat = True
            elif re.match("degrees?_e", var.attributes.get('units', ''), re.IGNORECASE): lon = True
        return lat and lon
...
def _dispatch(dapvar, query, environ):
    ...
    for super_, func in dispatchers:
        if isinstance(dapvar, super_):
            return func(dapvar, query, environ)


def _dataset(dapvar, query, environ):
    """
    ...
    for var in dapvar.walk():
        if var.id in layers:
            try:
                layer = _dispatch(var, query, environ)
...
def _structure(dapvar, query, environ):
...
def _grid(dapvar, query, environ):
    # Get lat & lon.
    lat = lon = None
    for var in dapvar.maps.values():
        if re.match("degrees?_n", var.attributes.get('units', ''), re.IGNORECASE):
            lat = numpy.array(dapvar.maps[var.name][:])
        elif re.match("degrees?_e", var.attributes.get('units', ''), re.IGNORECASE):
    ...
    while (minlon + offset) < lon[-1]:
        ...
        jstep = min(jstep, len(y)-1)

        dx = (maxlon - minlon) / width
        dlon = x[1] - x[0]
        istep = max(int(dx/dlon), 1)
        istep = min(istep, len(x)-1)
        ...
        if hasattr(dapvar, 'missing_value'):
            data = ma.masked_equal(data, dapvar.missing_value)
        else:
            data = ma.array(data)
        mask = data.mask
        ...
        if hasattr(dapvar, 'actual_range'):
            ...
            alldata = alldata * getattr(dapvar, 'scale_factor', 1) + getattr(dapvar, 'add_offset', 0)
            actual_range = numpy.amin(alldata), numpy.amax(alldata)
        ...
        dx = x[1] - x[0]
        dy = y[1] - y[0]
        lon0, lon1 = x[0] - dx/2, x[-1] + dx/2
        lat0, lat1 = y[0] - dy/2, y[-1] + dy/2
...
def _sequence(dapvar, query, environ):
    # Find lat & lon.
    lat = lon = None
...
def _array(dapvar, query, environ):
    pass
