...
def _grid(dapvar, query):
    ...
    offset = - math.ceil((maxlon - lon[-1])/360.0) * 360.0
    while (minlon + offset) < lon[-1]:
        ...
        if hasattr(dapvar, 'actual_range'): actual_range = dapvar.actual_range
        else: actual_range = min(dapvar[:].flat), max(dapvar[:].flat)
        ...
        # Paste image. Here we map the image from the data boundary
        # to the BBOX limits.
        x0 = layer.size[0] * (minlon - x[0]) / (x[-1] - x[0])
        x1 = layer.size[0] * (maxlon - x[0]) / (x[-1] - x[0])
        y0 = layer.size[1] * (maxlat - y[0]) / (y[-1] - y[0])
        y1 = layer.size[1] * (minlat - y[0]) / (y[-1] - y[0])
...
def _sequence(dapvar, query):
    ...
    offset = - math.ceil((maxlon - query['lon_range'][1])/360.0) * 360.0
