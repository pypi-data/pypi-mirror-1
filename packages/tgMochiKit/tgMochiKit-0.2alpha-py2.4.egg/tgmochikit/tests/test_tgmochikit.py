...
def test_all_14_versions():
    ...
    latest_base_path = base_path

    # now test that giving the '1.4' version results in the latest of all known 1.4-versions
    reload(tgm_base)
    buffer = StringIO.StringIO()
    handler = logging.StreamHandler(buffer)
    tgm_logger.addHandler(handler)
    tgm.init(register_static_directory, version='1.4')
    base_path = static_path[-1]
    tgm_logger.removeHandler(handler)
    assert latest_base_path == base_path
