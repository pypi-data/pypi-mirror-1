...
class ConfigurationService(object):
    ...
    def __init__(self, reload_after=None, watch=False):
        ...
        if self.watch and have_pyinotify:
            self.notify_mask = EventsCodes.IN_MODIFY | EventsCodes.IN_DELETE | EventsCodes.IN_ATTRIB |  EventsCodes.IN_CREATE | EventsCodes.IN_MOVE_SELF
    ...
    def registerModel(self, filename):
        ...
        LOG.info('register model: %s' % filename)
    ...
    def getConfigurationForPrefix(self, prefix):
        """ Return all configuration entries as dict for a given prefix"""
        opts = self.getConfiguration().copy()
        for opt in opts.keys():
            opt_prefix, opt_key = opt.rsplit('.' , 1)
