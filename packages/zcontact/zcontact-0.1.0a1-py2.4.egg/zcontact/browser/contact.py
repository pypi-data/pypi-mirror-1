...
class ContactFieldAddForm(layout.AddFormLayoutSupport, form.AddForm):
    ...
    def nextURL(self):
        url = absoluteURL(self.context[self.newObjectName], self.request)
        return url + '/@@inline.html'

    def __call__(self):
        if self.request.get('noscript', None):
            return layout.AddFormLayoutSupport.__call__(self)
        else:
            return form.AddForm.__call__(self)
...
class ContactFieldContainerInlineForm(form.Form):
    ...
    template = ViewPageTemplateFile("templates/inlinefieldcontainer.pt")
    layout = None
    contentName = None
    label = u''
    fields = field.Fields()
    ...
    addButton = jsaction.JSButton(title=u'+')
