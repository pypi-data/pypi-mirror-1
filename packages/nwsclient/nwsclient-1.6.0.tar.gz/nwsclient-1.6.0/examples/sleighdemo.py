...
import sys, getopt, time, math, socket
...
if __name__ == '__main__':
    ...
    try:
        ...
        def f(x):
            time.sleep(random.gauss(mu, 2.0))
            return math.exp(x)

        # import necessary modules, and compute 'mu', the mean compute time
        s.eachWorker("import math, time, random; mu = abs(random.gauss(10.0, 4.0))")
        ...
        for i in xrange(numtasks):
            ...
            s.eachElem(f, range(numtasks), **eo)
