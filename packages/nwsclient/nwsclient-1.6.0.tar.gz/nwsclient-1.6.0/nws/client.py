...
class NwsServer(object):
    ...
    def __init__(self, host='localhost', port=NWS_PORT):
        ...
        try:
            ...
            raise NwsConnectException('unable to connect to the NWS server at %s:%d' % (host, port))
...
class NetWorkSpace(object):
    ...
    def __init__(self, wsName='__default', serverHost='localhost', serverPort=NWS_PORT, useUse=False, server=None, **opt):
    ...
    def __retrieve(self, varName, op, missing=None, fobj=None):
        _sanityFileObj(fobj, 'w')

        self.send('0003%020d%s%020d%s%020d%s' % \
                (len(op), op, len(self.curWs), self.curWs,
                 len(varName), varName))
    ...
    def fetchTry(self, varName, missing=None):
        ...
        try:
            return self.__retrieve(varName, 'fetchTry', missing)
    ...
    def findTry(self, varName, missing=None):
        ...
        try:
            return self.__retrieve(varName, 'findTry', missing)
        except NwsOperationException:
            return missing
    ...
    def store(self, varName, val):
        ...
        self.send('0005%020d%s%020d%s%020d%s%020d%s%020d' % \
                (len(op), op, len(self.curWs), self.curWs,
                 len(varName), varName, len(descTxt), descTxt, len(pVal)))
        self.send(pVal)
    ...
    def fetchFile(self, varName, fobj):
        ...
        return self.__retrieve(varName, 'fetch', fobj=fobj)
    ...
    def fetchTryFile(self, varName, fobj):
        ...
        try:
            return self.__retrieve(varName, 'fetchTry', fobj=fobj)
        except NwsOperationException:
            return False
    ...
    def findFile(self, varName, fobj):
        ...
        return self.__retrieve(varName, 'find', fobj=fobj)
    ...
    def findTryFile(self, varName, fobj):
        ...
        try:
            return self.__retrieve(varName, 'findTry', fobj=fobj)
        except NwsOperationException:
            return False

    def storeFile(self, varName, fobj, n=0):
    ...
    def _iretrieve(self, varName, op, varId, valIndex, fobj=None):
        ...
        self.send('0005%020d%s%020d%s%020d%s%020d%-20.20s%020d%020d' % \
                (len(op), op, len(self.curWs), self.curWs,
                 len(varName), varName, 20, varId, 20, valIndex))
        status = int(self.recv(4)) # barely used at the moment.
        ...
        if fobj is None:
            if desc & _DirectString:
                ...
                return (status, cPickle.loads(pVal), varId, valIndex, n)
            else:
                raise StopIteration
        else:
