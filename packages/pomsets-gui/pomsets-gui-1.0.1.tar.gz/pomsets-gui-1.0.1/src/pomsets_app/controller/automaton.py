...
class Automaton(AutomatonModule.Automaton):
    ...
    def executeCommand(self, command):
        ...
        try:
            ...
            """
            # we do this by sending an event
            # notifying that the event has failed to execute
            import pomsets_app.gui.event as EventModule
            event = EventModule.CommandExecutionFailedEvent(
                command=command
            )
    ...
    def enqueueRequest(self, request, shouldWait=True):
        ...
        if hasattr(self.contextManager(), 'canvas'):
            ...
            if canvas.hasGuiObjectForDataNode(definition):
                ...
                logging.debug("canvas does not have gui object for task %s's definition %s" % (task, definition))
                pass
        else:
