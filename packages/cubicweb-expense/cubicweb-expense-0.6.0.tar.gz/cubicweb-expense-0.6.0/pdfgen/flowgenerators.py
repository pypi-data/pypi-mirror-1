...
class ExpenseFlowGenerator (AbstractFlowGenerator) :
    ...
    def get_metadata(self, entity) :
        ...
        s = u"%s%s%s" %( html_escape(_(u"Subject")),
                         html_escape(_(u": ")),
        ...
        try:
            ...
            s = html_escape(wcase.ref)
            s = u"%s%s%s" %( html_escape(_(u"Workcase")),
                             html_escape(_(u": ")),
                             html_escape(wcase.ref) )
            col_data.append( Paragraph(s, style_metadata) )

        s = u"%s%s#%s" %( html_escape(_(u"Reference")),
                          html_escape(_(u": ")),
                          entity.eid )
        col_data.append( Paragraph(s, style_metadata) )
    ...
    def get_total(self, entity) :
        ...
        # First line
        lin_data = []
        s = "<b>%s</b>" % html_escape(_(u"Total (EUR)"))
    ...
    def get_total_relative_data(self, entity) :
        ...
        for euser in refundable_tot.keys():
            s = u"%s %s %s %s" %(
                          ...
                          html_escape(_(u"EUR")),
                          html_escape(_(u"to be refunded to")),
                          html_escape(euser.dc_title()) )
    ...
    def get_expenses_table(self, entity) :
        ...
        s = u"<b>%s</b>" %html_escape(_(u"Date"))
        lin_data_1.append( Paragraph(s,style_table_header) )

        s = u"<b>%s</b>" %html_escape(_(u"Description"))
        lin_data_1.append( Paragraph(s,style_table_header) )

        s = u"<b>%s</b>" %html_escape(_(u"Expense Type"))
        lin_data_1.append( Paragraph(s,style_table_header) )

        # Second line
        lin_data_2=[]

        s = u"<b>%s</b>" %html_escape(_(u"Paid by"))
        lin_data_2.append( Paragraph(s,style_table_header) )

        s = u"<b>%s</b>" %html_escape(_(u"Paid for"))
        lin_data_2.append( Paragraph(s,style_table_header) )

        s = u"<b>%s</b>" %html_escape(_(u"Amount"))
        lin_data_2.append( Paragraph(s,style_table_header) )

        s = u"<b>%s</b>" %html_escape(_(u"Curr."))
        lin_data_2.append( Paragraph(s,style_table_header) )

        s = u"<b>%s</b>" %html_escape(_(u"Rate"))
        lin_data_2.append( Paragraph(s,style_table_header) )

        s = u"<b>%s</b>" %html_escape(_(u"Amount (EUR)"))
        lin_data_2.append( Paragraph(s,style_table_header) )

        s = u"<b>%s</b>" %html_escape(_(u"Taxes (EUR)"))
        lin_data_2.append( Paragraph(s,style_table_header) )
        ...
        for exp_line in entity.has_lines:
            ...
            s = html_escape(exp_line.printable_value('diem'))
            lin_data_1.append( Paragraph(s,style_table_data) )

            s = html_escape(exp_line.title)
            lin_data_1.append( Paragraph(s,style_table_data) )

            s = html_escape(_(exp_line.type))
            lin_data_1.append( Paragraph(s,style_table_data) )

            # Second line
            lin_data_2 = []

            s = html_escape(exp_line.paid_by[0].label)
            lin_data_2.append( Paragraph(s,style_table_data) )

            col_data = []
            for dest in exp_line.paid_for:
                s = html_escape(dest.label)
                col_data.append( Paragraph(s,style_table_data) )
            ...
            s = html_escape(exp_line.currency)
            lin_data_2.append( Paragraph(s,style_table_data) )
...
class RefundFlowGenerator (AbstractFlowGenerator) :
    ...
    def get_person_name(self, entity) :
        ...
        if euser:
            s = u"%s%s%s" %( html_escape(_(u"SS number")),
                             html_escape(_(u": ")),
                             html_escape(euser.ssnum or u"missing SS number") )
            col_data.append( Paragraph(s, style_ids) )

            col_data.append( Spacer(1,0.2*cm) )

            s = u"<b>%s</b>" % html_escape(euser.name())
            col_data.append( Paragraph(s ,style_address) )
            
            if euser.lives_at:
                addr = euser.lives_at[0]

                s = html_escape(addr.street)
                col_data.append( Paragraph(s ,style_address) )

                s = u"%s %s" %( html_escape(addr.postalcode), 
                                html_escape(addr.city)     )
                col_data.append( Paragraph(s, style_address) )
    ...
    def get_total(self, entity) :
        ...
        s = "<b>%s</b>" % html_escape(_(u"To refund (EUR)"))
        lin_data.append( Paragraph(s, style_table_data) )
    ...
    def get_total_relative_data(self, entity) :
        ...
        s = u"%s %s %s %s" %( html_escape(_(u"Refunded on")),
                              html_escape(date),
                              html_escape(_(u"by")),
                              html_escape(mode) )
        col_data.append( Paragraph(s, style_table_data) )
    ...
    def get_expenses_table(self, entity) :
        ...
        s = u"<b>%s</b>" %html_escape(_(u"Workcase"))
        lin_data.append( Paragraph(s,style_table_header) )
