# ByDate.py is part of The Ming Blog Server
...
class ByDate:
	...
	def __get_links(self, dir, titles):
		...
		for x in titles:
			line = x.split(":")
			date = self.lib.get_date(line[0])
			y = date[-4:]
			if year != y:
				if year != "":
					links.append("</ul></ul>\n")
					month = ""
				links.append("<h3>"+y+"</h3>\n<ul>\n")
				year = y
			m = (date.split())[0]
			if month != m:
				if month != "":
					links.append("</ul>\n")
				links.append("<h4>"+m+"</h4>\n<ul>\n")
				month = m
			d = (date.split())[1].replace(",","")
			links.append("<li>"+d+": <a href=\"/"+self.CGI+"/ming.cgi?blog="+dir+"&amp;article="+\
						 line[0]+"\">"+line[1]+"</a>")
		links = "\n".join(links)
